...
\end{lemma}
\begin{proof}
The DEC implies $\mathcal{S} \ge 2|q|^2_{\bg}$. We can decompose the potential as $V = \frac{1}{8}\mathcal{S} - \frac{1}{4}\Div_{\bg}(q)$. Although $\Div_{\bg}(q)$ may change sign, making the potential indefinite, the Spectral Condition $\lambda_1(L) > 0$ ensures the operator is coercive. By the Fredholm alternative and the variational characterization of the eigenvalues, the positivity of the first eigenvalue implies that the Green's function for $L$ is positive. Thus, solutions with non-negative boundary data are non-negative.

Specifically, we can construct a supersolution $\phi_+$ and a subsolution $\phi_-$ to barrier-trap the solution away from zero. The strict stability of the MOTS $\Sigma$ implies that near the boundary, the potential is dominated by the positive eigenvalue of the stability operator, providing a local barrier. In the bulk, the Spectral Condition ensures the absence of negative eigenvalues that would allow $\phi$ to cross zero.
\end{proof}
