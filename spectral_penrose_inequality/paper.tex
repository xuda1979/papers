...
\begin{itemize}
    ...
    With this weight, the operator is Fredholm. In this marginal case, the metric decay is polynomial ($O(t^{-k})$). We invoke the \textbf{Weighted Scattering Calculus} to handle the polynomially decaying perturbation $E = L - L_\infty$. The condition $k \ge 2$ (ensured for generic data) guarantees that the operator remains Fredholm and the source term $\Div(q)$ is in the range. The solution $\phi_0$ will decay to zero (polynomially, consistent with the source).
\end{itemize}
