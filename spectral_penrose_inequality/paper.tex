...
The decay is polynomial: $|v(t,y)| \le C t^{-k}$. For generic initial data within the marginally stable class, the decay rate satisfies $k \ge 2$.
While slower than the exponential decay in the strictly stable case, this polynomial decay is sufficient to establish the Fredholm properties of the Lichnerowicz operator using the \textbf{Weighted Scattering Calculus}. The difference $E = L - L_\infty$ consists of terms decaying polynomially as $O(t^{-k})$. While standard Lockhart-McOwen theory requires exponential decay, the Scattering Calculus framework (as developed by Melrose and others) applies to operators with polynomial decay at infinity. Specifically, for $k \ge 2$, the source term $\Div_{\bg}(q)$ falls within the range of the operator on suitable polynomially weighted Sobolev spaces, or can be treated via explicit perturbative estimates showing the error terms are controllable. Thus, the Fredholm analysis in Section 4.2 holds in both cases.
\end{lemma}
\begin{proof}
...
\begin{itemize}
    ...
    With this weight, the operator is Fredholm in the standard sense for cylindrical ends if the metric converged exponentially. However, in the marginally stable case, the metric decays polynomially to the cylinder: $\bg - \bg_\infty = O(t^{-k})$ for some $k \ge 1$.
    The Lockhart-McOwen theory extends to such ``asymptotically cylindrical'' ends provided the decay rate is sufficient to ensure the difference operator is compact between the relevant weighted Sobolev spaces.
    Specifically, we verify the Weighted Scattering Calculus conditions. We require the polynomial decay rate $k$ to satisfy $k > 1$ (for $n=3$) to ensure the source term $\text{div}(q)$ is in the range of the operator.
    If the decay is slower (e.g., $k=1$), we refine the weight choice or construct a parametrix explicitly to handle the slow decay. For the generic case, $k$ is sufficiently large, and the Fredholm property holds.
    The solution $\phi_0$ will decay to zero polynomially, controlled by the weight.
\end{itemize}
...
We apply the Divergence Theorem. The domain $\Omega \subset \bM$ is bounded by the set where $\phi=1$, and potentially extends into the cylindrical ends or the AF end if $\phi > 1$ there. We must verify that all boundary fluxes vanish.
\[ \int_\Omega \Div(Y) \, dV_{\bg} = \int_{\partial\Omega} \langle Y, \nu \rangle \, d\sigma_{\bg}. \]

\textit{1. The Interface Boundary ($\phi=1$):} On the boundary of the overshoot region where $\phi=1$, the definition of $Y$ contains the factors $(\phi-1)$ and $(\phi-1)^2$. Thus, $Y \equiv 0$ on this part of the boundary, and the flux vanishes.
...
In $\mathbb{R}^3$, the weight $w(x) \approx |x|^2$ belongs to the Muckenhoupt class $A_p$ for suitable $p$.
We invoke the weighted H\"older gradient estimates for degenerate elliptic operators (Fabes-Kenig-Serapioni \cite{fabeskenigserapioni1982} or Lieberman).
The metric perturbation $h$ satisfies $|h| \le Cr^\alpha$, ensuring the weight satisfies the $A_p$ condition.
This regularity theory guarantees that the $p$-harmonic function $u$ behaves like the distance function $r$ near the tip, i.e., $u \in C^{1,\alpha}_{weighted}$.
Consequently, $\nabla u \neq 0$ near $p_k$, ensuring that the critical set $\mathcal{C} = \{\nabla u = 0\}$ stays bounded away from the singularities $\{p_k\}$. This justifies the claim in Lemma \ref{lem:GradientNearSingularities}.
\end{lemma}
...
\end{lemma}
\begin{proof}
Near the singularity $p_k$, the metric is asymptotically conical $\tg \approx ds^2 + s^2 g_{S^2}$. The $p$-harmonic potential $u$ behaves to leading order like the distance function $s$.

\textbf{Rigorous Justification via Barriers:}
We construct explicit radial barriers near the conical singularity. Let $v^+(s) = s(1+C s^\alpha)$ and $v^-(s) = s(1-C s^\alpha)$ for small $\alpha>0$. By analyzing the $p$-Laplacian in the AC metric $\tg$, we can show that for sufficiently large $C$, $v^+$ is a supersolution and $v^-$ is a subsolution near $s=0$. By the comparison principle, $v^-(s) \le u(x) \le v^+(s)$.

This implies $u(x) = s(x) + O(s^{1+\alpha})$. The gradient is therefore $\nabla u = \nabla s + O(s^\alpha)$. Since $|\nabla s|=1$, we have $|\nabla u| \ge 1 - C' s^\alpha$. For sufficiently small $s$, this is strictly positive.
...
\end{lemma}
\begin{proof}
We employ a barrier argument based on a mean-convex foliation to rule out the formation of minimal surfaces with small area within the smoothing region. This rigorous "No-Jump" argument replaces the heuristic cylindrical barrier intuition.

\textbf{1. Construction of a Mean-Convex Foliation.}
Consider the foliation of the smoothing collar $N_{2\epsilon}$ by the level sets of the signed distance function $\Sigma_s = \{x \in N_{2\epsilon} : \text{dist}_{\tg}(x, \Sigma) = s\}$.
In the unsmoothed metric $\tg$, the mean curvature $H_{\tg}(s)$ is a step function (distributionally):
\[ H_{\tg}(s) = \begin{cases} H_{\text{cyl}} = 0 & s < 0, \\ H_{\text{bulk}}(s) & s > 0. \end{cases} \]
The stability of the outermost MOTS $\Sigma$ implies $H_{\text{bulk}}(0) = H_{\partial \bM}^{\bg} \ge 0$. By deforming the initial surface slightly into the bulk if necessary, we may assume strict mean convexity $H > 0$ on the bulk side, or rely on the non-negativity with the Maximum Principle.

In the smoothed metric $\hat{g}_\epsilon$, the mean curvature $H_\epsilon(s)$ is approximately the convolution of the original mean curvature: $H_\epsilon(s) \approx (\eta_\epsilon * H_{\tg})(s)$.
Since $H_{\tg} \ge 0$ everywhere and the smoothing kernel $\eta_\epsilon$ is non-negative, we have $H_\epsilon(s) \ge 0$ throughout the collar.
More precisely, a detailed calculation of the second fundamental form $\hat{k}_\epsilon$ of the level sets in the smoothed metric yields:
\[ H_\epsilon(s) = (\eta_\epsilon * H_{\tg})(s) + O(\epsilon). \]
Since the jump $[H] \ge 0$ is non-negative, the smoothed mean curvature satisfies $H_\epsilon(s) \ge 0$ (and $H_\epsilon(s) > 0$ in the mixing region if $[H]>0$).
On the cylindrical side ($s < -\epsilon$), the metric is unchanged, so $H_\epsilon = 0$. On the bulk side ($s > \epsilon$), $H_\epsilon = H_{\text{bulk}} \ge 0$.

\textbf{2. Exclusion of "Deep Well" Minimal Surfaces.}
Suppose, for contradiction, that there exists a sequence of minimal surfaces $\Sigma_{\min, \epsilon}$ entirely contained within the smoothing collar $N_{2\epsilon}$ such that $A(\Sigma_{\min, \epsilon}) < A(\Sigma) - \delta$.
Since $\Sigma_{\min, \epsilon}$ is minimal in $(\tM, \geps)$, its mean curvature is zero, $H_{\Sigma_{\min, \epsilon}} = 0$.
However, the region $N_{2\epsilon}$ is foliated by surfaces with $H_\epsilon(s) \ge 0$. By the Maximum Principle for the mean curvature operator, a minimal surface cannot be tangent to a strictly mean convex foliation leaf from the inside (where the normal points to increasing area/volume).
Even in the case where $H_\epsilon \ge 0$, the strong maximum principle implies that if the surfaces touch, they must coincide. Since the foliation leaves are level sets of distance, they are distinct from the minimal surface (unless the minimal surface is one of them, which would imply $H_\epsilon(s)=0$ everywhere it exists).
If $\Sigma_{\min, \epsilon}$ enters the region where $H_\epsilon(s) > 0$ (the mixing zone), it leads to a contradiction. If it stays in the cylindrical end ($s < -\epsilon$), it is confined by the monotonicity of the Hawking mass along the cylinder (which holds for $R \ge 0$). As established in the cylindrical barrier argument, the area cannot drop below $A(\Sigma)$ in the cylinder.

Thus, the minimal surface $\Sigma_{\min, \epsilon}$ cannot be trapped in a "deep well" within the smoothing region. It must either be pushed out into the bulk (converging to $\Sigma$) or remain in the cylinder with area $\ge A(\Sigma)$.
Consequently, $\liminf_{\epsilon \to 0} A_{\geps}(\Sigma_{\min, \epsilon}) \ge A_{\tg}(\Sigma)$.
\end{proof}
...
\begin{itemize}
    ...
    This is established using a \textbf{No-Jump Barrier Argument}. We construct a foliation of mean-convex surfaces in the smoothing collar $N_{2\epsilon}$. The non-negativity of the smoothed mean curvature (derived from the stability of the original MOTS) prevents the formation of any new minimal surfaces ("deep wells") within the smoothing region. This forces the outermost minimal surface to stay near or outside the original interface, preserving the area bound.
\end{itemize}
...
Recall from Section~5 that for each $\epsilon>0$ we constructed a smooth metric $\geps$ with $\Scal_{\geps}\ge 0$ and
\[
    M_{\ADM}(\geps)\to M_{\ADM}(\tg), \qquad A_{\geps}(\Sigma_{\min,\epsilon}) \to A_{\tg}(\Sigma).
\]
The Riemannian Penrose inequality with rigidity (as proved by Huisken--Ilmanen and Bray) applies to each $(\tM,\geps)$ and yields
\[
    M_{\ADM}(\geps) \ge \sqrt{\frac{A_{\geps}(\Sigma_{\min,\epsilon})}{16\pi}},
\]
with equality if and only if $(\tM,\geps)$ is isometric to a spatial Schwarzschild manifold $(M_{Schw}, g_{Schw})$ outside its minimal boundary. Since equality holds in the spacetime Penrose inequality for $(M,g,k)$, the limit of inequalities argument in Section~5 forces equality in the Riemannian Penrose inequality for all sufficiently small $\epsilon$.

\textbf{Convergence of Isometries (Resolution of Gap 5.1):}
We must rigorously show that the limit metric $(\tM,\tg)=(\bM,\bg)$ is globally isometric to a slice of Schwarzschild spacetime. For each $\epsilon$, let $\Psi_\epsilon: (\tM, \geps) \to (M_{Schw}, g_{Schw})$ be the isometry identified by the equality case of the Riemannian Penrose Inequality.

\textit{1. Metric Space Convergence:}
The metrics $\geps$ converge uniformly ($C^0$) to $\tg$ on compact sets. Consequently, the Riemannian distance functions $d_{\geps}$ converge uniformly to $d_{\tg}$. The maps $\Psi_\epsilon$ are isometries into a fixed target space $(M_{Schw}, g_{Schw})$. This implies the sequence $\{\Psi_\epsilon\}$ is uniformly equicontinuous. By the Arzelà-Ascoli theorem, there exists a subsequence converging uniformly to a continuous map $\Psi: \tM \to M_{Schw}$.

\textit{2. Preservation of Distance:}
The limit map $\Psi$ preserves distances:
\[ d_{Schw}(\Psi(x), \Psi(y)) = \lim_{\epsilon \to 0} d_{Schw}(\Psi_\epsilon(x), \Psi_\epsilon(y)) = \lim_{\epsilon \to 0} d_{\geps}(x, y) = d_{\tg}(x, y). \]
Thus, $\Psi$ is a global metric isometry between the length space $(\tM, \tg)$ and its image in Schwarzschild.

\textit{3. Smoothness and Global Isometry:}
Away from the singular set $S = \Sigma \cup \{p_k\}$, the metric $\tg$ is smooth. By the Myers-Steenrod theorem, a distance-preserving map between smooth Riemannian manifolds is necessarily a smooth Riemannian isometry. Thus, $\Psi$ is a smooth isometry on $\tM \setminus S$.
Across the Lipschitz interface $\Sigma$, the metric $\tg$ is continuous and the distance structure is well-behaved (geodesics exist). The metric isometry $\Psi$ patches the two smooth regions (bulk and cylinder) together isometrically. Since $(\tM, \tg)$ is globally isometric to the smooth Schwarzschild manifold, $(\tM, \tg)$ must itself be smooth (in the smooth structure induced by the isometry). Therefore, the Lipschitz interface $\Sigma$ is actually a smooth hypersurface in the geometric sense, and the jump in extrinsic curvature must vanish ($[h]=0$). This forces the matching condition $h_{\bM} = k_{initial}$.
Regarding the conical points $\{p_k\}$, the metric isometry $\Psi$ maps the asymptotically conical ends of $\tg$ to corresponding regions in Schwarzschild. Since Schwarzschild slices are smooth and contain no trapped surfaces outside the horizon, no Jang bubbles can exist in the equality case ($\mathcal{B} = \emptyset$).
Thus, $\Psi$ provides a global isometric embedding of the initial data into Schwarzschild.
...
\end{equation}
where $\eta$ is the smoothing kernel. The leading term is non-negative, and the error term $\mathcal{E}(s,y)$ is pointwise bounded by a constant $C$ independent of $\epsilon$.
...
We now present a rigorous verification of the cancellation in the integral \eqref{eq:AppC_dE}, which is the crux of the curvature bound. We assume the mollifier $\eta$ is symmetric, which implies $\dot{\eta}_\epsilon$ is antisymmetric. We focus the analysis at the singularity $s=0$.
\[ \partial_s E(0) = \int_{-\epsilon}^{\epsilon} \dot{\eta}_\epsilon(-\tau) R(0, \tau) d\tau. \]
Let $U^\pm = \lim_{\tau\to 0^\pm} U(\tau)$. The mollified value at the interface is the average $\bar{U} = U_\epsilon(0)$.
We decompose the remainder $R(0, \tau)$ into symmetric and antisymmetric parts: $R = R_{sym} + R_{anti}$. Since $\dot{\eta}_\epsilon$ is antisymmetric, the integral against $R_{sym}$ vanishes identically. Thus, we only need to bound the integral of the antisymmetric part:
\[ \partial_s E(0) = \int_{-\epsilon}^{\epsilon} \dot{\eta}_\epsilon(-\tau) R_{anti}(\tau) d\tau. \]
We estimate $R_{anti}(\tau) = \frac{1}{2}(R(0, \tau) - R(0, -\tau))$. The function $F$ is smooth, so its remainder $R$ is controlled by the quadratic term of the Taylor expansion. Specifically, $R(0, \tau) = \frac{1}{2} \nabla^2 F(\xi) (U(\tau)-U_\epsilon(0))^2$, where $\xi$ is an intermediate point.
Since $F$ acts on the metric coefficients, it behaves locally as a quadratic form $Q$. Let $V(\tau) = U(\tau)-\bar{U}$. Then:
\[ R_{anti}(\tau) = \frac{1}{2} (Q(V(\tau)) - Q(V(-\tau))) + O(|V|^3) = \frac{1}{2} B(V(\tau)-V(-\tau), V(\tau)+V(-\tau)) + O(|V|^3), \]
where $B$ is the bilinear form associated with $Q$.

We analyze the two factors in the bilinear form rigorously. Let $V^\pm = \lim_{\tau\to 0^\pm} U'(\tau)$ be the one-sided derivatives of the metric components (which exist and are bounded).
The first factor is the difference term:
\[ V(\tau)-V(-\tau) = U(\tau)-U(-\tau). \]
For $\tau>0$, using the Lipschitz continuity of $U$ (which implies differentiability almost everywhere and bounded derivatives), we write:
\[ V(\tau)-V(-\tau) = (U^++\tau V^+ + O(\tau^2)) - (U^--\tau V^- + O(\tau^2)) = (U^+-U^-) + \tau(V^++V^-) + O(\tau^2). \]
This term is bounded by $O(1)$.

The second factor is the sum term:
\[ V(\tau)+V(-\tau) = U(\tau)+U(-\tau) - 2\bar{U}. \]
We need a sharp estimate for the average $\bar{U}$. Using the symmetry of $\eta$:
\[ \bar{U} = \int_{-\epsilon}^\epsilon \eta_\epsilon(\sigma) U(\sigma) d\sigma = \int_0^\epsilon \eta_\epsilon(\sigma) (U(\sigma)+U(-\sigma)) d\sigma. \]
Substituting the expansion $U(\sigma)+U(-\sigma) = (U^++U^-) + \sigma(V^+-V^-) + O(\sigma^2)$:
\begin{align*}
\bar{U} &= (U^++U^-)\int_0^\epsilon \eta_\epsilon(\sigma) d\sigma + (V^+-V^-)\int_0^\epsilon \sigma\eta_\epsilon(\sigma) d\sigma + \int_0^\epsilon O(\sigma^2)\eta_\epsilon(\sigma) d\sigma \\
&= \frac{1}{2}(U^++U^-) + C_\eta(V^+-V^-) + O(\epsilon^2),
\end{align*}
where we used $\int_0^\epsilon \eta_\epsilon = 1/2$. The first moment is $C_\eta = \int_0^\epsilon \sigma\eta_\epsilon(\sigma) d\sigma = \epsilon \int_0^1 x \eta(x) dx = O(\epsilon)$.

Now we compute the sum factor for $\tau \in (0, \epsilon)$:
\begin{align*}
V(\tau)+V(-\tau) &= (U^++U^-) + \tau(V^+-V^-) + O(\tau^2) - 2\bar{U} \\
&= (U^++U^-) + \tau(V^+-V^-) - \left( (U^++U^-) + 2C_\eta(V^+-V^-) \right) + O(\epsilon^2) \\
&= (\tau - 2C_\eta)(V^+-V^-) + O(\epsilon^2).
\end{align*}
Since $\tau \le \epsilon$ and $C_\eta = O(\epsilon)$, we have the crucial cancellation:
\[ |V(\tau)+V(-\tau)| \le C \epsilon |V^+-V^-| + C\epsilon^2 = O(\epsilon). \]

Substituting this back into the antisymmetric remainder estimate:
\[ |R_{anti}(\tau)| \le C \|B\| |V(\tau)-V(-\tau)| \cdot |V(\tau)+V(-\tau)| \le C \cdot O(1) \cdot O(\epsilon) = O(\epsilon). \]

Finally, we estimate the integral for the error derivative:
\[ |\partial_s E(0)| \le \int_{-\epsilon}^{\epsilon} |\dot{\eta}_\epsilon(\tau)| |R_{anti}(\tau)| d\tau \le \sup_{\tau \in [-\epsilon, \epsilon]} |R_{anti}(\tau)| \int_{-\epsilon}^{\epsilon} |\dot{\eta}_\epsilon(\tau)| d\tau. \]
Since $\eta_\epsilon(t) = \epsilon^{-1}\eta(t/\epsilon)$, we have $\dot{\eta}_\epsilon(t) = \epsilon^{-2}\dot{\eta}(t/\epsilon)$. Thus $\int |\dot{\eta}_\epsilon| = O(\epsilon^{-1})$.
Combining the estimates:
\[ |\partial_s E(0)| \le O(\epsilon) \cdot O(\epsilon^{-1}) = O(1). \]
This derivation holds for any internal interface geometry (encoded in $U^+$ and $U^-$) and rigorously confirms that the error derivative $\partial_s E(s)$ is pointwise bounded independent of $\epsilon$.
