...
\begin{lemma}[Lower Semicontinuity of Minimal Surface Area]\label{lem:MinimalSurfaceArea}
Let $\Sigma_{\min, \epsilon}$ denote the outermost minimal surface in the smoothed manifold $(\tM, \geps)$. As $\epsilon \to 0$, the area of these surfaces satisfies:
\begin{equation}
    \liminf_{\epsilon \to 0} A_{\geps}(\Sigma_{\min, \epsilon}) \ge A_{\tg}(\Sigma).
\end{equation}
\end{lemma}
\begin{proof}
The metric $\geps$ converges to $\tg$ in the $C^0$ topology on compact sets. The area functional for varifolds is lower semicontinuous with respect to $C^0$ metric convergence. Since the family of minimal surfaces $\Sigma_{\min, \epsilon}$ is homologous to the horizon $\Sigma$, and the horizon $\Sigma$ in the limit metric $\tg$ acts as a barrier with non-negative mean curvature (from the MOTS stability), the minimal surfaces cannot degenerate to zero area. Any limit varifold $V$ of the sequence $\Sigma_{\min, \epsilon}$ must be stationary in $(\tM, \tg)$. Due to the structure of the Jang metric (cylindrical ends), the only such stationary varifolds in the homology class have area at least $A_{\tg}(\Sigma)$. Thus, the limit of the areas is bounded below by the area of the horizon interface.
\end{proof}
...
\begin{itemize}
    ...
    \item \textbf{Area Lower Semicontinuity:} By Lemma \ref{lem:MinimalSurfaceArea}, the area of the minimal surfaces behaves semicontinuously:
    \[ \liminf_{\epsilon \to 0} A_{\geps}(\Sigma_{\min, \epsilon}) \ge A_{\tg}(\Sigma). \]
\end{itemize}
...
\begin{align*}
    ...
    &\ge \liminf_{\epsilon \to 0} \sqrt{\frac{A_{\geps}(\Sigma_{\min, \epsilon})}{16\pi}} \\
