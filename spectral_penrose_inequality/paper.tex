...
\begin{proof}
The proof relies on the stability of the Fredholm index under compact perturbations. The operator $L$ can be written as $L = L_\infty + E$, where $L_\infty$ is the translation-invariant operator on the exact cylinder, and $E$ is a perturbation term with decaying coefficients (due to the exponential convergence of the Jang metric to the cylinder). By the Lockhart-McOwen theory, the indicial roots of $L$ are exactly those of $L_\infty$, provided the perturbation decays sufficiently fast (which holds here).
...
While $\Div(q)$ is indefinite, the stability of the outermost MOTS provides a favorable sign at the boundary. We assume the \textbf{Spectral Condition}: the operator $L$ has no negative eigenvalues (bound states) on the Jang manifold. This is physically motivated by the requirement that the Jang surface does not penetrate the black hole interior.
Under the DEC and this spectral assumption, the operator $L$ satisfies the Maximum Principle.
Assume, for the sake of contradiction, that $\phi$ is not strictly positive. Since the boundary condition is $\phi \to 1$ on all non-compact ends of $\bM$ (at infinity and over the horizon), if $\phi$ takes non-positive values, its infimum must be attained at an interior point $x_0 \in \bM$. Let $\phi(x_0) = \inf_{\bM} \phi \le 0$.
...
From the DEC, we know that $\mathcal{S} \ge 0$.
...
To complete the argument, we must ensure that the area of the minimal surface in the smoothed manifold does not degenerate. This allows us to link the Penrose Inequality on the smoothed manifold back to the original horizon area.
...
\end{align*}
Combining this with the mass reduction property of the Jang map ($M_{\ADM}(g) \ge M_{\ADM}(\tg)$) and the area preservation, we obtain:
\[ M_{\ADM}(g) \ge \sqrt{\frac{A(\Sigma)}{16\pi}}. \]
