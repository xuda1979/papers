...
\textit{3. Regularity of the Limit Isometry via Reshetnyak's Theorem:}
The limit map $\Psi: (\tM, \tg) \to (M_{Schw}, g_{Schw})$ is a global metric isometry. The domain $(\tM, \tg)$ is a priori only Lipschitz regular (across $\Sigma$) and contains conical points. The classical Myers-Steenrod theorem requires smooth manifolds. To bridge this gap, we invoke the regularity theory for isometries of manifolds with lower curvature bounds, specifically the work of Reshetnyak.
Since the target manifold $(M_{Schw}, g_{Schw})$ is smooth, and $\Psi$ is a distance-preserving map, $\Psi$ satisfies the harmonic map equation in the distributional sense. Regularity results for isometries (e.g., Reshetnyak's theorem) imply that a metric isometry from a space with a lower curvature bound into a smooth Riemannian manifold is necessarily a smooth map (a diffeomorphism onto its image).
Because $\Psi$ is a smooth diffeomorphism and is an isometry ($\Psi^* g_{Schw} = \tg$), the regularity of the target metric $g_{Schw}$ forces the domain metric $\tg$ to be smooth.
This smoothness has two critical geometric consequences:
\begin{itemize}
    \item The metric $\tg$ cannot have true conical singularities at $\{p_k\}$; the points must be regular, implying the absence of bubbles ($\mathcal{B} = \emptyset$).
    \item The metric must be smooth across the interface $\Sigma$. The Lipschitz continuity is upgraded to $C^\infty$. This forces the mean curvature jump (and indeed the jump in the full second fundamental form) to vanish: $[h] = 0$.
\end{itemize}
This implies $h_{\bM} = k_{initial}$ at the interface, completing the rigidity argument.
