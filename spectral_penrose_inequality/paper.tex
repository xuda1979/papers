...
\end{theorem}
\begin{proof}
Let $p_k$ be one of the singular points. By construction (Theorem \ref{thm:Deformation}), the metric $\tg$ near $p_k$ is asymptotically conical. Let $(r, \theta)$ be local coordinates centered at $p_k$, where $r$ is the geodesic distance from $p_k$ and $\theta$ are coordinates on the unit sphere. In these coordinates, the metric takes the form $\tg \approx dr^2 + r^2 g_{S^2}$, and the volume element is $d\text{Vol}_{\tg} \approx r^2 \sin\theta \,dr\,d\theta\,d\phi$.

The $p$-capacity of the set $\{p_k\}$ is defined as:
\[ \Cap_p(\{p_k\}) = \inf \left\{ \int_{\tM} |\nabla \psi|_{\tg}^p \, \dVol_{\tg} \right\}, \]
where the infimum is taken over all compactly supported smooth functions $\psi \in C^\infty_c(\tM)$ such that $\psi \ge 1$ in a neighborhood of $p_k$.

To prove the capacity is zero, we construct a sequence of test functions $\psi_\epsilon$ and show that their $p$-energy tends to zero as $\epsilon \to 0$. For $\epsilon > 0$ small enough that the $2\epsilon$-ball $B_{2\epsilon}(p_k)$ lies within the conical region, define the Lipschitz cutoff function $\psi_\epsilon: \tM \to \R$ as:
...
\end{cases} \]
This function is equal to 1 in the $\epsilon$-ball around $p_k$ and is supported in the $2\epsilon$-ball. It can be approximated by smooth functions in $W^{1,p}$ without changing the energy limit.

The gradient of $\psi_\epsilon$ is non-zero only on the annulus $A_\epsilon = \{x \in \tM : \epsilon < r(x) < 2\epsilon\}$. In this region, the gradient is radial, and its magnitude is given by:
\[ |\nabla \psi_\epsilon|_{\tg} = \left| \frac{d}{dr} \left( \frac{2\epsilon - r}{\epsilon} \right) \right| = \frac{1}{\epsilon}. \]
Now, we compute the $p$-energy integral:
\begin{align*}
...
\end{align*}
The volume of the annulus $A_\epsilon$ is:
\begin{align*}
    ...
    &= 4\pi \left( \frac{(2\epsilon)^3}{3} - \frac{\epsilon^3}{3} \right) = 4\pi \frac{7\epsilon^3}{3}.
\end{align*}
...
By definition, the capacity must be less than or equal to this value for any $\epsilon > 0$:
\[ 0 \le \Cap_p(\{p_k\}) \le \frac{28\pi}{3} \epsilon^{3-p}. \]
Since we are given that $1 < p < 3$, the exponent $3-p$ is strictly positive. Therefore, taking the limit as $\epsilon \to 0$:
\[ \lim_{\epsilon \to 0} \frac{28\pi}{3} \epsilon^{3-p} = 0. \]
This forces $\Cap_p(\{p_k\}) = 0$. Since this holds for each singular point, the capacity of the finite set $\{p_k\}$ is also zero. This result is fundamental for the well-posedness of the weak formulation of the $p$-Laplace equation, as sets of zero $p$-capacity are removable for functions in the Sobolev space $W^{1,p}$.
\end{proof}
...
To verify that the area does not shrink in the limit, we analyze the behavior of $u_\epsilon$. The Green's function representation for $v_\epsilon = u_\epsilon - 1$ is given by
\[ v_\epsilon(x) = -\frac{1}{8} \int_{\tM} G(x, y) R^-_\epsilon(y) u_\epsilon(y) dV_y. \]
Since the Green's function $G(x,y)$ is positive, $R^-_\epsilon \le 0$, and $u_\epsilon > 0$, the integrand is non-positive. The minus sign in front ensures $v_\epsilon \ge 0$, which confirms $u_\epsilon \ge 1$.
The uniform convergence to 1 is made precise by Lemma \ref{lem:GreenEstimate}, which shows $\|u_\epsilon - 1\|_{L^\infty} \to 0$ as $\epsilon \to 0$.

This uniform convergence is key for the area semicontinuity. For the horizon area:
