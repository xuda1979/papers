...
\end{equation}
where $X$ is the auxiliary vector field introduced by Han and Khuri \cite{hankhuri2013} to absorb trace terms of $k$ along the Jang graph.
Under the Dominant Energy Condition, the non-divergence terms are non-negative, while the divergence term
\begin{equation}
    -2 \int_{\overline{M}} \text{div}_{\overline{g}}(X) \, dV_{\overline{g}} = -2 \int_{\partial \overline{M}} \langle X, \nu \rangle \, d\sigma_{\overline{g}}
\end{equation}
reduces to a boundary flux. The blow-up behavior of the Jang solution enforces $H - \text{tr}_{\Sigma} k \to 0$ along the cylindrical end, and $X$ is chosen so that $\langle X, \nu \rangle$ matches the limiting trace term of $k$. This cancels the extrinsic curvature contribution in the Hawking mass when applying Stokes' theorem, yielding an effective scalar curvature that is non-negative in the weak sense required for the AMO monotonicity formula.

\paragraph{Step 2: Conformal Deformation to Zero Scalar Curvature.}
To ensure pointwise non-negativity of scalar curvature required by the monotonicity formula, we perform a conformal deformation $\tilde{g} = \phi^4 \overline{g}$.
We solve the conformally invariant Laplace equation:
\begin{equation}
    L_{\overline{g}}\phi := \left( \Delta_{\overline{g}} - \frac{1}{8} R_{\overline{g}} \right) \phi = 0 \quad \text{on } \overline{M},
\end{equation}
subject to $\phi \to 1$ at infinity and $\phi = 1$ on the horizon cross-section $\Sigma$ (equivalently, homogeneous Neumann data on the cylindrical end).
The operator $L_{\overline{g}}$ is coercive due to the positivity of the principal eigenvalue on asymptotically flat manifolds with non-negative scalar curvature (modulo divergence terms), guaranteeing a unique positive solution $\phi$ with these boundary values.
This deformation yields $R_{\tilde{g}} \equiv 0$, preserves the horizon area $A(\tilde{\Sigma}) = A(\Sigma)$ under the normalization $\phi|_{\Sigma} = 1$, and satisfies $M_{ADM}(\tilde{g}) \le M_{ADM}(\overline{g})$ by the positive mass theorem argument for the conformal Laplacian.
...
We integrate the differential inequality derived from the AMO monotonicity.
The convergence of the mass functional is established via the $\Gamma$-convergence of the $p$-capacity to the $1$-capacity (Inverse Mean Curvature functional).
