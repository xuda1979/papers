...
\paragraph{Case 2: Marginal Stability ($[H] = 0$).}
    ...
    This monotonicity argument rigorously excludes the existence of a "deep well" minimal surface $\Sigma'$ with $A(\Sigma') < A(\Sigma)$ that encloses $\Sigma$.
    If $\Sigma'$ is homologous to $\Sigma$ but does not strictly enclose it (e.g., it intertwines), we appeal to the definition of the Weak IMCF as a level set flow of a function $u$. The level sets $\Sigma_t = \partial \{u < t\}$ are minimizers of the functional $J_t(E) = \text{Per}(E) - \int_E H$.
    Specifically, Huisken and Ilmanen proved that for any $t > 0$, the area of the level set satisfies $A(\Sigma_t) \ge A(\Sigma_0) e^t$.
    If there existed a stable minimal surface $\Sigma'$ in the collar with area strictly less than $A(\Sigma)$, it would act as an obstacle to the flow. The flow starting from $\Sigma$ would have to "jump" to $\Sigma'$ (or a surface enclosing it) to minimize energy. However, since $A(\Sigma') < A(\Sigma)$, this would violate the exponential area growth property required by the weak flow existence theory in non-negative scalar curvature.
    Thus, the only minimal surfaces compatible with the ambient non-negative scalar curvature and the outermost condition of $\Sigma$ must have area $A(\Sigma') \ge A(\Sigma)$.
...
A priori, the metric $\bg$ is only Lipschitz continuous across the interface $\Sigma$. However, the mass equality implies that $q=0$ and $\Rg=0$.
The condition $q=0$ implies that the vector field $X = \frac{\nabla f}{\sqrt{1+|\nabla f|^2}}$ is divergence-free in the metric $\bg$.
We construct a spacetime metric $g_{static} = -V^2 dt^2 + \bg$ on $\mathbb{R} \times \bM$, where the lapse function $V$ is identified with the inverse of the area radius (or explicitly constructed from the level set flow in the limit).
...
Specifically, the system \eqref{eq:StaticVacuum} is an elliptic system for the pair $(\bg, V)$. Since the metric is Lipschitz and the curvature is bounded (it is zero), we can apply elliptic bootstrapping.
Since the interface $\Sigma$ is a minimal surface in a static vacuum spacetime, it corresponds to the bifurcation surface of the Killing horizon.
The analyticity of the solution implies that the metric $\bg$ extends analytically across the interface $\Sigma$. The "corner" is therefore a coordinate singularity, not a geometric one. In analytic coordinates, the metric components are analytic functions.
Consequently:
\begin{itemize}
...
\section{Appendix D: Derivation of the Bray-Khuri Divergence Identity}
...
\section{Appendix E: Solvability and Flux Cancellation in the Marginally Trapped Limit}
\label{app:Flux}
