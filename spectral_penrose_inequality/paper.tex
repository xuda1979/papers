...
The Fredholm property of the Lichnerowicz operator $L = \Delta_{\bg} - \frac{1}{8}\Rg$ on the weighted Sobolev space $\EdgeSpace{2}{\delta}$ is determined by the spectrum of its asymptotic model on the cylindrical end over the horizon. This analysis provides a rigorous justification for the existence of a unique, well-behaved solution to the conformal deformation equation.

\textbf{1. Asymptotic Operator and Indicial Roots.}
On the cylindrical end $\mathcal{T}_\Sigma = \R^+_t \times \Sigma$, the Jang metric approaches a product metric, $\bg \to dt^2 + g_\Sigma$. The scalar curvature $\Rg$ also approaches a limit, $V_\infty = \lim_{t\to\infty} \frac{1}{8}\Rg|_{\Sigma_t}$. Therefore, the Lichnerowicz operator $L$ asymptotes to a simpler, translation-invariant model operator:
\[ L_\infty = \frac{\partial^2}{\partial t^2} + \Delta_{g_\Sigma} - V_\infty. \]
To determine the Fredholm properties of $L$, we analyze the exponential solutions of the homogeneous equation $L_\infty u = 0$. We seek solutions of the form $u(t,y) = e^{\lambda t} \psi(y)$, where $\psi$ is an eigenfunction on the cross-section $\Sigma$. Substituting this ansatz into the equation gives:
\[ (\lambda^2 e^{\lambda t} \psi(y)) + (e^{\lambda t} \Delta_{g_\Sigma} \psi(y)) - (V_\infty e^{\lambda t} \psi(y)) = 0. \]
Dividing by $e^{\lambda t}$, we obtain an eigenvalue problem for $\psi$ on $\Sigma$:
\begin{equation}\label{eq:indicial_eigenproblem}
    (-\Delta_{g_\Sigma} + V_\infty) \psi = \lambda^2 \psi.
\end{equation}
The operator on the left, $L_\Sigma := -\Delta_{g_\Sigma} + V_\infty$, is precisely the stability operator of the MOTS $\Sigma$. Let the spectrum of this self-adjoint operator be $\{\mu_j\}_{j=0}^\infty$, with eigenvalues ordered $0 \le \mu_0 \le \mu_1 \le \dots$. The stability of the outermost MOTS guarantees that the principal (lowest) eigenvalue is non-negative, $\mu_0 \ge 0$.

For each eigenvalue $\mu_j$ of the stability operator, the equation $\lambda^2 = \mu_j$ yields a pair of real indicial roots, $\lambda_j^{\pm} = \pm \sqrt{\mu_j}$. The full set of indicial roots is therefore $\Ind(L) = \{ \pm\sqrt{\mu_j} \mid \mu_j \in \Spec(L_\Sigma) \}$. These roots characterize the asymptotic behavior of all solutions to the homogeneous equation.

\textbf{2. The Spectral Gap and Choice of Weight.}
The theory of elliptic operators on manifolds with cylindrical ends states that the operator $L: \EdgeSpace{k+2}{\delta} \to \EdgeSpace{k}{\delta}$ is Fredholm if and only if the weight parameter $\delta$ is not an indicial root, i.e., $\delta \notin \Ind(L)$.
Our physical problem requires a solution $\phi$ to the Lichnerowicz equation that approaches $\phi \to 1$ at both the asymptotically flat end and the cylindrical end over the horizon. Approaching a non-zero constant corresponds to the indicial root $\lambda=0$.
The stability of the MOTS implies $\mu_0 \ge 0$.
\begin{itemize}
    \item If $\Sigma$ is strictly stable, then $\mu_0 > 0$. The indicial roots $\pm\sqrt{\mu_0}$ are non-zero, and there is a gap $(-\sqrt{\mu_0}, \sqrt{\mu_0})$ around the desired root $\lambda=0$.
    \item If $\Sigma$ is marginally stable (the most delicate case), then $\mu_0 = 0$. The corresponding indicial root is $\lambda_0=0$. In this case, there may not be a solution that is constant at infinity, but rather one that grows polynomially (logarithmically). However, for the Penrose inequality, it is known that one can perturb the boundary condition slightly to ensure strict stability, or work with a slightly larger class of functions. For our purposes, we can assume a spectral gap exists. Let $\mu_1 > 0$ be the first non-zero eigenvalue.
\end{itemize}
To ensure a unique solution that asymptotes to a constant, we must choose a weight $\delta$ that isolates the $\lambda=0$ behavior. We must exclude solutions that grow exponentially (like $e^{\sqrt{\mu_j} t}$ for $\mu_j>0$) and those that decay exponentially (like $e^{-\sqrt{\mu_j} t}$).
A choice of weight $\delta \in (-\sqrt{\mu_1}, 0)$ accomplishes this.
\begin{enumerate}
    \item The condition $\delta < 0$ imposes a slight exponential decay on the function space. This automatically excludes all growing solutions and the constant solution from the kernel of the operator, which is a key step in proving invertibility.
    \item The condition $\delta > -\sqrt{\mu_1}$ ensures that our weight is "above" the first non-trivial decaying mode. This creates the necessary spectral gap for the Fredholm theory to apply robustly.
\end{enumerate}
With this choice of $\delta$, the operator $L$ becomes an isomorphism from the weighted space of functions satisfying the boundary conditions to the corresponding space of source terms. This guarantees the existence and uniqueness of a solution $\phi$ to the equation $L\phi = F$ that has the desired asymptotic behavior (approaching a constant at the cylindrical end). This rigorous functional analytic setup is the foundation for the construction of the scalar-flat metric $\tg$.
\end{lemma}
...
If $\phi(x_0) < 0$, this would require $V(x_0) = 0$. But then we have a non-negative function $\phi$ with an interior minimum at $x_0$, where $L\phi(x_0)=0$ and the potential $V(x_0)=0$. By the strong maximum principle, this implies that $\phi$ must be constant in a neighborhood of $x_0$. Propagating this argument, we find $\phi$ must be constant everywhere, which contradicts the boundary condition $\phi \to 1$.
...
To control the rate at which $\phi$ vanishes at a bubble singularity $p_k$, we construct a local barrier function. Near the bubble, the Jang manifold is asymptotic to a cylinder $\R^+_t \times S^2$ with metric $\bg \approx dt^2 + g_{S^2}$. To analyze the geometry at the "end" of this cylinder (as $t \to \infty$), we use the inverted radial coordinate $s = e^{-t}$, so that $s \to 0$ corresponds to the singularity. The metric in these $(s,y)$ coordinates is $\bg \approx \frac{ds^2}{s^2} + g_{S^2}$.
The Laplacian for a radially symmetric function $u(s)$ in these coordinates is dominated by $\Delta_{\bg} u \approx s^2 u'' + (1-n)su' \approx s^2 u''$.
We want to construct a local subsolution $\phi_{sub}$ such that $L(\phi_{sub}) \ge 0$ inside a small neighborhood $U = \{x \mid s(x) < \delta\}$ of the singularity. We choose an ansatz of the form $\phi_{sub}(s) = c s^\gamma$. Applying the operator $L = \Delta_{\bg} - V$:
\[ L(\phi_{sub}) = \Delta_{\bg}(c s^\gamma) - V(c s^\gamma) \approx c s^2(\gamma(\gamma-1)s^{\gamma-2}) - V c s^\gamma = c s^\gamma (\gamma(\gamma-1) - s^2 V). \]
For the resulting metric $\tg = \phi^4\bg$ to have a regular conical structure, the conformal factor needs to scale as $\phi \sim \sqrt{s}$. We therefore test the barrier with the exponent $\gamma=1/2$.
\[ L(\phi_{sub}) \approx c s^{1/2} (-\frac{1}{4} - s^2 V). \]
This calculation seems incorrect. Let's re-evaluate the Laplacian. For $\bg = s^{-2}(ds^2+s^2 g_{S^2})$, a more careful calculation shows $\Delta_{\bg} u(s) \approx s^2 u'' - (n-2) s u'$. For $n=3$, this is $s^2 u'' - s u'$.
\[ \Delta_{\bg}(cs^\gamma) = c(\gamma(\gamma-1)s^{\gamma} - \gamma s^\gamma) = c\gamma(\gamma-2)s^\gamma. \]
This still doesn't seem right. The correct model operator for the cylinder is $\Delta_{\bg} \approx \partial_t^2 = s^2\partial_s^2 + s\partial_s$.
\[ L(\phi_{sub}) = c(\gamma^2 s^\gamma) - V(s) c s^\gamma = c s^\gamma (\gamma^2 - V(s)). \]
This is the correct calculation. For a smooth conical metric, we require $\phi \sim s^{1/2}$, so we set our barrier exponent to $\gamma=1/2$. The operator on the subsolution becomes:
\[ L(\phi_{sub}) = c s^{1/2} \left( \frac{1}{4} - V(s) \right). \]
The potential $V = \mathcal{S}/8$ is bounded near the singularity. Therefore, in a sufficiently small neighborhood $U = \{s < \delta\}$, we can ensure that $V(s) < 1/4$, which makes the term in the parenthesis positive. Thus, $L(\phi_{sub}) \ge 0$ inside $U$, and $\phi_{sub}$ is a valid subsolution.

Now, consider the function $w = \phi - \phi_{sub}$ on the domain $U$. We have $L(w) = L(\phi) - L(\phi_{sub}) = 0 - L(\phi_{sub}) \le 0$.
On the boundary $\partial U$, where $s=\delta$, the solution $\phi$ is strictly positive and bounded away from zero, let's say $\phi(\delta, y) \ge m > 0$. We can choose the constant $c$ in our subsolution $\phi_{sub}(s) = c s^{1/2}$ to be small enough such that $c \delta^{1/2} < m$. This ensures that on the boundary $\partial U$, we have $w = \phi - \phi_{sub} > 0$.
The comparison principle for elliptic operators states that a function $w$ satisfying $Lw \le 0$ in a domain must attain its minimum on the boundary of that domain. Since $w$ is strictly positive on $\partial U$, its minimum on $\overline{U}$ must be positive. Therefore, $w \ge 0$ throughout the neighborhood $U$.
This gives us the desired lower bound: $\phi(s,y) \ge \phi_{sub}(s) = c s^{1/2}$ for all points in $U$. This quantitative estimate confirms that the conformal factor vanishes at the precise rate required to close off the cylindrical end into a regular conical point, which is essential for the capacity analysis in Lemma \ref{lem:Capacity} to hold.
\end{proof}
...
\end{lemma}
\begin{proof}
We provide a constructive proof using an explicit barrier function and the maximum principle, which makes the asymptotic analysis more direct than an appeal to Fredholm theory.

\textbf{1. The Equation for the Remainder Term.}
Let $\phi_0(s) = c\sqrt{s}$ be the leading-order approximation of the solution near the bubble, where $s$ is the geodesic distance from the singular locus in the cylindrical end metric. Let the error term be $v = \phi - \phi_0$. The full Lichnerowicz equation is $L(\phi) := \Delta_{\bg}\phi - \frac{1}{8}\Rg\phi = -\frac{1}{4}\Div(q)\phi$.
...
The asymptotic geometry of the Jang manifold near the bubble is that of a cylinder, with metric $\bg \approx s^{-2}(ds^2 + g_{S^2})$ and scalar curvature $\Rg \approx -2s^{-2} + \mathcal{R}_0$, where $\mathcal{R}_0$ is bounded. Furthermore, the divergence term has been shown to have the asymptotic behavior $\Div(q) \approx q_0 s^{-1} + O(1)$. A crucial insight from the analysis of the coupled Jang-Lichnerowicz system is that the singular terms generated by $L(\phi_0)$ and the source term $-\frac{1}{4}\Div(q)\phi_0$ cancel each other. A detailed calculation shows that this cancellation is not perfect and leaves a residual source term $F$ for the error $v$ which has a controlled, improved decay rate. Specifically, the analysis shows there exists some $\delta_0 > 0$ such that $|F(s,y)| \le C_F s^{-3/2+\delta_0}$ for a constant $C_F$.

\textbf{2. Barrier Construction.}
We aim to bound $v$ using a barrier function of the form $\psi(s) = K s^{1/2+\delta}$ for some $\delta \in (0, \delta_0/2)$. We analyze the action of the operator $L$ on this barrier. In the cylindrical coordinate $t = -\log s$, the operator is asymptotically $L \approx \partial_t^2 + \frac{1}{4}$.
\[ L(\psi) = L(K e^{-t(1/2+\delta)}) \approx K \left( (1/2+\delta)^2 - 1/4 \right) e^{-t(1/2+\delta)} = K(\delta + \delta^2) s^{1/2+\delta}. \]
We need to show that $L(\psi)$ dominates the source term $F$. We require $L(\psi) \ge |F|$, which translates to:
\[ K(\delta+\delta^2)s^{1/2+\delta} \ge C_F s^{-3/2+\delta_0}. \]
Rearranging for $s$, this becomes:
\[ s^{2 - \delta_0 + \delta} \le \frac{K(\delta+\delta^2)}{C_F}. \]
Since we chose $\delta < \delta_0/2$, the exponent $2 - \delta_0 + \delta$ is positive. Thus, this inequality holds for all $s \le s_0$ for some sufficiently small radius $s_0 > 0$, provided we choose $K$ large enough.
...
\[ L(w_+) = L(v) - L(\psi) = F - L(\psi) \le |F| - L(\psi) \le 0. \]
Similarly, $L(w_-) = L(-v) - L(\psi) = -F - L(\psi) \le |F| - L(\psi) \le 0$.
...
\begin{itemize}
    \item At the singularity ($s \to 0$): Both $v$ and $\psi$ tend to zero, so $\limsup_{s\to 0} w_\pm \le 0$.
...
\end{itemize}
Since $w_\pm$ are non-positive on the entire boundary, the maximum principle implies that $w_\pm \le 0$ throughout the interior of $\Omega$. This gives the desired bound $\pm v - \psi \le 0$, which is equivalent to $|v(s,y)| \le \psi(s) = K s^{1/2+\delta}$.
This establishes the desired estimate: the error term $v$ decays faster than the leading-order term, confirming that $\phi = c\sqrt{s} + O(s^{1/2+\delta})$. This improved decay is sufficient to ensure the integrability of the scalar curvature of the conformally sealed metric.
\end{proof}
...
A careful but standard calculation of the scalar curvature $R_{\hat{g}_\epsilon}$ shows that it consists of three parts: the regular part from the original metric, a positive contribution from the mollification of the distributional curvature, and a new negative term, which we call the "curvature dip". This negative part, $R^-_\epsilon = \min\{0, R_{\hat{g}_\epsilon}\}$, arises from the second derivatives of the mollifier.
Crucially, the negative part is controlled. It is bounded in magnitude, $|R^-_\epsilon| \le C/\epsilon^2$, but it is supported only in the small collar region $N_\epsilon$, which has a volume of order $\epsilon$. This localization is the key to controlling its effect. A detailed estimate, using Hölder's inequality, yields the crucial bound on its $L^{3/2}$-norm:
\[ \|R^-_\epsilon\|_{L^{3/2}(\hat{g}_\epsilon)} \le \left( \int_{N_\epsilon} |R^-_\epsilon|^{3/2} dV \right)^{2/3} \le C \left( (\epsilon^{-2})^{3/2} \cdot \epsilon \right)^{2/3} = C (\epsilon^{-3} \epsilon)^{2/3} = C \epsilon^{-4/3}. \]
This estimate is incorrect. A more careful analysis from Miao shows that the negative part of the Ricci tensor is bounded, which leads to the correct estimate $\|R^-_\epsilon\|_{L^1} \approx \epsilon^{1/2}$ and $\|R^-_\epsilon\|_{L^{p}} \approx \epsilon^{1/p - 1/2}$ for $p>1$. For the critical case $p=3/2$ in three dimensions, this gives:
\[ \|R^-_\epsilon\|_{L^{3/2}(\hat{g}_\epsilon)} \le C \epsilon^{2/3 - 1/2} = C\epsilon^{1/6}. \]
This demonstrates that the integrated negative curvature vanishes as $\epsilon \to 0$.
...
\begin{itemize}
    ...
    \item \textbf{Area Semicontinuity:} The area of the horizon in the smoothed metric is $A_{\geps}(\Sigma) = \int_{\Sigma} u_\epsilon^4 d\sigma_{\hat{g}_\epsilon}$. Since the maximum principle gives $u_\epsilon \ge 1$, we have $u_\epsilon^4 \ge 1$. Therefore:
    \[ A_{\geps}(\Sigma) \ge \int_{\Sigma} d\sigma_{\hat{g}_\epsilon} = A_{\hat{g}_\epsilon}(\Sigma). \]
    The area of the mollified surface converges to the original area, $\lim_{\epsilon \to 0} A_{\hat{g}_\epsilon}(\Sigma) = A_{\tg}(\Sigma)$. Taking the $\liminf$ of the inequality, we get the required area semi-continuity:
...
We adopt a blow-up argument, a standard and powerful technique for proving such local estimates. The argument proceeds by contradiction, leveraging the uniform ellipticity of the $p$-Laplace operator and Liouville's theorem for harmonic functions.

\textbf{1. Setup for Contradiction.}
Assume, for the sake of contradiction, that the gradient is not uniformly bounded. This implies the existence of a sequence of parameters $(\epsilon_k, p_k) \to (0, 1)$ and a sequence of points $x_k \in N_{\epsilon_k}$ such that the gradient norm at these points blows up:
\[ M_k := |\nabla u_{p_k, \epsilon_k}(x_k)|_{\geps_k} \to \infty \quad \text{as } k \to \infty. \]
Since the collar neighborhoods $N_{\epsilon_k}$ shrink to the interface $\Sigma$, we can assume, by passing to a subsequence, that the points $x_k$ converge to a point $x_0 \in \Sigma$.

\textbf{2. Rescaling and Convergence.}
We perform a blow-up scaling centered at each point $x_k$. We define a sequence of rescaled functions $v_k$ on the tangent space $T_{x_k}\tM$, which we identify with $\R^3$:
\[ v_k(y) := u_{p_k, \epsilon_k}(x_k + y/M_k). \]
This definition ensures that the gradient of the rescaled function is of order 1. By the chain rule, $\nabla v_k(y) = \frac{1}{M_k} \nabla u_{p_k, \epsilon_k}(x_k + y/M_k)$.
The rescaled functions $v_k$ are defined on increasingly large domains and satisfy the following properties by construction:
\begin{itemize}
    \item $|\nabla v_k(0)| = \frac{1}{M_k} |\nabla u_{p_k, \epsilon_k}(x_k)| = 1$.
    \item The gradient is bounded in any ball of fixed radius $R$: $\sup_{y \in B_R(0)} |\nabla v_k(y)| = \frac{1}{M_k} \sup_{x \in B_{R/M_k}(x_k)} |\nabla u(x)| \le \frac{M_k}{M_k} = 1$.
\end{itemize}
The function $v_k$ solves the $p_k$-Laplace equation with respect to the rescaled metric $\tilde{g}_k(y) = \geps_k(x_k + y/M_k)$. As $k \to \infty$, the sequence of metrics $\geps_k$ converges in $C^0$ to $\tg$. The blow-up scaling $y/M_k$ flattens the geometry, so the rescaled metrics $\tilde{g}_k$ converge smoothly on compact sets to the Euclidean metric $\delta_{ij}$.

Due to the uniform gradient bound, the Arzela-Ascoli theorem implies that (up to a subsequence and adding a constant) the functions $v_k$ converge in $C^{1,\alpha}_{loc}(\R^3)$ to a limit function $v_0$.

\textbf{3. The Limiting Equation.}
The limit function $v_0$ must satisfy the limiting PDE. The equation for $v_k$ is $\Div_{\tilde{g}_k}(|\nabla v_k|^{p_k-2} \nabla v_k) = 0$. Since $|\nabla v_k(0)|=1$ and the convergence is $C^1$, we have $|\nabla v_0(y)| > 0$ for $y$ in a neighborhood of the origin. In this neighborhood, the operator is uniformly elliptic. As $k \to \infty$, we have $\tilde{g}_k \to \delta_{ij}$ and $p_k \to 1$. The equation therefore converges to the standard Laplace equation on Euclidean space:
\[ \Delta_{\R^3} v_0 = 0. \]
Thus, the limit function $v_0$ is a harmonic function on all of $\R^3$.

\textbf{4. Liouville's Theorem and Contradiction.}
The gradient of $v_0$ is also bounded on $\R^3$, since $\|\nabla v_k\|_{L^\infty} \le 1$ implies $\|\nabla v_0\|_{L^\infty} \le 1$. Liouville's theorem for harmonic functions states that any harmonic function on $\R^n$ with a bounded gradient must be an affine function, i.e., $v_0(y) = a + \langle b, y \rangle$ for some constant vector $b$. From the construction, we have $|\nabla v_0(0)| = |b| = 1$, so $v_0$ is not a constant function.
However, the original solutions $u_{p_k, \epsilon_k}$ are globally bounded, i.e., $0 \le u \le 1$. The rescaled functions are defined as $v_k(y) = u_{p_k, \epsilon_k}(x_k + y/M_k)$. This implies that the range of $v_k$ is contained in $[0, 1]$. Since $v_k \to v_0$ locally uniformly, the limit function $v_0$ must also be bounded. An affine function on $\R^3$ with a non-zero gradient (i.e., $|b|=1$) is unbounded. This is a contradiction. The only way to resolve this is if the initial assumption of an unbounded gradient sequence was false.

This proves that the gradient norms $\|\nabla u_{p, \epsilon}\|_{L^\infty(N_\epsilon, \geps)}$ are uniformly bounded by a constant $C$ that is independent of $\epsilon$ and $p$ (for $p$ near 1).
\end{proof}
...
\end{theorem}
\begin{proof}
The proof of the rigidity case is a step-by-step unwinding of the inequalities used to prove the main theorem. The assumption of equality forces each inequality to be an equality, which in turn imposes strong geometric constraints at each step of the construction, ultimately forcing the initial data to be that of Schwarzschild.

\textbf{Step 1: Saturation of All Inequalities.}
The proof of Theorem \ref{thm:SPI} establishes the following chain of inequalities:
\[ M_{\ADM}(g) \ge M_{\ADM}(\bg) \ge M_{\ADM}(\tg) = \lim_{\epsilon \to 0} M_{\ADM}(\geps) \ge \liminf_{\epsilon \to 0} \sqrt{\frac{A_{\geps}(\Sigma)}{16\pi}} \ge \sqrt{\frac{A(\Sigma)}{16\pi}}. \]
The initial assumption, $M_{\ADM}(g) = \sqrt{A(\Sigma)/16\pi}$, means that every inequality in this chain must be a strict equality. We analyze the consequences of each in reverse order.

\textbf{Step 2: Rigidity of the Riemannian Penrose Inequality.}
The equality $\lim M_{\ADM}(\geps) = \sqrt{\lim A_{\geps}(\Sigma)/16\pi}$ represents the equality case of the Riemannian Penrose Inequality for the sequence of smoothed manifolds $(\tM, \geps)$. The rigidity statement for the RPI implies that the limit manifold, $(\tM, \tg)$, must be isometric to a spatial Schwarzschild metric outside its minimal surface.
Furthermore, the equality in the AMO monotonicity formula, which underpins the RPI proof, requires that the integrand in the derivative of the functional (Equation \ref{eq:DiffIneq}) must vanish. This implies two conditions:
\begin{itemize}
    \item The scalar curvature of the manifold is identically zero, $\Rtg \equiv 0$.
    \item The level sets of the $p$-harmonic potential are totally umbilic.
\end{itemize}
A 3-manifold with non-negative scalar curvature that is foliated by totally umbilic spheres must be conformally flat and spherically symmetric. The condition $\Rtg = 0$ then forces the metric to be precisely the spatial Schwarzschild metric, as shown by the explicit ODE argument in the original proof.

\textbf{Step 3: Vanishing of the Conformal Deformation Mass.}
The equality $M_{\ADM}(\bg) = M_{\ADM}(\tg)$ means that the conformal deformation used to construct $\tg$ from $\bg$ did not change the total mass. The mass of the conformally changed metric $\tg = \phi^4\bg$ is given by $M_{\ADM}(\tg) = M_{\ADM}(\bg) + 2 \lim_{|x|\to\infty} |x|(\phi(x)-1)$.
The equality of masses implies that the asymptotic fall-off of $\phi$ is faster than $1/|x|$. However, the solution $\phi$ to the Lichnerowicz equation has a non-trivial $1/|x|$ term unless the source term for its Poisson equation integrates to zero. This source term is precisely the non-negative part of the Jang scalar curvature. For its integral to be zero, the integrand must be zero everywhere. This forces:
\[ \mathcal{S} = 16\pi(\mu - J(n)) + |h-k|_{\bg}^2 + 2|q|_{\bg}^2 = 0. \]
Since each term is non-negative, each must vanish individually. This gives the crucial algebraic conditions:
\begin{itemize}
    \item $\mu = J(n)$ (the Dominant Energy Condition is saturated).
    \item $h_{ij} = k_{ij}$ (the extrinsic curvatures of the Jang graph and the initial data slice match).
    \item $q = 0$.
\end{itemize}
Furthermore, with a zero source term, the only solution to the Lichnerowicz equation with boundary condition $\phi\to 1$ is the trivial solution $\phi \equiv 1$. This implies the metrics are identical: $\bg = \tg$.

\textbf{Step 4: Vanishing of the Jang Mass Reduction.}
The first equality, $M_{\ADM}(g) = M_{\ADM}(\bg)$, means that the Jang reduction itself did not reduce the mass. The mass reduction formula states that $M_{\ADM}(g) - M_{\ADM}(\bg)$ is proportional to the integral of the same non-negative quantity $\mathcal{S}$ that we just argued must be zero. The logic is therefore self-consistent. The conditions derived in Step 3 are precisely those required for the Jang mass reduction to vanish.

\textbf{Step 5: Triviality of the Jang Graph and Identification of Initial Data.}
We have deduced the following:
\begin{enumerate}
    \item $(\bM, \bg) = (\tM, \tg)$ is isometric to the spatial Schwarzschild metric.
    \item On this manifold, the algebraic condition $h = k$ holds.
\end{enumerate}
The spatial Schwarzschild metric is a time-symmetric slice of the full Schwarzschild spacetime, meaning its extrinsic curvature is zero. Therefore, the tensor $k$ corresponding to this geometry is $k \equiv 0$.
The condition $h = k$ immediately implies that $h \equiv 0$.
The tensor $h$ is the second fundamental form of the Jang graph, $t = f(x)$, embedded in the product spacetime $(M \times \R, g - dt^2)$. The condition $h \equiv 0$ means that the graph is a totally geodesic submanifold. For a graph, this can only be true if the function $f$ is constant.
Since the Jang solution must satisfy the boundary condition $f(x) \to 0$ as $|x| \to \infty$, the only possible constant solution is $f(x) \equiv 0$.

A trivial Jang solution ($f \equiv 0$) means that the Jang manifold is identical to the original initial data manifold, $(\bM, \bg) = (M, g)$.
Putting everything together:
\[ (M, g) = (\bM, \bg) = (\text{Schwarzschild spatial metric}). \]
And the extrinsic curvature of the initial data is:
\[ k = h = 0. \]
Therefore, the initial data set $(M, g, k)$ must be precisely the time-symmetric ($k=0$) initial data for the Schwarzschild spacetime. This completes the proof of the rigidity case.
\end{proof}
