...
\end{lemma}
\begin{proof}
The convergence $\geps \to \tg$ is $C^0$ globally and smooth away from the interface $\Sigma$. We utilize tools from Geometric Measure Theory (GMT) to handle the convergence of the outermost minimal surfaces $\Sigma_\epsilon := \Sigma_{min, \epsilon}$ in $(\tM, \geps)$.

Since the manifolds are asymptotically flat and have non-negative scalar curvature, and the boundary areas are controlled, the sequence of surfaces $\Sigma_\epsilon$ lies in a compact region. We view each $\Sigma_\epsilon$ as a stationary integral varifold $V_\epsilon$ in $(\tM, \geps)$. By the uniform area bounds ($A(\Sigma_\epsilon) \le A_{\geps}(\Sigma) \le C$), Allard's compactness theorem ensures the existence of a subsequence that converges in the varifold sense to a limit stationary integral varifold $V$ in the limit metric $(\tM, \tg)$.

Under $C^0$ metric convergence, the mass (area) functional is lower semicontinuous:
\begin{equation}
    \liminf_{\epsilon \to 0} A_{\geps}(\Sigma_\epsilon) = \liminf_{\epsilon \to 0} \|V_\epsilon\|(\tM) \ge \|V\|(\tM).
\end{equation}
The support of the limit varifold, $\Sigma_{lim} = \supp(V)$, is a stationary set (minimal surface) in $(\tM, \tg)$. Furthermore, since each $\Sigma_\epsilon$ is an \emph{outermost} minimal surface enclosing the horizon, the limit set $\Sigma_{lim}$ must also enclose the horizon (homology is preserved in the limit).

In the metric $\tg$, the horizon interface $\Sigma$ is the unique outermost minimal surface by construction (any other minimal surface would have to be in the bulk, which is scalar-flat and essentially Schwarzschild-like in rigidity cases, or by the strict stability of the original MOTS). Since $\Sigma$ is the unique outermost minimal surface, and $\Sigma_{lim}$ is a minimal surface enclosing it (or equal to it), and the area of $\Sigma_\epsilon$ is bounded by $A(\Sigma)$ from above (due to the barrier construction), the limit must coincide with $\Sigma$. Specifically, if $\Sigma_{lim}$ were distinct from $\Sigma$ and "outside" it, it would contradict the uniqueness or area minimality properties. Thus, $V$ is supported on $\Sigma$.

Therefore, we have:
\[ \liminf_{\epsilon \to 0} A_{\geps}(\Sigma_\epsilon) \ge A_{\tg}(\Sigma). \]
This establishes the required semicontinuity.
\end{proof}
