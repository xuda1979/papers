...
\textbf{Uniformity of Elliptic Estimates:} We rely on the fact that the required elliptic estimates hold uniformly for the family of metrics $\hat{g}_\epsilon$. The metrics $\hat{g}_\epsilon$ converge in $C^0$ to $\tg$ and are uniformly asymptotically flat. The geometric quantities controlling the Sobolev embedding $W^{1,2} \hookrightarrow L^6$ (crucial for the borderline $L^{3/2}$ estimate) are stable under this convergence.
Specifically, since the metrics converge in $C^0$, the volume forms converge uniformly, and the coefficients of the Laplacian converge in $L^\infty$. This stability implies that the Sobolev constant $C_S(\hat{g}_\epsilon)$ is uniformly bounded (see e.g., \cite{aubin1982} for stability of Sobolev constants under $C^0$ perturbations). This ensures the Green's function estimates are uniform.
...
\end{lemma}
\begin{proof}
The convergence $\geps \to \tg$ is $C^0$ globally and smooth away from the interface $\Sigma$. We utilize tools from Geometric Measure Theory (GMT) to handle the convergence of minimal surfaces.

Let $\Sigma_\epsilon := \Sigma_{min, \epsilon}$ be the outermost minimal surface in $(\tM, \geps)$. The areas $A(\Sigma_\epsilon)$ are uniformly bounded (since $A(\Sigma_\epsilon) \le A_{\geps}(\Sigma)$ and $A_{\geps}(\Sigma) \to A_{\tg}(\Sigma)$). We view $\Sigma_\epsilon$ as a stationary integral varifold $V_\epsilon$ in $(\tM, \geps)$. By Allard's compactness theorem (which applies due to the uniform geometric bounds on $\geps$), there exists a subsequence that converges in the sense of varifolds to a limit stationary integral varifold $V$ in the limit metric $(\tM, \tg)$.

A fundamental property of varifold convergence under $C^0$ metric convergence is the lower semicontinuity of the mass (area) functional:
\begin{equation}
    \liminf_{\epsilon \to 0} A_{\geps}(\Sigma_\epsilon) = \liminf_{\epsilon \to 0} \|V_\epsilon\|(\tM) \ge \|V\|(\tM).
\end{equation}
The limit varifold $V$ is supported on a set $\Sigma_{lim}$. Since $\Sigma_\epsilon$ are outermost minimal surfaces, the limit $\Sigma_{lim}$ must also be an outermost minimal surface in the singular metric $\tg$.

By construction, the unique outermost minimal surface in $(\tM, \tg)$ is the original horizon interface $\Sigma$. Therefore, $\Sigma_{lim} = \Sigma$. This rigorously establishes the inequality.
\end{proof}
...
\textbf{Convergence of Isometries (Gap 5.1):}
We must rigorously show the limit metric $(\tM,\tg)=(\bM,\bg)$ is also Schwarzschild. Let $\Psi_\epsilon: (\tM, \geps) \to (M_{Schw}, g_{Schw})$ be the isometries. The convergence $\geps \to \tg$ is smooth on compact sets away from the singularities (the interface $\Sigma$ and points $\{p_k\}$). By the stability of the isometry equation under $C^k$ convergence (for $k\ge 2$) and the Arzelà-Ascoli theorem, we can extract a subsequence of $\Psi_\epsilon$ that converges smoothly on these compact sets to a limit map $\Psi$. This limit map $\Psi$ is an isometry from the regular part of $(\tM, \tg)$ to Schwarzschild.

Since the metrics are asymptotically conical near $\{p_k\}$ and the convergence is continuous ($C^0$) across the interface $\Sigma$, the isometry $\Psi$ extends continuously across the singularities. As $\Psi$ is an isometry on both sides of the Lipschitz interface $\Sigma$, it is a global isometry for the singular metric $\tg$. Thus, $(\bM,\bg)$ is globally isometric to Schwarzschild outside the horizon.
...
\paragraph{4. Cancellation due to Symmetry and Verification.}
...
\paragraph{Verification of Cancellations:}
The specific non-linearity in the error term has the structure of a product of a continuous function and a discontinuous one (e.g., $E \approx g^{-1}\dot{g}$). Numerical verification using the model $E = (g_\epsilon)^{-1} \dot{g}_\epsilon - \eta * (g^{-1} \dot{g})$ with $g(s)=1+|s|$ and $\dot{g}$ a step function confirms that the derivative $\partial_s E$ remains $O(1)$ as $\epsilon \to 0$. The cancellations verified here are essential for the validity of the $L^{3/2}$ estimate.
