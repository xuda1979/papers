...
\begin{remark}[Role of the Decay Rate $\tau$]\label{rem:DecayRateRole}
The decay rate of the initial data, $\tau>1/2$, is crucial here. The source term in the Lichnerowicz equation involves $\Div(q)$. Given the decay of the initial data $k=O(r^{-\tau-1})$, we have $q=O(r^{-\tau-1})$ and $\Div(q)=O(r^{-\tau-2})$.

For the Fredholm theory to apply, we must ensure $\Div(q)$ lies in the appropriate target space, typically $L^2_{\delta-2}(\bM)$. The condition for $r^{-\tau-2} \in L^2_{\delta-2}$ requires the integral of the weighted square to be finite, which leads to the condition $\delta < \tau$.

We seek a solution $\phi-1 \in H^2_\delta$ that decays as $O(r^{-1})$, requiring $\delta \in (-1, 1/2)$. Since $\tau>1/2$, we can always choose $\delta$ (e.g., $\delta=-1/2+\epsilon$ for small $\epsilon>0$) such that the operator is Fredholm ($\delta$ avoids the indicial roots) and the source term is in the correct space ($\delta < \tau$). This justifies the well-posedness of the PDE.
\end{remark}
...
\begin{itemize}
    ...
    With this weight, the operator is Fredholm. The solution $\phi_0$ will decay to zero (either exponentially or polynomially depending on the metric decay, but effectively controlled by the weight).
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Horizon End:} Here $\phi \to 1$. If $\Omega$ extends down the cylinder, we must check the flux.
    The decay of $\phi$ to 1 is at least polynomial (and exponential in the strict stability case). Let $\phi = 1 + \psi$ where $\psi \to 0$.
    Then $Y \approx 4\psi \nabla \psi - \psi^2 q$.
    If $\psi \sim e^{-\beta t}$ (strict stability), then $Y \sim e^{-2\beta t}$. The area of the cross section is constant. Thus the integral $\int Y \cdot \nu d\sigma \sim e^{-2\beta t} \to 0$.
    If $\psi \sim t^{-k}$ (marginal stability), then $\nabla \psi \sim t^{-k-1}$.
    $Y \approx 4 t^{-k} t^{-k-1} - t^{-2k} q \approx O(t^{-2k-1})$.
    The flux is $\int Y \cdot \partial_t d\sigma \sim t^{-2k-1} \cdot \text{Area} \to 0$ as $t \to \infty$.
\end{itemize}
...
\end{lemma}
\begin{proof}
We establish the area bound using the Barrier Principle for minimal surfaces in manifolds with non-negative scalar curvature.

\textbf{1. The Cylindrical Barrier.}
Consider the cylindrical end $\mathcal{C} = \Sigma \times [0, \infty)_t$ in the target manifold $(\tM, \tg)$. The metric is $\tg \approx dt^2 + g_\Sigma$. The scalar curvature is non-negative.
For any $L > 0$, the cylinder segment $\mathcal{C}_L = \Sigma \times [0, L]$ has boundary components $\Sigma_0 = \Sigma \times \{0\}$ and $\Sigma_L = \Sigma \times \{L\}$.
The mean curvature of the slices $\Sigma_t$ is approximately zero.
In the smoothed metric $\geps$, the scalar curvature is non-negative, $R_{\geps} \ge 0$.
The interface $\Sigma$ (corresponding to $t=0$) is a minimal surface in $\tg$ (or has $H \ge 0$ facing the bulk).
The smoothing creates a metric $\geps$ close to $\tg$.

\textbf{2. Minimal Surfaces are Trapped.}
Let $\Sigma_{\min, \epsilon}$ be the outermost minimal surface in $(\tM, \geps)$.
Since $\Sigma_{\min, \epsilon}$ is minimal ($H=0$), it acts as a barrier for the mean curvature flow.
More importantly, we compare $\Sigma_{\min, \epsilon}$ with the foliation of the cylinder.
If $A(\Sigma_{\min, \epsilon}) < A(\Sigma)$, then $\Sigma_{\min, \epsilon}$ would have to effectively "pinch" the cylinder.
However, a fundamental result for manifolds with non-negative scalar curvature (related to the Geroch monotonicity) implies that the area of a minimal surface is a local minimum.
Specifically, since the boundary $\Sigma$ is stable (outermost MOTS condition) and the cylinder extends this geometry, any surface homologous to $\Sigma$ in the neighborhood must have area at least $A(\Sigma)$.
The minimal surface $\Sigma_{\min, \epsilon}$ is stable. The second variation of area involves the scalar curvature:
\[ \int (|\nabla \psi|^2 - (\Ric(\nu, \nu) + |A|^2)\psi^2) \ge 0. \]
Using the Gauss-Codazzi equation $2\Ric(\nu,\nu) = R_{amb} - R_{\Sigma} + H^2 - |A|^2$, and $R_{amb} \ge 0, H=0$, we have a constraint on the area.
Furthermore, we can use the maximum principle with the cylindrical barriers. The surfaces $\Sigma_t$ essentially foliate the end. Since $R \ge 0$, the Hawking mass is non-decreasing. Since $m_H(\Sigma_t) \approx \sqrt{A/16\pi}$ (as $H \approx 0$), the area is non-decreasing along the cylinder.
Thus, $\Sigma_{\min, \epsilon}$ cannot lie "deep" inside the cylinder with smaller area. It must lie near $\Sigma$ or further out. In either case, $A(\Sigma_{\min, \epsilon}) \ge A(\Sigma) - o(1)$.

\textbf{Conclusion.}
The cylinder $\Sigma \times [0, L]$ acts as a mean-convex barrier (after slight perturbation to make $H>0$ if necessary) that traps the minimal surface. The non-negative scalar curvature prevents the existence of a minimal surface with area strictly less than the bottleneck $\Sigma$.
...
Across the Lipschitz interface $\Sigma$, the metric $\tg$ is continuous and the distance structure is well-behaved (geodesics exist). The metric isometry $\Psi$ patches the two smooth regions (bulk and cylinder) together isometrically. Since the image in Schwarzschild is smooth, the "kink" in $\tg$ (if any) must match the geometry of the image. However, we established earlier that in the equality case, $h=k$ and $q=0$, which implies the jump in extrinsic curvature vanishes ($[h]=0$) if the data comes from a smooth slice. Indeed, since $\Psi$ maps onto a smooth slice of Schwarzschild, and $\tg$ is isometric to it, $\tg$ must be smooth (or at least as regular as the slice). This implies the Lipschitz interface is actually a smooth transition in the embedding, consistent with the vanishing of the scalar curvature and the matching conditions.
