...
\end{enumerate}
    ...
    While slower than the exponential decay in the strictly stable case, this polynomial decay is sufficient to establish the Fredholm properties of the Lichnerowicz operator using the \textbf{Weighted Scattering Calculus} (Melrose's $b$-calculus).

    \textbf{Explicit Verification of Decay and Solvability:}
    The vector field $q$ essentially measures the deviation of the Jang surface's geometry from the target initial data $k$. In the cylindrical limit, $q$ is driven by the remainder term $v$ in the expansion $f \sim C_0 \log s + v$. Specifically, we have the structural relation:
    \[ |q|_{\bg} \lesssim |\nabla_{\bg} v| + |v| \lesssim t^{-k}. \]
    This link guarantees that the source term behaves well even in the marginal case:
    \begin{itemize}
        \item \textbf{$L^2$ Integrability of $q$:} Since $q \sim t^{-k}$, we have $|q|^2 \sim t^{-2k}$. The integral $\int^\infty |q|^2 dt$ converges for $2k > 1$, i.e., $k > 1/2$. Thus, even for the slow decay case $k=1$, $q \in L^2(\mathcal{E}_{Cyl})$. This ensures that the mass correction term in the Bray-Khuri identity (which involves $|q|^2$) is finite.
        \item \textbf{Source Term in Range (Horizon End):} The source term for the Lichnerowicz equation is $\Div_{\bg}(q) \sim t^{-k-1}$.
        \begin{itemize}
            \item \textbf{Integrability:} We require $\Div(q)$ to be in the weighted $L^2$ space. $\int^\infty (\Div q)^2 dt \sim \int^\infty t^{-2k-2} dt < \infty$ for $k > -1/2$. This is satisfied.
            \item \textbf{Orthogonality:} The solvability condition requires orthogonality to the constant mode (the kernel). The projection is $\int \Div(q) \cdot 1 \, dV$, which equals the boundary flux $\lim_{t\to\infty} \int_{\Sigma_t} \langle q, \nu \rangle$. Since $q \sim t^{-k}$, this flux vanishes for any $k > 0$.
        \end{itemize}
...
\paragraph{Case 2: Marginal Stability ($[H] = 0$).}
    In this case, the mean curvature buffer vanishes, and the foliation argument based on strict mean convexity is insufficient. To rule out a "deep well" of area loss, we rely on the \textbf{Minimizing Hull Property} of the weak Inverse Mean Curvature Flow (IMCF) in the scalar-non-negative smoothing collar.

    The smoothed manifold $(\tM, \geps)$ has non-negative scalar curvature. According to the theory of Huisken and Ilmanen \cite{huisken2001}, there exists a proper weak solution to the IMCF starting from $\Sigma$, defined via the level sets of a function $u$. A fundamental property of this flow is that for any level $t$, the surface $\Sigma_t = \partial \{u < t\}$ minimizes the Hawking mass among all surfaces homologous to $\Sigma$ and enclosing $\{u < t\}$.

    Specifically, the monotonicity formula states that $m_H(\Sigma_t)$ is non-decreasing. At the horizon ($t=0$), $H_\Sigma=0$, so $m_H(0) = \sqrt{A(\Sigma)/16\pi}$.
    Suppose there exists an outermost minimal surface $\Sigma_{\min, \epsilon}$ in the smoothed metric (which might be distinct from $\Sigma$ due to the perturbation). Since $\Sigma_{\min, \epsilon}$ is minimal ($H=0$) and homologous to $\Sigma$, it is a candidate for the long-time limit or a barrier for the flow.
    Crucially, the weak IMCF strictly expands area. Huisken and Ilmanen proved that $|\Sigma_t| = |\Sigma_0| e^t$. Any stable minimal surface $\Sigma'$ enclosing $\Sigma$ acts as a barrier that the flow cannot cross. In fact, if $\Sigma'$ is minimal, the flow must eventually coincide with it (or jump over it).

    If $\Sigma_{\min, \epsilon}$ had area $A' < A(\Sigma)$, it would violate the Initial Condition Regularity of the flow. The flow starting at area $A(\Sigma)$ cannot jump immediately to a surface of smaller area because the level sets minimize the functional $J_t(E) = |\partial E| - \int_E H \, dx$ (conceptually). A sudden drop in area would imply a negative variation, contradicting the minimality.
    More formally, since $\R_{\geps} \ge 0$, the Hawking mass is non-decreasing along the flow. Any minimal surface $\Sigma'$ in the exterior is a stationary solution. If $\Sigma'$ encloses $\Sigma$, we can run the flow from $\Sigma$ to $\Sigma'$. The monotonicity implies:
    \[ \sqrt{\frac{A(\Sigma')}{16\pi}} = m_H(\Sigma') \ge m_H(\Sigma) = \sqrt{\frac{A(\Sigma)}{16\pi}}. \]
    Thus $A(\Sigma') \ge A(\Sigma)$.
    This confirms that the area of the minimal surface cannot drop below the horizon area, even in the marginal case where local barriers are weak.
...
\end{lemma}
\begin{proof}
We must rule out the possibility that a sequence of level sets concentrates area at a tip $p_k$ as $p \to 1$. We establish a \textbf{Density Lemma}: Any surface passing through the vertex of the cone has strictly higher area than a surface smoothed to avoid it.
Consider the cone metric $g = dr^2 + r^2 g_{S^2}$. The area of a cone truncated at radius $R$ is $\frac{1}{2} R^2 \text{Area}(S^2)$. The area of the cross-section disk at $R$ is significantly less if the cone angle is small, but here we compare to a minimal surface.
More rigorously, since the capacity of the vertex is zero (Lemma \ref{lem:Capacity}), for any $u \in W^{1,p}$, we can construct a cut-off sequence $u_k$ vanishing near $p_k$ such that $\mathcal{E}_p(u_k) \to \mathcal{E}_p(u)$.
The perimeter functional is the limit of these energies. If a minimal surface $\Sigma$ passed through $p_k$, it would have to be singular there. However, the monotonicity formula implies that the density $\Theta(p_k)$ is bounded. Since the metric is regular conical (not a cusp), the area of balls $B_r(p_k)$ scales as $r^2$. A surface with finite area cannot "hide" infinite or non-zero measure at a single point without violating the monotonicity of the density ratio.
Thus, the singular set contributes zero to the limiting measure of the mean curvature energy $\int H^2$, ensuring that the Penrose Inequality limit is valid.
\end{proof}
...
Since $k \ge 1$, the limit as $t \to \infty$ is zero.
