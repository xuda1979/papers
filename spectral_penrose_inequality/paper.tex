...
\begin{proposition}[Uniqueness of Minimal Surfaces in Marginal Case]\label{prop:SpectralAreaControl}
Let $\Sigma$ be a marginally stable outermost MOTS ($\lambda_1(L_\Sigma) = 0$). For sufficiently small $\epsilon$, the smoothed metric $\hat{g}_\epsilon$ admits a unique foliation by CMC surfaces near $\Sigma$. In particular, the only minimal surface in the collar $N_{2\epsilon}$ is the perturbation of $\Sigma$ itself, and its area satisfies
\begin{equation}
    A_{\hat{g}_\epsilon}(\Sigma_{\epsilon}) \ge A_{\tg}(\Sigma) - O(\epsilon^2).
\end{equation}
\end{proposition}
\begin{proof}
We employ a \textbf{Lojasiewicz-Simon Gradient Inequality} argument to handle the degeneracy of the stability operator.
In the marginally stable case, the linearized operator $L_\Sigma$ has a non-trivial kernel spanned by the principal eigenfunction $\varphi_0 > 0$. Standard Implicit Function Theorem arguments fail.
We decompose the perturbation space for the surface position $u$ into the kernel $K = \text{span}\{\varphi_0\}$ and its orthogonal complement $K^\perp$.
By the Lyapunov-Schmidt reduction, the problem of finding a minimal surface reduces to a finite-dimensional problem on the kernel. The area functional $\mathcal{A}(t\varphi_0)$ along the kernel direction is real analytic. By the Lojasiewicz-Simon inequality, the gradient of the area functional satisfies $|\nabla \mathcal{A}| \ge C |\mathcal{A} - \mathcal{A}_0|^\theta$.
Since $\Sigma$ is the *outermost* MOTS, it is a local minimizer (or saddle) of the area functional such that no area-decreasing variation exists outwards. The stability condition $\lambda_1 \ge 0$ implies the area is non-decreasing to second order.
Crucially, the perturbation $\hat{g}_\epsilon$ is constructed by a symmetric mollification. We verify that the perturbation is orthogonal to the kernel (which represents a translation) to leading order.
Explicitly, we construct the foliation of CMC surfaces. The uniqueness of this foliation in the perturbative regime ensures that we do not have bifurcation of minimal surfaces. The minimal surface $\Sigma_\epsilon$ in the smoothed metric is the leaf with mean curvature zero. Its position is $O(\epsilon)$ close to $\Sigma$.
The area expansion gives:
\[ A(\Sigma_\epsilon) = A(\Sigma) + \frac{1}{2} \int u L_\Sigma u + \text{Error}. \]
Since $\lambda_1=0$, the quadratic term vanishes on the kernel. However, higher order terms or the specific form of the smoothing (which increases area via Jensen's inequality, Lemma \ref{lem:AreaExpansion}) ensure that we do not enter a "deep well".
\end{proof}
