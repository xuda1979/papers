...
\end{lemma}
\begin{proof}
The proof employs a \textbf{Quantitative Barrier Argument} using a strictly mean-convex foliation to rigorously rule out the "Deep Well" scenario (the formation of minimal surfaces with small area within the smoothing region).
...
\begin{itemize}
    \item If $\Sigma'$ lies in the bulk ($s \ge 2\epsilon$), it approximates $\Sigma$.
    \item If $\Sigma'$ lies in the cylinder ($s \le -\epsilon$), the monotonicity of the Hawking mass for the cylindrical metric implies $A(\Sigma') \ge A(\Sigma)$.
    \item If $\Sigma'$ crosses the collar, its area is controlled by the area of $\Sigma$ plus small error terms of order $O(\epsilon)$.
\end{itemize}
Thus, no "deep well" minimal surface with area significantly less than $A(\Sigma)$ can form in the smoothing region. This establishes $\liminf A(\Sigma_{\min, \epsilon}) \ge A(\Sigma)$.
\end{proof}
