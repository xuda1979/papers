...
\textbf{Step 2: Conformal Correction and Pointwise Estimate.}
To correct for the negative curvature, we define the final smoothed metric as $\geps = u_\epsilon^4 \hat{g}_\epsilon$, where the conformal factor $u_\epsilon$ solves the elliptic PDE
\begin{equation}
    8 \Delta_{\hat{g}_\epsilon} u_\epsilon - R^-_\epsilon u_\epsilon = 0, \quad \text{with} \quad u_\epsilon \to 1 \text{ at infinity.}
\end{equation}
This choice ensures that the scalar curvature of $\geps$ is non-negative. We now prove a quantitative estimate on this conformal factor, which is essential for area semicontinuity.

\begin{lemma}[Properties of the Conformal Factor]\label{lem:ConformalFactorEstimate}
The solution $u_\epsilon$ to the conformal correction equation satisfies:
\begin{enumerate}
    \item $u_\epsilon(x) \ge 1$ for all $x \in \tM$.
    \item There exists a constant $C$ independent of $\epsilon$ such that $\|u_\epsilon - 1\|_{L^\infty(\tM)} \le C \epsilon^{2/3}$.
\end{enumerate}
\end{lemma}
\begin{proof}
1. \textbf{Positivity:} Since $R^-_\epsilon = \min\{0, R_{\hat{g}_\epsilon}\} \le 0$, the equation is $8 \Delta_{\hat{g}_\epsilon} u_\epsilon = R^-_\epsilon u_\epsilon$. Because $u_\epsilon \to 1$ at infinity, if it were to drop below 1, it must attain an interior minimum. At such a minimum $x_0$, we would have $u_\epsilon(x_0) > 0$, $\Delta u_\epsilon(x_0) \ge 0$, and $R^-_\epsilon(x_0) \le 0$. The equation then implies $0 \le 8 \Delta u_\epsilon = R^-_\epsilon u_\epsilon \le 0$, forcing $\Delta u_\epsilon = 0$. The strong maximum principle then implies $u_\epsilon$ must be constant, a contradiction. Thus, $u_\epsilon \ge 1$ everywhere.

2. \textbf{Uniform Convergence:} Let $v_\epsilon = u_\epsilon - 1$. Since $u_\epsilon \ge 1$, $v_\epsilon \ge 0$. It solves the Poisson equation $8\Delta_{\hat{g}_\epsilon} v_\epsilon = R^-_\epsilon u_\epsilon$ on the asymptotically flat manifold $(\tM, \hat{g}_\epsilon)$, with $v_\epsilon \to 0$ at infinity.
Let $f_\epsilon = R^-_\epsilon u_\epsilon$. From the maximum principle argument above and the boundary condition at infinity, $u_\epsilon$ is uniformly bounded. Therefore, $\|f_\epsilon\|_{L^{3/2}} \le \|u_\epsilon\|_{L^\infty} \|R^-_\epsilon\|_{L^{3/2}} \le C_1 \|R^-_\epsilon\|_{L^{3/2}}$.
As established in Step 1, we have $\|R^-_\epsilon\|_{L^{3/2}} \le C_2 \epsilon^{2/3}$.
Standard elliptic estimates for the Laplacian on an asymptotically flat 3-manifold (which follow from the Hardy-Littlewood-Sobolev inequality) give a bound for the $L^\infty$ norm of a function in terms of the $L^{3/2}$ norm of its Laplacian. Specifically,
...
We now use the estimate on $u_\epsilon$ to prove the area does not shrink in the limit. The area of the horizon in the smoothed metric is $A_{\geps}(\Sigma) = \int_{\Sigma} u_\epsilon^4 d\sigma_{\hat{g}_\epsilon}$.
Since $u_\epsilon \ge 1$ (from Lemma \ref{lem:ConformalFactorEstimate}), we have $u_\epsilon^4 \ge 1$. Therefore,
...
This confirms the fourth property of the smoothing procedure.
\end{proof}
