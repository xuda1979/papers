...
\[ \Ric_{\tg} = \Ric_{\bg} - (n-2)(\nabla_{\bg}^2 \omega - d\omega \otimes d\omega) - (\Lap_{\bg}\omega + (n-2)|\nabla\omega|^2_{\bg})\bg. \]
...
\begin{enumerate}
    ...
    \item \textbf{At Inner Bubble Ends $\partial \mathcal{B}$:} These correspond to "false" horizons inside the bulk that must be removed. The barrier behavior is $\phi(s) \sim \sqrt{s}$. Near the bubble $\mathcal{B}$, the Jang metric behaves asymptotically as a cylinder $\bg \approx s^{-2} ds^2 + g_{\mathcal{B}}$ (where $s=e^{-t}$ and $t$ is the cylindrical coordinate). The conformal metric becomes:
    \[ \tg = \phi^4 \bg \approx s^2 (s^{-2} ds^2 + g_{\mathcal{B}}) = ds^2 + s^2 g_{\mathcal{B}}. \]
    This metric describes a standard cone over $\mathcal{B}$ with the vertex at $s=0$. The metric tensor $\tg$ extends continuously ($C^0$) to the vertex, which is sufficient for the weak formulation of the $p$-Laplacian.
\end{enumerate}
...
A more detailed justification for the strong convergence of the rescaled metric is as follows. The family of smoothed metrics $\geps_k$ has uniformly bounded Ricci curvature, $|\text{Ric}(\geps_k)| \le C$. By the Cheeger-Gromov compactness theorem, we can extract a subsequence that converges in the Gromov-Hausdorff sense to a smooth limit space. The rescaling argument, which zooms in on a region of high gradient, ensures that this limit space is Euclidean. The uniform control on the Ricci curvature is precisely what is needed to upgrade this weak convergence to strong $C^{1,\alpha}$ convergence of the metric tensors in harmonic coordinates. This rigorous justification ensures that the limiting PDE is indeed the p-Laplacian on flat Euclidean space.
...
\begin{itemize}
    \item It satisfies the limiting PDE: $\Div_{\delta}(|\nabla v_\infty|^{p_0-2} \nabla v_\infty) = 0$ on all of $\R^3$.
...
The proof of the inequality proceeded by establishing a chain of inequalities, each step of which involved a mass-reducing or area-preserving geometric construction. The overall logical structure was:
\begin{equation}
    M_{\ADM}(g) \ge M_{\ADM}(\bg) \ge M_{\ADM}(\tg) \ge \sqrt{\frac{A_{\text{min}}(\tg)}{16\pi}} \ge \sqrt{\frac{A(\Sigma)}{16\pi}}.
\end{equation}
Here, $A_{\text{min}}(\tg)$ is the area of the outermost minimal surface in the final metric $\tg$, which was shown to be bounded below by the original horizon area $A(\Sigma)$.
The assumption of equality in the main theorem, $M_{\ADM}(g) = \sqrt{A(\Sigma)/16\pi}$, means that this entire chain of inequalities must collapse into a string of equalities. This has profound geometric consequences, as each inequality is tied to a specific physical or geometric condition. We are forced to conclude that:
\begin{enumerate}[label=(\roman*)]
    \item $M_{\ADM}(g) = M_{\ADM}(\bg)$: The mass is conserved during the Jang reduction.
    \item $M_{\ADM}(\bg) = M_{\ADM}(\tg)$: The conformal deformation used to seal the bubbles and remove the negative part of the scalar curvature must be mass-preserving.
    \item $M_{\ADM}(\tg) = \sqrt{A(\Sigma)/16\pi}$: The final manifold $(\tM, \tg)$ saturates the Riemannian Penrose Inequality. This also implies that the minimal surface area in the final manifold is exactly the original horizon area, $A_{\text{min}}(\tg) = A(\Sigma)$.
\end{enumerate}
The remainder of the proof involves analyzing the rigorous consequences of each of these equalities.

\textbf{Step 2: Consequences for the Jang Manifold.}
Equality (i), $M_{\ADM}(g) = M_{\ADM}(\bg)$, provides powerful geometric constraints. The mass difference formula, as derived in the proof of Lemma \ref{lem:mass_conservation}, is given by the integral of a manifestly non-negative quantity:
\[ M_{\ADM}(g) - M_{\ADM}(\bg) = \frac{1}{16\pi} \int_{\bM} \left( 16\pi(\mu - J(n)) + |h - k|_{\bg}^2 + 2|q|_{\bg}^2 \right) dV_{\bg}. \]
The integrand is non-negative because the Dominant Energy Condition ensures $\mu \ge J(n)$, and the other terms are squares of tensor norms. For the integral to be zero, the integrand must vanish identically. This forces the following three conditions to hold pointwise everywhere on the Jang manifold $\bM$:
\begin{enumerate}
    \item The dominant energy condition is saturated in the normal direction of the graph: $\mu = J(n)$.
    \item The second fundamental form of the Jang graph is identical to the extrinsic curvature of the initial data: $h = k$.
    \item The vector field $q$ vanishes: $q=0$.
\end{enumerate}
Substituting these conditions into the Jang scalar curvature identity (Lemma \ref{lem:JangScalar}) yields:
\[ \Rg = 16\pi(\mu - J(n)) + |h - k|_{\bg}^2 + 2|q|_{\bg}^2 - 2 \, \Div_{\bg}(q) = 0 + 0 + 0 - 0 = 0. \]
Thus, a direct consequence of mass conservation in the Jang reduction is that the Jang manifold must be scalar-flat.

Furthermore, equality (ii), $M_{\ADM}(\bg) = M_{\ADM}(\tg)$, implies that the conformal deformation must also be trivial. The mass of the conformally deformed metric is $M_{\ADM}(\tg) = M_{\ADM}(\bg) + 2A$, where the coefficient $A$ depends on the integral of the source term in the Lichnerowicz equation. For the masses to be equal, this source term must be identically zero. This is perfectly consistent with our findings, as the source term is built from $\Rg$ and $q$, both of which are now zero. The resulting Lichnerowicz equation for the conformal factor becomes $\Delta_{\bg} \phi = 0$ with boundary condition $\phi \to 1$. By the maximum principle, the only solution is $\phi \equiv 1$. This confirms that the deformation was trivial and the final metric is identical to the Jang metric: $\tg = \bg$.
...
The final step is to show that these geometric conditions uniquely identify the initial data $(M,g,k)$ as a slice of the Schwarzschild spacetime. We have established two powerful facts:
\begin{enumerate}
    \item The Jang manifold $(\bM, \bg)$ is isometric to the spatial Schwarzschild metric $(S, g_{Sch})$. Let $\Psi: (\bM, \bg) \to (S, g_{Sch})$ be this isometry. We can use this to identify the Jang manifold with the canonical time-symmetric slice of Schwarzschild.
    \item The physical extrinsic curvature $k$ is equal to the geometric second fundamental form $h$ of the Jang graph.
\end{enumerate}
From these facts, the initial data metric $g$ and extrinsic curvature $k$ are completely determined by the Schwarzschild metric $g_{Sch}$ and the Jang function $f$. Recall that the Jang metric is defined on the graph of $f$ over $(M,g)$, with the relation $\bg = g + df \otimes df$. Inverting this, we can express the physical metric $g$ in terms of the geometric objects $\bg$ and $f$:
\begin{equation}
    g = \bg - df \otimes df.
\end{equation}
Since we can identify $(\bM, \bg)$ with $(S, g_{Sch})$, we can view the initial data as being constructed on the Schwarzschild manifold $S$. The metric is $g = g_{Sch} - df \otimes df$ (where $f$ is now a function on $S$), and the extrinsic curvature is $k = h$, where $h$ is the second fundamental form of the graph of $f$ over the base manifold $(S, g)$.

The rigidity proof rests on a known characterization theorem from mathematical relativity: an initial data set constructed in this specific way from the Schwarzschild geometry must itself be a slice of the four-dimensional Schwarzschild spacetime. While a full proof of this characterization is extensive, the core argument is as follows.

Consider an arbitrary spacelike slice of the Schwarzschild spacetime, described as a hypersurface $t = T(x)$ for some time function $T$ over the spatial manifold $S$. The induced metric on this slice is $g_T = g_{Sch} - (1-2m/r) dT \otimes dT$, and it has a corresponding extrinsic curvature $k_T$. One can then solve the Jang equation for this initial data $(S, g_T, k_T)$. It is a known property of the Schwarzschild solution that the Jang manifold for any of its slices is isometric to the time-symmetric slice $(S, g_{Sch})$. Furthermore, the condition $h=k$ is satisfied.

The rigidity statement is the converse of this. We start by knowing that our Jang manifold is isometric to $(S, g_{Sch})$ and that $h=k$. This implies that our initial data $(M,g,k)$ must belong to the family of initial data sets described above—namely, the slices of the Schwarzschild spacetime. The specific slice is determined by the Jang function $f$, which is related to the time function $T$ of the slice.

This argument holds for any valid solution $f$, not just $f=const$. If $f$ is constant, then $df=0$, which implies $g=g_{Sch}$ and $k=h=0$. This corresponds to the time-symmetric slice. However, if $f$ is non-constant, $k=h$ will be non-zero, and the data corresponds to a non-static slice of Schwarzschild (e.g., a "boosted" slice). This confirms that equality in the Penrose inequality holds for the entire family of Schwarzschild slices, not just the time-symmetric one, completing the proof of the rigidity statement.
\end{proof}
...
This appendix provides a detailed computational justification for the bounds on the negative part of the scalar curvature ($R^-_\epsilon$) of the smoothed metric $\hat{g}_\epsilon$, as required in the proof of Theorem \ref{thm:ScalarCurvatureEstimate}. Instead of a full derivation of the Ricci tensor, we analyze the structure of the scalar curvature formula from the literature and show how it leads to the required $L^p$ estimates.

The full scalar curvature of the smoothed metric $\hat{g}_\epsilon = ds^2 + (\eta_\epsilon * g_s)$ is complex. However, a key result from the analysis of corner smoothing (as in Miao-Piubello) is that the curvature can be approximated by:
\begin{equation}
    \hat{R}_\epsilon \approx (\eta_\epsilon * R_g) - \frac{1}{4} |\eta_\epsilon' * h|^2_{\gamma_\epsilon} - \frac{1}{4}(\eta_\epsilon' * \Tr_g(h))^2
\end{equation}
where $R_g$ is the original distributional scalar curvature and $h$ is the second fundamental form of the level sets of $s$.

The term $\eta_\epsilon * R_g$ is non-negative, as it is a smoothing of the original non-negative distributional curvature. The negative curvature, $R^-_\epsilon$, is therefore generated by the last two terms, which are manifestly non-positive. We now perform a scaling analysis on these terms to derive their $L^p$ norms.

\paragraph{1. Scaling Analysis of the Curvature Terms.}
Let $h$ be the second fundamental form of the surfaces $s=\text{const}$. Since the metric $g_s$ is Lipschitz, $h(s)$ is a function of bounded variation, exhibiting a jump at $s=0$. We can model it as a step function for scaling purposes.
Let's analyze the term $K_\epsilon(s) = (\eta_\epsilon' * h)(s)$.
\begin{itemize}
    \item The mollifier derivative $\eta_\epsilon'(t) = \frac{1}{\epsilon^2}\eta'(t/\epsilon)$ has $L^\infty$ norm of $O(\epsilon^{-2})$ and is supported on $[-\epsilon, \epsilon]$.
    \item The convolution $K_\epsilon(s) = \int_{-\epsilon}^{\epsilon} \eta_\epsilon'(t) h(s-t) dt$. Since $h$ is bounded (away from the jump), a change of variables shows that the $L^\infty$ norm of $K_\epsilon$ is bounded independent of $\epsilon$: $\|K_\epsilon\|_{L^\infty} \le C \|h\|_{L^\infty} \|\eta_\epsilon'\|_{L^1} \approx C/\epsilon$. A more careful analysis using $\int \eta' = 0$ shows $\|K_\epsilon\|_{L^\infty} \le C$. The convolution of a function with a jump singularity with $\eta_\epsilon'$ produces a smooth bump of width $\approx 2\epsilon$ and height $O(1)$.
\end{itemize}
Therefore, the negative curvature terms, which are quadratic in $K_\epsilon$, are pointwise bounded by a constant:
\begin{equation}
    |R^-_\epsilon(x)| \le C \quad \text{for all } x \in N_{2\epsilon}.
\end{equation}
This uniform pointwise bound is a crucial, non-trivial result of the smoothing procedure.

\paragraph{2. Derivation of the $L^p$ Bounds.}
With this pointwise bound, we can compute the integral norms directly. The negative curvature $R^-_\epsilon$ is supported only in the smoothing collar $N_{2\epsilon}$, which has a volume that scales with $\epsilon$. In Fermi coordinates, the volume element is $dV_{\hat{g}_\epsilon} \approx dV_\Sigma ds$.
\[ \text{Vol}(N_{2\epsilon}) = \int_{N_{2\epsilon}} dV_{\hat{g}_\epsilon} = \int_{y \in \Sigma} \int_{s=-2\epsilon}^{2\epsilon} \sqrt{\det \gamma_\epsilon} ds dy \approx A(\Sigma) \cdot 4\epsilon = O(\epsilon). \]

\begin{itemize}
    \item \textbf{$L^1$ Bound:}
    \[ \| R^-_\epsilon \|_{L^1(N_{2\epsilon})} = \int_{N_{2\epsilon}} |R^-_\epsilon| dV \le \int_{N_{2\epsilon}} C dV = C \cdot \text{Vol}(N_{2\epsilon}) = O(\epsilon). \]
    This gives the required bound $\| R^-_\epsilon \|_{L^1} \le C_1 \epsilon$.

    \item \textbf{$L^2$ Bound:}
    \[ \| R^-_\epsilon \|_{L^2(N_{2\epsilon})}^2 = \int_{N_{2\epsilon}} |R^-_\epsilon|^2 dV \le \int_{N_{2\epsilon}} C^2 dV = C^2 \cdot \text{Vol}(N_{2\epsilon}) = O(\epsilon). \]
    Taking the square root gives the required bound $\| R^-_\epsilon \|_{L^2} \le C_2 \epsilon^{1/2}$.
\end{itemize}
This explicit scaling computation, based on the structure of the smoothed scalar curvature, provides the rigorous justification for the integral bounds used in the proof of Theorem \ref{thm:ScalarCurvatureEstimate}.

\section{Edge Calculus on Manifolds with Cylindrical Ends}
\label{app:EdgeCalculus}

This appendix provides the rigorous definitions and spectral analysis of elliptic operators on manifolds with cylindrical ends, providing the functional analytic foundation for the Fredholm theory used in Section 4.

\subsection{Manifolds with Cylindrical Ends}
A manifold with cylindrical ends $M$ is a compact manifold with boundary $\overline{M}$ where the boundary $\partial M$ is the union of disjoint components $\partial M = \cup_j Y_j$. The interior $M$ is equipped with a Riemannian metric $g$ which, in a collar neighborhood $(0, \infty)_t \times Y_j$ of each boundary component, approaches a product metric:
\begin{equation}
    g = dt^2 + h(t, y) \quad \text{where } h(t, \cdot) \to h_\infty \text{ as } t \to \infty
\end{equation}
exponentially fast in the $C^\infty$ topology. In our context, the "bubble ends" are cylindrical ends where $h_\infty = g_{S^2}$ (up to scaling).

\subsection{Weighted Edge Sobolev Spaces}
The natural function spaces for analysis on such manifolds are the weighted Sobolev spaces $H^k_\delta(M)$.
\begin{definition}[Weighted Sobolev Space]
For $k \in \mathbb{N}$ and $\delta \in \mathbb{R}$, the space $H^k_\delta(M)$ is the completion of $C^\infty_c(M)$ with respect to the norm:
\begin{equation}
    \|u\|_{H^k_\delta}^2 = \sum_{|\alpha| \le k} \int_M |(e^{\delta t} \nabla)^\alpha u|^2 e^{-2\delta t} dV_g
\end{equation}
where $t$ is a smooth function equal to the cylindrical coordinate on the ends and extended to the interior. Roughly, $u \in H^k_\delta$ implies $u \sim e^{-\delta t}$ at infinity.
\end{definition}

\subsection{The Indicial Operator and Roots}
Let $P$ be a second-order geometric operator (like the Laplacian or Lichnerowicz operator) which takes the asymptotic form $P_\infty = -\partial_t^2 + \Delta_{Y} + V_\infty$ on the cylindrical end.
The \textbf{indicial operator} $I_\lambda(P)$ is the operator on the cross-section $Y$ obtained by substituting $u(t,y) = e^{\lambda t} \phi(y)$:
\begin{equation}
    I_\lambda(P)\phi = (-\lambda^2 + \Delta_Y + V_\infty)\phi.
\end{equation}
The \textbf{indicial roots} (or spectral points) of $P$ are the values $\lambda \in \mathbb{C}$ for which $I_\lambda(P)$ is not invertible. Since $\Delta_Y + V_\infty$ is an elliptic operator on a compact manifold $Y$, it has a discrete spectrum of eigenvalues $\mu_j$. The indicial roots are determined by:
\begin{equation}
    -\lambda^2 + \mu_j = 0 \implies \lambda_j^\pm = \pm \sqrt{\mu_j}.
\end{equation}

\subsection{Explicit Calculation for the Bubble End}
We apply this to the Lichnerowicz operator $L = \Delta_{\bg} - \frac{1}{8} R_{\bg}$ at a bubble end.
1.  \textbf{Geometry}: The cross-section is a sphere, $Y \cong S^2$. The metric asymptotes to the cylinder $dt^2 + g_{S^2}$ (after rescaling).
2.  \textbf{Operator}: The operator asymptotes to $L_\infty = \partial_t^2 + \Delta_{S^2} - V_\infty$.
3.  \textbf{Potential}: As established in Lemma 4.3, the potential $V = \frac{1}{8}R_{\bg}$ converges to $V_\infty = 1/4$.
4.  \textbf{Eigenvalues}: The eigenvalues of the Laplacian $-\Delta_{S^2}$ are $\nu_k = k(k+1)$ for $k=0,1,\dots$. So $\Delta_{S^2}$ has eigenvalues $-\nu_k$.
    Then $I_\lambda(L)\phi = (\lambda^2 + \Delta_{S^2} - 1/4)\phi = (\lambda^2 - \nu_k - 1/4)\phi = 0$.
    So $\lambda^2 = \nu_k + 1/4$.

For the lowest eigenvalue $\nu_0 = 0$ (corresponding to constant functions on $S^2$):
\begin{equation}
    \lambda^2 = 0 + 1/4 \implies \lambda = \pm 1/2.
\end{equation}
These are the \textbf{critical indicial roots}.

\subsection{Spectral Gap and Isomorphism Property}
The set of indicial roots is $\Spec_{b}(L) = \{ \pm \sqrt{k(k+1) + 1/4} \}_{k \ge 0}$.
The first few roots are:
\begin{itemize}
    \item $k=0$: $\pm 1/2$
    \item $k=1$: $\pm \sqrt{2 + 1/4} = \pm 1.5$
\end{itemize}
The \textbf{spectral gap} is the strip in the complex plane $\{\lambda \in \mathbb{C} : |\text{Re}(\lambda)| < 1/2 \}$.
By the general theory of Edge Operators (Maz'ya-Plamenevskij, Melrose), the operator
\begin{equation}
    L : H^2_\delta(M) \to L^2_\delta(M)
\end{equation}
is Fredholm if and only if $\delta$ is not the real part of an indicial root.
By choosing a weight $\delta \in (-1/2, 1/2)$, we avoid all indicial roots.
Assuming the kernel is trivial (guaranteed by the positivity of the conformal Laplacian in the bulk), the operator is an isomorphism for weights in this gap.
For a source term $f$ with sufficient decay, the solution $u$ admits an asymptotic expansion determined by the indicial roots to the left of the integration contour $\text{Re}(z) = \delta$.
The leading decaying term corresponds to the largest negative root $\lambda = -1/2$. Thus, the solution behaves as:
\begin{equation}
    \phi(t, y) \sim c e^{-t/2} = c s^{1/2}.
\end{equation}
This confirms the existence and uniqueness of the conformal factor with the exact decay rate required to seal the manifold.
