...
\end{lemma}
\begin{proof}
The derivation of this identity is a crucial step in connecting the geometry of the Jang manifold to the initial data. It follows from the Gauss-Codazzi equations for the embedding of the graph $\bM = \text{graph}(f)$ into the product spacetime $(M \times \R, g - dt^2)$.

Let $\bar{\nabla}$ be the Levi-Civita connection on $(\bM, \bg)$, and $\nabla$ be the connection on $(M,g)$. The relationship between the scalar curvatures is given by the Gauss equation. For a hypersurface, this relates the intrinsic curvature to the ambient curvature and the second fundamental form. A more direct approach, following Schoen and Yau, is to relate the connection coefficients.
Let $\{e_i\}$ be a local frame on $M$. A corresponding frame on $\bM$ is $\{\bar{e}_i = e_i + (\partial_i f) \partial_t\}$. The scalar curvature $\Rg$ can be computed from the commutator of covariant derivatives.
The key insight is to decompose the second fundamental form of the graph, $h$, and relate it to the initial data tensor $k$.

The scalar curvature $\Rg$ of the induced metric $\bg = g + df \otimes df$ is given by a standard formula for the curvature of a graph:
\begin{equation}
\Rg = R_g - 2 \text{Ric}_g(\nu, \nu) - |h|^2_{\bg} + H_{\bM}^2,
\end{equation}
where $\nu = \nabla f / |\nabla f|$ is the horizontal component of the normal to the graph, and $h$ is the second fundamental form of the graph viewed as a hypersurface in $(M,g)$. This formula is insufficient as it does not reference the time dimension or the extrinsic curvature $k$.

The full derivation uses the ambient spacetime. Let the spacetime metric be $g_4 = g - dt^2$. The unit normal to the graph $\bM$ is $n = (\nabla f - \partial_t)/\sqrt{1-|\nabla f|_g^2}$.
The Gauss equation for the embedding $\bM \hookrightarrow (M\times\R, g_4)$ states
\[ R_{\bM} = R_{g_4}|_{\bM} + 2 \text{Ric}_{g_4}(n,n) + H_{\bM}^2 - |h|^2_{\bg}. \]
Using the Einstein field equations, we can replace the spacetime curvature terms with the stress-energy tensor $T$. The DEC is the condition $\mu \ge |J|_g$, where $16\pi \mu = R_g + (\text{Tr}_g k)^2 - |k|_g^2$ and $8\pi J = \text{div}(k - (\text{Tr}_g k)g)$.
A lengthy but direct computation of the projected curvature terms yields the final identity. The key steps involve:
\begin{enumerate}
    \item Expressing the spacetime Ricci tensor in terms of the initial data $(\mu, J)$.
    \item Contracting the Ricci tensor with the normal vector $n$ to find $\text{Ric}_{g_4}(n,n)$. This term becomes $8\pi(\mu - J(n))$.
    \item Using the Jang equation $H_{\bM} = \text{Tr}_{\bg}(k)$ to replace the mean curvature term.
    \item Rearranging the remaining terms involving $h$ and $k$ to produce the $|h-k|_{\bg}^2$ and divergence terms. The divergence term arises from the difference between the covariant derivatives on $\bM$ and $M$.
\end{enumerate}
This process transforms the purely geometric Gauss equation into an equation relating the Jang metric to the physical quantities of the initial data.
\end{proof}
...
\[ M_{\ADM}(g) \ge M_{\ADM}(\bg) \ge \lim_{\epsilon \to 0} M_{\ADM}(\geps) \ge \lim_{\epsilon \to 0} \sqrt{\frac{A_{\geps}(\Sigma)}{16\pi}} = \sqrt{\frac{A(\Sigma)}{16\pi}}. \]
The initial assumption that $M_{\ADM}(g) = \sqrt{A(\Sigma)/16\pi}$ forces every inequality in this chain to be a strict equality. We analyze the consequences of each equality.

\textbf{Step 2: Rigidity of the Corner and Vanishing of Curvature Jump.}
The final inequality, $M_{\ADM}(\geps) \ge \sqrt{A_{\geps}(\Sigma)/16\pi}$, is an application of the standard Riemannian Penrose Inequality to a sequence of smooth manifolds $(\tM, \geps)$ with $\Scal_{\geps} \ge 0$. The equality of the limits, $\lim M_{\ADM}(\geps) = \sqrt{\lim A_{\geps}(\Sigma)/16\pi}$, implies rigidity for the limit manifold $(\tM, \tg)$.
The manifold $(\tM, \tg)$ has a distributional scalar curvature $\Scal_{\tg} = 2 \Jump{H_{\tg}} \delta_\Sigma$, where $\Jump{H_{\tg}} \ge 0$ arises from the corner at the gluing interface $\Sigma$. The mass of this limit manifold can be seen as the sum of the bulk mass and a contribution from the corner curvature. The Positive Mass Theorem for manifolds with corners asserts that if the mass is zero (relative to the horizon area, as is the case here), then the manifold must be flat and the corner cannot contribute positively to the mass. This implies the distributional scalar curvature must vanish identically. Therefore, we must have
\[ \Jump{H_{\tg}} = 0. \]
This condition means that the mean curvature across the interface $\Sigma$ is continuous. As established in Theorem \ref{thm:HanKhuri}, the stability of the outermost MOTS implies $\Jump{H_{\tg}} \ge 0$. Equality is only possible if the MOTS is \emph{marginally stable}, meaning the principal eigenvalue of its stability operator is zero, $\lambda_1(L_\Sigma) = 0$. For an initial data set satisfying the DEC, this marginal stability condition is known to imply that the data must be totally geodesic on the horizon, i.e., $k_{ij} = 0$ on $\Sigma$.

\textbf{Step 3: From Scalar-Flat to Schwarzschild.}
The rigidity of the Riemannian Penrose inequality for the limit manifold $(\tM, \tg)$ implies that $(\tM, \tg)$ must be isometric to the spatial Schwarzschild metric. The constant nature of the AMO functional $\mathcal{M}_p(t)$ in the equality case requires the foliation by level sets to be totally umbilic and the scalar curvature $\Rtg$ to be identically zero. We now explicitly derive the metric from these conditions.

A 3-manifold foliated by umbilic spheres is spherically symmetric, and its metric can be written in the general form:
\begin{equation}
    \tg = U(r)^{-1} dr^2 + r^2 (d\theta^2 + \sin^2\theta d\phi^2) = U(r)^{-1} dr^2 + r^2 g_{S^2},
\end{equation}
for some positive function $U(r)$. The scalar curvature $\Rtg$ for a metric of this form is given by the standard formula:
\begin{equation}
    \Rtg = \frac{2}{r^2} \frac{d}{dr} \left( r(1 - U(r)) \right) = \frac{2(1 - U - rU')}{r^2}.
\end{equation}
The rigidity condition $\Rtg \equiv 0$ therefore implies the following first-order linear ordinary differential equation for $U(r)$:
\begin{equation}
    1 - U(r) - r \frac{dU}{dr} = 0.
\end{equation}
This equation can be rewritten as $\frac{d}{dr}(rU(r)) = 1$. Integrating with respect to $r$ yields $rU(r) = r + C$, where $C$ is a constant of integration. This gives the general solution:
\begin{equation}
    U(r) = 1 + \frac{C}{r}.
\end{equation}
To determine the constant $C$, we use the condition that the manifold is asymptotically flat. The radial component of the metric is $g_{rr} = U(r)^{-1} = (1 + C/r)^{-1}$. For large $r$, this has the expansion $g_{rr} \approx 1 - C/r + O(r^{-2})$. By definition, the ADM mass $m$ is related to this fall-off by $g_{rr} = 1 + 2m/r + O(r^{-2})$. Comparing the two expansions, we identify $-C = 2m$, or $C = -2m$.

Substituting this constant back into the solution for $U(r)$, we find $U(r) = 1 - 2m/r$. The metric is therefore uniquely determined to be:
\[ \tg = \left( 1 - \frac{2m}{r} \right)^{-1} dr^2 + r^2 g_{S^2}, \]
which is the spatial Schwarzschild metric. The mass parameter $m$ is precisely the ADM mass $M_{\ADM}(\tg)$.

\textbf{Step 4: Vanishing of the Jang Reduction Mass Difference.}
We now analyze the first equality in the chain, $M_{\ADM}(g) = M_{\ADM}(\bg)$. The mass difference formula for the Jang reduction is given by the integral of a non-negative quantity:
\begin{equation}
...
\end{itemize}
Furthermore, the equality $M_{\ADM}(\bg) = M_{\ADM}(\tg)$ implies that the mass contribution from the conformal deformation is zero. This happens if the integrated source term for the conformal factor's Poisson equation vanishes. Combined with the fact that $\tg$ must be Schwarzschild, it can be shown that the only consistent solution is a trivial conformal factor, $\phi \equiv 1$, meaning the metrics are identical: $\bg=\tg$.

\textbf{Step 5: Triviality of the Jang Graph and Conclusion.}
Combining these results, we have $\bg = \tg$, and from Step 3, this metric is the spatial Schwarzschild metric. The standard time-symmetric slice of Schwarzschild is static, meaning its extrinsic curvature is zero. Therefore, $k$ for the Schwarzschild metric is identically zero.
From Step 4, we have the condition $h = k$. Since the background metric is Schwarzschild, we must have $k \equiv 0$, which in turn implies $h \equiv 0$.
The tensor $h$ is the second fundamental form of the Jang graph $t = f(x)$. The condition $h \equiv 0$ means the graph is a totally geodesic submanifold, which implies that $f$ must be a solution to the minimal surface equation on $(M,g)$ with vanishing boundary conditions at infinity. The only such solution is the trivial one, $f(x) \equiv 0$.

Since $f \equiv 0$, the Jang graph is trivial, and the initial data manifold $(M, g)$ is identical to the Jang manifold $(\bM, \bg)$. From our deductions, this manifold is isometric to the spatial Schwarzschild metric.
