...
\begin{abstract}
The Spacetime Penrose Inequality conjectures a fundamental relationship between the total mass of a spacetime and the area of the horizons it contains, specifically $M_{\ADM} \ge \sqrt{A/16\pi}$, provided the spacetime satisfies the dominant energy condition. We present a rigorous framework detailing the proof strategy for this conjecture in full generality. The approach integrates the Generalized Jang Equation (GJE) reduction with the $p$-harmonic level set method developed by Agostiniani, Mazzieri, and Oronzio (AMO). A critical aspect of this framework is addressing the analytical obstructions inherent in the Jang reduction, namely potential singularities (Jang bubbles) and the failure of the Jang metric to possess pointwise non-negative scalar curvature. We detail how a subsequent metric deformation procedure, following the Bray-Khuri construction, resolves these issues by producing an auxiliary Riemannian manifold with non-negative scalar curvature, enabling the rigorous application of the AMO monotonicity formula in conjunction with the generalized Riemannian Penrose inequality.
\end{abstract}
...
The Penrose Inequality is a cornerstone of geometric analysis and General Relativity, providing crucial support for the Weak Cosmic Censorship Hypothesis \cite{wald1984}. It establishes a sharp geometric inequality relating the total energy of an isolated gravitational system to the size of the black holes within it.
...
The general spacetime case ($k \ne 0$) requires a reduction to a Riemannian setting. The primary tool for this is the Generalized Jang Equation (GJE) \cite{schoen1981}. However, the resulting Jang manifold $(\bM, \bg)$ presents significant analytical challenges:
\begin{enumerate}
    \item It may possess singularities (Jang bubbles).
...
The rigorous proof strategy combines the GJE reduction, a sophisticated metric deformation to resolve these issues (following Bray and Khuri \cite{braykhuri2011}), and the application of robust methods for the Riemannian Penrose inequality, such as the Nonlinear Level Set Method (AMO) \cite{amo2022}.
...
Let $(\tM, \tg)$ be as above with $\Rtg \ge 0$. For $1 < p < 3$, there exists an explicitly defined geometric functional $\mathcal{M}_p(t)$, depending on the geometry of $\Sigma_t$ and the behavior of $u_p$, such that (in a suitable weak sense):
\[ \frac{d}{dt} \mathcal{M}_p(t) \ge 0. \]
\end{theorem}
\begin{proof}[Sketch of Proof]
The proof relies fundamentally on the Bochner identity applied to the $p$-harmonic function $u_p$. Differentiating $\mathcal{M}_p(t)$ leads to terms involving the Ricci curvature, specifically $\Ric_{\tg}(\nabla u_p, \nabla u_p)$. The assumption $\Rtg \ge 0$, combined with refined Kato-type inequalities, is crucial to control these terms locally and establish the non-negativity of the derivative.
\end{proof}
...
If $f$ is a smooth solution to the GJE (\ref{eq:GJE}), the scalar curvature $\Rg$ satisfies the identity:
\begin{equation}\label{eq:JangScalar}
    \Rg = 16\pi(\mu - J(v)) + |h - k|_{\bg}^2 + 2|X|_{\bg}^2 - 2 \, \Div_{\bg}(X).
\end{equation}
Here $v$ is the future-directed unit normal to the graph $\bM$, $h$ is the second fundamental form of the graph. $X$ is an auxiliary vector field defined component-wise by $X_i = \frac{\nabla^j f}{w} (h_{ij} - k_{ij})$, where $w=\sqrt{1+|\nabla f|_g^2}$.
\end{lemma}

If the DEC holds, then $\mu - J(v) \ge 0$. Consequently, the first three terms on the RHS of (\ref{eq:JangScalar}) are non-negative. Thus, $\Rg \ge - 2 \, \Div_{\bg}(X)$.
...
The term $- 2 \, \Div_{\bg}(X)$ means $\Rg$ is not necessarily non-negative pointwise. While the divergence term integrates to zero globally, this "weak non-negativity" is insufficient for the AMO monotonicity formula. The Bochner identity (used in Theorem \ref{thm:AMO}) requires local control over the curvature, which fails if $\Rg < 0$ locally.

\paragraph{Obstruction 2: Singularities (Jang Bubbles).}
The weak solution $f$ provided by Theorem \ref{thm:HanKhuri} may not be globally smooth. Singularities can form where $|\nabla f| \to \infty$ (where the graph becomes null). Near these singularities, the metric $\bg$ degenerates, and $\Rg$ can become strongly negative in a distributional sense (behaving like negative Dirac masses).
...
The construction involves several highly technical steps:

1. \textbf{Conformal Deformation and the Yamabe Problem:} We seek a conformal deformation $\tg = \phi^4 \bg$. The conformal factor $\phi$ is constructed by solving an associated Poisson-type equation (related to the Yamabe problem). This equation is designed utilizing the structure of the Jang scalar curvature identity (Lemma \ref{lem:JangScalar}) to effectively absorb the divergence term $- 2 \, \Div_{\bg}(X)$, thereby ensuring $\Rtg \ge 0$.

2. \textbf{Handling Jang Bubbles:} The conformal factor $\phi$ is constructed to have prescribed singularities (blow-up behavior) precisely at the locations of the Jang bubbles. This procedure effectively "smooths out" the singular regions by canceling the negative distributional curvature arising from the bubbles, while maintaining $\Rtg \ge 0$.

3. \textbf{Compactification:} The cylindrical end of $\bM$ (corresponding to the horizon $\Sigma$) is conformally compactified (by having $\phi$ vanish appropriately at the end), resulting in the compact minimal surface $\Sigma'$ in $(\tM, \tg)$.
\end{proof}
...
The deformation process, particularly the resolution of Jang bubbles, might alter the topology and the area of the horizon; i.e., $A(\Sigma')$ might differ from $A(\Sigma)$. Crucially, the construction in \cite{braykhuri2011} ensures that the resulting manifold $(\tM, \tg)$ satisfies the conditions for the \emph{generalized} Riemannian Penrose inequality (as established in \cite{bray2001, huisken2001} and applicable via the AMO framework). This generalized version guarantees that the mass $M_{\ADM}(\tg)$ is bounded below by the area of the \emph{original} horizon $A(\Sigma)$, even if the topology changes during deformation:
...
3. \textbf{Generalized RPI on $(\tM, \tg)$} (Section 4.2): The properties of the deformation and the application of the generalized Riemannian Penrose inequality (via the AMO method or otherwise) yield Inequality (\ref{eq:GeneralizedRPI}): $M_{\ADM}(\tg) \ge \sqrt{A(\Sigma)/16\pi}$.
