...
With the $L^1$ and $L^2$ bounds established, we use the general Hölder inequality to estimate the $L^{3/2}$ norm. Let $p=3/2$. We want to find a $\theta \in (0,1)$ such that $\frac{1}{p} = \frac{\theta}{1} + \frac{1-\theta}{2}$.
\[ \frac{2}{3} = \theta + \frac{1-\theta}{2} = \frac{2\theta + 1 - \theta}{2} = \frac{\theta+1}{2}. \]
Solving for $\theta$ gives $4 = 3\theta + 3$, which means $\theta = 1/3$.
Now, applying the Riesz-Thorin interpolation theorem (or more simply, Hölder's inequality in the form $\|f\|_{L^p} \le \|f\|_{L^1}^\theta \|f\|_{L^2}^{1-\theta}$), we have:
\begin{align*}
    \|R^-_\epsilon\|_{L^{3/2}} &\le \|R^-_\epsilon\|_{L^1}^{1/3} \cdot \|R^-_\epsilon\|_{L^2}^{2/3}.
\end{align*}
Substituting our known bounds:
\begin{equation}
    \|R^-_\epsilon\|_{L^{3/2}} \le (C_1 \epsilon)^{1/3} \cdot (C_2 \epsilon^{1/2})^{2/3} = C \epsilon^{1/3} \epsilon^{1/3} = C \epsilon^{2/3}.
\end{equation}
...
We establish the Fredholm property for the Lichnerowicz operator $L = \Lap_{\bg} - \frac{1}{8}\Rg$ on the weighted Sobolev space $\EdgeSpace{2}{\delta}$. A direct computation of the operator's indicial roots on the asymptotic cylindrical end demonstrates the existence of a spectral gap. By selecting a weight $\delta$ within this gap, we rigorously justify that $L$ is an isomorphism, guaranteeing a unique solution to the conformal deformation equation with the required asymptotic behavior, $\phi \to 1$.
...
\textbf{1. The Equation for the Remainder Term.}
Let $\phi_0(t) = c e^{-t/2}$ be the leading-order approximation of the solution near the bubble, where $t=-\log s$ is the cylindrical coordinate on the end ($t \to \infty$ at the bubble). Let the error term be $v = \phi - \phi_0$. The full Lichnerowicz equation is $L(\phi) := \Lap_{\bg}\phi - \frac{1}{8}\Rg\phi = 0$.
...
\begin{enumerate}
    \item The Jang manifold $(\bM, \bg)$ is isometric to the spatial Schwarzschild metric. Let $\Psi: (\bM, \bg) \to (\R^3 \setminus B_{M/2}, g_{sch})$ be this isometry.
    \item The second fundamental form of the Jang graph is equal to the initial data's extrinsic curvature: $h = k$.
\end{enumerate}
These two conditions provide a powerful characterization of the initial data. We follow the argument of Bray and Khuri to show this forces the initial data itself to be a slice of the Schwarzschild spacetime.

Consider the embedding of the initial data manifold $(M,g)$ into the Jang manifold's ambient space, given by the map $\mathcal{I}: M \to M \times \R$ where $\mathcal{I}(x) = (x, 0)$. Now, compose this with the geometric realization of the Jang graph and the isometry to Schwarzschild. This gives a map from the original manifold to the spatial Schwarzschild manifold.
The crucial observation is that the original metric $g$ can be recovered from the Jang metric $\bg$ and the function $f$ via $g = \bg - df \otimes df$. Since we know $\bg$ is the Schwarzschild metric and $h=k$ relates the derivatives of $f$ to the physical data, we can analyze the geometry of $(M,g)$ directly.

The condition $h=k$ implies that the graph function $f$ integrates the extrinsic curvature in a specific way. When combined with the fact that the underlying metric is Schwarzschild, this forces $(M,g,k)$ to correspond to an embedding in the full Schwarzschild spacetime. More explicitly, the data $(M,g,k)$ can be identified with a spacelike hypersurface within the Schwarzschild spacetime, making it a "slice" of that spacetime. The resulting slice is not necessarily the time-symmetric $t=const$ slice (where $k \equiv 0$). For instance, a boosted slice of Schwarzschild would have $k \ne 0$ but would still lead to equality in the Penrose inequality. This completes the proof of rigidity.
\end{proof}
