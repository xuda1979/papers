...
\paragraph{Case 2: Marginal Stability ($[H] = 0$).}
    In this case, the mean curvature buffer vanishes, and the foliation argument based on strict mean convexity is insufficient. To rule out a "deep well" of area loss, we rely on a \textbf{Perturbative Stability Argument}.

    In the marginal case, $\Sigma$ acts as a degenerate critical point for the area functional. We must show that the second variation of area is robust under the specific perturbation $\hat{g}_\epsilon \to u_\epsilon^4 \hat{g}_\epsilon$.

    Since $u_\epsilon \ge 1$ everywhere (by Lemma \ref{lem:GreenEstimate}), the area form on the horizon scales by $u_\epsilon^4 \ge 1$. Thus, pointwise area density increases. The only risk is if the minimal surface moves \emph{off} $\Sigma$ to a location of smaller area. However, since $\Sigma$ is outermost, any nearby surface has larger area (stability). The perturbation $u_\epsilon$ is $O(\epsilon^{2/3})$ in $C^0$, which is small enough to ensure the deformed stability operator preserves non-negativity.

    Specifically, let $\Sigma_t$ be a variation of $\Sigma$ in the smoothed metric. The area functional can be expanded as:
    \[ A_{\geps}(\Sigma_t) = A_{\tg}(\Sigma) + \frac{1}{2} \int_\Sigma (\phi_0^T L_\Sigma \phi_0) t^2 + \int_\Sigma (\delta u_\epsilon) + O(t^3), \]
    where $L_\Sigma$ is the stability operator. Even if $\lambda_1(L_\Sigma)=0$, the perturbation term from the conformal factor $u_\epsilon$ (which satisfies $u_\epsilon \ge 1$) provides a strictly positive zero-order term to the area expansion, or at worst a non-negative one. The interaction term involving the normal derivative of $u_\epsilon$ is controlled by the $L^{3/2}$ bounds on the scalar curvature deficit. Thus, for sufficiently small $\epsilon$, the stability is preserved, and $A(\Sigma_{\min, \epsilon}) \ge A(\Sigma) - o(\epsilon)$.
