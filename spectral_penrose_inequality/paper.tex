...
\begin{equation} \label{eq:flow_system}
    ...
    L_{g_t} \phi_t = 0 & \text{(Harmonic Potential)}, \\
    \phi_t(x) = 0 & \text{on } \Sigma, \\
    \phi_t(x) \to -1 & \text{as } |x| \to \infty, \\
    u(x,t) \to 1 & \text{as } |x| \to \infty.
    \end{cases}
\end{equation}

Here $L_{g_t} = -8\Delta_{g_t} + R_{g_t}$ is the conformal Laplacian. The function $\phi_t$ is the unique harmonic potential vanishing at the horizon and approaching a constant at infinity.
...
\begin{equation} \label{eq:mass_mono}
    \frac{d}{dt} M_{ADM}(g_t) = - \int_M 8 u^2 |\nabla \phi_t|^2 + R_{g_t} \phi_t^2 u^4 \, dV_0 \le 0.
\end{equation}
...
\end{align}
Substituting the spectral condition $L_{g_t}\phi=0$ yields the strictly negative integral form involving $|\nabla \phi|^2$.
Because $\phi_t$ is harmonic and non-trivial (due to boundary conditions), the mass decrease is strict unless the metric is already Schwarzschild (where $\phi$ would be static).
\end{proof}
