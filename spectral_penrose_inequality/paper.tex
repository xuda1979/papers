...
which ensures $\mathcal{K}_p(u) \ge 0$ distributionally, even across the critical set $\{ \nabla u = 0 \}$. The non-negativity of each term in the integrand is thus guaranteed: $\Rtg \ge 0$ by our construction, the algebraic inequality $|A|^2 \ge H^2/2$ holds for the extrinsic curvature of any surface, the term involving $|\nabla_T \nu|^2$ is manifestly non-negative, and $\mathcal{K}_p(u) \ge 0$ is guaranteed by the refined Kato inequality. Consequently, the entire integrand is non-negative.
...
\end{lemma}
\begin{proof}
The proof that the conical singularities have zero $p$-capacity is a standard calculation. We present the full argument for completeness in Appendix A, as it is a foundational step for the entire analytic framework. The calculation shows that for a test function localized around a singularity, the $p$-energy integral is bounded by $C\epsilon^{3-p}$. Since $p<3$, this term vanishes as $\epsilon \to 0$, proving $\Cap_p(\{p_k\})=0$. This result is critical as it guarantees that the singularities are removable for weak solutions of the $p$-Laplacian and do not affect the validity of the integration-by-parts arguments used in the monotonicity formula.
\end{proof}
...
Let $u_\epsilon$ be the solution to the conformal correction equation $8 \Delta_{\hat{g}_\epsilon} u_\epsilon - R^-_\epsilon u_\epsilon = 0$ with $u_\epsilon \to 1$ at infinity, where $\|R^-_\epsilon\|_{L^{3/2}} \le C_0 \epsilon^{2/3}$ as established in Theorem \ref{thm:ScalarCurvatureEstimate}. The solution satisfies:
\begin{enumerate}
...
\end{equation}
Then the initial data set $(M,g,k)$ is isometric to a slice of the Schwarzschild spacetime outside the horizon.
\end{theorem}
\begin{proof}
...
\appendix
\section{Vanishing Capacity of Singularities}
