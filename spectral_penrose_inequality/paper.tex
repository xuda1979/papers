...
\begin{abstract}
We establish a \emph{conditional} form of the Spacetime Penrose Inequality $M_{\ADM} \ge \sqrt{A/16\pi}$ for asymptotically flat initial data sets satisfying the Dominant Energy Condition, contingent upon a crucial (and currently open) spectral positivity hypothesis regarding a Lichnerowicz-type operator on the associated Jang manifold (the \emph{Spectral Condition}). The proof unifies the generalized Jang reduction with the $p$-harmonic level set method via a rigorous analysis of the \textbf{Jang--Lichnerowicz System}. A central obstruction---the non-smooth nature of the Jang metric at the horizon interface---is resolved by demonstrating that the distributional scalar curvature possesses a favorable sign structure due to the stability of the outermost MOTS. We construct a scalar-curvature preserving smoothing of the resulting Lipschitz manifold. Finally, we establish the rigidity of the equality case, showing that equality forces the initial data to arise from a Schwarzschild slice.
\end{abstract}
...
Throughout the paper we work in dimension $n=3$ and with initial data sets $(M,g,k)$ satisfying the following standing hypotheses. The manifold $M$ is complete and has a single asymptotically flat end with decay rate $\tau>1/2$, as in Definition~\ref{def:AF} below. The matter fields obey the Dominant Energy Condition ($\mu \ge |J|_g$). The boundary of the trapped region is an outermost MOTS $\Sigma\subset M$, which we assume to be compact; in addition, any Jang bubbles arising from the generalized Jang equation have spherical boundary components, cf.\ Assumption~\ref{assump:BubbleTopology}. Finally, we assume that the associated Jang manifold satisfies the \emph{Spectral Condition} (Definition~\ref{def:SpectralCondition}), a positivity condition on a Lichnerowicz-type operator which is currently open in general. Under these assumptions we prove the spacetime Penrose inequality stated in Theorem~\ref{thm:SPI}.
...
To avoid ambiguity, we briefly summarize the status of the various ingredients. The Positive Mass Theorem and the Riemannian Penrose Inequality (including rigidity) are taken from the work of Schoen--Yau, Witten, Huisken--Ilmanen, and Bray. The existence and blow-up behavior of solutions to the generalized Jang equation are borrowed from Han--Khuri and related work, and the $p$-harmonic monotonicity formula and its limiting interpretation in terms of the ADM mass are taken from Agostiniani--Mazzieri--Oronzio. On the other hand, the Spectral Condition and the spherical topology of Jang bubbles are explicit assumptions of this paper. Our main contributions are: (i) the analysis of the Jang scalar curvature in the distributional sense and its favorable sign structure; (ii) the construction of a scalar-curvature-preserving smoothing of the resulting Lipschitz manifold, adapted to an \emph{internal} corner; and (iii) the verification that the smoothed metrics are compatible with the $p$-harmonic level set method, leading to a conditional proof of the spacetime Penrose inequality.
...
Let $(M, g, k)$ be a complete, 3-dimensional, asymptotically flat initial data set satisfying the dominant energy condition ($\mu \ge |J|_g$). Let $\Sigma \subset M$ be the outermost apparent horizon, assumed to be compact, and denote its area by $A(\Sigma)$. Assume moreover that any Jang bubbles arising in the generalized Jang construction have spherical boundary components, as in Assumption~\ref{assump:BubbleTopology}. \textbf{Assuming further that the associated Jang manifold satisfies the Spectral Condition (Definition~\ref{def:SpectralCondition})}, the ADM mass satisfies:
\begin{equation}
...
The lowest eigenvalue of the operator $-\Delta_{g_{\partial\mathcal{B}}}$ on the compact manifold $\partial\mathcal{B}$ (which is $S^2$ by Assumption~\ref{assump:BubbleTopology}) is $\nu_0=0$, which corresponds to the constant eigenfunction $\psi_0=1$. Substituting this into the eigenvalue problem gives the principal indicial equation:
...
\begin{remark}
For operators with lower-order terms such as $L$, the \emph{principal eigenvalue} can be understood in the generalized sense of Berestycki--Nirenberg--Varadhan: it is the supremum of those $\lambda\in\mathbb{R}$ for which there exists a positive function $\psi>0$ satisfying $(L+\lambda)\psi\le 0$ in the weak sense with Dirichlet boundary values on the horizon. The positivity $\lambda_1(L)>0$ then yields a strong maximum principle for $L$ and, combined with the Fredholm properties of Section~4.2, leads to uniqueness and positivity of the conformal factor. In this paper we use the Spectral Condition only through these two consequences.
\end{remark}
...
\end{lemma}
\begin{proof}
The assumption $\lambda_1(L)>0$ means precisely that the generalized principal eigenvalue of $L$ (with Dirichlet boundary conditions along the horizon) is positive. By the general theory of second order elliptic operators with bounded measurable coefficients, this implies a strong maximum principle: if $L\psi \ge 0$ in a connected domain and $\psi$ attains a non-positive minimum in the interior, then $\psi$ is constant. See for example the classical work of Berestycki--Nirenberg--Varadhan.

Apply this to $\phi$, which solves $L\phi=0$ and has strictly positive boundary values (equal to~1 at infinity and along the outer cylindrical end). If $\phi$ attained a negative minimum in the interior, the maximum principle would force $\phi$ to be constant, contradicting the boundary behavior. Thus $\phi\ge 0$, and the strong form of the maximum principle then gives $\phi>0$ everywhere.

To obtain the upper bound, observe that $1-\phi$ is a subsolution for $L$ with non-negative boundary values. The same maximum principle applied to $1-\phi$ implies that $1-\phi$ cannot attain a negative minimum, so $\phi\le 1$ throughout $\bM$.
\end{proof}
...
The proof follows the conformal smoothing strategy for manifolds with corners, as developed by Miao and Piubello, which we adapt to our setting. In their work the corner is a boundary hypersurface, whereas our interface $\Sigma$ is internal. Locally, however, one can reduce the internal corner to a boundary corner by doubling across $\Sigma$, apply their result, and then restrict back to one side. The goal is to construct a sequence of smooth metrics $\geps$ that approximate the singular metric $\tg$ while satisfying the necessary geometric conditions.
...
\]
with equality if and only if $(\tM,\geps)$ is isometric to a spatial Schwarzschild manifold outside its minimal boundary. Since we already know that equality holds in the spacetime Penrose inequality for $(M,g,k)$, the limit of inequalities argument in Section~5 forces equality in the Riemannian Penrose inequality for all sufficiently small $\epsilon$. Consequently each $(\tM,\geps)$ is Schwarzschild outside its horizon, and the convergence $\geps\to\tg$ implies that $(\tM,\tg)=(\bM,\bg)$ is itself isometric to a spatial Schwarzschild manifold away from the conical and corner singularities. The smoothing procedure then shows that $(\bM,\bg)$ is globally Schwarzschild outside the horizon.
