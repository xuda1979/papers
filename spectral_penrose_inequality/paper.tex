...
Let $(M, g, k)$ be a complete, 3-dimensional, asymptotically flat initial data set satisfying the dominant energy condition ($\mu \ge |J|_g$). Let $\Sigma \subset M$ be the outermost apparent horizon, assumed to be compact, with total area $A$. \textbf{Assuming the associated Jang manifold satisfies the Spectral Condition (Definition~\ref{def:SpectralCondition})}, the ADM mass satisfies:
\begin{equation}
