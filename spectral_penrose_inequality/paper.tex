...
We perform a blow-up analysis around $x_k$. Let $y$ be Fermi coordinates centered at $x_k$ (where $y_1$ is the signed distance to $\Sigma$ if close, or geodesic coords). Define the rescaled coordinates $\tilde{y} = M_k y$.
...
\begin{enumerate}
    ...
    \item Since $u_k$ is bounded, $v_k$ is uniformly bounded on any compact set.
\end{enumerate}
...
The blow-up scale $M_k^{-1}$ is much smaller than the smoothing scale $\epsilon_k$.
If $x_k$ is within the smoothing collar (distance $\lesssim \epsilon_k$), we are zooming in on a tiny portion of the smoothed corner. Since the smoothed metric is smooth, at sufficiently small scales it looks Euclidean.
...
Since $L_k \to \infty$, the first order term vanishes. The metric converges to the flat Euclidean metric on $\mathbb{R}^3$.
(If $x_k$ is far from the corner, it also converges to Euclidean).

\textbf{Case 2: $L_k \to \lambda \in (0, \infty)$ (Comparable Scales).}
The blow-up scale matches the smoothing scale. The limit metric captures the profile of the smoothing.
Since the interface $\Sigma$ is smooth, blowing it up by $M_k \to \infty$ flattens it to a plane $\mathbb{R}^2$.
The limit manifold is $\mathcal{M}_\infty = \mathbb{R} \times \mathbb{R}^2$ with a metric of the form $ds^2 + \gamma(s) \delta_{\mathbb{R}^2}$, where $\gamma(s)$ is the fixed profile of the Miao-Piubello smoothing.
Crucially, the smoothing procedure ensures $\Scal_{\geps} \ge 0$ everywhere. Thus, the limit manifold $\mathcal{M}_\infty$ has non-negative scalar curvature. Furthermore, the metric is quasi-isometric to Euclidean space $\mathbb{R}^3$.

\textbf{Case 3: $L_k \to 0$ (Slow Blow-up).}
This implies $M_k \ll \epsilon_k^{-1}$.
If this were the case, the rescaling would "zoom out" relative to the smoothing, seeing the macroscopic corner singularity.
However, we know from Theorem \ref{thm:Reg_p} that solutions on the singular Lipschitz manifold $(\tM, \tg)$ are $C^{1,\alpha}$. Thus, the gradient is bounded by some constant $C_{sing}$ on the singular limit.
The assumption $M_k \to \infty$ contradicts the $C^{1,\alpha}$ regularity of the limit problem. Thus, this case cannot occur for large $k$.

\textbf{Contradiction via Liouville Theorem.}
In Cases 1 and 2, we obtain a subsequence of $v_k$ converging (in $C^{1,\alpha}_{loc}$) to a limit function $v_\infty$ on a complete manifold $\mathcal{M}_\infty$ (either $\mathbb{R}^3$ or the scalar-flat slab).
The limit function $v_\infty$ is:
1. Globally bounded (since $u_k \in [0,1]$).
2. $p_0$-harmonic.
3. Non-constant ($|\nabla v_\infty(0)|=1$).

For Case 1 ($\mathbb{R}^3$), the Liouville theorem for bounded $p$-harmonic functions implies $v_\infty$ must be constant.
For Case 2, the manifold $\mathcal{M}_\infty$ is quasi-isometric to $\mathbb{R}^3$. A generalized Liouville theorem for such spaces (e.g., via Moser iteration and Nash-De Giorgi estimates for quasi-Euclidean geometries) enforces constancy for bounded harmonic functions.
In both valid blow-up scenarios, we deduce $v_\infty$ is constant, contradicting $|\nabla v_\infty(0)|=1$.
Thus, the gradient $M_k$ must be uniformly bounded.
\end{proof}
