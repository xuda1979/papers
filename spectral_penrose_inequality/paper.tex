...
\end{enumerate}
    ...
    \[ q = O(t^{-2}). \]
    \textit{Proof Sketch:} The linear growth $Ct$ corresponds to the kernel of the operator. The next order term in the expansion of the GJE on the cylinder is determined by the interaction of the kernel with the quadratic non-linearities. For 3D gravity, the non-linear terms induce a decay of $t^{-2}$, not $t^{-1}$. This faster decay ensures all flux integrals in the Bray-Khuri identity vanish identically.
