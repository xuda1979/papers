...
\[ Y = 4(\phi-1)\phi^{-2}\nabla\phi - (\phi-1)^2\phi^{-1} q. \]
The terms involve $\phi$, $\nabla \phi$, and $q$.
\begin{itemize}
    \item The conformal factor $\phi$ and its gradient $\nabla \phi$ are continuous across $\Sigma$ (in fact $\phi \in C^{1,\alpha}$), as established in Lemma~\ref{lem:InterfaceRegularity}.
    \item The vector field $q$ is continuous across $\Sigma$ by the geometric matching conditions of the GJE (Corollary~\ref{cor:MetricAsymptotics}).
\end{itemize}
Therefore, $Y$ is continuous across the interface, and the flux jump vanishes: $\Jump{\langle Y, \nu \rangle} = 0$.
