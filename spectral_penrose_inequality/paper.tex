...
To control the rate at which $\phi$ vanishes at a bubble singularity $p_k$, we construct a local barrier function. Near the bubble, the Jang manifold is asymptotic to a cylinder $\R^+_t \times S^2$ with metric $\bg \approx dt^2 + g_{S^2}$. The model operator for the Laplacian is therefore $\Delta_{\bg} \approx \partial_t^2 + \Delta_{S^2}$. For a radially symmetric function $u(t)$, this simplifies to $\Delta_{\bg}u \approx \partial_t^2 u$.
We seek a local subsolution $\phi_{sub}$ in a neighborhood $U = \{x \mid t(x) > T\}$ of the singularity (where $t \to \infty$ corresponds to the bubble) such that $L(\phi_{sub}) \ge 0$. To achieve a regular conical metric, the conformal factor must vanish at the appropriate rate. In the cylindrical coordinate $t$, this corresponds to exponential decay. We choose the ansatz $\phi_{sub}(t) = c e^{-\gamma t}$ for a positive constant $\gamma$.
Applying the operator $L = \Delta_{\bg} - V$ to this ansatz:
\[ L(\phi_{sub}) = \Delta_{\bg}(c e^{-\gamma t}) - V(t) (c e^{-\gamma t}) \approx c\gamma^2 e^{-\gamma t} - V(t) c e^{-\gamma t} = c e^{-\gamma t} (\gamma^2 - V(t)). \]
The potential $V = \mathcal{S}/8$ is bounded and non-negative near the singularity. To create a conical singularity of the correct type, the conformal factor must scale like the square root of the geodesic distance from the tip of the cone. In the cylindrical coordinate, this corresponds to $\gamma=1/2$. We set our barrier exponent to $\gamma=1/2$. The operator on the subsolution becomes:
\[ L(\phi_{sub}) \approx c e^{-t/2} \left( \frac{1}{4} - V(t) \right). \]
Since the potential $V(t)$ is bounded and arises from the regular part of the Jang scalar curvature, we can choose the neighborhood $U$ (i.e., choose $T$ large enough) such that $V(t) < 1/4$ for all $t > T$. This makes the term in the parenthesis positive, ensuring $L(\phi_{sub}) \ge 0$ inside $U$. Thus, $\phi_{sub}$ is a valid subsolution.

Now, consider the function $w = \phi - \phi_{sub}$ on the domain $U$. We have $L(w) = L(\phi) - L(\phi_{sub}) = 0 - L(\phi_{sub}) \le 0$.
On the boundary $\partial U = \{x \mid t(x) = T\}$, the solution $\phi$ is strictly positive and bounded away from zero, say $\phi(T, y) \ge m > 0$. We can choose the constant $c$ in our subsolution $\phi_{sub}(t) = c e^{-t/2}$ small enough such that $c e^{-T/2} < m$. This ensures that on the boundary, $w = \phi - \phi_{sub} > 0$.
The comparison principle for elliptic operators states that a function $w$ satisfying $Lw \le 0$ in a domain and $w > 0$ on the boundary must be non-negative throughout the domain. Therefore, $w \ge 0$ throughout the neighborhood $U$.
This gives us the desired lower bound: $\phi(t,y) \ge \phi_{sub}(t) = c e^{-t/2}$ for all points in $U$. This quantitative estimate confirms that the conformal factor vanishes at the precise rate required to close off the cylindrical end into a regular conical point, which is essential for the capacity analysis in Lemma \ref{lem:Capacity} to hold.
\end{proof}
