...
A naïve heuristic based on viewing $\Div_{\bg}(q)$ as an $L^1$ source for a Poisson equation on $\mathbb{R}^3$ would suggest the stricter requirement $\tau>1$, since the volume element is $r^2dr$ and
\[
...
Let $(M, g, k)$ be a complete, 3-dimensional, asymptotically flat initial data set satisfying the dominant energy condition ($\mu \ge |J|_g$). Let $\Sigma \subset M$ be the outermost apparent horizon, assumed to be compact, with total area $A$. Assume moreover that any Jang bubbles arising in the generalized Jang construction have spherical boundary components, as in Assumption~\ref{assump:BubbleTopology}. \textbf{Assuming further that the associated Jang manifold satisfies the Spectral Condition (Definition~\ref{def:SpectralCondition})}, the ADM mass satisfies:
\begin{equation}
