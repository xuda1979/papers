...
The scalar curvature of $\tg$ is given by the conformal transformation law $\Rtg = \phi^{-5}(-8\Delta_{\bg}\phi + \Rg \phi)$.
Recall from Lemma \ref{lem:JangScalar} that $\Rg = 16\pi(\mu - J(v)) + |h-k|_{\bg}^2 + 2|X|_{\bg}^2 - 2 \, \Div_{\bg}(X)$.
By substituting the PDE \eqref{eq:BK_PDE}, specifically $-8\Delta_{\bg}\phi = - \Rg^{reg}\phi + 2\Div_{\bg}(X)\phi$, we obtain:
\begin{align*}
    \phi^5 \Rtg &= \left( - \Rg^{reg} + 2\Div_{\bg}(X) \right) \phi + \Rg \phi \\
    &= \left( - \Rg^{reg} + 2\Div_{\bg}(X) + \Rg^{reg} - 2\Div_{\bg}(X) + 2|X|_{\bg}^2 + \dots \right) \phi \\
    &= \left( 16\pi(\mu - J(v)) + |h - k|_{\bg}^2 + 2|X|_{\bg}^2 \right) \phi.
\end{align*}
Since the initial data satisfies the Dominant Energy Condition ($\mu \ge |J|$), the term $\mu - J(v) \ge 0$.
Thus, we explicitly obtain:
\[ \Rtg = \phi^{-4} \left( 16\pi(\mu - J(v)) + |h-k|_{\bg}^2 + 2|X|_{\bg}^2 \right) \ge 0. \]
...
\[ A_{\tg}(\Sigma') = \lim_{t \to \infty} \int_{\Sigma_t} \phi^4 \, d\sigma_{\bg} = A_{\bg}(\Sigma) = A(\Sigma). \]
