...
\end{equation}
where $Q$ is a term involving derivatives of the mollifier and the metric components. This term contains the negative part $R^-$ that must be controlled.

% The user will provide the explicit calculation for Q and the resulting L^p bounds on R^- here.
...
\end{lemma}
\begin{proof}
The proof relies on a local application of the maximum principle to a specific auxiliary function involving the gradient of the potential. This is a standard technique in the regularity theory for quasilinear elliptic equations, often attributed to De Giorgi and Moser.

\textbf{1. Auxiliary Function and Differential Inequality.}
Let $w = |\nabla u_{p, \epsilon}|^2$. A direct but lengthy computation, similar to the derivation of the Bochner identity, shows that $w$ satisfies a differential inequality of the form:
\begin{equation}\label{eq:grad_inequality}
    \Delta_{p, \epsilon} w \le C_1(p) w + C_2(p) |\Ric_{\geps}| w,
\end{equation}
where $\Delta_{p, \epsilon}$ is the appropriate quasilinear operator and the constants depend on $p$ but are uniformly bounded for $p$ in a compact interval like $[1.1, 2]$. The Ricci curvature term $\Ric_{\geps}$ arises from commuting covariant derivatives.

\textbf{2. Curvature Control.}
From the smoothing procedure in Theorem \ref{thm:MiaoPiubelloSmoothing}, we have control over the geometry of $(\tM, \geps)$. The Ricci tensor of the smoothed metric, $\Ric_{\geps}$, is uniformly bounded. This is because the smoothing only occurs in a small collar, and the conformal correction factor $u_\epsilon$ converges uniformly to 1. Therefore, there exists a constant $K$ independent of $\epsilon$ such that $|\Ric_{\geps}| \le K$. The inequality simplifies to $\Delta_{p, \epsilon} w \le C_3 w$.

\textbf{3. Maximum Principle Argument.}
Let $x_0$ be a point where $w$ achieves its maximum value. If $x_0$ is at infinity or on the horizon boundary, the gradient is controlled by the boundary conditions (it asymptotes to a constant value related to the capacitary potential). The concern is an interior maximum.
At an interior maximum point $x_0$, we must have $\nabla w(x_0) = 0$ and $\Delta_{p, \epsilon} w(x_0) \le 0$.
The differential inequality $\Delta_{p, \epsilon} w \le C_3 w$ at $x_0$ implies $0 \ge \Delta_{p, \epsilon} w(x_0)$. This does not immediately lead to a contradiction. However, a more refined version of the maximum principle for elliptic operators (the Hopf maximum principle) can be applied.

A simpler approach is to use a global Moser iteration argument. Let $v = |\nabla u_{p, \epsilon}|$. The Caccioppoli inequality, derived by testing the weak form of the $p$-Laplacian with a test function $\eta^p v$, where $\eta$ is a cutoff function, yields a reverse Hölder-type inequality. A standard iteration scheme on this inequality over concentric balls shows that the $L^\infty$ norm of $v$ is controlled by its $L^p$ norm over a slightly larger domain.

Given that our domain is the entire manifold, the global version of this estimate relies on the uniform geometry. Since the manifold $(\tM, \geps)$ is uniformly equivalent to the target manifold $(\tM, \tg)$ (which is asymptotically flat and has bounded geometry), the constants in the Sobolev and Poincaré inequalities are uniform in $\epsilon$. The Moser iteration procedure then yields a bound of the form:
\[ \sup_{\tM} |\nabla u_{p, \epsilon}| \le C_{geom} \left( \int_{\tM} |\nabla u_{p, \epsilon}|^p dV_{\geps} \right)^{1/p}. \]
The integral on the right is the total $p$-energy, which is bounded by the energy of the linear potential ($p=2$) due to the variational definition of the solution. This energy is finite and uniformly controlled because the asymptotic geometry is fixed. Therefore, we obtain a uniform upper bound $C_{grad}$ that depends only on the geometry of the target manifold, not on the specific values of $p$ or $\epsilon$.
\end{proof}
...
\begin{enumerate}
    ...
    \item By Lemma \ref{lem:UniformGradientControl}, the gradient term $|\nabla u_{p, \epsilon}|$ is uniformly bounded above by a constant $C_3$, independent of $p$ and $\epsilon$. Thus, $|\nabla u_{p, \epsilon}|^{p-2}$ is also uniformly bounded for $p \in (1, 2]$.
\end{enumerate}
Combining these uniform bounds, we can pull a single constant $C_{bound}$ out of the integral:
\begin{align*}
    |\mathcal{E}(\epsilon, p)| &\le \frac{1}{2} \int_{N_\epsilon} (C_1 \cdot C_2 \cdot C_3^{p-2}) |R^-_\epsilon| \, dV_{\geps} \\
    &\le C_{bound} \int_{N_\epsilon} |R^-_\epsilon| \, dV_{\geps} = C_{bound} \|R^-_\epsilon\|_{L^1(N_\epsilon, \geps)}.
\end{align*}
From the analysis of the Miao-Piubello smoothing (Theorem \ref{thm:MiaoPiubelloSmoothing}), the negative scalar curvature satisfies the crucial estimate $\|R^-_\epsilon\|_{L^1} \le C_4 \epsilon^{1/2}$. Substituting this gives the final bound on the deficit:
...
\end{lemma}
\begin{proof}
The mass difference between the initial metric and the Jang metric is given by the integral of the non-negative quantity $\mathcal{S}$ from the scalar curvature identity (Lemma \ref{lem:JangScalar}):
\[ M_{\ADM}(g) - M_{\ADM}(\bg) = \frac{1}{16\pi} \int_{\bM} \mathcal{S} \, dV_{\bg} = \int_{\bM} \left( (\mu - J(n)) + \frac{1}{16\pi}|h-k|_{\bg}^2 + \frac{1}{8\pi}|q|_{\bg}^2 \right) dV_{\bg}. \]
By assumption, the left-hand side is zero. Since the integrand is a sum of non-negative terms (due to the Dominant Energy Condition), the entire integrand must vanish identically. This forces each term to be zero pointwise:
...
\end{enumerate}
Since $(\bM, \bg)$ is the Schwarzschild metric, which is a totally geodesic (and thus time-symmetric) slice of the full spacetime, its extrinsic curvature is zero. Therefore, $k$ for this geometry is identically zero.
The condition $h=k$ now implies that $h \equiv 0$.

The tensor $h$ is the second fundamental form of the Jang graph $t=f(x)$ embedded in the product spacetime $(M \times \R, g - dt^2)$. The condition $h \equiv 0$ means the graph is a totally geodesic submanifold. For a graph of a function, this is only possible if the function is constant, $f(x) = C$.
The Jang solution must satisfy the asymptotic boundary condition $f(x) \to 0$ as $|x| \to \infty$. This forces the constant to be zero, so $f(x) \equiv 0$.
