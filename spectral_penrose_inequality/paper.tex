...
The Fredholm theory for elliptic operators on manifolds with cylindrical ends is governed by the indicial roots of $L$, which are complex numbers $\lambda$ for which the equation $L_\infty u = 0$ admits solutions of the form $u(t,y) = e^{\lambda t} \psi(y)$ for some non-trivial function $\psi(y)$ on the cross-section $\Sigma$. Substituting this ansatz into $L_\infty u = 0$ yields:
...
Our goal is to find a unique solution $\phi_0$ to $L(\phi_0) = -S_{source}$ that decays to zero, so we can define the final solution as $\phi = 1 + \phi_0$. A decaying solution requires a negative weight, $\delta < 0$. The trivial solution corresponds to the indicial root $\lambda=0$, associated with the eigenvalue $\mu_0$. The stability condition $\mu_0 \ge 0$ ensures there are no imaginary indicial roots. If we assume strict stability, $\mu_0 > 0$, then there is a spectral gap $(-\sqrt{\mu_0}, \sqrt{\mu_0})$ around zero.
...
\begin{enumerate}
    \item \textbf{Kernel is Trivial:} The choice $\delta < 0$ means we are seeking solutions in a function space where all functions decay like $e^{\delta t}$. Since all growing indicial roots are positive ($\sqrt{\mu_j} \ge 0$), no non-trivial solution to the homogeneous equation $Lu=0$ can exist in this space. This guarantees that $\ker(L) = \{0\}$.
    \item \textbf{Co-kernel is Trivial:} Since $L$ is a self-adjoint operator, its index is zero. Therefore, $\dim(\ker(L)) = \dim(\text{coker}(L))$. As the kernel is trivial, the co-kernel must also be trivial, which means the operator is surjective.
\end{enumerate}
With $\delta$ chosen in the spectral gap, the operator $L$ is an isomorphism from its domain to its range. By the Fredholm alternative, this guarantees that for any suitable source term on the right-hand side of the Lichnerowicz equation, there exists a unique solution in the weighted space $\EdgeSpace{2}{\delta}$. This solution has the precise exponential decay required, allowing us to define the physical solution $\phi = 1 + \phi_0$ which correctly asymptotes to $1$. This rigorous functional analytic setup is the foundation for constructing the scalar-flat metric $\tg$.
\end{lemma}
