...
The condition $q=0$ has a profound implication. From the definition of $q_i = \frac{\nabla^j f}{\sqrt{1+|\nabla f|^2}} (h_{ij} - k_{ij})$, the vanishing of $q$ implies specific relations between the second fundamental form $h$ and $k$.
More structurally, the equality case implies that $(\bM, \bg)$ corresponds to a static spacetime. The metric $g_{static} = -V^2 dt^2 + \bg$ (where $V$ is the lapse function related to the warping factor) satisfies the Static Vacuum Einstein Equations:
\[ \Ric_{\bg} - \frac{\nabla^2 V}{V} = 0, \quad \Delta_{\bg} V = 0. \]
In our context, the lapse $V$ is determined by the geometry (in Schwarzschild, $V = \sqrt{1-2M/r}$).
The vanishing of $q$ implies that the vector field $X = \frac{\nabla f}{\sqrt{1+|\nabla f|^2}}$ satisfies $\Div_{\bg}(X) = 0$ (since $\Div(q)=0$ and other terms vanish). This vector field $X$ is related to the Killing vector field of the static spacetime.
Since $\bg$ is Lipschitz and satisfies these equations in the distributional sense, we invoke the regularity theory for weak solutions of Einstein's equations.
Specifically, since the Ricci tensor is locally bounded (in fact zero for Schwarzschild) and the metric is static, elliptic regularity for the static vacuum system boosts the regularity of the metric from Lipschitz to real analytic (see \cite{anderson2001} for regularity of static metrics). This implies that the metric coordinates can be chosen to be harmonic, in which the metric coefficients are real analytic.
This analytic regularity forces the "corner" at $\Sigma$ to be a removable coordinate singularity. Consequently:
\begin{itemize}
...
Since $U(s)$ has a jump discontinuity (being the derivative of a Lipschitz metric), the term $|U(\tau) - U_\epsilon(s)|^2$ is generally $O(1)$ near the jump. Thus a naive estimate using $\int |\dot{\eta}_\epsilon(t)| dt = O(\epsilon^{-1})$ only gives $|\partial_s E(s)| = O(\epsilon^{-1})$, which is insufficient.
