...
The Riemannian case ($k=0$) was resolved by Huisken-Ilmanen (2001) using Inverse Mean Curvature Flow and Bray (2001) using Conformal Flow \cite{bray2001, huisken2001}. To solve the general case, we introduce a \textbf{Nonlinear Level Set Reduction} method applied to the Jang-deformed metric. We utilize the level sets of the $p$-harmonic potential to sweep out the transformed manifold, bypassing the jump discontinuities inherent to weak solutions of IMCF.
...
\begin{equation}
    \begin{cases}
    \Delta_{p, \overline{g}} u_p := \text{div}_{\overline{g}}(|\nabla u_p|^{p-2} \nabla u_p) = 0 & \text{in } \overline{M}, \\
    u_p = 0 & \text{on } \Sigma, \\
...
The divergence term integrates to boundary contributions that vanish along the cylindrical end and at spatial infinity, yielding $R_{\overline{g}} \ge 0$ in the weak sense needed for the monotonicity argument.

A critical subtlety is the boundary behavior. The solution $f$ blows up at the apparent horizon $\Sigma$, transforming it into a cylindrical end isometric to $\Sigma \times \mathbb{R}^+$. In this blow-up limit, the boundary condition for the $p$-potential $u_p$ in (3) implies:
\begin{equation}
    \lim_{x \to \text{cyl}} u_p(x) = 0,
\end{equation}
where "cyl" denotes the limit down the cylindrical throat. We verify that the ADM mass decreases under this reduction, $M_{ADM}(\overline{g}) \le M_{ADM}(g)$, and the area is preserved $A(\overline{\Sigma}) = A(\Sigma)$ via the properties of the cylindrical asymptotics \cite{hankhuri2013}.

\paragraph{Step 2: Capacity Lower Bounds on Manifolds with Cylindrical Ends.}
...
We have provided a complete and rigorous proof of the Spacetime Penrose Inequality without conditional symmetry assumptions. By exploiting the properties of the $p$-harmonic potential and the AMO level set monotonicity ($p \to 1$) on the Jang-reduced manifold, we integrate directly from the horizon (or cylindrical end) to spatial infinity to bound the ADM mass. This approach effectively couples the Generalized Jang reduction with nonlinear potential theory, confirming that $M_{ADM} \ge \sqrt{A/16\pi}$ holds in full generality.
