...
Throughout the paper we work in dimension $n=3$ and with initial data sets $(M,g,k)$ satisfying the following standing hypotheses. The manifold $M$ is complete and has a single asymptotically flat end with decay rate $\tau>1/2$, as in Definition~\ref{def:AF} below. The matter fields obey the Dominant Energy Condition ($\mu \ge |J|_g$). The boundary of the trapped region is an outermost MOTS $\Sigma\subset M$, which we assume to be compact. By established theorems on the topology of MOTS (see Proposition~\ref{prop:BubbleTopology}), any Jang bubbles arising from the generalized Jang equation necessarily have spherical boundary components. Under these assumptions we prove the spacetime Penrose inequality stated in Theorem~\ref{thm:SPI}.
...
The analysis in \cite{hankhuri2013} establishes the existence and regularity by applying the Implicit Function Theorem in appropriately weighted Hölder spaces. The decay rate $\beta$ is determined by the indicial roots of the linearized operator, which correspond to the spectrum of the MOTS stability operator $L_\Sigma$. The stability assumption ensures a spectral gap, guaranteeing exponential convergence to the leading order solution.
\end{proof}
...
\end{corollary}
\begin{proof}
The exponential decay of $|\bg - \bg_{\infty}|_{\bg}$ follows directly from the expansion of $f$ in Lemma~\ref{lem:SharpAsymptotics}. This exponential convergence is precisely the condition required by the Lockhart-McOwen theory \cite{lockhartmccowen1985} for the Fredholm analysis in Section 4.2.
