...
The decay is polynomial: $|v(t,y)| \le C t^{-k}$ for some $k \ge 1$.
While slower, this decay ensures that the metric coefficients $\bg_{ij}$ converge to the cylindrical limit $\bg_{\infty}$ as $t \to \infty$. The difference $E = L - L_\infty$ consists of terms decaying polynomially. Since the Fredholm property for operators on ends relies on the invertibility of the asymptotic symbol and the compactness of the error term (relative to the weighted spaces), the polynomial decay is sufficient. Specifically, the multiplication operator by a term decaying as $t^{-k}$ is compact between suitable weighted Sobolev spaces $W^{k,p}_{\gamma} \to W^{k-2,p}_{\gamma}$. Thus, the Fredholm analysis in Section 4.2 holds in both cases.
\end{lemma}
\begin{proof}
...
\begin{lemma}[Stability of the Minimal Surface Area]\label{lem:AreaSemicontinuity}
...
\end{lemma}
\begin{proof}
The proof relies on establishing a ``No-Jump'' condition by constructing a mean-convex foliation in the smoothing region. This rules out the formation of ``small'' minimal surfaces inside the collar $N_{2\epsilon}$ that could lower the area discontinuously.

\textbf{1. Construction of the Foliation.}
In the collar $N_{2\epsilon} \cong \Sigma \times (-2\epsilon, 2\epsilon)$, the metric is given by $\hat{g}_\epsilon = ds^2 + \gamma_\epsilon(s,y)$. The surfaces $\Sigma_s = \{s\} \times \Sigma$ form a foliation. We compute their mean curvature $H_\epsilon(s)$ with respect to the smoothed metric.
Recall that $\gamma_\epsilon = \eta_\epsilon * g_s$. The mean curvature is roughly the trace of the derivative, so $H_\epsilon(s) \approx \eta_\epsilon * H_g(s)$.
The original mean curvature $H_g(s)$ has a jump at $s=0$: $H_g(0^-) = 0$ (cylindrical end) and $H_g(0^+) = H_{\Sigma}^{\bg} \ge 0$ (stability).
Specifically, for $s<0$, $H_g(s) \approx 0$ (up to exponential decay). For $s>0$, $H_g(s) = H_{\Sigma} + O(s)$.
The smoothed mean curvature $H_\epsilon(s)$ transitions from 0 to $H_\Sigma$. Crucially, since $H_\Sigma \ge 0$ (and we can deform to strict positivity if $\lambda_1 > 0$), we can ensure $H_\epsilon(s) \ge 0$ everywhere in the collar, and strictly positive $H_\epsilon(s) > 0$ if we perturb the initial data slightly to be strictly stable (which is a valid density argument).
Alternatively, we construct an explicit foliation of surfaces $\Sigma_{s, \delta}$ with strictly positive mean curvature in the collar by solving the prescribed mean curvature equation with a small parameter, using the non-negative scalar curvature as a helping term.

\textbf{2. Maximum Principle and Trapping.}
Let $\Sigma_{\min, \epsilon}$ be the outermost minimal surface. Suppose $\Sigma_{\min, \epsilon}$ enters the collar $N_{2\epsilon}$.
Since the foliation leaves $\Sigma_s$ have $H_{\Sigma_s} \ge 0$ (and can be made $>0$ by the strict stability perturbation), the Maximum Principle for minimal surfaces implies that $\Sigma_{\min, \epsilon}$ cannot touch a leaf $\Sigma_s$ from the inside unless it coincides with it.
However, $\Sigma_{\min, \epsilon}$ is minimal ($H=0$), while leaves have $H \ge 0$. If we assume strict stability, $H > 0$, so no contact is possible.
Therefore, $\Sigma_{\min, \epsilon}$ must either lie entirely outside the collar (in the bulk or deep in the cylinder) or converge to the interface $\Sigma$ as $\epsilon \to 0$.

\textbf{3. Area Lower Bound.}
If $\Sigma_{\min, \epsilon}$ stays in the bulk, its area converges to $A(\Sigma)$ or greater by the lower semicontinuity of area under $C^0$ metric convergence.
If it moves into the cylinder (which is isometric to $\Sigma \times \R^+$ with $R \ge 0$), the monotonicity of the Hawking mass (or simple area monotonicity for non-negative scalar curvature cylinders) implies $A(\Sigma_{\min, \epsilon}) \ge A(\Sigma)$.
The ``No-Jump'' lemma thus ensures:
\[ \liminf_{\epsilon \to 0} A_{\geps}(\Sigma_{\min, \epsilon}) \ge A_{\tg}(\Sigma). \]
\end{proof}
...
\begin{itemize}
    ...
    This is established using a \textbf{Barrier Argument}. The cylinder $\Sigma \times [0, L]$ in the Jang manifold acts as a barrier for the minimal surfaces in the smoothed metrics. Because the scalar curvature is non-negative and the boundary mean curvature has the right sign (convex), the minimal surfaces are "trapped" and cannot shrink below the area of the bottleneck $\Sigma$.
\end{itemize}
