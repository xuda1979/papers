...
\subsection{Function Spaces and Analytic Framework}
To treat the non-compact asymptotic ends and the singular boundaries rigorously, we employ weighted H"older spaces. Let $\rho(x) = (1+|x|^2)^{1/2}$ be a weight function. For $k \in \N, \alpha \in (0,1)$, and decay rate $\tau \in \R$, the space $C^{k,\alpha}_{-\tau}(M)$ consists of functions $u$ such that:
\begin{equation}
    \|u\|_{C^{k,\alpha}_{-\tau}} := \sum_{j=0}^k \sup_{M} \rho^{\tau+j} |\nabla^j u| + \sup_{x \ne y} \rho(x)^{\tau+k+\alpha} \frac{|\nabla^k u(x) - \nabla^k u(y)|}{|x-y|^\alpha} < \infty.
\end{equation}
These spaces are essential for the Fredholm alternative arguments required for the surjectivity of the linearized operators in the Jang and Lichnerowicz equations.

\begin{definition}[Weak Formulation of $p$-Laplacian]
Given a Riemannian manifold $(\tM, \tg)$ with merely continuous metric components, a function $u \in \Wkp(\tM)$ is weakly $p$-harmonic if for all test functions $\psi \in C^\infty_c(\tM)$:
\begin{equation}
...
Let $(M, g, k)$ be an AF initial data set. We consider the regularized Jang equation with capillarity $\kappa > 0$: $\left( g^{ij} - \frac{f^i f^j}{1+|\nabla f|^2} \right) \left( \frac{\nabla_{ij}f}{\sqrt{1+|\nabla f|^2}} - k_{ij} \right) = \kappa f$. Taking the limit $\kappa \to 0$, there exists a smooth solution $f$ in the region exterior to $\Sigma$, satisfying:
\begin{enumerate}
    ...
    \item \textbf{Asymptotic Flatness:} $f \in C^{2,\alpha}_{-1}(M)$. Consequently, the induced metric $\bg_{ij} = g_{ij} + \partial_i f \partial_j f$ satisfies $\bg_{ij} - \delta_{ij} \in C^{0,\alpha}_{-1}$, ensuring $(\bM, \bg)$ is AF.
    \item \textbf{Bubbles:} $f$ may blow up on intermediate MOTS $\Sigma_{int}$. We denote the collection of these components as $\mathcal{B}$.
\end{enumerate}
\end{theorem}
...
Let $(\bM, \bg)$ be the Jang manifold with regular scalar curvature $\Rg^{reg}$.
We prove the solvability of the second component of \eqref{eq:System}. We seek $\phi > 0$ satisfying
\begin{equation}\label{eq:BK_PDE_Exact}
    \Delta_{\bg} \phi - \frac{1}{8} \Rg^{reg} \phi = - \frac{1}{4} \Div_{\bg}(q) \phi.
\end{equation}
Existence relies on the Fredholm properties of the conformal Laplacian on weighted spaces.

\begin{theorem}[Fredholm Alternative in Weighted Spaces]\label{thm:Fredholm}
Let $L_\bg = \Delta_{\bg} - Q$ with $Q \ge 0$. For weights $\delta \in (-1,0)$ and $p>1$, the operator
\[
    L_\bg : W^{2,p}_\delta(\bM) \to L^p_{\delta-2}(\bM)
\]
is an isomorphism. Under the DEC, $Q \approx \mu - |J|$ is non-negative, guaranteeing invertibility and decay $\phi-1 \in W^{2,p}_\delta$.
\end{theorem}
...
\begin{lemma}[Removable Singularities and Capacity]\label{lem:Capacity}
At the bubble locations $\mathcal{B}$, the metric $\tg$ behaves like a closed cone. Specifically:
\begin{enumerate}
    \item The condition $\phi \sim \sqrt{s}$ closes the infinite cylinder into a point $p_k$.
    \item The metric $\tg$ extends continuously ($C^0$) to $p_k$.
    \item \textbf{Capacity:} For the AMO method, we require that the set $S = \{p_k\}$ does not affect the integration by parts. In 3 dimensions, the $p$-capacity of a point is zero for $1 < p \le 3$. Since we operate with $p \in (1, 3)$, $\Cap_p(\{p_k\}) = 0$.
\end{enumerate}
Thus, $\Delta_p u = 0$ holds in the weak sense on all of $\tM$.
\end{lemma}
...
Following the conformal smoothing method of Miao and Piubello, we solve a Yamabe-type equation with measure data supported on $\Sigma$ to remove the corner while preserving the sign of the scalar curvature. The positivity of $\Jump{H_{\tg}}$ guarantees the maximum principle bounds needed for the conformal factor. Standard elliptic estimates and the definition of the ADM mass show the stated convergence of mass and area.
\end{proof}
...
\begin{enumerate}
    ...
    \item The vanishing of the Jang deformation terms implies the original initial data $(M, g, k)$ embeds isometrically into the static Schwarzschild spacetime.
\end{enumerate}
