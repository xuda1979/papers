...
The relationship between the geometry of this interface and the stability of the MOTS is established through a detailed analysis of the asymptotic behavior of the Capillarity Jang Equation (CJE) solutions near $\Sigma$. This analysis, rigorously carried out in \cite{braykhuri2011} and \cite{hankhuri2013}, utilizes the spectral data of $L_\Sigma$ to construct precise barrier functions.

A fundamental result derived from this asymptotic analysis is the identity relating the mean curvature of the interface in the Jang metric, $H_{\partial\bM}^{\bg}$, to the principal eigenvalue of the stability operator:
\begin{equation}\label{eq:MeanCurvatureIdentity}
    H_{\partial\bM}^{\bg} = \sqrt{\lambda_1(L_\Sigma)}.
\end{equation}
The sign convention is chosen such that positive mean curvature corresponds to a surface that is convex with respect to the bulk region.

Since we assume $\Sigma$ is stable, $\lambda_1(L_\Sigma) \ge 0$. Therefore, the identity \eqref{eq:MeanCurvatureIdentity} immediately implies that $H_{\partial\bM}^{\bg} \ge 0$.
...
\begin{lemma}[Topology of Jang Bubbles]\label{lem:BubbleTopology}
The boundaries of the Jang bubbles, $\partial\mathcal{B}_k$, arising in the context of the Generalized Jang Equation for asymptotically flat initial data, are necessarily topological spheres ($S^2$).
\end{lemma}
\begin{proof}
This is a foundational result concerning the structure of the singular set of the GJE. The topology of the cross-section is determined by the local analysis of the blow-up near the singularity. Detailed arguments establishing that these boundaries must be spherical can be found in the analysis of the GJE (see, e.g., the discussion in \cite{schoen1981}, and the detailed blow-up analysis in \cite{hankhuri2013}).
\end{proof}
