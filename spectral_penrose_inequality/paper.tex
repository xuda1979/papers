...
\begin{proof}
The ADM mass is encoded in the asymptotic behavior $u(x,t) = 1 + \frac{M(t)}{2|x|} + O(|x|^{-2})$.
The scalar curvature $R_{g_t}$ evolves according to a parabolic equation of the form $\partial_t R_{g_t} = \Delta_{g_t} R_{g_t} + \dots$.
By the maximum principle, the condition $R_{g_t} \ge 0$ is preserved for all $t > 0$ provided $R_{g_0} \ge 0$.
Explicit calculation of the mass derivative yields:
\begin{equation}
    \frac{d}{dt} M_{ADM}(g_t) = - \frac{1}{16\pi} \int_M \frac{1}{u} R_{g_t}^2 \, dV_{g_t} - \int_{\Sigma} H_{g_t} \, dA.
\end{equation}
Since $R_{g_0} \ge 0$ implies $\Delta_{g_0} u \ge 0$ (subharmonicity), the maximum principle with boundary condition $u|_{\Sigma}=1$ and $u \to 1$ at infinity ensures $u \le 1$ globally.
Consequently, the normal derivative at the inner boundary satisfies $\partial_\nu u \ge 0$, ensuring the mean curvature $H_{g_t} = u^{-2}(H_{g_0} + 4\partial_\nu \ln u) \ge 0$.
Therefore, the boundary term $-\int H_{g_t} dA$ is non-positive, guaranteeing $\frac{d}{dt} M_{ADM} \le 0$.
\end{proof}
...
\begin{itemize}
    ...
    \item The boundary $\Sigma$ remains mean-convex ($H_{g_\infty} \ge 0$) throughout the flow, preventing the formation of minimal surfaces enclosing $\Sigma$.
\end{itemize}

\paragraph{Step 3: Riemannian Penrose Inequality.}
The limit metric $g_\infty$ is asymptotically flat with zero scalar curvature and a mean-convex boundary.
 We invoke the generalized form of the Riemannian Penrose Inequality for mean-convex boundaries, as established by Huisken and Ilmanen (2001) via Inverse Mean Curvature Flow:
\begin{equation}
    M_{ADM}(g_\infty) \ge \sqrt{\frac{A(\Sigma_{g_\infty})}{16\pi}}.
\end{equation}
Equality holds in the Riemannian Penrose Inequality if and only if $(\overline{M}, g_\infty)$ is the spatial Schwarzschild manifold.

\paragraph{Step 4: Rigidity Analysis.}
Assume equality holds in the Spacetime Penrose Inequality: $M_{ADM}(g) = \sqrt{A/16\pi}$.
This forces equality in all intermediate inequalities:
\begin{enumerate}
    \item $M_{ADM}(g_\infty) = \sqrt{A/16\pi}$: By the rigidity of the RPI, $(M, g_\infty)$ must be isometric to the Schwarzschild 3-manifold.
    \item $M_{ADM}(\overline{g}) = M_{ADM}(g_\infty)$: The monotonicity formula \eqref{eq:mass_mono} implies $\frac{d}{dt}M = 0$, which requires $R_{g_t} \equiv 0$ and $H_{g_t} \equiv 0$ almost everywhere. Thus, the flow is static, and the Jang metric $\overline{g}$ is conformally equivalent to $g_\infty$ with zero scalar curvature; hence $\overline{g}$ is also Schwarzschild.
    \item $M_{ADM}(g) = M_{ADM}(\overline{g})$: Equality in the Inverse Jang reduction implies that the initial data $(M, g, k)$ satisfies the embedding equation into the static spacetime generated by $\overline{g}$. Since $\overline{g}$ is Schwarzschild, $(M, g, k)$ must be a slice of the Schwarzschild spacetime.
\end{enumerate}
This proves the ``if and only if'' condition of the conjecture.
