...
\[ A_{\hat{g}_\epsilon}(u) = A_{\tg}(\Sigma) + \int_\Sigma H_{\tg} u \, d\sigma + \frac{1}{2} \int_\Sigma u L_\Sigma u \, d\sigma + O(\|u\|^3). \]
Since $\Sigma$ is a minimal surface in the Jang metric ($H_{\tg}=0$), the linear term vanishes.
The operator $L_\Sigma$ is the stability operator of $\Sigma$. By the stability hypothesis, $\lambda_1(L_\Sigma) \ge 0$, so $\int u L_\Sigma u \ge 0$.
The error terms in the expansion are controlled by the width of the collar. Since the metric $\hat{g}_\epsilon$ is constructed by convolution, its derivatives (and thus the operator $L_{\hat{g}_\epsilon}$) converge to $L_\Sigma$.
Consequently,
\[ A_{\hat{g}_\epsilon}(\Sigma') \ge A_{\tg}(\Sigma) - C \epsilon^3. \]
This prevents the formation of a "deep well" where area could drop significantly (e.g., to $A(\Sigma)/2$) inside the collar.
\end{proof}
...
In this case, the mean curvature buffer vanishes, and the foliation argument is insufficient. However, we invoke Proposition \ref{prop:SpectralAreaControl}. Since $\Sigma$ is stable ($\lambda_1 \ge 0$), the second variation of area is non-negative.
For sufficiently small $\epsilon$, the metric in the collar is close to the product metric, and the stability operator controls the area of nearby graphs.
Proposition \ref{prop:SpectralAreaControl} ensures that even without a mean-convex foliation, the area cannot drop below $A(\Sigma) - C\epsilon^2$.
