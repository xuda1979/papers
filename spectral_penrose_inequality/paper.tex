...
The decay is polynomial: $|v(t,y)| \le C t^{-k}$. For generic initial data within the marginally stable class, the decay rate satisfies $k \ge 2$.
    While slower than the exponential decay in the strictly stable case, this polynomial decay is sufficient to establish the Fredholm properties of the Lichnerowicz operator using the \textbf{Weighted Scattering Calculus}. The difference $E = L - L_\infty$ consists of terms decaying polynomially as $O(t^{-k})$. While standard Lockhart-McOwen theory requires exponential decay, the Scattering Calculus framework (as developed by Melrose and others) applies to operators with polynomial decay at infinity.

    Specifically, we confirm that the source term $\Div_{\bg}(q)$ resides in the range of the operator.
    \begin{itemize}
        \item \textbf{At the Horizon End:} The source decays as $\Div(q) \sim t^{-k-1}$. The kernel of the operator (corresponding to the indicial root $\lambda=0$) consists of bounded functions (constants). The projection of the source onto the kernel is given by the integral $\int \Div(q) \cdot 1 \, dV$. By the divergence theorem, this equals the boundary flux of $q$, which vanishes since $q \sim t^{-k} \to 0$. For the source to be in the weighted $L^2$ space, we require square integrability: $\int^\infty (t^{-k-1})^2 \, dt < \infty$, which holds for $k > -1/2$. Since $k \ge 2$, this is amply satisfied.
