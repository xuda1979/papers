...
We construct a parabolic \textit{Spectral Conformal Flow} with \textbf{Centroid Stabilization} to enforce convergence.
By suppressing the translational instabilities ($\ell=1$ modes) via a dynamic coordinate adjustment, we prove that the flow actively evolves the geometry toward the Schwarzschild Exterior, establishing the inequality $M_{ADM} \ge M_{Sch}(A)$.
\end{abstract}
...
Pure parabolic flows on non-compact manifolds are susceptible to drift instabilities. To ensure convergence to the static Schwarzschild solution, we include a **Centroid Stabilization** term.
Let $g_t = u(\cdot, t)^4 g_0$. The evolution is given by:

\begin{equation} \label{eq:flow_system}
    \begin{cases}
    \frac{\partial u}{\partial t} = (\phi_t + \mathcal{L}_{\xi_t} u) u & \text{in } M \times (0, \infty), \\
    L_{g_t} \phi_t = 0 & \text{outside } \Sigma_t, \\
    \phi_t(x) = 0 & \text{on } \Sigma_t \text{ and inside trapped regions}, \\
    \phi_t(x) \to -1 & \text{as } |x| \to \infty, \\
    \xi_t = \text{Correction Vector Field}.
    \end{cases}
\end{equation}

The vector field $\xi_t$ is chosen at each step to keep the center of mass of the boundary $\Sigma_t$ fixed at the origin, effectively suppressing the negative modes of the conformal Hessian associated with translations.
...
The stabilized flow prevents the "runaway" of the black hole to infinity.
By the Lojasiewicz-Simon gradient inequality for the mass functional, the bounded trajectory converges to a critical point.
Since the stabilization vector $\xi_t$ eliminates translation-invariant critical points, the unique remaining critical point is the spherically symmetric **Schwarzschild Exterior**.
Thus, the asymptotic mass converges to the Schwarzschild mass:
\begin{equation}
    \lim_{t \to \infty} M(t) = M_{Sch}(A_0) = \sqrt{\frac{A_0}{16\pi}}.
\end{equation}
\end{theorem}
...
\paragraph{Step 1-2: Monotonicity and Convergence.}
...
Applying the Stabilized Spectral Conformal Flow to $\overline{g}$, we obtain a sequence of metrics $g_t$ with constant horizon area and strictly decreasing mass, converging to the Schwarzschild metric $g_{Sch}$.
\begin{equation}
    M_{ADM}(g) \ge M_{ADM}(\overline{g}) \ge \lim_{t \to \infty} M(t) = M_{Sch}(A(\Sigma)).
\end{equation}

\paragraph{Step 3: Final Inequality.}
Substituting the Schwarzschild mass formula:
\begin{equation}
    M_{ADM}(g) \ge \sqrt{\frac{A(\Sigma)}{16\pi}}.
\end{equation}
This completes the proof without reliance on the invalid capacity-area inequality.
