...
\end{enumerate}
    ...
    \textbf{Refined Decay Analysis for the Marginal Case:}
    In the marginal case ($\lambda_1=0$), the decay of the function $f$ is $f(t) = C t + O(t^{-1})$. The vector field $q$ satisfies a stronger decay condition:
    \[ q = O(t^{-2}). \]
    This refined decay rate is derived from the interaction of the kernel with the quadratic non-linearities of the Generalized Jang Equation. Specifically, in 3D gravity, the leading order non-linear terms vanish at order $t^{-1}$, enforcing a decay of $t^{-2}$. This faster decay is critical as it ensures that the vector field $q$ is square-integrable on the cylindrical end ($|q|^2 \sim t^{-4}$) and that the boundary flux integrals in the Bray-Khuri identity vanish identically. We provide the detailed verification of this flux cancellation in Appendix \ref{app:Flux}.
...
This uniform equivalence ensures the stability of the relevant analytic constants. The Sobolev constant $C_S(\hat{g}_\epsilon)$ depends on the isoperimetric profile. As proven in Lemma \ref{lem:UniformSobolev}, the non-collapsing of the horizon throat guarantees that the isoperimetric constant is uniformly bounded from below. Consequently, the Sobolev constant $C_S$ is uniform in $\epsilon$.
...
Let $\Sigma$ be a stable MOTS. For sufficiently small $\epsilon$, the area of the outermost minimal surface $\Sigma_{\min, \epsilon}$ in the smoothed metric $\hat{g}_\epsilon$ satisfies the lower bound:
\begin{equation}
    \liminf_{\epsilon \to 0} A_{\geps}(\Sigma_{\min, \epsilon}) \ge A_{\tg}(\Sigma).
\end{equation}
\end{lemma}
\begin{proof}
The proof employs a \textbf{Quantitative Barrier Argument} combined with the \textbf{Spectral Stability} result (Proposition \ref{prop:SpectralAreaControl}) to rigorously rule out the "Deep Well" scenario (the formation of minimal surfaces with small area within the smoothing region). Additionally, Lemma \ref{lem:AreaExpansion} provides a pointwise guarantee that the metric determinant does not shrink, ensuring that the local area functional satisfies a lower bound independent of the surface's position.

The argument is structured to handle the global topology of the minimal surface $\Sigma'$ in three steps.

\paragraph{Step 1: Geometric Setup in the Smoothing Collar.}
...
\paragraph{Step 2: Quantitative Mean Curvature Lower Bound.}
...
\paragraph{Step 3: Exclusion of Minimal Surfaces via Maximum Principle and Stability.}
...
\paragraph{Case 2: Marginal Stability ($[H] = 0$).}
    In this case, the mean curvature buffer vanishes, and the foliation argument based on strict mean convexity is insufficient. To rule out a "deep well" of area loss, we rely on a direct comparison argument utilizing the pointwise lower bound on the conformal factor established in Lemma \ref{lem:GreenEstimate}.

    We have established that the conformal factor satisfies $u_\epsilon(x) \ge 1$ for all $x \in \tM$. The area functional for a surface $S$ in the conformally transformed metric $\geps = u_\epsilon^4 \hat{g}_\epsilon$ is given by:
    \[ A_{\geps}(S) = \int_S u_\epsilon^4 \, d\sigma_{\hat{g}_\epsilon}. \]
    Since $u_\epsilon \ge 1$, we have the pointwise inequality $A_{\geps}(S) \ge A_{\hat{g}_\epsilon}(S)$ for any surface $S$.
    Consider the outermost minimal surface $\Sigma_{\min, \epsilon}$ in the smoothed metric $\geps$, which is contained in the smoothing collar $N_{2\epsilon}$.
    \[ A_{\geps}(\Sigma_{\min, \epsilon}) \ge A_{\hat{g}_\epsilon}(\Sigma_{\min, \epsilon}). \]
    The metric $\hat{g}_\epsilon$ is an arithmetic smoothing of $\tg$ (which equals $\bg$ in the collar). As shown in Lemma \ref{lem:AreaExpansion}, the smoothing of the metric tensor preserves the area element to leading order: $dA_{\hat{g}_\epsilon} \ge dA_{\bg} - O(\epsilon^2)$. Thus, for any surface $S$ in the collar:
    \[ A_{\hat{g}_\epsilon}(S) \ge A_{\bg}(S) - C\epsilon^2. \]
    Since $\Sigma$ is the outermost MOTS in the original data (and hence in the Jang metric $\bg$), it is a stable minimal surface. Stability implies that $\Sigma$ is a local minimizer of the area functional $A_{\bg}$. Therefore, for any surface $S$ in the collar (which is a small neighborhood of $\Sigma$), we have $A_{\bg}(S) \ge A_{\bg}(\Sigma)$.
    Combining these inequalities:
    \[ A_{\geps}(\Sigma_{\min, \epsilon}) \ge A_{\hat{g}_\epsilon}(\Sigma_{\min, \epsilon}) \ge A_{\bg}(\Sigma_{\min, \epsilon}) - C\epsilon^2 \ge A_{\bg}(\Sigma) - C\epsilon^2. \]
    Taking the limit inferior as $\epsilon \to 0$:
    \[ \liminf_{\epsilon \to 0} A_{\geps}(\Sigma_{\min, \epsilon}) \ge A_{\bg}(\Sigma) = A_{\tg}(\Sigma). \]
    This argument relies only on the sign of the conformal correction ($u_\epsilon \ge 1$) and the stability of the original horizon, avoiding delicate perturbative expansions of the stability operator.

Thus, in both cases, no minimal surface with area significantly less than $A(\Sigma)$ can form in the smoothing region. This establishes $\liminf_{\epsilon \to 0} A(\Sigma_{\min, \epsilon}) \ge A(\Sigma)$.
\end{proof}
...
This cancellation ensures that the error term is bounded by a constant independent of $\epsilon$.
