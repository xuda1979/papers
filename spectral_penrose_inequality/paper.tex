...
We establish the Fredholm property for the Lichnerowicz operator $L = \Delta_{\bg} - \frac{1}{8}\Rg = \Delta_{\bg} - V$ on weighted Sobolev spaces for the different types of cylindrical ends of the Jang manifold $\bM$. Since the metric $\bg$ is merely asymptotically cylindrical (as noted in Remark \ref{rem:AsymptoticCyl}), we invoke the theory of Lockhart-McOwen for elliptic operators on manifolds with ends asymptotic to cylinders. The key result is that the Fredholm properties and indicial roots are determined by the asymptotic model operator, provided the perturbations decay exponentially.

\begin{proof}
The proof proceeds by analyzing the indicial roots of the asymptotic operator at each type of cylindrical end. The Fredholm property of $L$ on a weighted space $\EdgeSpace{2}{\delta}$ is guaranteed if the weight $\delta$ is not an indicial root. The choice of weight then determines the asymptotic behavior of the solution. The exponential convergence of the metric to the cylinder $dt^2 + g_\Sigma$ ensures that the spectrum of the full operator is a compact perturbation of the asymptotic model.
...
On the cylindrical end $\mathcal{T}_\Sigma = \R^+_t \times \Sigma$, the Jang metric and scalar curvature approach a translation-invariant limit: $\bg \to dt^2 + g_\Sigma$ and $\frac{1}{8}\Rg \to V_\infty$ exponentially fast. Consequently, the Lichnerowicz operator $L$ asymptotes to the model cylindrical operator:
...
\end{equation}
\end{lemma}
\begin{proof}[Refined Gradient Estimate via Moser Iteration]
To ensure the constant $C_{grad}$ does not degenerate as $p \to 1$, we employ a Moser iteration technique adapted to the $p$-Laplacian.
Let $w = |\nabla u|^2$. The Bochner identity for the $p$-Laplacian yields an inequality of the form:
\[ L_p(w) \ge -C_1 |\Ric^-| w - C_2 |\nabla u|^2, \]
where $L_p$ is the linearized operator.
Crucially, while pointwise bounds on $\Ric^-$ diverge as $\epsilon \to 0$, we have established that $\| \Ric^-_{\geps} \|_{L^{3/2}} \to 0$.
By applying the Sobolev inequality and iterating the $L^q$ norms of $w$, the final $L^\infty$ bound depends on $\| \Ric^- \|_{L^{n/2}}$ (where $n=3$). Since this norm is controlled uniformly in $\epsilon$, and the coefficients of the iteration remain bounded as $p \to 1$, the maximum principle yields a uniform bound:
\[ \sup |\nabla u_{p, \epsilon}| \le C(\| \Ric^- \|_{L^{3/2}}, \|u\|_\infty) \le C_{uniform}. \]
This avoids the contradiction pitfalls of the blow-up method.
\end{proof}
...
\end{lemma}
\begin{proof}
The perimeter of a set $E$ passing through the vertex is well-defined. We must show that for any sequence $u_p \to \chi_E$ in $L^1$, $\liminf_{p \to 1} \int |\nabla u_p|^p \ge \text{Per}(E)$.
Consider the standard cone metric. The singularity is at $s=0$.
1.  **Metric Structure:** The volume element scales as $s^2$, and the gradient as $\partial_s$. The capacity of the point is zero for $p < 3$.
2.  **Lower Bound:** Let $u_p$ be a recovering sequence. Since the capacity is zero, we can modify $u_p$ in a small neighborhood $B_\delta(0)$ without affecting the energy limit.
3.  **No Ghost Term:** Unlike a cusp where area can accumulate, the cone has finite volume near the tip and the area of cross-sections scales as $s^2 \to 0$. Thus, minimal surfaces cannot "snag" or accumulate area at the vertex to artificially lower their energy. The variational limit of the $p$-energy correctly identifies the perimeter of the level sets in the regular part of the manifold.
\end{proof}
...
\begin{proof}
The proof proceeds in three rigorous steps, relying on the lower semicontinuity of the geometric quantities rather than the continuity of the $p$-harmonic flow itself.

\textbf{Step 1: The Inequality on the Smoothed Manifold.}
...
\end{itemize}
Consequently, the Rigidity and Existence results of the AMO framework (Theorem \ref{thm:AMO} and Proposition \ref{prop:AMO_limits}) apply directly to $(\tM, \geps)$ without modification. The $p$-harmonic level set flow exists, is regular, and the monotonicity formula holds. Taking the limit $p \to 1^+$ for this \textit{fixed} metric yields the Riemannian Penrose Inequality for $\geps$:
\begin{equation}\label{eq:FixedEpsilonRPI}
    M_{\ADM}(\geps) \ge \sqrt{\frac{A_{\geps}(\Sigma)}{16\pi}}.
\end{equation}

\textbf{Step 2: The Limit of the Geometry.}
...
\begin{itemize}
    ...
    \item \textbf{Area Lower Semicontinuity:} By Lemma \ref{lem:AreaSemicontinuity}, the area of the minimal boundary does not increase in the limit (or conversely, the area in the smoothed metric approximates the rough area from above):
    \[ \liminf_{\epsilon \to 0} A_{\geps}(\Sigma) \ge A_{\tg}(\Sigma). \]
\end{itemize}
...
\begin{align*}
    ...
    &\ge \liminf_{\epsilon \to 0} \sqrt{\frac{A_{\geps}(\Sigma)}{16\pi}} \\
    &\ge \sqrt{\frac{A_{\tg}(\Sigma)}{16\pi}}.
\end{align*}
Combining this with the mass reduction property of the Jang map ($M_{\ADM}(g) \ge M_{\ADM}(\tg)$) and the area preservation ($A_{\tg}(\Sigma) = A(\Sigma)$), we obtain:
\[ M_{\ADM}(g) \ge \sqrt{\frac{A(\Sigma)}{16\pi}}. \]
This completes the proof without requiring uniform $C^1$ estimates on the $p$-harmonic potential across the singular transition.
\end{proof}
...
Let $p_k$ be a compactified bubble point. The metric $\tg$ behaves asymptotically as a cone $\tg \sim ds^2 + s^2 g_{S^2}$. Although the Riemann curvature tensor blows up pointwise as $s \to 0$, the Ricci tensor belongs to $L^1_{loc}(\tM)$. Consequently, the distributional Bochner identity does not acquire a singular measure (Dirac mass) at the tip.
\end{lemma}

\begin{proof}
In local geodesic coordinates $(s, \theta)$ near the tip, the metric components behave as $O(1)$. The Riemann curvature tensor $\text{Riem}$ involves second derivatives of the metric. For a cone, the transverse derivatives scale as $1/s^2$. Thus, $|\Ric_{\tg}| \le C/s^2$.
However, the volume form in these coordinates is $d\text{Vol}_{\tg} = \sqrt{\det \tg} \, ds \, d\theta \approx s^2 \, ds \, d\theta_{S^2}$.
We compute the $L^1$ norm of the Ricci curvature over a small ball $B_\delta(p_k)$:
\begin{equation}
    \int_{B_\delta(p_k)} |\Ric_{\tg}| \, d\text{Vol}_{\tg} \approx \int_0^\delta \int_{S^2} \left( \frac{C}{s^2} \right) s^2 \, ds \, d\sigma
    = 4\pi C \int_0^\delta ds = 4\pi C \delta < \infty.
\end{equation}
Since the Ricci tensor is integrable, it does not concentrate mass at the vertex. Combined with the result that $\Cap_p(\{p_k\}) = 0$ for $1 < p < 3$, any test function $\phi$ in the weak Bochner identity can be cut off near $p_k$ with vanishing error:
\[ \left| \int_{B_\delta} \Ric(\nabla u, \nabla u) \phi \right| \le \|\nabla u\|_\infty^2 \|\phi\|_\infty \int_{B_\delta} |\Ric| \to 0 \quad \text{as } \delta \to 0. \]
This proves that the "ghost curvature" at the vertex is zero.
\end{proof}
