...
\textbf{Step 1: Asymptotic Operator at the Horizon.}
On the cylindrical end corresponding to the horizon, $\mathcal{T}_\Sigma = \R^+_t \times \Sigma$, the Jang metric and scalar curvature approach a translation-invariant limit: $\bg \to dt^2 + g_\Sigma$ and $\frac{1}{8}\Rg \to V_\infty$. The Lichnerowicz operator $L$ asymptotes to the model operator:
\[ L_\infty = \frac{\partial^2}{\partial t^2} + \Delta_{g_\Sigma} - V_\infty. \]
We seek solutions of the form $u(t,y) = e^{\lambda t} \psi(y)$, which leads to the eigenvalue problem on the cross-section $\Sigma$:
\begin{equation}\label{eq:indicial_eigenproblem_horizon}
    (-\Delta_{g_\Sigma} + V_\infty) \psi = \lambda^2 \psi.
\end{equation}

\textbf{Step 2: Identification with the MOTS Stability Operator.}
The operator on the left-hand side, $L_\Sigma := -\Delta_{g_\Sigma} + V_\infty$, is precisely the MOTS stability operator. Stability of the outermost MOTS implies that the principal eigenvalue $\mu_0$ of $L_\Sigma$ is non-negative, $\mu_0 \ge 0$. The indicial equation is $\lambda^2 = \mu_j$ for an eigenvalue $\mu_j$ of $L_\Sigma$. The set of indicial roots is therefore $\Ind_{horizon}(L) = \{ \pm\sqrt{\mu_j} \mid \mu_j \in \Spec(L_\Sigma) \}$.
