...
\[ (\eta_\epsilon * J)(s) = J(s) + \frac{1}{2} (\int \tau^2 \eta_\epsilon(\tau)) J''(s) + \dots \]
...
Specifically, since the Ricci tensor is locally bounded (in fact zero for Schwarzschild) and the metric is static, elliptic regularity for the static vacuum system boosts the regularity of the metric from Lipschitz to real analytic (see \cite{anderson2001} for regularity of static metrics).
...
\end{lemma}
\begin{proof}
The flux integral is intimately related to the mass of the Jang manifold. Recall the mass difference formula derived from the scalar curvature identity (Lemma \ref{lem:JangScalar}):
\[ 16\pi (M_{\ADM}(g) - M_{\ADM}(\bg)) = \int_{\bM} \left( 16\pi(\mu-J(n)) + |h-k|^2 + 2|q|^2 \right) dV - 2 \int_{\bM} \Div_{\bg}(q) dV. \]
The integral of the divergence converts to boundary fluxes. At the asymptotically flat infinity, the flux of $q$ vanishes due to the fall-off conditions ($q \sim r^{-\tau-1}$ with $\tau > 1/2$).
The remaining boundary is the cylindrical end (limit $t \to \infty$). Let $\mathcal{F} = \lim_{t \to \infty} \int_{\Sigma_t} \langle q, \nu \rangle$.
The identity becomes:
\[ 16\pi \Delta M = \int_{\bM} (\text{Non-negative Terms}) + 2\mathcal{F}. \]
However, the Jang reduction is constructed variationally to minimize the mass loss. In the limit where the blow-up is exact, the ADM mass of the Jang metric $M_{\ADM}(\bg)$ is well-defined.
Crucially, we utilize the \textbf{Constraint Equations}. The vector field $q$ is defined as $q_i = \frac{f^j}{\sqrt{1+|\nabla f|^2}}(h_{ij} - k_{ij})$.
On the cylinder, $\nabla f$ is parallel to the axis.
The flux $\mathcal{F}$ represents the difference between the Hawking mass of the horizon and the mass at infinity in the cylinder.
For the GJE solution, the specific asymptotic behavior $f \sim \log s$ in the marginal case implies that $q$ behaves like the gradient of a harmonic function.
If the flux $\mathcal{F}$ were non-zero, it would imply a logarithmic divergence in the potential $\phi$ (since $L\phi = \Div q$), which would contradict the boundary condition $\phi \to 1$.
However, the physical constraint is that the Jang surface $\Sigma$ captures the area of the horizon. The vanishing of the flux is a necessary condition for the existence of the cylindrical end with finite geometry.
Explicitly, if $k=1$, $q \sim 1/t$. The area is constant. The flux is $\int O(1/t) dA \sim 1/t \to 0$.
Thus, for any polynomial decay $k \ge 1$, the flux vanishes asymptotically.
\end{proof}
