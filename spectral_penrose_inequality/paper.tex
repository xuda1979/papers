...
\end{equation}
Let $t = -\log s$ be the cylindrical coordinate. The remainder term $v(t,y)$ exhibits exponential decay:
\begin{equation}
    |v(t,y)| + |\nabla v(t,y)| + |\nabla^2 v(t,y)| \le C e^{-\beta t}
\end{equation}
for some $\beta > 0$ determined by the spectral gap of the stability operator on $\Sigma$.
\end{lemma}
\begin{proof}
...
The analysis in \cite{hankhuri2013} establishes the existence and regularity by applying the Implicit Function Theorem in appropriately weighted Hölder spaces. The decay rate $\beta$ is determined by the indicial roots of the linearized operator, which correspond to the spectrum of the MOTS stability operator $L_\Sigma$.

\textbf{Handling Marginal Stability:} If $\Sigma$ is strictly stable ($\lambda_1(L_\Sigma)>0$), there is a spectral gap, guaranteeing exponential convergence ($\beta>0$). If $\Sigma$ is marginally stable ($\lambda_1(L_\Sigma)=0$), the decay may be slower (polynomial). However, the decay rate is still sufficient for the application of the Lockhart-McOwen Fredholm theory (Section 4.2), as the critical indicial roots remain isolated.
\end{proof}
...
The lowest eigenvalue of the operator $-\Delta_{g_{\partial\mathcal{B}}}$ on the compact manifold $\partial\mathcal{B}$ (which is $S^2$ by Proposition~\ref{prop:BubbleTopology}) is $\nu_0=0$, which corresponds to the constant eigenfunction $\psi_0=1$. Substituting this into the eigenvalue problem gives the principal indicial equation:
...
We apply the Divergence Theorem. We must verify that the boundary terms vanish.
\[ \int_\Omega \Div(Y) \, dV_{\bg} = \int_{\partial\Omega} \langle Y, \nu \rangle \, d\sigma_{\bg}. \]
The boundary $\partial\Omega$ consists of interior components where $\phi=1$, and the boundary at infinity.

\textit{Interior Boundary:} On components where $\phi=1$, the factor $(\phi-1)$ ensures $Y=0$.

\textit{Boundary at Infinity (Flux Calculation):} We analyze the asymptotics of $Y$ at the AF end. We use the asymptotics from Theorem~\ref{thm:MassReduction} and Corollary~\ref{cor:MetricAsymptotics}: $\phi-1 = O(r^{-1})$, $\nabla\phi = O(r^{-2})$, and $q=O(r^{-\tau-1})$ ($\tau>1/2$).
\begin{align*}
|Y|_{\bg} &\le C (|\phi-1||\nabla\phi| + (\phi-1)^2|q|) \\
    &\le C (O(r^{-1})\cdot O(r^{-2}) + O(r^{-2})\cdot O(r^{-\tau-1})) = O(r^{-3}).
\end{align*}
The flux over a large coordinate sphere $S_r$ is:
\[ \left|\int_{S_r} \langle Y, \nu \rangle \, d\sigma_{\bg}\right| \le \int_{S_r} |Y|_{\bg} \, d\sigma_{\bg} \approx O(r^{-3}) \cdot \text{Area}(S_r) = O(r^{-1}). \]
As $r\to\infty$, the flux vanishes.
...
\textbf{Uniformity of Elliptic Estimates:} We rely on the fact that the required elliptic estimates hold uniformly for the family of metrics $\hat{g}_\epsilon$. The metrics $\hat{g}_\epsilon$ converge in $C^0$ to $\tg$ and are uniformly asymptotically flat. The geometric quantities controlling the Sobolev embedding $W^{1,2} \hookrightarrow L^6$ (crucial for the borderline $L^{3/2}$ estimate) are stable under this convergence. Thus, the Sobolev constants $C_S(\hat{g}_\epsilon)$ are uniformly bounded.

We can represent the solution using the Green's function $G_\epsilon(x,y)$. The uniform geometric bounds ensure that $G_\epsilon(x,y) \le C_1/d_{\hat{g}_\epsilon}(x,y)$ uniformly in $\epsilon$.
...
\end{theorem}
\begin{proof}
Mosco convergence requires verifying the Liminf Inequality and the existence of a Recovery Sequence. This follows from the controlled convergence of the metrics. Since $\geps \to \tg$ in $C^0$ uniformly on compact sets, and the metrics are uniformly elliptic, the metric tensors and volume forms converge appropriately.

The Recovery Sequence is trivially satisfied by choosing $v_\epsilon = u$, as the Dominated Convergence Theorem ensures $\mathcal{E}_{p,\geps}(u) \to \mathcal{E}_{p,\tg}(u)$. The Liminf Inequality follows from the lower semicontinuity of convex functionals under weak $W^{1,p}$ convergence combined with the convergence of the coefficients and volume forms.
\end{proof}
...
This is the crucial step. By differentiating the regularized equation and using appropriate test functions (a technique known as the Caccioppoli inequality for the derivatives), one can establish a uniform local bound on the second derivatives of the solutions on compact subsets of the \emph{smooth} part of $(\tM,\tg)$. Specifically, if $K\Subset \tM\setminus\{p_k\}$ is a compact set avoiding the conical tips, there exists a constant $C_K$ independent of $\epsilon$ such that:

\textbf{Rigorous Justification (Caccioppoli Estimates):}
We apply the Caccioppoli inequality to the derivatives of $u_\epsilon$. This involves testing the differentiated equation against $\eta^2 \nabla u_\epsilon$, where $\eta$ is a cutoff function. The crucial aspect is that the ellipticity constants of the regularized operator depend on $\epsilon$, but the resulting integral estimates for $\nabla^2 u_\epsilon$ can be shown to be uniform in $\epsilon$ (see e.g., \cite{cheegernabervaltorta2015} for related estimates). This relies on the structure of the $p$-Laplacian and the fact that we are working in a fixed smooth metric $\tg$ (away from $\{p_k\}$).

\begin{equation}
    \| u_\epsilon \|_{W^{2,2}(K)} \le C_K.
\end{equation}
The existence of such estimates follows from standard interior elliptic regularity for uniformly elliptic divergence-form operators with bounded measurable coefficients, applied to the smooth metrics $\geps$ constructed in Section~4.6 and then passing to the limit as $\epsilon\to 0$ on sets away from the singular points. The fact that the conical tips $\{p_k\}$ have zero $p$-capacity (Appendix~\ref{app:Capacity}) ensures that no singular contribution arises in the limit.

This uniform bound allows us to extract a subsequence (which we continue to denote by $u_\epsilon$) that converges weakly in $W^{2,2}_{loc}(\tM\setminus\{p_k\})$ to the original solution $u$. Since the set of tips has zero capacity, this is enough to interpret all distributional identities on the whole of $\tM$, and we may write $u_\epsilon \rightharpoonup u$ in $W^{2,2}_{loc}(\tM)$ by abuse of notation.
