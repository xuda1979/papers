...
\begin{equation}
    |v(s,y)| \le C s^{1/2+\alpha}, \quad |\nabla v(s,y)|_{\bg} \le C s^{-1/2+\alpha}, \quad |\nabla^2 v(s,y)|_{\bg} \le C s^{-3/2+\alpha},
\end{equation}
for some $\alpha > 0$. These estimates are crucial for proving that the Ricci tensor of the conformally sealed metric $\tg = \phi^4\bg$ is bounded and integrable.
\end{lemma}
\begin{proof}
...
\begin{enumerate}
    ...
    where $r=s$ is the radial distance from the tip, and the perturbation tensor satisfies $|h| \le C r^\alpha$. As $s \to 0$, this metric describes an \emph{Asymptotically Conical} (AC) manifold with a singularity at the vertex $p_k$. The metric tensor $\tg$ extends continuously ($C^0$) to the vertex.
\end{enumerate}
