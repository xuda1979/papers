...
The limit value $1/4$ is structural. As the Jang blow-up creates a cylindrical end over the bubble boundary $\partial \mathcal{B} \cong S^2$, the metric approaches the product metric $dt^2 + g_{S^2}$ (up to scaling). The scalar curvature of this limit cylinder is simply the scalar curvature of the sphere, $\Rg \to \Scal_{S^2} = 2$.
Therefore, the potential term in the Lichnerowicz equation converges to $V_\infty = \frac{1}{8}(2) = \frac{1}{4}$. This precise value is what dictates the root $\lambda = 1/2$ and thus the $\phi \sim \sqrt{s}$ decay.
...
\end{equation}
\end{lemma}
\begin{proof}[Proof by Blow-up Analysis]
We proceed by contradiction. Suppose the uniform gradient bound does not hold. Then there exists a sequence of parameters $(p_k, \epsilon_k) \to (p_0, 0)$ (with $p_0 \in [1,2]$) and points $x_k \in \tM$ such that
\[ M_k := |\nabla u_{p_k, \epsilon_k}(x_k)|_{\geps_k} = \sup_{x \in \tM} |\nabla u_{p_k, \epsilon_k}(x)|_{\geps_k} \to \infty. \]
Since $u_{p, \epsilon}$ satisfies Dirichlet boundary conditions on $\partial \tM$ (or decays at infinity), and the boundary data is smooth/bounded, the maximum gradient must occur in the interior or approach the singular set. Given the local regularity away from the corner, $x_k$ must converge to the interface $\Sigma$.

We perform a blow-up analysis around $x_k$. Let $y$ be Fermi coordinates centered at $x_k$, where $y_1$ is the signed distance to $\Sigma$ (or geodesic coordinates if $x_k$ is not near the corner). Define the rescaled coordinates $\tilde{y} = M_k y$.
The rescaled solution is defined by
\[ v_k(\tilde{y}) = u_{p_k, \epsilon_k}(x_k + M_k^{-1} \tilde{y}) - u_{p_k, \epsilon_k}(x_k). \]
This function satisfies:
\begin{enumerate}
    \item $v_k(0) = 0$ and $|\nabla v_k(0)| = 1$.
    \item $|\nabla v_k(\tilde{y})| \le 1$ for all $\tilde{y}$.
    \item $v_k$ is a weak solution to the $p_k$-Laplace equation with respect to the rescaled metric $\tilde{g}_k(\tilde{y}) = \geps_k(x_k + M_k^{-1} \tilde{y})$.
    \item Since $u_k$ is uniformly bounded (between 0 and 1), $v_k$ is uniformly bounded on any compact set in the limit (modulo constants). However, since we are proving a Liouville theorem for gradients, the key property is that $v_k$ is defined on an exhausting sequence of domains converging to $\R^3$.
\end{enumerate}

We analyze the limit of the rescaled metric $\tilde{g}_k$ based on the rate of blow-up relative to the smoothing scale $\epsilon_k$.
Let $L_k = M_k \epsilon_k$.

\textbf{Case 1: $L_k \to \infty$ (Fast Blow-up).}
The blow-up scale $M_k^{-1}$ is much smaller than the smoothing scale $\epsilon_k$. This means we are zooming in on a scale where the smoothing appears infinitely large.
If $x_k$ is within the smoothing collar (distance $\lesssim \epsilon_k$), we are magnifying a microscopic neighborhood of a point inside the smoothed region. Since $\geps$ is smooth, at sufficiently small scales it is Euclidean.
Specifically, the metric components $\geps$ scale as $\delta_{ij} + O(\epsilon_k^{-1} |y|)$.
Rescaled: $\tilde{g}_k(\tilde{y}) \approx I + O(\epsilon_k^{-1}) M_k^{-1} \tilde{y} = I + O(L_k^{-1}) \tilde{y}$.
Since $L_k \to \infty$, the first-order error term vanishes. The metric converges uniformly on compact sets to the flat Euclidean metric on $\mathbb{R}^3$.
(If $x_k$ is far from the corner, it trivially converges to Euclidean space).
The limit function $v_\infty$ is a global solution to the $p_0$-Laplacian on $\R^3$ with bounded gradient ($|\nabla v_\infty| \le 1$). Crucially, since the original $u_k$ is bounded, $v_\infty$ grows at most linearly. However, if we assume $v_\infty$ arises from the blow-up of a bounded function, it must be bounded.
More precisely, Liouville's theorem for $p$-harmonic functions on $\R^3$ states that any solution bounded from one side (or with sufficiently slow growth) is constant. Since $|\nabla v_\infty(0)|=1$, this is a contradiction.

\textbf{Case 2: $L_k \to \lambda \in (0, \infty)$ (Comparable Scales).}
The blow-up scale matches the smoothing scale. The limit metric captures the fixed profile of the smoothing.
Since the interface $\Sigma$ is smooth, blowing it up by $M_k \to \infty$ flattens it to a plane $\mathbb{R}^2$.
The limit manifold is $\mathcal{M}_\infty = \mathbb{R} \times \mathbb{R}^2$ with a metric of the form $ds^2 + \gamma(s) \delta_{\mathbb{R}^2}$, where $\gamma(s)$ is the explicit profile of the Miao-Piubello smoothing of the corner $|s|$.
Crucially, the smoothing procedure ensures $\Scal_{\geps} \ge 0$ everywhere. Thus, the limit manifold $\mathcal{M}_\infty$ has non-negative scalar curvature.
Furthermore, the metric is \textbf{quasi-isometric to Euclidean space} $\mathbb{R}^3$. Specifically, there exist constants $c, C$ such that $c \delta_{ij} \le \tilde{g}_{ij} \le C \delta_{ij}$. This quasi-isometry preserves the fundamental analytic properties required for regularity theory: the Volume Doubling property and the Poincaré inequality.
By the De Giorgi-Nash-Moser theory extended to such spaces (or the specific Liouville theorems for manifolds with non-negative Ricci/Scalar curvature and Euclidean volume growth), any bounded $p$-harmonic function on $\mathcal{M}_\infty$ must be constant.
Since our limit function $v_\infty$ is the limit of bounded functions, it is bounded. Thus $v_\infty$ is constant. But $|\nabla v_\infty(0)|=1$, a contradiction.

\textbf{Case 3: $L_k \to 0$ (Slow Blow-up).}
This implies $M_k \ll \epsilon_k^{-1}$. The rescaling "zooms out" relative to the smoothing, effectively seeing the original macroscopic corner singularity of $(\tM, \tg)$.
The limit metric is the original singular metric $\tg$ (locally a product of a line and a cone).
However, we know from Theorem \ref{thm:Reg_p} that solutions on the singular Lipschitz manifold $(\tM, \tg)$ are locally $C^{1,\alpha}$. This regularity holds even across the Lipschitz interface $\Sigma$ (by Tolksdorf's regularity theory for $p$-harmonic functions on Lipschitz domains).
Therefore, the gradient of $u_{p, \epsilon}$ is bounded by a constant $C_{sing}$ determined by the singular geometry, plus a small error from the smoothing.
The assumption $M_k \to \infty$ contradicts the fact that the gradient is controlled by the $C^{1,\alpha}$ norm of the limit solution. Thus, this case cannot occur for sufficiently large $k$.

\textbf{Conclusion.}
In all valid blow-up scenarios, we derive a contradiction to the assumption $M_k \to \infty$. Thus, the gradient must be uniformly bounded by a constant $C_{grad}$ independent of $\epsilon$ and $p$.
\end{proof}
...
This term quantifies the convergence of the $p$-harmonic level set method to the weak Inverse Mean Curvature Flow (IMCF) as $p \to 1^+$. This is the central convergence result of the Agostiniani-Mazzieri-Oronzio framework \cite{amo2022}. For a fixed, asymptotically flat manifold with non-negative scalar curvature and a $C^0$ metric with removable conical singularities (as established in Section 4), the functional $\mathcal{M}_{p,0}(t)$ converges to the Hawking mass $\mathcal{M}_{1,0}(t)$ of the weak IMCF solution.

The rigorous justification relies on the variational structure of the problem. As $p \to 1^+$, the $p$-Dirichlet energy functional $\mathcal{E}_p(u) = \int |\nabla u|^p$ Gamma-converges to the Total Variation functional $\mathcal{TV}(u) = \int |\nabla u|$, which governs the perimeter minimization of the IMCF. The coercivity of the energy functionals ensures that the sequence of minimizers (the $p$-harmonic potentials) converges to a minimizer of the limit functional (the IMCF potential). Furthermore, explicit quantitative stability estimates for Gamma-convergent sequences of convex functionals allow us to bound the rate of convergence. Specifically, there exists a convergence rate $\alpha > 0$ (typically $\alpha = 1/2$ or related to the Hölder exponent of the solution) such that:
\[ \left| \mathcal{M}_{p, 0}(t) - \mathcal{M}_{1, 0}(t) \right| \le C_4 (p-1)^\alpha. \]
This bound is uniform for $t \in [0,1)$ due to the overall monotonicity of the functional and the uniform compactness of the level sets.
...
Armed with the uniform gradient bound, we now bound the deficit term by converting the integral over level sets into a volume integral using the coarea formula. Recall that the coarea formula relates the integral over level sets to the volume integral via the factor $|\nabla u|$: $dt \, d\sigma_t = |\nabla u| \, dV$.
\begin{align*}
    \mathcal{E}(\epsilon, p) &= \frac{1}{2} \left| \int_0^1 \left( \int_{\Sigma_t} \mathcal{A}_{p,\epsilon}(t) u_\epsilon^{-4} R^-_\epsilon |\nabla u_{p, \epsilon}|^{p-1} \, d\sigma_\epsilon \right) dt \right| \\
    &= \frac{1}{2} \left| \int_{\tM} \mathcal{A}_{p,\epsilon}(u(x)) u_\epsilon(x)^{-4} R^-_\epsilon(x) |\nabla u_{p, \epsilon}(x)|^{p-1} |\nabla u_{p, \epsilon}(x)| dV_{\geps} \right| \\
    &= \frac{1}{2} \left| \int_{\tM} \mathcal{A}_{p,\epsilon}(u(x)) u_\epsilon(x)^{-4} R^-_\epsilon(x) |\nabla u_{p, \epsilon}(x)|^{p} dV_{\geps} \right|.
\end{align*}
...
\begin{enumerate}
    ...
    \item By the crucial uniform gradient estimate established above (Lemma \ref{lem:UniformGradientControl}), the term $|\nabla u_{p, \epsilon}|^{p}$ is uniformly bounded for $p \in (1, 2]$. Specifically, $|\nabla u|^p \le (C_{grad})^p \le \max(1, C_{grad}^2)$. This justifies the necessity of the gradient bound: it ensures the energy density in the deficit term does not blow up.
\end{enumerate}
Combining these uniform bounds, we can pull a single constant $C_{bound}$ out of the integral:
\begin{align*}
    |\mathcal{E}(\epsilon, p)| &\le \frac{1}{2} \int_{N_\epsilon} (C_1 \cdot C_2 \cdot C_{grad}^{p}) |R^-_\epsilon| \, dV_{\geps} \\
...
This appendix provides a detailed computational justification for the bounds on the negative part of the scalar curvature ($R^-_\epsilon$) of the smoothed metric $\hat{g}_\epsilon$. We derive the explicit deficit term and prove its boundedness, drawing on the analysis of Miao and Piubello for manifolds with corners.

\paragraph{1. The Scalar Curvature Deficit.}
As stated in Section 4.1, the scalar curvature of the smoothed metric in the collar $N_{2\epsilon}$ is given by the Gauss-Codazzi relation for the foliation by level sets of the distance function $s$:
\begin{equation}
    R_{\hat{g}_\epsilon} = R^{\Sigma_s}(\gamma_\epsilon) - |\hat{k}_\epsilon|_{\gamma_\epsilon}^2 - (\Tr_{\gamma_\epsilon} \hat{k}_\epsilon)^2 - 2 \partial_s (\Tr_{\gamma_\epsilon} \hat{k}_\epsilon),
\end{equation}
where $\gamma_\epsilon = \eta_\epsilon * g_s$ is the mollified tangential metric and $\hat{k}_\epsilon = \frac{1}{2} \gamma_\epsilon^{-1} \partial_s \gamma_\epsilon$ is its second fundamental form.
The last term, $-2 \partial_s (\Tr \hat{k}_\epsilon)$, captures the smoothing of the distributional curvature of the original metric, which contains a Dirac delta measure supported on $\Sigma$ with weight $2[H] \ge 0$. The mollification spreads this positive measure into a large positive function of order $O(1/\epsilon)$.

The "negative dip" in the scalar curvature arises from the mismatch between the smoothed quadratic terms and the quadratic terms of the smoothed metric. Specifically, we define the deficit $\mathcal{D}(s)$ as:
\[ \mathcal{D}(s) = \left( -|\hat{k}_\epsilon|^2 - (\Tr \hat{k}_\epsilon)^2 \right) - \eta_\epsilon * \left( -|k|^2 - (\Tr k)^2 \right). \]
Using Jensen's inequality (viewing convolution as an expectation), one can show that $\eta_\epsilon * |k|^2 \ge |\hat{k}_\epsilon|^2$. This suggests the smoothed metric might actually have \emph{more} positive curvature from this term. However, the dominant error source is the intrinsic scalar curvature $R^{\Sigma_s}(\gamma_\epsilon)$, which involves derivatives of the metric parallel to $\Sigma$.
Since the original metric $\tg$ is Lipschitz across $\Sigma$, its tangential derivatives $\nabla_y g$ are bounded (though discontinuous). The mollified metric $\gamma_\epsilon$ has derivatives $\nabla_y (\eta_\epsilon * g) = \eta_\epsilon * (\nabla_y g)$.
The difference $R^{\Sigma_s}(\gamma_\epsilon) - \eta_\epsilon * R^{\Sigma_s}(g)$ involves products of Christoffel symbols. Since the metric is uniformly Lipschitz, these symbols are bounded in $L^\infty$. Thus, the error term is pointwise bounded:
\[ |R_{\hat{g}_\epsilon}(x) - (\eta_\epsilon * R_g)(x)| \le C \|\tg\|_{C^{0,1}}^2. \]
Consequently, the negative part of the scalar curvature, $R^-_\epsilon = \min(0, R_{\hat{g}_\epsilon})$, is bounded by a constant $C$ independent of $\epsilon$.

\paragraph{2. Integral Estimates.}
Since the negative curvature is supported only in the smoothing collar $N_{2\epsilon}$ (where the mollifier acts), and is pointwise bounded by a constant $C$, the integral estimates follow directly from the volume of this support.
The volume of the collar is $\text{Vol}(N_{2\epsilon}) \approx 4\epsilon \cdot \text{Area}(\Sigma)$.
Thus, we obtain the following bounds:
\begin{itemize}
    \item \textbf{$L^1$ Bound:} $\|R^-_\epsilon\|_{L^1} \le C \cdot \text{Vol}(N_{2\epsilon}) \le C' \epsilon$.
    \item \textbf{$L^2$ Bound:} $\|R^-_\epsilon\|_{L^2} = \left(\int_{N_{2\epsilon}} |R^-|^2 dV\right)^{1/2} \le (C^2 \cdot 4\epsilon A)^{1/2} \le C' \epsilon^{1/2}$.
\end{itemize}
\paragraph{3. Interpolation for the Sharp $L^{3/2}$ Estimate.}
...
\[ \|R^-_\epsilon\|_{L^{3/2}}^{3/2} \le (C'\epsilon^{1/2}) \cdot (C'\epsilon)^{1/2} = (C')^2 \epsilon. \]
