...
The metrics $\geps = u_\epsilon^4 \hat{g}_\epsilon$ converge in $C^0$ to the Lipschitz metric $\tg$ as $\epsilon \to 0$. This means the metric tensors are uniformly equivalent for small $\epsilon$; there exist constants $c_1, c_2 > 0$ such that $c_1 \tg \le \geps \le c_2 \tg$. Consequently, the ellipticity constants of the $p$-Laplace operator $\Delta_{p, \geps}$ are uniform in $\epsilon$.
The potential $u_{p, \epsilon}$ is a solution to a Dirichlet problem with fixed boundary values (0 and 1). The smoothing region $N_\epsilon$ is a thin shell whose geometry is converging to that of the interface $\Sigma$. By standard interior estimates for degenerate elliptic equations (specifically, the De Giorgi-Nash-Moser theory), the local $L^\infty$ norm of the gradient of a solution is controlled by its $L^p$ energy in a slightly larger ball.
Since the solutions $u_{p,\epsilon}$ have uniformly bounded Dirichlet energy (as $p \to 1$), and the underlying geometry of the domain is stable, the gradient cannot concentrate and blow up in the shrinking region $N_\epsilon$. A more formal argument involves a blow-up analysis: if the gradient were unbounded, a sequence of rescaled solutions would converge to a non-trivial harmonic function on an infinite cylinder, which must have a bounded gradient, a contradiction. Thus, the gradient must be uniformly bounded.
\end{proof}
