...
\[
    \|u\|_{W^{k,p}_{\delta, \gamma}}^p := \|u\|_{W^{k,p}(M_{bulk})}^p + \|u\|_{W^{k,p}_\delta(\mathcal{E}_{AF})}^p + \|u\|_{W^{k,p}_\gamma(\mathcal{E}_{Cyl})}^p.
\]
...
\end{equation}
The sign convention is chosen such that positive mean curvature corresponds to a surface that is convex with respect to the bulk region. We prove that $H_{\partial\bM}^{\bg} \ge 0$ using the variational characterization of the principal eigenvalue.

The stability condition $\lambda_1(L_\Sigma) \ge 0$ implies that the quadratic form associated with $L_\Sigma$ is non-negative for all functions $\psi \in H^1(\Sigma)$:
\[
    Q(\psi) = \int_\Sigma \left( |\nabla_\Sigma \psi|^2 + V_\Sigma \psi^2 \right) d\sigma_\Sigma \ge 0.
\]
Suppose, for the sake of contradiction, that $V_\Sigma$ is negative on some open set $\Omega^- \subset \Sigma$. Since $V_\Sigma$ (which depends on the smooth geometry of the initial data) is continuous, we can assume $V_\Sigma \le -\epsilon < 0$ on $\Omega^-$.
We construct a test function $\psi$ supported entirely within $\Omega^-$. By standard localization arguments, we can choose $\psi$ such that the gradient energy $\int |\nabla \psi|^2$ is arbitrarily small compared to the $L^2$ norm $\int \psi^2$.
Then, the quadratic form becomes negative:
\[ Q(\psi) = \int |\nabla \psi|^2 + \int_{\Omega^-} V_\Sigma \psi^2 \le \int |\nabla \psi|^2 - \epsilon \int \psi^2 < 0. \]
This contradicts the assumption $\lambda_1(L_\Sigma) \ge 0$. Therefore, $V_\Sigma$ must be non-negative everywhere on $\Sigma$. Since $H_{\partial\bM}^{\bg} = V_\Sigma$, we conclude $H_{\partial\bM}^{\bg} \ge 0$.
...
Crucially, the Spectral Condition guarantees that the solution satisfies $\phi \le 1$ (Lemma \ref{lem:MaxPrincipleDEC}). Since $\phi$ approaches 1 from below at infinity ($\phi = 1+A/r$), the flux $\partial_r \phi$ must be positive. This rigorously implies $A \le 0$.
...
We have presented a rigorous framework detailing the proof of the Spacetime Penrose Inequality. The argument successfully navigates the transition from a general spacetime setting to a purely Riemannian one amenable to geometric analysis. This requires a sophisticated two-step process: the Generalized Jang reduction, which introduces analytical difficulties related to singularities and curvature control, followed by a delicate metric deformation to resolve these issues. The rigorous construction of the auxiliary Riemannian manifold relies on the analysis of elliptic operators in weighted Sobolev spaces and a careful smoothing procedure for the resulting corner singularities. Once the smoothed manifold with non-negative scalar curvature is established, the AMO $p$-harmonic level set method provides a robust pathway to establish the geometric inequality, thereby confirming the fundamental relationship $M_{\ADM} \ge \sqrt{A/16\pi}$ in full generality.
