...
We construct a parabolic \textit{Spectral Conformal Flow} driven by the first eigenfunction of the conformal Laplacian to monotonically reduce the mass to the Conformal Capacity of the horizon.
By establishing the inequality $M_{ADM} \ge \text{Cap}(\Sigma)$ via the flow, and invoking the sharp Capacity-Area bound for manifolds with non-negative scalar curvature, we complete the proof.
\end{abstract}
...
As $t \to \infty$, the flow drives the conformal factor $u(x,t)$ to the harmonic capacitary potential of the boundary $\Sigma$.
Consequently, the limiting value of the mass functional coincides exactly with the Conformal Capacity of the horizon:
\begin{equation}
    \lim_{t \to \infty} M(t) = \text{Cap}(\Sigma, g_0) = \frac{1}{4\pi} \int_{M} |\nabla \phi_{cap}|^2 dV_{g_0}.
\end{equation}
By the strict monotonicity proved in Theorem 4.3, we establish the fundamental inequality:
\[ M_{ADM}(g_0) \ge \text{Cap}(\Sigma, g_0). \]
\end{theorem}
...
\paragraph{Step 1-2: Jang Reduction and Flow.}
...
\begin{equation}
    M_{ADM}(g) \ge M_{ADM}(\overline{g}) \ge \text{Cap}(\overline{\Sigma}, \overline{g}).
\end{equation}

\paragraph{Step 3: Capacity-Area Inequality.}
For an asymptotically flat 3-manifold with non-negative scalar curvature and minimal boundary $\Sigma$, the Conformal Capacity is bounded below by the area radius.
This is a sharp inequality derived from the Refined Kato Inequality and the Sobolev inequality on $R \ge 0$ manifolds (Bray-Miao 2008):
\begin{equation}
    \text{Cap}(\overline{\Sigma}, \overline{g}) \ge \sqrt{\frac{A(\overline{\Sigma})}{16\pi}}.
\end{equation}

\paragraph{Step 4: Rigidity Analysis.}
Combining the results:
\begin{equation}
    M_{ADM}(g) \ge \text{Cap}(\overline{\Sigma}) \ge \sqrt{\frac{A(\Sigma)}{16\pi}}.
\end{equation}
Equality holds in the first step iff the flow is static ($g_0$ is harmonic-flat), and in the second step iff the manifold is conformally Schwarzschild.
