...
\textit{3. Regularity of the Limit Isometry via Harmonic Map Theory:}
The limit map $\Psi: (\tM, \tg) \to (M_{Schw}, g_{Schw})$ is a global metric isometry. This implies that $\Psi$ is a harmonic map. The target manifold $(M_{Schw}, g_{Schw})$ is real-analytic. Regularity theory for harmonic maps into analytic targets implies that $\Psi$ is real-analytic wherever the domain is a manifold.
We must then treat the interface $\Sigma$ and points $p_k$ as removable singularities for the harmonic map equation, utilizing the fact that they have zero capacity (proven in Appendix A) and finite area/energy.
