...
The solution to this equation on an asymptotically flat manifold can be represented using the Green's function $G(x,y)$ for the operator $8\Delta_{\hat{g}_\epsilon}$:
\[ v_\epsilon(x) = -\frac{1}{8} \int_{\tM} G(x,y) R^-_\epsilon(y) u_\epsilon(y) \, dV_{\hat{g}_\epsilon}(y). \]
The Green's function for the Laplacian on a 3-manifold is positive, $G(x,y) > 0$, and it satisfies the pointwise bound $G(x,y) \le C_1/d(x,y)$, where $d(x,y)$ is the geodesic distance. The negative sign is crucial; since $R^-_\epsilon \le 0$, it ensures that $v_\epsilon \ge 0$.
...
The integral term is the application of the potential operator $I_1(|R^-_\epsilon|)$. The Hardy-Littlewood-Sobolev inequality states that for $f \in L^p(\R^n)$, the Riesz potential $I_\alpha(f)$ is in $L^q(\R^n)$ where $1/q = 1/p - \alpha/n$. For our case, $n=3, \alpha=1$, and we have $f=|R^-_\epsilon| \in L^{3/2}$. The inequality gives $\|I_1(f)\|_{L^3} \le C \|f\|_{L^{3/2}}$. To get an $L^\infty$ bound, we need a stronger statement, which holds for the potential operator:
\[ \|I_1(f)\|_{L^\infty} \le C_2 \|f\|_{L^{3/2}}. \]
This standard result from potential theory bounds the $L^\infty$ norm of the potential of an $L^{3/2}$ function in three dimensions.
