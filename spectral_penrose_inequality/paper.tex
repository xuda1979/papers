...
The stability of the outermost MOTS implies that $\theta_+ \ge 0$ for outward variations.
The correct matching condition derived from the blow-up analysis (see Metzger 2010) is that the mean curvature of the cylinder \emph{from the perspective of the conformal metric} satisfies:
\[ \Jump{H_{\tg}} = H_{\tg}^{cyl} - H_{\tg}^{bulk} \le 0. \]
This inequality is strict unless the horizon is already a stable minimal surface in the initial data.
This non-positive jump is the rigorous source of the "Capacity" term in the mass inequality.
\end{theorem}
