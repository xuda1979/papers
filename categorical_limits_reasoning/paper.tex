...
We model the "reasoning space" using category theory. Let $\C$ be a category representing the state space of the problem (e.g., $\Set$, $\Rel$ or a category of vector spaces).
...
Solving a reasoning task is equivalent to computing this catamorphism. By Ad\'amek's Theorem, if $T$ is $\omega$-continuous, the initial algebra $\mu T$ is the colimit of the initial chain:
