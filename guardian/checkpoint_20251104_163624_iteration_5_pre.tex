...
\pgfplotsset{
  ...
  % Journal-quality defaults for better print reproduction
  label style={font=\large},                    % Larger axis labels
  tick label style={font=\normalsize},          % Readable tick labels  
  legend style={font=\small, draw=black, fill=white, fill opacity=0.9, text opacity=1},
  title style={font=\Large\bfseries},           % Clear titles
  % Thicker lines and larger markers for print clarity
  every axis plot/.append style={line width=1.5pt, mark size=3.5pt},
  % Better grid styling
  grid style={line width=0.5pt, gray!30},
  % Improved error bars
...
\usepackage{siunitx}
\sisetup{per-mode=symbol,uncertainty-mode=separate,mode=match,propagate-math-font=true,reset-math-version=false,reset-text-family=false,reset-text-series=false,reset-text-shape=false,text-family-to-math=true,text-series-to-math=true}
\usepackage{caption}
...
mirror set--ups, offering a dynamical and experimentally accessible route to addressing key aspects
of the black-
...
\begin{figure}[htbp]
    ...
    \begin{axis}[
        ...
        \addplot[blue, very thick, domain=0:10, samples=100] {min(x, 10-x)} node[pos=0.4, above right, black, font=\normalsize] {$S(R) \sim t$};
        \addlegendentry{Unitary Evolution (Page Curve)}
        \addplot[red, dashed, very thick, domain=0:10, samples=100] {x};
        \addlegendentry{Thermal Radiation (Hawking)}
        \draw[gray, dotted, very thick] (axis cs:5,0) -- (axis cs:5,5) node[pos=0.5, right, black, font=\normalsize] {Page time, $t_{\text{Page}}$};
        \node[below, font=\normalsize] at (axis cs:10,0) {$t_{\text{evap}}$};
    \end{axis}
    \end{tikzpicture}
...
A compact gate-level realization with local CNOT+SWAP blocks follows directly from this dilation picture (see Appendix~\ref{app:decoupling} for details). 
...
\begin{figure}[htbp]
    ...
    \begin{axis}[
        ...
        \addplot[black, dashed, very thick] table[x=time, y=ideal_page] {\datatablePagecurve};
        \addlegendentry{Ideal Page Curve}
        \addplot[red, dashed, very thick] table[x=time, y=hawking] {\datatablePagecurve};
        \addlegendentry{Hawking (thermal)}
        \addplot[green!60!black, dotted, very thick] table[x=time, y=bh_entropy] {\datatablePagecurve};
...
\begin{figure}[htbp]
    ...
    \begin{axis}[
        ...
        \addplot+[orange!90!black, very thick, mark=*,
            error bars/.cd, y dir=both, y explicit]
...
\addplot+[mark=*,blue,very thick,error bars/.cd,y dir=both,y explicit]
...
\addplot[gray, dashed, very thick, domain=0:15, samples=100] {1};
...
\paragraph{Computational complexity summary (consolidated).}
For clarity of reproducibility and performance planning, we consolidate the key complexity scalings derived and used throughout Secs.~\ref{ssec:ptmpo} and \ref{sec:mpo_mps_validation}:
\begin{itemize}[leftmargin=*]
  \item Process-tensor MPO per-step cost with memory depth $\ell_{\rm mem}$ and local dimension $d$: 
  \[
    T_{\rm step} = O\!\big(D_{\Upsilon}^3 d^2\big), \quad D_{\Upsilon}\sim \chi^2 d^{\ell_{\rm mem}},
  \]
  and memory
  \[
    M = O\!\big(D_{\Upsilon}^2\big)=O\!\big(\chi^4 d^{2\ell_{\rm mem}}\big).
  \]
  \item For physically motivated $\ell_{\rm mem}\sim \log S_{\rm BH}$, these costs grow polynomially in $S_{\rm BH}$ for fixed $d$, and are practical for $\ell_{\rm mem}\lesssim 6$ with $\chi\lesssim 512$ on single-GPU nodes at $N\sim 10^2$ steps.
  \item Certified truncation error obeys 
  \[
    \|\rho_{R_{\le N}}-\tilde\rho_{R_{\le N}}\|_1 \le C_{\rm mem}\,\ell_{\rm mem}\,N\,\epsilon_{\rm SVD},
  \]
  implying an explicit entropy error control via continuity bounds and choice of $\epsilon_{\rm SVD}$.
\end{itemize}
We highlight that these summary expressions match the resource measurements in Table~\ref{tab:ptmpo_scaling} and the convergence behavior in Fig.~\ref{fig:ptmpo_error} and thus provide actionable guidance for reproducing the PT-MPO benchmarks. As emphasized at the start of Sec.~\ref{sec:numerical_validation}, all results are fully reproducible with deterministic seeding; see Appendix~\ref{app:code} for the seed ledger and generation workflow.
...
We consider a code subspace $\mathcal{H}_{\mathrm{code}}\subset \mathcal{H}_{\mathrm{infall}}\otimes \mathcal{H}_{\mathrm{mem}}$ and the effective channel $\Phi:\mathcal{B}(\mathcal{H}_{\mathrm{code}})\to \mathcal{B}(\mathcal{H}_{\mathrm{L}})$ induced by $\Upsilon_{n{:}0}$.
...
\paragraph{A decoupling inequality with local $2$--designs (expanded).}
...
\end{equation}
\textit{More explicit steps.} The proof refines the Hayden–Preskill decoupling by replacing Haar-averages with the local second-moment twirl restricted to the lightcone sub-algebra $\mathcal{B}(\mathcal{H}_X)$. The trace-norm control follows from an application of the non-commutative Cauchy–Schwarz inequality and the fact that $\varepsilon_2$-designs match the first two Haar moments. The term $e^{-\ell_{\rm scr}/\xi}$ controls out-of-lightcone contributions under exponential clustering with correlation length $\xi$. The spectral error $\varepsilon_{\rm spec}$ bounds deviations of the single-step entropic production from the KMS/greybody prediction. A telescoping sum over steps, combined with contractivity of CPTP maps in trace norm, yields the cumulative bound.

\paragraph{OTOC decay $\Rightarrow$ local $2$--design with constants}\label{prop:otoc_design_app}
...
\noindent\emph{Proof sketch (expanded).} The second frame potential of $\Phi^{(2)}_t$ can be written as a sum of (regulated) four-point functions of the Heisenberg-evolved local operator basis on $X$ and an auxiliary copy. The OTOC bound controls the decay of connected four-point functions, while Lieb--Robinson bounds localize the effects within the lightcone. The constants $\mu,C_1,C_2$ track the OTOC rate, the LR velocity, and the basis normalization. Bounding the diamond norm by the frame potential difference closes the argument.
