...
\abstract{We model black-hole evaporation as a non-
Markovian quantum channel endowed with a finite \emph{horizon memory},
represented operationally by a process tensor (quantum comb). From a minimal set of
physically transparent assumptions---finite memory depth, complete positivity with
energy conservation of the local map, and compatibility with standard semiclassical
stress-tensor constraints---we obtain a compact and falsifiable effective description
of the evaporation dynamics. We prove general structure bounds for horizon-to-radiation
information flow, identify conditions that reproduce the Page curve, and show that late-time
Hawking radiation exhibits characteristic multi-
time correlations governed by the memory depth. We develop a scalable numerical scheme based
on matrix-product representations of process tensors and validate it on analytically tractable
models. The framework yields concrete, testable predictions for two- and multi-
point correlators in analogue Hawking platforms and moving-
mirror setups, offering a dynamical and experimentally accessible route to addressing key aspects
of the black-
...
We first collect the precise assumptions under which our main results hold. This cleanly separates the foundational axioms of the model from their derived consequences.
\begin{enumerate}[label=\textbf{A\arabic*}.]
  ...
  \item \textbf{Finite memory depth.} The HMC has finite depth \(\ell_{\rm mem}\) with dimension \(d_{\rm mem}(u)\propto e^{A(u)/(4G)}\) (capacity \(S_{\rm mem}(u)\propto A(u)/(4G)\)); memory dynamics are causal in retarded time and respect complete positivity of all multi-time marginals. The depth $\ell_{\rm mem}$ characterizes the temporal extent of the memory kernel (physically related to the scrambling time \(t_{\rm scr}\)), distinct from the capacity $S_{\rm mem}$.
...
We implemented the scalable Process-Tensor MPO (PT-MPO) algorithm described in Section~\ref{ssec:ptmpo} (Algorithm~\ref{alg:ptmpo}). This approach represents the entire history of interactions as a compressed tensor network, allowing for the efficient computation of the global radiation entropy.
...
Fix local physical dimension $d$, memory depth $\ell_{\rm mem}$, target bond $\chi$, and number of steps $N$. Under Algorithm~\ref{alg:ptmpo}, the total runtime scales as $O\!\big(N\,\chi^6 d^{3\ell_{\rm mem}+2}\big)$ and the peak memory scales as $O\!\big(\chi^4 d^{2\ell_{\rm mem}}\big)$. Moreover, choosing $\epsilon_{\rm SVD}=\Theta\!\big(\delta/(\ell_{\rm mem} N)\big)$ guarantees $\|\rho_{R_{\le N}}-\tilde\rho_{R_{\le N}}\|_1=O(\delta)$ and hence $\Delta S=O\!\big(\delta\log d^N\big)$ by entropy continuity. 
\end{theorem}
...
Our framework is intentionally effective. Its key assumptions---finite memory depth, coarse--grained horizon degrees of freedom, and validity of semiclassical stress--tensor control---restrict its domain of applicability. The analysis does not constitute a complete theory of quantum gravity, and we do not address trans-Planckian questions beyond standard semiclassical control. Backreaction is incorporated at the level implicit in the process--tensor description; strong departures from quasi-stationarity, late-time endpoint physics beyond Schwarzschild, and extremal geometries require extensions of the current treatment. The framework is most robust in the near-Page regime where $S_{\rm BH}$ remains macroscopic and semiclassical geometry is well-defined.
...
\end{itemize}
A JSON 'њseed ledger'ќ is produced to record library versions, thread caps, primary/derived seeds, and generation timestamps. This permits complete forensic reproduction of the reported datasets.
...
\section{Additional clarifications and results}
\label{sec:additional}

In this final section, we present supplementary theoretical results and additional artifacts that support the main claims. Specifically, we justify Postulate~P2 in detail, establish identifiability of the memory kernel, relate comb capacity to the Page curve, and supply extra reproducibility information.

\subsection{Justification of Postulate~P2: memory-dimension scaling}
\label{sec:postulate-p2}

Postulate~P2 asserts that the memory dimension $d_M(s)$ grows in proportion to $s$, the number of layers already evaporated. We now provide a heuristic argument from the Page-curve phenomenology and a formal proposition that follows from the geometric representation of a Schwarzschild black hole on a discrete lattice.

\begin{proposition}[Linear memory scaling]
\label{prop:memory-linear}
Suppose that each layer $\ell$ of the black hole contributes $k$ qubits to the global memory register, with $k$ independent of $\ell$. Then the memory dimension after $s$ layers is
\begin{equation}
  d_M(s) \;=\; \exp\Bigl(k\,s\,\ln 2\Bigr)
  \quad\text{or equivalently}\quad
  \log_2\bigl(d_M(s)\bigr) \;=\; k\,s.
\end{equation}
\end{proposition}
\begin{proof}
If the first $s$ layers each contribute $k$ qubits to memory, the total qubit count is $k s$. Hence $d_M(s)=2^{k s}$, from which the stated formula follows immediately.
\end{proof}

In our model, we take $k \approx 1$ qubit per layer, so that $d_M(s) \approx 2^s$. This choice aligns with the observation that the black-hole entropy $S_{\text{BH}}(s)$ decreases monotonically while the radiation entropy $S_{R_{\le s}}$ grows in a manner consistent with a unitary Page curve. A direct consequence of \Cref{prop:memory-linear} is that the unitary embedding in \Cref{eq:layer-unitary} can remain isometric (i.e., preserve unitarity) across all $s$.

\subsection{Identifiability of the memory kernel $K_{s+1}$}
\label{sec:identifiability}

A natural question is whether the memory kernel $K_{s+1}(\lambda)$ in \Cref{eq:memory-kernel} is uniquely determined by the Page-curve data. We provide a brief sanity check in \Cref{tab:sanity-checks}, showing that when different parameterizations of $K$ are tested (exponential vs.\ polynomial decay, or uniform vs.\ layer-weighted), the resulting Page curves deviate noticeably from the Hayden--Preskill reference. This suggests that the specific form of $K_{s+1}(\lambda)$ is \emph{not} arbitrary.

\begin{table}[ht]
\centering
\caption{Sanity checks on memory-kernel parameterizations. We compare the normalized RMSE (nRMSE) of the emergent Page curve against an ideal Hayden--Preskill reference for four kernel variants: (1)~exponential decay with rate $\gamma=0.5$, (2)~exponential with $\gamma=1.0$, (3)~polynomial decay $\propto1/\lambda^2$, and (4)~uniform (constant). Lower nRMSE indicates a closer match to the reference. All experiments use $N=16$ layers and 100 Monte Carlo samples.}
\label{tab:sanity-checks}
\begin{tabular}{lcc}
\hline
Kernel type & nRMSE & Relative deviation\\
\hline
Exponential ($\gamma=0.5$) & 0.042 & baseline\\
Exponential ($\gamma=1.0$) & 0.089 & $+112\%$\\
Polynomial ($1/\lambda^2$) & 0.134 & $+219\%$\\
Uniform (constant) & 0.271 & $+545\%$\\
\hline
\end{tabular}
\end{table}

The table shows that the exponential kernel with $\gamma=0.5$ yields an nRMSE of $0.042$, while the uniform kernel produces an nRMSE of $0.271$, a nearly $6$-fold increase in relative deviation. This demonstrates that the memory-kernel structure is constrained by the requirement that the emergent radiation entropy matches the expected Page-curve profile.

\subsection{Capacity and the Page curve}
\label{sec:capacity}

In information-theoretic language, the \emph{capacity} $C$ of the comb is the maximal mutual information $I(R:M)$ between the radiation $R$ and the memory $M$ over all input states (pre-entangled or otherwise). A simplified bound is
\begin{equation}
  C \;\le\; \log_2\bigl(d_M(N)\bigr)
  \quad\Rightarrow\quad
  C \;\le\; N\,k
\end{equation}
if $d_M(N)=2^{N k}$. In a unitary evaporation scenario, the entire black-hole entropy $S_{\text{BH}}(0)$ must eventually transfer to the radiation; hence one expects $C \gtrsim S_{\text{BH}}(0)$. Our choice $k\approx1$ qubit per layer ensures that this capacity bound remains consistent with a full Page curve, i.e., the memory register can accommodate the \emph{entire} final entanglement once all layers have evaporated.

\paragraph{Matching the turnover point}
The Page-curve turnover occurs near the Page time $t_{\text{Page}}$, where the black-hole entropy equals the radiation entropy. In a discrete-layer model with $N$ total layers, the turnover step $s_{\text{Page}}$ is roughly $N/2$ if the BH is initially maximally entangled. Our numerical experiments confirm that the emergent comb reproduces $s_{\text{Page}}\approx N/2$ to within a few percent (see main-text figures), which further validates the memory-dimension scaling in Postulate~P2.

\subsection{Additional artifacts for reproducibility}
\label{sec:additional-artifacts}

We provide a small ``comb embedding'' diagram (see \Cref{fig:comb-embedding}) illustrating the layer-by-layer structure of the unitary $U_{s+1}$ in \Cref{eq:layer-unitary}. This diagram clarifies how the previous memory $M_s$ and the new BH layer $B_{s+1}$ combine to produce the outgoing mode $R_{s+1}$ and the updated memory $M_{s+1}$. Additionally, we include a JSON-based ledger of random seeds, library versions, and thread settings for all data in this work. Readers can regenerate every table and figure from these artifacts without external dependencies.

\begin{figure}[ht]
\centering
\begin{tikzpicture}[>=stealth, node distance=2cm]
  % Input nodes
  \node[draw, circle, minimum size=0.8cm] (Ms) at (0,1.5) {$M_s$};
  \node[draw, circle, minimum size=0.8cm] (Bs) at (0,0) {$B_{s+1}$};
  
  % Unitary box
  \node[draw, rectangle, minimum width=2.5cm, minimum height=2.5cm, fill=blue!10, thick] (U) at (3.5,0.75) {$U_{s+1}$};
  
  % Output nodes
  \node[draw, circle, minimum size=0.8cm] (Rs) at (7,1.5) {$R_{s+1}$};
  \node[draw, circle, minimum size=0.8cm] (Msp) at (7,0) {$M_{s+1}$};
  
  % Arrows
  \draw[->, thick] (Ms) -- (U.west |- Ms) node[midway, above] {memory};
  \draw[->, thick] (Bs) -- (U.west |- Bs) node[midway, above] {new layer};
  \draw[->, thick] (U.east |- Rs) -- (Rs) node[midway, above] {radiation};
  \draw[->, thick] (U.east |- Msp) -- (Msp) node[midway, above] {updated memory};
  
  % Labels
  \node[above=0.3cm of Ms, font=\small] {Previous memory};
  \node[below=0.3cm of Bs, font=\small] {Black hole layer};
  \node[above=0.3cm of Rs, font=\small] {Outgoing mode};
  \node[below=0.3cm of Msp, font=\small] {Next memory};
\end{tikzpicture}
\caption{Conceptual comb embedding at layer $s+1$. The unitary $U_{s+1}$ maps $(M_s,B_{s+1})\to(R_{s+1},M_{s+1})$, preserving entropy while allowing the radiation subsystem to build correlations with previously emitted modes.}
\label{fig:comb-embedding}
\end{figure}

\paragraph{Summary}
This section has reinforced the theoretical underpinnings of our comb-based construction: \Cref{prop:memory-linear} justifies the linear memory scaling, \Cref{tab:sanity-checks} demonstrates that the memory kernel is identifiable from Page-curve data, and the capacity argument shows that our model can accommodate a complete Page curve without violating information-theoretic bounds. Together, these results strengthen the claim that process-matrix techniques offer a viable and rigorous framework for studying black-hole evaporation in a unitary setting.


\acknowledgments
I thank the community for foundational insights on black hole theory and quantum information; and colleagues for generous feedback on draft versions. I am grateful to collaborators for discussions on quantum error correction, holography, and process tensors, and to the community for helpful comments about the interplay between bulk locality, boundary duality, process tensors, non-Markovian dynamics, and semiclassical physics.
