...
To illustrate the theoretical advantages of ARH's structural adaptation, we designed a stylized simulation of a Hierarchical Concept Drift (HCD) benchmark. This simulation is not a full implementation of the model, but rather a scripted demonstration of its expected behavior based on the principles outlined in this paper. The task is a continual classification stream with two phases:
\begin{itemize}
...
We model the behavior of ARH against two strong baselines:
\begin{enumerate}
...
\end{enumerate}
We measure the modeled performance based on: (1) \textbf{Phase 1 Accuracy}, (2) \textbf{Post-Shift Nadir} (the lowest accuracy after the shift), and (3) \textbf{Recovery Speed} (number of steps to return to 90\% of pre-shift accuracy).

The Python script \texttt{code/plot\_hcd.py} generates the performance curves shown in Figure~\ref{fig:performance_plot}. These curves are intended to be illustrative of the model's theoretical behavior. The script saves the visualization to \texttt{code/arh\_hcd\_visualization.pdf} and the underlying data to \texttt{code/arh\_hcd\_curves.csv}.
...
\begin{table}[h!]
\centering
\caption{Parameters for the stylized HCD benchmark simulation. These values, generated by the accompanying script, define the curves in Figure~\ref{fig:performance_plot} and are chosen for illustrative purposes.}
...
]{ARH/code/hcd_results_table.csv}
\end{table}
