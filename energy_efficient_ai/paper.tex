...
\begin{itemize}
    \item \textbf{Energy Efficiency:} We demonstrate a $5.2\times$ reduction in energy consumption for SSA compared to dense attention, scaling to $95\times$ for long sequences ($N=4096$).
    \item \textbf{Long-Range Dependency:} Our method maintains $99.6\%$ retrieval accuracy on "Needle-in-a-Haystack" tasks, significantly outperforming random and local sparse baselines.
    \item \textbf{Scalability:} We confirm $O(N \log N)$ runtime scaling, validating the theoretical complexity bounds.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Execution:} A PowerShell script \texttt{run\_experiments.ps1} is provided to automatically install dependencies, execute the full test suite, and regenerate all figures (Figures 1--6) and tables presented in this paper.
    \item \textbf{Hardware:} Experiments were conducted on a standard consumer workstation. Runtime metrics reported in Table 3 are median values over 5 runs with warmup.
\end{itemize}
