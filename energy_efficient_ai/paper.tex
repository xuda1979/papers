...
\section{Theoretical Analysis: Energy Efficiency, Expressivity and Generalization}
In this section, we analyze the energy efficiency, expressive power of Spectral Sparse Attention and derive generalization bounds based on the Vapnik-Chervonenkis (VC) dimension theory and Rademacher complexity.

\subsection{Energy Consumption Model}
To rigorously quantify the energy savings, we adopt a first-order energy model that accounts for both arithmetic operations (FLOPs) and memory access (DRAM/HBM).

Let $E_{op}$ be the energy cost per floating point operation (FLOP) and $E_{mem}$ be the energy cost per byte of memory access. The total energy $E_{total}$ for an attention layer can be approximated as:
\[
E_{total} \approx N_{FLOPs} \cdot E_{op} + N_{Bytes} \cdot E_{mem}
\]
For standard self-attention (SA), the complexity is dominated by the $QK^T$ and $AV$ matrix multiplications:
\[
N_{FLOPs}^{SA} \approx 4 N^2 d \quad \text{and} \quad N_{Bytes}^{SA} \approx 2 N^2 \cdot \text{sizeof(float)}
\]
where the memory term accounts for reading/writing the $N \times N$ attention matrix.

For Spectral Sparse Attention (SSA), with $k \approx \sqrt{N}$ clusters and $s \approx \sqrt{N}$ global keys, the number of active entries in the attention mask is $N_{active} \approx 2 N \sqrt{N}$. Thus:
\[
N_{FLOPs}^{SSA} \approx 2 N_{active} d \approx 4 N^{1.5} d \quad \text{and} \quad N_{Bytes}^{SSA} \approx N_{active} \cdot \text{sizeof(float)} \approx 2 N^{1.5} \cdot \text{sizeof(float)}
\]

Defining the Energy Saving Ratio ($\eta$) as $E_{total}^{SA} / E_{total}^{SSA}$:
\[
\eta(N) = \frac{4 N^2 d \cdot E_{op} + C N^2 \cdot E_{mem}}{4 N^{1.5} d \cdot E_{op} + C N^{1.5} \cdot E_{mem}} \approx \sqrt{N}
\]
This theoretical model predicts that as sequence length $N$ increases, the energy efficiency of SSA scales as $O(\sqrt{N})$. For a typical context window of $N=32k$, this implies a potential theoretical energy reduction of roughly $\sqrt{32768} \approx 180\times$, ignoring fixed overheads.
...
\subsection{Runtime Performance and Spectral Fidelity}
We measured inference time on a CPU for sequence lengths up to $N=4096$. As shown in Figure \ref{fig:performance}a, our Python prototype exhibits significant overhead at smaller sequence lengths ($N < 1000$) due to the non-vectorized clustering and loop operations. However, the theoretical $O(N^{1.5})$ scaling advantage becomes apparent as $N$ increases, with the crossover point occurring around $N=1500$.
...
\begin{figure}[h]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        ...
        \caption{Spectral Approximation}
    \end{subfigure}
    \caption{(a) Runtime of standard attention ($O(N^2)$) vs SSA ($O(N^{1.5})$). (b) Eigenvalue distribution of the normalized Laplacian for the full attention graph vs. the SSA sparsifier. The close alignment of the spectra confirms that SSA preserves the structural properties of the attention mechanism, satisfying the requirements for spectral sparsification.}
    \label{fig:performance}
\end{figure}

Table \ref{tab:results} summarizes the results. At $N=2048$, SSA achieves a \textbf{1.41x speedup}. At $N=4096$, the speedup is \textbf{1.80x}, with a cosine similarity of \textbf{0.76}, indicating that the sparse attention mechanism successfully captures the majority of the attention mass.

Crucially, Figure \ref{fig:performance}b provides the empirical validation of the method's theoretical premise. By plotting the sorted eigenvalues of the normalized Laplacian $\mathcal{L} = I - D^{-1/2} A D^{-1/2}$ for both the dense attention matrix and the SSA sparse approximation, we observe a strong spectral overlap. This confirms that our method constructs a high-fidelity spectral sparsifier, preserving the global connectivity and diffusion properties of the underlying graph despite the aggressive edge pruning.

Comparing the infrastructure performance, the transition from $O(N^2)$ to $O(N^{1.5})$ results in a widening performance gap. While overhead dominates at small $N$, the efficiency gains are realized at scale. For instance, at $N=4096$, SSA is nearly twice as fast as the baseline. When extrapolated to the context lengths of modern LLMs (e.g., 32k or 128k), the theoretical energy saving ratio $\eta \approx \sqrt{N}$ suggests orders-of-magnitude efficiency improvements.
...
\begin{table}[h]
    ...
    \midrule
    128 & 0.0006 & 0.0035 & 0.17x & 0.8134 & 0.9535 \\
    256 & 0.0031 & 0.0087 & 0.36x & 1.1678 & 0.9276 \\
    512 & 0.0141 & 0.0310 & 0.45x & 1.5747 & 0.8928 \\
    1024 & 0.0655 & 0.0768 & 0.85x & 2.3218 & 0.8675 \\
    2048 & 0.5403 & 0.3837 & 1.41x & 3.2712 & 0.8176 \\
    4096 & 2.8879 & 1.6044 & 1.80x & 4.2556 & 0.7601 \\
    \bottomrule
    \end{tabular}
