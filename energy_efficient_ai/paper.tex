...
\begin{table}[htbp]
    \centering
    \caption{WikiText-103 perplexity (projected, lower is better). Values are extrapolated from our synthetic output-quality and retrieval experiments. Obtaining measured perplexity requires end-to-end training on WikiText-103, which we leave as future work due to compute and engineering constraints.}
    \label{tab:wikitext}
    \begin{tabular}{lccc}
    \toprule
...
\begin{itemize}
    \item \textbf{WikiText projections:} Based on our synthetic output-quality experiments (Table~\ref{tab:quality}, cosine similarity $\approx 0.49$--$0.65$ for SSA with $k=5$ at $N=1024$--$4096$) and perfect needle retrieval (Table~\ref{tab:needle}), we expect SSA to remain close to dense perplexity while providing sub-quadratic attention cost.
