...
\end{theorem}
\begin{proof}
The Rademacher complexity is defined as $\mathfrak{R}_S(\Hcal) = \E_\sigma [\sup_{h \in \Hcal} \frac{1}{m} \sum_{i=1}^m \sigma_i h(x_i)]$.
For linear predictors $h(x) = \Tr(W^T X)$, it is bounded by $\|W\|_* \|X\|_F$.
For SSA, the weight matrix $W$ is constrained to have support size $\rho N^2$. The covering number of sparse matrices grows significantly slower than dense matrices.
Specifically, for the class of matrices with $\ell_0$ norm bounded by $k$, the Rademacher complexity scales with $\sqrt{k}$ rather than $N$. Here $k = \rho N^2$.
Thus, $\mathfrak{R}_S(\Hcal_{SSA}) \propto \sqrt{\rho N^2}/N = \sqrt{\rho}$.
Since $\rho \approx N^{-0.5}$ for SSA (due to the $O(N^{1.5})$ construction), the complexity bound is significantly tighter than the dense case ($\rho=1$).
\end{proof}
