...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=深度技术细节：JADC2 中的 AI 决策引擎]
\textbf{联合全域指挥控制（JADC2）的核心是"传感器到射手"的极速闭环。}

\textbf{1. 目标分配算法（Weapon-Target Pairing）}
\begin{itemize}
    \item 利用强化学习（RL）在毫秒级计算数千个目标与数百个火力单元的最优匹配。
    \item 考虑因素：弹药余量、拦截概率、附带损伤、后续威胁。
\end{itemize}

\textbf{2. 边缘计算部署}
\begin{itemize}
    \item 在F-35或无人机载板卡上部署量化后的轻量化大模型（如4-bit量化的Llama-3-8B级模型）。
    \item 实现断网环境下的自主决策。
\end{itemize}

\textbf{3. 认知电子战}
\begin{itemize}
    \item AI实时分析敌方雷达信号特征，动态生成干扰波形。
    \item 对抗敌方AI驱动的跳频技术。
\end{itemize}
\end{tcolorbox}

\begin{tcolorbox}[colback=red!5!white,colframe=red!75!black,title=技术细节：无人机蜂群的自主协同协议]
