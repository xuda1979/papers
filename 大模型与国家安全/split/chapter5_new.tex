...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=深度技术细节：万卡级智算中心的 TCO 与能效模型]
\textbf{1. 成本结构分析（Total Cost of Ownership）}
\begin{itemize}
    \item \textbf{硬件采购（CapEx）}：占比约 60-70\%。其中 GPU/NPU 芯片占硬件成本的 80\%。
    \item \textbf{能源成本（OpEx）}：占比约 15-20\%。一个 10 万卡集群的年耗电量可达 15-20 亿度，相当于一个中型城市的居民用电。
    \item \textbf{网络互联}：InfiniBand 或 RoCE v2 网络设备占比约 10\%。
\end{itemize}

\textbf{2. 芯片性能对比（2025年12月数据）}
\begin{table}[H]
\centering
\small
\begin{tabular}{lcccc}
...
\textbf{3. 绿色算力指标（PUE）}
\begin{itemize}
    \item 目标：国家级智算中心 PUE 控制在 1.15 以下。
    \item 技术手段：全液冷机柜、余热回收、AI 驱动的动态能效管理系统。
\end{itemize}
\end{tcolorbox}
