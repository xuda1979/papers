...
\textbf{场景一：自动化漏洞挖掘}
\begin{itemize}
    \item AI分析目标系统代码或行为，自动发现漏洞
    \item 生成漏洞利用代码（PoC/Exploit）
    \item 大规模扫描互联网资产，匹配已知漏洞
    \item \textbf{威胁}：漏洞从发现到被利用的时间窗口大幅缩短
\end{itemize}

\textbf{场景二：智能社会工程攻击}
\begin{itemize}
    \item 分析目标人物的公开信息，构建精准画像
    \item 生成高度个性化的钓鱼邮件/消息
    \item 实时语音克隆+AI对话，进行电话诈骗
    \item \textbf{威胁}：传统的安全意识培训难以应对AI级别的社工攻击
\end{itemize}

\textbf{场景三：多态恶意软件}
\begin{itemize}
    \item AI生成功能相同但代码特征不同的恶意软件变体
    \item 自动化规避杀毒软件检测
    \item 根据目标环境动态调整攻击载荷
    \item \textbf{威胁}：基于特征匹配的传统防护失效
\end{itemize}

\textbf{场景四：供应链攻击}
\begin{itemize}
    \item 分析开源项目的依赖关系，寻找薄弱点
    \item 自动化生成看似正常的恶意代码提交
    \item 利用AI生成的"合理"理由获得项目维护者信任
    \item \textbf{威胁}：攻击隐蔽性极高，影响范围极广
\end{itemize}
