...
\hypersetup{
    ...
    hypertexnames=false,
    pdfencoding=auto,
    pdftitle={大模型强国：不响的"原子弹"与新时代"两弹一星"工程},
    pdfauthor={许达}
}
\usepackage{enumitem}
\usepackage{titlesec}
...
\input{chapter1_infrastructure}  % 认知基础设施：重新定义大模型的战略本质
\input{chapter2_efficiency}      % 认知效率革命：大模型对国家核心能力的重塑
\input{chapter3_intelligence}    % 暗战前沿：认知基础设施的全球博弈
\input{chapter4_risks}           % 风险图谱：认知基础设施的安全威胁
\input{chapter5_roadmap}         % 破局之道：认知基础设施建设的具体路径
\input{chapter6_governance}      % 制度护航：治理体系与国际博弈
\input{chapter7_actions}         % 行动纲领：可操作的政策建议清单
\input{chapter8_science}         % 科研奇点：大模型驱动的颠覆性科学发现与技术革命
\input{chapter9_talent}      % 人才强基：大模型时代的"人才安全"战略
\input{chapter10_investment} % 资本赋能：大模型时代的国家投资战略
\input{chapter11_frontier}   % 技术前沿：推理模型、Agent与具身智能
\input{chapter12_newsecurity} % 安全新疆域：大模型时代的非传统安全挑战
