...
The QFIM $F(\boldsymbol{\theta}) \in \mathbb{R}^{M \times M}$ captures the local geometry of the parameter space:
\begin{equation}
...
\end{equation}
where $\hat{F}(\boldsymbol{\theta})$ is the normalized Fisher Information Matrix.
\end{definition}
...
\end{theorem}
\begin{proof}
The proof follows from covering number estimates of the hypothesis class using the Fisher-Rao metric. See \cite{abbas2021power} for the detailed derivation. This theorem implies that increasing the effective dimension increases the upper bound on the generalization gap (Variance).
\end{proof}
...
Consider a parameterized quantum circuit $U(\boldsymbol{\theta})$ that forms a unitary $t$-design for $t \ge 2$. Then, for any parameter $\theta_k$, the variance of the gradient of the cost function vanishes exponentially with the number of qubits $N$:
\begin{equation}
    \Var[\partial_k \mathcal{L}] \approx \mathcal{O}\left(\frac{1}{d_{eff}^{Hilbert}}\right) = \mathcal{O}\left(\frac{1}{2^N}\right)
\end{equation}
Conversely, ansatzes restricted to logarithmic entanglement (area-law) exhibit polynomial variance scaling, $\Var \sim \mathcal{O}(\text{poly}(1/N))$.
\end{theorem}
\begin{proof}
This result is established by \cite{mcclean2018barren} and refined by \cite{holmes2022connecting}. High entanglement capability allows the ansatz to explore the full Hilbert space, making it indistinguishable from a random unitary, which concentrates measure around the mean (zero gradient).
\end{proof}
...
The expected test error $\E[\mathcal{E}_{test}]$ of a VQC trained via gradient descent is a convex-like function of the ansatz entanglement capacity $\bar{S}$:
\begin{equation}
    \E[\mathcal{E}_{test}(\bar{S})] \approx \text{Bias}(\bar{S}) + \text{GenGap}(\bar{S}) + \text{OptError}(\bar{S})
\end{equation}
where:
\begin{enumerate}
    \item \textbf{Bias (Approximation Error):} Decreases with $\bar{S}$. Low entanglement implies a restricted hypothesis class $\mathcal{F}_{area} \subset \mathcal{F}_{vol}$, leading to underfitting.
    \item \textbf{GenGap (Variance):} Increases with $\bar{S}$. By Theorem \ref{thm:generalization}, higher capacity leads to larger generalization gaps.
    \item \textbf{OptError (Trainability):} Increases sharply for $\bar{S} \sim N$ (Volume Law). By Theorem \ref{thm:barren_plateau}, gradients vanish, causing the optimizer to get stuck at random initialization, resulting in high empirical risk.
\end{enumerate}
Consequently, there exists an optimal entanglement entropy $S^* \in (S_{min}, S_{max})$ that minimizes total error.
\end{theorem}
