...
\subsection{Effective Dimension and Generalization}

We can quantify the capacity of the QNN using the \textbf{Effective Dimension} ($d_{eff}$) based on the Fisher Information Matrix (FIM) $F(\theta)$:
\begin{equation}
    d_{eff}(\gamma, n) = 2 \sum_{k=1}^P \frac{\lambda_k}{\lambda_k + \gamma n}
\end{equation}
where $\lambda_k$ are the eigenvalues of the FIM, $n$ is the number of data samples, and $\gamma$ is a regularization parameter \citep{larocca2022theory}. The effective dimension measures the number of parameters that are "active" in the learning process. We conjecture that $d_{eff}$ is maximized in the critical entanglement regime, allowing for efficient learning without saturation.
