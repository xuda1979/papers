...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item \textbf{Intermediate Regime ($\beta_c \le \beta \le \beta_*$):} We rely on the standard conjecture that pure $SU(N)$ Yang-Mills theory undergoes no phase transition separating the strong and weak coupling regimes. This is supported by:
        \begin{itemize}
            \item Extensive Monte Carlo simulations showing smooth crossover.
            \item \textbf{Adjoint QCD Interpolation Argument:} We embed the pure gauge theory into a lattice gauge theory with heavy adjoint scalars. While Center Symmetry protects against confinement-deconfinement transitions, we assume no other bulk phase transitions (e.g., roughening) occur that would close the gap.
        \end{itemize}
    \end{enumerate}
    Under this continuity assumption, $\sigma(\beta) > 0$ for all $\beta \in (0, \infty)$.
\end{proof}
