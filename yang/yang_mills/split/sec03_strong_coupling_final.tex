...
To prove that this series converges (and thus the free energy is analytic), we replace heuristic small-parameter arguments with the rigorous **Mayer-Montroll Equations** (see Appendix \ref{ch:fix28_mayer_montroll_radius}).
...
\textit{Proof Summary:} We utilize the **Penrose Tree Identity** to map the Ursell functions to a sum over spanning trees. Using Cayley's formula to count trees on the lattice graph, we derive the explicit radius of convergence, ensuring that the mass gap is an analytic function of $\beta$ in this region.
...
A strict requirement for the continuum limit is that the field theory defines a tempered distribution (Osterwalder-Schrader Axiom 0). We prove this using the **Battle-Federbush Tree Decay** bounds (Appendix \ref{ch:fix6_battle_federbush}).
