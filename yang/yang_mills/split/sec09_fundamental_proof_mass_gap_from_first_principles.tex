...
techniques (string theory, etc.) are required.
...
\emph{not} the full mass gap problem, which requires the gap to persist in the 
...
\textbf{Update (December 2025):} The infinite-volume gap is the subject of the research program outlined in Appendix~\ref{sec:definitive-gap-closure}, which proposes using Reflection Positivity Monotonicity and Cheeger Isoperimetric Bounds to establish the result. This section serves as the rigorous finite-volume foundation.
\end{tcolorbox}
...
(uniform gap), which is needed for the full problem.
