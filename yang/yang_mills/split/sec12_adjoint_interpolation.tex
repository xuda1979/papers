...
\label{sec:unitarity}
The manuscript interpolates between $\mathcal{N}=1$ SYM and Pure YM by varying a mass parameter $M$. On a lattice, adding a naive mass term to fermions often breaks **Reflection Positivity (RP)**. If RP is broken, the transfer matrix is not Hermitian, eigenvalues become complex, and the "mass gap" becomes mathematically undefined during the interpolation.

\subsubsection{Reflection Positive Interpolating Action}
We explicitly construct a **Reflection Positive Interpolating Action**. The path must lie within the convex cone of RP-preserving measures.
\begin{definition}[RP-Preserving Action]
We use **Wilson Fermions** with hopping parameter $\kappa$ (which preserves RP) or an **Overlap Fermion** formulation. The action is:
\begin{equation}
...
We explicitly verify the **Linearized Chessboard Bound** for the interpolating action $d\mu_M$:
\begin{equation}
...
The claim of a smooth deformation from $N$ vacua (SYM) to 1 vacuum (Pure YM) violates topological invariants (Witten Index). We explicitly model the **Lifting of Vacuum Degeneracy** using **Pirogov-Sinai Theory**, replacing "global analyticity" with "metastability analysis."
...
\end{equation}
This picks exactly **one** vacuum (e.g., $k=0$) as the global minimum. The other $N-1$ vacua become **metastable** (false vacua).

\subsubsection{Peierls Contours and Stability}
To prove that the system relaxes to the unique vacuum of Pure YM, we must demonstrate stability by defining "contours" (domain walls) separating the true vacuum from false vacuum bubbles. We prove the **Peierls Condition**:
\begin{equation}
