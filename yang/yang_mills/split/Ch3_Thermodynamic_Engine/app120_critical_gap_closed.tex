...
\[
\frac{\lambda_F}{\lambda_0} = \frac{r_F(\beta)}{r_0(\beta)} = \frac{I_1(\beta)}{I_0(\beta)}
\]
where (for $SU(2)$) we use the unified manuscript convention that the normalized first excited eigenvalue is written as the Bessel ratio $\lambda_F(\beta)=I_1(\beta)/I_0(\beta)$.
\\
\emph{Remark (conventions):} Alternative normalizations of the plaquette weight (e.g. absorbing factors of $2$ into the argument of the Wilson action) shift which Bessel indices appear in intermediate formulas; throughout this appendix we keep the ratio $I_1(\beta)/I_0(\beta)$ as the canonical representation of $\lambda_F(\beta)$.
...
For $SU(2)$:
\begin{equation}
\lambda_F(\beta) = \frac{r_F(\beta)}{r_0(\beta)} = \frac{I_1(\beta)}{I_0(\beta)}
\end{equation}
\end{theorem}
...
For $SU(2)$:
\begin{equation}
\gamma_2(\beta) = 1 - \frac{I_1(\beta)}{I_0(\beta)} > 0
\end{equation}
for all $\beta > 0$.
\end{corollary}
...
\end{align}
We write the normalized first excited eigenvalue as $\lambda_F(\beta)=I_1(\beta)/I_0(\beta)$, hence
\begin{equation}
\gamma_2(\beta)=1-\lambda_F(\beta)=1-\frac{I_1(\beta)}{I_0(\beta)}.
\end{equation}
For small $\beta$, one gets $I_1/I_0 = \beta/2 + O(\beta^3)$ and hence $\gamma_2(\beta)=1-\beta/2+O(\beta^3)\ge 1/2$ for $\beta\le 1$.
...
&= \frac{1 - \frac{3}{8\beta} + O(1/\beta^2)}{1 + \frac{1}{8\beta} + O(1/\beta^2)}
= 1 - \frac{1}{2\beta} + O(1/\beta^2).
\end{align}
Therefore $\gamma_2(\beta)=1-I_1/I_0 = \frac{1}{2\beta}+O(1/\beta^2)\ge \frac{1}{4\beta}$ for $\beta\ge 1$.
