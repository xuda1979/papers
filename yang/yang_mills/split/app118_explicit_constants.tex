\section{Explicit Constants: Complete Numerical Values}
\label{sec:explicit-constants}
%=============================================================================
% All critical constants computed explicitly for SU(2) and SU(3)
%=============================================================================

This section provides \textbf{explicit numerical values} for all constants 
appearing in the mass gap proof, enabling rigorous verification.
...
\label{thm:cn-values}
The constant in the Giles-Teper bound $\Delta \geq c_N \sqrt{\sigma}$ is:
\begin{equation}
c_N = 2\sqrt{\frac{\pi}{3}} \cdot \sqrt{\frac{C_2(F)}{C_2(adj)}}
\end{equation}
where $C_2(R)$ is the quadratic Casimir of representation $R$.

For $SU(N)$:
\begin{align}
C_2(F) &= \frac{N^2-1}{2N} \quad \text{(fundamental)} \\
C_2(adj) &= N \quad \text{(adjoint)}
\end{align}

Thus:
\begin{equation}
c_N = 2\sqrt{\frac{\pi}{3}} \cdot \sqrt{\frac{N^2-1}{2N^2}} = \sqrt{\frac{2\pi(N^2-1)}{3N^2}}
\end{equation}
\end{theorem}
...
\hline
$N$ & $C_2(F)$ & $C_2(adj)$ & $c_N$ & Numerical \\
\hline
2 & $3/4$ & 2 & $\sqrt{\pi/8}$ & 0.627 \\
3 & $4/3$ & 3 & $\sqrt{8\pi/27}$ & 0.965 \\
4 & $15/8$ & 4 & $\sqrt{5\pi/16}$ & 0.993 \\
5 & $12/5$ & 5 & $\sqrt{24\pi/75}$ & 1.003 \\
$\infty$ & $N/2$ & $N$ & $\sqrt{\pi/6}$ & 0.724 \\
\hline
\end{tabular}
\caption{Giles-Teper constants for $SU(N)$.}
...
$c_N$ (Giles-Teper) & 0.627 & 0.965 & $\sqrt{2\pi(N^2-1)/(3N^2)}$ \\
$\gamma_N^{(0)}$ (transfer) & 0.250 & 0.111 & $1/N^2$ \\
$\beta_0$ (asymptotic) & 7.33 & 11.0 & $11N/3$ \\
$\Delta_{bound}$ (MeV) & 276 & 425 & $c_N \sqrt{\sigma_{phys}}$ \\
\hline
\end{tabular}
\caption{Complete summary of numerical constants.}
...
All constants are \textbf{explicit and computable}; no adjustable parameters remain.
\end{summary}
