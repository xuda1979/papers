...
\textbf{Step 1: Strong-to-weak interpolation.}

At strong coupling: $\sigma_{lattice} = -\ln\beta + O(1)$.

At weak coupling: By asymptotic freedom, $\sigma_{phys} = c\Lambda^2$ is fixed.

The lattice-physical relation is:
\[
\sigma_{lattice} = a^2 \cdot \sigma_{phys} = \frac{\sigma_{phys}}{\Lambda^2} \cdot e^{-\beta/(\beta_0 N)}
\]

\textbf{Step 2: Analyticity.}

$\sigma_{lattice}(\beta)$ is analytic for $\beta > 0$ (no phase transitions).

The strong and weak coupling limits must connect smoothly.

\textbf{Step 3: Dimensional transmutation.}

The only way to get a $\beta$-independent physical quantity is:
\[
\sigma_{phys} = \sigma_{lattice}(\beta) \cdot a^{-2} = \text{const} \cdot \Lambda^2
\]
