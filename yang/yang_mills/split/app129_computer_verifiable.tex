...
\begin{equation}
\boxed{c_N = \sqrt{\frac{2\pi(N^2-1)}{3N^2}}}
\end{equation}
...
\hline
2 & $\sqrt{\pi/2}$ & 1.253 \\
3 & $\sqrt{16\pi/27}$ & 1.362 \\
4 & $\sqrt{5\pi/8}$ & 1.401 \\
$\infty$ & $\sqrt{2\pi/3}$ & 1.447 \\
\hline
\end{tabular}
\end{center}
...
for N in [2, 3, 4, 10, 100]:
    c_N = np.sqrt(2*np.pi*(N**2-1)/(3*N**2))
    print(f"c_{N} = {c_N:.4f}")
\end{verbatim}
\end{inequality}
...
\item[$\square$] Giles-Teper: $c_N = \sqrt{2\pi(N^2-1)/(3N^2)}$ matches lattice data
