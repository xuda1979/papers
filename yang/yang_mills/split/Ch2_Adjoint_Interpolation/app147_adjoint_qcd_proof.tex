...
\begin{proof}
This theorem is a corollary of the \textbf{Tube Contraction Theorem} (Theorem~\ref{thm:crossover_contraction}, also known as Theorem K.1 in Appendix~\ref{sec:rigorous_proof}). The Interval Arithmetic verification establishes that the renormalization group flow maps the compact set $\mathcal{T}$ of effective actions into its interior for all $m \in (0, \infty)$, rigorously ruling out critical points and phase transitions. The preservation of center symmetry (Theorem~\ref{thm:adjoint-center}) combined with the contraction property proves adiabatic continuity unconditionally.
\end{proof}
