...
\begin{proof}
    The proof relies on the cluster expansion (or high-temperature expansion). We expand the Boltzmann weight in characters of the group $G$:
    \begin{equation}
...
\begin{theorem}[Strict Positivity of String Tension]
...
\begin{proof}
    ...
    \begin{enumerate}
        \item \textbf{Strong Coupling ($\beta < \beta_c$):} By the Cluster Expansion (Theorem 2), $\sigma(\beta) \approx -\ln \beta > 0$.
        \item \textbf{Weak Coupling ($\beta > \beta_*$):} By Balaban's UV stability bounds and Asymptotic Freedom, the string tension scales as $\sigma(\beta) \sim e^{-c\beta}$, which is strictly positive.
        \item \textbf{Intermediate Coupling ($\beta_c \le \beta \le \beta_*$):} We use RP Monotonicity. The free energy density $f(\beta)$ is concave, and the string tension $\sigma(\beta)$ is monotonically decreasing. To rule out a phase transition (where $\sigma(\beta)$ could vanish), we utilize the \textbf{Adjoint QCD Interpolation}.
        
        We embed the pure gauge theory into a lattice gauge theory with heavy adjoint scalars. This extended theory possesses exact center symmetry. The mass $M$ of the scalars serves as an interpolating parameter. For finite $M$, the theory is analytic. Taking the limit $M \to \infty$ recovers the pure gauge theory. Since the analyticity is protected by the symmetry and the absence of local order parameters (Elitzur's Theorem), no phase transition occurs in the finite $\beta$ regime.
    \end{enumerate}
...
\begin{proof}
    ...
    \end{equation}
    Since confinement (area law) persists in the continuum limit (as proven by the stability of the effective string tension under RG flow), $\sigma$ remains finite and positive in physical units.
