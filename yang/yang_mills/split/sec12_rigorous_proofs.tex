...
\section{Proof structure (programmatic): the adjoint interpolation strategy}
\label{sec:rigorous-proofs}

This section describes a \emph{proposed modular strategy} toward a proof of the four-dimensional Yang--Mills mass gap.
In the rigorous-scope version of the manuscript, these modules are treated as \textbf{conditional} unless each cited appendix provides a complete, independently checkable proof.
See Appendix~\ref{sec:open-problems} for the current list of missing inputs.
...
\subsection{Module 1: Supersymmetric anchor (conditional)}
Appendix~\ref{app:sym-gap-proof} is intended to supply an anchor statement asserting that $\mathcal{N}=1$ SYM is gapped.
\begin{itemize}
    \item \textbf{Status note:} The Witten index is a powerful topological invariant, but promoting it to a fully constructive lattice proof of a spectral gap requires careful control of lattice regularization and the thermodynamic/continuum limits.
\end{itemize}

\subsection{Module 2: Interpolation to pure Yang--Mills (conditional)}
Appendix~\ref{app:adjoint-proof} proposes an interpolation from an adjoint-fermion theory to pure Yang--Mills.
\begin{itemize}
    \item \textbf{Status note:} Exact center symmetry does not by itself exclude all bulk transitions.
    \item \textbf{Status note:} Complex deformations and zero-free regions must be established without assuming the very gap/uniqueness properties one aims to prove.
\end{itemize}

\subsection{Module 3: Continuum limit (conditional)}
Appendix~\ref{sec:continuum-limit-mosco} and Appendix~\ref{app:gap3-balaban-fermions} are intended to control UV behavior and take a continuum limit.
\begin{itemize}
    \item \textbf{Status note:} A fully rigorous OS reconstruction and a proof that a positive lattice gap persists in the scaling limit require uniform (in lattice spacing) estimates.
\end{itemize}

\subsection{Conclusion}
These modules describe a \emph{strategy}.
No complete unconditional proof of the 4D Yang--Mills mass gap is claimed in this version.

% End of Guide

\begin{theorem}[Free Energy Convexity and String Tension Monotonicity]
\label{thm:free-energy-convexity}
	extnormal{[PARTIAL/CONDITIONAL.]}
The finite-volume free energy $f_\Lambda(\beta)=-(1/|\Lambda|)\log Z_\Lambda(\beta)$ has standard convexity/concavity properties in $\beta$.
However, deriving an all-coupling monotonicity statement for the infinite-volume string tension $\sigma(\beta)$ in 4D non-Abelian $SU(N)$ requires additional inputs and is treated as an open problem.
\end{theorem}

\begin{proof}
The earlier draft of this section incorrectly invoked GKS/Griffiths-type inequalities for non-Abelian $SU(N)$ Wilson loop observables.
Such inequalities are not available in the required form.
Accordingly, we treat monotonicity of $\sigma(\beta)$ beyond the strong-coupling regime as an open problem/conditional input (Appendix~\ref{sec:open-problems}).
\end{proof}

\begin{theorem}[Convexity and Monotonicity]
\label{thm:rp-comparison}
\label{thm:rp-monotonicity}
The free energy density (when it exists) is concave in $\beta$ and the average plaquette action $\langle S_p \rangle_\beta$ is non-increasing in $\beta$.
No claim is made here that this implies an all-coupling monotonicity theorem for the string tension.
\end{theorem}
...
Controlling the interpolation between these regimes without additional nonperturbative input is an open problem.
\end{proof}

\begin{remark}[Replacement for FKG (limited)]
...
Any use of additional monotonicity principles for confinement-related observables must be stated as an explicit hypothesis.
\end{remark}

\begin{theorem}[String tension positivity for all $\beta$ \textnormal{[OPEN]}]
...
\emph{Open problem.} Prove $\sigma(\beta)>0$ for all finite $\beta$ in 4D pure $SU(N)$ Yang--Mills in infinite volume.
This manuscript only proves $\sigma(\beta)>0$ in a strong-coupling regime $0<\beta<\beta_0$.
\end{theorem}

\begin{proof}
This is recorded as an open problem.
The earlier draft sketched a three-stage argument (strong coupling anchor + an all-coupling monotonicity principle + weak-coupling scaling), but the required nonperturbative inputs are not established at a fully rigorous level.
\end{proof}

\begin{remark}[Status]
In the rigorous-scope version, only the strong-coupling confinement estimates are treated as proved.
All-coupling confinement/scale-setting steps are tracked as open obligations in Appendix~\ref{sec:open-problems}.
\end{remark}
...
\label{thm:block-zeg}
\emph{Open problem.} Establish a log-Sobolev inequality with a constant $\rho>0$ that is uniform in volume and controlled along a scaling trajectory.
\end{theorem}

\begin{remark}[Why this matters]
A uniform LSI is one possible route to uniform mixing/correlation control, but proving it in 4D non-Abelian gauge theory without circularity remains open.
\end{remark}
...
\label{thm:absolute-no-transition}
