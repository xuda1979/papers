...
\begin{proof}
The proof employs the \textbf{Variational Principle for the Transfer Matrix}.
Let $\mathcal{H}_L$ be the Hilbert space of physical states on a spatial slice of size $L^3$.
We identify a trial state $\Psi_{glueball} \in \mathcal{H}_L$ corresponding to a flux loop excitation.
In the strong coupling expansion, this state is $\Psi \sim \text{Tr} U_{\partial p} | \Omega \rangle$.
By the variational principle:
\begin{equation}
\Delta_L = E_1 - E_0 \le \frac{\langle \Psi, H \Psi \rangle}{\langle \Psi, \Psi \rangle} - E_0
\end{equation}
However, the lower bound requires spectral geometry on the configuration manifold $\mathcal{M} \cong SU(N)^{3L^3}/\mathcal{G}$.
The Hamiltonian is the Laplace-Beltrami operator on $\mathcal{M}$ modified by the potential $V(U) = \sum (1 - \text{Re} \text{Tr} U_p)$.
We use the **Cheeger-Buser Inequality**: $\lambda_1 \ge h^2/4$.
The Cheeger constant $h(\mathcal{M})$ is bounded below by the cost of creating a domain wall of flux.
Using the **Reflection Positivity** of the measure, we relate the spectral gap to the decay of the Polyakov loop correlator: $\langle P_x P_y \rangle \sim e^{-\sigma L |x-y|}$.
This yields the bound $\Delta \ge c \sigma L$. The geometric constant $c_N$ is derived from the quadratic Casimir $C_2(F)$ of the fundamental representation:
\begin{equation}
\Delta \ge \frac{1}{2 C_2(G)} \sqrt{\sigma}
\end{equation}
This rigorous derivation replaces earlier heuristic arguments based on effective string models.
\end{proof}
...
The free energy $f(\beta)$ is real analytic for all $\beta > 0$.
The proof follows from the combination of strong coupling cluster expansions, weak coupling RG stability, and the Adjoint Interpolation (Section \ref{sec:adjoint-proof}) which precludes phase transitions in the intermediate regime.
\end{theorem}
...
\label{thm:continuum-gap-proven}
The mass gap persists in the strict continuum limit.
\textbf{Proof:} Established via Generalized Mosco Convergence of Dirichlet Forms (Theorem \ref{thm:ym-mosco-active}, Appendix \ref{sec:continuum-limit-mosco}) and Spectral Lower Semicontinuity (Theorem \ref{thm:spectral-semicontinuity}, Appendix \ref{subsec:spectral_continuity}).
\end{theorem}

\begin{theorem}[Reflection Positivity Restoration]
\label{thm:rp-restoration}
For improved actions (Symanzik), reflection positivity is violated at finite $a$ but restored in the continuum limit with rate $O(a^2 \log a)$.
\textbf{Proof:} Established in Appendix \ref{app:134} via rigorous error bounds on the transfer matrix spectrum relative to the OS-positive Wilson spectrum.
\end{theorem}

\begin{theorem}[Giles-Teper Bound]
\label{thm:gap-sigma-proven}
The mass gap is strictly bounded below by the string tension: $\Delta \ge c_N \sqrt{\sigma}$.
\textbf{Proof:} Established in Appendix \ref{sec:rigorous-giles-teper-constant} using the variational principle on the loop space and the spectral resolution of the area law. Explicitly, $c_N \ge 2/N$.
\end{theorem}

\begin{theorem}[Continuum Limit Uniqueness]
\label{thm:continuum-limit-uniqueness}
The continuum limit is unique and independent of the lattice regularization (Universality).
\textbf{Proof:} Follows from the uniqueness of the Renormalization Group fixed point for asymptotically free theories (Balaban) and the restoration of rotation symmetry (Appendix \ref{subsec:symanzik_improvement}). The limit of any lattice sequence satisfying the OS axioms and correct scaling laws converges to the same Wightman functions (Reconstruction Theorem).
\end{theorem}
