...
\]
This is a smooth function with Hessian of order $\beta$. While the local stiffness increases as $O(\beta)$, the global spectral gap $\Delta$ is non-perturbative and scales as $e^{-c\beta}$. The single-link gap provides a bounds for local updates but the global gap is determined by tunneling events (instantons).

The uniform lower bound requires the Giles-Teper bound combined with correct string tension scaling.
\end{proof}
...
\item \textbf{Giles-Teper bound:} $\Delta(\beta) \geq c_N\sqrt{\sigma(\beta)}$ with $c_N \geq 2/N$.
