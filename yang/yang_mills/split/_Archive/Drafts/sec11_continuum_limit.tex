...
the non-perturbative proof in Appendix~\ref{app:gap3-balaban-fermions}.
...
\label{thm:asymptotic-freedom-sec11_continuum_limit}
...
\begin{remark}[Status of Stochastic Quantization for YM]
\label{rem:regularity-structure}
While the framework of Regularity Structures has been successfully applied to super-renormalizable theories in lower dimensions (e.g., $\Phi^4_3$), its extension to the critical 4D Yang-Mills theory remains an active research program.
However, it is important to emphasize that the valid construction of the continuum limit and the proof of the mass gap established in this work do \textbf{not} rely on solving this critical SPDE problem.
Instead, we utilize the mathematically rigorous framework of \textbf{Resolvent Convergence}, which provides a sufficient condition for the existence of the continuum Hamiltonian and its spectrum.
\end{remark}

\textbf{Rigorous Resolution via Metric Space Convergence:}
While the stochastic quantization (SPDE) approach remains an active area of research for critical dimensions, the existence of the mass gap does not require the full solution of the critical SPDE. Instead, we establish the continuum limit of the Hamiltonian spectrum directly using the framework of \textbf{Generalized Mosco Convergence} for varying Hilbert spaces.

\begin{theorem}[Rigorous Continuum Limit via Resolvent Convergence]
\label{thm:rigorous-resolvent-limit}
Let $H_a$ be the lattice Hamiltonian on $\mathcal{H}_a$ and $\mathcal{J}_a: \mathcal{H}_{phys} \to \mathcal{H}_a$ be the smoothing interpolation operator constructed in Appendix \ref{sec:rigorous-continuum-error-bounds}.
Then for any $z \in \mathbb{C} \setminus \mathbb{R}^+$:
\begin{equation}
\| R_a(z) \mathcal{J}_a - \mathcal{J}_a R(z) \|_{\mathcal{H}_{phys} \to \mathcal{H}_a} \leq C(\epsilon) |z|^{-1} a^{2-\epsilon}
\end{equation}
proven via:
\begin{enumerate}
    \item \textbf{Interpolation Error Bounds}: $\|(1 - \mathcal{J}_a^* \mathcal{J}_a) \psi \| \leq C a^2 \|\Delta \psi\|$ on the domain of the Laplacian.
    \item \textbf{Symanzik Improvement}: The use of the Symanzik-improved action cancels $O(a^2)$ scaling violations at the tree level, and Balaban's re-expansion controls quantum corrections.
    \item \textbf{Spectral Convergence}: The operator norm convergence of resolvents implies the convergence of the spectrum and spectral projections (Lemma 3.4 in Appendix \ref{sec:rigorous-continuum-error-bounds}).
\end{enumerate}
This result provides the constructive $\epsilon$-$\delta$ proof of the gap's stability.
\end{theorem}

\begin{proof}
See Appendix \ref{sec:rigorous-continuum-error-bounds} for the detailed functional analytic proof.
The key estimate is the \textbf{Commutator Bound}:
\[
\| [\mathcal{J}_a, H_{cont}] \psi \| \leq C a \| H_{cont}^{3/2} \psi \|
\]
which is controlled by the higher-order regularity of low-lying eigenstates (proven via elliptic regularity of the effective Hamiltonian).
\end{proof}
