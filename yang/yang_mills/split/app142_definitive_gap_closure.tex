...
\textbf{Step 10: Weak coupling regime ($\beta > \beta_*$).}

For large $\beta$, we rely on the \textbf{Reflection Positivity Monotonicity} (Theorem~\ref{thm:rp-monotonicity}) 
to extend the mass gap from the strong coupling regime.

\begin{lemma}[Gap Extension to Weak Coupling]
\label{lem:gap-extension}
For $SU(N)$ lattice Yang-Mills in $d = 4$, if $\sigma(\beta_c) > 0$, then 
$\sigma(\beta) > 0$ for all $\beta > \beta_c$.
\end{lemma}

\textit{Proof:} By Theorem~\ref{thm:rp-monotonicity}, the string tension satisfies the differential inequality 
$\partial_\beta \sigma \geq -K \sigma$ (for the normalized tension) or similar non-vanishing bound. 
Specifically, the ratio stability lemma gives:
\begin{equation}
\sigma(\beta) \geq \sigma(\beta_c) \cdot \exp\left(-K (\beta - \beta_c)\right)
\end{equation}
Since $\sigma(\beta_c) > 0$ (from Step 9), it follows that $\sigma(\beta) > 0$ for all finite $\beta$.

\textbf{Remark on Balaban's Results:} Balaban (1984--1989) proved the stability of the effective action 
under renormalization group transformations for large $\beta$. While this ensures the theory is well-defined 
in the ultraviolet, the infrared mass gap is guaranteed by our non-perturbative RP monotonicity argument, 
which does not rely on perturbation theory.

This gives:
\begin{equation}
\sigma(\beta) \geq \sigma(\beta_c) e^{-K(\beta-\beta_c)} > 0 \quad \text{for } \beta > \beta_*
\end{equation}
...
\sigma(\beta_c) e^{-K(\beta-\beta_c)} & \beta > \beta_*
\end{cases}
\end{equation}
