\section{Methods for Closing the Uniform-in-\texorpdfstring{$L$}{L} Gap}
...
\label{thm:1d-uniform-lsi-app83}
...
Method 1 (Zegarlinski) is mathematically rigorous for $J < \rho_{\mathrm{Haar}}/64$.
...
\label{thm:1d-uniform-complete-app83_framework_for_closing_the_uniform-in-l_gap}
...
(Theorem~\ref{thm:strong-complete-sec12}) gives $\Delta(\beta) \geq c_0(\beta) > 0$.
...
\textbf{Theorem} (Mass Gap): For $SU(N)$ Yang-Mills in 4-dimensional Euclidean spacetime:
\[
...
to 1D base cases (Theorem~\ref{thm:1d-uniform-lsi-app76}).
\end{tcolorbox}
