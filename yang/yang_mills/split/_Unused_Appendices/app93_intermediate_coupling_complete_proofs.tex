...
\label{sec:intermediate-complete}
%=============================================================================

This section provides \textbf{mathematically rigorous proofs} for all lemmas and theorems 
...
\textbf{Step 3: LSI Constant for SU(N).}

For $SU(N)$ with the standard metric normalization, the correct LSI constant is:
\[
\rho_N = \frac{N^2-1}{2N^2}
\]
See Appendix~\ref{sec:explicit-constants} for the detailed computation.
...
\[
\rho_{int}(B_\alpha) \geq \frac{N^2-1}{2N^2} \cdot e^{-2 \cdot 6\beta\ell^4}
\]
...
By Corollary~\ref{cor:lsi-sun-d}, this satisfies LSI with $\rho_0 = \frac{N^2-1}{2N^2}$.
...
We must precisely identify the degrees of freedom subject to variation. The conditional measure fixes $U_\Gamma$, so only interior plaquettes contribute to the oscillation when varying $U_{int}$.
...
The bound $\rho_{int} \geq \frac{N^2-1}{2N^2} e^{-12\beta\ell^4}$ depends only on $N$, $\beta$, and $\ell$.
...
of $m$ sites satisfies LSI with constant at least $\rho_* = \frac{N^2-1}{2N^2}e^{-8\beta}$.
...
\[
\rho_4 \geq \frac{N^2-1}{2N^2} \exp\left(-C_1\beta - C_2\beta\ell - C_3\beta\ell^2 - C_4\beta\ell^3\right)
\]
