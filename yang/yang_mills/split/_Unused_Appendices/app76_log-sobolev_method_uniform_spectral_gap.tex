...
\label{sec:hierarchical-lsi-app76_log-sobolev_method_uniform_spectral_gap}
...
\label{thm:conditional-tensorization-app76}
...
\textbf{Step 4 (Retracted): Iterative Dimensional Reduction.}
The proposed strategy of iterating the Zegarlinski-tensorization from $d \to (d-1) \to \dots \to 1$ is \textbf{invalid} for this problem.
Renormalization Group scaling implies that the effective couplings on the lower-dimensional boundaries do not remain small or local. The problem cannot be reduced to a 1D chain without creating non-local effective interactions that violate the assumptions of the 1D LSI Lemma.

\textbf{Correction:} The proof must stop at the conditional tensorization step (Level 1) and use \textbf{Cluster Expansion} for the effective boundary action, establishing weak coupling via high-temperature methods directly in the $(d-1)$-dimensional boundary, rather than reducing to 1D.
...
By conditional tensorization (Theorem~\ref{thm:conditional-tensorization-sec12}):
\[
...
provide a complete, detailed proof using the transfer matrix method.
...
For $SU(N)$ lattice gauge theory, the 1D LSI constant satisfies:
\[
...
The conditional tensorization theorem is the key tool bypassing the restrictive Zegarlinski criterion. We replace the heuristic argument with a two-scale LSI with explicit influence correction.

Let the full configuration split as $(x,y)\in \Omega_\Sigma\times \Omega_{\mathrm{int}}$. Let
\[
\mu(dx,dy)=\mu_\Sigma(dx)\mu(dy|x).
\]

\begin{definition}[Boundary force variance]
For a boundary coordinate direction $v$ (a unit tangent vector at $x$ in the Riemannian metric on $\Omega_\Sigma$), define the \emph{centered boundary force} under the interior conditional:
\[
F_v(x,y) := \langle v,\nabla_x H(x,y)\rangle - \mu(\langle v,\nabla_x H(x,Y)\rangle\mid x),
\]
and define the uniform variance proxy
\[
\sigma^2
:=\sup_{x}\sup_{|v|=1}\mathrm{Var}_{\mu(\cdot|x)}\bigl(F_v(x,Y)\bigr).
\]
\end{definition}

\begin{theorem}[Two-scale LSI with explicit influence correction]
\label{thm:conditional-tensorization-corrected}
Assume:
\begin{enumerate}
\item \textbf{Uniform interior conditional LSI}: for all $x$, the conditional $\mu(dy|x)$ satisfies LSI with constant $\rho_{\mathrm{int}}>0$ (uniform in $x$).
\item \textbf{Boundary marginal LSI}: $\mu_\Sigma$ satisfies LSI with constant $\rho_\Sigma>0$.
\item The influence proxy $\sigma^2$ is finite.
\end{enumerate}

Then the full measure $\mu$ satisfies an LSI with constant
\[
\rho_{\mathrm{full}}
\ge
\min\Bigl\{\frac{\rho_\Sigma}{1+\eta}, \frac{\rho_{\mathrm{int}}}{1+\frac{\sigma^2}{\eta\rho_\Sigma}}\Bigr\}
\quad\text{for every }\eta>0.
\]
In particular, choosing $\eta:=\sigma/\sqrt{\rho_\Sigma\rho_{\mathrm{int}}}$ (or $\eta=1$) yields an explicit strictly-positive bound whenever $\sigma^2<\infty$.
\end{theorem}

\begin{proof}
Let $h=h(x,y)$ be smooth. Let $g(x):=|h(x,\cdot)|_{L^2(\mu(\cdot|x))}=\Bigl(\int h^2 d\mu(\cdot|x)\Bigr)^{1/2}$. Then
\[
\mathrm{Ent}_\mu(h^2)
=
\mathbb E_{\mu_\Sigma}\bigl[\mathrm{Ent}_{\mu(\cdot|x)}(h^2)\bigr]
+
\mathrm{Ent}_{\mu_\Sigma}(g^2).
\]
By uniform interior LSI:
\[
\mathbb E_{\mu_\Sigma}\bigl[\mathrm{Ent}_{\mu(\cdot|x)}(h^2)\bigr] \le \frac{2}{\rho_{\mathrm{int}}}\int |\nabla_y h|^2 d\mu.
\]
By boundary LSI:
\[
\mathrm{Ent}_{\mu_\Sigma}(g^2) \le \frac{2}{\rho_\Sigma}\int |\nabla_x g|^2 d\mu_\Sigma.
\]
We control $|\nabla_x g|^2$. Fix $x$ and unit tangent $v$. Differentiating $g^2(x)=\int h^2 d\mu(\cdot|x)$:
\[
\partial_v g^2(x)
=
2\int h \partial_v h d\mu(\cdot|x)
-\mathrm{Cov}_{\mu(\cdot|x)}(h^2,F_v).
\]
Using $\partial_v g=(\partial_v g^2)/(2g)$ and Cauchy-Schwarz:
\[
|\partial_v g|
\le
\Bigl|\int \frac{h}{g}\partial_v h d\mu(\cdot|x)\Bigr|
+
\frac{1}{2g}|\mathrm{Cov}(h^2,F_v)|.
\]
The first term squared is $\le \int|\partial_v h|^2 d\mu(\cdot|x)$ by Cauchy-Schwarz.
For the second term, use conditional Poincaré (implied by LSI):
\[
\mathrm{Var}_{\mu(\cdot|x)}(h^2) \le \frac{4}{\rho_{\mathrm{int}}}\int h^2|\nabla_y h|^2 d\mu(\cdot|x).
\]
Thus
\[
|\mathrm{Cov}(h^2,F_v)|^2 \le \mathrm{Var}(h^2)\sigma^2 \le \frac{4\sigma^2}{\rho_{\mathrm{int}}}g^2\int|\nabla_y h|^2 d\mu(\cdot|x).
\]
Combining and using $(a+b)^2 \le (1+\eta)a^2 + (1+\eta^{-1})b^2$:
\[
|\nabla_x g|^2
\le
(1+\eta)\int|\nabla_x h|^2 d\mu(\cdot|x)
+
(1+\eta^{-1})\frac{\sigma^2}{\rho_{\mathrm{int}}}\int|\nabla_y h|^2 d\mu(\cdot|x).
\]
Integrating and substituting back gives the result.
\end{proof}

\textbf{Explicit evaluation of $\sigma^2$:}
For the Wilson gauge action, $\nabla_x H$ is a sum of a bounded number of plaquette forces. For the fermion determinant, the derivative with respect to a boundary link is uniformly bounded for $M>2d$. Thus $\sigma^2 < \infty$ uniformly in volume.
...
For Clay mathematical rigor standards, we specify the computer-assisted verification 
...
estimates, avoiding exponential degradation (Section~\ref{sec:variance-transport-sec12}).
...
\label{sec:variance-transport-app76}
%-----------------------------------------------------------------------------

We replace the oscillation-based Holley-Stroock bounds with a variance-based argument that avoids exponential degradation with boundary size.

\begin{lemma}[Variance-based transport bound via Poincaré]
\label{lem:variance-transport}
Let $(\mathcal M,g)$ be a compact Riemannian manifold and let $\{\mu_t\}_{t\in[0,1]}$ be a smooth path of probability measures with densities $\mu_t(dz)=\frac{1}{Z_t}e^{-H_t(z)}dz$.
Assume each $\mu_t$ satisfies a Poincaré inequality with a uniform constant $\lambda>0$.
Then for every 1-Lipschitz $f$ on $(\mathcal M,g)$,
\[
W_1(\mu_0,\mu_1)
\le
\frac{1}{\sqrt{\lambda}}\int_0^1 \sqrt{\mathrm{Var}_{\mu_t}(\dot H_t)} \,\mathrm{d}t.
\]
\end{lemma}

\begin{proof}
Differentiate $\mu_t(f)$: $\frac{d}{dt}\mu_t(f)=-\mathrm{Cov}_{\mu_t}(f,\dot H_t)$.
Then $|\mathrm{Cov}(f,\dot H_t)| \le \frac{1}{\sqrt{\lambda}}\|\nabla f\|_{L^2(\mu_t)}\sqrt{\mathrm{Var}(\dot H_t)} \le \frac{1}{\sqrt{\lambda}}\sqrt{\mathrm{Var}(\dot H_t)}$ since $|\nabla f|\le 1$. Integrate in $t$.
\end{proof}

\begin{corollary}[Explicit Lipschitz dependence on boundary conditions]
Apply Lemma~\ref{lem:variance-transport} to conditional interior measures $\mu(\cdot|x)$ when $x$ varies along a geodesic $x_t$. Then
\[
W_1\bigl(\mu(\cdot|x_0),\mu(\cdot|x_1)\bigr)
\le
\frac{\sigma}{\sqrt{\lambda_{\mathrm{int}}}} d(x_0,x_1),
\]
where $\sigma$ is the boundary-force variance proxy and $\lambda_{\mathrm{int}}$ is the interior conditional spectral gap.
\end{corollary}

This gives a clean quantitative bound on how the conditional interior law changes when boundary conditions change, paying only a quadratic price in the variance proxy rather than an exponential oscillation penalty.
...
For $\beta > \beta_G$, the Log-Sobolev constant satisfies:
\[
...
\[
\prod_{k=0}^{k^*} \left(1 - \frac{C_{\mathrm{RG}}}{(\beta^{(k)})^2}\right) \geq 
