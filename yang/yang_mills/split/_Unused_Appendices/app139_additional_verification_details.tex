...
\textbf{The Fix:} Replace the variational argument with \textbf{Temple's Inequality} or \textbf{Operator Monotonicity} is insufficient. The correct approach is \textbf{Spectral Representation}.

\begin{itemize}
    \item \textbf{Modification:} We rely on the established implication: Exponential decay of Euclidean correlators $\implies$ Mass Gap.
    \item \textbf{Theorem:} If $|\langle \Omega, \mathcal{O}(0) \mathcal{O}(t) \Omega \rangle_c| \le C e^{-m t}$, then the spectrum of the Hamiltonian $H$ has a gap $\Delta \ge m$.
    \item \textbf{Implementation:} We prove the exponential decay of the \textit{boundary-boundary} correlation function using the Reflection Positivity of the transfer matrix kernel.
    \item \textbf{Area Law Implication:} We derive the mass gap directly from the Area Law for Wilson loops, $W(R,T) \le e^{-\sigma RT}$. By taking $R$ large, the lowest excitation energy $E(R)$ behaves as $\sigma R$, but for fixed local operators (glueballs), we bound them by $c \sqrt{\sigma}$.
\end{itemize}
...
\subsection{Resolving the "Vacuum Energy" Curvature Sign Error}
\textbf{The Issue:} The manuscript uses a "Regularized Determinant" to control fermions/ghosts, leading to a vacuum energy term in the effective action. The text claims this regularized constant is positive.
\textbf{The Error:} The vacuum energy (Casimir energy) contribution to the Hessian is typically negative (attractive force), which subtracts from the Bakry-Émery curvature. Relying on its positivity is fatal.
\textbf{The Fix:} Use \textbf{O'Neill's Curvature Formula}.

\begin{itemize}
    \item \textbf{Modification:} Abandon the attempt to use the potential energy for convexity.
    \item \textbf{The Theorem to Prove:} Prove that the \textbf{Geometric Curvature} of the bundle (the O'Neill tensor term $\frac{3}{4} \|[X,Y]\|^2$) is strictly positive and dominates the negative fluctuations of the Hessian.
    \item This requires deriving the \textbf{Gårding Inequality} for the shifted operator: $\text{Hess}(S_{eff}) \ge c (\nabla A)^2 - C_{vac}$, and showing that the kinetic/curvature terms control the instability.
\end{itemize}
...
\textbf{The Issue:} The problem of gauge fixing in non-perturbative Yang-Mills theory involves the Gribov ambiguity (existence of multiple gauge copies).
\textbf{The Error:} Trying to define the theory via a single "Fundamental Modular Region" $\Omega$ leads to contradictions about whether the measure concentrates on the boundary (horizon) or the interior. The Gribov-Zwanziger (GZ) approach imposes a hard horizon cutoff which modifies the spectrum (complex poles).
\textbf{The Fix:} Use \textbf{Stochastic Quantization (Parisi-Wu)} as the primary definition.

\begin{itemize}
    \item \textbf{Modification:} We decline to use the Gribov-Zwanziger action $S_{GZ}$ as it modifies the fundamental theory by soft BRST breaking. Instead, we define the physical measure as the unique equilibrium distribution of the \textbf{Stochastic Quantization} process (Langevin equation).
    \item \textbf{The Theorem to Prove:} We rely on the property that the stochastic process "diffuses along gauge orbits" while restoring gauge-invariant quantities correctly (as proven by Zwanziger for drift processes, noting that observables are gauge invariant). This avoids the need for a rigid Gribov horizon constraint in the action itself. The massive spectrum is derived from the decay rates of the stochastic correlation functions, not from the poles of a gauge-fixed propagator.
\end{itemize}
...
\textbf{The Derivation:}
\textbf{Step 1: Relating LSI to the Generator Gap}

We must be precise about which operator the LSI controls.
\begin{lemma}[LSI $\Rightarrow$ Poincaré $\Rightarrow$ Markov Spectral Gap]
If a measure $\mu$ satisfies LSI($\rho$) in the rate form $\mathrm{Ent}_\mu(f^2) \le \frac{2}{\rho}\mathcal{E}(f,f)$, then $\mu$ satisfies the Poincaré inequality:
\begin{equation}
\mathrm{Var}_\mu(f) \le \frac{1}{\rho}\mathcal{E}(f,f).
\end{equation}
This implies that the spectral gap of the associated reversible Markov generator $L$ is at least $\rho$.
\end{lemma}
\emph{Note:} This is a gap for the Markov generator associated to the Dirichlet form, \textbf{not} automatically the physical Hamiltonian $H = -\frac{1}{a}\log T_a$. The identification requires a bridge theorem (e.g., relating stochastic quantization to Euclidean transfer matrix), or one must prove the gap directly for the transfer matrix via reflection positivity and correlation decay.

\textbf{Step 2: From Transfer Matrix Eigenvalues to Mass Gap}
To avoid ambiguity, we rely on the elementary relation between the eigenvalue and the generator gap.

\begin{lemma}[Elementary Bounds for the Logarithm]
For $0 < \lambda < 1$, we have $1 - \lambda \le -\log \lambda \le \frac{1-\lambda}{\lambda}$.
Hence, defining the physical mass gap $m_a = -\frac{1}{a}\log \lambda_1(a)$:
\begin{equation}
m_a \ge \frac{1-\lambda_1(a)}{a}.
\end{equation}
\end{lemma}
Thus, a sufficient condition for a non-vanishing physical mass gap $m \ge c$ is that the spectral gap of the transfer matrix satisfies $1 - \lambda_1(a) \ge c \cdot a$ uniformly.

\textbf{Step 3: Convergence}
Mosco convergence of the quadratic forms ensures that if these uniform bounds hold, the limiting Hamiltonian retains the spectral gap.
...
\subsection{Rigorous Control of Adjoint Fermion Quasi-Locality}
...
\textbf{Setup.} Let $\Lambda\subset\mathbb Z^d$ be finite. Let $E(\Lambda)$ be the directed nearest-neighbor edges. Let $U=\{U_e\}_{e\in E(\Lambda)}$ be gauge links, and let $R=\mathrm{Ad}$ be the adjoint representation (so $U_e$ is unitary on $\mathbb C^{\dim\mathrm{Ad}}$, $\dim\mathrm{Ad}=N^2-1$). Let $D_W(U)$ be a nearest-neighbor Wilson–Dirac operator acting on $\ell^2(\Lambda)\otimes\mathbb C^{4}\otimes\mathbb C^{N^2-1}$, with mass $M>0$.

Define $H(U)$ to be the purely hopping part normalized so that
\[
D_W(U)+M = M\bigl(I-\kappa H(U)\bigr),\qquad \kappa:=\frac1M.
\]
...
\[
\|H(U)\|_{\mathrm{op}} \le 2d,
\]
uniformly in $U$.

\begin{theorem}[Correct quasi-local loop expansion]
\label{thm:quasi-locality-corrected}
Assume
\[
\kappa \|H(U)\|_{\mathrm{op}} < 1\qquad\text{(in particular, it suffices that }M>2d\text{)}.
\]
Then:
\begin{enumerate}
\item The logarithm of the fermion determinant admits the absolutely convergent expansion
   \[
   \log\det(D_W(U)+M)
   =
   |\Lambda|\cdot\log M - \sum_{n\ge 1}\frac{\kappa^n}{n}\mathrm{Tr}\bigl(H(U)^n\bigr).
   \]

\item Each trace $\mathrm{Tr}(H^n)$ is a finite sum over \textbf{closed oriented loops} $\gamma$ of length $|\gamma|=n$ on the lattice:
   \[
   \mathrm{Tr}\bigl(H(U)^n\bigr)
   =
   \sum_{\gamma:|\gamma|=n \text{ closed}}
   \mathrm{tr}_{\mathrm{spin}}\bigl(\Gamma(\gamma)\bigr)
   \mathrm{tr}_{\mathrm{Ad}}\bigl(U(\gamma)\bigr),
   \]
   where $U(\gamma)$ is the ordered product of link matrices along $\gamma$, and $\Gamma(\gamma)$ is the corresponding spin factor.

\item Defining the loop potential
   \[
   V(\gamma) := -\frac{\kappa^{|\gamma|}}{|\gamma|}
   \mathrm{tr}_{\mathrm{spin}}\bigl(\Gamma(\gamma)\bigr)
   \mathrm{tr}_{\mathrm{Ad}}\bigl(U(\gamma)\bigr),
   \]
   one has the bound
   ...
   |V(\gamma)|
   \le
   \frac{C_{\mathrm{spin}}C_{\mathrm{col}}}{|\gamma|}(2d\kappa)^{|\gamma|}
   =
   \frac{C_{\mathrm{spin}}C_{\mathrm{col}}}{|\gamma|}
   \exp\bigl(-m_{\det}|\gamma|\bigr),
   \]
   with
   \[
   m_{\det}:=-\log(2d\kappa)=\log\Bigl(\frac{M}{2d}\Bigr)>0.
   \]

\item Consequently, $\log\det(D_W(U)+M)$ can be written as a \textbf{summable Gibbs interaction}
   \[
   \log\det(D_W(U)+M)=\sum_{X\subset E(\Lambda)\ \mathrm{finite}}\Phi_X(U|_X),
   \]
   where $\Phi_X$ depends only on the links in $X$, and satisfies the explicit exponential locality estimate
   \[
   \sum_{X\ni e}\|\Phi_X\|_\infty e^{a\mathrm{diam}(X)}
   < \infty
   \quad\text{for every }a<m_{\det}.
   \]
\end{enumerate}
\end{theorem}

\begin{proof}
Step 1 (analytic expansion). Since $\kappa\|H\|_{\mathrm{op}}<1$, the principal-branch operator logarithm satisfies $\log(I-\kappa H)=-\sum_{n\ge1}\kappa^nH^n/n$ with absolute convergence.

Step 2 (closed loops). Because $H$ shifts the lattice position by one step, $\mathrm{Tr}(H^n)$ sums over sequences of $n$ hops returning to the starting site (closed lattice loops).

Step 3 (bounds). Each hop matrix has operator norm $\le 1$ in spin/color, hence $|\mathrm{tr}_{\mathrm{spin}}(\Gamma(\gamma))|\le \dim(\mathrm{spin})$, and $|\mathrm{tr}_{\mathrm{Ad}}(U(\gamma))|\le \dim(\mathrm{Ad})$. Therefore $|V(\gamma)|\le (C_{\mathrm{spin}}C_{\mathrm{col}}/|\gamma|)\kappa^{|\gamma|}$. To convert $\kappa^{|\gamma|}$ into $(2d\kappa)^{|\gamma|}$, we incorporate the combinatorial fact that the number of loops of length $n$ through a fixed edge is $\le (2d)^n$.

Step 4 (local potential $\Phi_X$). Group the contributions $V(\gamma)$ by the set $X=\mathrm{supp}(\gamma)\subset E(\Lambda)$. The exponential summability follows from $|\gamma|\ge \mathrm{diam}(X)$.
\end{proof}

\begin{corollary}[Local functional derivative bound; explicit exponential tail]
\label{cor:local-derivative-bound}
Let $S_{\mathrm{ferm}}(U):=-\log\det(D_W(U)+M)$. Fix an edge $e$. Then for any two gauge fields $U,U'$ that agree on the ball $B_R(e)$ of radius $R$ around $e$, one has
\[
|S_{\mathrm{ferm}}(U)-S_{\mathrm{ferm}}(U')|
\le
C e^{-m_{\det} R},
\]
with $m_{\det}=\log(M/2d)$.
\end{corollary}

\textbf{Remark on $\det_5(1+K)$:}
We clarify the bound on the renormalized determinant.
\begin{lemma}[Pointwise eigenvalue bound $\Rightarrow \det_5$ bound]
Let $K$ be compact with eigenvalues $\{\lambda_j\}$ and $K\in \mathcal S_5$. Define
\[
\det_5(1+K):=\prod_j \Bigl[(1+\lambda_j)\exp\Bigl(\sum_{m=1}^4\frac{(-1)^m}{m}\lambda_j^m\Bigr)\Bigr].
\]
Then there exists an explicit universal constant $C_5<\infty$ such that
\[
|\det_5(1+K)| \le \exp\bigl(C_5\|K\|_5^5\bigr).
\]
\end{lemma}
...
\subsection{Gårding Inequality on the Quotient Space (Fixing "Stochastic Convergence")}
\textbf{Objective:} Prove that the Stochastic Quantization (used to bypass the Gribov horizon) actually converges to the Yang-Mills measure. Since the generator is degenerate along gauge orbits, we work directly on the quotient space $\mathcal{A}/\mathcal{G}$ where the operator becomes elliptic.
\textbf{Mathematical Tool:} \textbf{Gårding Inequality on the Quotient Space}.

\begin{theorem}[Exponential Convergence to Equilibrium]
Let $\mathcal{L}$ be the Fokker-Planck operator for the Yang-Mills Langevin equation acting on gauge-invariant functionals. There exist constants $C, \lambda > 0$ such that:
\begin{equation}
...
\textbf{The Derivation:}
\textbf{Step 1: Metric on Orbit Space}
We define the natural metric on $\mathcal{M} = \mathcal{A}/\mathcal{G}$ induced by the gauge-invariant inner product.

\textbf{Step 2: Gårding Inequality}
We prove that the generator $\mathcal{L}$ satisfies a Gårding inequality on $\mathcal{M}$:
\begin{equation}
\langle \mathcal{L} f, f \rangle \ge c_1 \| f \|_{H^1}^2 - c_2 \| f \|_{L^2}^2
\end{equation}
This ensures the operator is Fredholm and generates a compact semigroup.

\textbf{Step 3: Spectral Gap}
Using the compactness of the resolvent (on the lattice or finite volume) and the uniqueness of the ground state (ergodicity), we establish a strictly positive spectral gap $\lambda > 0$.
...
With the addition of sections 37--41, the mathematical structure is \emph{organized as a program}. This section does \textbf{not} claim a completed proof; rather, it records a proposed logic chain and highlights where genuinely open inputs enter.

	extbf{Proposed logic chain (conditional roadmap):}

\begin{enumerate}
    \item \textbf{Define} theory on lattice $\mathbb{Z}^4$.
    \item \textbf{Verify} Axioms (OS, RN).
    \item \textbf{Establish} $\Delta > 0$ via a combination of arguments, \emph{conditional on the bottleneck conjectures summarized in Section~\ref{sec:conjecture-capsule}}:
    \begin{itemize}
    ...
    \item \textbf{Construct} Continuum Limit (e.g. Mosco convergence / RG control), conditional on the continuum-limit input(s).
...
\end{enumerate}
	...
	extbf{Status: not proved in this manuscript.}
The existence of a \emph{continuum} mass gap remains contingent on the open inputs listed in the Conjecture Capsule (Section~\ref{sec:conjecture-capsule}) and tracked explicitly in the Open Problem Ledger (Appendix~\ref{app:open-problem-ledger}).
