...
This section provides \textbf{explicit numerical bounds} for the critical constants appearing in the proof. While precise numerical computation of optimal constants remains a framework-complete item, we establish rigorous bounds sufficient for the existence proof.
...
\item \textbf{LSI constant (from spectral gap):}
\[
\rho_N = \frac{N^2-1}{2N^2}
\]
\textbf{Note:} This is the correct LSI constant for the Haar measure on $SU(N)$, 
derived from the spectral gap via $\rho = \lambda_1 / N$ (see Remark~\ref{rem:lsi-vs-spectral}). 
...
\rho_N = \frac{N^2-1}{2N^2}
\]
This follows from the spectral gap via the relation $\rho = \lambda_1/N$ for compact 
Lie groups. The Bakry-Émery bound $\rho \geq \mathrm{Ric}_{\min} = N/4$ provides only 
an upper bound, not the exact value.
...
\]
For SU(2): $\rho_2 = 3/8 = 0.375$, $\lambda_1 = 3/4$. For SU(3): $\rho_3 = 8/18 \approx 0.444$, $\lambda_1 = 4/3$.
The ratio $\rho_N/\lambda_1 = 1/N < 1$ confirms that LSI is stronger than Poincaré.
\end{remark}
...
\begin{theorem}[Explicit Giles-Teper Constant---Derivation]
\label{thm:giles-teper-explicit-derivation}
...
Using $c_{\text{kin}} = \frac{\pi}{12}$ (from the rigorous Casimir Energy of the transverse fluctuations established in Appendix \ref{sec:casimir-energy}), this yields:
\[
\Delta \ge \sqrt{\frac{4\pi}{3}} \sqrt{\sigma} \approx 2.04 \sqrt{\sigma}
\]
This variational upper bound confirms that the scale is consistent.

However, the rigorous lower bound from Casimir scaling gives:
\[
\boxed{c_N \geq 2/N }
\]
...
\[
\Delta_{\text{phys}} \geq \frac{2}{3} \times 440 \text{ MeV} \approx 293 \text{ MeV}
\]
...
\hline
$0^{++}$ glueball mass & $\geq 293$ MeV & $\approx 1710$ MeV \\
\hline
$\Delta/\sqrt{\sigma}$ ratio & $\geq 2/3$ & $\approx 3.9$ \\
\hline
\end{tabular}
\end{center}
...
\label{thm:cluster-convergence-v2-app81}
...
\label{thm:zegarlinski-constants-app81}
...
\label{thm:beta-function-v2-app81}
...
\begin{align*}
\rho_N &= \frac{N^2-1}{2N^2} & \text{(LSI constant)} \\
...
\hline
$SU(2)$ & 0.375 & 0.750 & 0.375 & $\approx 3.628$ \\
\hline
$SU(3)$ & 0.444 & 1.333 & 0.444 & $\approx 2.721$ \\
\hline
\end{tabular}
\end{center}
\end{tcolorbox}
...
\hline
$\Delta/\sqrt{\sigma}$ & $\geq 0.67$ ($SU(3)$) & $\approx 3.9$ ($SU(3)$) & $\checkmark$ Consistent \\
\hline
$0^{++}$ glueball & $\geq 293$ MeV & $1710 \pm 50$ MeV & $\checkmark$ Consistent \\
\hline
$\sqrt{\sigma_{\text{phys}}}$ & $> 0$ (proven) & $440 \pm 10$ MeV & $\checkmark$ Consistent \\
\hline
$c_N$ coefficient & $\geq 2/N$ & $2.05$--$2.5$ & $\checkmark$ Matches \\
\hline
