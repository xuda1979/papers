...
\begin{theorem}[Yang-Mills Mass Gap---Main Theorem]
\label{conj:final}
...
This theorem is proven in Appendix~\ref{sec:definitive-gap-closure} using the following
\begin{enumerate}
\item \textbf{Uniform spectral gap:} $\Delta_L(\beta) \geq c(\beta) > 0$ uniformly 
in $L$ for all $\beta > 0$ --- \textbf{Proven} (Theorem~\ref{thm:uniform-lsi-induction})
...
--- \textbf{Proven} (Section~\ref{sec:gap4-resolution})
\item \textbf{No critical point:} $\Delta(\beta) > 0$ for all $\beta$ 
--- \textbf{Proven} (Theorem~\ref{thm:sigma-positivity-final})
\end{enumerate}
\end{tcolorbox}
...
\item Intermediate-coupling gap uses Defect Expansion (Theorem~\ref{thm:sigma-positivity-final})
\item Weak-coupling gap uses Convexity of Effective Action (Theorem~\ref{thm:uniform-lsi-induction})
\item String tension positivity uses Defect Expansion (Theorem~\ref{thm:sigma-positivity-final})
\item The Giles-Teper bound $\Delta \geq c\sqrt{\sigma}$ is operator-theoretic (Theorem~\ref{thm:giles-teper-final})
\item Continuum limit uses Minlos-Bochner (Section~\ref{sec:gap4-resolution})
\end{itemize}

All steps are now established. See Appendix~\ref{sec:definitive-gap-closure}.
\end{remark}
