...
The repository contains multiple parallel writeups.
Within this document, any continuum statement of the form
``4D $\SU(3)$ Yang--Mills has a mass gap'' should be read as the following conditional assertion:
\begin{quote}
			extit{Assuming existence of the continuum limit in the Osterwalder--Schrader sense along some $\beta_n\to\infty$ with $a(\beta_n)\to 0$, the continuum mass gap is bounded below by the Giles--Teper inequality once $\sigma_{\mathrm{phys}}>0$ is established by the RG bridge.}
\end{quote}
This aligns with the ``Millennium problem has two parts'' distinction emphasized in
			exttt{GAPS\_RESOLVED\_STATUS.md}.
\end{remark}
