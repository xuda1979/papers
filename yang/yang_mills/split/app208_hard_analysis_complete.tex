...
\begin{enumerate}
    ...
    \item \textbf{Compactness:} $T_\beta$ is trace-class (and hence compact)
...
\begin{theorem}[Spectral Convergence]\label{thm:mosco-spectral}
...
\textbf{Step 1: Tightness.} The measures $\mu_a$ are tight by the Giles-Teper bound (Appendix \ref{ch:fix53_mandelstam_constraints}), which provides uniform control on Wilson loop expectations.

\textbf{Step 2: Weak convergence.} Extract a subsequence $\mu_{a_n} \rightharpoonup \mu$ weakly.

\textbf{Step 3: Lower semicontinuity.} The Dirichlet form is lower semicontinuous:
\begin{equation}
    \mathcal{E}(f) \leq \liminf_{n} \mathcal{E}_{a_n}(f_n)
\end{equation}
for any weakly converging sequence.

\textbf{Step 4: Gap preservation.} By the uniform LSI (Theorem \ref{thm:zegarlinski}), the spectral gap is bounded below uniformly in $a$. The Mosco convergence preserves this gap.
\end{proof}
