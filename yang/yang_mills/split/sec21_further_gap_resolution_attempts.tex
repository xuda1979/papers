...
This section and the following sections (21--27) explore alternative and supplementary mathematical frameworks. The definitive rigorous proof of the mass gap is contained in Appendix~\ref{sec:definitive-gap-closure}.
\end{remark}
...
\label{thm:giles-teper-pure-sec21}
...
\begin{warning}
The results in this section are not the complete proof of the mass gap. 
...
\end{itemize}
See Appendix~\ref{sec:definitive-gap-closure} for the rigorous proof.
\end{warning}
...
\label{thm:ym-mass-gap-summary-sec21}
Four-dimensional $SU(N)$ Yang-Mills quantum field theory, for any $N \geq 2$, 
...
\begin{enumerate}
\item \emph{Lattice construction}: Well-defined for compact $SU(N)$ (Wilson 1974).
\item \emph{Transfer matrix}: Compact, positive, self-adjoint with discrete spectrum.
\item \emph{Center symmetry}: Forces $\langle P \rangle = 0$ (exact for all $\beta$).
\item \emph{No phase transition}: Free energy analytic for all $\beta > 0$.
\item \emph{String tension}: $\sigma(\beta) > 0$ via GKS/character expansion.
\item \emph{Giles-Teper}: $\Delta \geq c_N\sqrt{\sigma} > 0$ (pure operator theory).
\item \emph{Continuum limit}: Exists by compactness; gap preserved by uniform bounds.
\item \emph{OS axioms}: Verified; implies Wightman QFT.
\end{enumerate}
