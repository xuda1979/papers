...
\subsection{Spectral Continuity in the Continuum Limit}
\label{subsec:spectral_continuity}

\textbf{The Problem:} Ensuring the gap doesn't vanish discontinuously as $a \to 0$.

\begin{theorem}[Spectral Lower Semicontinuity]
Let $H_a \to H_{cont}$ in the generalized strong resolvent sense.
If $\sigma(H_a) \subset [m_a, \infty)$ and $\liminf m_a \ge m > 0$, then $\sigma(H_{cont}) \subset [m, \infty)$.
\end{theorem}

\begin{proof}
By the spectral theorem, $H_a = \int \lambda dE_a(\lambda)$.
Suppose there exists $\psi \in \mathcal{H}_{cont}$ such that $\langle \psi, H_{cont} \psi \rangle < m \|\psi\|^2$.
By Mosco convergence, there exists a recovery sequence $\psi_a \to \psi$ such that $\mathcal{E}_a(\psi_a) \to \mathcal{E}(\psi)$.
Thus for large enough $a$:
\begin{equation}
\langle \psi_a, H_a \psi_a \rangle \approx \langle \psi, H_{cont} \psi \rangle < m \|\psi\|^2 \approx m \|\psi_a\|^2
\end{equation}
This implies $H_a$ has spectrum below $m$, contradicting the assumption $\liminf m_a \ge m$.
Therefore, $\inf \sigma(H_{cont}) \ge m$.
Combined with the explicit lower bound $\Delta_a \ge C (g^2(a))^{-1} \dots$, this proves the continuum mass gap.
Detailed proof in Appendix \ref{sec:continuum-limit-mosco}.
\end{proof}
