...
To control the coefficients $c_k(a)$, we utilize Balaban's Renormalization Group flow explicit bounds. We compute the anomalous dimension $\gamma_k$ of the symmetry-breaking operators. The RG flow equation for a coefficient $C_k(a)$ is:
\begin{equation}
\beta_{C_k} = - a \frac{d C_k}{d a} = (4 - d_k - \gamma_k) C_k
\end{equation}

For the relevant/marginal operators (dimension $\le 4$), we must show that no new symmetry-breaking directions become unstable.
For dimension 6 operators, we strictly bound the coefficient:
\begin{equation}
|C_6(a)| \le K g^2(a)
\end{equation}
...
For the dangerous dimension 4 operators, we establish that they are forbidden by the exact $H_4$ symmetry and the reflection positivity of the Wilson action, or that their anomalous dimensions $\gamma$ render them irrelevant.
