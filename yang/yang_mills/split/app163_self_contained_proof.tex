...
\begin{align}
\langle F \cdot \theta F \rangle &= \frac{1}{Z} \int \mathcal{D}U \, F(U_+) \overline{F(\theta U_+)} e^{-S_+(U_+) - S_-(\theta U_+)} \prod_{p \in \Pi} \left( \sum_{r_p} c_{r_p} \chi_{r_p}(U_{p_+}^\dagger \theta U_{p_+}) \right) \\
&= \frac{1}{Z} \sum_{\{r_p\}} \left( \prod_p c_{r_p} \right) \int \mathcal{D}U_+ \mathcal{D}U_- \, F(U_+) \overline{F(\theta U_+)} e^{-S_+(U_+)} e^{-S_-(\theta U_+)} \prod_p \left( \sum_{i,j} \overline{D_{ji}^{r_p}(U_{p_+})} D_{ji}^{r_p}(\theta U_{p_+}) \right)
\end{align}
