...
\begin{proof}
    ...
    \end{equation}
    where $\sigma \approx - \ln \beta > 0$. The mass gap is defined by the exponential decay of the 2-point function of the lowest-dimension gauge-invariant operator (the scalar glueball). Since the cluster expansion converges, the correlation length $\xi$ is finite, implying a strictly positive mass gap $m(\beta) \sim 1/\xi > 0$.
...
\section{Rigorous Control of the Continuum Limit}
\label{sec:rigorous_weak_coupling}

The critical challenge is to show that the mass gap persists as $\beta \to \infty$, which corresponds to the continuum limit $a \to 0$ via Asymptotic Freedom. We utilize the Balaban block-spin renormalization group approach to control the effective action in this regime.
...
\begin{proof}
    The proof proceeds by a rigorous inductive analysis of the flow of the effective action. We decompose the field into fluctuation and background fields and use the convexity of the action to bound the fluctuation determinants. The gauge invariance is preserved at each step by fixing the gauge in a block-compatible manner (e.g., axial gauge within blocks) \cite{Balaban1985}.
\end{proof}

\subsection{Proof of the Mass Gap in the Continuum}

We now combine the strong coupling results with the renormalization group flow to establish the gap in the $a \to 0$ limit.

\begin{proof}[Proof of Continuum Mass Gap]
...
\begin{proof}
    The existence of the theory is guaranteed by the continuum limit construction (Section \ref{sec:continuum-limit}). The existence of the mass gap follows from the uniform lower bound on the spectral gap of the transfer matrix (Section \ref{sec:uniform-bounds} and \ref{sec:rigorous_weak_coupling}), which scales to a finite physical mass $\Delta = m_{\text{phys}} > 0$ in the continuum limit. The axiomatic properties (Reflection Positivity, Euclidean Invariance) are preserved in the limit, ensuring a valid Quantum Field Theory \cite{OsterwalderSchrader}.
\end{proof}
