...
The lattice spacing is determined by fixing the physical string tension $\sigma_{phys}$ (an experimental value):
\[
...
We explicitly note that defining the scale via $\sigma$ guarantees a mass gap of order $\sqrt{\sigma_{phys}}$ by construction (assuming the area law holds). The mathematical challenge of the Yang-Mills problem is therefore shifted to proving two properties:
\begin{enumerate}
    \item \textbf{Existence:} The string tension $\sigma_{lat}(\beta)$ is strictly positive for all $\beta$ (Confinement/Area Law).
    \item \textbf{Scaling:} The resulting function $a(\beta)$ must satisfy the asymptotic freedom law derived from perturbation theory:
    \[
    a(\beta) \sim \frac{1}{\Lambda} e^{-\frac{\beta}{2N\beta_0}} \quad \text{as } \beta \to \infty
    \]
\end{enumerate}
Our proof strategy demonstrates (1) via the cluster expansion and Pirogov-Sinai theory, and (2) via the Renormalization Group analysis of the effective action. We do \textbf{not} assume the scaling law holds a priori to prove the gap; rather, we define the theory physically and prove the scaling matches the perturbative prediction.
\end{definition}
