...
\textbf{Step 4: Extension to Weak Coupling.}
For large $\beta$ (weak coupling), we rely on asymptotic freedom scaling.
Both quantities scale with the renormalization group invariant scale $\Lambda_{QCD}$:
\begin{equation}
\Delta \sim C_m \Lambda_{QCD}, \quad \sqrt{\sigma} \sim C_\sigma \Lambda_{QCD}
\end{equation}
Assuming no phase transition (valid for $SU(N)$ via the Osterwalder-Seiler theorem and numerical evidence), the ratio $\Delta/\sqrt{\sigma}$ remains bounded away from zero.
Rigorous lower bounds on this ratio for all $\beta$ follow from Theorem \ref{thm:strong-coupling-gap-rigorous} combined with the absence of massless singularities (confinement).
\end{proof}
