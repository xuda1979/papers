...
Framework for continuum limit (conditional on uniform bounds):
\begin{itemize}
...
\item Physical string tension $\sigma_{\text{phys}} > 0$ (conditional---Theorem~\ref{thm:sigma-phys-positive})
\item Exchange of limits $a \to 0$, $L \to \infty$ (conditional---Theorem~\ref{thm:exchange-limits})
\item $SO(4)$ symmetry recovery (Theorem~\ref{thm:so4-recovery})
\item Full OS axioms verification (conditional---Theorem~\ref{thm:full-os})
\item Dimensionless ratio bound: $\Delta/\sqrt{\sigma} \geq c_N$ (preserved in limit)
\end{itemize}
\end{enumerate}

\textbf{Target Result (Conditional):}
\[
\boxed{\Delta_{\text{continuum}} = \lim_{a \to 0} \frac{\Delta_{\text{lattice}}(a)}{a} \geq c_N \sqrt{\sigma_{\text{phys}}} > 0}
\]

\textbf{Note:} This result is conditional on verifying uniform-in-$L$ bounds at 
intermediate and weak coupling. See Conjecture~\ref{conj:gap-all-beta} and 
Theorem~\ref{thm:rg-bridge-main} for the precise conditions.
