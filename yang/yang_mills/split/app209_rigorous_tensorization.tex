...
Assume $\mu_x$ satisfies Poincar\'e inequality with constant $\lambda_2 \geq \rho_2$. Define
\begin{equation}
    C_{\mathrm{int}}(x) := \sup_{|v|=1} \mathrm{Var}_{\mu_x}\big(\langle v, \nabla_x H(x,\cdot) \rangle\big).
\end{equation}
Then:
\begin{equation}
    \frac{1}{4g(x)} \left| \mathrm{Cov}_{\mu_x}(F, \nabla_x H) \right|^2 \leq \frac{C_{\mathrm{int}}(x)}{\rho_2} \int |\nabla_y f|^2 \, d\mu_x.
\end{equation}
\end{lemma}
...
\begin{equation}
    |\mathrm{Cov}_{\mu_x}(F, B)|^2 \leq \mathrm{Var}_{\mu_x}(F) \cdot \mathrm{Var}_{\mu_x}(B)
\end{equation}
where $B = \nabla_x H$.

For the variance of $F = f^2$, use Poincar\'e:
\begin{equation}
    \mathrm{Var}_{\mu_x}(f^2) \leq \frac{1}{\rho_2} \int |\nabla_y(f^2)|^2 \, d\mu_x = \frac{4}{\rho_2} \int f^2 |\nabla_y f|^2 \, d\mu_x.
\end{equation}

For the variance of $B$, we use the definition of $C_{\mathrm{int}}(x)$.

Combining and using $\int f^2 \, d\mu_x = g(x)$:
\begin{equation}
    |\mathrm{Cov}_{\mu_x}(F, B)|^2 \leq \frac{4 C_{\mathrm{int}}(x)}{\rho_2} g(x) \int |\nabla_y f|^2 \, d\mu_x.
\end{equation}
