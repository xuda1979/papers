...
\subsection{The Nuclearity Bound}
...
Let $\Theta_\beta: \mathcal{A}(\mathcal{O}) \to \mathcal{H}$ be the map from local observables to states at inverse temperature $\beta$. The nuclearity index $\nu(E) = \text{Tr}(e^{-\beta H} P_E)$ satisfies:
\begin{equation}
\ln \nu(E) \leq C \cdot V \cdot E^{d/m}
\end{equation}
\textbf{Derivation:} We derive this using the \textbf{Cwikel-Lieb-Rozenblum Bound} on the number of eigenvalues $N(E)$ for the lattice Hamiltonian $H_{lat} = -\Delta + V(A)$.
\begin{equation}
N(E) \leq L_d \int |V(x) - E|_-^{d/2} dx
\end{equation}
This bound ensures the density of states is sparse enough to support a particle structure rather than a continuous "jelly".
\end{theorem}
...
\subsection{The Borel Summability Fix}
\textbf{Objective:} Uniqueness of perturbative scaling.

\begin{theorem}[Watson-Nevanlinna Theorem]
The Schwinger functions are the \textbf{Borel Sum} of their perturbative series. The remainder $R_N(g^2)$ satisfies:
\begin{equation}
|R_N(g^2)| \leq C \sigma^N N! |g|^{2N}
\end{equation}
in a wedge of the complex plane. Balaban's Large Field bounds ensure no singularities on the positive real axis obstruct the transform.
\end{theorem}
...
\subsection{The Brezis-Wainger Inequality}
\textbf{Objective:} Critical dimension regularity.

\begin{theorem}[Logarithmic Sobolev Embedding]
In $d=4$, for $u \in W^{2,2}$, the pointwise bound is:
\begin{equation}
\|u\|_\infty \leq C (1 + \ln^{1/2}(1 + \|u\|_{W^{2,2}}))
\end{equation}
This logarithmic divergence is controllable by RG flow, replacing the failing Sobolev embedding $W^{2,2} \not\subset L^\infty$.
\end{theorem}
...
\subsection{The Malliavin Smoothness}
\textbf{Objective:} Absolute continuity of spectrum.

\begin{theorem}[Integration by Parts on Wiener Space]
The Malliavin covariance matrix $\sigma_F$ is non-degenerate (Hörmander condition).
Integration by parts implies the law has a smooth density:
\begin{equation}
E[f'(F)] = E[f(F) \text{div}(D F \sigma_F^{-1})]
\end{equation}
This rules out singular continuous spectrum.
\end{theorem}
