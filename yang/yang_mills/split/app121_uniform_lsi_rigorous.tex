...
%=============================================================================
\subsection{Step 2: 1D Boundary LSI (From Transfer Matrix)}
%=============================================================================

\begin{lemma}[Boundary System is 1D-like]
\label{lem:boundary-1d}
The boundary edges $E_k^{bdy}$ between blocks at scale $k$ form a system 
where each boundary component is effectively 1-dimensional.
\end{lemma}

\begin{proof}
Consider adjacent blocks $B_1$ and $B_2$ sharing a face $F$. The boundary 
plaquettes involving edges in $F$ have the form:
\begin{equation}
U_p = U_{e_1} U_{e_2} U_{e_3}^\dagger U_{e_4}^\dagger
\end{equation}
where $e_1, e_3 \in F$ (boundary) and $e_2, e_4 \in B_1 \cup B_2$ (interior).

When we condition on interior edges, the boundary measure becomes:
\begin{equation}
d\mu_F = \frac{1}{Z_F} \exp\left(\beta \sum_{e \in F} \mathrm{Re}\,\mathrm{Tr}(U_e W_e^\dagger)\right) \prod_{e \in F} dU_e
\end{equation}
where $W_e$ are fixed matrices depending on interior configurations.

This is precisely the 1D chain measure analyzed in Section \ref{sec:critical-gap-closed}!
\end{proof}

\begin{theorem}[Boundary LSI from 1D Gap]
\label{thm:boundary-lsi}
The LSI constant for the boundary system at scale $k$ satisfies:
\begin{equation}
\rho_k^{bdy} \geq \frac{1}{8N^2 |E_k^{bdy}| (1+\beta)}
\end{equation}
where $|E_k^{bdy}| = O((L/2^k)^{d-1} \cdot 2^k) = O(L^{d-1})$ is the number of 
boundary edges at scale $k$.
\end{theorem}

\begin{proof}
Each $(d-1)$-dimensional face contains $O(2^{k(d-1)})$ edges forming a 
$(d-1)$-dimensional lattice with 1D interactions.

By Theorem \ref{thm:1d-lsi-complete}:
\begin{equation}
\rho_{face} \geq \frac{1}{8N^2 \cdot 2^{k(d-1)} (1+\beta)}
\end{equation}

There are $(L/2^k)^{d-1}$ faces at scale $k$, but they are \textbf{independent} 
given interior conditioning. By tensorization:
\begin{equation}
\rho_k^{bdy} = \min_{faces} \rho_{face} \geq \frac{1}{8N^2 \cdot 2^{k(d-1)} (1+\beta)}
\end{equation}
\end{proof}
