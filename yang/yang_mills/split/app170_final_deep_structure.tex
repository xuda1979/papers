...
The manuscript utilizes "Adjoint QCD" as an interpolating theory to connect the strong coupling regime to the weak coupling regime. A central argument relies on the \textbf{Area Law} for the Wilson loop $\langle W_C \rangle \sim e^{-\sigma \text{Area}(C)}$ to demonstrate confinement and the mass gap.

However, in Adjoint QCD, the confining string can break. The adjoint fermions (gluinos) screen the color source at large distances. Consequently, the asymptotic behavior of the Wilson loop follows a \textbf{Perimeter Law}, $\langle W_C \rangle \sim e^{-\mu \text{Perimeter}(C)}$, which in pure gauge theory typically signals a deconfined phase. This creates a paradox: the theory is massive and confining in the sense of the spectrum, but the standard order parameter suggests deconfinement.

\subsubsection{The Mathematical Fix}
To resolve this, we replace the Wilson Loop with the \textbf{Fredenhagen-Marcu Order Parameter}, which is designed to detect confinement in theories with dynamical matter capable of screening.
...
\subsubsection{The Obstruction}
The manuscript previously invoked \textbf{GKS Inequalities} (Griffiths-Kelly-Sherman) to claim that correlations are positive and monotonic. While valid for ferromagnetic spin systems and Abelian gauge theories, GKS inequalities are generally \textbf{false} for non-Abelian gauge theories like $SU(N)$ ($N \ge 2$). The group characters are complex and oscillatory, leading to sign-indefinite correlations.

\subsubsection{The Mathematical Fix}
We replace the reliance on GKS with \textbf{Kato's Diamagnetic Inequality} and \textbf{Ginibre Inequalities}. Instead of claiming monotonicity of the gauge correlations themselves, we prove they are \textbf{dominated} by a scalar ferromagnet.
...
\subsubsection{The Mathematical Fix}
We implement a \textbf{Rigorous Computer-Assisted Proof} using \textbf{Interval Arithmetic} to bridge this specific finite interval.
...
\subsubsection{The Mathematical Fix}
We adopt \textbf{Balaban’s Geodesic Minimizer Map} to define the block spin transformation.
...
\begin{enumerate}
    ...
    \item \textbf{Gauge Covariance:} The definition is manifestly gauge covariant. This preserves the \textbf{Ward Identities} at each step of the RG transformation.
\end{enumerate}
\end{theorem}
