...
\[
h(\mathcal{B}_\Lambda, \mu_{\Lambda,\beta}) \geq c_N \geq 2/N
\]
...
\begin{theorem}[Correlation Inequality for Yang-Mills (via Character Expansion)]
\label{thm:correlation-ym}
...
\]
\textbf{Note:} This follows from the character expansion with non-negative 
Littlewood-Richardson coefficients, \textbf{not} from the classical FKG inequality, 
which does not apply to non-abelian gauge theories (see Appendix~\ref{sec:definitive-gap-closure}).
\end{theorem}
