...
\item The crossing term $S_0$ involves plaquettes $p$ that intersect the reflection hyperplane $\Pi$. For the Wilson action, the interaction term for such a plaquette is of the form $\frac{\beta}{N} \Re\Tr(U_{p_+} \theta(U_{p_+}))$, where $U_{p_+}$ is the part of the loop in $\Lambda_+$.
Using the character expansion of the Boltzmann weight:
\[
\exp\left(\frac{\beta}{N} \Re\Tr(U)\right) = \sum_r c_r(\beta) \chi_r(U)
\]
where $c_r(\beta)$ are positive coefficients (related to modified Bessel functions).
The interaction term becomes:
\[
\exp(-S_0) = \prod_{p \in \Pi} \left( \sum_{r_p} c_{r_p}(\beta) \chi_{r_p}(U_{p_+} \theta(U_{p_+})) \right)
\]
Using the property $\chi_r(AB) = \sum_{i,j} D_{ij}^r(A) D_{ji}^r(B)$, we can write:
\[
\chi_r(U_{p_+} \theta(U_{p_+})) = \sum_{i,j} D_{ij}^r(U_{p_+}) \overline{D_{ij}^r(\theta(U_{p_+}))}
\]
Thus, the Boltzmann weight factorizes into a sum of terms of the form $A_\alpha \overline{\theta(A_\alpha)}$ with positive coefficients.
