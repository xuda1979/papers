...
The manuscript relies on "Mosco Convergence" to claim the mass gap survives the limit $a \to 0$. This is a weak form of convergence that permits \textbf{Spectral Pollution}: spurious lattice eigenvalues (artifacts) can drift into the gap region $(0, \Delta)$ as $a \to 0$, causing the gap to vanish discontinuously in the limit.

\subsubsection{The Estimator}
We prove \textbf{Norm Resolvent Convergence}. Let $H_a$ be the lattice Hamiltonian and $H$ the continuum operator. We prove convergence in the \textbf{Operator Norm}:
\begin{equation}
