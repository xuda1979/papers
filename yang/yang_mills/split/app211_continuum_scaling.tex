...
\begin{definition}[Dirichlet Form]\label{def:dirichlet-form}
...
\begin{theorem}[Continuum Mass Gap from Uniform Lattice Bounds]\label{thm:continuum-mass-gap}
Suppose:
\begin{enumerate}
...
\begin{itemize}
    \item Direct analysis of the weak coupling regime (very hard)
    \item The adjoint interpolation strategy: anchor at SYM and deform
    \item Combining strong coupling results with analyticity/absence of phase transitions
\end{itemize}
\end{remark}

\subsection{Avoiding Circular Arguments}

\begin{caveat}[What NOT to Do]
The following arguments are \textbf{circular} and must be avoided:
\begin{enumerate}
    \item Defining $a(\beta) = 1/\Delta_{\text{lat}}(\beta)$ and concluding $\Delta_{\text{phys}} = 1$.
    \item Using Monte Carlo values as proof inputs.
    \item Assuming the string tension equals $(440 \text{ MeV})^2$ without proving $\sigma > 0$ intrinsically.
    \item Claiming the Cheeger constant is $\beta$-independent when it depends on the weighted measure.
\end{enumerate}
\end{caveat}
