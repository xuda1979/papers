\chapter{Fix 33: The Helffer-Sjöstrand Formula (Spectral Decay)}
\label{ch:fix33_helffer_sjostrand}
...
\end{align}
Optimizing the parameter $\eta$ (which depends on the gap size $\Delta$), we derive the \textbf{Exponential Clustering Property}:
\begin{equation}
\langle \mathcal{O}_x \mathcal{O}_y \rangle_{conn} \le C e^{- \frac{\Delta}{v} |x-y|}
\end{equation}
where $v$ is the maximal velocity (Lieb-Robinson velocity) of the lattice system.
