...
\[
(T_\beta \chi_R)(U) = \int e^{\beta \mathrm{Re} \Tr(UV^\dagger)} \chi_R(V) dV
\]
...
\[
r_R(\beta) = \frac{1}{d_R} \int_{SU(N)} e^{\beta \mathrm{Re} \Tr(U)} \chi_R(U) dU
\]
...
\[
\frac{\lambda_R}{\lambda_0} = \frac{r_R}{r_0} = \frac{\int e^{\beta \mathrm{Re} \Tr U} \chi_R(U) dU / d_R}{\int e^{\beta \mathrm{Re} \Tr U} dU}
\]
Wait, the $d_R$ factor cancels if we normalize properly.
Let $K(U) = e^{\beta \mathrm{Re} \Tr U} = \sum_R c_R(\beta) d_R \chi_R(U)$.
Then $\int K(UV^\dagger) \chi_S(V) dV = \sum_R c_R d_R \int \chi_R(U V^\dagger) \chi_S(V) dV$.
...
\[
c_R(\beta) = \frac{1}{d_R} \int_{SU(N)} e^{\beta \mathrm{Re} \Tr U} \overline{\chi_R(U)} dU = \frac{1}{d_R} I_R(\beta)
\]
...
\subsection{Step 3: The Gap is Fundamental}
...
Since $e^{\beta \mathrm{Re} \Tr U}$ is peaked at the identity, the integral is maximized when $\chi_R(U)$ is closest to $d_R$ near identity.
...
\subsection{Step 4: Explicit Bessel Estimates}
