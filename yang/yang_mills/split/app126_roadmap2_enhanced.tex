...
\Delta_L(m) \geq c_N > 0
\]
where $c_N$ depends only on $N$ and not on $m$ or $L$. This combines finite-volume positivity (Perron-Frobenius), continuity in $m$ (analytic perturbation theory), and uniform-in-$L$ control via hierarchical Zegarlinski (Theorem~\ref{thm:block-zeg}).
\end{theorem}
