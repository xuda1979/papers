...
This section provides mathematically rigorous proofs for the adjoint interpolation 
...
\label{thm:uniform-lee-yang-adjoint}% Alias for cross-references
...
\begin{corollary}[Analyticity of Spectral Gap in Fermion Mass]
\label{cor:analyticity-adjoint}
The spectral gap $\Delta(m)$ is real-analytic in $m$ for $m \in (0, \infty)$. This follows from Theorem~\ref{thm:lee-yang-gap} via Montel's theorem applied to the family of analytic functions $\{\Delta_L(m)\}_{L \in \mathbb{N}}$ with uniform bounds.
\end{corollary}

\begin{theorem}[Uniform Lower Bound on Adjoint QCD Gap]
\label{thm:uniform-bound-adjoint}
For all $m \in (0, \infty)$ and all lattice sizes $L$:
\[
\Delta_L(m) \geq c_N > 0
\]
where $c_N$ depends only on $N$ and not on $m$ or $L$. This combines finite-volume positivity (Perron-Frobenius), continuity in $m$ (analytic perturbation theory), and uniform-in-$L$ control via hierarchical Zegarlinski (Theorem~\ref{thm:block-zeg-sec12}).
\end{theorem}
...
\label{thm:decoupling-rigorous-126}
\label{thm:decoupling-formula}% Alias for cross-references
...
\label{thm:adjoint-complete-126}
