\chapter{Derivation Q: The Stratified Hardy Inequality (Singularity Control)}
\label{ch:derivation_q_stratified_hardy}
...
\end{equation}
This proves:
\begin{theorem}[Q.1: Cone Spectral Stability]
...
The wavefunction decays at the singular strata as $\Psi(r) \sim r^{(d_{codim}-2)/2}$, ensuring no probability leakage.
\end{theorem}
...
\section{Alternative Rigorous Formulation: Gribov-Zwanziger Action}
\label{sec:gz_formulation}
...
To resolve the Gribov ambiguity without relying on the difficult infinite-dimensional limit of the Hardy inequality, we modify the action to explicitly restrict the measure to the Gribov region $\Omega$ (where the Faddeev-Popov operator is positive).
\begin{definition}[Gribov-Zwanziger Action]
The local Gribov-Zwanziger action is defined as:
\begin{equation}
S_{GZ} = S_{YM} + \gamma^4 \int d^4x \, H(A) - d(N^2-1) \ln \gamma
\end{equation}
where $H(A)$ is the horizon function and $\gamma$ is the Gribov mass parameter determined self-consistently by the gap equation $\langle H(A) \rangle = d(N^2-1)$.
\end{definition}
This formulation bypasses the need to prove the essential self-adjointness of the Laplacian on the singular quotient space $\mathcal{A}/\mathcal{G}$, as the GZ action includes a "soft wall" potential that naturally confines the dynamics to the region $\Omega$, avoiding the singularities at the boundary (the Gribov horizon).
