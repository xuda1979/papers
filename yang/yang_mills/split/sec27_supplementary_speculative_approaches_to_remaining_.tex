\section{Supplementary / Speculative Approaches to Remaining Gaps}
\label{sec:definitive-gaps}
%=============================================================================

This section collects \textbf{exploratory} ideas that appeared in earlier drafts.
They are \textbf{not used} in the rigorous chain of implications summarized in
Remark~\ref{rem:rigorous-gap-closure-summary}.
Where appropriate, we explicitly label statements here as programs/heuristics/conditional.
...
\textbf{Status:} The material below is included for \textbf{speculative/heuristic interest only}. 
It should \textbf{NOT} be cited as evidence for the mass gap. Readers seeking the rigorous 
framework should focus on Sections with ``Rigorous'' labels.
\end{tcolorbox}

\begin{tcolorbox}[colback=red!10,colframe=red!60!black,title=\textbf{Warning: Speculative Framework}]
Tropical geometry describes the ``skeleton'' of algebraic varieties via piecewise-linear 
structures. Applying tropical methods to quantum expectation values in gauge theory is 
\textbf{speculative} and requires:
\begin{itemize}
\item Rigorous justification that tropical limits capture full quantum behavior
...
\end{itemize}
The arguments below are \textbf{heuristic} and do not constitute a rigorous proof.
\end{tcolorbox}

We \textit{conjecture} $\sigma(\beta) > 0$ for all $\beta > 0$ using \textbf{tropical geometry}, 
which provides a piecewise-linear skeleton of the algebraic structure.

\subsubsection{Tropical Limit of Wilson Loops (Heuristic)}
...
\begin{conjecture}[Tropical Area Law]
...
\begin{conjecture}[Tropical-to-Quantum Lift]
...
\begin{remark}[Rigorous gap-closure summary]
\label{rem:rigorous-gap-closure-summary}
The manuscript contains (for historical and exploratory reasons) several "kitchen sink" 
frameworks (e.g. tropical, adelic, derived-category, and noncommutative-geometry sketches).
\textbf{None of those are required for the rigorous chain of implications used in this paper.}

The \emph{actual} gap-closure mechanisms used for the four-dimensional lattice-to-continuum argument 
are the following (with red-team-validated, stand-alone writeups in the repository):
\begin{enumerate}[label=(\roman*)]
...
(see \texttt{UNIFIED\_GAP\_RESOLUTION.tex} and \texttt{VULNERABILITY\_FIXES.tex}).

\item \textbf{Weak coupling control:} replace informal "Gaussian" steps by constructive bounds
(Balaban-style large/small field decomposition), as summarized in \texttt{VULNERABILITY\_FIXES.tex}.

\item \textbf{Strong coupling anchor:} standard cluster expansion with uniform exponential decay.

\item \textbf{Continuum bridge:} the RG-invariance evaluation of $\sigma_{\mathrm{phys}}$ at a strong-coupling 
reference point ("RG bridge"), together with the reflection-positivity Giles--Teper inequality 
to transport $\sigma_{\mathrm{phys}}>0$ into $\Delta_{\mathrm{phys}}>0$.
\end{enumerate}

In particular:
\begin{itemize}
\item The tropical/adelic subsection (Section~\ref{subsec:gap-ii-tropical}) is explicitly marked as \textbf{non-rigorous} 
and is not used.
\item Any "proof complete" statements should be interpreted as "complete \emph{given} the above rigorous modules" 
and are tracked against \texttt{GAPS\_RESOLVED\_STATUS.md} / \texttt{CORRECTED\_STATUS\_2025.md}.
\end{itemize}
\end{remark}
