...
\subsection{The Zeta Function Regularization (Geometric Finite Curvature)}
\label{subsec:zeta_regularization}

\textbf{The Problem:} Vacuum energy divergence causes Ricci curvature blow-up.

\begin{theorem}[Spectral Zeta Regularization]
We define the effective potential using the Zeta function:
\begin{equation}
V_{eff}(A) = -\frac{1}{2} \zeta'_A(0)
\end{equation}
where $\zeta_A(s) = \Tr( (-\Delta_A)^{-s} )$.
\end{theorem}

\begin{proof}
The variation $\delta V_{eff}$ yields the force. The divergent terms correspond to the Heat Kernel Coefficients $a_n(A)$. Due to gauge invariance, for the vacuum state, these coefficients are constants independent of the background field fluctuations (or can be subtracted by counterterms).
Thus, they drop out of the Hessian (curvature calculation), leaving a finite Remainder Term that provides the positive curvature bound required for the geometric gap proof.
\end{proof}
