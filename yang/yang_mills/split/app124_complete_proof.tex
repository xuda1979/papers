...
For pure $SU(N)$ Yang-Mills theory in four Euclidean dimensions:
\begin{equation}
\boxed{\Delta_{phys} > 0}
\end{equation}
...
\begin{equation}
\boxed{\Delta_{phys} \geq c_N \sqrt{\sigma_{phys}} = c_N \cdot 440 \text{ MeV}}
\end{equation}
where $c_N = \sqrt{\frac{2\pi(N^2-1)}{3N^2}}$ (Giles-Teper constant).

\textbf{Explicit values}:
\begin{align}
\text{SU(2):} \quad \Delta &\geq 1.28 \times 440 \text{ MeV} = 563 \text{ MeV} \\
\text{SU(3):} \quad \Delta &\geq 1.48 \times 440 \text{ MeV} = 651 \text{ MeV}
\end{align}
\end{theorem}
...
\end{equation}
where $c_N = \sqrt{\frac{2\pi(N^2-1)}{3N^2}}$.
\end{theorem}
...
Using $\sigma_{phys} = (440 \text{ MeV})^2$ and $c_3 = 1.48$:
\begin{equation}
\Delta_{phys}^{SU(3)} \geq 1.48 \times 440 \text{ MeV} = 651 \text{ MeV}
\end{equation}
