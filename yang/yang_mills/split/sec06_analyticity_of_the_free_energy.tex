...
\label{subsec:bessel-nevanlinna-su23}
...
\begin{theorem}[Bessel--Nevanlinna Analyticity---Finite Volume]
\label{thm:bessel-su2}
For $SU(2)$ Yang--Mills on any \textbf{fixed finite lattice} $\Lambda$:
\[
...
\textbf{Crucial Distinction:} This theorem does \textbf{not} automatically imply analyticity of the infinite-volume free energy $f(\beta)$. The accumulation of zeros on the real axis in the limit $|\Lambda| \to \infty$ (a phase transition) remains a logical possibility that must be excluded by other means (see Conjecture~\ref{conj:no-phase-transition}).
\end{theorem}
