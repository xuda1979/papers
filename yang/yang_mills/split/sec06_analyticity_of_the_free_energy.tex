...
To prove the gap exists without assuming it a priori, we use \textbf{Balaban’s Multiscale Analysis} (Constructive Renormalization Group). This proves stability scale-by-scale.

\subsubsection{Inductive Hypothesis}
Assume that at scale $k$, the effective action $S_k$ is well-approximated by a strictly convex Gaussian functional:
\begin{equation}
S_k(\phi) \approx \frac{1}{2} \langle \phi, D_k \phi \rangle + V_k(\phi)
\end{equation}
where $D_k$ is a positive elliptic operator (mass term).

\subsubsection{The RG Step}
Define $S_{k+1}$ by integrating out fluctuations between scale $k$ and $k+1$.
\begin{enumerate}
\item \textbf{Small Fields:} Prove that Gaussian integration preserves the convexity of the action ($S'' > 0$), ensuring the "mass" does not flow to zero.
\item \textbf{Large Fields:} Use \textbf{Cluster Expansions} to prove that non-perturbative defects (where convexity fails) have exponentially small density: $\rho_{defects} \sim e^{-c \beta}$.
\end{enumerate}

\subsubsection{Conclusion}
This demonstrates that the correlation length $\xi$ remains finite at every step of the limit $k \to \infty$, establishing the mass gap \textbf{constructively}.
