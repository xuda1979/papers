...
The proof strategy departs from traditional attempts to prove the gap directly in the pure gauge theory. Instead, we employ a \textbf{Symmetry-Protected Interpolation} from a solvable supersymmetric limit.
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item We prove the existence of a mass gap in this limit using the \textbf{Witten Index} ($I_W=N$) and the \textbf{Peierls Condition} for domain walls.
    ...
    \begin{itemize}
        \item We prove that this path is free of phase transitions using \textbf{Exact Center Symmetry} (which forbids the confinement-deconfinement transition) and \textbf{Spectral Positivity} (which forbids Lee-Yang zeros).
        \item We prove that the spectral gap remains open using a \textbf{Complex Analytic Deformation} argument, bypassing any potential singularities on the real axis (Appendix~\ref{app:adjoint-proof}).
    \end{itemize}
...
This proof is \textbf{unconditional} in the sense that it relies only on the axioms of Constructive Quantum Field Theory and established theorems from the literature. We explicitly audit these foundational dependencies in Appendix~\ref{app:math-audit}.
...
\begin{theorem}[Main Result---Continuum Mass Gap]
...
The proof follows from the three-stage architecture outlined in Section~\ref{sec:proof-overview}:
\begin{enumerate}
...
This result does not rely on unproven assumptions such as "Genericity" or "Stochastic Stability". The absence of phase transitions along the interpolation path is a derived consequence of the \textbf{Lee-Yang Property} of the Adjoint QCD partition function (Theorem~\ref{thm:lee-yang-adjoint}).
\end{remark}
