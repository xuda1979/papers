...
\textbf{Step 5: Fixed point analysis.}

Define $x_n = 1/\rho_n$. The recursion becomes:
\[
x_n \leq x_{n-1} + C\beta^2
\]

Wait, this still grows linearly in $n$! We need a better argument.

\textbf{Step 6: Martingale method (Zegarlinski's key insight).}

The correct approach uses the \textbf{spectral gap} of the boundary marginal.

For a 1D chain, the boundary marginal on $(U_1, U_n)$ has:
\[
\rho_{\partial} \geq \rho_{SU(N)}^2 \cdot e^{-C\beta}
\]
because the boundary variables are \textbf{nearly independent} (correlation decays exponentially).

\textbf{Step 7: Correlation decay.}

The 1D system has correlation length $\xi = O(1/|\ln\tanh(\beta)|)$ for small $\beta$.

For $n \gg \xi$:
\[
|\langle U_1 U_n^\dagger \rangle| \leq e^{-n/\xi}
\]

This exponential decay means the boundary marginal factorizes up to $e^{-n/\xi}$ corrections.

\textbf{Step 8: Uniform bound.}

Using the block decomposition with block size $b = 2\xi$:
\begin{itemize}
\item Number of blocks: $n/b = n/(2\xi)$
\item Each block interior has LSI constant $\geq \rho_{SU(N)}/2$ (by Holley-Stroock with bounded oscillation)
\item The boundary system has $n/b$ nearly independent variables
\end{itemize}

The total LSI constant:
\[
\rho_n \geq \min\left(\frac{\rho_{SU(N)}}{2}, \rho_{SU(N)} \cdot c(\beta)\right) =: \rho_\infty
\]
where $c(\beta) > 0$ depends on $\beta$ but \textbf{not on $n$}.

\textbf{Step 9: Explicit formula.}

For the model with $\beta < 1$, the correlation length $\xi \approx 1/(2|\ln\beta|)$.

Taking block size $b = \lceil 4\xi \rceil$:
\[
\rho_\infty = \frac{\rho_{SU(N)}}{1 + 4\beta^2 \rho_{SU(N)}^2}
\]

This is strictly positive for all $\beta > 0$.
\end{proof}
