...
\textbf{Context:} This appendix details the "Extension A" derivation, replacing "Numerical Evidence" with a rigorous Computer-Assisted Proof (CAP).
...
\begin{enumerate}
    ...
    \item \textbf{Interval Bound:} For each ball, compute the image of the action under one RG step $\mathcal{R}(B_i)$ using rigorous interval bounds on the fluctuation determinant (which captures the "interaction cost").
    \item \textbf{Contraction:} If the output intervals lie strictly within $\mathcal{T}$, the \textbf{Banach Fixed Point Theorem} applies. This guarantees a unique fixed point (the massive trajectory) and analytic dependence on $g$, rigorously ruling out critical points (singularities) in the intermediate regime.
\end{enumerate}
