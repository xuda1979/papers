...
For \textbf{strong coupling} $\beta < \beta_0$ (where $\beta_0 \sim 0.44/N^2$ for $SU(N)$), 
...
\end{enumerate}
All ingredients are established mathematics. \textbf{However, this only proves 
finite-volume positivity}---the infinite-volume extension is separate.
\end{remark}

\begin{remark}[What Is Actually Proven]
\label{rem:sigma-what-proven}
To be precise:
\begin{enumerate}
\item \textbf{RIGOROUS:} $\sigma_L(\beta) > 0$ for all $\beta > 0$ and finite $L$.
\item \textbf{RIGOROUS:} $\sigma(\beta) > 0$ for $\beta < \beta_0$ (strong coupling) 
via cluster expansion with uniform-in-$L$ bounds.
\item \textbf{CONDITIONAL:} $\sigma(\beta) > 0$ for all $\beta$ in infinite volume 
requires proving the spectral gap doesn't close as $L \to \infty$.
\end{enumerate}
\end{remark}
