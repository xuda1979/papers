...
\begin{theorem}[RP monotonicity / all-coupling confinement \textnormal{[CONDITIONAL]}]
The following implication is treated as \emph{conditional} in this manuscript:
if one can establish a rigorous monotonicity principle for an appropriate confinement order parameter (e.g. vortex free energy or suitably defined Wilson loop observables) in non-Abelian $SU(N)$ lattice gauge theory, then the strong-coupling anchor $\sigma(\beta)>0$ for $\beta<\beta_0$ can be propagated to all finite $\beta$.
\end{theorem}

\begin{proof}
This is a roadmap-level statement.
At present, a fully rigorous monotonicity theorem suitable for concluding $\sigma(\beta)>0$ for all $\beta$ in 4D non-Abelian Yang--Mills is an open problem; see Appendix~\ref{sec:open-problems}.
\end{proof}
