...
Based on the comprehensive mathematical audit provided in this document (specifically \textbf{Appendix R.102}, \textbf{R.161}, and \textbf{R.165}), this section summarizes the critical \textbf{Math Errors} and \textbf{Math Gaps} that historically obstruct a rigorous proof, along with candidate \textbf{derivation modules} proposed in earlier drafts.
In the rigorous-scope framing, modules that are not fully proved are treated as \textbf{conditional} or \textbf{open problems}.

\subsection{Part 1: Identified Math Errors \& Gaps}

Several ``standard'' approaches fail upon rigorous inspection. Earlier drafts proposed replacing these heuristic arguments with the following mathematical ``patches.''
...
\begin{enumerate}
    \item \textbf{Intrinsic Scale (conditional):} Define the lattice spacing $a(\beta)$ strictly via the string tension:
    $$ \sigma a^2 = \text{constant (physical value)} $$
    This becomes non-circular only once all-coupling positivity of $\sigma(\beta)$ is established. In the rigorous-scope framing, this is an open problem.
    \item \textbf{The Dimensionless Ratio:} Construct the ratio $R = \frac{m}{\sqrt{\sigma}}$.
    \begin{itemize}
        \item A uniform lower bound $R(\beta)\ge C>0$ is a major desired input; earlier drafts attributed it to a Giles--Teper-type argument. In this version it is treated as an open problem.
    ...
    This would confirm the physical gap is non-zero without assuming it a priori, conditional on the preceding inputs.
\end{enumerate}
