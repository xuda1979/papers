...
\begin{itemize}
\item \textbf{Rigorous}: The bound $c_N \geq 2/N$ follows from the RP variational 
principle combined with Casimir scaling (see Appendix~\ref{app:definitive-gap-closure}).

\item \textbf{Numerical estimates}: Lattice simulations and effective string 
theory suggest $c_N \sim 1.4$ for $SU(3)$, but the rigorous bound $c_N \geq 2/3$ 
suffices for the existence proof.
\end{itemize}

\textbf{For our purposes}: The rigorous bound $\Delta \geq (2/N)\sqrt{\sigma}$ 
proves that $\Delta_{phys} > 0$ in the continuum limit. The stronger numerical 
value affects only predictions (293 MeV from rigorous bound vs 440 MeV from 
numerical estimates), not the existence of the gap.
\end{remark}
