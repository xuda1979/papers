...
\begin{theorem}[RG Stability with Multiscale Polymer Control]
For a fixed blocking factor $L\ge 2$ and scales $a_k=L^k a$, there exists a renormalization group map $\mathcal{R}$ such that the $k$-step effective measure $\mu^{(k)}$ satisfies:
\begin{enumerate}
    \item \textbf{Structure:} $\mu^{(k)}$ has a local effective action plus a polymer remainder:
    \[
    d\mu^{(k)}(U^{(k)}) \propto \exp\bigl(-S^{(k)}_{\mathrm{loc}}(U^{(k)})\bigr)\Bigl(1+K^{(k)}(U^{(k)})\Bigr)\prod_{e} dU^{(k)}_e.
    \]
    \item \textbf{Uniform Control:} There exist Banach norms $|\cdot|_k$ such that $|K^{(k)}|_k \le \epsilon_k$ with $\epsilon_k \le \epsilon_0 L^{-\delta k}$.
    \item \textbf{Renormalized Trajectory:} The couplings in $S^{(k)}_{\mathrm{loc}}$ follow the beta function flow and remain in the stability domain.
\end{enumerate}
\end{theorem}

\begin{proof}[Proof Strategy (Full details in Appendix \ref{app:rigorous_rg_tightness})]
The proof is constructed via four technical pillars, fully detailed in Appendix \ref{app:rigorous_rg_tightness}:
\begin{enumerate}
    \item \textbf{Block Gauge Fixing:} We use tree gauge fixing within blocks to isolate physical transverse modes from gauge orbits (Lemma I.2.1).
    \item \textbf{Small-Field Convexity:} We prove uniform lower bounds on the Hessian of the effective action in physical directions (Lemma I.6.3).
    \item \textbf{Large-Field Polymer Control:} We use the Kotecký--Preiss criterion to control the polymer expansion in the large-field region (Lemma I.5.4).
    \item \textbf{RG Contraction:} We show that the RG step is a contraction on the Banach space of polymer activities (Lemma I.7.3).
\end{enumerate}
\end{proof}
