...
\label{thm:rg_stability_main}
\label{thm:rg_stability}
For a fixed blocking factor $L\ge 2$ and scales $a_k=L^k a$, there exists a renormalization group map $\mathcal{R}$ such that the $k$-step effective measure $\mu^{(k)}$ satisfies the inductive hypotheses of the cluster expansion. Specifically, the effective action decomposes into a local convex part and a polymer remainder controlled by a contractive Banach norm.
\end{theorem}

\begin{proof}
The detailed construction of the RG map, the definition of the polymer Banach spaces, and the rigorous proof of the contraction property are provided in Appendix \ref{app:rigorous_rg_tightness}, Part I.
\end{proof}
