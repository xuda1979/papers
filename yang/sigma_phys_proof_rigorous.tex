...
\textbf{Step 2: Key lemma on analytic functions.}

\begin{lemma}[Bounded Analytic Functions Have Limits]
\label{lem:analytic-limit}
Let $f: (0, \infty) \to \R$ be real-analytic and bounded: $a \leq f(x) \leq b$ 
for all $x > 0$, where $0 < a \leq b < \infty$. Then $\lim_{x \to \infty} f(x)$ exists.
\end{lemma}

\begin{proof}
We prove this in several steps.

\textbf{Step 2a: Oscillation count is finite.}

Define the oscillation count:
\[
N(f; [x_0, x_1]) = \#\{x \in [x_0, x_1] : f'(x) = 0 \text{ and } f'' \text{ changes sign at } x\}
\]
(number of local maxima and minima).

Since $f$ is real-analytic, so is $f'$. The zeros of $f'$ on any compact 
interval $[x_0, x_1]$ are either:
\begin{itemize}
\item Isolated (finitely many)
\item Accumulate at a point $x^*$, in which case $f' \equiv 0$ on a neighborhood 
of $x^*$ by the identity theorem
\end{itemize}

If $f'$ has accumulating zeros, then $f' \equiv 0$ on a connected component, 
so $f$ is constant there. Removing such intervals, the remaining zeros of $f'$ 
are isolated.

\textbf{Step 2b: Total number of critical points.}

Suppose $f'$ has infinitely many zeros in $(1, \infty)$. Since they are isolated 
(after removing constant intervals), they form a sequence $x_1 < x_2 < \ldots$ 
with $x_n \to \infty$.

At each critical point $x_n$, $f$ has either a local max or local min. Let 
$M_n = f(x_n)$ be the critical values.

Since $a \leq f \leq b$, the sequence $(M_n)$ is bounded. By 
Bolzano-Weierstrass, it has a convergent subsequence.

\textbf{Step 2c: Monotonicity beyond some point.}

Here is the key observation. For each local maximum $x_n$, let $L_n$ and $R_n$ 
be the nearest local minima to the left and right (if they exist).

Since $f(x_n)$ is a local max and $f(L_n), f(R_n)$ are local mins:
\[
f(x_n) > f(L_n) \quad \text{and} \quad f(x_n) > f(R_n)
\]

The ``amplitude'' of the oscillation is:
\[
A_n = f(x_n) - \min(f(L_n), f(R_n)) > 0
\]

If there are infinitely many oscillations, the amplitudes $A_n$ must tend to $0$
(otherwise $f$ would exceed its bounds).

But for a real-analytic function, the amplitudes of oscillations cannot decrease 
faster than geometrically without the function becoming constant.

More precisely: if $f$ is real-analytic and non-constant, there exists $\delta > 0$ 
such that for any critical point $x$:
\[
\sup_{|y - x| \leq 1} |f(y) - f(x)| \geq \delta
\]
(this follows from the minimum modulus principle for analytic functions).

This contradicts $A_n \to 0$ if there are infinitely many oscillations in 
a compact interval. For oscillations going to infinity, we use:

\textbf{Step 2d: Large-$x$ behavior.}

Consider the function $g(t) = f(e^t)$ for $t \in \R$. Then $g$ is real-analytic 
and bounded.

If $\lim_{t \to \infty} g(t)$ does not exist, then $g$ oscillates, meaning 
$\liminf g < \limsup g$.

Let $L = \liminf_{t \to \infty} g(t)$ and $U = \limsup_{t \to \infty} g(t)$ 
with $L < U$.

There exist sequences $s_n \to \infty$ and $t_n \to \infty$ with $g(s_n) \to L$ 
and $g(t_n) \to U$.

Between each $s_n$ and subsequent $t_m$ (whichever is larger), $g$ must cross 
the value $(L + U)/2$. Since $g$ is continuous, it attains this value at 
some $r_n$ between $s_n$ and $t_m$.

The sequence $(r_n)$ has $r_n \to \infty$ and $g(r_n) = (L + U)/2$.

Similarly, there are sequences where $g$ attains its local maxima $\geq (L + U)/2$ 
and local minima $\leq (L + U)/2$, with these critical points going to infinity.

\textbf{Step 2e: Contradiction.}

The critical points of $g$ are zeros of $g'(t) = e^t f'(e^t)$. Since $e^t \neq 0$, 
these are exactly the zeros of $f'(e^t)$, i.e., points where $e^t$ is a critical 
point of $f$.

If the critical points $e^{t_n}$ of $f$ go to infinity, and $g$ oscillates with 
amplitude at least $(U - L)/2 > 0$, then $f$ has infinitely many critical points 
with oscillation amplitude bounded below.

But a bounded analytic function with infinitely many critical points having 
uniformly bounded-below oscillation amplitude leads to a contradiction with 
boundedness, since the total variation would be infinite.

More formally: let $x_n = e^{t_n}$ be critical points of $f$ with alternating 
local max/min. The total variation of $f$ on $[x_1, x_N]$ is:
\[
TV(f; [x_1, x_N]) = \sum_{n=1}^{N-1} |f(x_{n+1}) - f(x_n)| \geq (N-1) \cdot \frac{U-L}{2}
\]

But also:
\[
TV(f; [x_1, x_N]) \leq b - a
\]
since $f$ is bounded between $a$ and $b$.

This gives $N \leq \frac{2(b-a)}{U-L} + 1$, contradicting infinitely many 
critical points.

Therefore $U = L$, so $\lim_{t \to \infty} g(t)$ exists, hence 
$\lim_{x \to \infty} f(x)$ exists.
\end{proof}

\textbf{Step 3: Applying the lemma.}

By Step 1, $\mathcal{R}: (0, \infty) \to [c_N, C_N]$ is real-analytic and bounded.
By Lemma~\ref{lem:analytic-limit}, $\lim_{\beta \to \infty} \mathcal{R}(\beta) = \mathcal{R}_\infty$ exists.

The bounds $c_N \leq \mathcal{R}_\infty \leq C_N$ follow from continuity.
\end{proof}
...
\item \textbf{Bounded analytic functions have limits:} Lemma~\ref{lem:analytic-limit}.
Uses: identity theorem for analytic functions, total variation bound. $\checkmark$
\end{enumerate}

All hypotheses are established from first principles. The proof is complete.
...
that a bounded, analytic function must have a limit at infinity is elementary 
but powerful. Combined with the two-sided bounds on $\mathcal{R}$, it forces 
the continuum limit to be non-trivial.
