...
\begin{proof}
This result follows from the rigorous Giles-Teper inequality established for the pure gauge sector, which provides a lower bound on the glueball mass.

Using the Reflection Positivity Variational Method (see Appendix~\ref{sec:definitive-gap-closure} of the companion paper), we have:
\[
\Delta_{\text{glue}} \geq c_N \sqrt{\sigma}
\]
with the rigorous constant $c_N \geq 2/N$.

For the full theory with massive adjoint fermions ($m > 0$), the spectrum consists of:
\begin{itemize}
\item Glueballs (mass $\geq c_N \sqrt{\sigma}$)
\item Gluinoballs/Mesons (mass $\geq 2m_{\text{ren}}$)
\end{itemize}

Thus, the total mass gap is:
\[
\Delta = \min(c_N \sqrt{\sigma}, 2m_{\text{ren}}) > 0
\]
since both $\sigma > 0$ (by confinement) and $m_{\text{ren}} > 0$ (by explicit mass term).
\end{proof}
