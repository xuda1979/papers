...
\hmcinlineGtwo
\hmcinlineAblation
\hmcinlineAblationSig
\hmcinlineExactComb
\hmcinlinePTMPO
\hmcinlinePTMPOscaling
\hmcinlinePTMPOerror
\hmcinlineCVsummary
\hmcinlineQEC
}
...
Gentleness bound (\Cref{lem:firewall_bound}) & QEIs (A3), Hadamard (A4) & \(O(S_{\rm BH}^{-\alpha})\) & Eq.~\eqref{eq:gentleness} \\
...
\begin{itemize}[leftmargin=*,itemsep=0.2em]
  ...
  \item \textbf{Algorithm~\ref{alg:echo-stack} (Echo-stacking):} Sensitivity estimates in \cref{sec:snr_estimates}; null-result baseline in \cref{tab:echo-worked-examples,tab:echo_mapping}; control tests detailed in \cref{tab:observability}.
...
The \HMC{} framework makes specific, falsifiable predictions that distinguish it from other resolution proposals. We focus on two primary observational channels: gravitational wave echoes and analogue gravity experiments.

\subsection{Gravitational Wave Echoes}
The memory-induced modulation of the Hawking flux imprints a characteristic structure on the ringdown phase of gravitational waves emitted by a perturbed black hole. Unlike the perfectly absorbing horizon of classical GR, the \HMC{} predicts a series of "echoes" with a time delay $\Delta t_{\rm echo} \sim \tau_{\rm mem} \sim \beta \log S_{\rm BH}$.

The amplitude of these echoes is suppressed by $1/S_{\rm BH}$, making them challenging to detect for astrophysical black holes. However, the coherent "comb" structure allows for signal stacking. We estimate the Signal-to-Noise Ratio (SNR) for a template-matched filter search as:
\begin{equation}
\text{SNR}_{\rm total} \approx \sqrt{N_{\rm events}} \cdot \frac{A_{\rm echo}}{\sqrt{S_n(f)}} \cdot \sqrt{T_{\rm obs} \Delta f},
\end{equation}
where $N_{\rm events}$ is the number of merger events, $A_{\rm echo}$ is the echo amplitude, and $S_n(f)$ is the detector noise power spectral density.

\begin{table}[h]
\centering
\caption{Estimated SNR requirements for detecting \HMC{} echoes with future detectors.}
\label{tab:snr_estimates}
\begin{tabular}{lccc}
\toprule
Detector & Frequency Band & Sensitivity ($S_n^{1/2}$) & Est. SNR (Stacking 100 events) \\
\midrule
LIGO/Virgo (O5) & 10-1000 Hz & $\sim 10^{-24} \text{ Hz}^{-1/2}$ & $< 1$ \\
Einstein Telescope & 1-1000 Hz & $\sim 10^{-25} \text{ Hz}^{-1/2}$ & $\sim 3-5$ \\
LISA & 0.1-100 mHz & $\sim 10^{-21} \text{ Hz}^{-1/2}$ & $\sim 10$ (SMBH mergers) \\
\bottomrule
\end{tabular}
\end{table}

As shown in Table~\ref{tab:snr_estimates}, while current detectors are unlikely to resolve individual echoes, next-generation facilities like the Einstein Telescope and LISA may achieve sufficient sensitivity through stacking analysis of multiple merger events.

\subsection{Analogue Gravity Signatures}
...
\end{tabular}
\end{center}
These thresholds are $\gg 1/S_{\rm BH}$ for astrophysical black holes (\Cref{tab:observability}), underscoring that \emph{direct} detection of \HMC{}-induced echoes in current GW data is unlikely unless the amplitude is parametrically enhanced. Future third-generation (3G) detectors like the Einstein Telescope (ET) and Cosmic Explorer (CE) will improve strain sensitivity by an order of magnitude, lowering $\epsilon_{\rm min}$ by a factor of $\sim 10$. While this still leaves a gap to the baseline $1/S_{\rm BH}$ prediction, it will tightly constrain scenarios with enhanced coherence or effective entropy.
...
\subsection{Experimental Feasibility and Measurement Strategies}
\label{sec:exp_feasibility}
To bridge the gap between theoretical predictions and observational reality, we outline specific measurement strategies and precision requirements for detecting \HMC{} signatures.

\paragraph{Precision Requirements.}
The primary challenge is the suppression of non-Markovian effects by the black hole entropy $S_{\rm BH}$. For a signal amplitude $\epsilon \sim 1/S_{\rm BH}$:
\begin{itemize}
    \item \textbf{Analogue Systems:} With $S_{\rm BH}^{\rm eff} \sim 10^3$, we require relative precision $\Delta g^{(2)}/g^{(2)} \sim 10^{-3}$. This is achievable with high-flux sources and integration times $T_{\rm int} \gtrsim 10^6 \tau_{\rm mem}$.
    \item \textbf{Astrophysical Black Holes:} With $S_{\rm BH} \sim 10^{77}$, direct detection is impossible. However, if the effective entropy governing the memory is much smaller (e.g., due to holographic screening), signals might be enhanced.
\end{itemize}

\paragraph{Enhanced Sensitivity Techniques.}
To improve detectability, we propose:
\begin{enumerate}
    \item \textbf{Interferometric Homodyne Detection:} In optical analogue gravity, replacing direct intensity counting with homodyne detection can improve the signal-to-noise ratio for phase-sensitive memory effects.
    \item \textbf{Cavity-Enhanced Memory:} Placing the analogue system in a high-finesse cavity can effectively increase the memory time $\tau_{\rm mem}$, making the sidebands narrower and higher amplitude.
    \item \textbf{Stacking Analysis:} For GW data, coherently stacking $N$ ringdown residuals (aligned by fundamental mode phase) grows the SNR as $\sqrt{N}$.
\end{enumerate}

\paragraph{Observational Pathways for Planck Scale Physics.}
While the \HMC{} is an effective field theory framework, its breakdown points (e.g., at the scrambling time) offer windows into Planck scale physics. Deviations from the predicted $g^{(2)}$ shape at high frequencies $\omega \gg \omega_{\rm mem}$ would signal corrections to the semiclassical memory kernel, potentially revealing the granular structure of spacetime.
