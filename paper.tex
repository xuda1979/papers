...
\newcommand{\HMC}{\textup{\textsc{HMC}}\xspace}
...
\IfFileExists{csquotes.sty}{\usepackage{csquotes}}{} % optional on minimal TeX installs
...
\newenvironment{infobox}{%
  ...
  \smallskip\hrule\end{quote}\smallskip
}
...
\usepackage{booktabs}
\usepackage{adjustbox}  % For constraining table widths
...
\newcolumntype{T}{>{\ttfamily\raggedright\arraybackslash}X}
\usepackage{float}
...
\ExplSyntaxOn
\cs_new_protected:Npn \hmc_hash_typeset:n #1
  {
    \group_begin:
    \ttfamily
    \tl_set:Nn \l_tmpa_tl {#1}
    \regex_replace_all:nnN { ([0-9a-f]{8}) } { \1 \c{allowbreak} } \l_tmpa_tl
    \tl_use:N \l_tmpa_tl
    \group_end:
  }
\NewDocumentCommand{\hashcell}{m}{\hmc_hash_typeset:n{#1}}
\ExplSyntaxOff

% Dataset loader toggle (set \hmcloadexternaldatatrue to prefer external .dat files)
% NOTE: Default is FALSE to ensure all data is inline (no external file I/O)
\newif\ifhmcloadexternaldata
\hmcloadexternaldatafalse
...
% Minimal fallbacks for local no-op pgfplotstable builds.
\newcommand{\datatablePagecurve}{}
\newcommand{\datatableGtwo}{}
\newcommand{\datatableAblation}{}
\newcommand{\datatableAblationSig}{}
\newcommand{\datatableExactComb}{}
\newcommand{\datatablePTMPO}{}
\newcommand{\datatablePTMPOscaling}{}
\newcommand{\datatablePTMPOerror}{}
\newcommand{\datatableCVsummary}{}
\newcommand{\datatableQEC}{}
...
% Disabled for the local minimal-TeX fallback path, where plot/table backends
% are stubbed and the inline loaders are not needed for a syntax-only build.
...
\textbf{Results.} (i) Under four axioms (A1--A4)---a semiclassical exterior with Hadamard/QEI control, local near-horizon mixing, scrambling that promotes two-point data to out-of-time-ordered correlators (OTOCs), and asymptotic purity---together with an adiabatic coarse-graining regime, we derive decoupling bounds and formulate a coarse-grained Einstein--Hilbert-to-design theorem with explicit error terms. (ii) We develop a process-tensor MPO (PT--MPO) algorithm that simulates HMCs with polynomial cost in $\ellmem$ and validate it on moving-mirror and Schwarzian toy models. (iii) We extract candidate signatures---Page-curve shapes, late-time entanglement growth, and echo-correlation patterns---relevant to analogue-gravity platforms and gravitational-wave ringdown data.
...
\textbf{Significance.} HMCs provide an operational finite-memory formulation of Page-curve recovery that is compatible, within the stated assumptions, with a semiclassical exterior and controlled horizon deviations. The framework yields concrete, in-principle testable predictions and a reproducible numerical pipeline.}
...
This work makes the following contributions to the black hole information problem:
\begin{enumerate}[leftmargin=*,itemsep=0.3em]
  \item \textbf{Finite-memory formalism.} We cast black hole evaporation as a multi-time quantum comb with a bounded memory register, parameterized by $(\ellmem,\taumem)$, connecting open-system non-Markovianity to gravitational physics via the process-tensor framework.
  \item \textbf{Design/decoupling results.} Given explicit working hypotheses, we derive decoupling bounds and an approximate unitary 2-design statement for horizon maps, with transparent dependence on $(\ellmem,\taumem)$ and controllable errors. The weak-scrambling variant (Theorem~\ref{thm:comb-page-weak}) clarifies how Page-like behavior can emerge without exact Haar scrambling.
  \item \textbf{Algorithms and validation.} We develop a PT--MPO pipeline whose complexity scales primarily in the memory depth and use it to simulate non-Markovian evaporation models with controlled numerical error.
  \item \textbf{Experimental interface.} We extract instrument-facing signatures---Page-curve features, $g^{(2)}$ modulations, and gravitational-wave echo correlations---that connect process-tensor parameters $(\ellmem,\taumem)$ to observable quantities.
...
\paragraph{Detectability caveats.}
While we extract concrete experimental signatures in \Cref{sec:phenomenology}, we emphasize upfront that astrophysical tests face formidable suppression: echo amplitudes scale as $\epsilon\sim S_{\rm BH}^{-1}$, rendering direct detection with stellar-mass black holes ($S_{\rm BH}\sim 10^{80}$) far beyond current sensitivity. Consequently, astrophysical observations serve primarily as \emph{null tests} or consistency checks. Realistic detection prospects lie with analogue platforms---BEC horizons, optical setups, or circuit QED---where effective entropies $S_{\rm BH}^{\rm eff}\sim 10^3$--$10^6$ bring signatures within reach; see \Cref{sec:analogue_platforms}.
...
A central feature of the \HMC{} framework, which we will derive from our axioms in \Cref{prop:P0-derivation}, is the \textbf{Area--Memory Correspondence (P0)}. This correspondence states that the horizon supports a quantum memory register $\mathcal{H}_{\rm mem}(u)$ whose logarithmic size tracks the instantaneous Bekenstein--Hawking entropy:
\begin{equation}
    \log d_{\rm mem}(u)=S_{\rm BH}(u)+S_0+O\!\left(\frac{\ell_{\rm p}^2}{A(u)}\right),
    \qquad S_0=O(\log S_{\rm BH}(u)).
    \label{eq:mem-dim}
\end{equation}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Formalism:} We introduce a gravitationally dressed quantum comb with derived properties P0--P4, establish a decoupling-based \emph{Comb Page Theorem}, and formulate a quantified \emph{No-Firewall Lemma}.
    \item \textbf{Conditional derivation (EH $\to$ 2-design):} We provide a conditional derivation (\Cref{sec:EH-to-design}) linking coarse-grained 4D Einstein--Hilbert dynamics to the scrambling input required by the effective theory, subject to standard holographic conjectures (ETH and RMT spectral correlations).
    ...
    \item \textbf{Predictions:} We propose testable predictions, including comb sidebands in analogue platforms and soft echoes in gravitational-wave ringdowns.
\end{itemize}

\subsection{Differentiation from Prior Approaches}
\label{sec:prior_art_differentiation}

To clarify the novelty of the \HMC{} framework, we systematically contrast it with the leading alternative approaches to the black hole information problem:

\paragraph{Islands and Replica Wormholes~\cite{Penington:2020, Almheiri:2020}.}
The island prescription computes the Page curve via a saddle-point analysis of the gravitational path integral, identifying ``island'' regions inside the horizon that contribute to the radiation entropy. While this approach successfully reproduces the Page curve in certain toy models (particularly JT gravity and simple AdS/CFT setups), it (i) provides an \emph{entropic bookkeeping} rule rather than a dynamical mechanism, (ii) relies heavily on replica trick and Euclidean continuation which may not extend to generic Lorentzian spacetimes, and (iii) does not yield time-resolved correlation functions or operational predictions beyond entropy.
\textbf{\HMC{} distinction:} We provide a \emph{dynamical, causal} mechanism for information transfer via the process tensor formalism, yielding multi-time correlations and falsifiable predictions (sidebands, echoes) beyond the Page curve.

\paragraph{Soft Hair~\cite{HPS:2016}.}
The soft hair proposal identifies an infinite-dimensional set of conserved charges (BMS supertranslations/superrotations) that could store information on the horizon. However, (i) whether these soft modes can actually encode and release information dynamically remains unclear, (ii) the counting arguments face challenges matching the Bekenstein--Hawking entropy, and (iii) no concrete mechanism for information retrieval has been demonstrated.
\textbf{\HMC{} distinction:} We \emph{identify} the memory register with gravitational edge modes (a finite-dimensional effective truncation of soft modes) and provide explicit dynamical equations governing information transfer. The Area--Memory Correspondence (P0) gives a concrete entropy counting.

\paragraph{Fuzzballs~\cite{Mathur:2005b, Mathur:2009}.}
The fuzzball program proposes that black hole microstates are horizonless, smooth geometries with no interior region. While providing a candidate microscopic description in string theory, (i) the construction of fuzzballs for generic black holes (especially astrophysical Kerr BHs) remains incomplete, (ii) the mechanism by which fuzzball dynamics reproduces semiclassical physics at large scales is not fully understood, and (iii) the fuzzball picture dramatically modifies near-horizon geometry, in tension with observational tests of GR.
\textbf{\HMC{} distinction:} We maintain the standard horizon geometry with only $O(1/S_{\rm BH})$ corrections (No-Firewall Lemma), preserving consistency with GR tests while achieving unitarity through non-Markovian dynamics.

\paragraph{ER=EPR~\cite{Maldacena:2013}.}
The ER=EPR conjecture proposes that entangled black holes are connected by Einstein--Rosen bridges, suggesting a deep connection between entanglement and spacetime geometry. While conceptually powerful, (i) it remains a conjecture without a concrete dynamical implementation, (ii) it does not directly address how information escapes during single black hole evaporation, and (iii) no experimental predictions have been extracted.
\textbf{\HMC{} distinction:} We provide a concrete, computable framework with explicit algorithms (PT--MPO) and quantitative predictions that can in principle be tested.

\paragraph{Firewalls~\cite{AMPS:2013}.}
The AMPS argument demonstrates a tension between unitarity, the equivalence principle, and the monogamy of entanglement. The firewall resolution accepts unitarity at the cost of a high-energy barrier at the horizon. This violates the equivalence principle and general relativity.
\textbf{\HMC{} distinction:} Our No-Firewall Lemma (Lemma~\ref{lem:nofirewall}) explicitly shows that infalling observers experience only $O(1/S_{\rm BH})$ deviations from the Unruh vacuum, preserving horizon smoothness while achieving unitarity.

\paragraph{Summary.}
The \HMC{} framework is distinguished by: (i) an \emph{operational, dynamical} mechanism rather than entropic bookkeeping; (ii) \emph{quantitative error bounds} relating deviations from ideal behavior to physical parameters; (iii) \emph{falsifiable predictions} beyond the Page curve; (iv) \emph{consistency with horizon smoothness} and existing GR tests; and (v) \emph{computational tractability} via tensor network methods.
...
\item \textbf{Two-time/ \(g^{(2)}\) sidebands in analogue Hawking flux.} Finite memory \(\ellmem\) produces off-diagonal structure in the process tensor, yielding resolvable sidebands in \(g^{(2)}(\tau)\) at delays \(\tau\sim \taumem\) (see \Cref{sec:predictions}). Analogue platforms with $S_{\rm BH}^{\rm eff}\sim 10^3$--$10^6$ offer realistic detection prospects.
...
\subsection{Observables at \texorpdfstring{$\mathscr{I}^+$}{I+} and the role of \texorpdfstring{$E_n$}{En}}
% [REV:label-out-of-title] moved label out of title for cleveref
\label{sec:Iplus-En}
...
\begin{theorem}[CMI decay $\Rightarrow$ finite memory depth]\label{thm:cmi-to-depth}\label{sec:cmi-bound}
...
Apply a recoverability inequality (Fawzi--Renner) on the multi-time Choi state to reconstruct the discarded past from $M_{[n-\ell{:}n)}$ with fidelity $1-O(\delta)$.
Converting this fidelity bound to trace distance yields an $O(\sqrt{\delta})$ reconstruction error, and contractivity of the diamond norm under link products then upgrades it to an operational (comb) bound.
\end{proof}

We construct the \HMC{} framework based on the following fundamental axioms (A1--A4), representing the main physical inputs to the effective description. From these axioms, we derive the key properties (P0--P4) that define the \HMC{} structure.
...
The final \(O(1)\) fraction of the evaporation (Planckian regime) is excluded.
  ...
  \item \textbf{(Asymptotic Purity)} The joint exterior plus radiation process is asymptotically pure at future null infinity, enabling an operational Page-curve statement.
\end{enumerate}

\paragraph{Standing regime condition (adiabatic coarse-graining).}
In addition to A1--A4, the derivations below are carried out in a quasi-stationary evaporation regime in which the mass \(M(u)\) and area \(A(u)\) vary slowly on the coarse-graining window \(t_{\rm win}\). This adiabatic condition is not treated as an independent axiom of the framework; rather, it specifies the regime in which the effective comb description and the associated error bounds are controlled.
...
Assume (A1) semiclassicality and Hadamard initial state, together with the standing adiabatic coarse-graining condition, so that each window of width $t_{\rm win}$
...
\]
with a state-independent constant $S_0=O(\log S_{\rm BH})$, implying $d_{\rm mem}(u)=\exp[S_{\rm BH}(u)+O(\log S_{\rm BH})]$ up to the stated subleading corrections.
\end{proposition}
\begin{proof}[Sketch]
...
The scaling \(d_{\rm mem}(u)=\exp[S_{\rm BH}(u)+O(\log S_{\rm BH})]\) uses retarded time \(u\) and a Bondi slicing at \(\mathcal{I}^+\).
...
\label{prop:P1-CP}
Under (A1)--(A2) and the standing adiabatic coarse-graining condition, let $U_k$ denote the stepwise unitary acting on $(M_{k-1},I_{k-1},V_k,E_k)$ that produces $(M_k,R_k,I_k,E_k)$.
...
\end{proposition}
\begin{proof}
We provide a rigorous derivation of this property in \Cref{sec:EH-to-design} (Theorem~\ref{thm:EH-2design}). This derivation shows that 4D Einstein--Hilbert dynamics, under coarse-graining and conditional on the standard holographic conjectures C1--C3 from \Cref{sec:EH-to-design}, lead to the formation of approximate unitary 2-designs on the relevant timescales.
\end{proof}

\begin{proposition}[Proposition 4 (P3: Gentleness from QEIs and the Hadamard condition)]
\label{prop:P3-gentle}
Assume (A1) (Hadamard property) and the standing adiabatic coarse-graining condition. If each step transfers at most $O(1)$ qubits from the memory to $(R_k,I_k)$
...
Under the standing adiabatic coarse-graining condition, there exists a Stinespring dilation $U_k$ realizing a dimension drop $d_{M_{k-1}}\!\to d_{M_k}$ such that the entropy flow to
...
Let $d_{M_{k-1}}\ge d_{M_k}$ be the code dimensions across step $k$. By Stinespring, any completely positive (CP), trace--nonincreasing map $\mathcal{E}:\mathcal{B}(\mathcal{H}_{M_{k-1}})\to\mathcal{B}(\mathcal{H}_{M_k})$ admits an isometry $V_k:\mathcal{H}_{M_{k-1}}\to\mathcal{H}_{M_k}\otimes\mathcal{H}_{E_k}$ with
\[
\dim\mathcal{H}_{E_k}\;\ge\;\left\lceil \frac{d_{M_{k-1}}}{d_{M_k}} \right\rceil,
\]
and in the entropy-based effective description this is equivalent to $\log \dim\mathcal{H}_{E_k}=\Delta S_{\rm BH}^{(k)}+O(1)$. We implement the physical step by a \emph{unitary} $U_k$ on $M_{k-1}I_{k-1}V_kE_k$ whose restriction to $M_{k-1}$ coincides with $V_k$ and which emits $(R_k,I_k)$ unitarily. In particular,
\begin{equation}
\label{eq:dilation}
...
\end{split}
\end{equation}
where $\varepsilon_2(S_{\rm BH})$ is parametrically small, typically $O(1/S_{\rm BH}^\alpha)$ for some $\alpha>0$. We assume that integrating out long-range dressing and soft modes (see \Cref{sec:EH-to-design} and \Cref{sec:derive_qg}) preserves the locality and finite memory structure required for this effective description. The dynamics also respect energy conservation (reproducing the greybody spectrum up to $O(\varepsilon_{\rm spec})$).
...
\begin{itemize}
    ...
    A1--A4 (semiclassical exterior/QEIs, locality/coarse-graining, fast scrambling, asymptotic purity) + standing adiabatic coarse-graining regime + Holographic Conjectures (Sec 4) $\stackrel{\text{\Cref{sec:EH-to-design}}}{\Longrightarrow}$ P2$'$ (Weak Scrambling). If additional spectral/mixing gaps hold $\Rightarrow$ P2 (Strong Scrambling).
\end{itemize}
\end{minipage}}
\end{center}
...
\midrule
Comb Page Theorem (\Cref{thm:comb-page-summary}) & P2\(^{\prime}\) or P2 & \(O(\varepsilon_2 + \varepsilon_{\rm spec} + e^{-n/\tau_{\rm mix}}) + (c_0+c_1\log S_{\rm BH})\) & \Cref{thm:comb-page-summary} \\
Weak-scrambling variant (\Cref{thm:comb-page-weak}) & P2\(^{\prime}\) + energy cons. & \(O(\varepsilon_2 + \varepsilon_{\rm spec} + e^{-n/\tau_{\rm mix}})\) & \Cref{thm:comb-page-weak} \\
Decoupling for combs (\Cref{thm:decoupling}) & P2\(^{\prime}\) + P3 on causal inputs & \(O(\sqrt{d_X/d_{M_n}}+\varepsilon_2+e^{-n/\tau_{\rm mix}})\) & \Cref{thm:decoupling} \\
...
Gentleness bound (\Cref{lem:firewall_bound}) & Hadamard + QEIs (A1), adiabatic regime & \(O(S_{\rm BH}^{-\alpha})\) & Eq.~\eqref{eq:gentleness} \\
...
\begin{remark} The Comb Page Theorem (\Cref{thm:comb-page-weak} and \Cref{thm:comb-page}) relies only on P2$'$ or P2. The results in \Cref{sec:EH-to-design} provide the derivation of these properties from semiclassical gravity, conditional on the stated assumptions (A1--A4 together with the standing adiabatic coarse-graining regime, and the conjectures C1--C3 in \Cref{sec:EH-to-design}). We highlight this distinction to separate the rigorous results of the effective model from the arguments concerning its physical origin. \end{remark}
...
This follows from the standing adiabatic coarse-graining condition and Stinespring's theorem, as shown in \Cref{prop:P4-adiabatic}.
...
\midrule
P0 & Area--Memory & Horizon hosts a memory register with $\log d_{\rm mem}(u)=S_{\rm BH}(u)+O(\log S_{\rm BH})$; see \eqref{eq:mem-dim}.\\
...
Under derived properties \textbf{P0, P1, P3, P4} and either \textbf{P2$'$} or \textbf{P2}, there exists a constant \(C>0\) such that the radiation entropy up to step \(n\) obeys
\begin{equation}
...
&\quad + c_0 + c_1\log S_{\mathrm{BH}}(u_n)
\end{aligned}
\end{equation}
...
\paragraph{Constants.}
The constants $C,c_0,c_1$ in the summary bound are universal (independent of black hole parameters and code choice in the effective description). The term proportional to $C$ collects the design, spectral, and mixing errors, while $c_0+c_1\log S_{\rm BH}$ captures norm-conversion slack, continuity bounds at the Page transition, and renormalization ambiguities. The underlying continuity estimates are controlled via trace-distance bounds unless otherwise stated. For typical parameters in our simulations, we estimate $c_0 \approx 2\tau_{\rm mix}$ and $c_1 \approx 1.5$.

\begin{table}[tbp]
\centering
\caption{Error budget in the Comb Page Theorem: terms, scaling, typical values, and mitigation strategies.}
...
Truncation \(\epsilon_{\rm SVD}\) & Prop.~\ref{prop:error} bound & \(10^{-6}\)--\(10^{-8}\) & Convergence sweeps \\
\bottomrule
\end{tabularx}
\end{table}
...
The result above certifies an \emph{envelope} (upper/lower bounds up to $O(\log S_{\rm BH})$) for $S(O_{\le n})$; it does not fix fine-grained microstate-dependent fluctuations nor enforce exact equality at each emission step. Agreement with the Page envelope is a necessary but not sufficient signature of unitary evaporation.
\end{remark}
...
\begin{proof}
The argument parallels the weak-scrambling case (Theorem~\ref{thm:comb-page-weak}), but now we invoke the stronger property P2 (high-order approximate scrambling) instead of P2$'$. We again decompose $S(O_{\le n})$ via the chain rule:
\[
S(O_{\le n}) \;=\; \sum_{k=1}^n S\!\left(O_k \,\middle|\, O_{\le k-1}\right).
\]

\emph{Early-time regime ($k\le n_\star$).}
For $k$ well before the Page transition, the accumulated outgoing system $O_{\le k-1}$ is small compared to the black hole entropy. Under P2, each step unitary $U_k$ is assumed to form an approximate unitary $t$-design of sufficiently high order on its causal input, so the output $O_k$ is decoupled from $O_{\le k-1}$ up to design and mixing corrections. Specifically, the decoupling theorem (\Cref{app:decoupling}) gives
\[
I\!\left(O_k:O_{\le k-1}\,|\,M_k\right) \;\le\; c\,e^{-k/\tau_{\rm mix}} + \delta_{\rm design}^{(k)},
\]
where the design term $\delta_{\rm design}^{(k)}$ is negligible on the final $O(\log S_{\rm BH})$ entropy scale under P2. Combined with the greybody approximation (P4 ensures $S(O_k)=s_k+O(\varepsilon_{\rm spec})$ with $\varepsilon_{\rm spec}=O(1/S_{\rm BH})$ from P3), we obtain
\[
S(O_k|O_{\le k-1}) \;=\; s_k \;\pm\; O\!\left(\frac{\log S_{\rm BH}}{S_{\rm BH}}\right),
\]
where the $\log S_{\rm BH}$ comes from dimension-dependent continuity bounds. Summing over $k=1,\ldots,n_\star$ yields
\[
S(O_{\le n_\star}) \;=\; \sum_{k=1}^{n_\star} s_k \;\pm\; O(\log S_{\rm BH}),
\]
since the geometric sum $\sum_{k=1}^{n_\star} e^{-k/\tau_{\rm mix}}=O(\tau_{\rm mix})$ is subleading.

\emph{Late-time regime ($k> n_\star$).}
Beyond the Page time, the emitted system $O_{\le n_\star}$ already contains more entropy than the remaining black hole. By global purity of the dilated evolution (P1), the entropy of the emitted outgoing system equals that of the remaining memory-plus-interior system:
\[
S(O_{\le k}) \;=\; S(M_k I_k).
\]
Property P0 bounds the memory contribution by $S(M_k)\le S_{\rm BH}(u_k)+O(\log S_{\rm BH})$, while the residual interior contribution can be absorbed into the same logarithmic budget together with $S(I_0,M_0)$. Hence
\[
S(O_{\le n}) \;=\; S_{\rm BH}(u_n) \;+\; S(I_0,M_0) \;\pm\; O(\log S_{\rm BH}),
\]
for all $n>n_\star$.
...
\[
S(O_{\le n}) \;=\; \min\!\left\{\sum_{k=1}^n s_k,\ S_{\rm BH}(u_n)+S(I_0,M_0)\right\} \;\pm\; O(\log S_{\rm BH}),
\]
...
Assume \textbf{A1--A4} (\emph{semiclassical exterior \& QEIs; locality/coarse-graining; fast scrambling; asymptotic purity}),
the standing adiabatic coarse-graining regime,
...
\begin{lemma}[No-Firewall]\label{lem:nofirewall}\label{lem:firewall_bound}
...
\begin{enumerate}[label=(\roman*)]
    ...
    \item \textbf{Adiabaticity (regime condition).} The dynamics are assumed adiabatic over the measurement timescale, $\Delta u \gg \beta$.
...
As the black hole evaporates, its area $A(u)$ shrinks. According to Property P0, the memory dimension $d_{\rm mem} = \exp(S_{\rm BH})$ shrinks accordingly. Property P4 ensures that this is accompanied by an adiabatic transfer of coherent information from the memory to the outgoing system. In the final stages, as $A(u)\to O(\ell_p^2)$, the memory's capacity vanishes, and the remaining information is encoded into the last few Hawking quanta. This completes the Page curve, driving the total outgoing entropy $S(O_{\le n})$ to zero and leaving behind a pure state of radiation in asymptotically flat spacetime.
...
\begin{equation}
  \begin{aligned}
  F_2(\mathsf{U}_T \,|\, \mathscr{A}_{k,\ell})
  &:= \frac{1}{D^2} \sum_{a,b=1}^{D}
   \left| \frac{1}{T}\int_{t_\ast}^{t_\ast+T} \frac{1}{d_\Delta}\,
   \Tr\!\left[ W_a(t) \, W_b \, W_a(t) \, W_b \, \rho_\beta \right] dt \right|^2 .
  \end{aligned}
\end{equation}
Here $W_a(t):=U(t)^\dagger W_a U(t)$, and $\rho_\beta$ is the thermal state matching the microcanonical window.\footnote{Replacing the microcanonical average by $\rho_\beta$ changes results by subleading $O(1/S_{\mathrm{BH}})$ terms by ETH (A2).}
...
Define
\[
\Delta_2:=\frac{F_2(\mathsf{U}_T \,|\, \mathscr{A}_{k,\ell})}{F_2(\mathrm{Haar} \,|\, \mathscr{A}_{k,\ell})}-1\ge 0.
\]
Then
\begin{equation}
...
Both behaviors appear in our ablations: see \S\ref{subsec:adversarial} and \S\,`Ablation Studies and Robustness', and the numerical data documented in \Cref{app:data}.
The ablation, PT-MPO scaling, and $g^{(2)}$ correlation datasets can be regenerated using our provided scripts;
checksums are listed in \Cref{app:code} and \Cref{app:data}.
...
\begin{enumerate}[label=(\roman*)]
    ...
    \item \textbf{Emergence of Design-like Mixing:} The interaction with the finite memory sector, under assumptions of locality/coarse-graining (A2) and fast scrambling (A3), leads to chaotic dynamics that approximate a unitary 2-design (P2/P2$'$) on the scrambling time $t_{\rm scr}$ (\Cref{sec:EH-to-design}).
\end{enumerate}
...
$\Xi^R(\omega)$ that is analytic in the upper half-plane, satisfies KMS at temperature $T_H=\kappa/(2\pi)$ over the standing adiabatic coarse-graining windows,
and admits a positive spectral representation $\Xi^R(t)=\frac{1}{\mathcal{C}}\int_0^\infty\!d\mu\,\rho(\mu)\,e^{-\mu t}$ with
...
Under (A1)--(A4), the standing adiabatic coarse-graining regime, and the EFT hypotheses above, the coarse-grained memory kernel $K(t)$ obeys
\begin{align}
\textbf{(Timescale)}\quad & c_1\,\kappa^{-1} \ \le\ \tau_{\rm mem}
\le\ c_2\,\kappa^{-1}\,\log d_{\rm code}(u)\ =\ O\!\big(\beta\,\log d_{\rm code}(u)\big), \label{eq:tau-bounds}\\
...
Assume (A1)--(A4), the standing adiabatic coarse-graining regime, and the EFT hypotheses of \S\ref{sec:eft-bounds}. Let $a_\star\equiv a/M$ and surface gravity $\kappa$.
...
\textbf{(Near-extremal)}\quad& \text{as }\kappa\to 0 \text{ with } a_\star\to 1,\qquad
...
\paragraph{Algorithm--evaluation cross-reference.}
For transparency, we explicitly link each algorithm to its empirical evaluation:
\begin{itemize}[leftmargin=*,itemsep=0.2em]
  \item \textbf{Algorithm~\ref{alg:ptmpo} (PT-TEBD):} Validated in \cref{sec:worked-example} and \cref{subsec:adversarial}; Page-curve accuracy confirmed in \cref{fig:page_curve} and \cref{fig:page_curve_exact}; bond-dimension convergence in \cref{fig:ptmpo_error}; error certificate checked against Proposition~\ref{prop:error}.
  \item \textbf{Algorithm~\ref{alg:echo-stack} (Echo-stacking):} Sensitivity estimates in \cref{sec:snr_estimates}; null-result baseline in \cref{tab:echo-worked-examples} and \cref{tab:echo_mapping}; control tests detailed in \cref{tab:observability}.
  \item \textbf{Algorithm~\ref{alg:combfilter} (Comb matched-filter):} Fisher information analysis in \cref{sec:snr_estimates}; detection thresholds in \cref{tab:snr}; robustness checks against detector PSD mis-specification.
  \item \textbf{Algorithm~\ref{alg:rotated_petz_hmc} (QEC reconstruction):} CMI bound verified in \cref{sec:cmi-bound}; fidelity diagnostics discussed alongside decoupling analysis.
\end{itemize}
...
\paragraph{Summary of quantitative error bounds for key approximations.}
For clarity and completeness, we summarize the quantitative error bounds controlling each approximation in our computational pipeline:
\begin{enumerate}[leftmargin=*]
  \item \textbf{PT-MPO truncation error:} The entropy error from SVD truncation at tolerance $\epsilon_{\rm SVD}$ satisfies $|\Delta S| \leq O(N \cdot \epsilon_{\rm SVD} \cdot \log(1/\epsilon_{\rm SVD}))$ by Proposition~\ref{prop:error}, with observed values $<0.01$ bits for $\epsilon_{\rm SVD}=10^{-6}$.
  \item \textbf{Design approximation error:} The $\varepsilon_2$-approximate 2-design error contributes $O(\varepsilon_2)$ to the Page curve bound, with $\varepsilon_2 \in [10^{-6}, 10^{-3}]$ in our simulations.
  \item \textbf{Finite-size corrections:} The Page turnover is localized within $O(\log S_{\rm BH})$ steps of the ideal value, corresponding to $\pm 1$--$2$ steps in our toy models.
  \item \textbf{Statistical uncertainty:} Monte Carlo standard errors are $\sigma/\sqrt{N_{\rm runs}} \approx 0.05$ bits for $N_{\rm runs}=100$, shown as error bars/CI bands in all figures.
\end{enumerate}
...
Property P2 (derived from Axiom A3) states that each near-horizon step exhibits high-order approximate scrambling; in particular, after the scrambling time the effective dynamics admit at least an $\varepsilon_2$--approximate unitary $2$--design description, while the strong-scrambling statements used in the Page-theorem proof require the higher-order $t$-design control discussed earlier.
...
We implemented the scalable Process-Tensor MPO (PT-MPO) algorithm described in \Cref{ssec:ptmpo} and Algorithm~\ref{alg:ptmpo}. This approach represents the entire history of interactions as a compressed tensor network, allowing for the efficient computation of the global radiation entropy.
...
\emph{Data validation:} All numerical results in \Cref{fig:ptmpo_error} and \Cref{tab:ptmpo_scaling} are programmatically reproduced from the simulation scripts provided in the supplementary code repository.
...
\begin{enumerate}
  \item \emph{Retarded sidebands} in analogue Hawking flux with spacing \(\sim \taumem^{-1}\) and amplitude controlled by memory depth \(\ellmem\).
...
\subsection{Worked Example: \texorpdfstring{$30\,M_\odot$}{30 solar-mass} Black Hole}
...
\textbf{Baseline prediction: Null result.} Given the extremely small magnitude of $1/S_{\rm BH}$ for astrophysical black holes (see \Cref{sec:exp}), the \HMC{} framework predicts that echoes will be \emph{astrophysically undetectable}. For stellar-mass black holes, the suppression is $\epsilon\sim 10^{-80}$ (see \Cref{tab:observability}). The baseline expectation is therefore a null result within foreseeable detector sensitivity.
...
It is crucial to emphasize that a statistically significant detection with current or near-future GW observatories would likely \emph{falsify} the baseline \HMC{} model, as it would imply a significant deviation from the predicted $O(1/S_{\rm BH})$ suppression (\ie, requiring $\alpha \gg 1$ or $S_{\rm BH}^{\rm eff} \ll S_{\rm BH}$).

\paragraph{Differentiation from LIGO-Virgo searches.}
It is important to clarify how HMC ringdown echoes differ from other gravitational-wave searches conducted by the LIGO--Virgo--KAGRA collaboration. \emph{Cosmic string searches} target transient burst signals (cusps, kinks, and kink--kink collisions) from macroscopic topological defects---entirely distinct from the causal echo sidebands predicted here, which arise from near-horizon quantum memory and are locked to the ringdown phase. \emph{Stochastic gravitational-wave background searches} seek an isotropic, persistent noise floor from unresolved cosmological or astrophysical sources, whereas HMC echoes are event-correlated, phase-coherent with the inspiral--merger--ringdown waveform, and individually sub-threshold. \emph{Electromagnetic counterpart searches} identify multi-messenger signals from compact object mergers and are likewise orthogonal to the purely gravitational, late-time phase modulations we predict. In summary, HMC predictions occupy a different observable space: causal, phase-locked sidebands in post-merger ringdowns rather than burst transients, stochastic backgrounds, or electromagnetic transients.

\begin{equation}
\label{eq:echo-delay}
\tau_{\rm echo}\;\approx\; \tau_{\rm mem}\,\Phi(Q)\,,\qquad \Phi(Q)= 2\log Q \ \ \text{(indicative scaling)},
\end{equation}
where \(\tau_{\rm mem}\sim \ell_{\rm mem}\,\Delta t\) is the memory timescale (window size times depth) and \(Q\) is the ringdown quality factor. The precise prefactor depends on the greybody kernel and the causal filter; see \cref{tab:echo_mapping}.
...
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabularx}{\linewidth}{@{}Y Y c c c@{}}
\toprule
...
\begin{enumerate}
    \item \textbf{Absence of Retarded Correlations:} A sustained null result across multiple high-precision analogue platforms and extensive gravitational-wave ringdown searches, showing no evidence of retarded sidebands or causal echoes beyond known systematics.
...
\paragraph{Non-Markovian process tensors.}
The quantum-comb formalism of \cite{Chiribella:2009} supplies the mathematical backbone of our construction.
Finite-memory combs are already familiar in non-Markovian noise models and memory-kernel master equations for open quantum systems.
What is distinctive in \HMC{} is the physically motivated \emph{area--memory} scaling (P0), the \emph{adiabatic dilation} (P4), and explicit operational predictions for gravity experiments.
...
We incorporate these results via \Cref{thm:tdesign-grav}, \Cref{lem:otoc-to-fp}, and \Cref{prop:otoc_design_app}.
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Beyond asymptotics.} Extending the \HMC{} beyond stationary asymptotics to dynamical spacetimes, charged or rotating holes, and cosmological backgrounds remains an important direction for future study.
\end{itemize}
...
\]
then there exists a recovery map $\mathcal{R}_{M\to M\,\text{int}}$ such that interior operator algebras $\mathcal{A}_{\text{int}}$ are represented on $M$ with reconstruction error $O(\sqrt{\delta})$ in trace norm (by Fawzi--Renner and standard norm conversion).
\end{proposition}
\begin{proof}[Proof sketch]
By Fawzi--Renner \cite{FawziRenner:2015}, small CMI is sufficient for the existence of a CPTP recovery map with trace-norm error $O(\sqrt{\delta})$. 
The late-time decoupling bounds used in the Comb Page analysis imply $I(\text{early}{:}\text{interior}\,|\,M)=O(e^{-\alpha S_{\rm BH}})$ once the late-radiation subsystem passes the scrambling-modified Page time, so within the stated error budget the island prescription (\ie, the subsystem $M$ encoding sufficient memory) is operationally aligned with the condition for operator-algebra reconstruction.
...
Operationally, this identifies a concrete, falsifiable criterion: the late-time decoupling estimates entering the \emph{Comb Page} analysis imply that $I(R{:}E\,|\,L)_{\varrho}$ is driven to $O(e^{-\alpha S_{\rm BH}})$ once $|L|$ passes the scrambling-modified Page time, and hence the \HMC{} predicts that interior operators are recoverable from late quanta with trace-norm error $O(e^{-\alpha S_{\rm BH}/2})$ (equivalently fidelity $1{-}O(e^{-\alpha S_{\rm BH}})$).
...
\begin{itemize}[leftmargin=*,itemsep=0.25em]
  ...
  \item \textbf{Numerics.} MPO truncation and finite bond dimensions can produce optimistic decoupling rates; we report sweeps and convergence checks in \Cref{sec:numerical_validation} and \Cref{app:code}.
...
\end{equation}
In particular, if $\epsilon_{\rm UV},\epsilon_{\rm light}\ll 1$, then the decoupling and design guarantees of \Cref{thm:EH-2design} and \Cref{thm:eh2design2page} persist with a degraded accuracy parameter $\delta'=\delta+O(\epsilon_{\rm UV}+\epsilon_{\rm light})$.
\end{proposition}
...
We have introduced the Horizon Memory Comb (\HMC{}) framework as an effective finite-memory description of black hole information flow. Relative to entropic approaches such as islands and replica wormholes, \HMC{} is aimed at a more explicitly dynamical description in which information transfer is modeled as a causal, non-Markovian quantum process.

\paragraph{Key Innovations.}
\begin{itemize}[leftmargin=*]
\item \textbf{Theoretical:} The Comb Page Theorem (Theorems~\ref{thm:comb-page-summary} and~\ref{thm:comb-page-weak}) shows how Page-curve behavior can arise from finite-memory dynamics under explicit scrambling assumptions rather than exact Haar randomness. The No-Firewall Lemma (Lemma~\ref{lem:nofirewall}) supplies quantitative bounds on horizon deviations within the stated regime.

\item \textbf{Computational:} The PT--MPO algorithm (Algorithm~\ref{alg:ptmpo}) provides a scalable simulation route for the finite-memory effective theory, with certified error bounds (Theorem~\ref{thm:ptmpo-complexity}).

\item \textbf{Experimental:} Our framework links process-tensor parameters $(\ellmem, \taumem)$ to observable signatures ($g^{(2)}$ sidebands, echo correlations), providing a concrete interface between theory and experiment.
\end{itemize}
...
UV robustness (Prop.~\ref{prop:robustness-uv}) & Conditional & Assumes causality bounds, QEIs, $\epsilon_{\rm UV}\ll 1$ \\
\bottomrule
\end{tabular}
\end{table}

By grounding the model in explicit axioms (A1--A4), together with a clearly stated adiabatic coarse-graining regime, we obtain key properties (P0--P4) that support a finite-memory Page-curve mechanism while keeping horizon deviations parametrically small within the effective theory. The framework offers a dynamical interpretation of information recovery, supported by scalable PT--MPO simulations and microscopic motivations based on gravitational edge modes and Schwarzian-inspired kernels. It also yields concrete signatures, including retarded sidebands and echo-like correlations, that sharpen the distinction between astrophysical null tests and analogue-platform discovery prospects. The main open issues remain the derivation of the scrambling input from first principles and the severe observational suppression of astrophysical signals by $O(1/S_{\rm BH})$ factors.

\subsection{Scope and limitations}
The \HMC{} description targets semiclassical, quasi-stationary evaporation under axioms A1--A4 together with the standing adiabatic coarse-graining regime: a Hadamard/QEI-controlled exterior, locality/coarse-graining with finite effective memory, fast (local) scrambling on a timescale $t_{\rm scr}$, and asymptotic purity. It does \emph{not} address highly non-adiabatic dynamics (violent accretion or mergers), near-extremal or Planckian curvatures (where QEIs may fail to give useful bounds), long-range spatial nonlocality or state-dependent operator assignments, or possible remnant scenarios. Identifying sharp breakdown thresholds and embedding \HMC{} in a complete UV theory is an open problem; nonetheless, the predicted signatures are falsifiable within the stated domain of validity.
...
This appendix elaborates a decoupling theorem tailored to combs and their multi-time structure. We first state the result in a form that controls the detectable-radiation sector $R_{\le n}$; combined with the bookkeeping relation $O_{\le n}=R_{\le n}E_{\le n}$ from the main text, this yields the entropy structure used in the Comb Page Theorem.
...
Effect of memory shrink. The shrink map on the memory sector can be realized by an isometric dilation followed by a partial trace. Applying the same CPTP post-processing to both the exact and reference dilated evolutions preserves contractive trace-distance bounds, so the early-time decoupling estimate remains stable under memory shrink up to the same logarithmic slack. A union bound across steps then yields a cumulative correction of $O(\log S_{\rm BH})$ in entropy units, as stated in \eqref{eq:comb-page}.

\paragraph{Unitary dilation of the memory shrink}
Let $d_{M_{k-1}}\ge d_{M_k}$ denote the code dimensions chosen by \eqref{eq:mem-dim}. For each step there exists an isometry $V_k:\mathcal{H}_{M_{k-1}}\to\mathcal{H}_{M_k}\otimes\mathcal{H}_{E_k}$ with
\[
\dim \mathcal{H}_{E_k}\;\ge\;\left\lceil \frac{d_{M_{k-1}}}{d_{M_k}} \right\rceil,
\qquad
\log \dim \mathcal{H}_{E_k}\;=\;S_{\rm BH}(u_{k-1})-S_{\rm BH}(u_k)+O(1),
\]
such that the effective channel $\Phi_k(\rho)=\Tr_{E_k}[V_k \rho V_k^\dagger]$ realizes the subspace-projection map employed in our exact simulations up to the rounding implicit in the effective code dimension. Writing the step map as $(\mathbbm{1}_{R_k}\otimes V_k)U_k$ shows that the global evolution is isometric and that our decoupling estimates apply verbatim to the dilated dynamics.
...
\[
\begin{aligned}
\mathcal{I}[q,q'] \;=\; \exp\!\Bigg\{&
 i\!\int dt\,dt'\!\sum_{x,y}\big[q_x(t)-q'_x(t)\big]K^{\rm ret}_{xy}(t-t')
 \frac{q_y(t')+q'_y(t')}{2} \\
 &\qquad
 -\;\frac{1}{2}\!\int dt\,dt'\,[q-q']\,\mathcal{N}\,[q-q']
\Bigg\},
\end{aligned}
\]
...
\end{enumerate}
Because analogue platforms can realize $S_{\rm BH}^{\rm eff}\sim 10^3$--$10^6$, the $O(1/S_{\rm BH}^{\rm eff})$ sidebands are experimentally accessible, providing a practical pathway for kernel tomography constrained by complete positivity (CP)/KMS/causality.
...
The time-domain kernel follows from the inverse transform
\[
K(t)=\int \frac{d\omega}{2\pi}\,e^{-i\omega t}\Xi^R(\omega),
\]
and has the causal form
\[
K(t)\propto \Theta(t)\,[a\,t^{-2}+b\,e^{-t/\tau_{\rm mem}}\cos(\Omega_\ast t)+\cdots],
\]
with $\tau_{\rm mem}\sim \beta\log S_{\rm BH}$ in the semiclassical window. The oscillatory part sets the comb period $\Omega_\ast^{-1}$, while the envelope determines the sideband decay rate.
...
\quad --s-initial 12 --steps 12 --num-runs 100 --g2-runs 200\\
\quad --kfolds 5 --seed 42 --threads 1 --pythonhashseed 0\\
\quad --save-ledger seed\_ledger.json\\
\\
...
\midrule
\texttt{data\_ablation.dat} & \hashcell{9fa7055e3a566dbd00ec0ab4efba56e7e3de8d64604ac304eb111ba16f7e2ebc} \\
\texttt{data\_ablation\_sig.dat} & \hashcell{2af783261bdbed64102dd3f0143cfbc842fc024a32ab9aeb47136012f1029d87} \\
\texttt{data\_cv\_summary.dat} & \hashcell{5cda6d08e4aab1a8bdc012f8c96c4c5d85eb428ce47071fe6b4e59f36af8637c} \\
\texttt{data\_exact\_comb.dat} & \hashcell{a71d0004e1621b6173a189706171a691f81e9ee997f8e56a20e418c785d9648f} \\
\texttt{data\_g2.dat} & \hashcell{5a97d052c182b1a9a583c527e291ce3cbd52261e00fe6a916684cf4028d5a19e} \\
\texttt{data\_ptmpo.dat} & \hashcell{70103ec26265f0dea58c900a9f7042a79a81fab45ce50c2c81b0de9b54ab9729} \\
\texttt{data\_ptmpo\_error.dat} & \hashcell{2d2f18556ca8b7454f7fb787d6000808a745e32bf89b83bce6e38c5ba63f278b} \\
\texttt{data\_ptmpo\_scaling.dat} & \hashcell{222574e45a4a4721f30f43e226c31ac9c0824871a5484a55c9055be6a1612537} \\
\texttt{data\_page\_curve.dat} & \hashcell{19f61e03c220ca4dba3e2e5ee3b1a5defc470ef2fc76bea945dfa6a7c4765007} \\
\texttt{data\_qec.dat} & \hashcell{e110c888caf200292e6b7ae2ee31e91a741fcfd03554c1137fa651bc29dada39} \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{quote}\small\ttfamily
python simulation.py --save-ledger seed\_ledger.json\\
\quad --threads 1 --pythonhashseed 0
\end{quote}
...
All figures and tables in this manuscript are generated from deterministic, self-contained simulations implemented in Python. Our approach to data availability follows best practices for scientific reproducibility:

\begin{enumerate}[leftmargin=*]
\item \textbf{Inline Data Embedding:} The exact ASCII tables used by PGFPlots are embedded directly into the \LaTeX{} source as \texttt{pgfplotstableread} blocks. This eliminates external file dependencies and ensures that the PDF can be compiled identically on any system with a standard \LaTeX{} distribution.

\item \textbf{Deterministic Seeding:} All random number generation uses fixed seeds (documented in Appendix F) with explicit control over Python's hash seed and thread counts. This guarantees byte-identical reproduction across runs.

\item \textbf{Cryptographic Verification:} SHA256 checksums are provided for all generated datasets, enabling independent verification of data integrity.

\item \textbf{Full Provenance:} The complete generation pipeline, including all hyperparameters, library versions, and system configuration, is recorded in the seed ledger.
\end{enumerate}

\subsection{Dataset Descriptions}
Each dataset serves a specific purpose in validating the \HMC{} framework:

\begin{itemize}[leftmargin=*]
\item \textbf{\texttt{data\_page\_curve}}: Contains the simulated Page curve entropy trajectory with mean, standard deviation, and 95\% confidence intervals over 100 Monte Carlo runs. Columns include time step, mean entropy, standard deviation, upper/lower CI bounds, ideal Page value, Hawking (thermal) reference, and black hole entropy.

\item \textbf{\texttt{data\_g2}}: Second-order correlation function $g^{(2)}(\tau)$ data showing the predicted sideband structure. Generated from 200 independent runs with temporal resolution $\Delta\tau = 0.1\tau_{\rm mem}$.

\item \textbf{\texttt{data\_ablation}}: Ablation study results varying memory depth $\ell_{\rm mem}$ and bond dimension $\chi$. Reports normalized RMSE to the ideal Page curve for each parameter combination.

\item \textbf{\texttt{data\_ablation\_sig}}: Statistical significance data for ablation comparisons, including $p$-values from Bonferroni-Holm corrected paired $t$-tests.

\item \textbf{\texttt{data\_exact\_comb}}: Exact small-comb calculations using Haar-random unitaries and explicit partial traces. Limited to $N \leq 8$ steps due to exponential cost, but provides ground-truth validation.

\item \textbf{\texttt{data\_ptmpo}}: PT-MPO simulation outputs for larger system sizes ($N$ up to 128 steps), demonstrating scalability of the tensor network approach.

\item \textbf{\texttt{data\_ptmpo\_error}}: Error analysis data showing convergence of PT-MPO entropy estimates as a function of bond dimension $\chi$.

\item \textbf{\texttt{data\_ptmpo\_scaling}}: Computational resource scaling (runtime, memory) as functions of $(\chi, r, L, T)$ parameters.

\item \textbf{\texttt{data\_cv\_summary}}: Cross-validation summary statistics from 5-fold CV analysis.

\item \textbf{\texttt{data\_qec}}: QEC fidelity data under temporally correlated noise as a function of correlation parameter $\rho$.
\end{itemize}
...
\footnotesize
\begin{tabularx}{\textwidth}{@{}T T r@{}}
\toprule
File & SHA256 & Size (KB) \\
\midrule
\texttt{data\_ablation.dat} & \hashcell{9fa7055e3a566dbd00ec0ab4efba56e7e3de8d64604ac304eb111ba16f7e2ebc} & 0.3 \\
\texttt{data\_ablation\_sig.dat} & \hashcell{2af783261bdbed64102dd3f0143cfbc842fc024a32ab9aeb47136012f1029d87} & 0.5 \\
\texttt{data\_cv\_summary.dat} & \hashcell{5cda6d08e4aab1a8bdc012f8c96c4c5d85eb428ce47071fe6b4e59f36af8637c} & 0.1 \\
\texttt{data\_exact\_comb.dat} & \hashcell{a71d0004e1621b6173a189706171a691f81e9ee997f8e56a20e418c785d9648f} & 0.3 \\
\texttt{data\_g2.dat} & \hashcell{5a97d052c182b1a9a583c527e291ce3cbd52261e00fe6a916684cf4028d5a19e} & 0.5 \\
\texttt{data\_ptmpo.dat} & \hashcell{70103ec26265f0dea58c900a9f7042a79a81fab45ce50c2c81b0de9b54ab9729} & 0.7 \\
\texttt{data\_ptmpo\_error.dat} & \hashcell{2d2f18556ca8b7454f7fb787d6000808a745e32bf89b83bce6e38c5ba63f278b} & 0.1 \\
\texttt{data\_ptmpo\_scaling.dat} & \hashcell{222574e45a4a4721f30f43e226c31ac9c0824871a5484a55c9055be6a1612537} & 0.1 \\
\texttt{data\_page\_curve.dat} & \hashcell{19f61e03c220ca4dba3e2e5ee3b1a5defc470ef2fc76bea945dfa6a7c4765007} & 0.6 \\
\texttt{data\_qec.dat} & \hashcell{e110c888caf200292e6b7ae2ee31e91a741fcfd03554c1137fa651bc29dada39} & 0.2 \\
\bottomrule
\end{tabularx}
\end{table}
...
\midrule
$d_{\rm mem}$ & Dimension of the horizon memory register; in the effective description $\log d_{\rm mem}=S_{\rm BH}+O(\log S_{\rm BH})$. \\
...
$\epsilon_{\rm SVD}$ & Per-step SVD truncation tolerance; local error threshold for bond truncation. \\
\midrule
...
\begin{itemize}[leftmargin=*]
\item Hilbert spaces are denoted by calligraphic letters ($\mathcal{H}$), operators by Roman capitals ($A, B, U$), and quantum states by Greek letters ($\rho, \sigma, \psi$).
\item We use natural units where $\hbar = c = k_B = 1$ unless explicitly restoring constants for phenomenological estimates.
\item Subscripts generally indicate system labels ($R$ for radiation, $M$ for memory, $I$ for interior), while superscripts may indicate time indices or functional dependence.
\item The diamond norm $\|\cdot\|_\diamond$ refers to the completely bounded trace norm, appropriate for comparing quantum channels.
\end{itemize}
...
Operator-algebra quantum error correction (OA-QEC) provides a powerful framework for understanding how quantum information can be protected and recovered in systems with non-trivial algebraic structure~\cite{Harlow:2016,Pastawski:2015}. In the context of black hole physics, OA-QEC offers a natural language for discussing how interior observables can be reconstructed from radiation degrees of freedom, connecting our \HMC{} framework to the broader literature on holographic error correction.

The key insight is that the \HMC{} defines an effective quantum channel from the initial infalling matter to the late-time radiation. When this channel satisfies the late-time decoupling conditions used in the Comb Page analysis, an approximate recovery map exists that can reconstruct logical information about the interior.
...
A subalgebra $\mathcal{A} \subset \mathcal{B}(\mathcal{H}_{\rm code})$ is \emph{$\varepsilon$-correctable} on $L$ if there exists a CPTP recovery map $\mathcal{R}_{L\to \rm code}$ such that for all states $\rho$ supported on $\mathcal{H}_{\rm code}$,
\[
\|(\mathcal{R} \circ \Phi)(\rho) - \rho\|_1 \leq \varepsilon.
\]
\end{definition}

Writing $\Phi^{c}$ for a complementary channel (with output going to the environment/interior), a standard approximate-correctability criterion is that there exists a recovery $\mathcal{R}_{L\to \mathrm{code}}$ such that
\begin{equation}
\bigl\| \Phi^{c} - \Phi^{c}\circ \mathcal{R}\circ \Phi \bigr\|_{\diamond} \le \varepsilon\,,
\end{equation}
which is equivalent to approximate recoverability on the code subspace up to standard OA-QEC reformulations.

\subsection{Constructive Recovery via Twirled Petz Map}
We give a concrete approximate recovery map $\mathcal{R}$ onto the radiation algebra using a twirled Petz construction with memory-aware modular operators. The Petz recovery map~\cite{Petz:1986} is the canonical choice for approximate quantum error correction, and its twirled version provides universal guarantees independent of the encoded state.

If $\mathcal{N}$ denotes the comb channel per window and $(\sigma_R,\sigma_L)$ are suitable reference marginals, then the rotated Petz map is
\begin{align}
\mathcal{R}^{(\theta)}_{L\to R}(\cdot)
=
\sigma_R^{\frac{1+i\theta}{2}}\,
\mathcal{N}_{R\to L}^{\dagger}\!\Bigl(
\sigma_L^{-\frac{1+i\theta}{2}}
(\cdot)\,
\sigma_L^{-\frac{1-i\theta}{2}}
\Bigr)\,
\sigma_R^{\frac{1-i\theta}{2}}\,.
\end{align}
Twirling over $\theta$ yields a state-independent recovery channel. In the \HMC{} regime this construction achieves recovery error $\varepsilon=O(e^{-\alpha S_{\rm BH}/2})$, with $\alpha$ set by the scrambling gap and finite memory rank.

\subsection{Main Recovery Theorem}
\begin{theorem}[Recovery for \HMC{}]\label{thm:hmc_recovery}
In the \HMC{} framework, $\varepsilon$ can be chosen $O(e^{-\alpha S_{\rm BH}/2})$ once $|L|$ exceeds the scrambling-modified Page time $n_* = \frac{1}{2}S_{\rm BH} + O(\log S_{\rm BH})$. The recovery map is constructively given by the rotated Petz map built from the late-radiation steady state (Algorithm~\ref{alg:rotated_petz_hmc}).
\end{theorem}
...
\]
By the universal recovery theorem of~\citet{FawziRenner:2015}, whenever the conditional mutual information is small, there exists a CPTP recovery map $\mathcal{R}_{L\to RL}$ with fidelity loss bounded by $O(\delta)$. Specifically:
\[
F(\varrho_{REL}, (\mathrm{id}_E \otimes \mathcal{R})(\varrho_{EL})) \geq 2^{-\delta/2}.
\]
Translating to the OA-QEC setting via standard fidelity-to-trace-distance and channel-continuity bounds gives the stated scaling $\varepsilon=O(\sqrt{\delta})$.
\end{proof}

\subsection{Physical Interpretation}
The recovery theorem has a clear physical interpretation: after the Page time, the Hawking radiation contains sufficient quantum information to reconstruct interior observables, but this reconstruction requires a complex decoding operation (the twirled Petz map) that acts on the full radiation Hilbert space. The exponentially small error $\varepsilon = O(e^{-\alpha S_{\rm BH}/2})$ reflects the fundamental tension between unitarity and the semiclassical approximation---perfect recovery would require access to Planck-scale degrees of freedom.
...
\begin{equation}
\big| S(R_{\le n}) - S_{\rm Page}(n) \big| \ \le\ C\left(\varepsilon_2 + e^{-\ell_{\rm scr}/\xi} + \varepsilon_{\rm spec}\right) + (c_0+c_1\log S_{\rm BH}).
\end{equation}
Here the logarithmic term collects the same continuity and finite-code-size contributions as in \Cref{lem:comb-error-budget}.
...
\end{equation}
where the second term accounts for the finite mixing time of the memory-lightcone graph. For $N \sim S_{\rm BH}$ (full evaporation) and $\varepsilon_2 \sim 1/S_{\rm BH}$, the design/mixing contribution remains $O(1)$ in entropy units; after reinstating the continuity and finite-code-size terms, this is consistent with the overall \(O(\log S_{\rm BH})\) slack in Theorem~\ref{thm:comb-page-weak}.
