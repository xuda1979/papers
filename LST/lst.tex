...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\linewidth]{code/scaling_comparison.png}
\caption{Measured comparison of training time vs. sequence length. The Transformer (green) exhibits quadratic scaling, whereas LST (blue) and Mamba (orange) demonstrate near-linear scaling, confirming the efficiency of the hybrid architecture.}
\label{fig:scaling}
\end{figure}

\section{Experimental Evaluation}

We implemented the LST architecture in PyTorch, along with a custom simplified selective SSM kernel for portability. We compare it against a standard Transformer Encoder and a pure Mamba baseline on the Associative Recall synthetic task.

### 5.1 Associative Recall

The Associative Recall task tests a model's ability to retrieve specific values associated with keys appearing earlier in the sequence. Figure~\ref{fig:associative_recall} presents the accuracy of the models as the sequence length increases.

\begin{figure}[h]
\centering
\includegraphics[width=0.9\linewidth]{code/associative_recall_accuracy.png}
\caption{Associative Recall Accuracy vs. Sequence Length. LST matches the Transformer's high accuracy (near 100\%) even at long sequence lengths, while the pure SSM baseline (Mamba) shows degradation as context length increases, illustrating the benefit of the attention mechanism for retrieval tasks.}
\label{fig:associative_recall}
\end{figure}

The results indicate that LST successfully combines the strengths of both architectures: it retains the retrieval accuracy of attention-based models while enjoying the computational efficiency of SSMs.

### 5.2 Trade‑off between $K$ and Performance

We investigated the sensitivity of LST to the latent dimension $K$. Figure~\ref{fig:k_tradeoff} shows the accuracy on the Associative Recall task for varying $K$.

\begin{figure}[h]
\centering
\includegraphics[width=0.9\linewidth]{code/k_ablation.png}
\caption{Effect of Latent Dimension $K$ on Accuracy. Performance improves with $K$, saturating around $K=64$. This confirms that a small latent bottleneck is sufficient to capture the necessary global context for this task.}
