...
# -------- Policies --------
def sc_fixed_generate(
    model, tokenizer, prompt, n: int, temperature: float, max_new_tokens: int, stop=None
):
        ...
        with meter.session() as m:
            for ex in tqdm(data, ncols=100):
                ...
                for k in range(1, pconf["n"] + 1):
                    ...
                    if pname == "adaptive_margin" and adaptive_margin_halt(
                        samples, pconf["margin_th"]
                    ):
                        break
