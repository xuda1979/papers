...
\begin{table}[h!]
\centering
\caption{Compute budget allocation and performance on Simulated Math Reasoning (SMR). Budgets are in normalized simulation units. Quality is measured as final accuracy. Our methods (IGD, RS-MCTT, ADR, CSC) achieve competitive quality at varying compute budgets.}
...
\midrule
Greedy CoT (Proxy: IGD@10) & 10 & 23.5 & - & 2.35 \\
Self-Consistency (Proxy: CSC@10) & 10 & 46.0 & - & 4.60 \\
\midrule
IGD (ours) & 40 & 46.4 & 62.6 & 1.16 \\
UCB-IGD (ours) & 40 & 49.3 & 64.3 & 1.23 \\
RS-MCTT (ours) & 40 & 28.9 & 31.9 & 0.72 \\
CSC (ours) & 40 & 72.0 & 78.1 & 1.80 \\
ADR (ours) & 40 & \textbf{95.5} & \textbf{104.1} & \textbf{2.39} \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\caption{Budget--Performance Frontiers for Simulated Math Reasoning (SMR) and Simulated Code Generation (SCG). The plot compares Index-Guided Deliberation (IGD), UCB-IGD, Risk-Sensitive MCTT (RS-MCTT), Counterfactual Self-Consistency (CSC), and Abduction-Deduction-Refutation (ADR) across varying compute budgets. Note: SMR baselines are represented by the start of the curves (lower budgets).}
