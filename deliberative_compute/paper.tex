...
\begin{figure}[t]
    \centering
    \input{tex/fig/schematic_framework.tex}
    \caption{A schematic view of the Budgeted Deliberation Framework. The agent maintains a deliberation state $s_t$, consults a policy $\pi$ to estimate the Expected Value of Computation (EVC) or dynamic indices for possible cognitive actions, and interacts with the LLM or environment. The loop continues until the estimated value of further computation falls below the cost threshold $\lambda$.}
    \label{fig:framework}
\end{figure}
...
Map to monotone submodular maximization under a cardinality constraint and apply the Nemhauser bound \cite{nemhauser1978}. The per-step EVC ordering is equivalent to the marginal gains ordering. See \Cref{app:proofs} for the full proof.
\end{proof}
...
To validate our proposed algorithms, we conduct a series of experiments in controlled synthetic environments that capture the core challenges of deliberative problem-solving: search, verification, and hypothesis testing. We instantiate several of our algorithms---specifically IGD, RS-MCTT, and ADR---and compare them against standard baselines.

\subsection{Setup}

\paragraph{Tasks.} We evaluate on two synthetic task environments designed to mimic the structure of real-world reasoning challenges while allowing for rapid, deterministic evaluation:
\begin{itemize}[leftmargin=1.4em, itemsep=0.25em]
    \item \textbf{Arithmetic Search (Game of 24):} A canonical arithmetic reasoning task where the agent must combine four numbers using basic operations $(+, -, *, /)$ to reach the target value of 24. This task represents a dense search space with clear goal criteria but multiple potential paths, mimicking mathematical derivation.
    \item \textbf{Combinatorial Search (Bitstring/Code Proxy):} A "Mastermind"-style task where the agent attempts to discover a hidden target bitstring (representing a correct program) by proposing candidates and receiving partial feedback (fraction of correct bits, representing unit test pass rates). This task captures the dynamics of code generation and iterative debugging.
\end{itemize}

\paragraph{Baselines.} We compare against: (i) Greedy CoT (approximated by single-path depth-first search); (ii) Self-Consistency (best-of-$N$ parallel sampling); and (iii) a vanilla Tree-of-Thought (ToT) proxy. All methods are rigorously compute-matched using operation counts.
...
\begin{table}[h!]
\centering
\caption{Compute budget allocation and performance on Arithmetic Search (Game of 24). Budgets are in normalized simulation units. Quality is measured as solution success rate. Our methods (IGD, RS-MCTT, ADR) achieve competitive quality at varying compute budgets.}
\label{tab:compute-budget}
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Method} & \textbf{Budget} & \textbf{Quality (\%)} & \textbf{Frontier Area (FA)} & \textbf{Efficiency (Qual/Cost)} \\
\midrule
Greedy CoT (Proxy) & 10 & 20.0 & - & 2.00 \\
Self-Consistency (Proxy) & 10 & 0.0 & - & 0.00 \\
\midrule
IGD (ours) & 20 & 100.0 & 10.0 & 5.00 \\
UCB-IGD (ours) & 20 & 73.2 & 7.3 & 3.66 \\
RS-MCTT (ours) & 20 & 91.8 & 9.2 & 4.59 \\
ADR (ours) & 20 & 41.1 & 4.1 & 2.05 \\
\bottomrule
\end{tabular}%
}
\end{table}

\Cref{fig:bpf-curves} shows the BPF curves for our methods versus the baselines on both tasks. Our algorithms consistently trace a more favorable frontier, achieving higher quality at every compute budget. IGD shows particularly strong anytime performance on the arithmetic task, quickly finding valid solution paths. RS-MCTT demonstrates robustness, efficiently exploring the search space to find high-value solutions. The poor performance of naive Self-Consistency in these dense search tasks highlights the necessity of structured deliberation.

\begin{figure}[h!]
\centering
\input{tex/fig/bpf_curves_generated.tex}
\caption{Budget--Performance Frontiers for Arithmetic Search (Math) and Combinatorial Search (Code). The plot compares Index-Guided Deliberation (IGD), UCB-IGD, Risk-Sensitive MCTT (RS-MCTT), and Abduction-Deduction-Refutation (ADR) across varying compute budgets.}
...
The development of algorithms for allocating test-time compute has significant implications for the efficiency and capabilities of AI systems. By formalizing "deliberation" as a budgeted resource, we enable systems that can adaptively scale their cognitive effort to the difficulty of the task, potentially reducing energy consumption for simple queries while unlocking deeper reasoning for complex ones. However, this also introduces risks: systems might optimize for metrics that do not align with human values if the utility function $U(y;x)$ is not carefully designed. Furthermore, the ability to "think harder" could make systems more persuasive or capable of executing complex, multi-step plans that are harder to audit. We emphasize the need for transparency in how these budgets are allocated and for the integration of safety constraints directly into the deliberation policy.
