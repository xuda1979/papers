# Ascend 910B ai2 microbenchmark summary

Five-trial rerun collected on 2026-03-25 with PyTorch + torch_npu on Huanxin ai2 using `npu:0` and `float16`.
...
- The strongest verified crossover remains at `d=4096`, where the factorized path wins by about `2.26x` to `2.56x`.
- `d=1024` loses across all tested batch sizes.
- `d=2048` does not show a robust win region in the repeated-run data. The `bt=8` bucket is only a near-tie on the mean (`1.005x`) and should be treated as parity, not a strong win.
- On a strict mean-latency rule, best-ephemeral wins in `4/9` buckets, but only `3/9` are clearly above parity.
- The roofline model still identifies the correct large-shape regime, but it materially overpredicts the breadth and magnitude of the measured win region.
