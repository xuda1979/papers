\section{The \texorpdfstring{$\theta^+$}{θ⁺}-Flow Method}
\label{sec:theta-flow}

In this section, we develop a geometric flow approach to the spacetime Penrose inequality. The $\theta^+$-flow provides a Hamilton-style program analogous to Ricci flow for the Poincaré conjecture: a natural geometric flow that evolves trapped surfaces to marginally outer trapped surfaces (MOTS). \textbf{Important:} The flow converges to a MOTS in all cases, but the final step (applying Penrose inequality to the MOTS) requires additional conditions for unfavorable jump $\tr_\Sigma k < 0$ (see \S\ref{subsec:AreaComparison}).
...
\begin{equation}
\boxed{\frac{\partial S}{\partial t} = -\theta^+(S) \cdot \nu}
...
\begin{equation}
\boxed{\frac{d}{dt}\mathrm{Area}(S_t) = -\int_{S_t} H \cdot \theta^+ \, dA}
...
\end{itemize}
Equality holds if and only if $\theta^+ \equiv 0$ (i.e., $S_t$ is a MOTS).
\end{theorem}
...
The structure of this proof has similarities to Hamilton's resolution of the Poincaré conjecture:
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
