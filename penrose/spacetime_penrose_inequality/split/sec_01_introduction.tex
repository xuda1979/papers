...
\textbf{Step 4: What this (does and does not) prove.}
The identities above show that along any \emph{outward, future-directed null deformation} for which the first variation is governed by $\theta^+$, the area is \emph{non-increasing} whenever $\theta^+\le 0$:
\begin{equation}
    \frac{dA}{ds} = \int_{\Sigma_s} \theta^+\, dA_s \le 0.
\end{equation}
Consequently, a naive attempt to compare a trapped surface $\Sigma_0$ to the outermost MOTS $\Sigma^*$ by flowing outward yields at best the \emph{opposite} inequality
\begin{equation}
    A(\Sigma^*) \le A(\Sigma_0),
\end{equation}
which is exactly the wrong direction for reducing the Penrose inequality from $\Sigma^*$ to $\Sigma_0$. Moreover, since every (future) trapped surface satisfies $H=\tfrac12(\theta^+ + \theta^-)<0$ (Remark~\ref{rem:UniversalMeanCurvature}), any \emph{spacelike} outward foliation has first variation
\begin{equation}
    \frac{dA}{dt} = \int_{\Sigma_t} H_t\,\phi_t\, dA_t < 0 \qquad (\phi_t>0),
\end{equation}
so area decreases along outward spacelike deformations as well. This explains why purely local foliation arguments cannot, in general, establish $A(\Sigma^*)\ge A(\Sigma_0)$.

\textbf{Conclusion of Approach A:} The preceding computation clarifies the sign obstruction; it does \emph{not} produce the desired area comparison.

\textbf{Approach B:} Under the compactness assumptions (C1)--(C3), the conditional area comparison is obtained by constrained variational methods (Theorem~\ref{thm:MaxAreaTrapped}), not by a monotonicity/foliation argument.
\end{proof}
...
We present Penrose's original 1973 argument as a \emph{conditional} spacetime argument. The missing input is an \textbf{area comparison} between a trapped surface on a Cauchy slice and an event-horizon cross-section; a naive attempt to obtain this by using past-directed outgoing null geodesics from the trapped surface fails (wrong sign).
...
One might hope to justify (OM) by comparing $\Sigma$ to $\mathcal{H}_\mathcal{C}$ using \textbf{ingoing null hypersurfaces from the event horizon} together with \textbf{null focusing under NEC}. This is a natural geometric idea, but turning it into a complete proof requires global control of caustics/conjugate points and a rigorous ``sweeping'' or covering argument.
...
\begin{itemize}
    ...
    \item \textbf{Past-directed outgoing rays from $\Sigma$}: Area \emph{decreases} toward the past (wrong sign for comparison).
\end{itemize}
...
\end{enumerate}
	...
	extbf{Status:} This is currently a \emph{heuristic outline}. In this paper we therefore keep (OM) as an explicit hypothesis in Theorem~\ref{thm:HAD}.
\end{remark}
