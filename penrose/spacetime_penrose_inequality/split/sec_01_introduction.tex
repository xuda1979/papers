...
The Riemannian case ($k = 0$) was resolved by Huisken--Ilmanen \cite{huisken2001} and Bray \cite{bray2001} around 2001. The general spacetime case has remained open for over 50 years. This work proves the sharp inequality in the direct favorable-jump MOTS regime and isolates a new adjoint-cone variational structure for the remaining non-time-symmetric sign problem.
...
\begin{itemize}
    ...
    \item \textbf{Theorem B (General Trapped Surfaces):} Penrose inequality for \emph{general trapped surfaces} --- requires either a Lorentzian area-comparison input motivated by cosmic censorship, the time-symmetric case $k=0$, or a reduction to a MOTS satisfying the favorable jump condition.
    \item \textbf{Theorem C (Adjoint-Cone Euler Condition):} The constrained max-area variational argument yields a sharp adjoint-cone sign condition for $\tr_\Sigma k$. This is the correct replacement for the previously attempted integral-to-pointwise upgrade.
\end{itemize}
\end{framed}
...
\hline
Theorem C (Adjoint cone) & Thm.~\ref{thm:AdjointConeEuler} & App.~\ref{subsec:GapClosureMath} \\
\hline
\end{tabular}
\end{center}
...
\begin{remark}[Why Stability Appears]\label{rem:StabilityEssential}
For outermost MOTS, the stability hypothesis $\lambda_1(L_{\Sigma^*}) \ge 0$ is automatic in the standard existence theory. We keep it explicit because stability is one of the geometric properties used throughout the analytic setup. Moreover:
\begin{enumerate}
    ...
    \item Even in the marginally stable case ($\lambda_1 = 0$), stability alone still does not determine the sign of the jump term; the separate favorable-jump hypothesis remains the decisive interface condition.
\end{enumerate}
...
\begin{enumerate}
    \item[(i)] \textbf{Lorentzian area-comparison input:} The data embeds in a globally hyperbolic spacetime satisfying weak cosmic censorship together with a comparison principle $A(\Sigma)\le A(\mathcal H_M)$ between the trapped surface and an event-horizon cross-section;
    \item[(ii)] \textbf{Time-symmetric:} $k = 0$ (the Riemannian Penrose inequality, previously established by Huisken--Ilmanen and Bray);
    \item[(iii)] \textbf{Direct MOTS regime:} $\Sigma$ is in fact a MOTS and satisfies $\tr_{\Sigma} k \ge 0$ \emph{pointwise};
\end{enumerate}
...
\begin{remark}[Integral vs.\ Pointwise Condition]\label{rem:intro-gap}\label{rem:NonSelfAdjointGap}
For general trapped surfaces with $k \neq 0$ and without cosmic censorship, the variational method gives a sharp but weaker condition than pointwise favorable jump:
\begin{itemize}
    \item The maximum-area argument establishes an adjoint-cone condition, equivalently a sign statement for the adjoint inverse applied to $\tr_\Sigma k$ (Theorem~\ref{thm:AdjointConeEuler}).
    \item The Jang equation method requires the \emph{pointwise} condition $\tr_\Sigma k \geq 0$ to ensure $[H]_{\bar{g}} \ge 0$.
\end{itemize}
Theorem~\ref{thm:NoGoVariational} explains why the adjoint-cone condition cannot be replaced by pointwise positivity using only the naive scalar variational argument.
\end{remark}

\begin{theorem}[Theorem C: Adjoint-Cone Euler Condition]\label{thm:IntroAdjointConeEuler}
Let $\Sigma$ be a strictly stable MOTS in initial data $(M,g,k)$ and suppose $\Sigma$ is a constrained local area maximum among surfaces satisfying $\theta^+\le0$. Let $L_\Sigma$ be the MOTS stability operator. Then $\tr_\Sigma k$ lies in the polar cone of the Green cone generated by $L_\Sigma$: for every smooth $\eta\ge0$,
\[
    \int_\Sigma (\tr_\Sigma k)\, L_\Sigma^{-1}\eta\,dA \le 0
\]
with the sign convention fixed by the outward normal used in the first-variation formula. Equivalently, $L_\Sigma^{-*}(\tr_\Sigma k)$ has a definite distributional sign.

\textbf{Status:} The full proof is given in Theorem~\ref{thm:AdjointConeEuler}. This theorem is unconditional within the stated strict-stability and constrained-maximality hypotheses. It is the exact variational output; a further pointwise favorable-jump conclusion would require an additional calibration identity not proved here.
\end{theorem}

\begin{remark}[Why Theorem~C Is the Sharp Variational Output]\label{rem:ConjectureCFundamental}
The adjoint-cone formulation is not merely a technical artifact but reflects deep mathematical structure:
...
\begin{enumerate}
    ...
    \item \textbf{Comparison with $k = 0$ Case:} When $k = 0$ (time-symmetric data), the favorable jump is automatic because $\tr_\Sigma k\equiv0$. No non-self-adjoint sign upgrade is needed.
...
\textbf{Bottom Line:} Theorem~C gives a new exact variational sign law. Turning it into pointwise $\tr_\Sigma k\ge0$ requires a separate geometric calibration or a different reduction mechanism.
\end{remark}
...
Conformal methods cannot simultaneously achieve area preservation and mass reduction when $\tr_\Sigma k < 0$. This explains why the outermost stable MOTS case under the explicit favorable jump hypothesis $[H]_{\bar{g}} \ge 0$ succeeds, while general trapped surfaces with unfavorable $\tr_\Sigma k$ require additional hypotheses.
\end{theorem}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Maximum Area Trapped Surface Framework (Theorem B):} We introduce a variational approach to the problem, identifying the "Maximum Area Trapped Surface" as a natural candidate for the inequality. Under the compactness hypotheses used in the variational program, this yields the adjoint-cone Euler condition of Theorem~\ref{thm:AdjointConeEuler}; obtaining the pointwise favorable jump from that condition would require an additional calibration input.
        \item \textbf{Lorentzian Area-Comparison Route:} We isolate a spacetime area-comparison input motivated by weak cosmic censorship and show how it would imply the Penrose inequality through the Hawking area theorem and the Kerr bound (Program~F).
