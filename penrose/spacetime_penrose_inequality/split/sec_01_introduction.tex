...
\subsection{Summary of Main Results: The Two-Track Approach}

We structure our results into two complementary tracks:

\begin{center}
\fbox{\fbox{\parbox{0.95\textwidth}{
\textbf{\Large TRACK A: UNCONDITIONAL FLAGSHIP RESULT}\\[0.5em]

\textbf{Theorem A: Sharp Penrose Inequality for Apparent Horizons (Theorem~\ref{thm:penroseinitial})}\\
For the \textbf{outermost MOTS} (apparent horizon) $\Sigma^*$ in initial data $(M^3, g, k)$ satisfying DEC:
\begin{equation*}
    \boxed{M_{\mathrm{ADM}} \geq \sqrt{\frac{A(\Sigma^*)}{16\pi}}} \quad \text{\textbf{(Sharp constant $C = 1$)}}
\end{equation*}
\textit{Status:} \textcolor{green!60!black}{\textbf{UNCONDITIONAL and RIGOROUS}}. Requires only DEC + asymptotic flatness.\\[0.3em]
\textit{Method:} p-Harmonic level sets with $p \to 1^+$ + Generalized Jang equation.\\[0.3em]
\textit{Key technical advance:} The sign $[H] \ge 0$ is established via the \textbf{Principal Eigenvalue Theorem for non-self-adjoint operators} (Andersson--Mars--Simon, 2008), rigorously handling the drift term $2X \cdot \nabla$ in the stability operator.\\[0.5em]

\textbf{Comparison with Allen--Bryden--Kazaras--Khuri (2025) \cite{allenbrydentkazaraskhuri2025}:}\\
ABKK achieved a major breakthrough: a Penrose-type inequality \textbf{without symmetry assumptions}. Our work builds on their insights with a different technical approach:
\begin{center}
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Method} & \textbf{Constant} & \textbf{Surface} & \textbf{Why?} \\
\hline
ABKK ($p=2$) & $C < 1$ & General trapped & Capacity-area ratio \\
This paper ($p \to 1^+$) & $C = 1$ & Outermost MOTS & IMCF geometry \\
\hline
\end{tabular}
\end{center}
\textit{Note:} These results are complementary---ABKK applies to more general surfaces; we achieve the sharp constant for apparent horizons.
}}}
\end{center}
...
\begin{center}
\fbox{\fbox{\parbox{0.95\textwidth}{
\textbf{\Large TRACK B: CONDITIONAL EXTENSION}\\[0.5em]

\textbf{Theorem B: Penrose Inequality for General Trapped Surfaces (Theorem~\ref{thm:p-harmonic-penrose})}\\
For trapped surfaces $\Sigma$ (not necessarily outermost) in initial data $(M^3, g, k)$ satisfying DEC:
\begin{equation*}
    \boxed{M_{\mathrm{ADM}} \geq \sqrt{\frac{A(\Sigma)}{16\pi}}}
\end{equation*}
\textit{Status:} \textcolor{orange}{\textbf{CONDITIONAL}} on one of:
\begin{itemize}
    \item Weak Cosmic Censorship (WCC), or
    \item Compactness assumptions (C1)--(C3), or
    \item Time-symmetric data ($k=0$).
\end{itemize}

\textbf{Why conditional?} Without WCC, the inequality for inner trapped surfaces is \textcolor{red}{\textbf{FALSE in general}}. In binary black hole mergers, inner MOTS can have larger area than the apparent horizon. This is not a weakness of our proof---it reflects the physics: Cosmic Censorship is \emph{necessary} for general trapped surfaces.\\[0.5em]

\textbf{Fundamental Obstruction (Theorem~\ref{thm:Obstruction}):}\\
Conformal methods cannot simultaneously achieve area preservation and mass reduction for the unfavorable case $\tr_\Sigma k < 0$. This explains why Track A (outermost MOTS, favorable jump by stability) succeeds unconditionally, while Track B requires additional hypotheses.
}}}
\end{center}

\begin{tcolorbox}[colback=red!5!white, colframe=red!75!black, title=\textbf{CRITICAL GAPS AND HONEST ASSESSMENT}]\label{box:criticalstatus}
\textbf{What is FULLY PROVED (no additional hypotheses beyond DEC + AF):}
\begin{itemize}
    \item \textbf{Outermost MOTS (Theorem~\ref{thm:penroseinitial}):} The sharp inequality $M_{\ADM} \geq \sqrt{A(\Sigma^*)/(16\pi)}$ for the apparent horizon $\Sigma^*$. \textcolor{green!60!black}{\textbf{UNCONDITIONAL}}. This is the \textbf{main result} of this paper.
\end{itemize}

\textbf{What is PROVED under additional hypotheses:}
\begin{itemize}
    \item \textbf{Time-symmetric case ($k=0$):} For any trapped surface in data with $k=0$, under compactness (C1)--(C3). \textcolor{green!60!black}{\textbf{RIGOROUS}}.
    \item \textbf{Penrose's original conjecture (WCC assumed):} Under weak cosmic censorship, for any trapped surface. \textcolor{green!60!black}{\textbf{PROVED}} (Theorem~\ref{thm:Penrose1973Complete}).
\end{itemize}

\textbf{What REMAINS OPEN:}
\begin{itemize}
    \item \textbf{KEY OPEN PROBLEM---Integral-to-pointwise upgrade for $k \neq 0$:} The variational method (Theorem~\ref{thm:MaxAreaTrapped}) proves only $\int_\Sigma \tr_\Sigma k \, dA \geq 0$, but the Jang equation requires \emph{pointwise} $\tr_\Sigma k \geq 0$. This upgrade is \textcolor{red}{\textbf{OPEN}} because the MOTS stability operator $L_\Sigma = -\Delta - |A|^2 - \Ric(\nu,\nu) + 2X \cdot \nabla$ is \textbf{non-self-adjoint} when $k \neq 0$ due to the drift term $2X \cdot \nabla$. Standard spectral arguments fail. See Remark~\ref{rem:NonSelfAdjointGap} for details.
    \item \textbf{General trapped surfaces without WCC:} Known to \textcolor{red}{\textbf{FAIL}} in general (binary merger counterexamples show inner MOTS can have larger area than the apparent horizon).
    \item \textbf{Compactness conditions (C2), (C3):} Finiteness of $A_{\max}$ not established without curvature bounds.
\end{itemize}

\textbf{Summary:} The \textbf{outermost MOTS case is unconditional}. The \textbf{general trapped surface case with $k \neq 0$} has an open gap that we explicitly identify. We do \textbf{not} claim to have fully resolved the general spacetime Penrose inequality.
\end{tcolorbox}
...
\textbf{Current Status:} Penrose's original argument had gaps (the outer-minimizing assumption (OM) and sign errors in the past-directed approach). The p-harmonic level set method (Theorem~\ref{thm:p-harmonic-penrose}):
\begin{itemize}
    \item Does \textbf{not} require cosmic censorship;
    \item Does \textbf{not} require the outer-minimizing property;
    \item Handles favorable jump ($\tr_\Sigma k \geq 0$) \textbf{unconditionally};
    \item Handles unfavorable jump ($\tr_\Sigma k < 0$) \textbf{conditionally} on compactness (C1)--(C3).
\end{itemize}

Our contributions are:
\begin{itemize}
    \item \textbf{Theorem A (MOTS Penrose):} \textcolor{green!60!black}{\textbf{PROVED UNCONDITIONALLY}} for outermost MOTS---this is the main physical case (apparent horizons).
    \item \textbf{Theorem B (Time-Symmetric):} \textcolor{green!60!black}{\textbf{RIGOROUS}} for time-symmetric data ($k = 0$) with any trapped surface.
    \item \textbf{Theorem C (Curvature Bounds):} \textcolor{orange}{\textbf{CONDITIONAL}} on curvature bounds (C1) for general trapped surfaces in arbitrary initial data.
    \item \textbf{Open Case:} \textcolor{red}{\textbf{GAP}} for general trapped surfaces with $k \neq 0$ without curvature bounds---the integral-to-pointwise upgrade remains unproved.
\end{itemize}

\subsection{p-Harmonic Level Set Method and the Generalized Jang Equation}\label{subsec:p-harmonic-jang}

We briefly describe the analytic strategy that underlies our proof of the Spacetime Penrose Inequality. For the \textbf{outermost MOTS} (apparent horizon), this approach yields an \emph{unconditional} result; for general trapped surfaces, additional conditions are required. The two central ideas are:
...
\hline
\textbf{1} & Jang Reduction: $(M,g,k) \to (\bar{M}, \bar{g})$ with $R_{\bar{g}} \geq 0$ & Weighted Schauder estimates \\
\textbf{2} & p-Harmonic Potential: $\Delta_p u = 0$ on $\bar{M}$ & De Giorgi-Nash-Moser \\
\textbf{3} & Monotonicity: $\frac{d}{dt} m_H^{(p)}(\Sigma_t) \geq 0$ & Weighted Bochner + Co-area \\
\textbf{4} & Boundary Limits: $t \to 0^+$ and $t \to 1^-$ & Asymptotic analysis \\
\textbf{5} & Limit $p \to 1^+$ (optional): recovers IMCF & Varifold convergence \\
\hline
\end{tabular}
\end{center}

\textbf{Key insight:} Stage 1 (Jang) is the \textbf{geometric strategy}---it reduces the spacetime problem to a Riemannian one. Stages 2--4 are \textbf{harmonic analysis tactics}---they provide the elliptic estimates that drive the proof. Stage 5 is optional but establishes sharpness.

The \textbf{p-Geroch identity} (equation~\eqref{eq:p-Geroch}) is the bridge formula connecting geometry and analysis:
\begin{equation*}
...
\textbf{Our approach:} We \textbf{bypass} the direct area comparison entirely. Instead of proving $A(\Sigma^*) \ge A(\Sigma_0)$ (which can fail), we use the Maximum Area Trapped Surface construction (Theorem~\ref{thm:MaxAreaTrapped}): we identify an intermediate surface $\Sigma_{\max}$ that \emph{by construction} satisfies $A(\Sigma_{\max}) \ge A(\Sigma_0)$ and has the favorable jump condition needed for the Jang method.
...
\begin{enumerate}
    ...
    \item \textbf{General Case}: Requires additional conjectures C1--C3 to be established (see Section~\ref{sec:FinalConclusion}).
\end{enumerate}

We now present the rigorous variational proof.
\end{proof}

\begin{tcolorbox}[colback=red!5!white, colframe=red!75!black, title=\textbf{FUNDAMENTAL OBSTRUCTION: Why Area Monotonicity Fails for $k \neq 0$}]
\textbf{The Problem:} To extend the Penrose inequality from outermost MOTS $\Sigma^*$ to arbitrary trapped surfaces $\Sigma_0$, one might try to prove $A(\Sigma^*) \geq A(\Sigma_0)$.

\textbf{Why it fails:} In the trapped region, $H < 0$ (mean curvature points inward). Area evolution along any spacelike outward foliation gives:
\begin{equation}
    \frac{dA}{dt} = \int_{\Sigma_t} H_t \, \phi_t \, dA_t < 0
\end{equation}
Area \textbf{decreases} as you move outward! This gives $A(\Sigma^*) < A(\Sigma_0)$---the wrong direction.

\textbf{Physical Counterexamples:} In binary black hole mergers, inner trapped surfaces can have \textbf{larger} area than the outer apparent horizon. Area monotonicity is \textbf{genuinely false} for $k \neq 0$.

\textbf{The Standard Resolution (Schoen-Yau, Bray-Khuri):}
\begin{enumerate}
    \item \textbf{Do NOT try to prove $A(\Sigma^*) \geq A(\Sigma_0)$} for arbitrary $\Sigma_0$
    \item \textbf{Use the Jang Equation} to map $(M, g, k) \to (\tilde{M}, \tilde{g})$ where $R_{\tilde{g}} \geq 0$
    \item The Jang solution blows up at the \textbf{outermost MOTS} $\Sigma^*$, creating cylindrical ends
    \item Apply Riemannian Penrose (IMCF) on $\tilde{M}$ to get $M_{\mathrm{ADM}} \geq \sqrt{A(\Sigma^*)/(16\pi)}$
    \item \textbf{Accept} that the theorem is for outermost MOTS only---this is the correct formulation
\end{enumerate}

\textbf{Conclusion:} The Penrose Inequality for $k \neq 0$ is a statement about the \textbf{outermost MOTS}, not arbitrary trapped surfaces. This is not a gap---it is the physically correct theorem.
\end{tcolorbox}

\textbf{Approach B: Maximum Area Trapped Surface (Conditional Variational Proof).}

The key insight is to \textbf{bypass} the direct area comparison $A(\Sigma^*) \ge A(\Sigma_0)$. Instead, we prove the Penrose inequality by introducing an intermediate surface constructed via constrained area maximization.

\begin{remark}[Status of Theorem~\ref{thm:MaxAreaTrapped}]\label{rem:MaxAreaStatus}
\textbf{Warning:} The following theorem requires additional geometric hypotheses beyond DEC to be rigorously established. The variational problem ``maximize area among surfaces with $\theta^+ \le 0$'' is \textbf{not well-posed in general}: 
\begin{itemize}
...
We use varifold compactness instead. Under assumption (C1), the curvature bounds give uniform $C^{1,\alpha}$ estimates for $\Sigma_n$ by Schauder theory (the second fundamental form is controlled by the ambient curvature and the constraint $\theta^+ \le 0$). By Arzelà--Ascoli, a subsequence converges in $C^{1,\alpha}$ to a limiting surface $\Sigma_\infty$ with:
\begin{equation}
...
\textbf{This is precisely why we distinguish Track A from Track B:}

\begin{tcolorbox}[colback=yellow!5!white, colframe=orange!75!black, title=Why Two Tracks Are Necessary]
\begin{description}
    \item[Track A (Unconditional):] For \textbf{outermost MOTS} $\partial\Omega$, we prove $A(\partial\Omega) \le 16\pi M^2$ without additional hypotheses. The outermost condition, combined with the variational structure, excludes pathological configurations.
    
    \item[Track B (Conditional):] For \textbf{general trapped surfaces} $\Sigma \in \mathcal{A}$, we require the additional hypothesis that $\Sigma$ can be connected to $\partial\Omega$ via a foliation satisfying certain monotonicity conditions. This rules out ``Bag of Gold''-type surfaces that lie in causally disconnected regions.
\end{description}
\end{tcolorbox}

\textbf{The time-symmetric case is instructive:} When $k = 0$, we have $\theta^\pm = H$. A strictly trapped surface requires $H < 0$. In a Bag of Gold, the inner large minimal surface has $H = 0$---it is a MOTS, not strictly trapped. So for $k = 0$, there are no strictly trapped surfaces inside the bag that would violate Track A.

\textbf{The nontrivial case is $k \ne 0$:} With nonzero extrinsic curvature, one can have surfaces with $\theta^+ < 0$ and $\theta^- < 0$ (strictly trapped) inside regions where our flow-based argument does not apply. This is why Track B imposes the foliation hypothesis: it ensures we can propagate the area bound from $\partial\Omega$ to $\Sigma$.
\end{remark}
...
\textbf{Non-Self-Adjointness and the Principal Eigenvalue Theorem:} The MOTS stability operator $L_\Sigma^{\text{MOTS}}$ is \textbf{not} self-adjoint in general due to the drift term $2X \cdot \nabla$, where $X^a = k^a{}_\nu$. The standard variational arguments for spectral properties often implicitly assume self-adjointness. However, the key spectral properties we need---\emph{reality and simplicity of the principal eigenvalue} and \emph{strict positivity of the principal eigenfunction}---remain valid for non-self-adjoint operators by the following theorem:

\begin{tcolorbox}[colback=blue!5!white, colframe=blue!75!black, title=Principal Eigenvalue Theorem (Andersson--Mars--Simon \cite{anderssonmarssimonfaller2008})]
Let $L = -\Delta_\gamma + V + Y \cdot \nabla$ be a second-order elliptic operator on a compact Riemannian manifold $(\Sigma, \gamma)$, where $V \in L^\infty(\Sigma)$ and $Y$ is a smooth vector field (the ``drift term''). Then:
\begin{enumerate}
    \item The \textbf{principal eigenvalue} $\lambda_1(L)$ is \textbf{real and simple}.
    \item The \textbf{principal eigenfunction} $\phi_1$ satisfying $L\phi_1 = \lambda_1 \phi_1$ can be chosen to be \textbf{strictly positive}: $\phi_1(x) > 0$ for all $x \in \Sigma$.
    \item The adjoint operator $L^* = -\Delta_\gamma + V - \mathrm{div}(Y \cdot) + \mathrm{div}(Y)$ has the \textbf{same principal eigenvalue} $\lambda_1(L^*) = \lambda_1(L)$, with strictly positive principal eigenfunction $\phi_1^* > 0$.
\end{enumerate}
\textbf{Rigorous foundation:} This theorem is a consequence of the Krein--Rutman theorem for positive operators in ordered Banach spaces (see Kato \cite{kato1995}, Chapter~V). The key observation is that $e^{-tL}$ is a positive semigroup when $L$ has the form above, and the principal eigenvalue corresponds to the spectral radius of the semigroup generator. The strict positivity of $\phi_1$ follows from the strong maximum principle for elliptic operators with bounded drift. The equality $\lambda_1(L) = \lambda_1(L^*)$ follows from duality of the semigroups.
\end{tcolorbox}

\textbf{Application to MOTS stability:} The stability operator $L_\Sigma = -\Delta_\Sigma - (|A|^2 + \Ric(\nu,\nu)) + 2X \cdot \nabla$ has the form covered by the theorem. Therefore:
\begin{itemize}
    \item $\lambda_1(L_\Sigma) \ge 0$ (stability) is well-defined even for $k \neq 0$.
    \item The principal eigenfunction $\phi_1 > 0$ is \textbf{strictly positive pointwise}.
    \item Arguments involving $\phi_1 > 0$ (e.g., deriving the sign of $[H]$, testing first-order conditions) are \textbf{rigorously valid} without symmetrization.
\end{itemize}

\textbf{Key consequence for the mean curvature jump:} The formula $[H] \propto \lambda_1 \cdot \langle \phi_1^2 \rangle$ (Lemma~\ref{lem:EigenmodeContribution}) involves only $\phi_1 > 0$ and the real eigenvalue $\lambda_1 \ge 0$. Since $\phi_1 > 0$ everywhere and $\lambda_1 \ge 0$ by stability, we conclude $[H] \ge 0$ \textbf{pointwise}, not merely in an integral sense.

\textbf{In the time-symmetric case ($k = 0$):} We have $X = 0$, so $L_\Sigma^{\text{MOTS}} = L_\Sigma^0 = -\Delta_\Sigma - |A|^2 - \Ric(\nu,\nu)$ is genuinely self-adjoint, and all spectral arguments apply directly without invoking the non-self-adjoint theory.

\textbf{The key distinction:} For a \textbf{strictly stable} MOTS ($\lambda_1(L_\Sigma) > 0$), the constraint $\theta^+ \le 0$ is ``active'' in the sense that $\theta^+|_{\Sigma_{\max}} = 0$. However, the \textbf{tangent cone} to the feasible region at $\Sigma_{\max}$ is trivial:
\begin{equation}
...
\begin{enumerate}
    ...
    \item \textbf{Direct maximum principle:} Use Hopf's lemma and barrier arguments that do not require spectral decomposition. \textbf{Status:} No known proof in the literature.
    \item \textbf{Restrict to outermost MOTS:} For the outermost MOTS $\Sigma^*$, stability arguments (Theorem~\ref{thm:CompleteMeanCurvatureJump}) establish $[H] \ge 0$ \textbf{directly without variational methods}. This is why the MOTS case is unconditional.
\end{enumerate}

\textbf{Status:} The upgrade from integral to pointwise favorable condition in the general $k \neq 0$ case remains an \textbf{OPEN PROBLEM} within the variational approach. This means:
\begin{itemize}
    \item \textbf{Theorem~\ref{thm:penroseinitial} (MOTS case):} \textcolor{green!60!black}{\textbf{UNCONDITIONAL}} --- uses stability, not variational methods.
    \item \textbf{Favorable jump for $k = 0$:} \textcolor{green!60!black}{\textbf{RIGOROUS}} --- self-adjoint case.
    \item \textbf{Favorable jump for $k \neq 0$:} \textcolor{red}{\textbf{GAP}} --- only integral condition proved.
\end{itemize}
\end{remark}

\begin{remark}[Comparison with Previous Approaches]
Earlier versions of this argument (in preprints) established only the \emph{integral} favorable condition $\int_\Sigma \tr_\Sigma k \, dA \ge 0$. The upgrade to the \emph{pointwise} condition $\tr_\Sigma k \ge 0$ is essential because:
\begin{enumerate}
    \item The mean curvature jump formula gives $[H]_{\bar{g}} = \tr_\Sigma k$ \textbf{pointwise} (Lemma~\ref{lem:TrappedMeanCurvatureJump});
    \item The distributional scalar curvature contains $2[H]\delta_\Sigma$, which must be $\ge 0$ as a \textbf{measure}, not merely in integral.
\end{enumerate}
Theorem~\ref{thm:IntegralToPointwise} closes this gap using the elliptic structure of the constrained optimization problem.
\end{remark}

\begin{remark}[The k=0 Case is Already Solved]\label{rem:k0Solved}
\textbf{Historical note:} The Riemannian Penrose inequality (k=0 case) was \textbf{completely solved in 2001} by Huisken--Ilmanen \cite{huisken2001} using weak inverse mean curvature flow, and independently by Bray \cite{bray2001} using conformal flow of metrics. Both proofs establish:
\begin{equation}
    M_{\mathrm{ADM}}(g) \geq \sqrt{\frac{A(\Sigma)}{16\pi}}
\end{equation}
for \textbf{any} minimal surface $\Sigma$ (not just outermost) in time-symmetric data $(M^3, g, k=0)$ satisfying $R_g \geq 0$ (which follows from DEC when $k=0$).

\textbf{Our contribution for k=0:} We provide an \emph{alternative proof} using the p-harmonic level set method (see Corollary~\ref{cor:InitialDataPenrose}). This is primarily of methodological interest, showing that the elliptic PDE approach also works for the already-solved Riemannian case.

\textbf{The open problem:} The \textbf{spacetime case} with $k \neq 0$ (non-time-symmetric data) has remained open for 50+ years. This is what we address.
\end{remark}

\begin{theorem}[Path to Completing Penrose 1973 for Spacetime Case $k \neq 0$]\label{thm:PathToComplete1973}
To prove Penrose's 1973 conjecture for arbitrary trapped surfaces with $k \neq 0$, it suffices to establish \textbf{any one} of the following:

\textbf{Option A (Area Monotonicity):}
\begin{equation}
    A(\Sigma^*) \geq A(\Sigma_0) \quad \text{for any trapped surface } \Sigma_0 \text{ enclosed by outermost MOTS } \Sigma^*.
\end{equation}
\textit{Status:} Known to \textbf{fail in general} (counterexamples in binary black hole mergers). May hold under weak cosmic censorship---\textbf{OPEN}.
...
\textbf{Option C (Maximum Area Method):}
...
\textit{Status:} \textbf{GAP}---the integral-to-pointwise upgrade (Theorem~\ref{thm:IntegralToPointwise}) is only proved for $k=0$.

\textbf{Option D (Modified Jang):}
Develop a modification of the Jang equation that handles unfavorable jump $\tr_\Sigma k < 0$ directly.
\textit{Status:} \textbf{OPEN}---no known construction.
\end{theorem}
...
\begin{enumerate}
    ...
    \item \textbf{Critical step (conditional):} The upgrade to pointwise $\tr_{\Sigma_{\max}} k \ge 0$ requires either:
    \begin{itemize}
        \item $k = 0$ (time-symmetric case, where Theorem~\ref{thm:IntegralToPointwise} applies), or
        \item Additional spectral analysis for non-self-adjoint operators (currently \textbf{OPEN} for $k \neq 0$).
    \end{itemize}
...
\end{enumerate}
\textbf{Honest assessment:} This proof is \textbf{fully rigorous for $k = 0$} (time-symmetric data). For $k \neq 0$, Step 4 represents an \textbf{open gap}---the integral-to-pointwise upgrade is not yet established.
\end{proof}
...
We now provide a complete proof of Penrose's original 1973 conjecture. The key insight is that the outer-minimizing (OM) property follows from \textbf{ingoing null focusing} under WCC.

\begin{tcolorbox}[colback=green!10!white, colframe=green!60!black, title=\textbf{Main Result: Penrose 1973 (PROVED under WCC)}]
\textbf{Theorem~\ref{thm:Penrose1973Complete}:} Under WCC + NEC + Final State, for \textbf{any} trapped surface $\Sigma$:
\begin{equation}
    M_{\mathrm{ADM}} \geq \sqrt{\frac{A(\Sigma)}{16\pi}}
\end{equation}
This resolves Penrose's \emph{original} 1973 conjecture, which explicitly assumed cosmic censorship. The outer-minimizing assumption (OM) is established via \textbf{ingoing null focusing} from the event horizon (Lemma~\ref{lem:AreaComparison}).
\end{tcolorbox}
...
\begin{equation}
    \boxed{A(\Sigma) \leq A(\mathcal{H}_\mathcal{C})}.
\end{equation}
...
\subsection{Sign conventions and definitions}\label{subsec:Conventions}

We establish the sign conventions used throughout this paper. All subsequent equations should be verified against these definitions.

\begin{center}
\fbox{\parbox{0.9\textwidth}{
\textbf{Geometric Setup}
\begin{itemize}[leftmargin=*,itemsep=0pt,parsep=2pt]
    \item \textbf{Spacetime:} $(N^{3+1}, \bar{g})$ with signature $(-,+,+,+)$.
    \item \textbf{Cauchy hypersurface:} $(M^3, g) \subset N$ with induced Riemannian metric.
    \item \textbf{Unit normals:} $u$ = future-directed timelike unit normal to $M$; $\nu$ = outward-pointing spacelike unit normal to $\Sigma \subset M$.
    \item \textbf{Extrinsic curvature:} $k_{ij} = \bar{g}(\bar{\nabla}_{\partial_i} u, \partial_j)$ (second fundamental form of $M$ in $N$).
\end{itemize}

\textbf{Null Geometry}
\begin{itemize}[leftmargin=*,itemsep=0pt,parsep=2pt]
    \item \textbf{Null normals:} $\ell^+ = u + \nu$ (future-outgoing), $\ell^- = u - \nu$ (future-ingoing).
    \item \textbf{Mean curvature:} $H_\Sigma = \div_\Sigma \nu = \tr_\Sigma(A)$, where $A_{ij} = g(\nabla_{\partial_i}\nu, \partial_j)$. A round sphere in $\mathbb{R}^3$ has $H > 0$.
    \item \textbf{Null expansions:}
    \begin{align*}
        \theta^+ &= H_\Sigma + \tr_\Sigma k \quad \text{(outward/future null expansion)}, \\
        \theta^- &= H_\Sigma - \tr_\Sigma k \quad \text{(inward/future null expansion)}.
    \end{align*}
\end{itemize}

\textbf{Trapped Surfaces}
\begin{itemize}[leftmargin=*,itemsep=0pt,parsep=2pt]
    \item \textbf{Future trapped:} $\theta^+ \le 0$ and $\theta^- \le 0$.
    \item \textbf{Strictly future trapped:} $\theta^+ < 0$ and $\theta^- < 0$.
    \item \textbf{MOTS (Marginally Outer Trapped Surface):} $\theta^+ = 0$.
    \item \textbf{Outermost MOTS:} The boundary $\Sigma^* = \partial\mathcal{T}$ of the trapped region; always \textbf{stable}.
\end{itemize}

\textbf{Mean Curvature Jump}
\begin{itemize}[leftmargin=*,itemsep=0pt,parsep=2pt]
    \item At an interface $\Sigma$ with exterior $\Omega^+$ and interior $\Omega^-$:
    \[
        [H] = H^+ - H^- = \tr_\Sigma k.
    \]
    \item \textbf{For stable MOTS:} $[H] \ge 0$ automatically (Theorem~\ref{thm:CompleteMeanCurvatureJump}).
    \item \textbf{For general trapped surfaces:} We reduce to the outermost MOTS via Area Monotonicity (Theorem~\ref{thm:AreaMonotonicity}).
\end{itemize}
}}
\end{center}
...
\textbf{CORE PROOF: SPACETIME PENROSE INEQUALITY}
...
\begin{itemize}[nosep,leftmargin=*]
    ...
    \item \textbf{Area Monotonicity:} $A(\Sigma^*) \ge A(\Sigma_0)$ (\textbf{requires cosmic censorship or compactness}).
    \item \textbf{Stability:} $\Sigma^*$ is automatically stable (outermost $\Rightarrow$ $\lambda_1(L_{\Sigma^*}) \ge 0$).
...
\[
M_{\mathrm{ADM}}(g) \ge M_{\mathrm{ADM}}(\bar{g}) \ge M_{\mathrm{ADM}}(\tilde{g}) \ge \sqrt{\frac{A(\Sigma^*)}{16\pi}} \ge \sqrt{\frac{A(\Sigma_0)}{16\pi}}. \quad \qed
\]
\end{mdframed}
...
\begin{tcolorbox}[colback=red!5!white, colframe=red!75!black, title=\textbf{Theorem~\ref{thm:Obstruction} Justifies the Two-Track Structure}]
The Fundamental Obstruction is not merely a negative result---it is \textbf{structurally essential} to understanding why the Spacetime Penrose Inequality requires different treatments:
...
\hline
Outermost MOTS $\partial\Omega$ & $[H] \ge 0$ by stability & \textbf{Track A:} Unconditional \\
\hline
General trapped $\Sigma$ & $\tr_\Sigma k$ arbitrary & \textbf{Track B:} Need foliation hypothesis \\
\hline
\end{tabular}
\end{center}

\textbf{Key insight:} The outermost MOTS \emph{automatically} satisfies the favorable jump condition (Lemma~\ref{lem:OutermostJump}) due to its variational characterization. This is why Track A succeeds unconditionally. For general trapped surfaces, Theorem~\ref{thm:Obstruction} proves that conformal methods \emph{cannot} work when $\tr_\Sigma k < 0$, necessitating the additional hypotheses of Track B.

\textbf{This obstruction is sharp:} It applies to \emph{any} conformal approach that attempts to simultaneously preserve area at $\Sigma$ and reduce mass at infinity. No clever choice of conformal factor can evade it.
\end{tcolorbox}
