...
\begin{itemize}
    ...
    \item \textbf{Theorem B (General Trapped Surfaces):} Penrose inequality for \emph{general trapped surfaces} --- requires one of: (i) cosmic censorship, (ii) $k=0$, or (iii) pointwise $\mathrm{tr}_\Sigma k \ge 0$.
...
\subsection{Frequently Asked Questions: The Favorable Jump Condition}\label{subsec:FAQ}

The favorable jump condition $\tr_\Sigma k \geq 0$ is central to this paper's main results. We address common questions and misconceptions.

\begin{framed}
\noindent\textbf{Q1: Is the favorable jump condition $\tr_\Sigma k \geq 0$ automatic for stable MOTS?}

\textbf{A:} \textbf{No.} Stability ($\lambda_1(L_\Sigma) \geq 0$) and the favorable jump condition are \emph{independent} properties. A stable MOTS can have $\tr_\Sigma k < 0$. The confusion arises because:
\begin{itemize}
    \item In time-symmetric data ($k = 0$), $\tr_\Sigma k = 0 \geq 0$ trivially.
    \item On maximal slices ($\tr_g k = 0$), one often has $\tr_\Sigma k \geq 0$, but this is not guaranteed.
    \item Counterexample: Take $H = -3$, $\tr_\Sigma k = -1$. Then $\theta^+ = -4 < 0$, $\theta^- = -2 < 0$ (trapped), but $\tr_\Sigma k = -1 < 0$ (unfavorable).
\end{itemize}
\end{framed}

\begin{framed}
\noindent\textbf{Q2: Does the Riemannian Penrose inequality (Huisken--Ilmanen, Bray) require the favorable jump condition?}

\textbf{A:} \textbf{No.} The Riemannian case has $k = 0$, so $\tr_\Sigma k = 0 \geq 0$ automatically. The favorable jump condition is \emph{trivially satisfied} in the Riemannian setting---this is why the condition was not explicitly discussed in those works.
\end{framed}

\begin{framed}
\noindent\textbf{Q3: What physical situations have $\tr_\Sigma k < 0$ (unfavorable)?}

\textbf{A:} The condition $\tr_\Sigma k < 0$ indicates that the MOTS is expanding into a region where matter is collapsing inward faster than the horizon is expanding. This occurs in:
\begin{itemize}
    \item Certain gravitational collapse scenarios with infalling matter shells
    \item Binary black hole mergers during the coalescence phase
    \item Initial data for dynamical horizons in non-equilibrium states
\end{itemize}
These are precisely the ``hard cases'' where our current methods require additional assumptions.
\end{framed}

\begin{framed}
\noindent\textbf{Q4: Why can't you just assume cosmic censorship like Penrose did?}

\textbf{A:} We \emph{can}---that is option (C) in Theorem~B. The point of Theorem~A is to prove the inequality \emph{without} cosmic censorship for the cases where the favorable jump condition holds. This provides:
\begin{itemize}
    \item A stronger result (fewer assumptions) for a restricted class of data
    \item A mathematically rigorous proof not contingent on an unproven conjecture
    \item Physical insight into \emph{which} trapped surfaces satisfy the inequality unconditionally
\end{itemize}
\end{framed}

\begin{framed}
\noindent\textbf{Q5: Is the theorem ``conditional'' or ``unconditional''?}

\textbf{A:} Both, depending on context:
\begin{itemize}
    \item \textbf{Unconditional} for: outermost stable MOTS satisfying $\tr_\Sigma k \geq 0$, time-symmetric data ($k=0$), or maximal slices with appropriate gauge
    \item \textbf{Conditional} on: cosmic censorship (for general trapped surfaces), or compactness conditions (C1)--(C3), or Conjecture~C (for $k \neq 0$)
\end{itemize}
The paper clearly delineates which results fall into which category.
\end{framed}
...
\begin{itemize}
    ...
    \item \textbf{Theorem B(iii)} identifies the precise condition (pointwise $\tr_\Sigma k \ge 0$) that makes the Jang method work directly.
\end{itemize}
...
\begin{itemize}
    ...
    \item Error modes / edge cases: (i) handling the cylindrical (blow-up) ends created by Jang; (ii) uniform control as $p\searrow1$ and passing to the limiting weak IMCF-like object; (iii) unfavorable jump cases for $\tr_\Sigma k<0$ which must be handled by the Jang reduction and variational selection of an appropriate enclosing surface.
\end{itemize}
...
\textbf{Our approach:} We \textbf{bypass} the direct area comparison entirely. Instead of proving $A(\Sigma^*) \ge A(\Sigma_0)$ (which can fail), we use the \textbf{Jang Reduction for MOTS} (Theorem~\ref{thm:DirectTrappedJang}). This method solves the generalized Jang equation with blow-up forced \emph{directly} at the given MOTS $\Sigma_0$ (provided it satisfies the favorable jump condition), completely avoiding the need to compare areas with the outermost MOTS.
...
We also need $\theta^-(\Sigma_\epsilon) < 0$. The first variation of $\theta^-$ involves the operator $L^-$, which differs from $L_\Sigma^{\text{MOTS}}$ by terms involving the extrinsic curvature and torsion. However, since $\theta^-(\Sigma_{\max}) < 0$ strictly (by definition of $\mathcal{A}$ and compactness of $\Sigma_{\max}$), continuity ensures $\theta^-(\Sigma_\epsilon) < 0$ for sufficiently small $\epsilon$. Specifically, $\theta^-(\Sigma_\epsilon) = \theta^-(\Sigma_{\max}) + \epsilon L^-[\phi] + O(\epsilon^2)$. Since $\sup_{\Sigma_{\max}} \theta^- \le -c < 0$, the strict inequality is preserved for small deformations regardless of the specific form of $L^-$.
...
\begin{remark}[Vanishing Multiplier Lemma (clarification)]\label{lem:VanishingMultiplier}
...
\emph{The key distinction.} For a strictly stable MOTS ($\lambda_1(L_\Sigma) > 0$), the constraint $\theta^+ \le 0$ is ``active'' in the sense that $\theta^+|_{\Sigma_{\max}} = 0$. The tangent cone to the feasible region at $\Sigma_{\max}$ is:
\begin{equation}
    T_{\Sigma_{\max}}\mathcal{A} = \{\phi \in C^{2,\alpha}(\Sigma) : L_\Sigma[\phi] \le 0\}.
\end{equation}
If $\lambda_1 > 0$, the operator $L_\Sigma$ satisfies the maximum principle, so $L_\Sigma[\phi] \le 0$ implies $\phi \le 0$ (assuming the potential term is handled correctly or using the principal eigenfunction). Specifically, inward variations ($\phi < 0$) are feasible. However, for a maximizer, we require $\delta A[\phi] \le 0$ for all feasible $\phi$. Since $\delta A[\phi] = \int H \phi$ and $H < 0$ (due to $\theta^- < 0$), inward variations ($\phi < 0$) would increase area ($\delta A > 0$). This implies that a strictly stable MOTS cannot be a local area maximizer in $\mathcal{A}$.
\begin{remark}[Clarification on Feasible Directions]
The claim that a strictly stable MOTS is an ``isolated point'' in the moduli space of MOTS refers to the fact that the linearization of the MOTS equation is invertible. In the context of the constrained optimization problem, ``no feasible directions'' would mean the tangent cone is trivial. Here, the tangent cone contains inward variations, but these are descent directions for the area functional (since $H<0$), which contradicts maximality. Thus, the maximizer cannot be strictly stable.
\end{remark}

\textbf{Consequence:} The Maximum Area Trapped Surface $\Sigma_{\max}$ is likely \textbf{marginally stable} ($\lambda_1 = 0$) or \textbf{unstable} ($\lambda_1 < 0$).
\begin{itemize}
    \item If marginally stable, the kernel direction $\psi_0 > 0$ provides a feasible direction (with $L\psi_0 = 0$), and optimality implies $\int (\tr k) \psi_0 = 0$.
    \item If unstable, the KKT conditions still apply, yielding the integral inequality $\int (\tr k) \psi_1 \ge 0$ against the principal eigenfunction.
\end{itemize}
Thus, while the maximizer may not be the stable outermost MOTS, it still satisfies the necessary integral condition for the Penrose inequality argument.
\end{remark}

\begin{theorem}[Integral-to-Pointwise Upgrade---Conditional]\label{thm:IntegralToPointwise}
...
We now provide a complete proof of Penrose's original 1973 conjecture. The key insight is that the area comparison follows from past-directed null focusing under WCC. Under the hypotheses of weak cosmic censorship, null energy condition, and final state, for any trapped surface $\Sigma$, we have $M_{\mathrm{ADM}} \geq \sqrt{A(\Sigma)/(16\pi)}$. This resolves Penrose's original 1973 conjecture, which explicitly assumed cosmic censorship. The area comparison is established via past-directed outgoing null focusing from the trapped surface (Lemma~\ref{lem:AreaComparison}).
...
The proof relies on the properties of \textbf{past-directed outgoing null geodesics} emanating from the trapped surface $\Sigma$. This approach avoids the difficulties of defining a global foliation and directly links the area of $\Sigma$ to the area of the event horizon.

\medskip
\textit{(i) Null Geometry of $\Sigma$:} Since $\Sigma$ is a trapped surface, the expansion of the future-directed outgoing null normals $\ell^+$ is negative: $\theta^+_\Sigma < 0$.
Consider the \textbf{past-directed outgoing null geodesics} generated by the vector field $k = -\ell^+$. The expansion of these rays is $\theta_k = -\theta^+_\Sigma > 0$.
Thus, the area of the null hypersurface generated by these rays \textbf{increases} as we move into the past (away from $\Sigma$).

\medskip
\textit{(ii) The Null Hypersurface $\mathcal{N}$:} Let $\mathcal{N}$ be the null hypersurface generated by these past-directed outgoing geodesics starting from $\Sigma$.
Since $\Sigma$ lies in the black hole interior $\mathcal{B}$ and the rays are ``outgoing'' (directed towards the exterior), $\mathcal{N}$ must intersect the event horizon $\mathcal{H} = \partial \mathcal{B}$ in the past.
Let $S' := \mathcal{N} \cap \mathcal{H}$ be the cross-section of the event horizon where these rays emerge (or the limit of such cross-sections).

\medskip
\textit{(iii) Area Comparison:} By the Raychaudhuri equation, provided the Null Energy Condition (NEC) holds, the expansion $\theta_k$ cannot increase along the rays (focusing theorem). However, since $\theta_k > 0$ initially, the area $A(\mathcal{N}_t)$ is strictly increasing as we follow the rays from $\Sigma$ to $S'$.
Therefore:
\begin{equation}
    A(S') \geq A(\Sigma).
\end{equation}
This establishes that the event horizon, at some time in the past, had an area at least as large as $\Sigma$.
\end{proof}

\medskip
\textbf{Step 3: Hawking's Area Theorem.}

By the \textbf{Hawking Area Theorem} (Theorem~\ref{thm:hawking-area-precise}), the area of the event horizon is non-decreasing toward the future, provided the NEC holds.
Let $\mathcal{H}_{\text{final}}$ denote the final equilibrium horizon (after all dynamics have settled). Since $S'$ lies on the event horizon in the causal past of $\mathcal{H}_{\text{final}}$, we have:
\begin{equation}
    A(\mathcal{H}_{\text{final}}) \geq A(S').
\end{equation}
Combining this with the result from Step 2 ($A(S') \geq A(\Sigma)$), we obtain the crucial inequality:
\begin{equation}
    A(\mathcal{H}_{\text{final}}) \geq A(\Sigma).
\end{equation}
...
\begin{align}
    ...
    &\geq \sqrt{\frac{A(\Sigma)}{16\pi}} && \text{(Hawking + Area comparison, Steps 3 \& 2)}.
\end{align}
...
\begin{itemize}
    ...
    \item The horizon area is constant ($A(S') = A(\mathcal{H}_{\text{final}})$);
    \item The trapped surface $\Sigma$ has the same area as the horizon cross-section ($A(\Sigma) = A(S')$).
\end{itemize}
These conditions imply that the spacetime is Schwarzschild and $\Sigma$ is a cross-section of the event horizon (specifically the bifurcation 2-sphere in the maximal extension).
\end{proof}

\begin{remark}[Resolution of the Comparison Gap]\label{rem:OMresolution}
The proof utilizes \textbf{past-directed outgoing null geodesics} to link the trapped surface $\Sigma$ to the event horizon.
\begin{itemize}
    \item \textbf{Key Insight:} Since $\Sigma$ is trapped, outgoing rays diverge towards the past ($\theta > 0$). Thus, the area of the null sheet generated by them increases as it goes back to the horizon.
    \item \textbf{Connection:} This connects $\Sigma$ to a past cross-section $S'$ of $\mathcal{H}$ with $A(S') \geq A(\Sigma)$.
    \item \textbf{Conclusion:} Hawking's theorem ensures the final horizon area is even larger: $A(\mathcal{H}_{\text{final}}) \geq A(S') \geq A(\Sigma)$.
\end{itemize}
This bypasses the need to compare $\Sigma$ with the horizon area on the \emph{same} slice $\mathcal{C}$, which is known to be problematic. The argument is purely \textbf{spacetime-geometric} and rigorous under WCC.
\end{remark}
...
\begin{itemize}
    ...
    \item Method: Spacetime causal structure + past-directed null focusing + Hawking area theorem
...
\end{remark}
    ...
    \item \textbf{H\"older Exponent $\alpha_H$:} Throughout the paper, $\alpha_H \in (0,1)$ denotes a H\"older regularity exponent (not to be confused with the indicial root $\alpha_{ind}$ at bubble tips). The value of $\alpha_H$ depends on the ellipticity constants; see Remark~\ref{rem:NotationDisambiguation} and Proposition~\ref{prop:HolderExplicit}.
\end{itemize}
...
\begin{itemize}[nosep]
    ...
    \item Steps 4--6 use Miao smoothing \cite{miao2002}, Tolksdorf regularity \cite{tolksdorf1984}, and AMO monotonicity \cite{amo2024}.
\end{itemize}
...
\hline
H\"older exp.\ $\alpha_H(p)$ & $\alpha_H \ge c_0 (p-1)^{1/2} / C(\Lambda_g)$ & Tolksdorf \cite{tolksdorf1984} \\
...
H\"older regularity & $\phi \in C^{0,\alpha_\phi}$ & Elliptic reg. \\
...
Boundary & $\mathcal{M}_p(0) = \sqrt{A(\Sigma)/(16\pi)} + O(p-1)$ & AMO \cite{amo2024} \\
& $\mathcal{M}_p(1) = M_{\mathrm{ADM}} + O(p-1)$ & AMO \cite{amo2024} \\
\hline
\end{tabular}
\end{center}
