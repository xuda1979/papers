...
\begin{enumerate}
    ...
    \item Under this convention, the favorable-jump condition $[H] \geq 0$ is \emph{equivalent} to $H^+ \geq H^-$, meaning the exterior side has larger (or equal) mean curvature.
\end{enumerate}
This convention is consistent with the distributional identity $R = R_{\text{bulk}} + 2[H]\delta_\Sigma$, where the coefficient $2$ arises from the codimension-1 Gauss--Codazzi analysis. In the rigorous core of the paper, Theorem~\ref{thm:CompleteMeanCurvatureJump} gives $[H]=\tr_\Sigma k \ge 0$ under the explicit favorable-jump hypothesis.
\end{remark}
...
As an independent check, Appendix~\ref{app:Schwarzschild} records the exact Schwarzschild identities in the time-symmetric equality case, where the Jang step is trivial and the Hawking mass of the spherical foliation is constant.
...
\begin{equation*} \begin{aligned} \tg_{ij} - \delta_{ij} &= \phi^4 \bg_{ij} - \delta_{ij} = (\phi^4 - 1)\delta_{ij} + \phi^4(\bg_{ij} - \delta_{ij}) \\
...
Theorem~\ref{thm:CompleteMeanCurvatureJump} establishes $[H]_{\bg} = \tr_\Sigma k \ge 0$ under the favorable-jump hypothesis. The conformal transformation affects the mean curvature via:
\begin{equation}
