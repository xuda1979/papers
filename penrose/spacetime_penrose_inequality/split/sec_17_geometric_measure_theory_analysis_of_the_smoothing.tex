...
Since $\hat{g}_\epsilon$ is $(1+K\epsilon)$-bi-Lipschitz to $\bg$, this yields $I(\hat{g}_\epsilon) \ge (1-2K\epsilon) I(\bg)$.
...
\textbf{3. Regularity:} The limit metric $\bg$ is Lipschitz. Stationary varifolds in Lipschitz metrics are not necessarily smooth. However, $\bg$ is special: it is the Jang metric, and in the direct favorable-jump regime the interface is treated as an internal corner. Separate zero-jump situations may simplify that interface structure, but no $C^{1,1}$ conclusion is used here.
...
\begin{enumerate}
    ...
    \item[(c)] \textbf{Calibration identifies the minimizing class:} The calibration shows that every minimizing cross-section $\Sigma_t$ of the cylinder has area $\Area(\Sigma)$, so any minimizing limit supported in the collar has mass at least $\Area(\Sigma)$. Combined with the trial-surface upper bound and the barrier argument in (d), this identifies the limit with the distinguished horizon cross-section $\Sigma$.

    \item[(d)] \textbf{Maximum principle:} The argument here uses only the geometric barrier structure of the collar and the minimizing property of the calibrated cylinder. It does \emph{not} require the additional claim that stability alone implies a favorable sign for the bulk-side mean curvature. Since $\Sigma_\epsilon$ are outermost, they cannot penetrate into the cylindrical region beyond $\Sigma$. Combined with (c), this forces $V = \Sigma$.
\end{enumerate}
