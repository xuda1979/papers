...
\begin{theorem}[Integral-to-Pointwise Favorable Condition (Time-Symmetric Case)]\label{thm:IntegralToPointwiseAppendix}\label{thm:IntegralToPointwise}
Let $\Sigma \subset M$ be a stable MOTS ($\theta^+ = 0$ and $\lambda_1(L_\Sigma) \ge 0$) in a time-symmetric initial data set ($k=0$). Suppose $\Sigma$ is a constrained area maximum among surfaces with $\theta^+ \le 0$. If the integral favorable condition
