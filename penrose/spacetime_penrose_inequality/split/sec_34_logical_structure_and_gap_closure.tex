...
\subsection{On the Failed Integral-to-Pointwise Bridge}
\label{subsec:BridgeTheorem}

The mean curvature jump formula (Theorem~\ref{thm:CompleteMeanCurvatureJump}) requires a pointwise favorable condition. Earlier drafts attempted to upgrade a weaker integral condition to a pointwise sign conclusion. That bridge is \emph{not} established here in general.

\begin{theorem}[Time-Symmetric Case]\label{thm:IntegralToPointwiseAppendix}\label{thm:IntegralToPointwise}
...
\begin{equation}
    H^+_{\bar{g}}(\Sigma) = \frac{H_\Sigma + \tr_\Sigma k}{2} = \frac{\theta^+}{2},
\end{equation}
where $\theta^+ = H + \tr_\Sigma k$ is the outward null expansion.

Similarly, approaching from the cylindrical end (the ``inside'' where $f \to +\infty$), the relevant limiting mean curvature is determined by the Jang graph condition (see, e.g., Han--Khuri):
\[
    H^-_{\bar{g}}(\Sigma) = -\tr_\Sigma k.
\]

Therefore:
\begin{equation}
    [H]_{\bar{g}} = H^+_{\bar{g}} - H^-_{\bar{g}} = \frac{H_\Sigma + \tr_\Sigma k}{2}.
\end{equation}

\textbf{Step 5: Sign verification in the direct Jang regime.}
...
At this point the variational route does \emph{not} provide the pointwise sign $\tr_{\Sigma_{\max}} k \ge 0$. Lemma~\ref{lem:VanishingMultiplier} yields only a weighted orthogonality relation in the marginally stable case, which is insufficient to deduce the favorable jump
\[
    [H]_{\bar{g}} = \tr_{\Sigma_{\max}} k \ge 0.
\]
Therefore this max-area route does not by itself close the direct Jang proof. The rigorous core of the paper instead assumes the favorable jump explicitly when invoking Theorem~\ref{thm:CompleteMeanCurvatureJump}.
\end{proof}
