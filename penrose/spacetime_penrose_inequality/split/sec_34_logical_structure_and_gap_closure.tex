...
The fundamental challenge in extending the spacetime Penrose inequality from the time-symmetric case to the general case is establishing the pointwise favorable sign of the trace of the extrinsic curvature, $\tr_\Sigma k \ge 0$, on the relevant MOTS. The first-variation argument naturally yields an adjoint-cone condition. This section records that condition and the precise obstruction to upgrading it to pointwise sign without an additional geometric input.
...
The adjoint-cone condition of Theorem~\ref{thm:AdjointConeEuler} does not, by itself, imply the pointwise sign $\tr_\Sigma k\ge0$.
\end{theorem}

\begin{proof}
On the round sphere take \(L=-\Delta_{S^2}+1\). Let \(Y\) be a first spherical harmonic normalized by \(-1\le Y\le1\), choose \(1/3<\varepsilon<1\), and set
\[
    \rho=1+\varepsilon Y>0,\qquad h=L(-\rho)=-(1+3\varepsilon Y).
\]
Then \(h\) is sign-changing, since \(1+3\varepsilon Y\) changes sign. However \(L^{-*}h=L^{-1}h=-\rho<0\). Therefore, for every \(\eta\ge0\), if \(w=L^{-1}\eta\), self-adjointness gives
\[
    \int_{S^2} h w\,dA = \int_{S^2} (L^{-1}h)\eta\,dA
    = -\int_{S^2}\rho\,\eta\,dA \le 0.
\]
Thus the adjoint-cone inequality can hold while the source term is not pointwise nonnegative. Any pointwise favorable-jump conclusion in the non-time-symmetric case therefore requires extra geometric information beyond this bare variational condition.
\end{proof}

\begin{theorem}[Time-symmetric favorable jump]\label{thm:IntegralToPointwise}
If \(k=0\), then every surface \(\Sigma\) satisfies the favorable jump condition \(\tr_\Sigma k=0\). In particular, the direct Jang reduction has no integral-to-pointwise sign gap in the time-symmetric case.
\end{theorem}

\begin{proof}
The restriction of the zero tensor \(k\) to \(T\Sigma\) is identically zero, hence its surface trace is identically zero.
\end{proof}

\begin{remark}[Remaining non-time-symmetric input]\label{rem:PointwiseGapRemaining}
