...
Note that the favorable condition $\tr_\Sigma k \ge 0$ is precisely what is needed for the mean curvature jump in the Jang metric. With the convention $\theta^+ = H_\Sigma + \tr_\Sigma k$, a MOTS satisfies $\theta^+=0$ and hence $H_\Sigma = -\tr_\Sigma k$. The Jang blow-up analysis (Corollary~\ref{cor:FavorableForJump}) shows that $[H]_{\bar{g}} = \tr_\Sigma k$, so $\tr_\Sigma k \ge 0$ implies the favorable sign for the distributional scalar curvature.
\end{remark}
...
Therefore:
\begin{equation}
    [H]_{\bar{g}} = H^+_{\bar{g}} - H^-_{\bar{g}} = 0 - (-\tr_\Sigma k) = \tr_\Sigma k.
\end{equation}

\textbf{Step 5: Sign verification for stable MOTS.}
For a MOTS, $\theta^+ = H + \tr_\Sigma k = 0$, so $H = -\tr_\Sigma k$. In particular $\theta^+[\Sigma]=0$ and the computation above gives $[H]_{\bar{g}}=\tr_\Sigma k$, confirming \eqref{eq:JumpFormulaAppendix}.
...
\end{itemize}
The key relation for a MOTS is: $H = -\tr_\Sigma k$ (since $\theta^+ = H+\tr_\Sigma k = 0$), while $[H]_{\bar{g}} = \tr_\Sigma k = -H$.
\end{remark}
