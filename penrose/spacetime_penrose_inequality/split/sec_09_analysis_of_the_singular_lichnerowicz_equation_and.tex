...
\begin{tcolorbox}[colback=yellow!5!white, colframe=yellow!75!black, title=\textbf{Sequential Stability Strategy for the Double Limit}]
The double limit $(p, \epsilon) \to (1^+, 0)$ presents two potentially competing difficulties:
\begin{enumerate}
    \item As $p \to 1^+$: The $p$-Laplacian degenerates (becomes the 1-Laplacian), and the regularity theory weakens.
    \item As $\epsilon \to 0$: The curvature concentrates (distributional scalar curvature $\mathcal{R} = R^{\mathrm{reg}} + 2[H]\delta_\Sigma$).
\end{enumerate}

\textbf{The Sequential Stability Strategy:}
\begin{enumerate}
    \item \textbf{Fix $\epsilon > 0$ first.} On the smoothed manifold $(\tM, \hat{g}_\epsilon)$, the metric is smooth and all standard elliptic theory applies.
    \item \textbf{Send $p \to 1^+$.} Establish the \textbf{Distributional Bochner Identity} (Theorem~\ref{thm:DistrBochner}) on the smooth metric $\hat{g}_\epsilon$. The singular curvature term $2[H]\delta_\Sigma$ has been smoothed away, so standard Bochner formulas apply. The $p$-harmonic level sets converge to IMCF-type level sets with quantitative rate $(p-1)^{1/2}$.
    \item \textbf{Send $\epsilon \to 0^+$ last.} Use \textbf{Mosco convergence} to pass the $p \to 1$ result to the Lipschitz limit metric $\tg$. The key is that the Tolksdorf regularity bounds are \textbf{uniform in $\epsilon$} (by the uniform ellipticity of $\{\hat{g}_\epsilon\}$), ensuring the $p$-harmonic convergence rate is $\epsilon$-independent.
\end{enumerate}

\textbf{Why this order works:} The Tolksdorf--Lieberman gradient estimates depend on the ellipticity ratio $\Lambda/\lambda$, which is \textbf{uniform in $\epsilon$} by Lemma~\ref{lem:UniformEllipticity}. This uniformity propagates to all downstream estimates, enabling the Moore--Osgood interchange of limits.
\end{tcolorbox}
...
\begin{equation}\label{eq:ConeMetric}
    \boxed{\tg \approx d\rho^2 + (2\alpha)^2 \rho^2 \gamma_\Sigma.}
\end{equation}
...
\textbf{Conclusions:}
\begin{enumerate}
    \item The \textbf{correct coordinate transformation} from cylinder $(t, y)$ to punctured ball $(r, y)$ is $r = e^{-t}$, giving $dt^2 = dr^2/r^2$ (not $dr^2$).
    \item The \textbf{indicial equation} $\lambda^2 = \mu_0$ gives the decay exponent $\alpha = \sqrt{\mu_0} > 0$.
    \item The sealed metric is a \textbf{3D cone} $\tg \approx d\rho^2 + (2\alpha)^2 \rho^2 \gamma_\Sigma$.
    \item The \textbf{3D scalar curvature} at cones is $R \sim (R_h - 2)/\rho^2$, NOT a Dirac mass.
    \item For $\alpha > 1/2$, $R_{\tg} < 0$ near the tip, but $R_{\tg} \in L^1_{\text{loc}}$.
    \item The negative curvature is \textbf{invisible to the AMO analysis} because $\Cap_p(\{p_k\}) = 0$ for $1 < p < 3$.
    \item \textbf{No Dirac mass formula} is used; the capacity bypass is the rigorous resolution.
\end{enumerate}
\end{mdframed}
\end{lemma}
...
\begin{tcolorbox}[colback=green!5!white, colframe=green!75!black, title=\textbf{Key Clarification: Regularity vs.\ Singularity}]
\textbf{Potential referee question:} ``How can $\phi \in C^{1,\alpha}$ if $R_{\bar{g}}$ contains a Dirac mass $2[H]\delta_\Sigma$?''

\textbf{Answer:} The Lichnerowicz equation uses only the \textbf{regular part} of the curvature as the PDE potential:
\[
-8\Delta_{\bar{g}}\phi + \underbrace{R^{\mathrm{reg}}_{\bar{g}}}_{\text{in }L^p_{\text{loc}}} \phi = 0
\]
The Dirac mass $2[H]\delta_\Sigma$ is a \textbf{geometric feature} of the manifold that appears in the scalar curvature \emph{identity} (via the distributional decomposition $R_{\bar{g}} = R^{\mathrm{reg}} + 2[H]\delta_\Sigma$), but it is \textbf{not a coefficient} in the PDE for $\phi$.

\textbf{Consequence:} Since $R^{\mathrm{reg}} \in L^p_{\text{loc}}$ for all $p < \infty$, standard elliptic regularity gives $\phi \in W^{2,p}_{\text{loc}} \hookrightarrow C^{1,\alpha}$. The Dirac mass appears only in the final geometric scalar curvature $R_{\tg}$ of the conformal metric $\tg = \phi^4\bar{g}$, where it contributes a \emph{nonnegative measure} (since $[H] \ge 0$ by stability) that aids the AMO monotonicity.
\end{tcolorbox}
