...
\begin{proof}
See Appendix~\ref{app:InternalSmoothing}. We establish $\|R^-_\epsilon\|_{L^1} \le C\epsilon$ and $\|R^-_\epsilon\|_{L^2} \le C\epsilon^{1/2}$. Interpolation via H\"older's inequality yields the result. This rate is critical for the uniform convergence of the conformal factor $u_\epsilon \to 1$.
\end{proof}
...
\[
    \partial_s \gamma(0^\pm,y) = -2 h^\pm(y), \qquad [H]=H^+-H^- .
\]
The \(+\) and \(-\) labels are the global exterior/interior labels from the sign convention section, not merely the sign of the local coordinate \(s\). In the direct MOTS proof the favorable-jump hypothesis gives \([H]\ge0\); if a strictly positive lower bound \([H]\ge c_0>0\) is available, the positive spike below dominates pointwise in the center of the collar.
...
\begin{equation}\label{eq:QuantScalar}
    R_{\hat{g}_\epsilon}(s,y) = 2[H] \, \rho_\epsilon(s) + E_\epsilon(s,y), \qquad |E_\epsilon(s,y)| \le C.
\end{equation}
Consequently
\begin{equation}\label{eq:PointwiseLower}
    R_{\hat{g}_\epsilon}(s,y) \ge - C \quad \text{on } N_{2\epsilon}.
\end{equation}
\end{proposition}
...
\[
    \partial_s H_\epsilon = -[H] \, \rho_\epsilon(s) + \mathcal{R}_\epsilon(s,y), \qquad \|\mathcal{R}_\epsilon\|_{C^0} \le C.
\]
Substituting this expression in \eqref{eq:FermiScalar} shows that the leading distributional contribution is the positive spike $2[H]\rho_\epsilon$, while the remainder collects the terms $R_{\gamma_\epsilon} - |A_\epsilon|^2 - H_\epsilon^2 - 2\mathcal{R}_\epsilon$. These terms are uniformly bounded by the one-sided smooth geometry and the Lipschitz control on $\gamma$ (cf. Miao~\cite[Prop.~3.1]{miao2002}). This proves \eqref{eq:QuantScalar} and the stated lower bound. The smallness needed later comes from the collar volume \(O(\epsilon)\), not from a pointwise \(o(1)\) bound on the raw scalar curvature.
\end{proof}
...
\]
where $2[H] \rho_\epsilon(s) \ge 0$ under the favorable-jump hypothesis $[H] \ge 0$, and $|E_\epsilon(s,y)| \le C$.
...
In the positive-jump case ($[H] > 0$), the spike $2[H]\rho_\epsilon(s) \sim [H]/\epsilon$ for $|s| < \epsilon$ dominates the bounded error $E_\epsilon$, so $R_{\hat{g}_\epsilon}^- = 0$ in most of the collar.

In the zero-jump case (when $[H] = 0$ is known separately), the negative part satisfies $|R_{\hat{g}_\epsilon}^-| \le C$ pointwise.
...
\]
The principal eigenvalue is $\mu_0 = 1/4$ (corresponding to $\ell=0$), giving the indicial root $\alpha = \sqrt{1/4} = 1/2$.
...
\end{lemma}
\begin{proof}
Recall $Y = \frac{(\phi-1)^2}{\phi} \nabla \phi + \frac{1}{4}(\phi-1)^2 q$. By Lemma~\ref{lem:InterfaceRegularity}, the solution of the Lichnerowicz equation satisfies $\phi \in C^{1,\alpha}$ across $\Sigma$, so the first term has matching one-sided normal traces.

For the second term, the manuscript does not use a separate pointwise continuity statement for $q$ across $\Sigma$. Instead, one integrates the weak Lichnerowicz equation on the two sides of a shrinking collar around $\Sigma$. Because the PDE potential contains no delta mass at $\Sigma$, the weak transmission condition identifies the one-sided fluxes and shows that the Bray--Khuri vector field produces no additional interface measure. Equivalently, the two one-sided normal traces of $\langle Y,\nu\rangle$ agree in the weak sense.

Hence the interface contribution vanishes:
\begin{equation}
    \Jump{\langle Y,\nu\rangle}=0.
\end{equation}
...
We substitute $V = \frac{1}{8}\mathcal{S} - \frac{1}{4}\Div_{\bg}(q)$ (cf.\ Remark~\ref{rem:SignConventionsSummary} for sign conventions). Integrating the divergence term by parts (boundary terms vanish):
...
Since $\mathcal{S} \ge 0$ pointwise, the operator $L$ satisfies the maximum principle (recall the sign convention from Remark~\ref{rem:SignConventionsSummary}). Specifically, the associated quadratic form is:
...
\textit{Boundary (b): Lipschitz interface $\Sigma$.} By Lemma~\ref{lem:Transmission}, the conformal factor $\phi$ is $C^{1,\alpha}$ across $\Sigma$. The corresponding Bray--Khuri flux has no interface jump by Lemma~\ref{lem:TransmissionGlobal}:
\begin{equation}
...
\textbf{Proof of (2):} By the transmission Lemma~\ref{lem:Transmission}, $\phi \in C^{1,\alpha}(\overline{M})$ across $\Sigma$. The required cancellation of the interface term for the Bray--Khuri vector field is precisely Lemma~\ref{lem:TransmissionGlobal}. Hence
\[
    [Y \cdot \nu]_\Sigma = 0.
\]
...
\begin{remark}[Distinction Between H\"older Exponent and $L^\infty$ Gradient Bound as $p \to 1^+$]\label{rem:HolderVsGradient}
...
\textbf{(I) The H\"older regularity exponent $\alpha_H(p)$:} The Tolksdorf--DiBenedetto theory provides $u_p \in C^{1,\alpha_H}$ locally, where the H\"older exponent $\alpha_H$ depends on $p$ and \emph{may degenerate} as $p \to 1^+$. Specifically:
\[
...
\textbf{(III) Why this distinction matters:} The double limit argument requires uniform \emph{$L^\infty$ gradient bounds} to control the energy integrals, not uniform H\"older exponents. The key estimate
\[
    |E_{p,\epsilon} - E_p| \le C \epsilon^{1/2}
\]
uses only the bound $\|\nabla u_p\|_{L^\infty} \le C_\nabla$ (Lemma~\ref{lem:TolksdorfUniformity}), which holds uniformly in $p$. The degeneration of the H\"older exponent $\alpha_H(p) \to 0$ does not affect this estimate.
...
\textbf{Critical clarification:} The degeneration $\alpha_H \to 0$ does \textbf{not} affect our argument. What matters is the uniform $L^\infty$ gradient bound in Part (A), which remains stable as $p \to 1^+$. The H\"older exponent controls higher-order oscillations, not the $L^\infty$ norm of the gradient.
...
\textbf{Part (II):} This follows from the AMO identification theorem. The convergence rate $(p-1)^{1/2}$ comes from the BV convergence of $p$-harmonic functions to the 1-harmonic (IMCF) limit and the H\"older continuity of the associated geometric quantities.
...
\begin{enumerate}
    \item[(a)] The transmission condition $[H]_{\tg} \ge 0$ (assumed via favorable jump);
...
\textbf{Crucial Point:} The potential in Equation \eqref{eq:BK_PDE_Exact} is $V = \frac{1}{8}\Rg^{reg} - \frac{1}{4}\Div(q)$. Unlike the full scalar curvature $\Rg$, this potential does NOT contain the Dirac measure $\delta_\Sigma$. The manuscript uses $\Div(q)$ only through its bulk regularity and weak transmission formulation, not through a separate pointwise continuity statement for $q$ at $\Sigma$. Together with the local boundedness of $\Rg^{reg}$ away from the cylindrical ends, this gives $V \in L^p_{\text{loc}}$ for appropriate $p > 3/2$. (On the cylindrical ends, $V$ decays like $O(t^{-4})$ and thus belongs to the weighted spaces $L^2_\beta$ discussed in Section~\ref{sec:Fredholm}.)
\end{lemma}
...
More precisely, the weak formulation on the two sides of $\Sigma$ yields matching conormal traces across the interface, because the potential $V$ has no delta contribution there. Standard transmission regularity for uniformly elliptic equations with Lipschitz coefficients then gives the claimed $C^{1,\alpha}_{\mathrm{loc}}$ regularity across $\Sigma$.
\end{proof}
...
Since $R^{\mathrm{reg}} \in L^p_{\text{loc}}$ for all $p < \infty$, standard elliptic regularity gives $\phi \in W^{2,p}_{\text{loc}} \hookrightarrow C^{1,\alpha}$. The Dirac mass appears only in the final geometric scalar curvature $R_{\tg}$ of the conformal metric $\tg = \phi^4\bar{g}$, where it contributes a nonnegative measure under the favorable-jump hypothesis $[H] \ge 0$, which is the sign condition used in the AMO monotonicity argument.
\end{remark}
...
\begin{enumerate}
    \item \textbf{Incorrect interpretation:} If the PDE potential were $V \in L^{3/2}_{\text{loc}}$ only (borderline case), then $\phi \in W^{2,3/2}_{\text{loc}}$, and the Sobolev embedding $W^{2,3/2} \hookrightarrow C^{0,\alpha}$ would give only H\"older continuity, not $C^1$ regularity.
...
\begin{enumerate}
    \item \textbf{The Lichnerowicz equation:} We solve $\Delta_{\bg}\phi - \frac{1}{8}R_{\bg}\phi = \frac{1}{4}\Div(q)\phi$. Formally rearranging using $R_{\bg} = \mathcal{S} - 2\Div(q) + 2[H]\delta_\Sigma$ gives
    \[
        \Delta_{\bg}\phi = \frac{1}{8}(\mathcal{S} - 2\Div(q) + 2[H]\delta_\Sigma)\phi + \frac{1}{4}\Div(q)\phi = \frac{1}{8}\mathcal{S}\phi + \frac{1}{4}[H]\delta_\Sigma \cdot \phi.
    \]
    
    \item \textbf{Why the Dirac mass does not appear in the PDE:} The displayed identity is only formal. The rigorous formulation used in the paper solves the equation on $\tM \setminus \Sigma$ with the regular potential and enforces the interface behavior through the weak transmission condition. In particular, one does \emph{not} deduce $[\partial_\nu\phi]_\Sigma=0$ from the sign of $[H]$ or from the fact that $\Sigma$ has measure zero; rather, continuity of the normal flux is part of the weak formulation across the Lipschitz interface.
    
    \item \textbf{Result:} The conformal factor $\phi$ satisfies a uniformly elliptic PDE with $L^p_{loc}$ coefficients, yielding $\phi \in C^{1,\alpha}$. The geometric scalar curvature $R_{\tg}$ of the final metric $\tg = \phi^4\bg$ \emph{does} include the distributional contribution from $\Sigma$, and under the favorable-jump hypothesis this contribution is a nonnegative measure that contributes favorably to the AMO monotonicity.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{The Dirac measure does not enter the PDE potential.} The Lichnerowicz equation~\eqref{eq:BK_PDE_Exact} is solved in the weak sense on $\tM \setminus \Sigma$, with transmission conditions at $\Sigma$. The potential $V = \frac{1}{8}R^{reg} - \frac{1}{4}\Div(q)$ does \emph{not} include the singular term $2[H]\delta_\Sigma$. This is because the Dirac mass contributes to the geometric scalar-curvature measure, not to the bulk coefficient of the elliptic equation. The transmission condition $[\partial_\nu \phi]_\Sigma = 0$ is imposed by the weak formulation across the Lipschitz interface; it does not follow merely from the fact that $\Sigma$ has measure zero.
...
Let $Y$ be any vector field of the form $Y=F(\phi,q)$ used in the Bray--Khuri divergence identity. For the specific Bray--Khuri flux considered in this manuscript, the weak transmission argument above shows that $Y$ has matching normal components on both sides of $\Sigma$.
...
As $\epsilon \to 0$:
\begin{equation}
    \frac{\epsilon^{-(3-p)}}{(\log(1/\epsilon))^p} \to \infty.
\end{equation}
Therefore the logarithmic competitor is \emph{not} suitable for $1<p<3$ (it is the correct borderline choice for $p=n$). We instead use a power-law competitor.
...
\textbf{Step 5 (Combined estimate):} Combining Steps 3-4 via H\"older's inequality with exponents $(p, q, r)$ where $1/p + 1/q + 1/r = 1$:
\begin{align}
...
\]
The first term converges to zero in $L^p$ by the same volume argument. For the second term, by H\"older's inequality with exponents $(p/(p-1), p)$:
\[
...
\textbf{(4) Scalar Curvature Control:} This is Corollary~\ref{cor:L32}. The explicit computation in the collar gives $R_{g_\epsilon} = 2[H]\rho_\epsilon(s) + E_\epsilon$ where $[H] \ge 0$ under the favorable-jump hypothesis and $|E_\epsilon| \le C\epsilon^{1/2}$. The positive spike $2[H]\rho_\epsilon$ integrates to $2[H] > 0$ when the jump is strictly positive, while the error term satisfies $\|E_\epsilon\|_{L^{3/2}} \le C'\epsilon^{1/2}$.
...
\begin{equation}
    \Scal_{\tg} = \Scal_{\tg}^{reg} + 2[H]_{\tg}\,\delta_\Sigma.
\end{equation}
Assume the interface coefficient is nonnegative, i.e. $[H]_{\tg} \ge 0$; in the direct MOTS argument this is provided by the favorable-jump hypothesis. Then the singular contribution to $\Scal_{\tg}$ is a nonnegative measure.
...
Using H\"older's inequality and the Sobolev inequality ($n=3$) with $L^2$ norms (noting $L^2 \subset L^{3/2}$ on compact domains, but we proceed with the stronger norm):
...
If one additionally knows that the interface has zero jump, then the distributional scalar curvature has no positive delta mass at $\Sigma$. This zero-jump statement is not being derived here from marginal stability alone.
...
If the outermost MOTS $\Sigma$ is marginally stable ($\lambda_1(L_\Sigma)=0$), one expects a softer interface regime than in the strictly positive-jump case. However, the present paper does \emph{not} use marginal stability alone to deduce an exact value of the jump. The rigorous sign input remains the explicit favorable-jump hypothesis in the direct MOTS theorem. When $[H]=0$ is known by separate argument, the smoothing procedure at the interface simplifies substantially.
\end{remark}
...
When the interface satisfies the \emph{explicit} positive jump condition $[H] > 0$, the mollification produces a large positive scalar curvature term $2[H]/\epsilon$ which dominates the $O(1)$ error terms arising from tangential variations and the smoothing error. This positivity is useful as a buffer in the smoothing argument. The manuscript no longer claims that strict stability \emph{by itself} yields $[H] > 0$.
\end{remark}

\begin{lemma}[Quantitative Spike Domination in the Strictly Stable Case]\label{lem:SpikeDomination}
Let $\Sigma$ be a MOTS satisfying the positive jump hypothesis $[H] > 0$. Assume in addition a spectral lower bound $\lambda_1(L_\Sigma) \ge \lambda_0 > 0$ as a separate geometric input controlling the collar geometry. For the smoothed metric $\hat{g}_\epsilon$ constructed via collar mollification:
\begin{enumerate}
    ...
    \item[(iv)] Pointwise domination in the central collar: If $[H] \ge [H]_{\min} > 0$, then for $\epsilon$ sufficiently small the positive spike dominates on every subcollar where the mollifier has a fixed positive lower bound. Globally in $N_{2\epsilon}$ one retains the uniform estimate
    \[
        R_{\hat{g}_\epsilon}(s, y) \ge -C_Q,
    \]
    hence $\|R^-_{\hat g_\epsilon}\|_{L^{3/2}(N_{2\epsilon})}\le C\epsilon^{2/3}$.
\end{enumerate}
\end{lemma}
...
\]
The spectral bound $\lambda_1 \ge \lambda_0$ is relevant for the stability of the interface geometry, but the sign of $[H]$ is controlled independently by the favorable jump condition $[H]=\tr_\Sigma k \ge 0$.
...
\textbf{Part (iv):} At any point $(s, y)$ in a fixed central subcollar where $\rho(s/\epsilon)\ge c_\rho>0$:
\[
    R_{\hat{g}_\epsilon} \ge S^+_\epsilon - |Q_\epsilon| \ge \frac{2[H]_{\min}c_\rho}{\epsilon} - C_Q.
\]
This is positive for $\epsilon$ sufficiently small. Near the cutoff boundary the mollifier may be small, so the proof only uses the global lower bound $R_{\hat g_\epsilon}\ge -C_Q$ on a set of volume $O(\epsilon)$. This yields the stated $L^{3/2}$ control of the negative part; pointwise nonnegative scalar curvature is imposed only after the conformal correction.
\end{proof}

\begin{remark}[Transition from Strictly Stable to Marginally Stable: Detailed Analysis]\label{rmk:MarginalTransition}
A natural concern arises regarding the limit $\lambda_1 \to 0$: the strictly stable case relies on a positive curvature ``spike'' from the mean curvature jump $[H] > 0$, which dominates the quadratic errors in the smoothing. In a separate zero-jump regime, this spike vanishes. We address this transition carefully:

\textbf{(i) Boundedness of the Quadratic Deficit:} The key observation is that when one passes to a separately assumed zero-jump regime, the positive spike $2[H]/\epsilon$ disappears, while the \emph{quadratic error terms} $Q_\epsilon$ (arising from nonlinear commutators of mollification and curvature) remain \textbf{uniformly bounded} independently of the stability parameter. Explicitly, in Fermi coordinates the error satisfies
\[
...
\textbf{(ii) Continuity of the Corner Regularity:} As one approaches a zero-jump regime, the corner weakens and the interface approaches a $C^1$ profile. Geometrically, this means the ``spike'' in the distributional curvature shrinks. The manuscript does not assert that this happens solely because $\lambda_1 \to 0$; it is a separate zero-jump assumption in this discussion.

\textbf{(iii) The $L^{3/2}$ Estimate Degrades Gracefully:} In the positive-jump case ($[H] > 0$), the positive contribution from the mollified delta function reduces the negative part of the raw smoothing. In the zero-jump case, there is no positive contribution, but the negative part still satisfies the same smallness bound:
\[
    \|R^-_{\hat{g}_\epsilon}\|_{L^{3/2}(N_{2\epsilon})} \le C \epsilon^{2/3}.
\]
The only difference is that for $[H] > 0$, the constant $C$ can be taken smaller (the positive spike provides extra margin). In the zero-jump case, the bound is tight but still sufficient for the conformal correction.
...
\end{tikzpicture}
\caption{Profile of the scalar curvature during smoothing. In a separate zero-jump case (blue curve), the Dirac mass $\frac{2}{\epsilon}[H]$ disappears, revealing the bounded quadratic deficit $D_\epsilon$. Because the deficit is $O(1)$ on a collar of thickness $O(\epsilon)$, its $L^{3/2}$ norm decays like $\epsilon^{2/3}$.}
