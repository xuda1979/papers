...
\[
    \partial_s \gamma(0^\pm,y) = -2 h^\pm(y), \qquad H^{\mathrm{bulk}} := \tr_{\gamma_0} h^-(y), \qquad H^{\mathrm{cyl}} := \tr_{\gamma_0} h^+(y), \qquad [H] := H^{\mathrm{bulk}} - H^{\mathrm{cyl}} > c_0.
\]
...
\textbf{(iii) The $L^{3/2}$ Estimate Degrades Gracefully:} In the positive-jump case ($[H] > 0$), the positive contribution from the mollified delta function \emph{exceeds} the negative quadratic error, giving $R_{\hat{g}_\epsilon} \ge 0$ in an averaged sense. In the zero-jump case, there is no positive contribution, but the negative part satisfies the same bound:
\[
    \|R^-_{\hat{g}_\epsilon}\|_{L^{3/2}(N_{2\epsilon})} \le C \epsilon^{2/3}.
\]
