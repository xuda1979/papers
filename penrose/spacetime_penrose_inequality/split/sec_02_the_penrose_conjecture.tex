...
\begin{itemize}
    \item[(A)] \textbf{Favorable jump:} $\tr_{\Sigma_0} k \ge 0$ \emph{pointwise}, or
    \item[(B)] \textbf{Compactness plus sign calibration:} One of conditions (C1)--(C3) of Theorem~\ref{thm:MaxAreaTrapped} holds, together with an additional calibration converting the adjoint-cone condition of Theorem~\ref{thm:AdjointConeEuler} into the pointwise favorable jump $\tr_{\Sigma_{\max}}k\ge0$, or
...
\textbf{Proof B: Two-Stage Reduction (Requires Lorentzian Area Comparison).}
\begin{itemize}
    \item \textbf{Stage A (Lorentzian Area Comparison):} By Theorem~\ref{thm:HAD}, if $\Sigma^*$ is the relevant horizon cross-section associated with $\Sigma_0$, then $A(\Sigma^*) \ge A(\Sigma_0)$.
    \item \textbf{Stage B (MOTS Penrose):} For the outermost MOTS $\Sigma^*$, assuming the favorable jump condition $[H] \ge 0$ (which corresponds to $\tr_{\Sigma^*} k \ge 0$), the Jang-based argument applies directly.
...
\begin{enumerate}
    ...
    \item \textbf{Compactness (Theorem~\ref{thm:MaxAreaTrapped}):} Under conditions (C1)--(C3), there exists a maximum area trapped surface $\Sigma_{\max}$ with $A(\Sigma_{\max}) \ge A(\Sigma_0)$. The available first-variation output is the adjoint-cone condition of Theorem~\ref{thm:AdjointConeEuler}; converting it into the pointwise favorable jump remains open for $k \neq 0$ without an additional calibration.
\end{enumerate}

\textbf{Open Problem:} 
\begin{itemize}
    \item Find a geometric calibration that upgrades the adjoint-cone condition of Theorem~\ref{thm:AdjointConeEuler} to the pointwise sign $\tr_\Sigma k \ge 0$ for non-time-symmetric data.
...
\begin{enumerate}
    ...
    \item[\textbf{Stage 3:}] \textbf{Corner Smoothing.} The conformally sealed metric $\tg = \phi^4 \bar{g}$ is only Lipschitz across the outermost MOTS $\Sigma^*$. Assuming $\Sigma^*$ satisfies the \textbf{favorable jump condition}, the mean curvature jump $[H] \ge 0$ holds. We apply Miao's corner-smoothing technique to produce smooth approximants $\hat{g}_\epsilon$ with $R_{\hat{g}_\epsilon} \ge 0$.
...
\noindent\textit{$^*$Applies to dynamical Kerr formation and perturbations of subextremal Kerr. $^\dagger$Sharp constant for outermost MOTS; extension to arbitrary trapped surfaces requires compactness (C1)--(C3) or a Lorentzian area-comparison input. $^\ddagger$Extension to $\tau \in (1/2, 1]$ via harmonic coordinates (Remark~\ref{rem:BorderlineDecayResolution}).}
...
\xrightarrow{\text{smooth}} (\tM,\hat{g}_\epsilon)
\]
\end{center}
\begin{itemize}
...
\begin{itemize}
    \item \textbf{Positive-jump case ($[H] > 0$):} The mollified mean curvature jump produces a positive spike $\frac{2[H]}{\epsilon}\eta(s/\epsilon)$ that dominates the $O(1)$ quadratic error terms. Thus $R_{\hat{g}_\epsilon} \ge 0$ \emph{pointwise} everywhere.
...
\medskip
The Penrose inequality proved here applies to closed trapped surfaces under one of the following conditions: (i) the direct MOTS regime, meaning the surface is a MOTS with favorable jump $\tr_\Sigma k \ge 0$, (ii) compactness conditions (C1)--(C3), or (iii) an additional Lorentzian area-comparison input motivated by cosmic censorship. For MOTS, the direct theorem is Theorem~\ref{thm:penroseinitial}, which still requires the favorable jump hypothesis.
...
We can now state the main theorem precisely. The theorem requires one of the following conditions: (i) favorable jump, (ii) compactness, or (iii) Lorentzian area comparison.
...
Let $(M, g, k)$ satisfy asymptotic flatness ($\tau > 1$) and DEC. Let $\Sigma_0$ be a closed trapped surface ($\theta^+ \le 0$, $\theta^- < 0$). Under (A) the direct MOTS regime (namely $\Sigma_0$ is a MOTS with favorable jump), (B) compactness, or (C) Lorentzian area comparison:
