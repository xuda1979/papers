...
\label{sec:penrose_conjecture}

\begin{remark}[Section Organization and Theorem Relationships]
This section provides detailed theorem statements and proof strategies. For the executive summary of results (what is proved unconditionally vs.\ conditionally), see Section~\ref{sec:intro}.

\textbf{Guide to theorem numbering:} The main results appear in several equivalent forms throughout the paper:
\begin{itemize}
    \item \textbf{Theorem A (Stable MOTS):} For outermost \emph{stable} MOTS --- see Theorem~\ref{thm:intro-main} (Introduction), Theorem~\ref{thm:penroseinitial} (summary form), and Theorem~\ref{thm:SPI} (detailed form below).
    \item \textbf{Theorem B (Conditional):} For general trapped surfaces --- see Theorem~\ref{thm:intro-conditional} (Introduction), Theorem~\ref{thm:MainTheorem} (detailed form below), and Theorem~\ref{thm:CompleteProof} (consolidated proof in Section~\ref{sec:Consolidated}).
\end{itemize}
\textbf{Key distinction:} Theorem~\ref{thm:penroseinitial}/\ref{thm:SPI} applies to MOTS satisfying the \textbf{favorable jump condition} $\tr_\Sigma k \ge 0$; for the outermost MOTS, stability is automatic by Andersson--Metzger but the favorable jump condition remains an explicit hypothesis. Theorem~\ref{thm:MainTheorem}/\ref{thm:CompleteProof} addresses general trapped surfaces and requires either: (A) reduction to a MOTS in the direct favorable-jump regime, (B) compactness plus sign calibration, or (C) a separate Lorentzian area-comparison input motivated by cosmic censorship.
\end{remark}
...
\textbf{Results proved here:} We prove the following conditional spacetime Penrose inequalities via the p-harmonic level set method combined with the Generalized Jang equation:
\begin{itemize}
    \item \textbf{Conditional for outermost stable MOTS} (Theorem~\ref{thm:penroseinitial}/\ref{thm:SPI}): For the apparent horizon $\Sigma^*$ that is \textbf{stable} ($\lambda_1(L_{\Sigma^*}) \ge 0$), the inequality holds provided the \textbf{favorable jump condition} $\tr_{\Sigma^*} k \ge 0$ is satisfied.
    \item \textbf{Conditional for general trapped surfaces} (Theorem~\ref{thm:MainTheorem}/\ref{thm:CompleteProof}): For arbitrary trapped surfaces, one needs either (A) the direct MOTS regime (the given surface is a MOTS with $\tr_\Sigma k \ge 0$ pointwise), (B) compactness (C1)--(C3) plus an additional pointwise favorable-jump calibration for $k \neq 0$, or (C) cosmic censorship.
\end{itemize}

\textbf{Note:} Condition (C) is a strengthened Lorentzian version of Penrose's 1973 intuition: weak cosmic censorship together with the needed trapped-surface/event-horizon area comparison. Our Theorem~\ref{thm:penroseinitial} provides a different sharp result for MOTS with favorable jump, without requiring cosmic censorship.
...
\begin{enumerate}
    \item[\textup{(AF)}] \textbf{Asymptotic flatness:} Definition~\ref{def:GlobalAF} with decay rate $\tau > 1/2$ (typically $\tau=1$), including all derivative bounds (AF1)--(AF5) and the constraint equations (AF6).
...
\begin{itemize}
    \item[(A)] \textbf{Direct favorable-jump MOTS regime:} $\Sigma_0$ is a MOTS and $\tr_{\Sigma_0} k \ge 0$ \emph{pointwise}, or
    \item[(B)] \textbf{Compactness plus sign calibration:} Conditions (C1)--(C3) of Theorem~\ref{thm:MaxAreaTrapped} hold, together with an additional calibration converting the adjoint-cone condition of Theorem~\ref{thm:AdjointConeEuler} into the pointwise favorable jump $\tr_{\Sigma_{\max}}k\ge0$, or
    \item[(C)] \textbf{Lorentzian area comparison:} The data embeds in a globally hyperbolic spacetime satisfying weak cosmic censorship and the area-comparison input of Theorem~\ref{thm:HAD},
\end{itemize}
Then:
\begin{equation}
...
\textbf{Proof A: Maximum Area Trapped Surface (Requires Compactness (C1)--(C3) plus $k=0$ or sign calibration).}
\begin{itemize}
    \item By Theorem~\ref{thm:MaxAreaTrapped}, under compactness conditions the area-maximizing trapped surface $\Sigma_{\max}$ is a MOTS. If it is strictly stable, the first variation gives the adjoint-cone condition of Theorem~\ref{thm:AdjointConeEuler}.
    \item \textbf{Critical step:} The Jang method requires \emph{pointwise} $\tr_{\Sigma_{\max}} k \ge 0$. Converting the adjoint-cone condition into that pointwise sign is:
    \begin{itemize}
        \item \textbf{Automatic for $k = 0$} (Theorem~\ref{thm:IntegralToPointwise}).
...
\textbf{Proof B: Direct Lorentzian Route (Requires Lorentzian Area Comparison).}
\begin{itemize}
    \item \textbf{Stage A (Lorentzian Area Comparison):} By Theorem~\ref{thm:HAD}, the trapped surface area is bounded by the relevant event-horizon cross-section area.
    \item \textbf{Stage B (Hawking--Kerr Mass Bound):} Theorem~\ref{thm:Penrose1973Complete} combines that comparison with Hawking's area theorem, the Kerr final-state bound, and Bondi mass loss.
    \item \textbf{Conclusion:} $M_{\mathrm{ADM}} \ge \sqrt{A(\Sigma_0)/(16\pi)}$ under the explicit Lorentzian hypotheses.
\end{itemize}
\end{theorem}

\noindent\emph{In words:} Under the stated conditions, \textbf{every} closed trapped surface has area bounded above by $16\pi M_{\mathrm{ADM}}^2$, as conjectured by Penrose. See Remark~\ref{rem:ProofStatus} below for the complete status of each case.
...
\begin{enumerate}
    \item \textbf{MOTS with favorable jump ($\tr_\Sigma k \geq 0$):} Direct Jang applies.
\end{enumerate}

\textbf{Conditional on compactness (C1)--(C3) for:}
\begin{enumerate}
    \setcounter{enumi}{2}
    \item \textbf{Trapped surfaces with unfavorable jump ($\tr_\Sigma k < 0$):} Requires area comparison or a new calibration mechanism. The adjoint-cone Euler condition is proved in Theorem~\ref{thm:AdjointConeEuler}, but it does not by itself imply pointwise favorable jump.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Compactness (Theorem~\ref{thm:MaxAreaTrapped}):} Under conditions (C1)--(C3), there exists a maximum-area trapped surface $\Sigma_{\max}$ with $A(\Sigma_{\max}) \ge A(\Sigma_0)$. The available first-variation output is the adjoint-cone condition of Theorem~\ref{thm:AdjointConeEuler}. Theorem~\ref{thm:NoGoVariational} shows that this condition alone does not imply the pointwise favorable jump when $k\neq0$.
\end{enumerate}

\textbf{Precise status of the gap:}
\begin{itemize}
    \item The time-symmetric case has automatic favorable jump, since $k=0$ gives $\tr_\Sigma k=0$ (Theorem~\ref{thm:IntegralToPointwise}).
    \item For non-time-symmetric data, an additional geometric calibration or a separate area-comparison input is still required to pass from the adjoint-cone condition to the pointwise sign $\tr_\Sigma k\ge0$.
\end{itemize}
\end{remark}
...
\begin{enumerate}
    \item[\textbf{Stage 1:}] \textbf{Direct Jang Construction.} Given a MOTS $\Sigma_0$ satisfying $\theta^+ = 0$, $\theta^- < 0$, and the \textbf{favorable jump condition} $\tr_{\Sigma_0} k \ge 0$, we solve the generalized Jang equation on $(M,g,k)$ with blow-up forced at $\Sigma_0$ (Theorem~\ref{thm:DirectTrappedJang}). The MOTS condition is essential here: it is what makes the cylindrical blow-up model compatible with the Jang equation. This produces a Jang metric $\bar{g}$ with $R_{\bar{g}} \ge 0$ (from DEC) and a cylindrical end over $\Sigma_0$. For arbitrary trapped surfaces one must first pass to a suitable enclosing MOTS or use a separate area-comparison input.
    
    \textbf{Note on the favorable jump condition:} This is an additional hypothesis that ensures the corner smoothing preserves $R \ge 0$. It is not automatic for stable MOTS.
    ...
    \item[\textbf{Stage 3:}] \textbf{Corner Smoothing.} The conformally sealed metric $\tg = \phi^4 \bar{g}$ is only Lipschitz across the outermost MOTS $\Sigma^*$. Assuming $\Sigma^*$ satisfies the \textbf{favorable jump condition}, the mean curvature jump $[H] \ge 0$ holds. We first mollify the corner to obtain raw metrics $\hat{g}_\epsilon$ with controlled negative scalar-curvature part, and then apply the conformal correction to obtain smooth metrics $\geps$ with $R_{\geps} \ge 0$.
...
\item \textbf{Smoothing (Appendix~\ref{app:InternalSmoothing}):} The metric $\tg$ is only Lipschitz continuous across the original horizon location. We mollify it into raw smooth metrics $\hat{g}_\epsilon$ with $\|R^-_{\hat{g}_\epsilon}\|_{L^{3/2}}\to0$, then conformally correct to metrics $\geps$ with $R_{\geps}\ge0$. The mass and horizon area are stable under this smoothing-correction procedure.

\item \textbf{Running the AMO flow (Section~\ref{sec:AMO}):} On each corrected smooth approximant $(\tilde{M}, \geps)$, we consider a family of $p$-harmonic functions $u_p$ (for $1 < p < 3$) that equal 0 on the horizon and approach 1 at infinity. As $p \to 1^+$, the level sets of $u_p$ behave like inverse mean curvature flow (IMCF), and a monotonicity formula yields:
\[
M_{\mathrm{ADM}}(\geps) \ge \sqrt{\frac{A(\Sigma)}{16\pi}}.
\]
...
The technical heart of the paper lies in justifying each ``$\ge$'' in the presence of low regularity: Lipschitz metrics, measure-valued curvature, and singular limits. The key innovations are (i) verifying that the AMO monotonicity extends to distributional curvature, (ii) identifying the interface sign condition needed for corner smoothing and tracking it through the Jang construction, and (iii) rigorously interchanging the double limit $(p, \epsilon) \to (1^+, 0)$.
...
The geometric contributions include: the precise jump identity $[H]_{\bar{g}}=\tr_\Sigma k$ together with the favorable-jump criterion used in the rigorous core argument (Theorem~\ref{thm:CompleteMeanCurvatureJump}); and characterization of the equality case via static vacuum bootstrap and Bunting--Masood-ul-Alam uniqueness.
...
\begin{itemize}
    ...
    \item $\tr_{\Sigma_0} k \ge 0$ \textbf{(favorable trace condition)}.
\end{itemize}
No restriction is placed on stability, outermost position, or topology. The Jang Reduction for MOTS (Theorem~\ref{thm:DirectTrappedJang}) handles such surfaces directly without reduction to outermost MOTS.

\textbf{Important:} The favorable jump condition does \textbf{not} follow from $\theta^\pm$ alone (see Lemma~\ref{lem:TrappedMeanCurvatureJump}). It is an independent hypothesis.
\end{assumption}
...
\begin{enumerate}
    ...
    \item Favorable jump condition implies $[H] \ge 0$;
    \item Apply the Jang-based proof to get $M_{\mathrm{ADM}} \ge \sqrt{A(\Sigma^*)/(16\pi)}$.
\end{enumerate}

\textbf{Conclusion:} Under compactness conditions (C1)--(C3) plus the favorable-jump sign input for the comparison MOTS, $M_{\mathrm{ADM}} \ge \sqrt{A(\Sigma^*)/(16\pi)} \ge \sqrt{A(\Sigma_0)/(16\pi)}$ for the trapped surfaces covered by the reduction.
...
\textbf{Distinction from Penrose 1973:} This remark concerns comparison to the \emph{outermost MOTS} $\Sigma^*$. Penrose's original 1973 argument compares to the \emph{event horizon} $\mathcal{H}_\mathcal{C}$, which is a different surface. Theorem~\ref{thm:Penrose1973Complete} records that spacetime route only after explicitly assuming the required event-horizon area comparison.
\end{remark}
...
\begin{enumerate}[(RW1)]
    ...
    \end{itemize}
    \item \textbf{Suboptimal Constant Result.} Most relevantly, Allen, Bryden, Kazaras, and Khuri \cite{allenbrydenkazaraskhuri2025} recently established a spacetime Penrose-type inequality with a \emph{suboptimal} constant $C < 1$:
    \[
    M_{\mathrm{ADM}} \ge C \sqrt{\frac{A(\Sigma)}{16\pi}}, \quad C < 1.
    \]
    Their result holds under general hypotheses (AF, DEC, no symmetry), representing a major breakthrough. \textbf{Key insight:} Their method uses harmonic level sets (rather than $p$-harmonic with $p \to 1^+$), which inherently produces a suboptimal constant. Our approach using AMO $p$-harmonic monotonicity \emph{could} achieve the sharp constant $C = 1$ for MOTS, but requires the favorable jump condition $\tr_\Sigma k \ge 0$.
...
The spacetime Penrose inequality with sharp constant for \emph{arbitrary} trapped surfaces remains open. Allen et al.\ \cite{allenbrydenkazaraskhuri2025} achieved suboptimal constant unconditionally; An--He \cite{anhe2025} achieved sharp constant in dynamical formation settings. Our Theorem~\ref{thm:penroseinitial} achieves sharp constant for MOTS satisfying the favorable jump condition; extension to arbitrary trapped surfaces requires compactness (Theorem~\ref{thm:MaxAreaTrapped}) or an additional Lorentzian area-comparison hypothesis of the type isolated in Program~F.
\end{remark}
...
\midrule
Allen--Bryden--Kazaras--Khuri \cite{allenbrydenkazaraskhuri2025} & \textbf{Suboptimal} & \textbf{None} & $\tau > 1/2$ & Harmonic level sets \\
...
\noindent\textit{$^*$Applies to dynamical Kerr formation and perturbations of subextremal Kerr. $^\dagger$Sharp constant for outermost MOTS; extension to arbitrary trapped surfaces requires compactness (C1)--(C3) plus the favorable-jump sign input for the comparison MOTS, or a Lorentzian area-comparison input. $^\ddagger$Extension to $\tau \in (1/2, 1]$ via harmonic coordinates (Remark~\ref{rem:BorderlineDecayResolution}).}
...
\begin{enumerate}
    ...
    \item \textbf{Interface analysis:} Analysis of the mean curvature jump $[H]_{\bar{g}} \ge 0$ and its relation to the favorable jump condition.
...
The Bray--Khuri program \cite{braykhuri2010} introduced the generalized Jang equation to reduce the spacetime inequality to a Riemannian one. Their framework was conditional on: (i) mean curvature jump positivity $[H]_{\bar{g}} \ge 0$; (ii) the conformal factor bound $\phi \le 1$; and (iii) passage from the Jang metric to the Penrose inequality via classical methods. In our rigorous core, item (i) is handled by the explicit jump identity $[H]_{\bar{g}}=\tr_\Sigma k$ together with the favorable-jump hypothesis in Theorem~\ref{thm:CompleteMeanCurvatureJump}; Theorem~\ref{thm:PhiBound} resolves (ii) via transmission conditions and flux analysis, and the AMO $p$-harmonic method addresses (iii) in a low-regularity setting.
...
First, the Jang equation produces metrics that are only Lipschitz continuous across the MOTS interface, while classical elliptic theory requires at least $C^2$ regularity. We address this via a distributional calculus (Lemma~\ref{lem:Transmission}, Theorem~\ref{thm:DistrBochner}) for Lipschitz metrics with measure-valued curvature. The scalar curvature distribution $\mathcal{R} = R^{reg} + 2[H]\delta_\Sigma$ has nonnegative singular part once the explicit favorable-jump hypothesis $[H] \ge 0$ is imposed; this sign is an input in the rigorous direct theorem, not a consequence of stability alone.

Second, for monotonicity methods to work, one needs $R \ge 0$ in an appropriate sense. However, the Jang scalar curvature satisfies only $R_{\bar{g}} = \mathcal{S} - 2\Div(q)$ where $\mathcal{S} \ge 0$ by DEC but $\Div(q)$ has no definite sign. We require the favorable jump condition to ensure $[H] \ge 0$.
...
\begin{center}
\[
(M,g,k) \xrightarrow{\text{GJE}} (\bM,\bg)
\xrightarrow{\phi^4} (\tM,\tg)
\xrightarrow{\text{smooth + correct}} (\tM,\geps)
\]
\end{center}
\begin{itemize}
...
\item $(\tM, \hat{g}_\epsilon)$ and $(\tM,\geps)$: Raw smoothed metric and conformally corrected metric. \emph{Regularity:} Smooth. \emph{Curvature:} $\|R^-_{\hat{g}_\epsilon}\|_{L^{3/2}} \le C\epsilon^{2/3}$ for the raw mollification, while $R_{\geps}\ge0$ for the corrected metric used in AMO. \emph{Ends:} AF + truncated.
\end{itemize}
\textbf{Key estimates preserved:} $M_{\mathrm{ADM}}(g) \ge M_{\mathrm{ADM}}(\bg) \ge M_{\mathrm{ADM}}(\tg) \approx M_{\mathrm{ADM}}(\geps)$ and $A(\Sigma)$ stable.
\end{minipage}}
\end{center}

\begin{remark}[Clarification: Scalar Curvature of Smoothed Metric]\label{rmk:SmoothedScalarCurvature}
The scalar curvature $R_{\hat{g}_\epsilon}$ of the smoothed metric behaves differently depending on whether the interface has positive jump or vanishing jump:
\begin{itemize}
    \item \textbf{Positive-jump case ($[H] > 0$):} The mollified mean curvature jump produces a positive spike $\frac{2[H]}{\epsilon}\eta(s/\epsilon)$ that dominates the $O(1)$ quadratic error terms in the center of the collar. The corrected metric $\geps$ is the object with pointwise nonnegative scalar curvature used in the AMO step.
    \item \textbf{Zero-jump case (when $[H] = 0$ is known separately):} The positive spike vanishes, but the quadratic error remains bounded: $|R_{\hat{g}_\epsilon}| \le C$ in the collar. The negative part satisfies $\|R^-_{\hat{g}_\epsilon}\|_{L^{3/2}} \le C\epsilon^{2/3} \to 0$.
\end{itemize}
For the AMO method, what matters is that the corrected smooth metrics have \(R_{\geps}\ge0\) and converge back to \(\tg\) with stable mass and area. The \(L^{3/2}\) bound on the raw negative part is the input that makes the conformal correction and the limit arguments small.
\end{remark}
...
\textbf{Claim 3} (Mean Curvature Jump): In the direct MOTS regime, $[H]_{\bar{g}} = \tr_\Sigma k \ge 0$ by the explicit favorable-jump hypothesis.
\begin{itemize}
    \item \emph{Method}: MOTS identity $\theta^+ = H + \tr_\Sigma k = 0$ together with the imposed interface sign condition.
...
\begin{itemize}
    \item \emph{Method}: Corner smoothing $\hat{g}_\epsilon$ with $\|R^-_{\hat g_\epsilon}\|_{L^{3/2}}\to0$, conformal correction to $g_\epsilon$ with $R_{g_\epsilon}\ge0$, then Mosco convergence.
    ...
    \item \emph{Note}: This is the main technical extension beyond \cite{amo2024}.
\end{itemize}
...
\paragraph{Comparison with prior work.} The recent result of Allen--Bryden--Kazaras--Khuri \cite{allenbrydenkazaraskhuri2025} establishes a spacetime Penrose-type inequality with a suboptimal constant:
\[
M_{\mathrm{ADM}} \ge C \sqrt{\frac{A(\Sigma)}{16\pi}}, \quad C < 1.
\]
...
\medskip
The Penrose inequality proved here applies to closed trapped surfaces under one of the following conditions: (i) the direct MOTS regime, meaning the surface is a MOTS with favorable jump $\tr_\Sigma k \ge 0$, (ii) compactness conditions (C1)--(C3) plus an additional sign input giving favorable jump on the comparison MOTS, or (iii) an additional Lorentzian area-comparison input motivated by cosmic censorship. For MOTS, the direct theorem is Theorem~\ref{thm:penroseinitial}, which still requires the favorable jump hypothesis.

Two physical hypotheses are essential:
\begin{enumerate}
    \item[(P1)] \textbf{Dominant Energy Condition (DEC):} $\mu \ge |J|_g$ pointwise. This is required for the Positive Mass Theorem. Without DEC, the ADM mass can be negative (see Schoen--Yau~\cite{schoenyau1981}).
...
\begin{enumerate}
    ...
    \item \textbf{Positive Mass Theorem:} The foundation of the entire argument---that $M_{\mathrm{ADM}} \ge 0$---fails without DEC. Schoen--Yau~\cite{schoenyau1981} construct explicit examples with $\mu < |J|$ having $M_{\mathrm{ADM}} < 0$.
...
\[
    [H]_\Sigma = H^+ - H^-.
\]
Here $H^\pm$ are computed with respect to the normal \emph{pointing into} $\Omega^\pm$. In the direct MOTS argument used later, the rigorous identity is
\[
    [H] = \tr_\Sigma k,
\]
so the sign is controlled by the explicit favorable-jump hypothesis rather than by stability alone.
...
\begin{itemize}
    ...
    \item In the direct MOTS theorem, the favorable jump $[H] = \tr_\Sigma k \ge 0$ is imposed explicitly; it is not inferred from stability alone.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Theorem~\ref{thm:CompleteMeanCurvatureJump}}: Mean curvature jump positivity $[H]_{\bg} \ge 0$ (under favorable jump condition).
...
\emph{Resolution:} The rigorous core proof does not derive the sign of $[H]_{\bar{g}}$ from the stability eigenvalue. Instead it uses the direct MOTS identity $[H]_{\bar{g}} = \tr_\Sigma k$ together with the explicit favorable-jump assumption $\tr_\Sigma k \ge 0$. Spectral expansions in $\lambda_1$ may be heuristic diagnostics, but they are not used to justify the sign in the proof.
...
The scalar curvature of the Jang metric contains a Dirac measure $2[H]\delta_\Sigma$. In the rigorous core argument this term is nonnegative because $[H]=\tr_\Sigma k\ge 0$ under the favorable-jump hypothesis (Theorem~\ref{thm:CompleteMeanCurvatureJump}). The conformal factor $\phi$ solving the Lichnerowicz equation uses only the regular part $V = \frac{1}{8}R^{reg} - \frac{1}{4}\Div(q)$ as potential (Lemma~\ref{lem:InterfaceRegularity}).
...
\begin{itemize}
    ...
    \item \textbf{Stage B (MOTS Penrose):} For stable $\Sigma^*$ satisfying favorable jump, the Jang-based proof applies with $[H] \ge 0$;
...
\textbf{(E3) AMO $p$-Harmonic Level Sets \cite{amo2024}:}
...
\begin{itemize}
    \item \textbf{Published and peer-reviewed:} Han--Khuri \cite{hankhuri2013}, AMO \cite{amo2024}, Miao \cite{miao2002}, Andersson--Metzger \cite{anderssonmetzger2009}, Galloway--Schoen \cite{gallowayschoen2006}, Bray--Khuri \cite{braykhuri2010}, Lockhart--McOwen \cite{lockhartmccowen1985}, Cheeger--Naber--Valtorta \cite{cheegernabervaltorta2015}. These foundational results have undergone peer review and are established in the literature.
    \item \textbf{Preprints (as of 2025):} Allen--Bryden--Kazaras--Khuri \cite{allenbrydenkazaraskhuri2025} (preprint). This preprint establishes a suboptimal-constant Penrose inequality; we cite it for context but our proof does \textbf{not} depend on this result.
\end{itemize}
The logical structure of our proof depends only on the published, peer-reviewed results listed above. The comparison with \cite{allenbrydenkazaraskhuri2025} is provided for context regarding the state of the field, not as a logical dependency.
\end{remark}
...
\begin{itemize}
    ...
    \item \textit{Marginal stability case $\lambda_1 = 0$:} When $\lambda_1 = 0$, the indicial roots are $\alpha \in \{0, -2\}$, which still avoid $(-1, 0)$. Thus the Fredholm discussion at this weight remains unaffected. No additional jump-sign conclusion is being drawn here from a marginal-stability perturbation argument; compare Remark~\ref{rem:EdgeCases}.
\end{itemize}
...
\begin{itemize}
    \item \textit{Potential issue:} The H\"older exponent $\alpha_H(p) \to 0$ as $p \to 1^+$, and constants might blow up.
    ...
    \]
    Only the H\"older exponent degenerates, not the $L^\infty$ bound needed for our estimates.
\end{itemize}

\textbf{(B4) AMO monotonicity at Lipschitz metrics:}
The AMO theorem \cite{amo2024} assumes smooth metrics, but we apply it to smoothed approximants $\hat{g}_\epsilon$:
\begin{itemize}
...
\textbf{(B5) Miao smoothing in the zero-jump interface case:}
Miao's corner smoothing \cite{miao2002} requires $[H] \ge 0$:
\begin{itemize}
    \item \textit{Potential issue:} When $[H] = 0$ exactly, the smoothing construction might degenerate.
    \item \textit{Verification:} In situations where $[H]_{\bar{g}} = 0$ is known by a separate argument, the interface Dirac term in the scalar curvature disappears, so one may work directly with the unsmoothed metric at $\Sigma$. The smoothing procedure in Appendix~\ref{app:InternalSmoothing} handles $[H] > 0$ with explicit bounds on $R_{\hat{g}_\epsilon}$. The manuscript does not use the stronger claim that zero jump by itself upgrades the interface to a specific pointwise regularity class.
\end{itemize}
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}[
        ...
        cert/.style={align=center}
    ]
        \node[stage] (a1) at (0,0) {$(M,g,k)$};
        \node[stage] (a2) at (3,0) {$(\bM,\bg)$};
        \node[stage] (a3) at (6,0) {$(\tM,\tg)$};
        \node[stage] (a4) at (9,0) {$(\tM,\hatgeps)$};
        \node[stage] (a5) at (12,0) {SPI};

        \node[cert] (b1) at (0,-1.8) {DEC+MOTS};
        \node[cert] (b2) at (3,-1.8) {Fredholm};
        \node[cert] (b3) at (6,-1.8) {$\phi \le 1$};
        \node[cert] (b4) at (9,-1.8) {Mosco};
        \node[cert] (b5) at (12,-1.8) {Rigidity};

        \draw[->] (a1) -- node[above] {\text{GJE}} (a2);
        \draw[->] (a2) -- node[above] {$\phi$} (a3);
        \draw[->] (a3) -- node[above] {$\hatgeps$} (a4);
        \draw[->] (a4) -- node[above] {$p$-flow} (a5);
...
The main proof (Sections~\ref{sec:Jang}--\ref{sec:Synthesis}) uses Miao's corner smoothing \cite{miao2002} together with the conformal correction to produce smooth approximants $\geps$ with $R_{\geps} \ge 0$. This is not a circular dependency: Miao's result is an established theorem, and we verify its hypotheses in our setting (Appendix~\ref{app:InternalSmoothing}).
...
\begin{itemize}
    ...
    \item \textbf{Mean Curvature Jump Positivity} (Theorem~\ref{thm:CompleteMeanCurvatureJump}): Analysis of $[H]_{\bar{g}} \ge 0$ under the favorable jump condition.
    ...
    \item \textbf{Jang Reduction for MOTS} (Theorem~\ref{thm:DirectTrappedJang}): Proves the Penrose inequality directly for MOTS $\Sigma_0$ with \textbf{favorable jump} $\tr_{\Sigma_0} k \ge 0$, bypassing the problematic area comparison reduction to the outermost MOTS.
...
\begin{itemize}
    ...
    \item \textbf{Program F: Direct Spacetime Route} (Section~\ref{sec:ProgramF}, Theorem~\ref{thm:DirectSpacetime}): Lorentzian area-comparison route via the event horizon and Hawking area theorem. Status: \textbf{CONDITIONAL}. In addition to weak cosmic censorship, it requires a global horizon-area comparison input; it is not part of the rigorous Jang/AMO core proof.
\end{itemize}

\paragraph{Summary:}
The \textbf{core analytical machinery} (Jang reduction, Lichnerowicz sealing, AMO monotonicity) is \textbf{rigorously complete with explicit calculations}. The main theorem (Theorem~\ref{thm:MainTheorem}) is \textbf{CONDITIONAL}: for arbitrary trapped surfaces it requires either a reduction to the direct MOTS regime, compactness (C1)--(C3) plus sign calibration, or a separate Lorentzian area-comparison input. For MOTS, the sharp theorem remains \textbf{conditional on favorable jump} (Theorem~\ref{thm:penroseinitial}). Extended results (Programs A, B, E) are rigorous within their stated hypotheses; Programs C and D are exploratory; Program F records a conditional Lorentzian route and is not used in the rigorous core proof. The three main theorems and all reduction theorems \textbf{do not depend on Programs C, D, or F}.
...
\end{enumerate}
The key distinction: $\Sigma$ contributes a distributional curvature term $2[H]\delta_\Sigma$ that affects the proof and is nonnegative only under the explicit favorable-jump hypothesis, while $\{p_k\}$ contribute nothing to the $p$-harmonic analysis because they have zero capacity. Both are essential for the complete argument but involve fundamentally different PDE techniques.
\end{remark}
...
\end{equation}
where $R_{\bg}^{reg} \in L^{3/2}_{loc}(\bM)$ is the regular part, $[H] = H^+ - H^- \ge 0$ is the mean curvature jump supplied in the direct MOTS argument by the favorable-jump hypothesis (see Remark~\ref{rem:SignConventionsSummary}(S5)), and $\mathcal{H}^2|_\Sigma$ is the 2-dimensional Hausdorff measure on $\Sigma$.
...
\begin{enumerate}
    \item The singular term $\frac{2[H]}{\epsilon}\rho(s/\epsilon)$ is \textbf{nonnegative} (since $[H] \ge 0$ under the favorable-jump hypothesis and $\rho \ge 0$).
...
\begin{enumerate}
    ...
    \item \textbf{Near the interface $\Sigma$:} By Lemma~\ref{lem:Transmission}, $\phi \in C^{1,\alpha_H}$ across $\Sigma$, so $\Omega_+ \cap \Sigma$ is either empty or an open subset of $\Sigma$. The required cancellation of the interface contribution is the weak transmission statement proved for the Bray--Khuri flux, so the boundary contribution from approaching $\Sigma$ from either side cancels.
...
\begin{equation}
    \int_{\Omega_+} \Div(Y) \, dV = 0 \text{ but } \int_{\Omega_+} \Div(Y) \, dV > 0 \text{ if } \Omega_+ \neq \emptyset
\end{equation}
forces $\Omega_+ = \{\phi > 1\} = \emptyset$, establishing $\phi \le 1$ globally.

\textbf{Step 2: Distributional extension across $\Sigma$.}
By Lemma~\ref{lem:Transmission}, the conformal factor $\phi \in C^{1,\alpha_H}$ across $\Sigma$, with continuous normal derivative: $[\partial_\nu \phi]_\Sigma = 0$. For the Bray--Khuri vector field $Y$, the needed interface cancellation is obtained from the weak transmission identity rather than from a separate pointwise continuity statement for $q$. Therefore:
\begin{itemize}
    \item no extra distributional boundary term is created at $\Sigma$;
    \item the normal component $\langle Y, \nu \rangle$ has matching one-sided traces in the weak sense: $[\langle Y, \nu \rangle]_\Sigma = 0$.
\end{itemize}
...
\begin{equation}\label{eq:DivYIntegralSense}
    \int_\Omega \Div(Y) \, dV \ge 0 \text{ with equality forcing } \Omega = \emptyset \text{ for } \Omega = \{\phi > 1\}.
\end{equation}
...
\begin{proposition}[H\"older Exponent Dependence]\label{prop:HolderExplicit}
The H\"older regularity exponents appearing throughout this paper depend only on the geometric data of the initial data set and can be bounded from below by universal positive constants.

\textbf{Part I: Sources of H\"older Exponents.}
Three distinct mechanisms produce H\"older regularity in this paper:
...
\end{equation}
This vanishes as $p \to 1^+$, which raises the concern that estimates depending on $C^{1,\alpha_H}$ norms might degenerate. However, \textbf{the uniform estimates required for the double limit do not depend on H\"older regularity}. Specifically:
\begin{itemize}
    ...
    \end{enumerate}
    \item The convergence rate $|\mathcal{M}_{p,\epsilon} - \mathcal{M}_{1,\epsilon}| \le C_A(p-1)^{1/2}$ derives from the $BV$ convergence, \emph{not} from H\"older interpolation. The constant $C_A$ depends on the geometry of $(\tM, \hat{g}_\epsilon)$ but is \textbf{independent of $p$}.
\end{itemize}
...
The H\"older exponents depend on the initial data only through:
\begin{enumerate}
...
\end{equation}
where $s = \mathrm{dist}(\cdot, \Sigma)$ and $C_0 = |\theta^-|/2 > 0$ is determined by the (negative) expansion $\theta^- = H_\Sigma - \tr_\Sigma k$ (independent of marginal stability). The function $B(y)$ solves an elliptic problem with solvability condition:
\begin{equation}
    \int_\Sigma (-C_0 H_\Sigma + Q) \psi_0 \, dA = 0,
\end{equation}
where $Q$ is the potential of the stability operator.

\textbf{(2) Scalar Curvature Consistency:} The Jang identity $R_{\bar{g}} = \mathcal{S} - 2\Div(q)$ with $\mathcal{S} = 16\pi(\mu - J(\nu)) + |h-k|^2 + 2|q|^2$ guarantees $\mathcal{S} \ge 0$ under DEC. However, the present proof does \emph{not} deduce the interface sign from marginal stability. The only rigorous sign statement used later is the explicit favorable-jump hypothesis $[H]_{\bar{g}} = \tr_\Sigma k \ge 0$ when invoking the direct MOTS theorem.
...
\textbf{(5) Compatibility with Bulk $\mathcal{S} \ge 0$:} The DEC margin on $(M,g,k)$ propagates to the Jang surface $(M,g,f)$ through the constraint equations. The bound $\mathcal{S} \ge C_{\text{DEC}} > 0$ in a neighborhood of $\Sigma$ prevents the Jang metric from developing \emph{integrable} regions of negative scalar curvature in the interior (away from the interface singularity). If, in a particular zero-jump situation, one also knows $[H]=0$, then no singular delta-mass appears at the interface.

\textbf{Conclusion:} Marginal stability $\lambda_1 = 0$ produces polynomial rather than exponential decay along the cylindrical end. Any additional conclusion that the interface singularity vanishes must come from a separate zero-jump argument; it is not inferred here from marginal stability alone.
\end{remark}
...
Marginal stability by itself does \emph{not} determine the interface jump $[H]_{\bar{g}}$. In particular, the present paper does not prove a perturbative formula such as
\[
    [H]_{\bar{g}} = 2C_0 \cdot \lambda_0 + O(\lambda_0^2).
\]
The rigorous direct MOTS theorem instead uses the explicit favorable-jump hypothesis
\[
    [H]_{\bar{g}} = \tr_\Sigma k \ge 0.
\]
If one separately knows that $[H]_{\bar{g}} = 0$, then the interface Dirac contribution vanishes and Miao smoothing is unnecessary at $\Sigma$:
\begin{equation}
    R_{\tilde{g}} = R_{\tilde{g}}^{\mathrm{reg}} \quad \text{(no } 2[H]\delta_\Sigma \text{ term)}.
\end{equation}
That simplification is therefore conditional on additional zero-jump input, not a consequence of marginal stability alone.
...
\end{equation}
where the unperturbed Schwarzschild operator on the horizon sphere is
\begin{equation}
    L_0 = -\Delta_{\Sigma_0} + \frac{1}{R_0^2}, \qquad R_0 = 2m,
\end{equation}
with principal eigenvalue $\lambda_1^{(0)} = 1/R_0^2 > 0$ and principal eigenfunction given by the positive constant mode. The first-order correction is:
\begin{equation}
    L_1 = -\delta(\Delta_{S^2}) - \delta\left( |A|^2 + \Ric(\nu, \nu) - \mu + J(\nu) \right),
\end{equation}
where $\delta(\cdot)$ denotes the linearized change. Accordingly,
\begin{equation}
    \lambda_1 = \frac{1}{R_0^2} + \epsilon \cdot \langle L_1 \psi_0, \psi_0 \rangle_{L^2(\Sigma_0)} + O(\epsilon^2),
\end{equation}
where $\psi_0$ is the normalized positive constant eigenfunction. In particular, for sufficiently small perturbations one still has $\lambda_1 > 0$, so the perturbed horizon remains strictly stable.
\textbf{Step 3: Jang function blow-up asymptotics.}
In the genuinely non-time-symmetric regime one expects the ingoing expansion to satisfy $\theta^- < 0$ on the perturbed MOTS, and then the nondegenerate Jang asymptotic has the form
\begin{equation}
    f(s,y) = C_0 \ln s + B_0(y) + O(s), \qquad C_0 = \frac{|\theta^-|}{2} > 0.
\end{equation}
The key point is that this coefficient depends on the non-time-symmetric perturbation and \emph{degenerates} in the exact Schwarzschild limit: for $\epsilon=0$ one has $k=0$ and $\theta^- = 0$ on the minimal sphere, so the logarithmic blow-up model does not apply there.

\textbf{Step 4: Mean curvature on each side of the interface.}
The Jang metric near $\Sigma$ takes the form $\bar{g} = g + df \otimes df$. In the nondegenerate trapped regime $C_0>0$, the graph acquires a cylindrical end and the pointwise mean-curvature expressions in singular coordinates become poor proxies for the interface term. What survives rigorously is not a naive difference of divergent one-sided limits, but the distributional corner quantity determined by the Miao-type jump formula.
...
After this careful regularization one obtains a perturbative relation between the jump and the deformation parameters. We stress, however, that this calculation is only a model-space consistency check. It does \emph{not} prove a general implication ``stability $\Rightarrow [H]\ge 0$''. The rigorous sign input used elsewhere in the paper is the explicit favorable-jump hypothesis together with the identity $[H]_{\bar g}=\tr_\Sigma k$ from Theorem~\ref{thm:CompleteMeanCurvatureJump}.
\end{example}
...
where:
\begin{enumerate}
    \item $C_0 = \frac{|\theta^-|}{2} = \frac{|H_\Sigma - \tr_\Sigma k|}{2} > 0$ (trapped surface condition).
...
We can now state the main theorem precisely. The theorem requires one of the following conditions: (i) the direct favorable-jump MOTS regime, (ii) compactness plus an additional sign calibration, or (iii) Lorentzian area comparison.
...
Let $(M, g, k)$ satisfy asymptotic flatness ($\tau > 1$) and DEC. Let $\Sigma_0$ be a closed trapped surface ($\theta^+ \le 0$, $\theta^- < 0$). Under (A) the direct MOTS regime (namely $\Sigma_0$ is a MOTS with favorable jump), (B) compactness together with a sign calibration converting the adjoint-cone condition into favorable jump, or (C) Lorentzian area comparison:
...
% Alias label for cross-referencing convenience (this theorem is also known as thm:penroseinitial)
\makeatletter
\newcommand{\labelaliasaliases}{}% placeholder
\makeatother

\begin{theorem}[Conditional Spacetime Penrose Inequality]\label{thm:SPI}\label{thm:penroseinitial}
Let $(M, g, k)$ be a three-dimensional asymptotically flat initial data set with decay rate $\tau > 1$ satisfying the Dominant Energy Condition. Let $\Sigma_0$ be a \textbf{MOTS} satisfying the \textbf{favorable jump condition} $\tr_{\Sigma_0} k \ge 0$. Then:
...
Under compactness conditions together with the favorable-jump sign input for the comparison MOTS, the \textbf{two-stage reduction} proves the Penrose inequality:
\begin{enumerate}
    ...
    \item \textbf{MOTS Penrose (Theorem~\ref{thm:SPI}):} For MOTS $\Sigma^*$ satisfying the favorable jump condition, the Jang-based proof applies.
    \item \textbf{Conclusion:} Under these conditions, $M_{\mathrm{ADM}} \ge \sqrt{A(\Sigma^*)/(16\pi)} \ge \sqrt{A(\Sigma_0)/(16\pi)}$.
\end{enumerate}
\textbf{Warning:} Without compactness conditions, the area comparison to outermost MOTS can fail---binary BH merger counterexamples exist. The comparison $A(\Sigma^*) \ge A(\Sigma_0)$ using only initial data methods remains \textbf{OPEN}. \textit{Note:} Theorem~\ref{thm:Penrose1973Complete} uses a different route through event-horizon comparison, and that comparison is an additional explicit hypothesis in Program~F.
\end{remark}

\begin{proof}[Proof of Theorem~\ref{thm:SPI}]
Since $\Sigma_0$ is a MOTS with $\theta^+=0$, $\theta^-<0$, and favorable jump $\tr_{\Sigma_0}k\ge0$, Theorem~\ref{thm:DirectTrappedJang} applies directly and produces a Jang metric $\bar{g}$ that:
\begin{itemize}
...
\textbf{Step 2: Mean curvature jump from the favorable condition.}
By Theorem~\ref{thm:CompleteMeanCurvatureJump},
\[
    [H] = H^+ - H^- = \tr_{\Sigma_0}k \ge 0 \quad \text{at } \Sigma_0.
\]
...
\]
\textbf{General trapped surfaces.}
Theorem~\ref{thm:SPI} is the direct MOTS statement. The extension from an arbitrary trapped surface to a suitable MOTS is handled separately in Theorem~\ref{thm:MainTheorem}, which requires one of the additional hypotheses listed there (compactness, a Lorentzian area-comparison input, or a direct favorable-jump route).
\end{proof}

\begin{remark}[On the Term ``Unconditional'' and Essential Hypotheses]
The MOTS Penrose inequality (Case A) is \textbf{conditional on the favorable jump hypothesis}---it applies to the outermost MOTS $\Sigma^*$ without requiring cosmic censorship or compactness, but it still assumes $\tr_{\Sigma^*} k \ge 0$ (equivalently $[H]_{\bar g}\ge0$ in the Jang reduction).

For \textbf{general trapped surfaces} $\Sigma_0$ (Case B), the Penrose inequality requires one of: a Lorentzian area-comparison input, compactness conditions, or a reduction to the direct MOTS regime.
...
\begin{tikzpicture}[node distance=1.5cm, auto,
    box/.style={rectangle, draw, text width=4.5cm, text centered, rounded corners, minimum height=1cm}]
    \node[box] (main) {\textbf{Thm.~\ref{thm:MainTheorem}}\\Spacetime Penrose\\(conditional)};
    \node[box, below left=1.5cm and 0.3cm of main] (area) {\textbf{Thm.~\ref{thm:AreaMonotonicity}}\\Area Monotonicity\\$A(\Sigma^*) \ge A(\Sigma_0)$};
    \node[box, below right=1.5cm and 0.3cm of main] (jump) {\textbf{Thm.~\ref{thm:CompleteMeanCurvatureJump}}\\$[H]=\tr_\Sigma k \ge 0$\\under favorable jump};
    \node[box, below=3cm of main] (amo) {\textbf{Thm.~\ref{thm:DistrBochner}}\\AMO Monotonicity};
    \node[box, right=1.5cm of main] (rigidity) {\textbf{Thm.~\ref{thm:RigidityAMO}}\\Rigidity};
    \node[box, below=1.5cm of area, xshift=-0.5cm] (degenerate) {\textbf{Prop.~\ref{prop:DegeneratePI}}\\Degenerate Case};
...
\begin{itemize}
    \item \textbf{Theorem~\ref{thm:CompleteMeanCurvatureJump}}: Mean curvature jump identity and favorable-jump sign condition
...
\begin{proof}
On each smooth conformally corrected approximation $(\tM,g_\epsilon)$, $g_\epsilon=u_\epsilon^4\hat g_\epsilon$, with $R_{g_\epsilon}\ge 0$, AMO monotonicity implies $\mathcal{M}_p'(t)\ge 0$. Equality of the Penrose bound forces $\mathcal{M}_p(t)$ to take the same value at the horizon and at infinity in the limit $p\to 1^+$, hence $\mathcal{M}_p'(t)\equiv 0$ for a.e. $t$.
...
\textbf{Step 4c: Why Ricci flatness is \emph{not} the needed input.}
...
For the Schwarzschild metric $f^{-2} = 1 - 2m/r$, a direct computation shows that the \emph{scalar curvature} vanishes but the 3-dimensional Ricci tensor does not. Using the standard formulas for a metric of the form
\[
g = \frac{dr^2}{h(r)} + r^2 g_{S^2}, \qquad h(r)=1-\frac{2m}{r},
\]
one obtains:
\begin{align}
    \Ric_{\theta\theta} &= 1 - h - \frac{rh'}{2} = \frac{m}{r}, \\
    \Ric_{rr} &= -\frac{h'}{rh} = -\frac{2m}{r^2(r-2m)}.
\end{align}
...
\begin{equation}
    R_{g_{\mathrm{Sch}}} = 0.
\end{equation}
This is the Hamiltonian-constraint consequence of time symmetry and vacuum. It does \emph{not} mean that the 3-dimensional Ricci tensor vanishes.
...
\end{equation}
For the horizon sphere, $|A|^2 = 0$ because the surface is totally geodesic. Since $R=0$, $H=0$, and the intrinsic Gauss curvature is $K = 1/R_\Sigma^2$, the Gauss equation gives
\begin{equation}
    K = -\Ric(\nu,\nu),
\end{equation}
so $\Ric(\nu,\nu) = -1/R_\Sigma^2$ with $R_\Sigma = 2M$. Hence
\begin{equation}
    L_\Sigma = -\Delta_\Sigma + \frac{1}{R_\Sigma^2},
\end{equation}
whose principal eigenvalue is
\begin{equation}
    \lambda_1 = \frac{1}{R_\Sigma^2} = \frac{1}{4M^2} > 0.
\end{equation}
Thus the Schwarzschild horizon is strictly stable.
...
$[H]_{\bar{g}}$ & $0$ & model-dependent & requires verification & len$^{-1}$ \\
...
\hline
\multicolumn{5}{|l|}{\textit{Stage 3: Smoothing and correction $(\tilde{M}, g_\epsilon)$}} \\
\hline
Smoothing? & No & Yes & Yes & -- \\
$R_{\hat{g}_\epsilon}^-$ & $=0$ & $L^{3/2}$-small & $L^{3/2}$-small & len$^{-2}$ \\
$R_{g_\epsilon}$ & $= 0$ & $\ge 0$ & $\ge 0$ & len$^{-2}$ \\
$|M(g_\epsilon)-M(\tilde{g})|$ & $0$ & $o(1)$ & $o(1)$ & mass \\
\hline
...
\begin{itemize}
    ...
    \item \textbf{Extremal or sub-extremal Kerr:} Stability properties alone do not determine the sign of $[H]$ in the present manuscript. To use the direct theorem one would still need to verify the favorable jump condition $\tr_\Sigma k \ge 0$ for the chosen horizon.
\end{itemize}
