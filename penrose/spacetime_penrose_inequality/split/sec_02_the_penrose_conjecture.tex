...
\end{itemize}
\textbf{Key distinction:} Theorem~\ref{thm:penroseinitial}/\ref{thm:SPI} requires the MOTS to be \textbf{stable} ($\lambda_1(L_\Sigma) \ge 0$), which is automatic for the outermost MOTS by Andersson--Metzger. Theorem~\ref{thm:MainTheorem}/\ref{thm:CompleteProof} addresses general trapped surfaces and requires one of: (A) favorable jump, (B) compactness, or (C) cosmic censorship.
\end{remark}
...
\begin{itemize}
    ...
    \item \textbf{Conditional for general trapped surfaces} (Theorem~\ref{thm:MainTheorem}/\ref{thm:CompleteProof}): For arbitrary trapped surfaces, one of (A) favorable jump ($\tr_\Sigma k \ge 0$ pointwise), (B) compactness (C1)--(C3) \textbf{plus Conjecture~C for $k \neq 0$}, or (C) cosmic censorship is required.
\end{itemize}
