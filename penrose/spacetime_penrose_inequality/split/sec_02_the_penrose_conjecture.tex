...
\begin{itemize}
    ...
    \item \textbf{Conditional for general trapped surfaces} (Theorem~\ref{thm:MainTheorem}/\ref{thm:CompleteProof}): For arbitrary trapped surfaces, one needs either (A) the direct MOTS regime (the given surface is a MOTS with $\tr_\Sigma k \ge 0$ pointwise), (B) compactness (C1)--(C3) \textbf{plus Conjecture~C for $k \neq 0$}, or (C) cosmic censorship.
\end{itemize}
...
\begin{itemize}
    ...
    \item[(B)] \textbf{Compactness + Conjecture~C:} One of conditions (C1)--(C3) of Theorem~\ref{thm:MaxAreaTrapped} holds. \textbf{Important:} For $k \neq 0$, this additionally requires Conjecture~\ref{conj:IntegralToPointwise} (integral-to-pointwise upgrade); for $k = 0$, this is proved in Theorem~\ref{thm:IntegralToPointwise}, or
...
\begin{enumerate}
    \setcounter{enumi}{2}
    \item \textbf{Trapped surfaces with unfavorable jump ($\tr_\Sigma k < 0$):} Requires area comparison. Additional gap for $k \neq 0$: The integral-to-pointwise upgrade is currently open.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Compactness (Theorem~\ref{thm:MaxAreaTrapped}):} Under conditions (C1)--(C3), there exists a maximum area trapped surface $\Sigma_{\max}$ with $A(\Sigma_{\max}) \ge A(\Sigma_0)$ and $\int_{\Sigma_{\max}} \tr_\Sigma k \, dA \ge 0$ (integral condition only; pointwise upgrade remains open for $k \neq 0$).
\end{enumerate}
