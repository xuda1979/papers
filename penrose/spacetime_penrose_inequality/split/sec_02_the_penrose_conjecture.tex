...
\begin{itemize}
    \item \textbf{Theorem A (Unconditional):} For outermost \emph{stable} MOTS --- see Theorem~\ref{thm:intro-main} (Introduction), Theorem~\ref{thm:penroseinitial} (summary form), and Theorem~\ref{thm:SPI} (detailed form below).
...
\begin{itemize}
    \item \textbf{Unconditional for outermost stable MOTS} (Theorem~\ref{thm:penroseinitial}/\ref{thm:SPI}): For the apparent horizon $\Sigma^*$ that is \textbf{stable} ($\lambda_1(L_{\Sigma^*}) \ge 0$), the inequality holds without cosmic censorship or symmetry assumptions. Stability implies $[H]_{\bar{g}} \ge 0$ via spectral analysis (Theorem~\ref{thm:CompleteMeanCurvatureJump}).
...
\begin{itemize}
    \item \textit{Potential issue:} The Holder exponent $\alpha_H(p) \to 0$ as $p \to 1^+$, and constants might blow up.
    ...
    \]
    Only the Holder exponent degenerates, not the $L^\infty$ bound needed for our estimates.
\end{itemize}
...
\begin{proposition}[Holder Exponent Dependence]\label{prop:HolderExplicit}
The Holder regularity exponents appearing throughout this paper depend only on the geometric data of the initial data set and can be bounded from below by universal positive constants.

\textbf{Part I: Sources of Holder Exponents.}
Three distinct mechanisms produce Holder regularity in this paper:
...
\end{equation}
This vanishes as $p \to 1^+$, which raises the concern that estimates depending on $C^{1,\alpha_H}$ norms might degenerate. However, \textbf{the uniform estimates required for the double limit do not depend on Holder regularity}. Specifically:
\begin{itemize}
    ...
    \end{enumerate}
    \item The convergence rate $|\mathcal{M}_{p,\epsilon} - \mathcal{M}_{1,\epsilon}| \le C_A(p-1)^{1/2}$ derives from the $BV$ convergence, \emph{not} from Holder interpolation. The constant $C_A$ depends on the geometry of $(\tM, \hat{g}_\epsilon)$ but is \textbf{independent of $p$}.
\end{itemize}
...
The Holder exponents depend on the initial data only through:
\begin{enumerate}
