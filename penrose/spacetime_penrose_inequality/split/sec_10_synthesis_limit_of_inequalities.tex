...
\begin{enumerate}
    \item \textbf{Direct MOTS regime:} $\Sigma_0$ is a MOTS and $\tr_{\Sigma_0} k \ge 0$ pointwise;
    ...
    \item \textbf{Cosmic censorship plus Lorentzian area comparison:} The data embeds in a spacetime satisfying WCC together with the event-horizon area-comparison input isolated in Program~F.
\end{enumerate}
...
\begin{itemize}
    ...
    \item For general trapped surfaces, even the pointwise sign $\tr_{\Sigma_0} k \ge 0$ is not enough by itself to trigger the direct Jang theorem unless the surface is a MOTS. The trapped conditions certainly do \textbf{not} imply this sign: for example $H = -3$, $\tr k = -1$ gives $\theta^+ = -4$, $\theta^- = -2$ (both trapped), but $\tr k = -1 < 0$.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Mean curvature jump:} By the favorable jump condition, $[H]_{\bar{g}} \ge 0$. If a separate argument yields $[H]_{\bar{g}} = 0$, then the interface Dirac term disappears; no stronger regularity conclusion is used here.
    \item \textbf{Distributional curvature:} With $[H] = 0$, the interface Dirac term vanishes, so only the bulk scalar-curvature contribution remains.
\end{itemize}
...
\begin{itemize}
    \item \textbf{No interface smoothing needed:} When $[H] = 0$ is known separately, Miao's corner smoothing is not needed at that interface because there is no interface curvature measure to regularize there. Any bubble tip singularities are still handled separately.
    \item \textbf{Scalar curvature:} In this zero-jump regime, the distributional scalar curvature has no interface Dirac part. The later approximation arguments may therefore be carried out directly on the unsmoothed interface metric, or with smoothing restricted to the remaining singular loci.
\end{itemize}
...
\textbf{Conclusion:} The marginal stability case flows through all stages consistently. If a separate zero-jump input is available, then the interface regularity improves and the corner smoothing step simplifies. Independently, the decay changes from exponential to polynomial, and that change is handled by adjusted weight choices.
\end{remark}
