...
Let $(M, g, k)$ be an asymptotically flat initial data set satisfying the Dominant Energy Condition. Let $\Sigma_0$ be any closed trapped surface (satisfying $\theta^+ \le 0$). Then $M_{\ADM}(g) \ge \sqrt{A(\Sigma_0)/16\pi}$.
\end{theorem}
...
\begin{enumerate}
    ...
    The Miao corner formula gives $[H]_{\bar{g}} = \tr_{\Sigma_0} k$ at any trapped surface. For \textbf{stable MOTS}, Theorem~\ref{thm:CompleteMeanCurvatureJump} establishes $[H]_{\bar{g}} \ge 0$ via spectral analysis. For \textbf{general trapped surfaces}, the favorable jump condition $\tr_{\Sigma_0} k \ge 0$ is an \emph{additional hypothesis} (not implied by $\theta^\pm$ conditions alone).
...
If the proof of $[H]_{\bg} \ge 0$ (Theorem~\ref{thm:CompleteMeanCurvatureJump} for stable MOTS) were incorrect, the distributional scalar curvature could be negative as a measure: $R_{\tg} = R_{\tg}^{reg} + 2[H]_{\tg}\delta_{\Sigma_0}$ with $[H]_{\tg} < 0$. In this case:
\begin{itemize}
...
\begin{enumerate}
    \item \textbf{Theorem~\ref{thm:DirectTrappedJang}} (Direct Trapped Surface Construction) -- Jang blow-up at trapped surfaces with $\theta^- < 0$ and favorable jump $\tr_\Sigma k \ge 0$.
