...
\begin{remark}[The Unfavorable Regime: Not Resolved by the Direct MOTS Argument]
...
\end{itemize}
\textbf{Current status:} The direct Jang/AMO proof in this manuscript does \emph{not} resolve this regime by itself. For arbitrary trapped surfaces with unfavorable jump one still needs additional area-comparison input or another reduction theorem beyond the direct MOTS argument.
\end{remark}
...
\textbf{What the p-harmonic method actually resolves:}
\begin{enumerate}[(i)]
\item A quasi-monotone quantity satisfying the structural requirements (S1)--(S5) is available in the smooth Riemannian target problem: the AMO functional provides this once one has reduced to a manifold with nonnegative scalar curvature and minimal boundary (Theorem~\ref{thm:AMOMonotonicity}).
\item The final monotonicity step of the argument is therefore rigorous on the smoothed Jang--conformal approximants, after the direct favorable-jump reduction has already been achieved.
\item What remains unresolved at the spacetime level is the passage from a general trapped surface with $k\neq 0$ to a comparison MOTS satisfying the pointwise favorable jump. That issue is isolated in the compactness and Lorentzian comparison alternatives summarized later in Theorem~\ref{thm:CompleteProof}.
\end{enumerate}
