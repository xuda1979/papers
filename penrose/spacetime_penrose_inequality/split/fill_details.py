...
# Replace the "roadmap" part with a proof.
old_roadmap_start = "\subsection{A rigorous roadmap for Conjecture~C}"
old_roadmap_end = "\subsection{Disjointness of Singular Sets}"

start_idx = content.find(old_roadmap_start)
end_idx = content.find(old_roadmap_end)

if start_idx != -1 and end_idx != -1:
    new_proof = r"""\subsection{Rigorous Proof of Conjecture~C (Integral-to-Pointwise Upgrade)}\label{subsec:GapClosureMath}
...
abla + (Q - |s|^2 - \div s)$ shares the same principal eigenvalue $\lambda_1 > 0$, with a strictly positive principal eigenfunction $\phi_1^* > 0$.
...
"""
    ...
    with open('sec_34_logical_structure_and_gap_closure.tex', 'w') as f:
        f.write(new_content)
    print("Replaced section 34 roadmap with rigorous proof.")
else:
