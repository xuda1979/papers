...
\begin{itemize}
    \item \textbf{MOTS Penrose:} For the outermost MOTS $\Sigma^*$, Theorem~\ref{thm:penroseinitial} applies directly via the Jang equation, provided the favorable jump condition holds (see Conjecture C).
...
\begin{enumerate}
    ...
    \item[(B)] \textbf{Compactness:} Conditions (C1)--(C3) of Theorem~\ref{thm:MaxAreaTrapped}. Note: For $k \neq 0$, this additionally requires the integral-to-pointwise upgrade (currently open);
...
For \textbf{general trapped surfaces} $\Sigma_0$, the Penrose inequality requires one of: a reduction to the direct MOTS regime, compactness plus an integral-to-pointwise upgrade, or a separate Lorentzian area-comparison input. These are precisely the places where the proof remains conditional.
