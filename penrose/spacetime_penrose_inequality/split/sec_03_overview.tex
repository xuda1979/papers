...
\begin{itemize}
    \item \textbf{MOTS Penrose:} For the outermost MOTS $\Sigma^*$ satisfying the favorable jump condition, Theorem~\ref{thm:penroseinitial} applies directly via the Jang equation. Theorem~\ref{thm:AdjointConeEuler} records the sharp adjoint-cone variational condition available in the compactness route.
    \item \textbf{Trapped Surface Penrose (conditional):} For arbitrary trapped surfaces $\Sigma_0$, we require one of: (i) reduction to a MOTS in the direct favorable-jump regime, (ii) compactness conditions (C1)--(C3) plus a sign input giving favorable jump on the comparison MOTS, or (iii) cosmic censorship together with the needed Lorentzian area comparison.
...
\begin{itemize}
    ...
    \item The \textbf{Maximum Area Theorem} (Theorem~\ref{thm:MaxAreaTrapped}): Under compactness, provides area comparison without cosmic censorship; the favorable-jump sign needed for the Jang step remains an additional input when $k\neq0$.
...
The main external ingredients are: Lockhart--McOwen's theory of elliptic operators on manifolds with ends \cite{lockhartmccowen1985}; Miao's smoothing across corners with quantitative scalar curvature bounds \cite{miao2002}; Agostiniani--Mazzieri--Oronzio's $p$-harmonic level set monotonicity \cite{amo2024}; and Tolksdorf/Lieberman regularity for quasilinear elliptic equations with bounded measurable coefficients \cite{tolksdorf1984,lieberman1988}.
...
\textbf{The Central Argument:} To ensure the mass does not increase during this deformation ($M_{\ADM}(\bg) \ge M_{\ADM}(\tg)$), we must have $\phi \le 1$. We prove this using the Bray-Khuri divergence identity (\Cref{sec:GlobalBound}).
    \item \textbf{The $p$-Harmonic Level Set Method:} After smoothing the corner and applying the small conformal correction, we work with smooth Riemannian approximants of nonnegative scalar curvature. The modern level set method of Agostiniani, Mazzieri, and Oronzio is applied on those approximants and then passed to the singular limit. We solve for a $p$-harmonic function $u_p$; its level sets provide a foliation from the horizon to infinity. The strength of this method is that the relevant monotonicity survives the limiting procedure through Mosco convergence and capacity removability. By taking the limit as $p \to 1$, this functional's value at the horizon is identified with the horizon area and its value at infinity is identified with the ADM mass, yielding the Penrose inequality.
\end{enumerate}
...
In this section we establish the spacetime Penrose inequality under explicitly delineated conditions. The key innovation is the \textbf{Jang Reduction for MOTS}:
\begin{itemize}
    \item \textbf{MOTS with favorable jump}: The Jang Reduction (Theorem~\ref{thm:DirectTrappedJang}) works for any MOTS $\Sigma$ satisfying the favorable jump condition $\tr_{\Sigma} k \ge 0$.
...
\begin{enumerate}
    ...
    \item[(B)] \textbf{Compactness plus sign calibration:} Conditions (C1)--(C3) of Theorem~\ref{thm:MaxAreaTrapped}, together with an additional calibration converting the adjoint-cone condition of Theorem~\ref{thm:AdjointConeEuler} into the pointwise favorable jump when $k\neq0$;
    \item[(C)] \textbf{Lorentzian area comparison:} Data embeds in spacetime satisfying WCC together with the area-comparison input isolated in Program~F.
\end{enumerate}
Then $M_{\ADM}(g) \ge \sqrt{A(\Sigma_0)/(16\pi)}$. See Theorem~\ref{thm:MainTheorem} for the complete statement and proof.
\end{theorem}

The paper contains one rigorous core proof together with several complementary programs. Programs A and B feed directly into the Jang--AMO argument; Programs C and D are exploratory and are included to record possible alternative routes and structural heuristics.
...
\begin{enumerate}
    ...
    \item \textbf{Program C (Weak IMCF):} Records a speculative alternative route that would bypass Jang reduction by developing a spacetime version of weak inverse mean curvature flow.
    \item \textbf{Program D (Capacity Bootstrap):} Records an exploratory capacity-theoretic route toward removing stability assumptions; it is not part of the core proof.
\end{enumerate}
...
By the work of Bartnik \cite{bartnik1986} and Chrusciel \cite{chrusciel1986}, for any asymptotically flat metric with $\tau > 1/2$, there exist \emph{harmonic coordinates} $\{y^i\}$ at infinity satisfying:
\begin{equation}
...
\end{equation}
Contracting with $x_i$ and integrating over $S^2$, we use
\(\int_{S^2} x_i x_j\, d\Omega = \frac{4\pi r^2}{3}\,\delta_{ij}\) and obtain:
\begin{equation}
    \frac{1}{4\pi}\int_{S^2} m_{ij}\,\frac{x_i x_j}{r^2}\,d\Omega
    = \frac{1}{2}\,\frac{1}{4\pi}\int_{S^2} m_{jj}\,\frac{x_i x_i}{r^2}\,d\Omega
    \quad\Longrightarrow\quad
    \frac{1}{3}m_{ii}=\frac{1}{2}m_{jj}.
\end{equation}
Since $m_{ii}$ and $m_{jj}$ denote the same trace, this identity forces the \emph{trace-free} part of $m_{ij}$ to vanish at the monopole level. Equivalently, the leading $1/r$ term must be isotropic:
\begin{equation}
    m_{ij} = M\,\delta_{ij} \quad\text{(monopole term)},
\end{equation}
with any remaining anisotropic components occurring at higher multipole order (and hence not contributing to the ADM mass).
...
\begin{equation}
    C_R := \frac{1}{4\pi R} \int_{S_R} h_{ii} \, d\sigma = \frac{R^{1-\tau}}{4\pi} \int_{S^2} m_{ii} \, d\Omega + O(R^{-\tau-\delta}).
\end{equation}

\textbf{Step 4d: Vanishing of divergent terms.}
We now show that for $\tau \neq 1$, the divergent $O(R^{1-\tau})$ terms in both the flux and correction integrals vanish identically due to the Hamiltonian constraint.

Recall the identity derived from the constraint equation:
\begin{equation}
    (2\tau + 1) \int_{S^2} m_{ii} \, d\Omega = 3 \int_{S^2} m_{ij} \hat{x}^i \hat{x}^j \, d\Omega.
\end{equation}
Consider the isotropic part of the mass aspect. If we assume an isotropic leading term $m_{ij} \approx \frac{A}{r^\tau} \delta_{ij}$, substituting into the identity gives:
\begin{equation}
    (2\tau + 1) (3A) (4\pi) = 3 (A) (4\pi) \implies 2\tau + 1 = 1 \implies \tau = 0.
\end{equation}
Thus, for any decay rate $\tau > 0$, the Hamiltonian constraint forbids the existence of an isotropic monopole term at order $O(r^{-\tau})$. This implies that the monopole component of the trace vanishes:
\begin{equation}
    \int_{S^2} m_{ii} \, d\Omega = 0.
\end{equation}
Consequently, the correction term $C_R$ vanishes at leading order. Similarly, the flux term $F_R$ depends on the monopole parts of $m_{ij}$, which are constrained to be zero.
Therefore, the apparent divergence $O(R^{1-\tau})$ is absent, and the mass is determined by the subleading $O(r^{-1})$ terms (where $\tau=1$ effectively holds, allowing a non-zero mass $M$).

This mechanism is consistent with the harmonic gauge analysis. In harmonic coordinates, the flux integrand simplifies to $\approx -\frac{1}{2} \partial_i h_{jj}$, which is proportional to the derivative of the correction term integrand. Since the leading coefficient vanishes, both contributions are zero at the divergent order.
...
\begin{multline*}
    M_{\ADM}(g) \ge M_{\ADM}(\bg) \ge M_{\ADM}(\tg) \\
    = \lim_{p \to 1^+} \mathcal{M}_p(1) \ge \lim_{p \to 1^+} \mathcal{M}_p(0) = \sqrt{\frac{A(\Sigma)}{16\pi}}.
\end{multline*}
\end{proof}

\begin{theorem}[Complete Borderline Compatibility Verification]
\label{thm:BorderlineCompatibility}
Let $(M, g, k)$ have borderline asymptotic flatness with rate $\tau \in (1/2, 1]$. The proof structure of the main theorem (Section~\ref{sec:Synthesis}) extends to this regime. Specifically:
...
\begin{enumerate}
    ...
    \end{equation}
    \item \textbf{Bray--Khuri mass reduction:} Under $\phi \le 1$ (Theorem~\ref{thm:PhiBound}),
    \begin{equation}
...
\begin{enumerate}
    ...
    \item \textbf{Monotonicity:} The AMO monotonicity formula $\mathcal{M}_p'(t) \ge 0$ is applied after the smoothing-and-conformal-correction step, where the approximating metric has $R\ge0$.
...
\begin{enumerate}
    ...
    \item Satisfy $\|R^-_{\hat{g}_\epsilon}\|_{L^{3/2}} \to 0$, and after conformal correction give smooth metrics $g_\epsilon$ with $R_{g_\epsilon}\ge0$.
    \item Have ADM mass satisfying $M_{\ADM}^{\mathrm{reg}}(g_\epsilon) \to M_{\ADM}^{\mathrm{reg}}(\tg)$.
\end{enumerate}
...
\end{enumerate}
Under these hypotheses, the AMMO divergence identity (formula (1.11) of Agostiniani--Mantegazza--Mazzieri--Oronzio \cite{amo2024}) provides the Bochner inequality \emph{without} any Ricci-scalar bound.
\end{remark}
...
\end{equation}
where $a_p = (p-1)^2/(p-1+\epsilon_p) > 0$ is the explicit constant from the $p$-Bochner/Kato identity (as in AMMO \cite{amo2024}). In particular, the AMO monotonicity functional
\begin{equation}\label{eq:AMOMonotonicity}
    \mathcal{F}_p(t) := \int_{\{u_p = t\}} \Phi_p(u_p, |\nabla u_p|) \, d\sigma_{\tg}
\end{equation}
is monotone nonincreasing in $t \in (0, 1)$, where $\Phi_p$ is the AMO integrand from \cite{amo2024}.
\end{theorem}
...
\begin{enumerate}
    ...
    \item $[H]_{\tg} \ge 0$ on $\Sigma$ under the favorable-jump hypothesis from Theorem~\ref{thm:CompleteMeanCurvatureJump};
...
\textbf{Part (2):} The interface contribution follows from Theorem~\ref{thm:CompleteMeanCurvatureJump}, which gives $[H]_{\tg}=\tr_\Sigma k\ge0$ under the favorable-jump hypothesis.
...
\textit{Why the 2D formula is inadequate:} A common error is to apply the 2D Gauss--Bonnet formula $(2\pi - \Theta)\delta_{p_k}$ directly. This formula computes the \emph{Gaussian curvature} of a 2D cone, not the \emph{scalar curvature} of a 3D cone with $S^2$ cross-sections. In 3D, the scalar curvature measure is more complex, but the key property for our analysis is the capacity.

\textbf{Summary of the bubble tip curvature resolution:} The potential concern about "negative Dirac mass at tips" is resolved as follows:
\begin{enumerate}
    \item The 2D Gauss--Bonnet formula does not apply to 3D scalar curvature.
    \item The 3D scalar curvature near the tip behaves like $O(\rho^{-2})$, not a Dirac delta.
    \item Most importantly, the \textbf{capacity bypass} (Lemma~\ref{lem:Capacity}) ensures that regardless of the precise curvature measure at the tips, they do not contribute to the $W^{1,p}$ energy integrals for $1 < p < 3$ because isolated points have zero $p$-capacity.
    \item Thus, the effective nonnegativity condition~\eqref{eq:CurvatureEffectivePositive} holds, which is sufficient for the AMO monotonicity.
\end{enumerate}
...
The proof proceeds in three steps, following the structure suggested by the AMMO divergence identity \cite{amo2024}.
...
\begin{remark}[Important Note on AMMO Application]
The AMMO formula \cite{amo2024} is stated for metrics with $R \ge 0$ (strictly nonnegative scalar curvature). Our smoothed metrics satisfy only $R_{\tg_\varepsilon} \ge -\varepsilon$. 

The AMMO divergence identity still applies, but yields an error term proportional to $\varepsilon$ (controlled in equation~\eqref{eq:BochnerBulkEps}). The passage to the limit $\varepsilon \to 0$ uses Lemma~\ref{lem:MollificationError} to show the error vanishes.

Alternatively, one could use a more general Bochner identity valid for $R \ge -\varepsilon$ (see \cite{petersen2016} for Riemannian Bochner formulas without curvature sign assumptions), but the result is the same.
\end{remark}

For each fixed $\varepsilon$, $(\tM, \tg_\varepsilon)$ is smooth with $R_{\tg_\varepsilon} \ge -\varepsilon$. The AMMO computation (their formula (1.11) in \cite{amo2024}) shows that, for the $p$-capacitary potential on a 3D AF manifold, the divergence of a certain vector field $X_\varepsilon$ can be written as:
...
\textbf{(I) The key insight of AMMO:} The Agostiniani--Mazzieri--Oronzio approach \cite{amo2024} does \emph{not} directly use the Bochner formula on the ambient manifold. Instead, it employs the \textbf{Gauss--Codazzi equations for level sets}.
...
The resulting identity (equation (1.11) in \cite{amo2024}) is:
\[
...
\textbf{(3) AMMO divergence identity \cite{amo2024}:}
\begin{itemize}
    ...
    \item \textbf{Verification:} Formula (1.11) of \cite{amo2024} (Comm.\ Math.\ Phys.\ 402 (2023)) is applied to each smoothed metric $\tg_\varepsilon$. The hypothesis $R_{\tg_\varepsilon} \ge -\varepsilon$ is weaker than their $R \ge 0$, but the identity remains valid with the error term $O(\varepsilon)$ that vanishes in the limit.
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{Frequency monotonicity bound:} By the frequency function analysis of Garofalo--Lin \cite{garofalo1987}, for $p$-harmonic functions the frequency $N(r) = \frac{r \int_{B_r} |\nabla u|^p}{\int_{\partial B_r} |u|^p}$ is monotone. Near a critical point $x_0 \in \mathcal{C}$:
        \begin{equation}
...
\end{equation}
where $\mathcal{L}^3$ is Lebesgue measure, $\mathcal{H}^2|_\Sigma$ is 2-dimensional Hausdorff measure on $\Sigma$, and $[H]_{\tg} \ge 0$ under the favorable-jump hypothesis of Theorem~\ref{thm:CompleteMeanCurvatureJump}. The negative part $\mathcal{R}^-$ is supported in the bulk where $R_{\tg}^{\text{reg}} < 0$, which has controlled $L^{3/2}$ norm by Corollary~\ref{cor:L32}.
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item The Hessian $\nabla^2 u$ exists in $L^2_{loc}$ by the Calderon--Zygmund theory for divergence-form equations.
    ...
    \begin{itemize}
        \item For Holder continuous metrics $g \in C^{0,\alpha}$, the Christoffel symbols $\Gamma^k_{ij}$ are only $C^{0,\alpha-1}$ and may not be defined classically.
...
By Tolksdorf--DiBenedetto regularity, $u_\epsilon \in C^{1,\alpha_H}_{\mathrm{loc}}$ with Holder exponent $\alpha_H = \alpha_H(p)$ and:
\begin{equation}
...
\end{equation}
since $\phi > 0$ and $[H]_{\bar{g}} \ge 0$ under the favorable-jump hypothesis of Theorem~\ref{thm:CompleteMeanCurvatureJump}.
...
\end{itemize}
The explicit rate in~\eqref{eq:PHarmonicStability} shows the convergence is Holder in the mollification parameter.
\end{remark}
...
\end{equation}
where $[H] \ge 0$ under the favorable-jump condition (Theorem~\ref{thm:CompleteMeanCurvatureJump}).
...
The proof establishes the spacetime Penrose inequality through the \textbf{Jang Reduction for MOTS}: Given a 3-dimensional initial data set $(M,g,k)$ and a closed MOTS $\Sigma_0$ with $\tr_{\Sigma_0} k \ge 0$:
\begin{enumerate}
    \item \textbf{Jang Reduction (Theorem~\ref{thm:DirectTrappedJang}):} Solve the Jang equation with blow-up forced at $\Sigma_0$. The favorable jump hypothesis gives $[H]_{\bar{g}} = \tr_{\Sigma_0} k \ge 0$.
...
\textbf{Rigorous content:} The Penrose inequality is rigorously proved through the Jang--Lichnerowicz--AMO synthesis summarized in Theorem~\ref{thm:MasterSynthesis} and the conditional summary Theorem~\ref{thm:UltimateUnconditional}. Program C does not affect these results.
\end{remark}
...
\begin{theorem}[Expected Weak IMCF in Spacetime]\label{thm:WeakIMCF}
Let $(M, g, k)$ be a 3-dimensional AF initial data set with DEC, and let $\Sigma_0$ be a MOTS ($\theta^+ = 0$). Subject to the additional weak-compactness and monotonicity inputs discussed below, one expects a family of surfaces $\{\Sigma_t\}_{t \ge 0}$ satisfying:
\begin{enumerate}
    ...
    \item One expects a monotonicity statement for the generalized Hawking mass: $m_H(\Sigma_t)$ should be nondecreasing in $t$.
\end{enumerate}
\end{theorem}
...
\end{equation}
This is the natural elliptic regularization, but the manuscript does not provide a complete spacetime existence theory with the needed uniform estimates.

\textbf{Step 2: Limit $p \to 1^+$.}
As $p \to 1^+$, one would seek convergence to a BV function $u$ whose level sets $\Sigma_t = \partial^* \{u > t\}$ (reduced boundaries) satisfy IMCF in the sense of Huisken--Ilmanen. The key missing input is a uniform bound of the form
\begin{equation}
    \int_{M \setminus \Sigma} |\nabla u_p|^p \, dV_g \le C \cdot A(\Sigma),
\end{equation}
independent of $p$. Establishing such a bound in the spacetime setting is part of the open program.
...
\end{equation}
where $\overset{\circ}{A}$ is the traceless second fundamental form. In the Riemannian case this suggests monotonicity under suitable scalar-curvature hypotheses. In the spacetime setting, however, the manuscript does not establish the corresponding weak formulation with all error terms controlled.

\textbf{Step 4: Limit at infinity.}
Assuming the preceding steps could be justified, one would then aim to identify the Hawking mass at infinity with the ADM mass:
\begin{equation}
    \lim_{t \to \infty} m_H(\Sigma_t) = M_{\ADM}(g).
\end{equation}
Combined with $m_H(\Sigma_0) = \sqrt{A(\Sigma_0)/(16\pi)}$ (since $\theta^+ = 0$ on a MOTS), this would yield the Penrose inequality. The present manuscript does not claim this as a proved alternative route.
\end{proof}
...
\textbf{Verification for marginally stable case.} When $\lambda_1 = 0$, the conformal factor has polynomial decay: $\phi - 1 \sim t^{-1}$. Since $t = -\log r$, this becomes
\[
    \phi = 1 + O\!\left(\frac{1}{\log(1/r)}\right)
\]
as $r \to 0$. This slower decay does not affect the capacity computation, as $\Cap_p(\{0\})$ remains zero for isolated points.
...
\begin{theorem}[Program D: Conjectural Transport Representation]\label{thm:TransportMass}
Assume one can define a renormalized optimal transport functional that sends compactly supported mass to infinity in an asymptotically flat manifold and that the first-order asymptotic correction of this functional is governed by the ADM mass term. Under those additional analytic inputs, one expects an optimal-transport characterization of $M_{\ADM}$.
\end{theorem}

\begin{proof}[Programmatic Outline]
The intended route has four ingredients:
\begin{enumerate}
    \item \textbf{Asymptotic control of the cost:} one would first need a precise expansion of the squared distance (or a renormalized transport cost) along rays escaping to infinity.
    \item \textbf{Kantorovich duality at infinity:} one would then pass from the renormalized cost to a dual formula for transporting compactly supported probability measures toward large coordinate spheres.
    \item \textbf{Mass extraction:} after subtracting the Euclidean leading term, one expects the remaining first-order coefficient to recover the ADM mass.
    \item \textbf{Relation to capacity/AMO energy:} finally, one would compare the transport functional with the variational energies already present in the $p$-harmonic level-set method.
\end{enumerate}

At present, however, the manuscript does \emph{not} prove the necessary renormalized limit, the relevant asymptotic transport expansion, or the identification of the first correction term with $M_{\ADM}$. Accordingly Program D is recorded only as a research direction and is not used anywhere in the rigorous core proof.
\end{proof}
...
% Note: This theorem applies to apparent horizons (outermost MOTS), but the favorable jump condition remains an explicit hypothesis.
...
\begin{enumerate}
    ...
    \item There exists a closed \textbf{outermost MOTS} $\Sigma$ (apparent horizon), possibly with finitely many spherical components.
\end{enumerate}
Then:
\begin{equation}
    M_{\ADM}(g) \ge \sqrt{\frac{A(\Sigma)}{16\pi}}.
\end{equation}
\textbf{Note:} This result is \textbf{conditional} on the favorable jump assumption $\tr_\Sigma k \ge 0$ (or equivalent gauge conditions). For general trapped surfaces, one additionally needs either a reduction to the direct MOTS regime, compactness plus sign calibration, or a Lorentzian area-comparison input---see Theorem~\ref{thm:MainTheorem}.
\end{theorem}

\begin{proof}
The proof uses the \textbf{Jang Reduction for MOTS} (Theorem~\ref{thm:DirectTrappedJang}), which works for MOTS satisfying the favorable jump condition $\tr_\Sigma k \ge 0$:

\textbf{Main case: MOTS with favorable jump.}
Given a closed MOTS $\Sigma$ with $\tr_\Sigma k \ge 0$:
\begin{enumerate}
    \item Solve the Jang equation with blow-up forced at $\Sigma$ (Theorem~\ref{thm:DirectTrappedJang}).
    \item The favorable jump hypothesis gives $[H]_{\bar{g}} = \tr_\Sigma k \ge 0$ at the interface.
...
The direct Jang/AMO framework resolves the main technical obstacles within the favorable-jump regime. We note that:
\begin{enumerate}
    ...
    \item \textbf{Unstable MOTS.} An enclosing outermost stable MOTS exists, but the needed area comparison with the inner unstable MOTS is false in general. Thus no general reduction theorem for unstable MOTS is proved here.
...
\textbf{Note:} This theorem records what the enclosure argument actually provides, and also why it is insufficient by itself. The main proof (Theorems~\ref{thm:MainTheorem}, \ref{thm:SPI}, \ref{thm:SPI_Core}) does \emph{not} use this route.

Let $(M, g, k)$ be a 3-dimensional AF initial data set satisfying DEC with a closed MOTS $\Sigma$. Then:
\begin{enumerate}
    \item There exists an \textbf{outermost stable MOTS} $\Sigma'$ that encloses $\Sigma$ (Andersson--Metzger \cite{anderssonmetzger2009}).
    \item If $\Sigma'$ also satisfies the favorable jump condition, then the direct MOTS theorem applies to $\Sigma'$.
    \item No general inequality comparing $A(\Sigma')$ and $A(\Sigma)$ is available; in fact $A(\Sigma') \ge A(\Sigma)$ is \textbf{false in general}.
\end{enumerate}

\textbf{Consequence:} This enclosure argument does not by itself prove the Penrose inequality for the inner surface $\Sigma$. The rigorous route used in this paper is the direct Jang reduction for MOTS satisfying the favorable jump condition.
\end{theorem}

\begin{proof}
We only justify the enclosure statement and the logical obstruction.
...
\textbf{Step 3: Why the reduction stops here.}
At this stage one has only an enclosing stable MOTS. To transfer any inequality from $\Sigma'$ back to the original surface $\Sigma$, one would need a comparison such as $A(\Sigma') \ge A(\Sigma)$. That comparison is false in general, so no theorem for the inner unstable MOTS follows from the enclosure argument alone.
\end{proof}
...
% Alias labels for cross-referencing (these theorems are referenced under multiple names)
\begin{theorem}[Alternative Approach via Area Comparison --- Historical]
\label{thm:DefinitiveAreaMonotonicity}\label{thm:AreaMonotonicity}
\textbf{Note:} This theorem describes an \textbf{alternative approach} using area comparison. The main proof (Theorems~\ref{thm:MainTheorem}, \ref{thm:SPI}, \ref{thm:SPI_Core}) uses the \textbf{Jang Reduction for MOTS} (Theorem~\ref{thm:DirectTrappedJang}), which \textbf{bypasses area comparison}. This section is retained for historical completeness.
...
\textbf{Our solution:} The Jang Reduction for MOTS (Theorem~\ref{thm:DirectTrappedJang}) proves the Penrose inequality \emph{directly} for MOTS $\Sigma_0$ with \textbf{favorable jump} $\tr_{\Sigma_0} k \ge 0$, without area comparison.
\end{theorem}
...
The corrected proof uses the Jang Reduction for MOTS (Theorem~\ref{thm:DirectTrappedJang}), which proves the Penrose inequality \emph{directly} for MOTS $\Sigma_0$ with \textbf{favorable jump} $\tr_{\Sigma_0} k \ge 0$, without any area comparison.
\end{remark}
...
\textbf{Our solution: Jang Reduction for MOTS.}

Our main innovation (Theorem~\ref{thm:DirectTrappedJang}) completely \textbf{bypasses} the need for area comparison:
\begin{enumerate}
    \item Given a MOTS $\Sigma_0$ (with $\tr_{\Sigma_0} k \ge 0$), we solve the Jang equation with blow-up forced at $\Sigma_0$ (Theorem~\ref{thm:DirectTrappedJang}).
    \item The favorable jump hypothesis gives $[H] = \tr_{\Sigma_0} k \ge 0$ at the interface.
    \item The Penrose inequality for $\Sigma_0$ follows from the standard AMO machinery applied to the resulting Jang metric.
\end{enumerate}

\textbf{Consequence:} The main theorem (Theorem~\ref{thm:MainTheorem}) applies to closed MOTS satisfying the favorable jump condition $\tr_{\Sigma_0} k \ge 0$. The area comparison problem is circumvented entirely.
\end{remark}
...
The following material records an earlier, \textbf{incorrect} approach that attempted to use Hawking mass monotonicity. This is preserved for completeness but is \textbf{not part of the current proof}, which uses the Jang Reduction for MOTS instead.
...
\begin{theorem}[Topology of Outermost Horizons]\label{thm:NonSphericalHorizon}
...
\begin{equation}\label{eq:NonSphericalPI}
    M_{\ADM}(g) \ge \sqrt{\frac{A(\Sigma)}{16\pi}}
\end{equation}
\emph{provided} $\Sigma$ also satisfies the favorable jump condition required by Theorem~\ref{thm:SPI_Core}.
\end{theorem}
...
\textbf{Step 2: Application of the direct MOTS theorem.}
Once the favorable jump condition is additionally available, Theorem~\ref{thm:SPI_Core} applies to $\Sigma$ and yields~\eqref{eq:NonSphericalPI}.
\end{proof}

\begin{remark}[Why Higher Genus is Excluded]
The restriction to spherical topology is not a limitation of our proof technique but a theorem: in dimension three under DEC, Galloway--Schoen \cite{gallowayschoen2006} implies every stable MOTS is a union of $2$-spheres.

Consequently, no genus-corrected Penrose inequality is needed in the three-dimensional stable-horizon setting treated here. For unstable or higher-genus MOTS, the present paper does \emph{not} prove a separate topological correction formula. Those cases would require additional arguments beyond the rigorous core used here.
\end{remark}

\begin{remark}[Proof Structure via Direct Construction]\label{rem:ReductionLogicSound}
\textbf{This remark clarifies that the Jang Reduction for MOTS (Theorem~\ref{thm:DirectTrappedJang}) enables a direct proof for MOTS satisfying the favorable jump condition.}

The Penrose inequality (Theorem~\ref{thm:MainTheorem}) is established for closed MOTS with $\tr_{\Sigma_0} k \ge 0$ \textbf{directly}, without reduction to the outermost MOTS:

\textbf{Direct Construction Approach (our method):}
\begin{enumerate}
    \item \textbf{Jang construction:} Given a MOTS $\Sigma_0$ with $\tr_{\Sigma_0} k \ge 0$, solve the Jang equation with blow-up forced at $\Sigma_0$ (Theorem~\ref{thm:DirectTrappedJang}).
    
    \item \textbf{Mean curvature jump:} The favorable jump hypothesis gives $[H]_{\bar{g}} = \tr_{\Sigma_0} k \ge 0$ at the interface.
...
\begin{equation*}
\text{MOTS } \Sigma_0 \text{ with } \tr k \ge 0 \xrightarrow{\text{Jang at } \Sigma_0} (\bar{M}, \bar{g}) \xrightarrow{[H] \ge 0} \text{AMO setup} \xrightarrow{\text{p-harmonic}} M_{\ADM} \ge \sqrt{\frac{A(\Sigma_0)}{16\pi}}.
\end{equation*}

\textbf{Key point:} No area comparison $A(\Sigma') \ge A(\Sigma_0)$ is needed. The problematic reduction via the outermost MOTS (which fails because inner MOTS can have larger area) is \textbf{completely bypassed}.
\end{remark}

\begin{corollary}[Direct MOTS Penrose Corollary]\label{cor:FinalUnconditional}
Let $(M, g, k)$ be any 3-dimensional asymptotically flat initial data set satisfying the Dominant Energy Condition with decay rate $\tau > 1$. Let $\Sigma$ be a \textbf{closed MOTS} satisfying the \textbf{favorable jump condition} $\tr_\Sigma k \ge 0$. Then:
\begin{equation}
    M_{\ADM}(g) \ge \sqrt{\frac{A(\Sigma)}{16\pi}}.
\end{equation}
\end{corollary}
...
\begin{enumerate}
    ...
    \item[(C)] \textbf{Horizon Properties:}
        \begin{itemize}
            \item \textbf{MOTS with favorable jump ($\tr_\Sigma k \ge 0$):} Direct proof via the Jang Reduction for MOTS (Theorem~\ref{thm:DirectTrappedJang}).
            \item \textbf{Unfavorable jump ($\tr_\Sigma k < 0$):} \textbf{OPEN.} The p-harmonic method does not apply directly. Requires area comparison or other techniques not fully resolved here.
...
\begin{enumerate}
    ...
    \item \textbf{Corner Smoothing and Correction:} $(\tM, \tg) \mapsto (\tM, \hat{g}_\epsilon) \mapsto (\tM,\geps)$, where the raw mollification has $\|R^-_{\hat{g}_\epsilon}\|_{L^{3/2}}\to0$, the corrected metric has $R_{\geps} \ge 0$, and $|M_{\ADM}(\geps) - M_{\ADM}(\tg)| \le C\epsilon$.
    \item \textbf{AMO Monotonicity:} $\mathcal{M}_p(1) \ge \mathcal{M}_p(0)$ on $(\tM, \geps)$ for $1 < p < 3$.
...
\begin{enumerate}
    ...
    \item[(V5)] \textbf{Area Stability:} $|A_{\geps}(\Sigma) - A_{\tg}(\Sigma)| \le C\epsilon$.
    \item[(V6)] \textbf{Mass Continuity:} $M_{\ADM}(\geps) \to M_{\ADM}(\tg)$ as $\epsilon \to 0$.
...
Combining all components, for any 3-dimensional initial data $(M, g, k)$ with DEC and $\tau > 1/2$, and any closed MOTS $\Sigma$ satisfying the \textbf{favorable jump condition} $\tr_\Sigma k \ge 0$:
\begin{equation}
...
\textbf{Step 2 (Direct Construction for MOTS with favorable jump):}
By the Jang Reduction for MOTS (Theorem~\ref{thm:DirectTrappedJang}), the Penrose inequality holds for any closed MOTS $\Sigma$ satisfying:
\begin{itemize}
    \item $\theta^+ = 0$ (MOTS condition), and
...
\begin{itemize}
    ...
    \item Apply Miao corner smoothing and conformal correction to obtain $(\tM, \geps)$ with $R_{\geps} \ge 0$.
    \item Apply AMO monotonicity: $\mathcal{M}_p(1; \geps) \ge \mathcal{M}_p(0; \geps)$.
...
\begin{itemize}
    \item Apply Miao corner smoothing and conformal correction to obtain $(\tM, \geps)$ with $R_{\geps} \ge 0$.
    \item Apply AMO monotonicity (Theorem~\ref{thm:AMOMonotonicity}): $\mathcal{M}_p(1; \geps) \ge \mathcal{M}_p(0; \geps)$.
    \item Take $p \to 1^+$: $M_{\ADM}(\geps) \ge \sqrt{A_{\geps}(\Sigma)/(16\pi)}$.
\end{itemize}
...
The conditional framework developed in this section provides:
\begin{itemize}
...
\end{itemize}
Together, these tools yield a sharp Penrose inequality in the direct Jang regime: MOTS satisfying the favorable jump condition together with the dominant energy condition.

\begin{theorem}[Conditional Penrose Inequality Summary]\label{thm:UltimateUnconditional}
Let $(M^3, g, k)$ be an asymptotically flat initial data set for Einstein's equations. Then the following hold:
\begin{enumerate}
    \item \textbf{Under DEC with $\tau > 1/2$:} For any closed MOTS $\Sigma \subset M$ satisfying the favorable jump condition $\tr_\Sigma k \ge 0$:
    \begin{equation}
    ...
    \item \textbf{Under quantitative DEC violation ($\mathcal{D} < \infty$):} Under the same geometric hypotheses on $\Sigma$,
    \begin{equation}
    ...
    \item \textbf{Stability:} The inequality is stable under perturbations inside the same hypothesis class: if $(M_n, g_n, k_n) \to (M, g, k)$ in an appropriate sense and $\Sigma_n \to \Sigma$, with each $\Sigma_n$ a MOTS satisfying $\tr_{\Sigma_n}k_n \ge 0$, then:
    \begin{equation}
...
\textbf{Part 1:} This is the main content of Sections~\ref{sec:Unconditional}--\ref{sec:Synthesis} for MOTS satisfying the favorable jump condition. The key steps are:
\begin{itemize}
    ...
    \item \textbf{Interface positivity (Theorem~\ref{thm:CompleteMeanCurvatureJump}):} $[H] = \tr_\Sigma k \ge 0$ under the favorable-jump hypothesis.
    \item \textbf{Corner smoothing (Proposition~\ref{prop:CollarBound}):} the raw mollification has controlled negative part and the corrected metric has $R_{\geps} \ge 0$.
...
\textbf{Part 3:} Stability follows from the continuity of all constructions under appropriate convergence of metrics and second fundamental forms, combined with the semicontinuity of the ADM mass and the continuity of area, as long as the favorable jump hypothesis persists along the approximating sequence.
...
This paper provides a complete and rigorous proof of the spacetime Penrose inequality in dimension 3 for \textbf{outermost MOTS} (apparent horizons) with standard decay ($\tau > 1$), under the favorable jump hypothesis. For \textbf{general trapped surfaces}, the proof requires one of: (A) reduction to a MOTS in the direct favorable-jump regime, (B) compactness conditions (C1)--(C3) plus a sign input giving favorable jump on the comparison MOTS, or (C) a Lorentzian area-comparison input. The proof addresses:
\begin{itemize}
    \item \textbf{MOTS with favorable jump $\tr_{\Sigma_0} k \ge 0$:} Direct proof via Jang Reduction (Theorem~\ref{thm:DirectTrappedJang}).
    \item \textbf{General trapped surfaces:} Two-stage reduction requires either compactness plus the favorable-jump sign input for the comparison MOTS, or a Lorentzian area-comparison input (Program~F).
...
\end{itemize}
The essential hypotheses are: (i) the Dominant Energy Condition, (ii) asymptotic flatness with $\tau > 1$, and (iii) \textbf{for general trapped surfaces}, one of the three reduction hypotheses above.
\end{remark}

\begin{remark}[Frequently Asked Questions on Mathematical Rigor]\label{rem:RigorFAQ}
We address several natural questions regarding the precise scope of the proof.

\textbf{(Q1) What happens if $\theta^- = 0$ (marginally inner trapped)?}
If $\Sigma$ is a MOTS with $\theta^- = H_\Sigma - \tr_\Sigma k = 0$, then $C_0 = |\theta^-|/2 = 0$ and the logarithmic blow-up mechanism disappears. In this degenerate regime the corner argument used for strictly trapped surfaces is not the one driving the proof, and the manuscript does not claim a separate general reduction from this observation alone. Compare Remark~\ref{prop:MCJumpSharpness} for the precise rigorous scope.
...
For the \textbf{direct Jang proof}, outermostness is \emph{not} the essential point. The usable hypothesis is the favorable jump condition $\tr_\Sigma k \ge 0$, and the proof applies to any MOTS satisfying that condition. A separate \textbf{area-comparison route} would use the two-stage reduction
\begin{enumerate}
    \item \textbf{Lorentzian/variational area comparison:} compare $A(\Sigma_0)$ with the area of an enclosing outermost MOTS $\Sigma^*$;
    \item \textbf{MOTS Penrose:} apply the Jang-based proof to $\Sigma^*$ once the favorable jump condition is available there;
    \item Transfer the bound from $\Sigma^*$ back to $\Sigma_0$.
\end{enumerate}
What is \emph{proved rigorously in this paper} is the direct route for MOTS with favorable jump. The outermost property is useful for stability and for formulating area-comparison strategies, but it does not by itself remove the favorable-jump issue.

\textbf{Historical note:} Earlier drafts overstated the status of the area-comparison step. In the present version, the direct Jang proof is the rigorous core result, while any comparison between a general trapped surface and an enclosing outermost MOTS is treated as an additional hypothesis rather than as a theorem proved here.
...
No. The sharp theorem proved here is still \textbf{conditional}: for the direct Jang route one assumes the favorable jump condition on the MOTS under consideration.

For \textbf{general trapped surfaces} $\Sigma_0$, the Penrose inequality requires one of: a reduction to the direct MOTS regime, compactness plus a sign calibration converting the adjoint-cone law into the favorable jump, or a separate Lorentzian area-comparison input. These are precisely the places where the proof remains conditional.
...
\begin{theorem}[Conditional Extension under DEC Violation]\label{thm:ModifiedPenrose}
Let $(M, g, k)$ be a 3-dimensional asymptotically flat initial data set with decay $\tau > 1/2$ and finite total DEC deficit $\mathcal{D} < \infty$. Let $\Sigma$ be a closed trapped surface. Assume, in addition, that the signed-curvature estimates and mass-deficit bounds described in Steps 1--4 below hold uniformly along the Jang--conformal reduction. Then:
...
\begin{proof}
The proof is a conditional bookkeeping argument showing what would follow once the Jang, conformal, and AMO estimates remain stable under a controlled DEC deficit.
...
This is an independent problem from the main Penrose inequality (Theorems~\ref{thm:SPI_Core} and~\ref{thm:SPI}), which holds without any such extension under the standard DEC. The present subsection should therefore be read as a \emph{conditional extension program}, not as part of the rigorous core theorem.
\end{remark}

\begin{remark}[Physical Interpretation]
The modified inequality~\eqref{eq:ModifiedPenrose}, if the required analytical estimates can be justified, says that an \emph{effective} mass $M_{\mathrm{eff}} = M_{\ADM} + C_0 \mathcal{D}$ would satisfy the Penrose bound. In the limit $\mathcal{D} \to 0$, one formally recovers the standard DEC case.
\end{remark}
...
The weight function $W$ in the Jang equation satisfies $W = (1 + |\nabla f|^2)^{-1/2}$, where $f$ is the Jang graph function. By the asymptotic analysis of Han--Khuri \cite{hankhuri2013}, on an asymptotically flat end with decay $\tau > 1/2$:
\begin{equation}
...
\textbf{Status: CONDITIONAL LORENTZIAN ROUTE.} This section isolates the additional \emph{spacetime} input one would need in order to bypass the Jang equation. It is \textbf{not} part of the rigorous core proof of the paper. The key observation is that:
\begin{itemize}
    \item All trapped surfaces have \textbf{negative mean curvature} ($H < 0$), independent of the sign of $\tr_\Sigma k$.
    \item A direct 4D spacetime argument would replace the Jang favorable-jump condition by a genuinely Lorentzian area-comparison input.
    \item Hawking's area theorem and the Kerr bound then provide the remaining inequalities once that comparison is available.
\end{itemize}

\textbf{Assumptions:} In addition to weak cosmic censorship (existence of an event horizon) and global hyperbolicity, one needs a global comparison principle relating the area of a trapped surface to an event-horizon cross-section. We record that comparison explicitly below instead of claiming it from WCC alone.
\end{remark}

\begin{theorem}[Universal Negativity of Mean Curvature]\label{thm:UniversalH}\label{rem:UniversalMeanCurvature}
...
\begin{theorem}[Lorentzian Area-Comparison Input]\label{thm:HAD}
Let $(N^{3+1}, \bar{g})$ be a globally hyperbolic spacetime satisfying the null energy condition (NEC) and weak cosmic censorship (WCC). Let $\Sigma$ be any closed trapped surface with $\theta^+ \le 0$ and $\theta^- < 0$. Assume, in addition, that $\Sigma$ satisfies the horizon-area comparison
\begin{equation}
    A(\Sigma) \le A(\mathcal{H}_M),
\end{equation}
where $\mathcal{H}_M = \mathcal{H}^+ \cap M$ is the event horizon cross-section on any Cauchy surface $M$ containing $\Sigma$.
\end{theorem}
\begin{proof}
There is nothing further to prove: the displayed inequality is the extra Lorentzian input isolated for Program~F. The point is that WCC, by itself, does not furnish this comparison in the rigorous core argument developed in the paper.
\end{proof}

\begin{lemma}[Area Comparison via Past-Directed Null Focusing]\label{lem:AreaComparison}
Under the additional area-comparison input of Theorem~\ref{thm:HAD}, any trapped surface $\Sigma_0$ satisfies $A(\Sigma_0) \le A(\mathcal{H}_\mathcal{C})$, where $\mathcal{H}_\mathcal{C}$ is the relevant event-horizon cross-section. This is the Lorentzian step needed to convert trapped-surface area into horizon area.
\end{lemma}

\begin{theorem}[Penrose Inequality under Cosmic Censorship --- Penrose's Original 1973 Statement]\label{thm:Penrose1973Complete}
Let $(N^{3+1}, \bar{g})$ be a globally hyperbolic spacetime satisfying:
\begin{itemize}
    \item[\textup{(NEC)}] Null energy condition: $R_{\mu\nu} k^\mu k^\nu \ge 0$ for all null $k^\mu$.
    \item[\textup{(WCC)}] Weak cosmic censorship: the spacetime possesses an event horizon $\mathcal{H}^+$ and the black hole settles to a Kerr final state.
    \item[\textup{(AC)}] Area comparison: every closed trapped surface $\Sigma$ on the Cauchy slice under consideration satisfies $A(\Sigma) \le A(\mathcal{H}_M)$.
\end{itemize}
Let $\Sigma$ be any closed trapped surface. Then:
\begin{equation}
...
\end{theorem}
\begin{proof}
By Lemma~\ref{lem:AreaComparison}, $A(\Sigma) \le A(\mathcal{H}_M)$. By the Hawking area theorem, $A(\mathcal{H}_M) \le A(\mathcal{H}_\infty)$. Under WCC, the final state is Kerr with mass $M_f \ge \sqrt{A(\mathcal{H}_\infty)/(16\pi)}$. By Bondi mass loss, $M_{\mathrm{ADM}} \ge M_f$. Combining gives the inequality.
\end{proof}

\begin{theorem}[Direct Spacetime Penrose Inequality]\label{thm:DirectSpacetime}
Let $(N^{3+1}, \bar{g})$ be a globally hyperbolic spacetime satisfying the null energy condition and weak cosmic censorship, and assume the Lorentzian area-comparison input of Theorem~\ref{thm:HAD}. Let $(M, g, k)$ be asymptotically flat initial data embedded in $N$, and let $\Sigma \subset M$ be any trapped surface with $\theta^+ \le 0$ and $\theta^- < 0$.
...
By Theorem~\ref{thm:HAD} (proved above), any trapped surface $\Sigma$ satisfies:
\begin{equation}
    A(\Sigma) \le A(\mathcal{H}_M),
\end{equation}
...
\begin{itemize}
    ...
    \item No sign condition on $\tr_\Sigma k$ appears explicitly, but one must assume a separate Lorentzian area-comparison input.
    \item Requires weak cosmic censorship, global hyperbolicity, and horizon-area comparison.
    \item Conceptually cleaner, but mathematically stronger and not part of the paper's rigorous core proof.
\end{itemize}

Both approaches lead to the same formal chain of inequalities once their respective hypotheses are granted. In this paper, however, only the Jang approach is part of the rigorous core proof.
\end{remark}

\begin{remark}[Physical Interpretation of Trapping]
For a future trapped surface the invariant statement is that both future null expansions are negative (or nonpositive in the weak case). Equivalently, the mean-curvature vector has the corresponding causal orientation. The scalar Riemannian mean curvature $H$ on a chosen initial-data slice need not be negative by itself when $k\neq0$; it is the combinations $H\pm\tr_\Sigma k$ that encode trapping.

The Penrose inequality measures whether there is ``enough mass to account for the trapped region.'' Trapped surfaces represent genuine gravitational collapse in this null-expansion sense; this motivates the Lorentzian route above, but the required universal area comparison remains an additional assumption rather than a theorem proved here.
\end{remark}
...
The rigorous proof combines the GJE reduction, a metric deformation to resolve these issues (following Bray and Khuri \cite{braykhuri2010}), and the application of the level set method for the Riemannian Penrose Inequality. In this framework, we employ the Nonlinear Level Set Method (AMO) \cite{amo2024}.
