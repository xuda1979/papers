...
\begin{itemize}
    ...
    \item \textbf{Trapped Surface Penrose (conditional):} For arbitrary trapped surfaces $\Sigma_0$, we require one of: (i) reduction to a MOTS in the direct favorable-jump regime, (ii) compactness conditions (C1)--(C3), or (iii) cosmic censorship together with the needed Lorentzian area comparison.
...
\begin{equation}
    M_{\ADM}(g) \ge \sqrt{\frac{A(\Sigma)}{16\pi}}.
\end{equation}
\textbf{Note:} This result is \textbf{conditional} on the favorable jump assumption $\tr_\Sigma k \ge 0$ (or equivalent gauge conditions). For general trapped surfaces, one additionally needs either a reduction to the direct MOTS regime, compactness, or a Lorentzian area-comparison input---see Theorem~\ref{thm:MainTheorem}.
\end{theorem}
...
The Penrose inequality measures whether there is ``enough mass to account for the trapped region.'' Since all trapped surfaces have $H < 0$, they all represent genuine gravitational collapse; this motivates the Lorentzian route above, but the required universal area comparison remains an additional assumption rather than a theorem proved here.
\end{remark}
