...
This section records the part of the constrained-variation calculation that is actually justified in the marginally stable case. In particular, the earlier claim that marginal stability forces the \emph{pointwise} identity $\tr_\Sigma k \equiv 0$ was too strong.

\begin{theorem}[First-Order Consequence in the Marginally Stable Case]\label{thm:SecondVariationComplete}
Let $\Sigma$ be a marginally stable MOTS ($\lambda_1(L_\Sigma) = 0$) that is a constrained area maximum in $\mathcal{A} = \{\theta^+ \le 0\}$. Let $\psi_0 > 0$ be the kernel eigenfunction with $L_\Sigma[\psi_0] = 0$. Then the first-order optimality condition implies only the weighted orthogonality relation
\begin{equation}
    \int_\Sigma (\tr_\Sigma k)\psi_0\, dA = 0.
\end{equation}
No pointwise sign conclusion for $\tr_\Sigma k$ is obtained from this argument alone.
\end{theorem}
...
\textbf{Step 5: Where the earlier argument overreached.}
The relations above yield only the weighted orthogonality~\eqref{eq:IntegralVanish}. They do \emph{not} imply that $\tr_\Sigma k$ has a fixed sign, and they do not justify the construction of admissible test functions supported on the positive or negative sets of $\tr_\Sigma k$ while preserving the constraint $L_\Sigma[\phi] \le 0$.

\textbf{Step 6: Correct conclusion.}
Hence the constrained first-order analysis proves
\[
    \int_\Sigma (\tr_\Sigma k)\psi_0\, dA = 0,
\]
but no further pointwise conclusion follows without additional structure. In particular, this appendix does \emph{not} prove $\tr_\Sigma k \equiv 0$ or $\tr_\Sigma k \ge 0$ in the marginally stable case.
\end{proof}

\begin{remark}[Connection to the earlier vanishing-multiplier heuristic]
This appendix now records only the weighted orthogonality relation available from first-order constrained optimality. It does \emph{not} justify the stronger pointwise sign claims that appeared in earlier drafts.
\end{remark}
...
\textit{(b) Lipschitz interface $\Sigma$:} By the transmission lemma (Lemma~\ref{lem:Transmission}), $\phi \in C^{1,\alpha}$ across $\Sigma$. The corresponding Bray--Khuri flux has matching one-sided normal traces, so:
...
\begin{enumerate}
    \item[(I)] For the conformally corrected metrics $g_\epsilon=u_\epsilon^4\hat g_\epsilon$, $M_{\ADM}(g_\epsilon) \to M_{\ADM}(\tg)$;
...
The scalar curvature of $\hat g_\epsilon$ contains an $O(\epsilon^{-1})$ positive spike approximating the interface measure and a uniformly bounded negative part supported in $N_{2\epsilon}$. Thus a crude absolute-value estimate is not sufficient for an $O(\epsilon)$ mass bound; one must use distributional convergence of the scalar curvature measure together with the conformal correction. The estimates used here are:
\begin{equation}\label{eq:CurvatureDiff}
    \|R^-_{\hat g_\epsilon}\|_{L^{3/2}} \le C\epsilon^{2/3},
    \qquad
    u_\epsilon \to 1 \text{ in } C^0_{\mathrm{loc}} \text{ and at infinity},
\end{equation}
where $g_\epsilon=u_\epsilon^4\hat g_\epsilon$ and $R_{g_\epsilon}\ge0$. The conformal mass formula and the smallness of $u_\epsilon-1$ at infinity give
\begin{equation}\label{eq:MassBound}
    M_{\ADM}(g_\epsilon) \to M_{\ADM}(\tg).
\end{equation}

\textbf{Step 2: Energy difference bound (Part II).}
Let $u_{p,\epsilon}$ and $u_p$ be $p$-harmonic functions on $(\tM, g_\epsilon)$ and $(\tM, \tg)$ respectively, with boundary data $u = 0$ on $\Sigma$ and $u \to 1$ at infinity.

The $p$-energy difference is:
\begin{align}
    |E_{p,\epsilon} - E_p| &= \left| \int_{\tM} |\nabla u_{p,\epsilon}|^p_{g_\epsilon} \, dV_{g_\epsilon} - \int_{\tM} |\nabla u_p|^p_{\tg} \, dV_{\tg} \right| \nonumber\\
    &\le \int_{N_{2\epsilon}} \left| |\nabla u_{p,\epsilon}|^p_{g_\epsilon} - |\nabla u_p|^p_{\tg} \right| dV + O(\text{outside collar}). \label{eq:EnergyDiff}
\end{align}
...
\begin{equation}\label{eq:GradientPerturbation}
    \left| |\nabla u|_{g_\epsilon} - |\nabla u|_{\tg} \right| \le C \|g_\epsilon - \tg\|_{C^0} |\nabla u| = o(1)\,|\nabla u|.
\end{equation}

For the $p$-th power:
\begin{equation}\label{eq:PowerPerturbation}
    \left| |\nabla u|^p_{g_\epsilon} - |\nabla u|^p_{\tg} \right| \le p C_T^{p-1}\,o(1)\,C_T.
\end{equation}
...
Condition (a) holds because for fixed $\epsilon > 0$, the smooth metric $g_\epsilon$ satisfies all AMO hypotheses, and the standard AMO convergence theorem applies.
...
Let $(M^3, g, k)$ be an asymptotically flat initial data set with $\tau > 1/2$ satisfying the Dominant Energy Condition $\mu \ge |J|_g$. Let $\Sigma \subset M$ be a MOTS with $\theta^+=0$, $\theta^-<0$, and favorable jump $\tr_\Sigma k \ge 0$. Then:
...
By Theorem~\ref{thm:DirectTrappedJang}, there exists a solution $f: M \setminus \Sigma \to \mathbb{R}$ to the generalized Jang equation:
\begin{equation}\label{eq:GJEFinal}
    H_{\Gamma(f)} - \tr_{\Gamma(f)}(k) = 0,
\end{equation}
where $\Gamma(f) = \{(x, f(x)) : x \in M \setminus \Sigma\}$ is the graph in $(M \times \mathbb{R}, g + dt^2)$. The solution blows up at the prescribed surface $\Sigma$.
...
\begin{equation}\label{eq:JumpPositiveFinal}
    [H]_{\bg} = \tr_\Sigma k \ge 0,
\end{equation}
so the interface contribution has the correct sign.
...
\begin{enumerate}
    \item the raw smoothing has $\|R^-_{\hat{g}_\epsilon}\|_{L^{3/2}}\le C\epsilon^{2/3}$;
    \item the conformally corrected metric $\geps=u_\epsilon^4\hat{g}_\epsilon$ has $R_{\geps} \ge 0$ pointwise;
    \item $|M_{\mathrm{ADM}}(\geps) - M_{\mathrm{ADM}}(\tg)| \le C_M \epsilon$;
    \item $|A_{\geps}(\Sigma_\epsilon) - A_{\tg}(\Sigma)| \le C_A \epsilon$.
\end{enumerate}

By the AMO monotonicity theorem (Theorem~\ref{thm:AMOMonotonicity}), for each smooth corrected metric $(\tM, \geps)$:
\begin{equation}\label{eq:AMOAppliedFinal}
    M_{\mathrm{ADM}}(\geps) \ge \sqrt{\frac{A_{\geps}(\Sigma_\epsilon)}{16\pi}}.
\end{equation}
...
\begin{align}
    M_{\mathrm{ADM}}(\tg) &= \lim_{\epsilon \to 0} M_{\mathrm{ADM}}(\geps) \quad \text{(mass continuity)} \nonumber\\
    &\ge \lim_{\epsilon \to 0} \sqrt{\frac{A_{\geps}(\Sigma_\epsilon)}{16\pi}} \quad \text{(AMO on smooth approximants)} \nonumber\\
...
\begin{tikzpicture}[node distance=1.8cm, auto, >=Stealth, thick,
    ...
    \node[box, below=of jang] (stab) {Favorable jump\\$\tr_\Sigma k \ge 0$};
...
Thus the formal leading-order exterior mean curvature is:
...
At the same purely formal level,~\eqref{eq:MCExterior} and~\eqref{eq:MCCylinder} suggest the model value
\begin{equation}
    [H]_{\bg}^{\mathrm{model}} = 0 - \left( -\frac{1}{C_0} \right) = \frac{1}{C_0}.
\end{equation}
This is not a rigorous identity for the actual interface jump.

\textbf{Step 5: Scope of the asymptotic calculation.}

The computation above is only a \textit{formal leading-order model} for the bulk-side mean curvature in the chosen collar coordinates. It should \emph{not} be read as a rigorous universal formula for the jump. In particular:
\begin{itemize}
    \item the model value $[H]_{\bg}^{\mathrm{model}} = 1/C_0$ is not established as the actual jump;
    \item the spectral correction formula
    \begin{equation}\label{eq:MCJumpStability}
        [H]_{\bg}^{\mathrm{model}} = \frac{1}{C_0} + O(\lambda_1)
    \end{equation}
    should be regarded only as heuristic bookkeeping for subleading terms;
    \item no exact conclusion for the marginally stable case follows from this expansion.
\end{itemize}

\textbf{Rigorous sign input used in the paper.}
The rigorous argument elsewhere uses the explicit favorable-jump hypothesis
\[
    [H]_{\bg} = \tr_\Sigma k \ge 0
\]
at the MOTS interface. This appendix no longer attempts to derive that sign from stability or from the asymptotic constant $C_0$.
\end{proof}
...
\textit{Flux at interface ($\delta \to 0$):} By Lemma~\ref{lem:Transmission}, $\phi \in C^{1,\alpha}$ across $\Sigma$, and the associated Bray--Khuri flux satisfies the weak transmission condition. Hence:
\begin{equation}
    [Y \cdot \nu]_\Sigma = 0.
\end{equation}
...
Let $\{\geps\}_{\epsilon > 0}$ be the conformally corrected smooth approximations to the Jang-conformal metric $\tg$ with $R_{\geps} \ge 0$. Let $\mathcal{M}_{p,\epsilon}(t)$ be the AMO monotonicity functional for $p$-harmonic potentials on $(\tM, \geps)$. Then:
...
For fixed $p \in (1, 2]$, the $p$-harmonic potential $u_{p,\epsilon}$ on $(\tM, \geps)$ converges to $u_p$ on $(\tM, \tg)$ as $\epsilon \to 0$ by Mosco convergence of the $p$-energy functionals:
\begin{equation}
    \mathcal{E}_{p,\epsilon}(v) := \int_{\tM} |\nabla v|^p_{\geps} \, dV_{\geps} \xrightarrow{\text{Mosco}} \mathcal{E}_p(v) := \int_{\tM} |\nabla v|^p_{\tg} \, dV_{\tg}.
\end{equation}

The AMO functional $\mathcal{M}_{p,\epsilon}(t)$ depends continuously on $u_{p,\epsilon}$ and the metric. Since $\geps \to \tg$ in $C^0$ (Proposition~\ref{prop:CollarBound} together with the conformal correction estimate), we have:
\begin{equation}
...
\begin{equation}\label{eq:UniformPLimit}
    \sup_{\epsilon \in (0, \epsilon_0]} \left| \mathcal{M}_{p,\epsilon}(0) - \sqrt{\frac{A_{\geps}(\Sigma_\epsilon)}{16\pi}} \right| \to 0 \quad \text{as } p \to 1^+.
\end{equation}

The AMO identification theorem on smooth manifolds gives:
\begin{equation}
    \mathcal{M}_{p,\epsilon}(0) = \sqrt{\frac{A_{\geps}(\Sigma_\epsilon)}{16\pi}} + O((p-1)^{1/2}),
\end{equation}
where the implied constant depends on:
\begin{itemize}
    \item The Sobolev constant of $(\tM, \geps)$;
    \item The gradient bound $\|\nabla u_{p,\epsilon}\|_{L^\infty}$;
    \item The curvature integral $\|R_{\geps}\|_{L^1}$.
\end{itemize}

\textit{Uniform bounds:} By Proposition~\ref{prop:CollarBound} and the conformal correction estimates, all corrected metrics $\geps$ have:
\begin{align}
    \Lambda^{-1} |\xi|^2 \le \geps(\xi, \xi) &\le \Lambda |\xi|^2 \quad \text{(uniform ellipticity)}, \\
    \|R_{\geps}\|_{L^{3/2}(\tM)} &\le C \quad \text{(curvature bound)}, \\
    C_S(\geps) &\ge c_0 > 0 \quad \text{(Sobolev constant)}.
\end{align}
...
\begin{equation}
    \sup_{\epsilon} \left| \mathcal{M}_{p,\epsilon}(0) - \sqrt{\frac{A_{\geps}(\Sigma_\epsilon)}{16\pi}} \right| \le C \cdot (p-1)^{1/2} \to 0.
\end{equation}
...
\begin{equation}
    |A_{\geps}(\Sigma_\epsilon) - A_{\tg}(\Sigma)| \le C \epsilon.
\end{equation}

Thus:
\begin{equation}
    h(\epsilon) := \lim_{p \to 1^+} \mathcal{M}_{p,\epsilon}(0) = \sqrt{\frac{A_{\geps}(\Sigma_\epsilon)}{16\pi}} \to \sqrt{\frac{A(\Sigma)}{16\pi}} \quad \text{as } \epsilon \to 0.
\end{equation}
...
\begin{theorem}[Complete Direct MOTS Penrose Inequality]\label{thm:CompleteSPI}
Let $(M^3, g, k)$ be asymptotically flat initial data satisfying the DEC with standard decay $\tau > 1$. Let $\Sigma$ be a MOTS with $\theta^+=0$, $\theta^-<0$, and favorable jump $\tr_\Sigma k \ge 0$. Then:
\begin{equation}
    M_{\mathrm{ADM}}(g) \ge \sqrt{\frac{A(\Sigma)}{16\pi}}.
\end{equation}
\end{theorem}
...
By Theorem~\ref{thm:DirectTrappedJang}, there exists a solution $f$ to the generalized Jang equation blowing up at $\Sigma$. The Jang metric $\bg = g + df \otimes df$ satisfies:
\begin{equation}
...
By Miao's technique plus the conformal correction, there exist smooth metrics $\geps \to \tg$ with $R_{\geps} \ge 0$ and:
\begin{align}
    |M_{\mathrm{ADM}}(\geps) - M_{\mathrm{ADM}}(\tg)| &\le C\epsilon, \\
    |A_{\geps}(\Sigma_\epsilon) - A(\Sigma)| &\le C\epsilon.
\end{align}

\textbf{Step 4: AMO monotonicity.}

On each smooth $(\tM, \geps)$ with $R_{\geps} \ge 0$, the AMO theorem gives:
\begin{equation}
    M_{\mathrm{ADM}}(\geps) \ge \sqrt{\frac{A_{\geps}(\Sigma_\epsilon)}{16\pi}}.
\end{equation}
