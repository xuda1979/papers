...
\textbf{Key Conceptual Point:} The mean curvature jump $[H]_{\bg}$ is \emph{not} defined as the difference of pointwise limits (which would give $0 - 0 = 0$), but rather as the coefficient of the Dirac measure in the distributional scalar curvature. We now derive this rigorously.
...
\begin{equation}\label{eq:ExtMCLimit}
    \lim_{s_0 \to 0^+} H^{\bg,+}(s_0) = 0 \quad \text{(pointwise limit)}.
\end{equation}

\textbf{Step 7d-i: Distributional definition via Gauss--Codazzi.}
...
\end{equation}
where $\mathcal{H}^2|_\Sigma$ is the 2-dimensional Hausdorff measure on $\Sigma$, and $[H]_{\bg}$ is determined by the integration-by-parts formula: for any test function $\varphi \in C^\infty_c(\bM)$,
\begin{equation}\label{eq:IBPDefinition}
    \int_{\bM} R_{\bg}^{\mathrm{dist}} \varphi \, dV_{\bg} = \lim_{\delta \to 0} \left( \int_{\{|s|>\delta\}} R_{\bg}^{\mathrm{bulk}} \varphi \, dV_{\bg} + 2\int_{\{s=\delta\}} H^+ \varphi \, dA - 2\int_{\{s=-\delta\}} H^- \varphi \, dA \right).
\end{equation}

\textbf{Step 7d-ii: Extracting $[H]$ from the expansion rate.}
The key insight is that $[H]_{\bg}$ encodes the \emph{rate} at which $H^{\bg,+}(s)$ approaches zero, not the limit itself. From~\eqref{eq:MCWithCorrection}, the dominant behavior for small $s > 0$ is:
\begin{equation}\label{eq:MCExpansionRate}
    H^{\bg,+}(s) \cdot \sqrt{\det \bg|_{\{s\}}} = \frac{s}{\sqrt{s^2 + C_0^2}} \cdot (1 + C_0^2)^{1/2} \cdot H_\Sigma \cdot \Area(\Sigma) + O(s^{1+\alpha}).
\end{equation}
The prefactor $\frac{s}{\sqrt{s^2 + C_0^2}} \cdot (1 + C_0^2)^{1/2} = \frac{s \sqrt{1+C_0^2}}{\sqrt{s^2 + C_0^2}}$ approaches $s/C_0$ for $s \ll C_0$.

For the \emph{cylindrical side}, the coordinate transformation $t = -\ln s$ gives level sets $\{t = T\}$ with $T \to \infty$ as $s \to 0^+$. On the product cylinder, these have $H^- = 0$.

\textbf{Step 7d-iii: Spectral decomposition and the jump formula.}
The stability operator $L_\Sigma$ has eigenvalues $0 = \lambda_1 \le \lambda_2 \le \cdots$ (using 1-indexing). The coefficient $D(y)$ in the Jang expansion~\eqref{eq:JangExpansion} satisfies the elliptic equation $L_\Sigma D = -C_0 \mathcal{G}$. 

For strictly stable MOTS ($\lambda_1 > 0$), the Green's function analysis gives:
\begin{equation}\label{eq:GreenDecomposition}
    D(y) = -C_0 \sum_{k=1}^\infty \frac{\langle \mathcal{G}, \psi_k \rangle}{\lambda_k} \psi_k(y),
\end{equation}
where $\psi_k$ are the $L^2$-normalized eigenfunctions. The leading term is:
\begin{equation}\label{eq:DLeadingTerm}
    D(y) = -\frac{C_0}{\lambda_1} \langle \mathcal{G}, \psi_1 \rangle \psi_1(y) + O(\lambda_2^{-1}).
\end{equation}

\textbf{Step 7d-iv: Computing the distributional coefficient.}
Substituting back into the IBP formula~\eqref{eq:IBPDefinition} and using the explicit expansion, the coefficient of $\mathcal{H}^2|_\Sigma$ in $R_{\bg}^{\mathrm{dist}}$ is:
\begin{equation}\label{eq:JumpFromIBP}
    [H]_{\bg} = \lim_{\delta \to 0} \frac{1}{2\delta} \left( \int_0^\delta H^+(s) \, ds - \int_{-\delta}^0 H^-(s) \, ds \right) \cdot (\text{area normalization}).
\end{equation}

The detailed calculation (expanding $H^+(s)$ to order $s^{1+\alpha}$ and integrating) yields:
...
\begin{remark}[Why Pointwise Limits Give Zero but Distributional Jump is Nonzero]\label{rem:PointwiseVsDistributional}
This apparent paradox is resolved by understanding that $[H]_{\bg}$ measures the \emph{singular concentration} of curvature at the interface. Analogously, the Dirac delta $\delta(x)$ satisfies $\lim_{x \to 0} \delta_\epsilon(x) = 0$ for any mollifier $\delta_\epsilon$, but $\int \delta(x) dx = 1$. Similarly, $\lim_{s \to 0} H^+(s) = 0$ pointwise, but the \emph{integral} $\int_0^\delta H^+(s) ds$ captures a finite contribution that becomes the Dirac coefficient.
\end{remark}
...
This is a direct consequence of the derivation in \S\ref{app:MCJDerivation}; we record it in the form used in the main text.

\begin{theorem}[Mean Curvature Jump Positivity]\label{thm:MCJumpComplete}
Let $(M^3,g,k)$ be asymptotically flat initial data satisfying the DEC, and let $\Sigma\subset M$ be a stable MOTS with stability eigenvalue $\lambda_1(L_\Sigma)\ge 0$. Let $\bg=g+df\otimes df$ be the induced Jang metric of the blow-up solution $f$ at $\Sigma$. With the convention
\begin{equation}\label{eq:MCJumpFinal}
    [H]_{\bg}:=H^{\bg,+}-H^{\bg,-},
\end{equation}
where $+$ denotes the asymptotically flat (exterior) side and $-$ the cylindrical side, one has
\begin{equation}
    [H]_{\bg}=\frac{2\lambda_1 C_0}{1+C_0^2}+O(\lambda_1^2)\ge 0.
\end{equation}
\end{theorem}
...
\begin{proof}
This is exactly Theorem~\ref{thm:MCJComplete}.
\end{proof}
