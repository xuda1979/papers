...
\begin{itemize}
    ...
    \item \textbf{(H3) Outermost MOTS:} Han--Khuri work with outermost MOTS to ensure the barrier argument applies. For non-outermost trapped surfaces, we apply the conditional Theorem~\ref{thm:trappedcond} under one of its three conditions (favorable jump, compactness, or cosmic censorship).
\end{itemize}
...
\textbf{Important clarification (Han--Khuri nonuniqueness):} This uniqueness is \textbf{conditional} on the prescribed blow-up surface $\Sigma$. Han and Khuri \cite{hankhuri2013} prove that different choices of blow-up surfaces (e.g., different MOTS in the same initial data) yield different Jang solutions---this is nonuniqueness of the \emph{blow-up locus}, not nonuniqueness for a fixed locus. Our theorem states: given a fixed blow-up surface $\Sigma$, the Jang solution is unique up to vertical translation. The Direct Trapped Surface Construction (Theorem~\ref{thm:DirectTrappedJang}) prescribes $\Sigma = \Sigma_0$ (the trapped surface of interest), making the construction well-defined.
...
\begin{enumerate}
    ...
    \item \textbf{A priori bounds from comparison:} On $\Omega_\delta$, the subsolution $\underline{f} = C_0^{\min} \ln(s^{-1}) - C_1$ satisfies $\underline{f}|_{\partial\Omega_\delta} = C_0^{\min}\ln(\delta^{-1}) - C_1 < C_0^{\max}\ln(\delta^{-1}) = f_\delta|_{\partial\Omega_\delta}$ (choosing $C_1$ large). By the maximum principle for the Jang operator, $f_\delta \ge \underline{f}$ on $\Omega_\delta$.
...
\textbf{Step 5: Existence via Arzelà--Ascoli.}
...
\end{itemize}
By Arzelà--Ascoli, a subsequence $f_{\delta_j} \to f$ converges in $C^{2,\alpha}_{\text{loc}}(M \setminus \Sigma_0)$ to a solution $f$ of the Jang equation. The lower bound ensures $f(s,y) \to +\infty$ as $s \to 0$, with the correct logarithmic rate $f \sim C_0(y) \ln(s^{-1})$ where $C_0(y) = |\theta^-(y)|/2$.
...
\begin{equation}\label{eq:FavorableJump}
    \boxed{\tr_{\Sigma_0} k \ge 0 \quad \text{(favorable jump condition)}.}
\end{equation}
...
\[
    \lambda |\xi|^2 \le a^{ij}\xi_i\xi_j \le \Lambda|\xi|^2, \quad \lambda = \frac{1}{(1+|\nabla f|^2)^{3/2}}, \quad \Lambda = 1.
\]
...
\]
where $L$ is a linearized operator that is degenerate elliptic. The strong maximum principle for degenerate elliptic operators (cf.~\cite{gilbargtrudinger}, Chapter 3) implies $w \equiv \text{const}$ if $w$ achieves an interior maximum, contradicting $w > 0$ at $x_*$ and $w \le 0$ on $\partial\Omega$.
...
\begin{enumerate}
    ...
    \item \textbf{Global Sobolev:} $W^{2,p}_{\mathrm{loc}}(M)$ convergence for $p < 3$ (the Jang metric $\bar{g} = g + df \otimes df$ is Lipschitz, so $f \in W^{2,p}$ for $p < 3$ by Calderón--Zygmund theory applied to the linearization).
\end{enumerate}
...
\begin{equation}\label{eq:MeanCurvatureJumpSign}
    \boxed{[H]_{\tg} := H^+_{\tg}(\Sigma) - H^-_{\tg}(\Sigma) \ge 0.}
\end{equation}
More precisely:
\begin{enumerate}
...
\textbf{Proof structure:} This theorem is established via \textbf{three independent methods} (detailed in Remark~\ref{rem:NonPerturbativeSign}): (1) limiting argument from strictly stable approximations, (2) the Bray--Khuri integral identity and DEC, (3) geometric convexity from stability. The proof below gives the detailed spectral derivation for method (1); methods (2) and (3) provide independent verification. 
