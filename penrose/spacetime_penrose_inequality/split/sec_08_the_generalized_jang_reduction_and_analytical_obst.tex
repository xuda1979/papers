...
\begin{enumerate}
    ...
    \item $\beta \in (-1, 0)$ for marginally stable MOTS (the indicial roots include $0$, so we work in the gap).
\end{enumerate}
...
While the existence theory is framed in Weighted Sobolev spaces, standard elliptic regularity bootstraps the solution $\phi$ into the Weighted Holder spaces $C^{2,\alpha}_{-\delta}(\bM)$. This justifies the pointwise asymptotic expansions $\phi = 1 + A/r + O(r^{-2})$ and $\nabla \phi = O(r^{-2})$ used in the mass flux calculations.
\end{remark}
\end{definition}
...
$f(s, y) = C_0(y) \ln(s^{-1}) + A(y) + O(s^\alpha)$, where $C_0(y) = |\theta^-(y)|/2 > 0$ is a smooth positive function on $\Sigma$ determined by the inward null expansion $\theta^-(y) = H_\Sigma(y) - \mathrm{tr}_\Sigma k(y) < 0$ (the trapped surface condition), $A \in C^\infty(\Sigma)$, and $\alpha > 0$ depends on the stability of $\Sigma$.
...
\begin{enumerate}
    ...
    \item The Direct Trapped Surface Construction (Theorem~\ref{thm:DirectTrappedJang}) shows such a solution exists.
...
%% ===========================================================================
%% DIRECT TRAPPED SURFACE CONSTRUCTION (DETAILED VERSION)
%% ===========================================================================

\begin{theorem}[Direct Trapped Surface Construction---Detailed]\label{thm:DirectTrappedJang-detailed}
Let $(M^3, g, k)$ be an asymptotically flat initial data set satisfying the Dominant Energy Condition with decay $\tau > 1/2$. Let $\Sigma_0 \subset M$ be a \textbf{closed future trapped surface} satisfying:
\[
    \theta^+ = H_{\Sigma_0} + \tr_{\Sigma_0} k \le 0, \quad \theta^- = H_{\Sigma_0} - \tr_{\Sigma_0} k < 0, \quad \text{and} \quad \tr_{\Sigma_0} k \ge 0 \;\; \text{(favorable jump)}.
\]
...
\]
where $C_0(y) = |\theta^-(y)|/2 > 0$ varies over $\Sigma_0$ (positive since $\theta^- < 0$).
...
\]
This positivity is \textbf{assumed} via the favorable jump hypothesis; it does \textbf{not} follow from $\theta^\pm$ alone. For stable MOTS, this follows from spectral positivity (Theorem~\ref{thm:CompleteMeanCurvatureJump}).
\end{theorem}

\begin{remark}[On the Future Trapped Condition]\label{rem:FutureTrappedCondition}
The hypothesis ``future trapped'' means $\theta^+ \le 0$ and $\theta^- < 0$, which is the standard definition in general relativity. This includes:
\begin{itemize}
    \item \textbf{Strictly trapped surfaces:} $\theta^+ < 0$ and $\theta^- < 0$.
    \item \textbf{Marginally outer trapped surfaces (MOTS):} $\theta^+ = 0$ and $\theta^- < 0$.
    \item \textbf{Apparent horizons:} The outermost MOTS in the trapped region.
\end{itemize}
The condition $\theta^- < 0$ is essential: it ensures the blow-up coefficient $C_0 = |\theta^-|/2 > 0$, which determines the logarithmic rate of the Jang function. A surface with $\theta^- = 0$ would give $C_0 = 0$, and the Jang function would not blow up logarithmically.

\textbf{Physical interpretation:} Future trapped surfaces exist inside black holes and are characterized by the property that both ingoing and outgoing light rays have decreasing area. The Penrose inequality bounds the area of any such surface in terms of the total mass.
\end{remark}

\begin{proof}
The key innovation is using the trapped surface $\Sigma_0$ as a \textbf{lower barrier} for the Jang equation, forcing the solution to blow up at $\Sigma_0$ regardless of whether it is a MOTS.

\textbf{Step 1: Trapped condition as barrier.}
The trapped condition $\theta^+ \le 0$ means:
\[
    H_{\Sigma_0} + \tr_{\Sigma_0} k \le 0.
\]
The key observation is that the Jang equation $\mathcal{J}(f) = 0$ describes the condition that the graph $\Gamma_f \subset M \times \mathbb{R}$ has vanishing null expansion $\theta^+_\Gamma = 0$ with respect to the spacetime $(M \times \mathbb{R}, g - dt^2, k)$. When the graph becomes vertical (i.e., $|\nabla f| \to \infty$), the null expansion of the graph approaches the null expansion of the base surface:
\[
    \theta^+_\Gamma \to \theta^+_{\Sigma_0} \le 0 \quad \text{as the graph becomes vertical over } \Sigma_0.
\]
Since $\theta^+_{\Sigma_0} \le 0$ (trapped condition), a vertical cylinder over $\Sigma_0$ is a \emph{subsolution} to the Jang equation. This provides the lower barrier that forces blow-up at $\Sigma_0$, regardless of whether $\Sigma_0$ is stable, outermost, or a MOTS.
...
\]
The condition $\theta^-(y) = H_{\Sigma_0}(y) - \tr_{\Sigma_0} k(y) < 0$ for all $y \in \Sigma_0$ (future trapped) ensures $C_0^{\min} > 0$. The constant $C_1 > 0$ is chosen large.

\textbf{Rigorous verification of the subsolution property:} We must show $\mathcal{J}(\underline{f}) \le 0$ in a neighborhood, not just in the limit. The Jang operator in Fermi coordinates $(s, y)$ near $\Sigma_0$ is:
\[
    \mathcal{J}(f) = \frac{1}{\sqrt{1+|\nabla f|^2}}\left[\Delta f - \frac{f_s^2 f_{ss}}{1+|\nabla f|^2} + H_{\Sigma_s} f_s\right] - \tr_{\Sigma_s} k + \frac{k(\nabla f, \nabla f)}{1+|\nabla f|^2} + O(s),
\]
where $H_{\Sigma_s}$ and $\tr_{\Sigma_s} k$ are the mean curvature and $k$-trace on the parallel surface at distance $s$.

For $\underline{f}(s,y) = C_0^{\min} \ln(s^{-1}) - C_1$:
\begin{itemize}
    \item $\underline{f}_s = -C_0^{\min}/s$, so $|\nabla \underline{f}|^2 \sim (C_0^{\min})^2/s^2 \to \infty$ as $s \to 0$.
    \item The dominant terms give: $\mathcal{J}(\underline{f}) = \frac{s}{C_0^{\min}}\left[H_{\Sigma_0} + \tr_{\Sigma_0} k\right] + O(s^2) = \frac{s}{C_0^{\min}}\theta^+_{\Sigma_0} + O(s^2)$.
\end{itemize}
Since $\theta^+_{\Sigma_0} \le 0$ (trapped condition), we have $\mathcal{J}(\underline{f}) \le 0$ for all sufficiently small $s > 0$. The constant $C_1$ is chosen large enough that $\underline{f}|_{s=s_0} < 0$ lies below the regularized solution on $\{s = s_0\}$.

\textbf{Step 3: Comparison with regularized solutions.}
Let $f_\kappa$ be the solution to the regularized Jang equation with capillary parameter $\kappa > 0$ on $\Omega_\delta = \{x : \dist(x, \Sigma_0) > \delta\}$ with boundary condition $f_\kappa|_{\partial\Omega_\delta} = 0$.

The maximum principle applied to $f_\kappa - \underline{f}$ shows:
\[
    f_\kappa(x) \ge \underline{f}(x) = C_0^{\min} \ln(\dist(x, \Sigma_0)^{-1}) - C_1.
\]
As $\delta \to 0$, the bound forces $f_\kappa(x) \to +\infty$ for any fixed $x$ near $\Sigma_0$.

\textbf{Step 4: Construction of supersolution via domain truncation.}
The supersolution is constructed using a \textbf{domain truncation} approach rather than singular perturbations, ensuring the correct asymptotic behavior.

\textbf{Why naive supersolutions fail:} A direct attempt $\overline{f}(s,y) = C_2 \ln(s^{-1}) + C_3$ with $C_2 > C_0^{\max} := \sup|\theta^-|/2$ gives:
\[
    \mathcal{J}(\overline{f}) = \frac{s}{C_2}\theta^+_{\Sigma_0} + O(s^2) \le 0 \quad \text{(wrong sign for supersolution)}.
\]
The issue is that the trapped condition $\theta^+ \le 0$ provides a \emph{subsolution} barrier, not a supersolution.

\textbf{Domain truncation method:} Instead of constructing a global supersolution, we use the following strategy:
\begin{enumerate}
    \item \textbf{Truncated domain:} For $\delta > 0$ small, solve the regularized Jang equation on $\Omega_\delta := \{x \in M : \dist(x, \Sigma_0) > \delta\}$ with Dirichlet boundary condition $f_\delta|_{\partial\Omega_\delta} = C_0^{\max} \ln(\delta^{-1})$.
    
    \item \textbf{A priori bounds from comparison:} On $\Omega_\delta$, the subsolution $\underline{f} = C_0^{\min} \ln(s^{-1}) - C_1$ satisfies 
    \[
    \underline{f}|_{\partial\Omega_\delta} = C_0^{\min}\ln(\delta^{-1}) - C_1 < C_0^{\max}\ln(\delta^{-1}) = f_\delta|_{\partial\Omega_\delta}
    \] 
    (choosing $C_1$ large). By the maximum principle for the Jang operator, $f_\delta \ge \underline{f}$ on $\Omega_\delta$.
    
    \item \textbf{Uniform upper bound away from $\Sigma_0$:} For any compact $K \Subset M \setminus \Sigma_0$, the solution $f_\delta$ is uniformly bounded above on $K$ by the comparison principle with the AF decay at infinity. Explicitly, the asymptotic flatness gives $|f_\delta(x)| \le C(K)$ for $x \in K$, independent of $\delta$.
    
    \item \textbf{Gradient bounds:} On $\Omega_\delta$, the Jang operator satisfies uniform ellipticity bounds away from $\partial\Omega_\delta$. Interior gradient estimates (Tolksdorf--Lieberman) give $|\nabla f_\delta| \le C(K)$ on compact subsets $K \Subset M \setminus \Sigma_0$.
\end{enumerate}

\textbf{Alternative: Supersolution via exterior data.} A true supersolution can be constructed using the \textbf{exterior solution}. Let $f_{\text{ext}}$ be the Jang solution on the exterior region $M \setminus \overline{B_\epsilon(\Sigma_0)}$ with boundary condition $f_{\text{ext}}|_{\partial B_\epsilon} = +\infty$. By the comparison principle:
\[
    f_\delta(x) \le f_{\text{ext}}(x) + C \quad \text{for } x \in \Omega_\delta \cap (M \setminus \overline{B_\epsilon(\Sigma_0)}),
\]
...
The Direct Trapped Surface Construction produces a Jang metric $\bar{g}$ with the same analytic properties as in the MOTS case, ensuring the rest of the proof applies:
...
\begin{enumerate}
    ...
    \item[(H2)] \textbf{Outermostness:} $\Sigma$ is an outermost MOTS (so Metzger's blow-up theorem \cite{metzger2010} applies).
    \item[(H3)] \textbf{Trapped condition:} $\theta^-|_\Sigma < 0$ (inward null expansion strictly negative).
\end{enumerate}

\textbf{Conclusion:} The Jang--conformal metric $\tg$ satisfies
...
\begin{enumerate}
    ...
    \item If $\lambda_1 = 0$ (marginally stable), then $[H]_{\tg} = 0$ and the metric $\tg$ is $C^1$ across $\Sigma$.
\end{enumerate}
In both cases, the distributional scalar curvature satisfies $\mathcal{R}_{\tg} \ge 0$, enabling the AMO monotonicity.

\textbf{Proof structure:} This theorem is established via \textbf{three independent methods} (detailed in Remark~\ref{rem:NonPerturbativeSign}):
\begin{enumerate}
    \item \textbf{Limiting argument:} Approximating by strictly stable MOTS (Method 1 in Remark~\ref{rem:NonPerturbativeSign}).
    \item \textbf{Bray--Khuri identity:} Using the integral identity and DEC (Method 2 in Remark~\ref{rem:NonPerturbativeSign}).
    \item \textbf{Geometric convexity:} Using the stability condition directly (Method 3 in Remark~\ref{rem:NonPerturbativeSign}).
\end{enumerate}
Additionally, Lemma~\ref{lem:TrappedMeanCurvatureJump} provides the \textbf{direct formula} $[H] = \tr_{\Sigma_0} k$ for general trapped surfaces. The proof below gives the detailed spectral derivation for method (1); methods (2) and (3) provide independent verification. 

\textbf{Important clarification:} For \emph{arbitrary trapped surfaces} (not necessarily MOTS), the formula $[H] = \tr_{\Sigma_0} k$ holds via Lemma~\ref{lem:TrappedMeanCurvatureJump}, but the \emph{sign} of $\tr_{\Sigma_0} k$ is \textbf{not} determined by the trapped conditions alone. The favorable jump condition $\tr_{\Sigma_0} k \ge 0$ must be assumed or derived from additional hypotheses (e.g., stability).
\end{theorem}

\begin{remark}[Clarification: Non-Perturbative Sign vs.\ Quantitative Formula]\label{rem:NonPerturbativeSign}
The central result of Theorem~\ref{thm:CompleteMeanCurvatureJump} is the \textbf{sign} $[H]_{\tg} \ge 0$, which is established \emph{non-perturbatively} via the following independent arguments:

\textbf{Method 1 (Limiting argument):} For any $\lambda_1 \ge 0$, approximate by strictly stable MOTS $\Sigma_n$ with $\lambda_1^{(n)} > 0$ converging to $\lambda_1$. By continuous dependence of solutions to the Jang equation on parameters, $[H]_{\tg_n} \to [H]_{\tg}$. Since $[H]_{\tg_n} > 0$ for each $n$ (by the strict stability case), we conclude $[H]_{\tg} \ge 0$.

\textbf{Method 2 (Bray--Khuri identity):} The DEC implies $\mathcal{S} := 16\pi(\mu - J(\nu)) + |h-k|^2 + 2|q|^2 \ge 0$ pointwise. The Jang scalar curvature decomposition $R_{\bg} = \mathcal{S} - 2\Div(q) + 2[H]_{\bg}\delta_\Sigma$ together with the flux vanishing at infinity (Lemma~\ref{lem:FluxVanishing}) implies that the boundary term must have the correct sign.

\textbf{Method 3 (Geometric convexity):} The Jang blow-up creates a cylindrical end approaching $\Sigma \times \mathbb{R}$. The stability condition $\lambda_1 \ge 0$ implies that the cylinder is ``outward-convex'' (cannot be deformed outward into a trapped region), which geometrically forces $[H] \ge 0$.

The \emph{quantitative} formula
\begin{equation}\label{eq:MeanCurvatureJumpQuantitative}
    [H]_{\tg} = 2\lambda_1 \cdot C_\Sigma + O(\lambda_1^{3/2})
\end{equation}
(with $C_\Sigma > 0$) provides additional information but is \textbf{not required} for the main proof. The big-O notation in~\eqref{eq:MeanCurvatureJumpQuantitative} is valid when $\lambda_1$ is small; for large $\lambda_1$, the sign $[H] > 0$ is directly implied by the geometric convexity argument.
\end{remark}
...
\textbf{Remark on the time-symmetric limit:} For $k=0$ (time-symmetric Schwarzschild), the horizon is a minimal surface ($H_\Sigma=0$), hence $\theta^+=0$ and $C_0=|\theta^-|/2=0$. In that case the Jang solution is trivial ($f\equiv 0$), so there is no interface and $[H]_{\bar g}=0$.
...
\begin{remark}[Sign Convention for Scalar Curvature]\label{rem:SignConventionScalar}\label{rem:SignConvention}
