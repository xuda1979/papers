...
Perelman's resolution of the Poincare conjecture via Ricci flow \cite{perelman2002,perelman2003} provides a paradigm for attacking geometric inequalities through:
...
\subsection{A $\theta^+$-Adjusted Entropy Functional (Heuristic Program)}
...
\begin{itemize}
\item a precise evolution equation for $\theta^+$ under the chosen flow and an identification of all lower-order terms (including those involving $k$ and ambient spacetime curvature);
...
On an evolving surface, commuting $\partial_t$ with intrinsic covariant derivatives produces additional curvature and second fundamental form terms. We suppress these and focus only on the standard Bochner identity contribution needed for the usual Perelman-style completion of squares.
\begin{align}
...
\midrule
\textbf{Main application} & Poincare conjecture & Riemannian Penrose & Spacetime Penrose \\
\bottomrule
\end{tabular}
\end{center}
