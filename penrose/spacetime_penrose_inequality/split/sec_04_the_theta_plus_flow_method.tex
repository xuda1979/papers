...
In this section, we develop a geometric flow approach to the spacetime Penrose inequality. The $\theta^+$-flow provides a Hamilton-style \emph{program} analogous to Ricci flow for the Poincar\'e conjecture: a candidate geometric evolution that should move trapped surfaces toward marginally outer trapped surfaces (MOTS). In the corrected manuscript, this section is used only as a conditional reduction strategy. Global existence, convergence, and the final area comparison are not part of the rigorous core proof unless additional hypotheses are imposed explicitly.
...
\item Serves as a geometric reduction mechanism whose usefulness for Penrose-type bounds still depends on \emph{additional} input beyond the flow itself.
\end{enumerate}
...
In particular, if $H$ is strictly positive (or strictly negative) on $S_t$, then equality forces $\theta^+\equiv 0$ (i.e., $S_t$ is a MOTS).
\end{theorem}
...
\end{itemize}
The integral vanishes identically if and only if $H\theta^+ = 0$ everywhere on $S_t$. If $H$ is non-vanishing, this implies $\theta^+ = 0$.
\end{proof}

\begin{remark}[Slice Dependence of Sign]
\label{rem:slice-dependence}
The sign of $H$ is \textbf{slice-dependent}: for a fixed 2-surface $\Sigma$ in spacetime, different Cauchy slices can give different values of $H$. However, the null expansion $\theta^+ = H + \mathrm{tr}_\Sigma k$ is a \textbf{spacetime quantity} that is slice-independent. Section~\ref{subsec:slice-independence} clarifies this decomposition issue, but it does \emph{not} prove that an arbitrary MOTS can always be moved to a slice with favorable jump.
\end{remark}
...
Even if the flow converges to a MOTS $\mathcal{M}$ enclosing $S_0$, this \emph{does not by itself} imply
\[
    \mathrm{Area}(\mathcal{M}) \ge \mathrm{Area}(S_0).
\]
Nested trapped surfaces need not satisfy monotone area ordering. Therefore the flow alone does not close the Penrose inequality for the initial surface; one still needs an \emph{independent} comparison theorem (for example a compactness-based area comparison or the Lorentzian event-horizon comparison isolated later in the paper).
\end{remark}

\subsection{Long-Time Existence}
\label{subsec:longtime-existence}

The key technical challenge is identifying hypotheses under which the $\theta^+$-flow exists for all time and converges to a MOTS.

\begin{theorem}[Conditional Long-Time Existence and Convergence]
\label{thm:longtime-existence}
Let $(M, g, k)$ be an asymptotically flat initial data set satisfying the Dominant Energy Condition. Let $S_0 \subset M$ be a smooth closed trapped surface with $\theta^+(S_0) < 0$. Assume that the $\theta^+$-flow admits the barrier, curvature, and compactness properties discussed below. Then:
\begin{enumerate}[(i)]
...
This statement is a conditional flow theorem rather than part of the rigorous Jang--AMO core proof. The discussion below records the barrier and compactness inputs one would need.
...
\begin{proposition}[Formal A Priori Curvature Estimate]
...
\begin{proof}
We record the formal maximum-principle computation that motivates such an estimate.
...
Computing $\nabla_i\nabla_j \theta^+ = \nabla_i\nabla_j H + \nabla_i\nabla_j(\tr_S k)$. We use the Simons identity for the Laplacian of the second fundamental form:
\begin{equation}
    \Delta A_{ij} = \nabla_i \nabla_j H + |A|^2 A_{ij} - H (A^2)_{ij} + A_{ij} \Ric(\nu, \nu) + \text{Rm} * A.
\end{equation}
Contracting with $A_{ij}$ yields the evolution of $|A|^2$:
\begin{equation}
    \frac{1}{2} \Delta |A|^2 = |\nabla A|^2 + \langle A, \Delta A \rangle = |\nabla A|^2 + \langle A, \nabla^2 H \rangle + |A|^4 + \text{l.o.t.}
\end{equation}
Substituting this into the evolution equation and using $\theta^+ = H + \tr_S k$:
\begin{multline}
...
But the ODE comparison only indicates the type of control one would need. If the flow reaches a MOTS before $T_{\mathrm{blow}}$, the smooth analysis closes. If curvature blows up, one would need a separate weak-solution theory to continue.

\textbf{Step 6: Conclusion.}
This calculation motivates the conditional continuation statement above, but by itself it does not constitute a complete proof of global existence or convergence.
\end{proof}
...
\begin{theorem}[Expected Weak Solution Statement]
\label{thm:weak-existence}
Assume an Ilmanen-style elliptic regularization and compactness theory is available for the $\theta^+$-flow. Then for any closed trapped surface $S_0$, one expects a weak solution defined for all $t \geq 0$ with the following properties:
\begin{enumerate}
...
\begin{theorem}[Conditional Flow Endpoint Statement]
...
\begin{proof}
\textbf{Part (i):} Under the conditional long-time existence and weak compactness assumptions encoded in Theorems~\ref{thm:longtime-existence} and \ref{thm:weak-existence}, the barriers in Lemmas~\ref{lem:mots-barrier} and \ref{lem:asymptotic-barrier} keep the flow in a compact region. Standard compactness for sets with bounded perimeter would then yield subsequential Hausdorff limits, and any such limit would satisfy $\theta^+ = 0$ in the weak sense.
...
\textbf{No area conclusion from the flow alone.}
The preceding argument identifies a limiting MOTS inside the trapped region, but it does \emph{not} produce a comparison between $\mathrm{Area}(\mathcal{M}_\infty)$ and $\mathrm{Area}(S_0)$. Such a comparison requires additional geometric input beyond the flow, and it is not derived here. \qedhere
\end{proof}

\begin{remark}[Connection to Maximum Area Trapped Surface]
\label{rem:flow-variational-connection}
Theorem~\ref{thm:flow-endpoint-rigorous} should be viewed only as a \emph{reduction} from a trapped surface to some limiting MOTS inside the trapped region. Turning that reduction into a Penrose inequality for the \emph{original} surface still requires a separate theorem comparing areas. In particular, neither enclosure nor the flow equation alone yields
\[
    \mathrm{Area}(\mathcal{M}_\infty) \ge \mathrm{Area}(S_0)
\]
in general.
\end{remark}
...
A crucial insight is that the decomposition $\theta^+ = H + \tr_{\mathcal{M}} k$ is slice-dependent, even though $\theta^+$ itself is spacetime-invariant. This does \emph{not} imply that the favorable-jump condition can always be achieved by changing slices.
...
\begin{itemize}
\item \textbf{Type I (Favorable Jump):} $\mathrm{tr}_{\mathcal{M}} k \geq 0$ (equivalently, $H \leq 0$);
\item \textbf{Type II (Unfavorable Jump):} $\mathrm{tr}_{\mathcal{M}} k < 0$ (equivalently, $H > 0$).
\end{itemize}
\end{definition}
...
For the Penrose inequality, we do not rely on achieving $H = 0$. Earlier drafts attempted to use a variational maximum-area theorem to extract a favorable sign condition, but that route is no longer part of the rigorous core proof.
\end{remark}

\begin{remark}[On the Former Variational Favorable Route]
\label{cor:reduction-favorable}
Earlier drafts attempted to use a maximum-area trapped surface $\Sigma_{\max}$ and an \emph{integral} condition
\[
    \int_{\Sigma_{\max}} \tr_\Sigma k \, dA \ge 0
\]
as a substitute for the pointwise favorable-jump condition. That route is not part of the rigorous core proof of this paper. The direct Jang reduction used later requires a \emph{pointwise} sign input at the MOTS interface, namely
\[
    [H] = \tr_\Sigma k \ge 0,
\]
and this is imposed explicitly in the direct MOTS theorem rather than derived here from a variational argument.
\end{remark}

\subsection{Complete Proof Strategy}
\label{subsec:complete-strategy}

We now present the conditional reduction strategy furnished by the $\theta^+$-flow. \textbf{Note:} This approach requires the compactness conditions (C1)--(C3) from the earlier variational MOTS program for general trapped surfaces, and it remains auxiliary to the rigorous Jang--AMO core proof.

\begin{theorem}[Conditional Reduction via $\theta^+$-Flow]
\label{thm:spacetime-penrose-theta}
Let $(M^4, g)$ be an asymptotically flat spacetime satisfying the Dominant Energy Condition, and let $\Sigma_0$ be a closed trapped surface. Suppose the $\theta^+$-flow starting from $\Sigma_0$ converges to a MOTS $\mathcal{M}_\infty$. Assume, in addition, that:
\begin{enumerate}
    \item[(A)] the limiting MOTS satisfies the favorable jump condition $\tr_{\mathcal{M}_\infty} k \ge 0$, and
    \item[(B)] an independent area comparison is available:
    \[
        \mathrm{Area}(\mathcal{M}_\infty) \ge \mathrm{Area}(\Sigma_0).
    \]
\end{enumerate}
Then
\begin{equation}
...
\end{equation}
\emph{provided} the rigidity hypotheses of the direct MOTS theorem apply to $\mathcal{M}_\infty$.
\end{theorem}

\begin{proof}
By Theorem~\ref{thm:flow-endpoint-rigorous}, the flow produces a limiting MOTS $\mathcal{M}_\infty$. Assumption (A) permits the application of the direct MOTS Penrose inequality (Theorem~\ref{thm:penroseinitial} or Theorem~\ref{thm:SPI}), giving
\[
    M_{\mathrm{ADM}} \ge \sqrt{\frac{\mathrm{Area}(\mathcal{M}_\infty)}{16\pi}}.
\]
Assumption (B) then yields
\[
    M_{\mathrm{ADM}} \ge \sqrt{\frac{\mathrm{Area}(\mathcal{M}_\infty)}{16\pi}}
    \ge \sqrt{\frac{\mathrm{Area}(\Sigma_0)}{16\pi}}.
\]
Thus the flow furnishes a reduction to a MOTS, but the crucial area comparison remains an additional hypothesis rather than an output of the flow itself.
\end{proof}

\begin{remark}[Comparison with Hamilton's Program]
\label{rem:hamilton-comparison}
The structure of this proof has similarities to Hamilton's resolution of the Poincar\'e conjecture:
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
...
\end{tabular}
\end{center}
Note: Unlike Ricci flow where Perelman entropy is monotone, the area under $\theta^+$-flow is not generally monotone. In the manuscript, any conditional area comparison for this route comes from the earlier variational MOTS program, not from flow monotonicity.
\end{remark}
...
\begin{enumerate}
\item \textbf{Universality as a program:} It is designed for arbitrary trapped surfaces, including the unfavorable case, but its global weak theory is not part of the rigorous core proof.
\item \textbf{Natural endpoint:} Under the conditional compactness hypotheses above, the flow is intended to terminate at a MOTS.
\item \textbf{Slice dependence clarified:} The decomposition of $\theta^+$ into $H+\tr_\Sigma k$ is slice-dependent, but this does not by itself remove the favorable-jump issue.
\item \textbf{Area comparison remains external:} Any Penrose application still needs a separate area comparison such as the Maximum Area Trapped Surface theorem or a Lorentzian comparison input.
\end{enumerate}
