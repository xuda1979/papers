...
The first term is nonnegative under the favorable-jump hypothesis $[H] \ge 0$. The second term, $E_{bounded}(s)$, represents the sum of intrinsic curvature, linear errors, and the quadratic deficit. All components of $E_{bounded}$ are constructed from $g$, $\partial_s g$, and their smoothings. Since $\partial_s g \in L^\infty$, we have:
...
\begin{remark}[The Vanishing Buffer in the Zero-Jump Case]
In a zero-jump regime ($[H]=0$ known by separate input), the large positive term $\frac{2}{\epsilon}[H]$ vanishes. However, the deficit term $D_\epsilon$ remains bounded pointwise by $C\|A\|_{L^\infty}^2$.
...
In the positive-jump case ($[H] > 0$), the linear term $\frac{2[H]}{\epsilon}\eta$ dominates the bounded error $E_{bounded}$ in the central part of the smoothing collar for sufficiently small $\epsilon$. Near the support boundary of the cutoff, and throughout any separately known zero-jump regime ($[H]=0$), the argument uses the uniform bound on the negative part rather than pointwise nonnegativity: the quadratic deficit is bounded and hence $\|R^-_{\hat g_\epsilon}\|_{L^{3/2}}\le C\epsilon^{2/3}$. The pointwise nonnegative scalar curvature metric used in the AMO step is obtained only after the conformal correction.
\end{lemma}
...
The jump in mean curvature at $\Sigma$ is taken with the global convention fixed in Section~\ref{sec:AMO}:
\begin{equation}
[H] := H^+ - H^- .
\end{equation}
Here \(+\) denotes the exterior side and \(-\) denotes the interior cylindrical side, both measured with the normal convention used in the jump formula for the Jang metric. In local Fermi coordinates this convention determines which one-sided traces are called \(H^+\) and \(H^-\); reversing the signed distance coordinate reverses both labels and leaves the global jump convention unchanged.
In the direct MOTS argument, the sign condition $[H] \ge 0$ is supplied by the favorable-jump hypothesis $[H]=\tr_\Sigma k \ge 0$. A zero-jump regime corresponds to the additional information $[H]=0$; it is not deduced here from stability alone.
\end{corollary}
...
\begin{theorem}[Fermi-Coordinate Scalar Curvature Expansion]
\label{thm:ScalarFermiExplicit}
For the metric $g = ds^2 + \gamma_{ab}(s, y) dy^a dy^b$, with the second fundamental form convention fixed above, the scalar curvature can be written in the schematic form
\begin{equation}
\label{eq:ScalarFermiExact}
R_g = R_\gamma + Q(A,H) + 2\sigma_\nu \partial_s H,
\end{equation}
where \(Q(A,H)\) is a universal quadratic expression in the second fundamental form and its trace, and \(\sigma_\nu\in\{\pm1\}\) is determined by the chosen signed-distance orientation. The only part of this formula used in the smoothing argument is the linear distributional term: a jump of \(H\) across the interface contributes \(2[H]\delta_\Sigma\) to the scalar curvature with the global convention of Corollary~\ref{cor:MeanCurvJump}. The remaining terms are regular away from \(\Sigma\) and are bounded in the smoothing collar under the Lipschitz bounds used below.

Equivalently, for the estimates in this appendix we use only
\begin{equation}
\label{eq:ScalarFermiSimplified}
R_g = R^{\mathrm{reg}}_g + 2[H]\delta_\Sigma ,
\end{equation}
where \(R^{\mathrm{reg}}_g\in L^\infty_{\mathrm{loc}}\) in the collar after the stated Lipschitz and one-sided smoothness assumptions.
\end{theorem}

\begin{proof}
The Gauss--Codazzi--Riccati identities express \(R_g\) as the intrinsic scalar curvature of the slices plus a universal quadratic expression in \(A\) and \(H\), plus a signed normal derivative of \(H\). Different authors absorb the sign of this derivative into the definition of \(A\) or into the signed-distance coordinate. The manuscript therefore fixes the jump convention directly rather than relying on a local coordinate sign. Since \(H\) has one-sided traces \(H^\pm\), the distributional derivative of \(H\) contains the Dirac mass \((H^+-H^-)\delta_\Sigma\) with the global orientation convention. This gives the singular coefficient \(2[H]\). All non-Dirac terms involve only one-sided smooth coefficients and products of \(A\), hence are locally bounded in the collar.
\end{proof}
...
\end{equation}
where $R^{\mathrm{reg}}_{\bg}$ is the pointwise scalar curvature (defined a.e.\ away from $\Sigma$), $[H]=H^+-H^-$ is the globally oriented mean-curvature jump, and $\delta_\Sigma$ is the Dirac distribution on $\Sigma$.
\end{theorem}

\begin{proof}
With the global orientation convention in Corollary~\ref{cor:MeanCurvJump}, the normal-derivative term in~\eqref{eq:ScalarFermiSimplified} contributes the derivative of the piecewise continuous function \(H(s)\). In the distributional sense:
\[
2 \partial_s H = 2 H'_{\mathrm{reg}} + 2[H] \delta_0(s),
\]
after the sign of \(s\) has been chosen consistently with the \(+\)/\(-\) labeling. Here $H'_{\mathrm{reg}}$ is the classical derivative away from $s = 0$. The coefficient $2[H] \ge 0$ in the direct theorem is supplied by the favorable-jump hypothesis, not by MOTS stability alone, ensuring the singular part contributes positively to the distributional scalar curvature.
\end{proof}
...
H_\epsilon(s) &= H_{\mathrm{reg},\epsilon}(s) + [H]\,\Theta_\epsilon(s) + O(\epsilon), \label{eq:MollH} \\
\sigma_\nu\partial_s H_\epsilon(s) &= [H]\rho_\epsilon(s) + E_\epsilon(s), \qquad \|E_\epsilon\|_{L^\infty}\le C, \label{eq:MollDH}
\end{align}
where \(\Theta_\epsilon\) is a smoothed step function, \(\rho_\epsilon=\partial_s\Theta_\epsilon\), and $C$ depends only on the $C^{0,1}$ norm of $\gamma$ and the geometry of $\Sigma$.
\end{proposition}

\begin{proof}
Equations~\eqref{eq:MollGamma0}--\eqref{eq:MollA1} follow from Lemma~\ref{lem:MollificationLipschitz} applied component-wise.

For~\eqref{eq:MollH}: $H_\epsilon = -\frac{1}{2} \gamma^{ab}_\epsilon \partial_s \gamma_{\epsilon, ab}$. Since $\gamma_\epsilon \to \gamma$ uniformly and $\partial_s \gamma_{\epsilon, ab} = (\rho_\epsilon * \partial_s \gamma)_{ab}$, convolution replaces the jump in \(H\) by a transition layer represented by the smoothed step \(\Theta_\epsilon\).

For~\eqref{eq:MollDH}: The key observation is that $\partial_s H_\epsilon = (\rho_\epsilon * \partial_s H)$ plus commutator terms from the inverse $\gamma^{ab}_\epsilon$. With the global orientation encoded in \(\sigma_\nu\), the main term is \([H]\rho_\epsilon(s)\). The commutator terms are bounded by the Friedrichs commutator lemma applied to the product $\gamma^{ab} \partial_s \gamma_{ab}$ and by the one-sided smooth geometry of the interface.
\end{proof}
...
\subsubsection{The Zero-Jump Case: Detailed Analysis}

When $[H] = 0$ is known by separate input, the positive spike vanishes and the analysis requires more care.

\begin{theorem}[Zero-Jump Smoothing]
...
\]
The existence of positive solutions follows from the positivity of the conformal Laplacian for sufficiently small $\epsilon$, obtained by combining the nonnegative limiting curvature measure with the smallness of $R^-_{\hat{g}_\epsilon}$ in $L^{3/2}$. In strictly positive-jump subcollars the positive spike gives an additional buffer, but the argument does not require raw pointwise nonnegativity of $R_{\hat g_\epsilon}$.
