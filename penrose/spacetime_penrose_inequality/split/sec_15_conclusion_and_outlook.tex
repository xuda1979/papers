...
The original 1973 conjecture of Penrose \emph{assumed} weak cosmic censorship. Under this assumption, the inequality $M_{\mathrm{ADM}} \geq \sqrt{A(\Sigma)/(16\pi)}$ holds for any trapped surface $\Sigma$, as established by Theorem~\ref{thm:Penrose1973Complete}. The proof proceeds via ingoing null focusing (Lemma~\ref{lem:AreaComparison}), the Hawking area theorem, the Kerr bound, and Bondi mass loss.

The following results have been established with complete rigor:
\begin{itemize}
    \item For the outermost MOTS $\Sigma^*$, the inequality holds unconditionally (Theorem~\ref{thm:penroseinitial}).
...
\textbf{Status Update:} 
\begin{itemize}
    \item \textbf{Original Penrose Conjecture (1973):} Assumed cosmic censorship (WCC) and aimed to prove $M_{\ADM} \ge \sqrt{A(\Sigma)/(16\pi)}$ for any trapped surface $\Sigma$. Established via Theorem~\ref{thm:Penrose1973Complete} (ingoing null focusing).
