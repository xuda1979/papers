...
\begin{enumerate}
    ...
    Since $\mathcal{S}_{bulk} \ge 0$ (DEC) and $[H] \ge 0$ by the favorable-jump hypothesis in Theorem~\ref{thm:SPI}, and $\phi > 0$, the vanishing of the integral forces both terms to vanish individually:
    ...
    \item \textbf{Interface Regularity:} The condition $[H]=0$ removes the corner measure at $\Sigma$. The manuscript does \emph{not} infer $C^{1,1}$ regularity from mean-curvature matching alone; instead it appeals to standard regularity theory for static vacuum metrics across non-degenerate horizons once the equality-case equations are in force.
\end{enumerate}
...
\textbf{Step 3: Horizon Regularity across the Interface.}
We now upgrade the regularity of the solution $(\bg, N)$. Initially $\bg$ is only Lipschitz across $\Sigma$, but the equality case gives $[H]=0$, $q=0$, and $\phi\equiv1$, so the corner measure disappears and the pair $(\bg,N)$ satisfies the static vacuum equations distributionally with a non-degenerate horizon.

At this point the manuscript does \emph{not} claim that $[H]=0$ by itself implies $C^{1,1}$ regularity. Instead we invoke the standard regularity theory for static vacuum metrics across non-degenerate Killing horizons, using $N$ as an adapted defining function near $\Sigma$; see Chru\'sciel \cite{chrusciel1990} and Anderson \cite{anderson2000}. These results promote the equality-case solution across the interface and yield smoothness of $(\bg,N)$ in a neighborhood of $\Sigma$.
...
\hline
3 & $[H] = 0$ & Removal of corner measure \\
  & $\Rightarrow$ static-horizon regularity applies across $\Sigma$ & \\
\hline
