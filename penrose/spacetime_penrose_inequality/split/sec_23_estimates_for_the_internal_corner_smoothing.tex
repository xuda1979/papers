...
This term is nonnegative under the favorable-jump hypothesis $[H]\ge0$. Stability of the MOTS alone is not used to infer this sign.
...
\begin{enumerate}
    \item \textbf{The Distributional Term (Linear):} The mean curvature $H_\epsilon = \Tr A_\epsilon$ approximates the smoothed mean curvature of the background. Under the favorable-jump hypothesis $[H] \ge 0$ at $s=0$, the derivative behaves as:
    \[ 2\partial_s H_\epsilon(s) \approx \frac{2}{\epsilon}[H] \eta\left(\frac{s}{\epsilon}\right) + O(1). \]
    In the positive-jump case ($[H]>0$), this provides a large positive contribution $\sim \epsilon^{-1}$. In a separate zero-jump regime, this term vanishes, leaving only bounded errors.
