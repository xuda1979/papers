...
Let $(M, g)$ satisfy $g \in C^{0,1}$ and assume the Ricci curvature is bounded from below in the distributional sense: $\Ric \ge -(n-1)\Lambda g$ for some $\Lambda \ge 0$. Let $u \in W^{1,p}(M)$ be weakly $p$-harmonic. Then for any nonnegative $\varphi \in C^\infty_c(M)$:
\begin{multline}\label{eq:DistrBochnerFull}
    \int_M \varphi \, |\nabla u|^{p-2} |\nabla^2 u|^2 \, dV_g \le \int_M |\nabla u|^{p-2} \left\langle \nabla \frac{|\nabla u|^2}{2}, \nabla \varphi \right\rangle dV_g \\
    + (n-1)\Lambda \int_M \varphi \, |\nabla u|^p \, dV_g.
\end{multline}
\end{theorem}
...
Using the assumption $\Ric_{g_\epsilon} \ge -(n-1)\Lambda - C\epsilon$ (which follows from the smoothing construction under the nonnegative interface-curvature hypothesis used in the manuscript), we have:
\begin{equation}
    \Ric_{g_\epsilon}(\nabla u_\epsilon, \nabla u_\epsilon) \ge -((n-1)\Lambda + C\epsilon) |\nabla u_\epsilon|^2.
\end{equation}
...
\begin{itemize}
    ...
    \item The Ricci term converges to the bound involving $\Lambda$.
\end{itemize}
...
\end{equation}
where $[H]_{\tg} \ge 0$ by the favorable jump condition.
...
\end{equation}
is well-defined in $[0, \infty]$. Moreover, in the direct MOTS regime one assumes $[H]_{\tg} \ge 0$, so $\mathcal{R}^- = 0$ at $\Sigma$, and the integral is identically zero in that setting.
