...
\begin{remark}[Treatment of the Lipschitz Interface]
While this appendix focuses on the critical set $\mathcal{C}$ and the conical singularities $\{p_k\}$, the complete distributional Bochner identity also requires handling the measure-valued curvature supported on the Lipschitz interface $\Sigma$. The rigorous treatment of this term, including the non-negativity of the distributional Ricci contribution $2[H]\delta_\Sigma$, is provided in \textbf{Appendix~\ref{app:WeakBochner} (Distributional Bochner Identity with Measure-Valued Curvature)}. The results here should be read in conjunction with Theorem~\ref{thm:DistrWeightedBochner} therein.
\end{remark}
...
The exponent $\lambda$ is governed by an eigenvalue problem on the link $\partial \mathcal{B}$; in the round cone model one has $\lambda(\lambda+1)\approx \mu_1$ with $\mu_1=2$. In the applications below we assume the first positive indicial root satisfies $\lambda>\tfrac12$. This holds for the model cone and for sufficiently small perturbations of it, but we do not derive it here merely from MOTS stability. Under that spectral assumption, $\nabla^2 u \in L^2_{loc}$ and the distributional Bochner identity is valid near $p_k$.
\end{lemma}
