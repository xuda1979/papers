...
The analysis in Section~\ref{sec:RicciFlowPenrose} revealed that the core obstruction to proving the spacetime Penrose inequality is the existence of ``bad terms'' in the variation of the Hawking mass that are not controlled by the dominant energy condition. In this section, we develop a boost-invariant quasi-local mass framework designed to absorb these bad terms.

\begin{remark}[Scope of This Section]
This section presents a research program rather than a completed proof. We clearly separate proven theorems (unconditional), conditional results (proven assuming stated hypotheses), and open problems (required for unconditional proof).
\end{remark}

\subsection{Program Roadmap: Three Gaps to the Spacetime Penrose Inequality}
\label{subsec:program-roadmap}

We target the following conjecture, using the Bondi mass formulation (null infinity endpoint):
...
\hline
\textbf{1} & Weak null flow existence (caustic handling) & Open & \S\ref{subsec:gap1-weak-flow} \\
\textbf{2} & Degeneracy control ($\theta^+ = 0$ crossings) & Partial & \S\ref{subsec:gap2-degeneracy} \\
\textbf{3} & Rigidity (equality $\Rightarrow$ Schwarzschild) & Conditional & \S\ref{subsec:gap3-rigidity} \\
\hline
\end{tabular}
\end{center}
...
\begin{remark}[Gap 1: Weak Null Flow Existence --- An Implicit PDE Problem]
...
\begin{remark}[Gap 2: MOTS Degeneracy --- Subsumed into Gap 1 via Surgery]
...
\begin{remark}[Gap 3: Rigidity --- A Unique Continuation Problem]
...
\begin{remark}[Summary: Three Analysis Problems for Unconditional Penrose]
\begin{center}
...
\begin{remark}[Strategy A: Elliptic Regularization for Gap 1]
...
\begin{remark}[Strategy B: Perturbation from Schwarzschild]
...
\begin{remark}[Strategy C: Carleman Estimates for Gap 3]
...
\begin{remark}[Strategy D: Goldberg--Sachs for Rigidity]
...
\midrule
$\frac{1}{2}\theta^+(\mu + J_\ell)$ & $\geq 0$ if $\theta^+ \geq 0$ & Good (for untrapped) \\
$-\sigma^+_{ab}\sigma^{-ab}$ & Indefinite & Bad \\
$-2|\zeta|^2$ & $\leq 0$ always & Bad \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{remark}[Critical: Domain Restrictions]
...
\end{equation}
The sign of $\theta^+\theta^-$ (and of $\theta^\pm$ individually) depends on the normalization conventions for the null normals. The constructions below only require that the sign of the product be fixed (so the correction term can be arranged to be nonnegative).

This defines two admissible sign regions:
\begin{itemize}
\item \textbf{Positive-product region}: $\theta^+\theta^- > 0$ a.e. on $\Sigma$;
\item \textbf{Negative-product region}: $\theta^+\theta^- < 0$ a.e. on $\Sigma$.
\end{itemize}
The mass is \textbf{not defined} at:
\begin{itemize}
\item MOTS ($\theta^+ = 0$): handled as a limit, see Lemma~\ref{lem:MOTS-limit};
\item Surfaces where $\theta^- = 0$ (degenerate null geometry): excluded from consideration.
\end{itemize}
\end{remark}

\begin{lemma}[MOTS Limit]
\label{lem:MOTS-limit}
Let $\Sigma$ be a marginally outer trapped surface with $\theta^+ = 0$ and $\theta^- \neq 0$. Then the corrected mass is defined by the limit:
\begin{equation}
...
\begin{definition}[Corrected Quasi-Local Mass---Positive-Product Version]
\label{def:corrected-mass-v1}
For a closed 2-surface $\Sigma$ in the \textbf{positive-product region} ($\theta^+\theta^- > 0$ a.e.), define:
\begin{equation}
...
\begin{definition}[Corrected Quasi-Local Mass---Negative-Product Version]
\label{def:corrected-mass-untrapped}
For a closed 2-surface $\Sigma$ in the \textbf{negative-product region} ($\theta^+\theta^- < 0$ a.e.), define:
\begin{equation}
...
\end{equation}
Note the sign change in the last term: $(-\theta^+\theta^-) > 0$ in the negative-product region ensures the correction term is well-behaved.
\end{definition}
...
In the time-symmetric Minkowski initial data $(\mathbb{R}^3,\delta,k\equiv 0)$ (a standard slice of Minkowski spacetime), for a round sphere $\Sigma = S_r$ of Euclidean radius $r$:
\begin{equation}
...
\begin{proof}
For $k\equiv 0$, our convention gives $\theta^\pm = H$ on any surface in the slice. For a round sphere $S_r \subset (\mathbb{R}^3,\delta)$,
\[
H = \frac{2}{r}, \qquad \theta^+\theta^- = H^2 = \frac{4}{r^2}.
\]
Moreover $\sigma^\pm \equiv 0$ and $\zeta \equiv 0$. Hence
\[
\frac{1}{16\pi}\int_{S_r} \theta^+\theta^-\, dA = \frac{1}{16\pi}\cdot \frac{4}{r^2}\cdot 4\pi r^2 = 1,
\]
so both $m_H(S_r)$ and $\mathcal{Q}(S_r)$ equal
\[
\sqrt{\frac{A(S_r)}{16\pi}}\left(1-\frac{1}{16\pi}\int_{S_r}\theta^+\theta^-\, dA\right)=\frac{r}{2}(1-1)=0.
\]
\end{proof}
...
In the time-symmetric Schwarzschild initial data $(M,g,k\equiv 0)$ of mass $M$ (so $g = u^4\delta$ with $u(\rho)=1+\frac{M}{2\rho}$ in isotropic coordinates), for any centered coordinate sphere $\Sigma = S_\rho$ with $\rho > M/2$:
\begin{equation}
\mathcal{Q}(S_\rho) = M \quad \text{(constant, independent of } \rho\text{)}.
\end{equation}
At the horizon ($\rho = M/2$, equivalently areal radius $r=2M$): $\mathcal{Q}(S_{M/2}) = M = \sqrt{\frac{A_H}{16\pi}}$ where $A_H = 16\pi M^2$.
\end{proposition}

\begin{proof}
Since $k\equiv 0$, we have $\theta^\pm = H$ and $\mathcal{Q}=m_H$ on each $S_\rho$ (the shear and torsion terms vanish by spherical symmetry).

Let $u(\rho)=1+\frac{M}{2\rho}$ so $g=u^4\delta$. The conformal mean-curvature transformation gives
\[
H_g = u^{-2}\left(H_\delta + 4\,u^{-1}\partial_\nu u\right), \qquad H_\delta=\frac{2}{\rho}, \qquad \partial_\nu u = -\frac{M}{2\rho^2}.
\]
Thus
\[
H_g = u^{-2}\left(\frac{2}{\rho}-\frac{2M}{\rho^2 u}\right).
\]
Since $H_g$ is constant on $S_\rho$ and $A_g(S_\rho)=4\pi\rho^2 u^4$, we compute
\[
\frac{1}{16\pi}\int_{S_\rho} H_g^2\, d\sigma_g
= \frac{1}{16\pi}\,H_g^2\,A_g(S_\rho)
= \left(1-\frac{M}{\rho u}\right)^2.
\]
Also $\sqrt{A_g(S_\rho)/(16\pi)} = \frac{\rho u^2}{2}$. Therefore
\[
m_H(S_\rho)=\frac{\rho u^2}{2}\left(1-\left(1-\frac{M}{\rho u}\right)^2\right)
= M,
\]
independent of $\rho$. In particular at the horizon $\rho=M/2$ (equivalently, areal radius $r=2M$), we recover $m_H=\sqrt{A_H/(16\pi)}=M$.
\end{proof}
...
\item $\theta^- \neq 0$ (the ingoing null expansion is nonvanishing on the horizon; the sign depends on the chosen null-normal normalization);
...
\begin{remark}[Complete Hypothesis List for Main Theorem]
...
\item $\Sigma$ is a \textbf{MOTS}: $\theta^+ = 0$ and $\theta^- \neq 0$ (often one assumes $\theta^-<0$ to exclude the degenerate case $\theta^-=0$).
\end{enumerate}
...
\item \textbf{Asymptotic decay:} The correction terms satisfy $|\sigma^\pm|, |\zeta| = O(r^{-2})$ and $|\theta^\pm - 2/r| = O(r^{-2})$ as $r \to \infty$.
\end{enumerate}
...
\begin{itemize}
\item $\theta^+\theta^- = \frac{4}{r^2} - \frac{8M_B}{r^3} + O(r^{-4})$ (Bondi expansion on large spheres);
...
\item \textbf{Null expansions:} $\theta^+ = 2/r + O(r^{-2})$, $\theta^- = 2/r + O(r^{-2})$, and more precisely
\[\theta^+\theta^- = \frac{4}{r^2} - \frac{8M_B}{r^3} + O(r^{-4}).\]
...
\begin{equation}
\sqrt{\frac{|\Sigma_r|}{16\pi}}\left(1 - \frac{1}{16\pi}\int_{\Sigma_r} \theta^+\theta^- \, dA\right)
= \left(\frac{r}{2}+O(1)\right)\left(1 - \frac{1}{16\pi}\left(16\pi - \frac{32\pi M_B}{r} + O(r^{-2})\right)\right)
= \left(\frac{r}{2}+O(1)\right)\left(\frac{2M_B}{r} + O(r^{-2})\right)
= M_B + O(r^{-1}).
\end{equation}
...
\begin{remark}[Open Problems for Unconditional Proof]
\begin{enumerate}
...
\begin{remark}[Core Philosophy: Geometric Analysis as Strategy, Harmonic Analysis as Tactics]
...
\begin{remark}[The p-Geroch Monotonicity Identity (Bridge Formula)]
...
\begin{remark}[The $p \to 1^+$ Limit: Summary]
...
\begin{remark}[Connection to Penrose's 1973 Conjecture]
...
\begin{remark}[Key Innovation: From Flows to Elliptic PDEs]
...
\begin{proof}
We provide a complete proof following the approach of Agostiniani--Mazzieri--Oronzio \cite{amo2022}.
...
For p-harmonic functions, the key identity is (see \cite{kotschwar2009,amo2022}):
...
By the co-area formula and careful manipulation (see \cite{amo2022}, Proposition 4.3), the boundary integral relates to the p-Hawking mass via:
\begin{equation}
...
See Schoen--Yau \cite{schoenyau1981}, Eichmair--Huang--Lee--Schoen \cite{EHLS2016} for complete details.
\end{proof}
...
\begin{enumerate}
\item \textbf{Cylindrical end existence (GAP 4 resolution):} The blow-up of the Jang solution creates a cylindrical end $\Sigma_0 \times [0, \infty)$. By Schoen--Yau \cite{schoenyau1981} and Eichmair--Huang--Lee--Schoen \cite{EHLS2016}, the asymptotic analysis shows: near the blow-up surface, $f(x) \sim d(x, \Sigma_0)^{-1}$, and the graph metric becomes $\bar{g} \approx g|_{\Sigma_0} + ds^2$ where $s = \arctan(f)$. The p-harmonic BVP on manifolds with cylindrical ends is well-posed in weighted Sobolev spaces $W^{1,p}_\delta$ with $\delta \in (2-n, 0)$; see Maz'ya--Rossmann \cite{mazya2011} for domains with conical/cylindrical singularities.
...
\begin{remark}[Summary: Spacetime Penrose Inequality---Current Status]
...
\begin{itemize}
\item[$\checkmark$] Outermost MOTS $\Sigma^*$ (stability $\Rightarrow$ favorable jump automatic)
\item[$\checkmark$] Any trapped surface with favorable jump ($\tr_\Sigma k \geq 0$)
\end{itemize}

\textbf{Conditional on compactness (C1)--(C3) for:}
\begin{itemize}
\item[$\times$] Trapped surfaces with unfavorable jump ($\tr_\Sigma k < 0$)
\end{itemize}
...
\begin{remark}[Rigorous Result: No Conjectures Required]
...
\begin{equation}
\mathcal{Q}(\Sigma_r)
= \sqrt{\frac{4\pi r^2}{16\pi}}\left(1 - \frac{1}{16\pi}\cdot 16\pi\left(1 - \frac{2m(r)}{r}\right)\right)
= \frac{r}{2}\left(\frac{2m(r)}{r}\right)
= m(r).
\end{equation}
...
\begin{proof}
The null flow is governed by the ODE $\frac{dx^\mu}{ds} = \ell^\mu(x)$ where $\ell$ is the outgoing null vector field. Standard ODE existence and uniqueness (Picard--Lindelof) gives local existence for each null generator. 
...
\begin{remark}[Mathematical Status: Open Problem]
...
\textbf{Remark on signs:} The conclusion here uses only that $\theta^-$ stays bounded away from $0$ with fixed sign along the caustic region; the specific sign is convention-dependent.
...
\begin{remark}[Summary: What Is Rigorously Established]
\begin{enumerate}
...
\begin{remark}[Key Insight: The $\sigma^+/\theta^+$ Singularity is Algebraically Unavoidable]
...
\begin{remark}[Main Conditional Theorem]
...
\begin{proof}
At a MOTS, $\theta^+ = 0$ and $\theta^- \neq 0$. The regularized mass becomes:
\begin{align}
...
\begin{remark}[Note: Rigidity Under Strong Assumptions]
...
\begin{remark}[Critical Warning: This is a CONJECTURE, Not a Theorem]
