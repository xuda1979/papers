...
\theta^+[\Sigma] := H_\Sigma + \tr_\Sigma k = 0
\end{equation}
...
\begin{proof}
We provide a complete proof following the approach of Agostiniani--Mazzieri--Oronzio \cite{amo2024}.
...
For p-harmonic functions, the key identity is (see \cite{kotschwar2009,amo2024}):
...
By the co-area formula and careful manipulation (see \cite{amo2024}, Proposition 4.3), the boundary integral relates to the p-Hawking mass via:
\begin{equation}
...
\end{align}
where $C_{ab}$ is the Weyl curvature term. If $\theta^+$ is bounded above and $|\sigma^+|$ remains finite, then $q_{ab}$ evolves by a bounded ODE. The shear satisfies a linear ODE with coefficients bounded by $|\theta^+|$ and curvature. By Gr\"onwall's inequality, $|\sigma^+|$ remains finite on compact time intervals.
...
\begin{itemize}
\item \textbf{Allen--Bryden--Kazaras--Khuri (2025)} \cite{allenbrydenkazaraskhuri2025}: Proved spacetime Penrose with \textbf{suboptimal constant} $c < 1$. Our approach, when (F1)--(F5) hold, achieves the \textbf{sharp constant} $c = 1$.
