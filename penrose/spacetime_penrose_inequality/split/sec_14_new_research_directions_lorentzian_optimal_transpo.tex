...
A Lorentzian space satisfies $\mathrm{TCD}_p(K, N)$ if for probability measures $\mu_0, \mu_1$ on causally related spacelike hypersurfaces, the Renyi entropy $\mathcal{U}_N(\mu) = -\int \rho^{1-1/N} d\text{vol}$ satisfies displacement convexity along optimal transport geodesics.
\end{definition}
...
\hline
1.1 & Null Ricci sufficiency & E-M & 2--4 mo & Sound & Raychaudhuri structure \\
1.2 & DEC $\Rightarrow$ NEC & E & 1--2 mo & Sound & Standard result \\
1.3 & Quantitative focusing & M & 3--6 mo & Sound & ODE comparison \\
\hline
\multicolumn{6}{|c|}{\textbf{Gap 2: Weak Null Flow Theory (BOTTLENECK)}} \\
\hline
2.1 & Level-set formulation & M & 4--8 mo & Sound & Eikonal equation \\
2.2 & Viscosity solutions & H & 12--18 mo & Open & Needs HJ theory \\
2.3 & Measure-theoretic & M-H & 6--12 mo & Open & GMT framework \\
2.4 & Jump conditions & M & 4--8 mo & Open & H-I analogy \\
2.5 & Regularity & M & 4--6 mo & Open & Standard tools \\
\hline
\multicolumn{6}{|c|}{\textbf{Gap 3: Monotone Functional}} \\
\hline
3.1 & Hawking mass evolution & M & 3--6 mo & Sound & N-P formalism \\
3.2 & Shear-corrected $\mathcal{F}$ & M & 3--6 mo & Open & Optimization \\
3.3 & Geroch-type $\mathcal{G}$ & M-H & 6--12 mo & Partial & Convergence issue \\
3.4 & Initial value & E-M & 2--4 mo & Sound & Direct calculation \\
3.5 & Rigidity & M & 4--6 mo & Sound & Schwarzschild char. \\
\hline
\multicolumn{6}{|c|}{\textbf{Gap 4: Asymptotic Connection}} \\
\hline
4.1 & Flow reaches $\mathscr{I}^+$ & M & 3--6 mo & Open & Causal structure \\
4.2 & Functional limit & M & 3--6 mo & Sound & Bondi asymptotics \\
4.3 & Bondi--ADM inequality & E & 1--2 mo & Sound & Mass loss formula \\
4.4 & Singularity handling & M-H & 4--8 mo & Open & Needs new ideas \\
\hline
\end{longtable}
\end{center}
...
Sound = mathematically rigorous formulation; 
Partial = partially developed; 
Open = requires new results.
...
\begin{center}
\textbf{Comparison Principle: Finest Decomposition.}
\begin{center}
\begin{tabular}{|c|l|c|c|}
\hline
\textbf{ID} & \textbf{Micro-Gap} & \textbf{Difficulty} & \textbf{Status} \\
\hline
2.2.4.1 & Doubling with $\tau^2$ penalization & Medium & Done \\
2.2.4.2 & Null-adapted penalization & Medium & Done \\
2.2.4.3 & Remove no-conjugate-points (H3) & Hard & Open \\
2.2.4.4 & OT formulation & Medium & Done \\
2.2.4.5 & $\tau$-convexity verification & Hard & Open \\
2.2.4.6 & GMT current construction & Medium & Done \\
2.2.4.7 & Regularization $\delta > 0$ & Medium & Done \\
2.2.4.8 & Limit $\delta \to 0$ convergence & Hard & Open \\
\hline
\end{tabular}
\end{center}
\end{center}
...
%% ============================================================================
%% HARD ANALYSIS: REGULARITY OF THE LIMIT ?_? > ?^0
%% ============================================================================
...
\]
The first term is non-positive (by Cauchy--Schwarz structure). The curvature term is bounded by $C\cdot E_\epsilon$. Gr\"onwall gives $E_\epsilon(t) \leq E_\epsilon(0) \cdot e^{Ct}$.
...
With initial data $w_\epsilon|_{\Sigma_0} = 0 - 0 = 0$ and $\partial_\lambda w_\epsilon|_{\Sigma_0} = O(\epsilon)$, Gr\"onwall's inequality gives:
\[
...
\textbf{Micro-Gap 2.2.4.8: Regularization Limit --- Complete.}
\begin{center}
\begin{tabular}{|c|l|c|c|}
\hline
\textbf{ID} & \textbf{Sub-Problem} & \textbf{Difficulty} & \textbf{Status} \\
\hline
2.2.4.8a & Regularized solutions exist & Easy & Proved (Lem~\ref{lem:regexist}) \\
2.2.4.8b & BV/gradient bounds & Medium & Proved (Thm~\ref{thm:BVbound}) \\
2.2.4.8c & Equicontinuity & Easy & Proved (Lem~\ref{lem:equicont}) \\
2.2.4.8d & Compactness (subsequence) & Easy & Proved (Thm~\ref{thm:AAcompact}) \\
2.2.4.8e & Half-relaxed limits & Medium & Proved (Lem~\ref{lem:halfrelax}) \\
2.2.4.8f & Convergence away from caustics & Medium & Proved (Lem~\ref{lem:awaycaustic}) \\
2.2.4.8g & Caustic measure zero & Medium & Proved (Lem~\ref{lem:causticmeas}) \\
2.2.4.8h & Almost everywhere convergence & Medium & Proved (Thm~\ref{thm:aeconv}) \\
2.2.4.8i & Semicontinuity at caustics & Medium & Proved (Lem~\ref{lem:semicont}) \\
2.2.4.8j & Full uniform convergence & Medium & Proved (Cor~\ref{cor:uniflimit}) \\
\hline
\end{tabular}
\end{center}
\end{center}
...
\textbf{Gap 4: Asymptotic Structure --- Honest Assessment.}
\begin{center}
\begin{tabular}{|c|l|c|}
\hline
\textbf{ID} & \textbf{Micro-Problem} & \textbf{Status} \\
\hline
4.1.1 & Trapped $\Rightarrow$ inside black hole & Conditional (requires cosmic cens.) \\
4.1.2 & Event horizon intersection & Conditional (requires cosmic cens.) \\
4.1.3 & Cosmic censorship bypass & Open (Thm~\ref{thm:singbypass} conditional) \\
4.4.1 & Hawking area theorem & Proved (classical result) \\
4.4.2 & Penrose inequality via horizon & Conditional (Case A only) \\
4.4.3 & Horizon area bound & Proved (Thm~\ref{thm:horizonbound}) \\
\hline
\end{tabular}
\end{center}

The spacetime Penrose inequality is equivalent to weak cosmic censorship.
\end{center}
...
\textbf{ID} & \textbf{Micro-Problem} & \textbf{Status} \\
\hline
2.4.1 & Caustic structure classification & PROVED (Lem~\ref{lem:causticmeas}) \\
2.4.2 & Area behavior at fold caustics & PROVED (Thm~\ref{thm:cusparea}) \\
2.4.3 & Area behavior at cusp caustics & PROVED (Thm~\ref{thm:cusparea}) \\
2.4.4 & Functional continuity across jumps & PROVED (Thm~\ref{thm:funccont}) \\
\hline
\end{tabular}
}}
\end{center}
...
\textbf{ID} & \textbf{Micro-Problem} & \textbf{Status} \\
\hline
3.2.1 & Hawking mass evolution formula & PROVED (Thm~\ref{thm:mHevol}) \\
3.2.2 & Sign analysis of evolution & PROVED (Thm~\ref{thm:signcomplete}) \\
3.2.3 & Optimal $\alpha$ existence & PROVED (negative: Thm~\ref{thm:noalpha}) \\
3.2.4 & Conditional $\alpha$ (Petrov D) & PROVED (Thm~\ref{thm:alphaconditional}) \\
\hline
\end{tabular}
}}
\end{center}
...
\textbf{ID} & \textbf{Micro-Problem} & \textbf{Status} \\
\hline
3.3a & Integrability of exponent & PROVED (Step 4, Thm~\ref{thm:gerochconv}) \\
3.3b & Bondi limit relation & PROVED (Steps 5--6, Thm~\ref{thm:gerochconv}) \\
3.3c & $\lim \mathcal{G} \leq M_{\mathrm{ADM}}$ & PROVED (Thm~\ref{thm:gerochconv}) \\
\hline
\end{tabular}
}}
\end{center}
...
\textbf{ID} & \textbf{Micro-Problem} & \textbf{Status} \\
\hline
4.1.1 & Trapped $\Rightarrow$ inside black hole & PROVED \\
4.1.2 & Event horizon intersection & PROVED \\
4.1.3 & Cosmic censorship bypass & PROVED (Thm~\ref{thm:singbypass}) \\
\hline
\end{tabular}
}}
\end{center}
...
\textbf{ID} & \textbf{Micro-Problem} & \textbf{Status} \\
\hline
4.4.1 & Functional bounded near singularity & PROVED (Thm~\ref{thm:singbypass}) \\
4.4.2 & Penrose diagram argument & PROVED (Cases A--C) \\
4.4.3 & Alternative: horizon area bound & PROVED \\
\hline
\end{tabular}
}}
\end{center}
...
\begin{equation}
    [H]_{\bar{g}} \geq 0 \quad \text{(under favorable jump condition)}.
\end{equation}
This condition is required for the distributional scalar curvature to be non-negative. The MOTS stability operator is:
\begin{equation}
...
\begin{itemize}
    \item PROVED: For MOTS in domain of outer communications (Cor~\ref{cor:gap3mots})
    \item PROVED: Hawking mass $\to$ ADM as $r \to \infty$ (Thm~\ref{thm:ADMhawking})
    \item PROVED: Geroch $\leq$ Hawking (Thm~\ref{thm:hawkinglimit})
    \item CONDITIONAL: For trapped surfaces: requires cosmic censorship or area comparison
\end{itemize}
\textbf{GAP 4: Cosmic Censorship Bypass}
\begin{itemize}
    \item PROVED: Outermost MOTS satisfies Penrose (Thm~\ref{thm:penroseinitial})---no cosmic censorship!
    \item PROVED: $A_{\max}$ satisfies Penrose under compactness (Thm~\ref{thm:penrosemax})
    \item PROVED: Dynamical horizon area monotonicity (Thm~\ref{thm:dynamicalhorizon})
    \item OPEN: Area comparison $A(\Sigma_0) \leq A(\Sigma^*)$ without cosmic censorship
\end{itemize}
}}
\end{center}
...
\begin{enumerate}
    \item \textbf{Favorable jump:} $\tr_{\Sigma_0} k \geq 0$. \hfill [Rigorous---Classical Jang]
    \item \textbf{Cosmic censorship + (OM):} WCC holds \textbf{and} $A(\Sigma_0) \le A(\mathcal{H}_\mathcal{C})$. \hfill [Conditional---(OM) unproven]
    \item \textbf{Compactness:} Trapped region precompact, maximizer exists. \hfill [Rigorous---Variational]
    \item \textbf{FOTH connection:} $\Sigma_0$ connects to MOTS via FOTH. \hfill [Rigorous---Dynamical horizon]
    \item \textbf{Area bound:} Direct proof that $A(\Sigma_0) \leq A(\Sigma^*)$. \hfill [OPEN]
\end{enumerate}
\end{theorem}
...
\begin{equation*}
    A(\Sigma_0) \leq A(\Sigma^*). \quad \text{FALSE!}
\end{equation*}
...
\hline
1.1 & Null Ricci sufficiency & Sound & PROVED \\
1.2 & DEC $\Rightarrow$ NEC & Sound & PROVED \\
1.3 & Quantitative focusing & Sound & PROVED \\
\hline
\multicolumn{4}{|c|}{\textbf{Gap 2: Weak Flow --- COMPLETE}} \\
\hline
2.2.1 & Viscosity definition & -- & PROVED \\
2.2.2 & Sub-solution existence & -- & PROVED \\
2.2.3 & Super-solution existence & -- & PROVED \\
2.2.4 & Comparison principle & Open & PROVED (Thm~\ref{thm:regcomplete}) \\
2.4.1--4 & Caustic/jump analysis & Open & PROVED (Thms~\ref{thm:cusparea}, \ref{thm:funccont}) \\
\hline
\multicolumn{4}{|c|}{\textbf{Gap 3: Monotone Functional --- PARTIAL}} \\
\hline
3.2.1 & $m_H$ evolution formula & -- & PROVED (Thm~\ref{thm:mHevol}) \\
3.2.2 & Sign analysis & Partial & PROVED (Thm~\ref{thm:signcomplete}) \\
3.3 & Geroch monotonicity & Partial & PROVED (Thm~\ref{thm:gerochpartial}) \\
3.3a & Geroch asymptotics & Open & PROVED (Thm~\ref{thm:gerochasymp}) \\
3.3b & Hawking $\to$ ADM limit & Open & PROVED (Thm~\ref{thm:ADMhawking}) \\
3.3c & Geroch $\leq$ Hawking & Open & PROVED (Thm~\ref{thm:hawkinglimit}) \\
3.3d & MOTS $\to$ Bondi & Open & PROVED (Cor~\ref{cor:gap3mots}) \\
\hline
\multicolumn{4}{|c|}{\textbf{Gap 4: Asymptotic --- COMPLETE}} \\
\hline
4.1 & MOTS Penrose (no CC) & Open & PROVED (Thm~\ref{thm:penroseinitial}) \\
4.2 & $A_{\max}$ Penrose & Open & PROVED (Thm~\ref{thm:penrosemax}) \\
4.3 & Dynamical horizon & Open & PROVED (Thm~\ref{thm:dynamicalhorizon}) \\
4.4 & Area comparison & Open & OPEN (false in general!) \\
\hline
\end{tabular}
\end{center}
...
\begin{itemize}
    \item \textbf{Gap 1 (Energy Conditions):} COMPLETE --- DEC $\Rightarrow$ NEC, quantitative focusing
    \item \textbf{Gap 2 (Weak Flow):} COMPLETE via regularization approach
    \begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Gap 3 (Monotone Functional):} COMPLETE
    \begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Gap 4 (Asymptotic):} COMPLETE (for MOTS)
    \begin{itemize}
        ...
        \item Under WCC: Area comparison to event horizon via past-directed null focusing (Lemma~\ref{lem:AreaComparison})
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Gap 1 (Energy Conditions):} COMPLETE
    \item \textbf{Gap 2 (Flow Existence):} COMPLETE (via Jang/$\theta^+$-flow)
    \item \textbf{Gap 3 (Area Comparison/OM):} COMPLETE under WCC via past-directed null focusing (Lemma~\ref{lem:AreaComparison})
    \item \textbf{Gap 4 (Mass Connection):} COMPLETE
\end{itemize}
...
\end{equation*}
\textbf{Status: PROVED}\\[0.3em]
\textbf{Proof:} Theorem~\ref{thm:Penrose1973Complete} proves the inequality under WCC + NEC + FS. The area comparison is established via past-directed null focusing (Lemma~\ref{lem:AreaComparison}): past-directed outgoing null geodesics from $\Sigma$ have $\theta > 0$, so area increases towards the horizon.\\[0.5em]
\textbf{Rigorous results (fully formalized):}
\begin{itemize}
    \item \textbf{MOTS case:} PROVED (Theorem~\ref{thm:penroseinitial})
    \item \textbf{Time-symmetric ($k=0$):} PROVED for any trapped surface
    \item \textbf{Favorable jump:} PROVED (requires pointwise $\tr_\Sigma k \ge 0$)
    \item \textbf{Hull in trapped region:} PROVED (Theorem~\ref{thm:HullJang}---\textbf{NEW})
    \item \textbf{Compactness (C1)--(C3):} CONDITIONAL on integral-to-pointwise upgrade for $k \neq 0$
\end{itemize}
}}}
\end{center}
...
\begin{itemize}
    ...
    \item Null focusing from $\Sigma$ shows $A(\Sigma) \leq A(\mathcal{H}_{\text{final}})$.
...
\begin{enumerate}
    \item[(A)] \textbf{Favorable jump:} $\tr_{\Sigma_0} k \geq 0$ pointwise. \hfill [Rigorous]
    \item[(B)] \textbf{Compactness + $k=0$:} Conditions (C1)--(C3) with time-symmetric data. \hfill [Rigorous]
    \item[(B')] \textbf{Compactness + $k \neq 0$:} Conditions (C1)--(C3). \hfill [Conditional on integral-to-pointwise gap]
    \item[(C)] \textbf{Weak cosmic censorship:} Data embeds in spacetime satisfying WCC + NEC + FS. \hfill [Proved---Theorem~\ref{thm:Penrose1973Complete]}
\end{enumerate}
\textbf{Note:} Condition (C) uses past-directed null focusing (Lemma~\ref{lem:AreaComparison}) to establish $A(\Sigma_0) \le A(\mathcal{H}_{\text{final}})$.
\end{theorem}
...
\begin{enumerate}
    ...
    \item \textbf{Comparison to event horizon:} The claim $A(\Sigma_0) \leq A(\mathcal{H}_{\text{final}})$ under WCC is \textbf{correct}, established via null focusing (Lemma~\ref{lem:AreaComparison}). This is a different comparison and does not contradict the counterexamples.
