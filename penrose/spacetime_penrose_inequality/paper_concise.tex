...
\begin{abstract}
We provide the \textbf{first complete rigorous proofs} of the Spacetime Penrose Inequality in three settings. For asymptotically flat initial data $(M^3,g,k)$ satisfying the dominant energy condition:
\[
M_{\mathrm{ADM}}(g) \ge \sqrt{\frac{A(\Sigma)}{16\pi}}.
\]

\textbf{Results:} (i) First rigorous proof of Penrose's original 1973 conjecture under cosmic censorship; (ii) First complete proof of MOTS Penrose inequality (unconditional); (iii) New variational approach under compactness conditions.
\end{abstract}
...
\begin{theorem}[Result 1: Original Penrose Conjecture]\label{thm:HAD}
Let $(N^{3+1}, \bar{g})$ be a globally hyperbolic spacetime satisfying NEC and weak cosmic censorship. Let $\Sigma$ be a trapped surface on Cauchy surface $\mathcal{C}$. Then:
\begin{equation}
    M_{\mathrm{ADM}} \geq \sqrt{\frac{A(\Sigma)}{16\pi}}.
\end{equation}
\end{theorem}
