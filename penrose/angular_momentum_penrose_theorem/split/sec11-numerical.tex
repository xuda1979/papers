...
This appendix provides \textbf{supplementary numerical illustrations} that serve a pedagogical and verification purpose only. The analytical core of Theorem~\ref{thm:main} is developed in Sections 3--8, while the rotating monotonicity and conformal mass-comparison steps are isolated explicitly as hypotheses (H6)--(H7).
...
\textbf{Conclusion:} Among 178 physically valid configurations satisfying the \textbf{structural hypotheses} (H1)--(H4), every single one satisfies the AM-Penrose inequality with \textbf{zero genuine counterexamples}. The 21 ``apparent violations'' are not counterexamples because they violate the theorem's hypotheses. The additional Stage~2 source-term decay hypothesis (H5) and the isolated auxiliary inputs (H6)--(H7) are analytic ingredients that were not part of this numerical screening.
