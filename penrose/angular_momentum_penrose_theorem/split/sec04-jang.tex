...
\begin{enumerate}
    ...
    \[
    \begin{aligned}
    |g_{ij}(x) - \delta_{ij}| &\leq C|x|^{-\tau}, \\
    |\partial_k g_{ij}(x)| &\leq C|x|^{-\tau-1}, \\
    |\partial_k\partial_\ell g_{ij}(x)| &\leq C|x|^{-\tau-2};
    \end{aligned}
    \]
...
L_\Sigma[\psi] := -\Delta_\Sigma \psi - \left(|A_\Sigma|^2 + \Ric_g(\nu,\nu)\right)\psi - \Div_\Sigma(X\psi) - X \cdot \nabla_\Sigma\psi,
\end{equation}
acting on $W^{2,2}(\Sigma) \to L^2(\Sigma)$, satisfies $\lambda_1(L_\Sigma) \geq 0$. Here:
\begin{itemize}
...
\begin{enumerate}[label=\textup{(\roman*)}]
    \item \textbf{Spectral correspondence:} By the asymptotic analysis of the stable-MOTS Jang equation, strict stability yields a positive exponential rate $\beta_0>0$ for convergence to the limiting cylinder. The size of $\beta_0$ is controlled by the spectral gap of $L_\Sigma$.
    
    \item \textbf{Decay rate implications:} For $\lambda_1(L_\Sigma) > 0$:
    \begin{itemize}
        \item The Jang metric converges \textbf{exponentially} to the cylinder: $\bg = dt^2 + g_\Sigma + O(e^{-\beta_0 t})$ in a suitable cylindrical coordinate $t$ on the end;
    ...
    \item \textbf{Marginally stable case:} We do \emph{not} treat the borderline case $\lambda_1(L_\Sigma)=0$ in this paper. The strict-stability hypothesis (H4) is imposed precisely to stay in the exponentially decaying cylindrical regime needed for the weighted-space analysis below.
...
\end{enumerate}
The hypothesis (H4) in Theorem~\ref{thm:main} requires $\lambda_1(L_\Sigma) > 0$. This places the paper in the non-degenerate horizon regime; standard sub-extremal Kerr slices provide model examples.
\end{remark}
...
\begin{enumerate}[label=\textup{(\roman*)}]
    ...
    \item \textbf{Blow-up asymptotics:} Near $\Sigma$, the solution blows up logarithmically:
    \[
    f(x) = C_0 \ln(1/s) + \mathcal{A}(y) + R(s,y), \quad C_0 > 0,
    \]
    where:
    \begin{itemize}
        \item $s = \mathrm{dist}_g(x, \Sigma)$ is the signed distance to $\Sigma$;
        \item $y \in \Sigma$ is the nearest point projection;
        \item $C_0$ is the positive logarithmic blow-up coefficient supplied by the untwisted Han--Khuri existence theory for the horizon class under consideration;
    ...
    \begin{itemize}
        ...
        \item The cylindrical end $\mathcal{C} := \{x : s < s_0\} \cong [0,\infty) \times \Sigma$ admits a cylindrical coordinate $t$ with metric
        \[
        \bg = dt^2 + g_\Sigma + O(e^{-\beta_0 t}), \quad \beta_0 > 0,
        \]
        where the error term and its first two derivatives decay exponentially, with $\beta_0$ determined by the strict-stability spectral gap.
    \end{itemize}
    \item \textbf{ADM mass agreement:} For the standard asymptotic normalization of the Jang solution, the graph metric satisfies
    \[
    M_{\ADM}(\bg) = M_{\ADM}(g).
    \]
\end{enumerate}
...
= \frac{\rho^2}{(1 + |\bar{\nabla} \bar{f}|^2)^{1/2}}
  \left(
      \omega_i \bar{\nu}^i
      - \frac{\bar{f}_{,i}\omega_j \bar{f}^{,j}}{1 + |\bar{\nabla} \bar{f}|^2}\bar{\nu}^i
  \right),
\end{equation}
...
\textbf{Critical observation:} The constant $C_\mathcal{T}$ depends \textbf{only on the initial data} $(g, K)$ together with the positive blow-up coefficient $C_0$ furnished by the untwisted Jang existence theory. In particular:
...
\end{enumerate}
See Appendix~\ref{app:schauder} for a detailed perturbative discussion showing that the twist term does not alter the leading existence mechanism or the blow-up character of the Jang solution.
...
\begin{enumerate}[label=\textup{(NC\arabic*)}]
    \item The constant $C_0 > 0$ is fixed \textbf{a priori} once the reference untwisted blow-up solution near $\Sigma$ is chosen; it is not determined by the perturbative correction $v$.
...
\end{enumerate}
The logical flow is: \textit{initial data} $\to$ \textit{untwisted blow-up solution and horizon geometry} $\to$ \textit{blow-up coefficient} $C_0$ \textit{and twist bound} $C_{\omega,\infty}$ $\to$ \textit{perturbation estimate} $|\mathcal{T}| \leq C_\mathcal{T} s$ $\to$ \textit{perturbative reduction to the untwisted Jang problem}. At no point does the constant $C_\mathcal{T}$ depend on the twisted solution $f$ being constructed.

\begin{mdframed}[linewidth=1pt, linecolor=green!60!black, backgroundcolor=green!3]
\textbf{Data Dependence of the Perturbative Constants.}

We record the source of the constants entering Lemma~\ref{lem:perturbation-stability}. The point is not to produce sharp formulas, but to show that the perturbative bounds are determined by the initial data and the chosen weighted spaces, rather than by the unknown solution itself.
...
\begin{enumerate}
    \item[(C1)] $C_0 > 0$: the leading logarithmic blow-up coefficient of the reference untwisted Han--Khuri solution near $\Sigma$.
...
\begin{proof}
The argument is the standard quasilinear fixed-point reduction in weighted Sobolev spaces. Writing $f=f_0+v$ and expanding around $f_0$ gives
\begin{equation}\label{eq:fixed-point}
L_0 v = -N[v] - \mathcal{T}[f_0+v],
\end{equation}
where $L_0=D\mathcal{J}_0|_{f_0}$ and $N[v]$ is quadratic in $v$.

Hypothesis (P2) provides a bounded inverse for $L_0$ on $W^{2,2}_\beta$, so the map
\[
\Phi(v):=-L_0^{-1}\bigl(N[v]+\mathcal{T}[f_0+v]\bigr)
\]
is well defined. The quadratic estimate for $N[v]$ and the weighted integrability/Lipschitz control coming from (P3) imply that $\Phi$ maps a sufficiently small ball in $W^{2,2}_\beta$ to itself and is a contraction there. Banach's fixed-point theorem therefore gives a unique correction $v$ in that ball.

Because $\beta\in(-1,0)$, functions in $W^{2,2}_\beta$ satisfy $v=o(1)$ toward the cylindrical end, so the perturbation does not alter the leading logarithmic coefficient $C_0$. Elliptic regularity then upgrades the remainder to the stated order
\[
O\!\bigl(s^{\min(\alpha,1+\gamma)}\bigr),
\]
after absorbing the lower-order contribution of $v$ into the profile function $\mathcal{A}(y)$.
\end{proof}

We verify conditions (P1)--(P3) for our setting with explicit references:
\begin{itemize}
    \item \textbf{Verification of (P1):} This is Han--Khuri \cite[Proposition 4.5]{hankhuri2013}. Specifically, for initial data $(M,g,K)$ satisfying DEC with a stable outermost MOTS $\Sigma$, the unperturbed Jang equation $\mathcal{J}_0[f] = 0$ admits a solution $f_0$ with blow-up asymptotics $f_0(s,y) = C_0 \ln s^{-1} + \mathcal{A}_0(y) + O(s^\alpha)$ for some coefficient $C_0>0$. The exponent $\alpha > 0$ depends on the spectral gap of the MOTS stability operator; for strictly stable MOTS, $\alpha = \min(1, 2\sqrt{\lambda_1(L_\Sigma)})$ where $\lambda_1(L_\Sigma) > 0$ is the principal eigenvalue.
    
    \item \textbf{Verification of (P2):} For the untwisted Han--Khuri solution $f_0$, the linearized Jang operator $L_0=D\mathcal{J}_0|_{f_0}$ is an asymptotically cylindrical elliptic operator. Strict stability of the MOTS provides a spectral gap for the limiting cylindrical operator, so one may choose a weight $\beta<0$ avoiding the indicial roots. Lockhart--McOwen theory \cite{lockhartmccowen1985} then places $L_0$ in the Fredholm regime on $W^{2,2}_\beta$. The additional coercive/invertibility estimate required in hypothesis (P2) is the standard linear input from the untwisted Han--Khuri analysis; the present perturbative argument does not attempt to re-prove that untwisted estimate from scratch. Lemma~\ref{lem:twisted-indicial} below records only the part needed for the twist perturbation, namely that the twist contribution is exponentially decaying at the linearized level and therefore does not alter the indicial roots or the Fredholm setup.
...
The perturbation stability argument depends on the following classes of constants:
\begin{itemize}
    \item An inverse bound for the untwisted linearization $L_0^{-1}$ on the chosen weighted space;
    \item Local $C^2$ control of the reference Han--Khuri solution $f_0$ on compact subsets away from the horizon;
    \item The twist-size bound $C_{\omega,\infty}$ together with the horizon geometry through $\rho_{\max}$ and the blow-up coefficient $C_0$;
    \item A small radius $\delta$ for the contraction ball, chosen after the previous constants are fixed.
\end{itemize}
For axisymmetric vacuum data with strictly stable MOTS, these quantities are finite and determined by the initial data together with the chosen weighted-space setup. The qualitative dependence is the expected one: stronger stability and weaker twist improve the perturbative control.
\end{remark}

\begin{lemma}[Indicial/Fredholm Stability for the Twisted Jang Operator]\label{lem:twisted-indicial}\label{lem:fredholm}
...
\begin{proof}
On the cylindrical end, the twist contribution to the linearized operator is exponentially decaying and of lower order:
\[
L_{\mathrm{axi}} = L_0 + B,
\qquad
B = O(e^{-\delta t})
\]
for some $\delta>0$, in the sense of coefficients of an operator of order at most one. Consequently the translation-invariant limit operator of $L_{\mathrm{axi}}$ agrees with that of $L_0$, so the indicial roots are unchanged. This proves (i).

For a weight $\beta<0$ avoiding those indicial roots, Lockhart--McOwen theory implies that both $L_0$ and $L_{\mathrm{axi}}$ are Fredholm on the corresponding weighted Sobolev spaces. Since the perturbation $B$ is exponentially decaying, it acts compactly from $W^{2,2}_\beta$ to $L^2_\beta$, so the Fredholm index is preserved. Thus $L_{\mathrm{axi}}$ has the same index as $L_0$, namely zero; this proves (ii).
\end{proof}
...
\]
where $C_0>0$ is the leading logarithmic coefficient supplied by the untwisted Han--Khuri asymptotic analysis.
...
\textit{Cylindrical end metric.} In a suitably normalized cylindrical coordinate $t$ on the end, the induced metric satisfies:
\[
\bg = dt^2 + g_\Sigma + O(e^{-\beta t})
\]
where $\beta > 0$ is related to the spectral gap of the stability operator $L_\Sigma$ for the strictly stable horizons considered here. The twist contribution to the metric correction is exponentially small:
\[
|\mathcal{T}| = O(e^{-t}) \quad \text{along the cylindrical end},
\]
...
\]
The barrier construction at infinity gives $f=O(r^{1-\tau+\epsilon})$ and hence $|\nabla f|=O(r^{-\tau+\epsilon})$ for any fixed $\epsilon \in (0,\tau-\tfrac12)$. Therefore
\[
df\otimes df = O(r^{-2\tau+2\epsilon}),
\qquad
\partial(df\otimes df)=O(r^{-2\tau+2\epsilon-1}).
\]
Since $2\tau-2\epsilon>1$, the rank-one correction $df\otimes df$ is lower order than the ADM part of $g$ and contributes zero to the ADM mass integral. Thus
\[
M_{\ADM}(\bg)=M_{\ADM}(g).
\]
\end{proof}
