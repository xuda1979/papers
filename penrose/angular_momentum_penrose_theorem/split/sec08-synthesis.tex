\section{Synthesis of the Conditional Theorem}\label{sec:synthesis}
%=============================================================================
...
\begin{enumerate}[label=(BC-L\arabic*)]
    ...
    \item \textbf{Exponential decay:} Under the source-term decay hypothesis of Theorem~\ref{thm:lich-exist}, $|\phi(t, y) - 1| \leq Ce^{-\kappa t}$ for some $\kappa = \kappa(\lambda_1(L_\Sigma)) > 0$.
...
\textbf{Hypothesis Usage Summary.} The seven stated hypotheses enter the proof as follows:
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
  ...
  \item[\textbf{(H4)}] \textbf{Strictly stable MOTS:} Provides the spectral gap needed for the Jang cylindrical-end analysis and the exponential decay statements used at the MOTS boundary (Stage 7).
  \item[\textbf{(H5)}] \textbf{Source-term decay:} Supplies the cylindrical-end decay/integrability needed to solve the AM-Lichnerowicz equation and control its asymptotics.
  \item[\textbf{(H6)}] \textbf{Auxiliary AM-Hawking monotonicity:} Provides the endpoint comparison $m_{H,J}(0)\le m_{H,J}(1)$ for the rotating flow.
  \item[\textbf{(H7)}] \textbf{Conformal mass comparison:} Provides the final inequality $M_{\ADM}(\tg)\le M_{\ADM}(g)$ needed to transfer the AMO lower bound for the conformal metric back to the physical ADM mass.
\end{itemize}
\end{mdframed}
...
\begin{itemize}[leftmargin=1.5em]
    \item \textbf{Strictly stable MOTS:} The principal eigenvalue satisfies $\lambda_1(L_\Sigma) > 0$, giving a positive spectral gap for the cylindrical operator governing the Jang end and subsequent elliptic problems.
    \item \textbf{Marginally stable MOTS:} $\lambda_1(L_\Sigma) = 0$, with a non-trivial kernel. In that regime the spectral gap closes and different asymptotic/function-space analysis is required.
...
\begin{enumerate}[label=(\arabic*)]
    \item \textbf{Jang equation blow-up rate (Stage 1):} The Jang solution $f$ satisfies $f \sim C_0 \ln(1/s)$ near the MOTS for some coefficient $C_0>0$ furnished by the untwisted Han--Khuri blow-up theory. Equivalently, after introducing a suitably normalized cylindrical coordinate $t$ on the end, the Jang metric is asymptotic to $dt^2 + g_\Sigma$. When $\lambda_1(L_\Sigma)=0$, the strict-stability input used to obtain the exponential cylindrical regime is lost, and the end analysis must be replaced by a different marginal theory.
...
Let $(M, g, K)$ satisfy the hypotheses of Theorem~\ref{thm:main}.
...
\[
\Delta_p u_p = 0, \quad u_p \to 0 \text{ along the cylindrical end}, \quad u_p \to 1 \text{ at infinity}.
\]
...
\textbf{Stage 6:} By the auxiliary monotonicity input Theorem~\ref{thm:monotone}, $m_{H,J}(t)$ is monotone increasing.
...
\[
m_{H,J}(0) = \sqrt{\frac{A_{\tg}(\Sigma)}{16\pi} + \frac{4\pi J^2}{A_{\tg}(\Sigma)}},
\]
...
\]
where $s = \mathrm{dist}_g(x, \Sigma)$ is the signed distance function and $C_0>0$ is the logarithmic blow-up coefficient from Theorem~\ref{thm:jang-exist}. The induced metric on $\Gamma_f$ is:
\[
\bg = g + df \otimes df = g + \frac{C_0^2 ds \otimes ds}{s^2} + O(s^{-1}).
\]
After introducing a suitably normalized cylindrical coordinate $t$ on the end, this becomes:
\[
...
\]
Near a MOTS with $\theta^+ = H_g + \tr_\Sigma K = 0$, the blow-up behavior $f \to \infty$ with $|\nabla f| \sim 1/s$ encodes the MOTS condition into the cylindrical end structure. The conclusion $H_{\bg}|_\Sigma = 0$ is obtained from the asymptotic product geometry just proved, not from a separate minimal-surface variational principle for the full Jang graph.
...
\]
Here $\phi|_\Sigma = 1$ means the limiting value of $\phi$ along the cylindrical end. For the normal derivative, it is enough to use the exponential decay from Lemma~\ref{lem:phi-bound}: write $\phi = 1 + \psi$ with $|\psi| = O(e^{-\kappa t})$. Then
\[
\partial_t\phi = \partial_t\psi = O(e^{-\kappa t}) \to 0 \quad \text{as } t \to \infty.
\]
Since the unit normals to the slices $\Sigma_t$ converge to $\partial_t$ in the asymptotic cylinder, this yields
\[
...
\]
This is the exact boundary value.
\end{proof}
...
The cylindrical asymptotics from Theorem~\ref{thm:jang-exist}(iii) imply that the slices $\Sigma_T:=\{t=T\}$ satisfy
\[
\bg|_{\Sigma_T} = (dt^2 + g_\Sigma + O(e^{-\beta_0 t}))|_{dt=0} = g_\Sigma + O(e^{-\beta_0 T}).
\]
Hence their area converges to the area of the limiting cross-section:
\[
...
\]
Since $\phi \to 1$ along the cylindrical end and the slices $\Sigma_T$ converge to $\Sigma$,
\[
...
\begin{enumerate}[label=\textup{(\arabic*)}, leftmargin=2em, itemsep=1pt]
    ...
    \item \textbf{Area preservation:} The cross-sections of the Jang cylinder converge to the physical MOTS metric and $\phi \to 1$ along the cylindrical end, so $A_{\tg}(\Sigma) = A_{\bg}(\Sigma) = A_g(\Sigma) = A$.
...
\begin{enumerate}
    \item \textbf{Cylindrical coordinate:} On the Jang manifold, the cylindrical end $\mathcal{C} \cong [0, \infty) \times \Sigma$ carries a cylindrical parameter $t$ tending to $+\infty$ at the MOTS. The ``boundary'' $\Sigma$ is therefore represented by the limit $t \to +\infty$.
    ...
    \item \textbf{Limit of $m_{H,J}(t)$:} The value $m_{H,J}(0)$ is defined as $\lim_{t \to 0^+} m_{H,J}(t)$. The endpoint evaluation is supplied by Lemmas~\ref{lem:mots-boundary} and \ref{lem:area-conformal}, which compute the limiting geometry directly from the cylindrical end.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Jang metric regularity:} The Jang metric $\bar{g} = g + df \otimes df$ on the cylindrical end $\mathcal{C} \cong [0,\infty) \times \Sigma$ converges exponentially to the product metric $dt^2 + g_\Sigma$ with rate $\beta_0 > 0$ (Theorem~\ref{thm:jang-exist}). Thus $\bar{g}$ has the regularity supplied by the Jang theory on the interior and controlled asymptotics along the cylinder.
    
    \item \textbf{Conformal factor regularity:} By Theorem~\ref{thm:lich-exist} and Lemma~\ref{lem:phi-bound}, the conformal factor $\phi$ is a positive $C^{2,\Hoelder}$ solution on $\bM$, satisfies $\phi = 1 + O(e^{-\kappa t})$ along the cylindrical end, and its first derivatives decay to zero there.
    
    \item \textbf{Conformal metric regularity:} Since $\tilde{g} = \phi^4 \bar{g}$ with $\phi \to 1$ and $\bar{g} \to dt^2 + g_\Sigma$ exponentially as $t \to \infty$, the conformal metric $\tilde{g}$ is again asymptotically cylindrical. This is the level of regularity needed for the endpoint evaluation and for applying the AMO theory on truncated domains and passing to the limit.
    
    \item \textbf{AMO flow well-posedness:} The relevant $p$-harmonic potentials are solved with asymptotic value $0$ on the cylindrical end and value $1$ at infinity. The standard AMO theory applies to the resulting truncated problems, and the endpoint passage is the same one used throughout Section~\ref{sec:amo}.
    
    \item \textbf{Level-set regularity:} For regular values of the $p$-harmonic potential, the level sets are smooth by the implicit function theorem. The AMO identities and the $p\to1^+$ passage are applied only on such regular levels (compare Section~\ref{sec:amo}).
\end{enumerate}
In summary, the conformal metric $\tilde{g}$ has sufficient interior regularity and asymptotically cylindrical structure for the endpoint analysis used here; no stronger smooth compactification statement is needed.
\end{remark}
...
\begin{itemize}
    ...
    By the standard AMO endpoint theorem \cite[Theorem~1.3]{amo2022},
    \[
    \lim_{t\to 1^-} m_H(t)=M_{\ADM}(\tg).
    \]
    For the angular momentum term, the level sets escape to the asymptotically flat end, so $A(t)\to\infty$ while $J(t)=J$ remains constant by Theorem~\ref{thm:J-conserve}. Therefore
    \[
...
\textbf{Conclusion:}
By the monotonicity input from Stage 6 and the conformal mass-comparison input (H7):
\[
M_{\ADM}(g) \geq M_{\ADM}(\tg) = m_{H,J}(1) \geq m_{H,J}(0) = \sqrt{\frac{A}{16\pi} + \frac{4\pi J^2}{A}}.
\]
The final identity is the Stage~7 boundary evaluation at the MOTS.
\end{proof}
