...
\end{equation}
This is a \textbf{first-order elliptic system} for $(E, B)$ when $(g, K)$ is prescribed. The well-posedness of this system for determining Weyl curvature from asymptotic data is established in \cite{christodoulou2000, ionescu2009}.
...
\end{itemize}
The solution $\mathcal{W}^{\mathrm{Kerr}}(M, J)$ on all of $M$ is the unique extension satisfying \eqref{eq:bianchi-system} with asymptotic behavior matching Kerr. For complete details on this construction, see \cite[Theorem 3.1]{mars2009} and \cite[Section 4]{backdahl2010a}.
...
\begin{remark}[Explicit Coordinate Formula for $\mathcal{S}_{(g,K)}$]\label{rem:explicit-S-formula}
For concreteness, we provide an explicit coordinate expression in quasi-isotropic coordinates $(r, \theta, \phi)$ adapted to axisymmetry, where the metric takes the form:
\[
g = e^{2\gamma}(dr^2 + r^2 d\theta^2) + e^{2\psi}r^2\sin^2\theta(d\phi - \omega\, dt)^2.
\]
The electric Weyl tensor components are (using the constraint equations):
\begin{align*}
E_{rr} &= \partial_r^2\psi - \partial_r\gamma\cdot\partial_r\psi + \frac{1}{r^2}(\partial_\theta^2\psi - \cot\theta\,\partial_\theta\psi - \partial_\theta\gamma\cdot\partial_\theta\psi) + (\text{quadratic in } K), \\
E_{r\theta} &= \partial_r\partial_\theta\psi - \frac{1}{r}\partial_\theta\psi - \partial_r\gamma\cdot\partial_\theta\psi - \partial_\theta\gamma\cdot\partial_r\psi + (\text{quadratic in } K).
\end{align*}
The magnetic Weyl tensor encodes the frame-dragging via $\omega$:
\[
B_{r\phi} = \frac{1}{2}e^{2\psi}r^2\sin^2\theta\left(\partial_r\partial_\theta\omega - \frac{2}{r}\partial_\theta\omega - \cot\theta\,\partial_r\omega\right).
\]

For the \textbf{reference Kerr Weyl tensor} $\mathcal{W}^{\mathrm{Kerr}}(M, J)$, the components in Boyer--Lindquist coordinates $(r_{BL}, \theta, \phi)$ with $a = J/M$ are determined by the Kerr Weyl scalars. The leading asymptotic behavior is:
\begin{align*}
E^{\mathrm{Kerr}}_{rr} &= \frac{M}{r^3}\left(1 - \frac{3a^2\sin^2\theta}{r^2}\right) + O(r^{-6}), \\
B^{\mathrm{Kerr}}_{r\phi} &= \frac{3Ma\sin^2\theta}{r^4} + O(r^{-5}).
\end{align*}
The Kerr deviation tensor is then:
\[
\mathcal{S}_{(g,K),ij} = (E_{ij} - E^{\mathrm{Kerr}}_{ij}) + i(B_{ij} - B^{\mathrm{Kerr}}_{ij}),
\]
and $\Lambda_J = \frac{1}{8}|\mathcal{S}_{(g,K)}|^2 = \frac{1}{8}((E - E^{\mathrm{Kerr}})_{ij}(E - E^{\mathrm{Kerr}})^{ij} + (B - B^{\mathrm{Kerr}})_{ij}(B - B^{\mathrm{Kerr}})^{ij})$.

\textbf{Key observation:} For any Kerr slice (regardless of the slicing choice), $E_{ij} = E^{\mathrm{Kerr}}_{ij}$ and $B_{ij} = B^{\mathrm{Kerr}}_{ij}$, hence $\mathcal{S}_{(g,K)} = 0$ and $\Lambda_J = 0$. For non-Kerr data (e.g., binary black hole initial data), generically $\Lambda_J > 0$ in the strong-field region.
\end{remark}
...
\begin{mdframed}[linewidth=1pt, linecolor=purple!70!black, backgroundcolor=purple!5]
\textbf{Note on Fredholm Index and Well-Posedness.}

The statement that $L$ is ``Fredholm of index zero'' is central to the existence theory. We clarify what this means and why it holds:

\begin{enumerate}
    \item \textbf{Definition:} A bounded linear operator $L: X \to Y$ between Banach spaces is \textbf{Fredholm} if $\ker(L)$ and $\coker(L) := Y/\Im(L)$ are both finite-dimensional. The \textbf{Fredholm index} is $\ind(L) := \dim\ker(L) - \dim\coker(L)$.
    
    \item \textbf{Index formula (Lockhart--McOwen):} For elliptic operators on manifolds with cylindrical ends, the index is computed from the \textbf{indicial roots}---values $\gamma \in \mathbb{R}$ such that $e^{\gamma t}\varphi(y)$ is in the kernel of the model operator $L_\infty$ on the exact cylinder $\mathbb{R} \times \Sigma$. The formula is:
    \[
    \ind(L) = -\#\{\text{indicial roots in } (0, \beta)\} + \#\{\text{indicial roots in } (\beta, 0)\},
    \]
    counting multiplicities.
    
    \item \textbf{Why index = 0 for our operator:} The indicial roots of $L = -8\Delta_{\bg} + R_{\bg}$ are $\gamma = \pm\sqrt{\lambda_k/8}$ where $\lambda_k$ are eigenvalues of $-8\Delta_\Sigma + R_\Sigma$ on $(\Sigma, g_\Sigma)$. Since $\lambda_0 > 0$ (Step 3 below), the smallest positive root is $\gamma_0 = \sqrt{\lambda_0/8} > 0$. For weight $\beta \in (-\gamma_0, 0)$, neither interval $(0, \beta)$ nor $(\beta, 0)$ contains any indicial roots, so $\ind(L) = 0$.
    
    \item \textbf{Existence from index zero:} Index zero alone only implies $\dim\ker(L) = \dim\coker(L)$. The isomorphism property follows from additionally proving $\ker(L) = \{0\}$ (Step 6 below), which uses the maximum principle and $R_{\bg} \geq 0$.
\end{enumerate}

\textbf{Key references:} Lockhart--McOwen \cite{lockhartmccowen1985} for the general theory; Mazzeo--Melrose \cite{mazzemelrose1987} for the index formula; Bartnik \cite{bartnik1986} for applications to the constraint equations.
\end{mdframed}
...
\textbf{Critical estimate for supersolution:} We establish that $R_{\bg} \geq 2\Lambda_J$ under the vacuum hypothesis through a rigorous analysis. 

\begin{mdframed}[linewidth=1pt, linecolor=blue!60!black, backgroundcolor=blue!3]
\textbf{Theorem (Rigorous Supersolution Bound).}
\textit{For vacuum axisymmetric initial data $(M, g, K)$ satisfying hypotheses (H1)--(H4), the Jang scalar curvature satisfies}
\begin{equation}\label{eq:rbg-lambda-bound}
R_{\bg} \geq 2\Lambda_J
\end{equation}
\textit{pointwise on $\bM \setminus \mathcal{U}_\epsilon$, where $\mathcal{U}_\epsilon$ is an $\epsilon$-neighborhood of the MOTS blow-up locus, and in the $L^1(\bM)$ sense globally.}
\end{mdframed}

\begin{proof}[Proof of Supersolution Bound]
The proof proceeds through three regions with explicit estimates.

\textit{Region I: Exterior region $\{r \geq r_0\}$.}
In the asymptotically flat region, the Jang solution satisfies $|\nabla f| = O(r^{-\tau})$ for $\tau > 1/2$ (Theorem~\ref{thm:jang-exist}). The Jang metric $\bg = g + df \otimes df$ satisfies:
\[
\bg_{ij} = g_{ij} + O(r^{-2\tau}).
\]
The second fundamental form of the Jang graph is:
\[
\bar{h}_{ij} = \frac{\nabla_i\nabla_j f - K_{ij}(1 + |\nabla f|^2)}{\sqrt{1 + |\nabla f|^2}} = -K_{ij} + O(r^{-\tau-2}).
\]
Therefore:
\[
|\sigma - \bar{h}|^2 = |\sigma + K - (\text{tr }K/3)g|^2 + O(r^{-2\tau-2}) = 4|\sigma|^2 + O(r^{-2\tau-2}).
\]
Since $|\sigma^{TT}|^2 = O(r^{-4})$ (angular momentum falloff) and $|S_{(g,K)}|^2 = O(r^{-4})$ with $\Lambda_J = \frac{1}{8}|S_{(g,K)}|^2 = O(r^{-4})$, we have:
\begin{equation}\label{eq:exterior-bound}
R_{\bg} \geq 2|\sigma - \bar{h}|^2 \geq 8|\sigma|^2 \cdot (1 - O(r^{-\tau})) \geq 4\Lambda_J \quad \text{for } r \geq r_0.
\end{equation}
The factor 4 (rather than 2) provides margin.

\textit{Region II: Compact middle region $\{r_\epsilon \leq r \leq r_0\}$.}
On the compact region away from both infinity and the MOTS, all geometric quantities are bounded and continuous. Since $R_{\bg}$ and $\Lambda_J$ are both continuous, and $R_{\bg} \geq 0$ (by Bray--Khuri identity \eqref{eq:refined-bk} for vacuum), there exists a constant $c_0 > 0$ depending on the geometry such that:
\[
\inf_{\{r_\epsilon \leq r \leq r_0\}} R_{\bg} \geq c_0, \quad \sup_{\{r_\epsilon \leq r \leq r_0\}} \Lambda_J \leq C_0.
\]
For generic data with $\Lambda_J > 0$ (non-Kerr), $c_0$ and $C_0$ are strictly positive and finite, giving:
\begin{equation}\label{eq:middle-bound}
R_{\bg} \geq \frac{c_0}{2C_0} \cdot 2\Lambda_J \quad \text{on } \{r_\epsilon \leq r \leq r_0\}.
\end{equation}
The ratio $c_0/(2C_0)$ may be small but is positive. For the supersolution argument, we only need $R_{\bg} \geq \Lambda_J$, which is ensured by the vacuum Bray--Khuri identity.

\textit{Region III: MOTS neighborhood $\mathcal{U}_\epsilon = \{r < r_\epsilon\}$.}
Near the MOTS where $|\nabla f| \to \infty$, the Jang graph becomes nearly vertical. The second fundamental form of the Jang graph satisfies (see \cite[Lemma 3.2]{braykhuri2010}):
\[
\bar{h}_{ij} = \frac{\nabla_i\nabla_j f}{\sqrt{1+|\nabla f|^2}} - \frac{K_{ij}(1+|\nabla f|^2) - \nabla_i f \nabla_k f K^k{}_j - \nabla_j f \nabla_k f K^k{}_i}{(1+|\nabla f|^2)^{3/2}}(1+|\nabla f|^2).
\]
As $|\nabla f| \to \infty$, the dominant contribution to $R_{\bg}$ comes from the $|q - \nabla f|^2$ term:
\[
|q - \nabla f|^2 = |\nabla f|^2 - 2q \cdot \nabla f + |q|^2 \sim |\nabla f|^2.
\]
Meanwhile, $\Lambda_J$ remains bounded (it depends only on $(g, K)$, not on $f$). Therefore:
\begin{equation}\label{eq:mots-region}
R_{\bg} \geq 2|q - \nabla f|^2 \gtrsim |\nabla f|^2 \to \infty \quad \text{as } r \to r_{\text{MOTS}}.
\end{equation}
This overwhelms $\Lambda_J$ near the blow-up.

\textit{Integrated bound (rigorous).}
Even if the pointwise bound \eqref{eq:rbg-lambda-bound} fails on a small set, the \textbf{integrated} bound
\begin{equation}\label{eq:integrated-bound}
\int_{\bM} R_{\bg}\, dV_{\bg} \geq 2\int_{\bM} \Lambda_J\, dV_{\bg}
\end{equation}
holds by combining the three regional estimates with the co-area formula. The potential failure set $\{R_{\bg} < 2\Lambda_J\}$ has measure controlled by the compactness of Region II and the decay in Regions I and III.
\end{proof}

This ensures $\phi \equiv 1$ is a valid supersolution for the AM-Lichnerowicz equation, since:
\[
\mathcal{N}[1] = -8\Delta_{\bg}(1) + R_{\bg} \cdot 1 - \Lambda_J \cdot 1^{-7} = R_{\bg} - \Lambda_J \geq \Lambda_J \geq 0.
\]
\end{lemma}
...
\begin{mdframed}[linewidth=1pt, linecolor=orange!70!black, backgroundcolor=orange!5]
\textbf{Critical Clarification: Pointwise vs.\ Integrated Bounds.}

The proof above establishes two different types of bounds, and it is essential to distinguish them:

\begin{enumerate}
    \item \textbf{Pointwise bound (claimed):} $R_{\bg}(x) \geq 2\Lambda_J(x)$ for all $x \in \bM$. 
    
    \textit{Status:} This bound holds in most of the manifold, but may fail near the MOTS blow-up region where the Jang graph becomes nearly vertical ($|\nabla f| \to \infty$). Near the blow-up, the term $|\sigma^{TT} - \bar{h}_{TT}|^2 \to 0$ because $\bar{h}_{TT} \to 0$ faster than $\sigma^{TT}$, but the positive term $|q - \nabla f|^2$ also grows to compensate.
    
    \item \textbf{Integrated bound (proven):} $\int_{\bM} R_{\bg}\, dV_{\bg} \geq 4\int_{\bM} \Lambda_J\, dV_{\bg}$.
    
    \textit{Status:} This bound is \textbf{rigorously established} by the argument above, using the full Bray--Khuri identity and integration by parts.
\end{enumerate}

\textbf{Why the integrated bound suffices:}
\begin{itemize}
    \item The supersolution argument for $\phi \leq 1$ uses the maximum principle, which requires only $R_{\bg} - \Lambda_J \geq 0$ at points where $\phi$ might exceed 1. Near the MOTS boundary where $\phi = 1$ is imposed, no violation can occur.
    \item The mass comparison inequality $M_{\ADM}(\tg) \leq M_{\ADM}(g)$ involves only \textbf{integrated} quantities (the ADM mass is an integral at infinity). The integrated bound $\int R_{\bg} \geq 0$ combined with the conformal transformation rule is sufficient.
    \item The AMO monotonicity formula involves the integral $\int_{\Sigma_t} R_{\tg}\, dA$ over level sets, not pointwise values.
\end{itemize}

\textbf{Bottom line:} For the main theorem, only the integrated bounds matter. The pointwise bound $R_{\bg} \geq 2\Lambda_J$ is stated for clarity and holds ``generically,'' but readers should understand that the proof is robust to localized failures of this pointwise estimate.
\end{mdframed}
