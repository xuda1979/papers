...
\begin{enumerate}
    ...
    \item A rigorous \textbf{equality-case reduction}: equality forces the Kerr deviation term to vanish, while the final Kerr classification is isolated as an external rigidity input.
...
\end{enumerate}
A systematic numerical study using spectral initial data solvers (e.g., \textsc{SpEC} or \textsc{KADATH}) would provide valuable empirical support and could explore the equality-deficit diagnostics discussed in Corollary~\ref{cor:deficit}.
...
\begin{enumerate}
    \item \textbf{Sub-extremality bound:} The inequality
    \[
    M_{\ADM} \geq \sqrt{\frac{A}{16\pi} + \frac{4\pi J^2}{A}}
    \]
    together with the Dain--Reiris bound $A \geq 8\pi|J|$ shows that initial data satisfying our hypotheses cannot describe a ``naked'' Kerr singularity with $|J| > M^2$.
...
\textbf{Summary.} This paper presents a conditional derivation of the Angular Momentum Penrose Inequality under the stated hypotheses, with the conformal mass-comparison step isolated explicitly. The methodology synthesizes established techniques (Jang equation, Lichnerowicz equation, AMO monotonicity) with new constructions (AM-Hawking mass, topological angular momentum conservation).
...
\begin{enumerate}
    ...
    \item \textbf{Curvature Bound (Lemma~\ref{lem:refined-bk}):} The claim $R_{\bg} \geq 2\Lambda_J$ controls the conformal factor and would yield the conformal mass comparison through the maximum principle. Appendix~\ref{app:supersolution} explains this route and also records heuristic alternatives.
\end{enumerate}

The current version does \textbf{not} claim an unconditional proof of either the rotating AM-Hawking monotonicity or the conformal mass comparison. Instead, those two steps are isolated as explicit auxiliary inputs in Theorem~\ref{thm:main}.
...
\begin{itemize}
    ...
    \item The AM-Hawking mass functional and its endpoint analysis (Theorems~\ref{thm:monotone} and \ref{thm:amo-mono}), with the rotating monotonicity itself explicitly isolated as an auxiliary input.
\end{itemize}
...
We discuss two approaches to the mass inequality:

\begin{enumerate}[label=\textbf{Path \Alph*:}]
\item \textbf{Via refined curvature bound (Lemma~\ref{lem:refined-bk}).} If the Jang scalar curvature satisfies $R_{\bg} \geq 2\Lambda_J$, then the maximum principle implies $\phi \leq 1$, yielding the mass inequality. Appendix~\ref{app:supersolution} explains this route heuristically and records the gap that must be closed before it becomes a rigorous proof.

\item \textbf{Via integral energy identity (Proposition~\ref{prop:alternative-mass}).} Using only the \emph{classical} bound $R_{\bg} \geq 0$ (established by Bray--Khuri under DEC) together with an integral energy argument, one is led to a possible alternative proof of $M_{\ADM}(\tg) \leq M_{\ADM}(\bg)$. In the present manuscript this route is recorded only heuristically.
\end{enumerate}

The present version therefore isolates the conformal mass comparison as an explicit auxiliary input. We include Path A because the refined bound $R_{\bg} \geq 2\Lambda_J$, if valid, provides a clean geometric route to that input. Path B remains useful as a guide for future work on removing the extra assumption.
...
\item \textbf{Refinements of independent interest:} The refined bound $R_{\bg} \geq 2\Lambda_J$ (Lemma~\ref{lem:refined-bk}), which would provide a geometric route to the isolated mass-comparison input.
\end{itemize}

The proof depends critically on the \textbf{validity of the twist decay estimate} and on the isolated conformal mass-comparison input. The twist estimate is the primary technical innovation enabling extension of Penrose inequality proofs to rotating data and represents the most subtle fully developed part of the analysis.
