...
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}p{2.4cm}p{3.1cm}p{3.1cm}p{3.6cm}@{}}
\toprule
...
The argument is \textbf{modular} in that it does not require prior results about the Penrose inequality as direct inputs. Each stage uses established techniques from geometric analysis: Han--Khuri \cite[Theorem~1.1, Proposition~4.5]{hankhuri2013} for Jang existence, standard elliptic theory for the Lichnerowicz equation, AMO \cite[Theorem~1.1]{amo2022} for the standard area/Hawking monotonicity statements, and Dain--Reiris \cite[Theorem~1]{dain2011} for the area-angular momentum inequality on MOTS. What is new is the synthesis of these methods and the introduction of the AM-Hawking mass for the rotating case. The present version isolates two additional inputs: the rotating monotonicity of $m_{H,J}$ in (H6) and the final conformal mass comparison in (H7).
\end{remark}
...
\begin{tikzpicture}[node distance=4.2cm, auto,
    manifold/.style={rectangle, draw, text width=3.1cm, text centered, rounded corners, minimum height=2.2cm, fill=blue!10, font=\small},
...
\begin{itemize}
    ...
    \item $J(\Sigma)$ is defined using the physical data $(M, g, K)$
    \item The same conserved value is evaluated on level sets in $\tM$
...
\begin{itemize}
    ...
    \item[\textbf{(D2)}] $R_{\bg} \geq 0$ + cylindrical decay of $\Lambda_J$ + $\phi^{-8}\Lambda_J \geq 0$ $\xrightarrow{\text{Lich.}}$ $R_{\tg} \geq 0$ on $(\tM,\tg)$ \hfill (Thm~\ref{thm:lich-exist})
...
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}p{2.3cm}p{3.1cm}p{3.7cm}p{3.1cm}@{}}
\toprule
...
Lem~\ref{lem:Lambda-J-welldef} ($\Lambda_J$ well-defined) & $(g,K)$ initial data, $(M,J)$ & $\Lambda_J \geq 0$; Kerr $\Rightarrow \Lambda_J = 0$ & Mars--Simon \cite{mars2009, simon1984} \\
Thm~\ref{thm:lich-exist} (AM-Lich existence) & Thm~\ref{thm:jang-exist}, $R_{\bg} \geq 0$, cylindrical decay of $\Lambda_J$ & $\phi > 0$ exists; $\phi|_\Sigma = 1$; $\phi \to 1$ at $\infty$; $R_{\tg} = \Lambda_J \phi^{-12} \geq 0$ & Lockhart--McOwen \cite{lockhartmccowen1985} \\
Lem~\ref{lem:phi-bound} (Conformal bounds) & Thm~\ref{thm:lich-exist} + refined bound & $0 < c \leq \phi \leq 1$; decay estimates & Maximum principle \\
Lem~\ref{lem:mass-bound-direct} (Mass comparison discussion) & Thm~\ref{thm:lich-exist} & Heuristic routes toward $M_{\ADM}(g) \geq M_{\ADM}(\tg)$ & --- \\
\midrule
...
Thm~\ref{thm:monotone} (Aux. AM-Hawking mono.) & Prop~\ref{prop:amo-formula}, Thm~\ref{thm:J-conserve}, Thm~\ref{thm:subext} & Auxiliary rotating monotonicity input for $m_{H,J}$ & additional input \\
\midrule
\multicolumn{4}{@{}l}{\textit{Synthesis and Rigidity}} \\
Thm~\ref{thm:main} (Main theorem) & All of Stage 1--4 + aux. monotonicity input + conformal mass comparison input & $M_{\ADM} \geq \sqrt{A/(16\pi) + 4\pi J^2/A}$ & --- \\
Thm~\ref{thm:rigidity} (Conditional equality case) & Thm~\ref{thm:main}, $\Lambda_J = 0$ analysis & Equality $\Rightarrow \Lambda_J=0$; Kerr classification conditional & Mars \cite{mars1999}, B\"ackdahl--Valiente Kroon \cite{backdahl2010a} \\
\bottomrule
\end{tabular}
}
...
Most results in Table~\ref{tab:dependency-graph} are \textbf{unconditional} once the geometric-analytic hypotheses of the paper are assumed. However, the final synthesis step requires clarification:
\begin{itemize}
    ...
    \item \textbf{Rotating monotonicity:} The manuscript explains how the ordinary AMO monotonicity, $J$-conservation, and sub-extremality would interact, but does not provide a rigorous derivation of the rotating monotonicity inequality for $m_{H,J}$. That step is therefore isolated as the auxiliary input in Theorem~\ref{thm:monotone}.
    \item \textbf{Mass comparison:} The manuscript's heuristic appendix records additional possible routes to $M_{\ADM}(g) \geq M_{\ADM}(\tilde g)$, but these are not used as completed proofs in the present version. Accordingly, Theorem~\ref{thm:main} is also stated with the conformal mass-comparison step isolated as an auxiliary input.
\end{itemize}
\textbf{The main theorem (Theorem~\ref{thm:main}) therefore depends on two explicit auxiliary inputs: the rotating AM-Hawking monotonicity and the conformal mass comparison.} See Section~\ref{sec:amo} for the status of the former and Section~\ref{sec:lichnerowicz} plus Appendix~\ref{app:supersolution} for the latter.
\end{remark}
...
\begin{enumerate}[label=\textup{(\roman*)}]
    ...
    \item \textbf{Mars--Simon Kerr characterization} \cite{mars1999, simon1984}: External rigidity input used only in the equality classification.
...
\begin{enumerate}[label=(T\arabic*)]
    ...
    \item \textbf{Sub-Extremality} (\S\ref{sec:subextremality}): The Dain--Reiris inequality \cite{dain2011} gives $A(t) \geq 8\pi|J|$ for all $t$, ensuring the sub-extremality factor available to any rotating monotonicity argument is non-negative.
\end{enumerate}
...
\setlength{\tabcolsep}{3pt}
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}p{3.5cm}p{5.8cm}p{2.2cm}@{}}
\toprule
...
Jang blow-up rate & $f(s,y) = C_0 \ln s^{-1} + O(1)$ with $C_0>0$ & Thm~\ref{thm:jang-exist}(ii) \\
Admissible weight selection & choose a real weight away from the discrete cylindrical indicial set & Prop.~\ref{prop:lm-compat} \\
...
AM-Hawking monotonicity input & monotonicity of $m_{H,J}$ along the rotating flow & Thm~\ref{thm:monotone} \\
...
\begin{enumerate}[label=\textup{(\roman*)}]
    ...
    \]
    away from the cylindrical end, and the cylindrical end metric satisfies exponential convergence to the product $dt^2 + g_\Sigma$ with some rate $\beta_0 > 0$ controlled by the strict-stability spectral gap.
...
\begin{itemize}
    \item The ``bounded geometry'' assumptions used in estimates throughout this paper (e.g., in Lemma~\ref{lem:uniform-p-estimates} and Remark~\ref{rem:explicit-constants}) are \textbf{verified} for our class of data by Lemma~\ref{lem:bounded-geometry}. Any additional rotating-monotonicity estimates are now isolated as auxiliary input rather than derived from a separate in-paper Willmore derivative formula.
