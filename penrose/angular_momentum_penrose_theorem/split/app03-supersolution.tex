...
Some readers may question whether the maximum principle bound $\phi \leq 1$ from Theorem~\ref{thm:lich-exist} is essential for the main result. This appendix records several routes that might lead to the needed conformal mass comparison. The pointwise route via the refined bound is geometrically natural; the integral routes are included as heuristic alternatives rather than completed proofs.
\end{remark}

This appendix provides a heuristic treatment of the super-solution issue raised in Remark~\ref{rem:supersolution-clarification}. Its purpose is to isolate the conformal mass-comparison step and explain possible approaches to it.

\subsection{The Mass Chain Without \texorpdfstring{$\phi \leq 1$}{phi <= 1}}

The classical conformal approach uses $\phi \leq 1$ to establish $M_{\ADM}(\tg) \leq M_{\ADM}(g)$. The discussion below explains why removing that pointwise input is subtle.

\begin{proposition}[Heuristic Mass Identity]\label{prop:mass-bound-energy}\label{lem:mass-bound-direct}
Let $\phi > 0$ solve the AM-Lichnerowicz equation \eqref{eq:am-lich} with $\phi|_\Sigma = 1$ and $\phi \to 1$ at infinity. Formally one is led to the comparison
\[
M_{\ADM}(\tg) \leq M_{\ADM}(\bg) \leq M_{\ADM}(g),
\]
but the sign analysis below does not, by itself, furnish a rigorous proof of the first inequality.
\end{proposition}
...
\textbf{Step 3: Sign analysis and obstruction.}
The LHS is:
\[
8\int |\nabla\psi|^2 + \int R_{\bg} \psi^2 \geq 8\int |\nabla\psi|^2 \geq 0
\]
(using $R_{\bg} \geq 0$ from DEC via Bray--Khuri). However, this does \emph{not} determine the sign of the right-hand side without additional input, so the argument does not yet prove the mass inequality.
...
\item \textbf{This estimate would imply the clean pointwise route to the mass comparison.}
...
\]
should hold for vacuum initial data, with equality at least on Kerr data (where both sides vanish).
...
This estimate is relevant because it would establish the pointwise bound $\phi \leq 1$, hence the conformal mass comparison. The discussion below explains why alternative routes remain heuristic in the present version.
...
\textbf{Alternative approach:} If Lemma~\ref{lem:refined-bk} proves too difficult to verify or if errors are found in the derivation, one may seek a different proof of the mass comparison. Proposition~\ref{prop:alternative-mass} records such a strategy, but only at a heuristic level.
\end{remark}
\end{mdframed}
...
In case the refined Bray--Khuri bound (Lemma~\ref{lem:refined-bk}) is not available, we record an alternative strategy for the mass inequality that relies only on the \textbf{classical} bound $R_{\bg} \geq 0$ and the structure of the AM-Lichnerowicz equation.

\begin{proposition}[Heuristic Alternative Mass Comparison]\label{prop:alternative-mass}
Let $\phi > 0$ solve the AM-Lichnerowicz equation \eqref{eq:am-lich} with $\phi|_\Sigma = 1$ and $\phi \to 1$ at infinity. A natural strategy is to try to prove, without assuming $R_{\bg} \geq 2\Lambda_J$, that
\[
...
\]
The goal would be to show this boundary flux is non-positive.
...
\[
\begin{aligned}
8\int_{\bM} |\nabla\phi|^2 \frac{d(\phi-1)}{d\phi} \, dV
&+ \int_{\bM} R_{\bg}\phi(\phi-1) \, dV \\
&= \int_{\bM} \Lambda_J \phi^{-7}(\phi-1) \, dV + \text{(boundary)}.
\end{aligned}
\]
...
\textbf{Conclusion:} As written, the argument does \emph{not} prove that the boundary flux is non-positive. The sign discussion identifies the relevant terms, but additional structure would be needed to turn this into a rigorous proof of $M_{\ADM}(\tg) \leq M_{\ADM}(\bg)$.
\end{proof}

\begin{remark}[Status of the Mass-Comparison Step]\label{rem:robustness}
The discussion in this appendix isolates two conceptual routes toward the conformal mass comparison:
\begin{enumerate}
    ...
    \item \textbf{Path B (via integral identity):} Use only $R_{\bg} \geq 0$ (classical Bray--Khuri) and try to control the boundary flux through an energy identity.
\end{enumerate}
Path A would be rigorous once Lemma~\ref{lem:refined-bk} is established. Path B, as written here, remains heuristic.

\textbf{Implications:}
\begin{itemize}
\item \textbf{Path A is the clean rigorous route currently identified in the manuscript.}
\item \textbf{Path B is useful diagnostically.} It clarifies what must still be controlled if one wants an unconditional mass comparison.
\item \textbf{The appendix no longer claims robustness of the main theorem against failure of the refined bound.} Instead, it isolates the mass-comparison step as the remaining auxiliary input.
\end{itemize}
\textbf{Why present both paths?} We include both discussions for the following reasons:
\begin{enumerate}[label=(\roman*)]
...
\begin{proposition}\label{prop:mono-independence}
