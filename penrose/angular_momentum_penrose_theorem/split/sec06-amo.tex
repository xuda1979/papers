...
\begin{enumerate}
    ...
    \item The Komar form $\alpha_J$ and its Hodge dual $\star_g\alpha_J$ are smooth differential forms on $M$, and pullback commutes with exterior differentiation: for the inclusion $\iota_t:\Sigma_t\hookrightarrow M$, one has $\iota_t^*(d\omega)=d(\iota_t^*\omega)$ for any form $\omega$.
...
\]
where $\star_g \alpha_J$ is the Hodge dual of the Komar 1-form (a 2-form). For axisymmetric data with $\nu \perp \eta$, Stokes' theorem applied to the 2-form $\star_g \alpha_J$ (equivalently, using $d(\star \alpha) = -\star(d^\dagger \alpha)$ for 1-forms in 3D, so $d^\dagger\alpha=0\Rightarrow d(\star\alpha)=0$) yields:
\[
J(t_2) - J(t_1) = \int_{\Sigma_{t_2}} \star_g\alpha_J - \int_{\Sigma_{t_1}} \star_g\alpha_J = \int_{\{t_1 < u < t_2\}} d(\star_g\alpha_J).
\]

\textbf{Step 5: Conservation via co-closedness (not $d\alpha_J=0$).}
The key calculation uses the momentum constraint and axisymmetry to show $d^\dagger_g\alpha_J$ is controlled. Define the 1-form:
\[
...
While one may also analyze $d\alpha_J$, the Stokes argument uses the 2-form $\omega_J := \star_g\alpha_J$ and the identity $d\omega_J = -\star_g(d^\dagger_g\alpha_J)$.
...
\end{remark}
\textit{(Non-vacuum source term).} In general one does not have $d^\dagger_g\alpha_J=0$. What matters for Stokes' theorem is the 2-form
\[
\omega_J := \star_g \alpha_J.
\]
Using the momentum constraint and axisymmetry (so $\mathcal{L}_\eta(\tr K)=0$ and $K$ is symmetric), one obtains
\[
d^\dagger_g \alpha_J = -(\momdens\cdot \eta),
\qquad\text{hence}\qquad
d\omega_J = (\momdens\cdot \eta)\, dV_g,
\]
where $(\momdens\cdot\eta):=\momdens_i\eta^i$ and $dV_g$ is the volume form of the physical metric.
In particular, in the vacuum region $\momdens\equiv 0$, we have $d\omega_J=0$.

\textbf{Step 6: Conservation in vacuum.}
Let $W=\{t_1\le u\le t_2\}$ be the cobordism between two regular level sets (as in Lemma~\ref{lem:homology}). In vacuum, Stokes' theorem gives
\[
J(t_2)-J(t_1)=\int_{\Sigma_{t_2}}\omega_J-\int_{\Sigma_{t_1}}\omega_J=\int_W d\omega_J=0.
\]
Thus $J(t)$ is constant along the foliation in the vacuum region swept out by the level sets.
\end{proof}
...
\begin{enumerate}[label=\textup{(\roman*)}]
    ...
    $\mathcal{W}(t) = 1 - \frac{2M_{\ADM}}{r(t)} + O(r^{-1-\tau}) \to 1^-$ (Lemma~\ref{lem:adm-convergence}).
\end{enumerate}
\end{lemma}
\end{mdframed}
...
\textbf{(iii)} The asymptotic statement as $t\to 1$ follows from the standard expansion for large coordinate spheres in the asymptotically flat end (Lemma~\ref{lem:adm-convergence}).
\end{proof}
...
\]
where $m_H(t) = \sqrt{A(t)/(16\pi)}\bigl(1 - W(t)/(16\pi)\bigr)$ is the standard Hawking mass, $A(t) = |\Sigma_t|_{\tg}$ is the area, $W(t) = \int_{\Sigma_t} H^2 \, dA_{\tg}$ is the Willmore functional, and $J$ is the conserved Komar angular momentum.
...
\]
Define the \textbf{unnormalized Willmore functional} $W(t) := \int_{\Sigma_t} H^2 \, d\sigma$ and its normalized version $\mathcal{W}(t) := \frac{W(t)}{16\pi} = \frac{1}{16\pi}\int_{\Sigma_t} H^2 \, d\sigma$. Then
\(m_H(t) = \sqrt{A(t)/(16\pi)}\bigl(1-\mathcal{W}(t)\bigr)\) and $m_H^2(t) = \frac{A(t)}{16\pi}(1-\mathcal{W}(t))^2$.

\textbf{Step 2: Derivative of $m_H^2$.}
With $m_H^2 = \frac{A}{16\pi}(1-\mathcal{W})^2$, we compute:
\begin{align}
\frac{d}{dt}m_H^2 &= \frac{d}{dt}\left[\frac{A}{16\pi}(1 - \mathcal{W})^2\right] \\
&= \frac{A'}{16\pi}(1 - \mathcal{W})^2 + \frac{A}{16\pi} \cdot 2(1-\mathcal{W})(-\mathcal{W}') \\
&= \frac{(1-\mathcal{W})}{16\pi}\left[A'(1 - \mathcal{W}) - 2A\mathcal{W}'\right].
\end{align}
...
\[
\mathcal{G}(t) := \mathcal{W}(t) - 1 + \frac{8\pi}{A(t)}.
\]
...
\end{align}
From Step 2, with $m_H^2 = \frac{A}{16\pi}(1-\mathcal{W})^2$:
\begin{align}
\frac{d}{dt}m_{H,J}^2 &= \frac{(1-\mathcal{W})}{16\pi}\left[A'(1 - \mathcal{W}) - 2A\mathcal{W}'\right] - \frac{4\pi J^2}{A^2}A'.
\end{align}
...
\frac{d}{dt}m_H^2 \geq \frac{1}{8\pi}\int_{\Sigma_t}\frac{R_{\tg} + 2|\mathring{h}|^2}{|\nabla u|}\,d\sigma \cdot \left(1 - \mathcal{W}\right),
\end{equation}
where $\mathcal{W} = \frac{1}{16\pi}\int_{\Sigma_t}H^2\,d\sigma$ is the normalized Willmore factor. This uses $m_H^2 = \frac{A}{16\pi}(1-\mathcal{W})^2$.
...
\[
A' \leq \sqrt{16\pi \mathcal{W} \cdot A}\cdot\sqrt{A\cdot \bar{|\nabla u|^{-1}}} = A\sqrt{16\pi \mathcal{W} \cdot \bar{|\nabla u|^{-1}}}.
\]

\textit{(8d) Combining the estimates.} From \eqref{eq:amo-simplified}:
\begin{align}
\frac{d}{dt}m_{H,J}^2 &= \frac{d}{dt}m_H^2 - \frac{4\pi J^2}{A^2}A' \\
&\geq \frac{1}{8\pi}\int_{\Sigma_t}\frac{R_{\tg} + 2|\mathring{h}|^2}{|\nabla u|}\,d\sigma \cdot (1-\mathcal{W}) - \frac{4\pi J^2}{A^2}\int_{\Sigma_t}\frac{H}{|\nabla u|}\,d\sigma.
\end{align}
...
We have:
\[
\frac{d}{dt}m_{H,J}^2 \geq \frac{(1-\mathcal{W})}{8\pi}I_R - \frac{4\pi J^2}{A^2}I_H.
\]
...
\textit{Step (i): Bound $A'$ in terms of $I_R$.} From the Hawking mass formula $m_H^2 = \frac{A}{16\pi}(1-\mathcal{W})^2$ and the AMO derivative \eqref{eq:amo-simplified}:
\[
\frac{d}{dt}m_H^2 \geq \frac{(1-\mathcal{W})}{8\pi}I_R.
\]
On the other hand, differentiating $m_H^2 = \frac{A}{16\pi}(1-\mathcal{W})^2$:
\[
\frac{d}{dt}m_H^2 = \frac{(1-\mathcal{W})}{16\pi}\left(A'(1-\mathcal{W}) - 2A\mathcal{W}'\right).
\]
The Willmore derivative $\mathcal{W}' = \frac{d}{dt}\left(\frac{1}{16\pi}\int H^2\right)$ requires explicit estimation. By the first variation of the Willmore functional along a foliation with lapse $|\nabla u|^{-1}$ (see \cite[Eq. (2.3)]{simonwillmore1993}):
\[
\mathcal{W}' = \frac{1}{16\pi}\int_{\Sigma_t} \left(2H\cdot \frac{\partial H}{\partial t} + H^2 \cdot \frac{A'}{A}\right)d\sigma.
\]
...
\[
|\mathcal{W}'| \leq \frac{1}{16\pi}\left(2\|H\|_{L^2}\|\partial_t H\|_{L^2} + \|H\|_{L^2}^2 \cdot \frac{A'}{A}\right) \leq C_W\left(\frac{A'}{A} + \frac{I_R}{A}\right),
\]
where $C_W = C_W(\|\Ric_{\tg}\|_{L^\infty}, \|A_\Sigma\|_{L^\infty})$ is an explicit constant depending on the geometry bounds from Lemma~\ref{lem:bounded-geometry}. For vacuum data with decay rate $\tau > 1/2$, these bounds are finite: $C_W \leq C(n, \tau, \|K\|_{C^2})$. In the regime where $\mathcal{W}$ is small (i.e., $m_H^2 \approx \frac{A}{16\pi}$), we have:
\[
A'(1-\mathcal{W}) \lesssim 16\pi \cdot \frac{(1-\mathcal{W})}{8\pi}I_R = 2(1-\mathcal{W})I_R.
\]
Hence $A' \lesssim \frac{2I_R}{1}$ when $(1-\mathcal{W}) \approx 1$. More precisely:
\begin{equation}\label{eq:Aprime-IR-bound}
A' \leq \frac{C \cdot I_R}{(1-\mathcal{W})} \quad \text{for some universal constant } C > 0.
\end{equation}
For our purposes, we use the weaker bound:
\begin{equation}\label{eq:subext-factor}
\frac{4\pi J^2}{A^2}I_H = \frac{4\pi J^2}{A^2}A' \leq \frac{C \cdot 4\pi J^2}{A^2(1-\mathcal{W})}I_R.
\end{equation}


\textit{Step (ii): Combined estimate.} Substituting \eqref{eq:subext-factor} into the derivative formula:
\begin{align}
\frac{d}{dt}m_{H,J}^2 &\geq \frac{(1-\mathcal{W})}{8\pi}I_R - \frac{C \cdot 4\pi J^2}{A^2(1-\mathcal{W})}I_R \\
&= \frac{I_R}{8\pi(1-\mathcal{W})}\left((1-\mathcal{W})^2 - \frac{32\pi^2 C J^2}{A^2}\right).
\end{align}
...
\]
When $(1-\mathcal{W})^2 \geq C/2$ (i.e., for surfaces with $(1-\mathcal{W})$ bounded away from $0$), the expression is non-negative.

\textit{(8g) Simplification using sub-extremality.} For $A \geq 8\pi|J|$:
\[
1 - \frac{64\pi^2 J^2}{A^2} = 1 - \left(\frac{8\pi|J|}{A}\right)^2 \geq 0.
\]
...
More directly, combining the Hawking mass derivative lower bound \eqref{eq:mH2-lower} with the area term estimate \eqref{eq:subext-factor} yields \eqref{eq:geroch-am-final}.
...
\begin{remark}[Clarification: The Willmore Factor $(1-\mathcal{W})$]\label{rem:willmore-clarification}
The normalized Willmore factor $\mathcal{W}(t) := \frac{1}{16\pi}\int_{\Sigma_t} H^2 \, d\sigma$ satisfies $\mathcal{W}(t) \geq 0$, hence $(1-\mathcal{W}(t)) \leq 1$ always. We clarify how this factor is handled:
\begin{enumerate}[label=\textup{(\roman*)}]
    \item \textbf{At $t = 0$ (MOTS):} The MOTS $\Sigma$ is minimal in the conformal metric $\tg$ (Lemma~\ref{lem:mots-boundary}), so $H|_\Sigma = 0$ and thus $\mathcal{W}(0) = 0$. Therefore $(1-\mathcal{W}(0)) = 1$.
    \item \textbf{Along the flow:} For $t > 0$, we have $\mathcal{W}(t) \geq 0$, so $(1-\mathcal{W}(t)) \leq 1$. The monotonicity argument requires $(1-\mathcal{W}) > 0$, i.e. $\mathcal{W} < 1$.
    \item \textbf{Absorption by sub-extremality:} The factor $(1-\mathcal{W})$ from the AMO formula and the angular momentum term $4\pi J^2/A^2$ appear in a combined expression where the sub-extremality factor $(1 - 64\pi^2 J^2/A^2)$ provides the dominant control. The final form \eqref{eq:geroch-am-final} incorporates both contributions correctly.
    \item \textbf{Integrated monotonicity:} The bound $m_{H,J}(0) \leq m_{H,J}(1)$ depends on the \textbf{integrated} behavior, not pointwise values of $(1-\mathcal{W}(t))$. Since $\frac{d}{dt}m_{H,J}^2 \geq 0$ for almost all $t$ (by the non-negativity of the integrand in \eqref{eq:geroch-am-final}), the monotonicity follows regardless of the local value of $\mathcal{W}(t)$.
\end{enumerate}
\end{remark}
...
In particular:
\begin{enumerate}
    \item At $t = 0$ (horizon): $m_{H,J}(0) = \sqrt{A/(16\pi) + 4\pi J^2/A}$, since a MOTS satisfies $\theta^+ = H + \tr_\Sigma K = 0$ (so $H = -\tr_\Sigma K$), and the Willmore integral $\int_\Sigma H^2 d\sigma$ is bounded by sub-extremality considerations. For a stable MOTS satisfying the Dain--Reiris bound, the Hawking mass satisfies $m_H(\Sigma) \geq \sqrt{A/(16\pi)}(1 - \epsilon)$ for small geometric corrections $\epsilon$.
