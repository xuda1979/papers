...
\begin{itemize}
    ...
    \item The preservation along the flow uses the $J$-conservation from Stage 3 and the area monotonicity $A(t) \ge A(0)$. The Dain--Reiris theorem is applied only at the MOTS $\Sigma$ ($t=0$) to establish the initial bound, which is then propagated by the flow.
\end{itemize}

\textbf{What this section does NOT use:}
\begin{itemize}
    \item The sub-extremality bound does \textbf{not} use the AMO monotonicity (Stage 3).
...
\begin{enumerate}[label=\textup{(\roman*)}]
    ...
    \]
    \emph{Note:} The Dain--Reiris inequality is applied \textbf{only} at the MOTS $\Sigma$ ($t=0$). The bound for $t > 0$ follows from the monotonicity of area $A(t)$ and the conservation of $J$. We do \textbf{not} claim that every level set $\Sigma_t$ is a stable MOTS.
...
\begin{enumerate}
    \item[(1)] \textbf{Initial bound (Dain--Reiris):} At $t = 0$, the outermost MOTS $\Sigma_0 = \Sigma$ satisfies $A(0) \geq 8\pi|J|$ by the Dain--Reiris theorem \cite{dain2011}. This is a \textbf{one-time application} at the MOTS only. We emphasize that the Dain--Reiris theorem is \textbf{not} applied to the level sets $\Sigma_t$ for $t > 0$, as they are not MOTS in general.
