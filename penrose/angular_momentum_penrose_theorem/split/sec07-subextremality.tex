...
\textbf{Step 2: Ancillary mass-angular momentum comparison.}
For orientation, Dain \cite{dain2008} proved a separate mass-angular-momentum inequality under additional global hypotheses:
\[
M_{\ADM} \geq \sqrt{|J|},
\]
with equality characterized by extreme Kerr in that setting. This comparison is \textbf{not used} in the argument below; we record it only as background. In particular, the preservation proof that follows uses only the Dain--Reiris inequality on the initial MOTS together with AMO area monotonicity and angular-momentum conservation.
\medskip

\noindent It implies the familiar total-angular-momentum sub-extremal bound
\[
...
\begin{enumerate}
    ...
    \item[(iii)] \textbf{$A$ is non-decreasing:} By the standard AMO area monotonicity formula, $A'(t) \geq 0$ for almost all $t \in (0,1)$:
    ...
    \end{itemize}
    Since $|\nabla u| > 0$ on regular level sets (and regular values form a full-measure subset of $(0,1)$ by Remark~\ref{rem:p-harmonic-regularity}), we conclude $A'(t) \geq 0$ for a.e.\ $t \in (0,1)$.
    
    \textit{Remarks on the input:}
    \begin{enumerate}
        \item The AMO formula \eqref{eq:amo-formula-subext} is a cited result from \cite[Theorem 3.1]{amo2022}, summarized in Proposition~\ref{prop:amo-formula}.
...
\begin{enumerate}
    \item \textbf{Jang equation blow-up (Theorem~\ref{thm:jang-exist}):} Stability is used in the cited Jang theory to obtain logarithmic blow-up at $\Sigma$ with positive coefficient $C_0>0$ and controlled cylindrical asymptotics. For unstable MOTS, the Jang solution may exhibit more complicated behavior (e.g., oscillatory or non-monotonic blow-up).
