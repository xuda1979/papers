...
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}p{2.3cm}p{3.8cm}p{3.8cm}p{1.7cm}@{}}
\toprule
\textbf{External Result} & \textbf{Their Hypotheses} & \textbf{Our Setting} & \textbf{Status} \\
\midrule
...
\]
where $\beta_0>0$ is the exponential cylindrical decay rate furnished by strict stability (Remark~\ref{rem:mots-decay-alignment}).
...
\]
where $\mu_k$ are eigenvalues of $-\Delta_\Sigma + R_\Sigma/8$ on $\Sigma$. The resulting indicial set is discrete, so one may choose a real weight $\delta<0$ that avoids it. This is the standard Lockhart--McOwen admissibility condition.

\textbf{(LM4) Decay rate requirement:} The geometric decay rate $\beta_0>0$ (from strict stability) means the actual coefficients converge exponentially to their cylindrical limits. Choosing $\delta<0$ sufficiently close to $0$ and away from the discrete indicial set places the problem in the usual Fredholm regime.
...
\begin{proposition}[Compatibility with the Conditional Rigidity Input {\cite{mars1999, mars2009}}]\label{prop:mars-compat}
The equality-case reduction in Theorem~\ref{thm:rigidity} is compatible with using an external Mars--Simon/KID rigidity input to identify Kerr.
\end{proposition}
...
\textbf{(MS4) Stationarity (for spacetime):} The Mars theorem originally assumed stationarity. In the present paper we do \textbf{not} claim to derive that hypothesis from the AMO argument itself. Instead, Theorem~\ref{thm:rigidity} isolates the vanishing of the Kerr deviation tensor as the output of the equality case, and any passage from $\mathcal{S}_{(g,K)}=0$ to ``Kerr slice'' is treated as an additional external rigidity input.

The Kerr deviation tensor $\mathcal{S}_{(g,K)}$ is constructed intrinsically from $(g, K)$ using the electric/magnetic decomposition of the Weyl tensor (see Appendix~\ref{app:mars-simon}), and vanishes on Kerr slices.
