...
\midrule
\textbf{ESTABLISHED in this paper} (with explicit auxiliary inputs) & 
\begin{itemize}[leftmargin=1em, nosep]
    \item Main Theorem~\ref{thm:main}: Angular Momentum Penrose Inequality, stated conditionally via the auxiliary rotating monotonicity input (H6) and the conformal mass-comparison input (H7)
    \item Conditional equality reduction (Theorem~\ref{thm:rigidity})
\end{itemize} \\
\addlinespace
...
\begin{align}
    R_g + (\tr_g K)^2 - |K|_g^2 &= 2\mu_{EM} = 2(|E|^2 + |B|^2), \label{eq:hamiltonian-EM}\\
    \Div_g(K - (\tr_g K)g) &= \momdens_{EM} = 2(E \times B), \label{eq:momentum-EM}
\end{align}
where the electromagnetic energy-momentum contributions are:
\begin{equation}
    \mu_{EM} = |E|^2 + |B|^2, \qquad \momdens_{EM} = 2(E \times B).
\end{equation}
\end{definition}
...
\end{itemize}
\textbf{Our contribution} is demonstrating that the Jang--conformal--AMO framework developed for the angular momentum case (Theorem~\ref{thm:main}) extends naturally to the charged case, providing a \textbf{unified methodological approach}. This contrasts with Theorem~\ref{thm:main}, for which the Jang, AM-Lichnerowicz, angular-momentum-conservation, and sub-extremality stages are verified in detail while the rotating monotonicity and conformal mass-comparison steps are isolated as the additional inputs (H6)--(H7).
\end{mdframed}
...
\begin{enumerate}[label=\textup{(C\arabic*)}]
    ...
    \[
    \mu = \frac{1}{2}\left(R_g + (\tr_g K)^2 - |K|_g^2\right) - (|E|^2 + |B|^2) \geq 0
    \]
...
\medskip
\noindent
	extit{Convention note.} Throughout the paper we use the scaled energy and momentum densities
\(\mu := \tfrac12(R_g + (\tr_g K)^2 - |K|_g^2)\) and \(\momdens := \Div_g(K - (\tr_g K)g)\). With this convention the electromagnetic contributions satisfy
\(\mu_{EM} = |E|^2 + |B|^2\) and \(\momdens_{EM} = 2(E\times B)\), so the matter density (excluding electromagnetic energy) is
\(\mu = \tfrac12(R_g + (\tr_g K)^2 - |K|_g^2) - (|E|^2+|B|^2)\).
...
\end{equation}
Then the classical charged Penrose inequality gives the sharp bound:
...
\end{equation}
In the classical theorem, equality characterizes Reissner--Nordstr\"om data. In this section, however, that statement is used only as background from the literature rather than re-proved here.
\end{theorem}
...
\textbf{Important distinction:} Unlike Theorem~\ref{thm:main}, which is stated conditionally through the isolated rotating monotonicity and conformal mass-comparison inputs (H6)--(H7), Theorem~\ref{thm:charged-penrose} is presented only as a \textbf{proof outline}. Readers seeking a rigorous proof of the charged Penrose inequality should consult \cite{mars2009, khuri2015charged}.
\end{remark}
...
\begin{proof}[Proof Strategy Outline for Theorem~\ref{thm:charged-penrose}]
We explain how the Jang--conformal--AMO method from Section~\ref{sec:proof-outline} should adapt in the charged, non-rotating setting. This is a \emph{programmatic outline}, not a completed proof.
...
\textbf{Stage 2: Charged conformal step.}

On the Jang manifold $(\bar{M}, \bar{g})$, one seeks a conformal factor $\phi$ solving an elliptic equation of the same general type as the AM-Lichnerowicz equation, but with a non-negative source built from the electromagnetic energy density on the Jang manifold. The exact charged analogue depends on how the fields $(E,B)$ transform under the Jang construction and on the precise conformal bookkeeping needed to preserve non-negative scalar curvature after the conformal change.

\begin{lemma}[Existence for Charge-Modified Lichnerowicz]\label{lem:lich-charged-exist}
Assuming the charged conformal equation is identified in a form compatible with the Jang geometry and cylindrical-end asymptotics, one expects a unique positive solution $\phi$ with:
...
\begin{proof}
This step should parallel the analysis in Section~\ref{sec:lichnerowicz}, once the correct charged source term and its cylindrical-end decay are established. Those analytic details are not carried out here.
\end{proof}
...
\textbf{Stage 5: Christodoulou mass as the candidate monotone quantity.}
...
\end{equation}
where $m_H(t) = \sqrt{A(t)/(16\pi)}(1 - W(t)/(16\pi))$ is the standard Hawking mass and $W(t) = \int_{\Sigma_t} H^2$ is the Willmore functional. For a MOTS ($H=0$), this reduces to 
\[
m_H = \sqrt{A/(16\pi)},
\]
the irreducible mass. This is defined for $m_H(t) > 0$.

\begin{lemma}[Heuristic Monotonicity Mechanism]\label{lem:mC-monotone}
The Christodoulou functional
\[
m_C(t)=m_H(t)+\frac{Q^2}{4m_H(t)}
\]
is the natural candidate for the charged monotone quantity along the AMO flow.
\end{lemma}

\begin{proof}
Formally,
\[
\frac{d}{dt}m_C(t)=\frac{d m_H}{dt}\left(1-\frac{Q^2}{4m_H(t)^2}\right),
\]
since $Q$ is constant along the flow. This identifies the structural issue: to prove monotonicity, one needs a charged lower bound on $m_H(t)$ strong enough to control the factor in parentheses on every regular leaf, not merely on the initial MOTS where $H=0$ and $m_H^2=A/(16\pi)$. Establishing that control is part of the missing charged analysis.
\end{proof}

\begin{remark}[Why the Christodoulou Form Works]
The Christodoulou functional $m_C = m_H + Q^2/(4m_H)$ is the right candidate because:
\begin{enumerate}
    \item It reduces to the irreducible-mass Christodoulou expression on the horizon;
    \item It incorporates the conserved charge $Q$ in the sharp algebraic form known from Reissner--Nordstr\"om;
    \item Any successful monotonicity proof must explain why the charge contribution does not overwhelm the increase in $m_H$ along the flow.
\end{enumerate}
...
\begin{equation}
    m_H(1) \to M_{\ADM}.
\end{equation}
For a rigorous charged proof, one must identify the correct asymptotic limit of the Christodoulou-type functional on large spheres and show that it converges to the physical ADM mass of the Einstein--Maxwell data. This matching is part of the missing asymptotic analysis and cannot simply be read off from the uncharged Hawking-mass limit.

\textbf{Stage 7: Conclusion.}

If the charged conformal step, the charged monotonicity statement, and the asymptotic identification at infinity are all established, then one would obtain
\[
M_{\ADM}\ge m_C(0)=M_{\mathrm{irr}}+\frac{Q^2}{4M_{\mathrm{irr}}},
\]
and hence the squared form \eqref{eq:charged-penrose-squared} by algebra.

\textbf{Equality-case discussion:}

In a completed charged proof, equality would force the charged monotonicity defects to vanish. One would then expect:
\begin{itemize}
    \item Vanishing of the relevant scalar-curvature and second-fundamental-form error terms;
    \item One is then reduced to a charged rigidity problem analogous to Section~\ref{sec:rigidity}: to conclude Reissner--Nordstr\"om, one needs an additional electrovacuum rigidity input not developed in this paper.
\end{itemize}
\end{proof}
...
\end{itemize}
Our contribution is to isolate a plausible Jang--AMO route to the Christodoulou form for non-rotating electrovacuum data and to identify the correct cross-term that must appear in any such argument.
\end{remark}
...
\begin{corollary}[Formal Extremal Consequence]\label{cor:extremal-bound}
The charged Penrose inequality implies that any charged black hole in the non-rotating Einstein--Maxwell setting satisfies
\begin{equation}
    M_{\ADM} \geq |Q|
\end{equation}
and, in the classical equality case, extremality corresponds to Reissner--Nordstr\"om with $A = 4\pi Q^2$ and $M = |Q|$.
\end{corollary}

\begin{proof}
Treat the right-hand side of \eqref{eq:charged-penrose-main} as a function of $M_{\mathrm{irr}}$:
\[
f(M_{\mathrm{irr}})=M_{\mathrm{irr}}+\frac{Q^2}{4M_{\mathrm{irr}}}.
\]
Its minimum occurs at
\[
\frac{d}{dM_{\mathrm{irr}}}f(M_{\mathrm{irr}})=1-\frac{Q^2}{4M_{\mathrm{irr}}^2}=0
\quad\Rightarrow\quad
M_{\mathrm{irr}}=\frac{|Q|}{2},
\]
and then
\[
f_{\min}=\frac{|Q|}{2}+\frac{|Q|}{2}=|Q|.
\]
The corresponding horizon area is $A=16\pi M_{\mathrm{irr}}^2=4\pi Q^2$, i.e. the extremal Reissner--Nordstr\"om value.
\end{proof}
...
\begin{enumerate}[label=(\Roman*)]
    ...
    M \geq M_{irr} + \frac{Q^2}{4M_{irr}} \qquad \Bigl(M_{irr}=\sqrt{\tfrac{A}{16\pi}}\Bigr)
    \end{equation}
    This is the charged Penrose (Christodoulou) form; squaring gives
    $M^2 \geq \tfrac{A}{16\pi} + \tfrac{Q^2}{2} + \tfrac{\pi Q^4}{A}$.
\end{enumerate}
...
\begin{itemize}
    \item Violation of \eqref{eq:mass-charge} means $|Q| > M$, which would destroy the Reissner-Nordstr\"om horizon;
...
M^2 \geq \left(M_{irr} + \frac{Q^2}{4M_{irr}}\right)^2
\end{equation}
...
\begin{equation}
M_{\ADM} \geq M_{irr} + \frac{Q^2}{4M_{irr}} \qquad \Bigl(M_{irr}=\sqrt{\tfrac{A}{16\pi}}\Bigr)
\end{equation}
