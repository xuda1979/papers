...
\end{lemma}
\begin{proof}
We must ensure that the singular points $p_k$ do not act as sinks for the area functional in the limit $p \to 1$. Let $E$ be a set of finite perimeter in $\tM$. The perimeter measure $P(E, \cdot)$ is defined variationally.
Since the $p$-capacity of a point is zero for $1 < p < 3$ (Lemma \ref{lem:Capacity}), it follows that the 1-capacity (which relates to the perimeter) is also zero in the appropriate sense for the singular set.
Specifically, for any set $E$ with boundary $\partial^* E$, we can modify the boundary inside a small ball $B_\epsilon(p_k)$ to avoid the singularity.
In a conical metric $dr^2 + r^2 g_{S^2}$, the area of the geodesic sphere $\partial B_\epsilon$ scales as $\epsilon^2$. If a sequence of boundaries passed through $p_k$, their area contribution inside $B_\epsilon$ would be bounded below by the monotonicity of the area ratio (for minimal surfaces).
However, the crucial property is that the \textbf{Perimeter Measure does not charge points}. For any Borel set $A$, if $\mathcal{H}^{n-1}(A) < \infty$, then the measure of a single point is zero.
Since the metric $\tg$ is continuous (and in fact asymptotically conical) at $p_k$, the Hausdorff measure is well-behaved: $\mathcal{H}^2(\{p_k\}) = 0$.
Consequently, for any sequence of smooth hypersurfaces $\Sigma_j$ converging to a limit varifold $V$, the mass of $V$ concentrated at $p_k$ is zero.
\[ \|V\|(\{p_k\}) = 0. \]
This ensures that the "area" appearing in the Penrose Inequality limit is the area of the regular part of the level set, and no "ghost area" is hidden at the singularities. The limit $\mathcal{M}_p(t)$ correctly measures the geometry of the foliation.
\end{proof}
...
\textbf{Regularity of the Interface via Static Vacuum Equations:}
A priori, the metric $\bg$ is only Lipschitz continuous across the interface $\Sigma$, and the potential $f$ blows up logarithmically. However, the mass equality implies that $q=0$ and $\Rg=0$.
The condition $q=0$ implies that the vector field $X = \frac{\nabla f}{\sqrt{1+|\nabla f|^2}}$ is divergence-free in the metric $\bg$.
We construct a static spacetime metric $g_{static} = -N^2 dt^2 + \bg$ on $\mathbb{R} \times (\bM \setminus \mathcal{C})$, where the lapse function $N$ is identified with the inverse of the area radius (or explicitly constructed from the level set flow in the limit).
Since $q=0$ and $\Rg=0$, this spacetime metric satisfies the Static Vacuum Einstein Equations in the weak sense:
\begin{equation}\label{eq:StaticVacuum}
    ...
    \end{cases}
\end{equation}
The regularity theory for the Static Vacuum Einstein Equations is well-developed. Anderson \cite{anderson2001} proved that weak solutions ($g \in C^{1,\alpha}$, or even Lipschitz with bounded curvature in distributions) to the static vacuum equations are in fact real analytic in harmonic coordinates.
Specifically, the system \eqref{eq:StaticVacuum} is a coupled elliptic system for the pair $(\bg, N)$. Since the metric is Lipschitz and the distributional curvature is bounded (it is zero), we can apply elliptic bootstrapping to improve regularity.
Since the interface $\Sigma$ is a minimal surface in a static vacuum spacetime, it corresponds to the bifurcation surface of the Killing horizon where the lapse $N$ vanishes.
The analyticity of the solution implies that the metric $\bg$ extends analytically across the interface $\Sigma$. The "corner" singularity at $\Sigma$ (where the metric was Lipschitz) is therefore a coordinate artifact, not a geometric singularity. In analytic coordinates (e.g., Kruskal-Szekeres type coordinates for the spacetime extension), the metric components are analytic functions.
Consequently:
\begin{itemize}
    \item The jump in the mean curvature must vanish: $[H] = 0$.
    \item The higher-order matching conditions must hold.
    \item The interface $\Sigma$ is a smooth, analytic hypersurface in the analytic manifold $\bM$.
\end{itemize}
Similarly, any internal bubbles must be regular, but since they have $S^2 \times \R$ topology which cannot be regularized to $\R^3$ without changing topology, the set of bubbles $\mathcal{B}$ must be empty.
