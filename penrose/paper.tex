...
\begin{enumerate}
    \item \textbf{Cylindrical Reformulation:} Near $p_k$, we pass to cylindrical coordinates $(t, \theta)$ with $t = -\log r$. The $p$-Laplace equation transforms into a nonlinear evolution equation for $v(t, \theta) = u(e^{-t}, \theta) - u(p_k)$. The energy functional associated with this flow is real-analytic.
    \item \textbf{Lojasiewicz-Simon Inequality:} Standard elliptic theory guarantees that along any sequence $t_j \to \infty$, $v(t_j)$ converges to an eigenfunction on $S^2$. The Lojasiewicz-Simon inequality for the $p$-energy functional ensures that this limit is \emph{unique}. That is, the solution converges to a single eigenfunction $\psi$ and does not drift or oscillate between multiple limit profiles.
    \item \textbf{Gradient Non-Vanishing:} Since $u$ is not constant, the limit eigenfunction $\psi$ is non-trivial. For the principal eigenfunction (which corresponds to the lowest energy state likely picked up by the minimizer), $\psi$ does not vanish. Even for higher modes on $S^2$, the quantity $|\nabla (r^\lambda \psi)|^2 \approx r^{2\lambda-2} (\lambda^2 \psi^2 + |\nabla_{S^2}\psi|^2)$ is strictly positive for $r>0$, as eigenfunctions on $S^2$ cannot vanish simultaneously with their gradient (by unique continuation for the linearized equation).
    \item \textbf{Separation:} The asymptotic uniqueness implies that for sufficiently large $t$ (small $r$), $\nabla u$ is a small perturbation of the non-vanishing vector field $\nabla (r^\lambda \psi)$. Thus, $|\nabla u| \ge c r^{\lambda-1} > 0$.
\end{enumerate}
