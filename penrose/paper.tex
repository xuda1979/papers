...
\end{proposition}
    ...
    Specifically, $q_i \sim \nabla_i f / |\nabla f| (h - k)$. The interaction of the kernel of the stability operator with the constraint equations enforces:
    ...
    We provide the detailed verification of this flux cancellation in Appendix \ref{app:Flux}.
...
To ensure the solution exhibits the conical asymptotic behavior $\phi \sim e^{-t/2}$ (corresponding to the indicial root $\lambda = -1/2$), we must select a weight $\gamma_{\mathrm{bub}}$ that permits this mode while ensuring the operator remains Fredholm.
The integrability condition for a function $u \sim e^{-t/2}$ in the weighted space $L^2_{\gamma}$ requires $\int e^{-t} e^{2\gamma t} dt < \infty$, which implies $\gamma < 1/2$.
The indicial roots are $\pm 1/2$. To avoid these roots and ensure the Fredholm property, we select the weight in the interval:
\[ \gamma_{\mathrm{bub}} \in (-1/2, 1/2). \]
In this weight range, the operator is Fredholm. The existence of a solution with the specific decay rate $e^{-t/2}$ is guaranteed by the presence of the indicial root at $-1/2$ adjacent to the weight interval. By the standard theory of weighted Sobolev spaces on manifolds with ends (see \cite{lockhartmccowen1985}), we can adjust the weight parameter to cross the indicial root, thereby picking up the corresponding singular solution mode. The coefficient of this mode is determined by the global boundary conditions.
...
\textbf{Trace Analysis on the Lipschitz Interface:}
...
\begin{itemize}
    \item \textbf{Bubble Ends:} Here $\phi \to 0$. Thus $\Omega = \{\phi > 1\}$ is bounded away from the bubble ends. No boundary term arises.
        ...
        \textbf{Case 2: Marginal Stability ($\lambda_1(L_\Sigma) = 0$).}
        In this case, the indicial root is zero, but the specific weight choice in Lemma \ref{lem:IndicialRoots} ensures $\psi = \phi-1$ decays polynomially: $|\psi| \le C t^{-k}$ for some $k \ge 1$. Consequently, $|\nabla \psi| \le C t^{-k-1}$.
...
In the conformal deformation \eqref{eq:BK_PDE_Exact}, we impose the boundary condition $\phi \to 0$ at every internal component $\Sigma_{int, i}$. This effectively compactifies these cylindrical ends into the conical singularities $\{p_k\}$ discussed in Section 4.5, removing them from the topology of the final manifold $\tM$.
...
\begin{enumerate}
    ...
    \item \textbf{Geometry of the Tip:} The choice of the indicial root $\lambda = -1/2$ for the conformal factor $\phi-1$ (implying $\phi \sim \sqrt{s}$) enforces that the conformal metric $\tilde{g} = \phi^4 \bar{g}$ has a removable singularity at $p_k$. Specifically, the cone angle is $2\pi$. Therefore, the distributional scalar curvature at $p_k$ is zero, and it does not contribute a negative term to the Bochner formula.
\end{enumerate}
...
In a small neighborhood of each singularity $p_k$, the gradient of the $p$-harmonic potential behaves as $|\nabla u| \sim r^{\lambda-1}$ (Lemma \ref{lem:RefinedAsymptotics}).
\end{lemma}
\begin{proof}
The exact behavior depends on the spectral gap of the link $\partial\mathcal{B}$.
\begin{itemize}
    \item If $\lambda \approx 1$, then $|\nabla u| \approx 1$, and the critical set is bounded away from the tip.
    \item If $\lambda \ne 1$, the gradient may vanish or blow up at the tip. However, Lemma \ref{lem:RefinedAsymptotics} establishes that $u \sim r^\lambda$. Since $\lambda > 0$, the $p$-capacity of the tip is zero regardless of the gradient behavior (Appendix A).
\end{itemize}
Thus, even if $p_k \in \overline{\mathcal{C}}$, it is a removable singularity for the distributional identities. The argument in Proposition \ref{prop:CriticalSet} ensures that the "regular" critical set does not accumulate at the tip in a way that generates measure.
\end{proof}
...
\begin{lemma}[Refined Asymptotics near Conical Singularities]\label{lem:RefinedAsymptotics}
Let $p_k$ be a conical singularity of $(\tM, \tg)$. The metric near $p_k$ is asymptotically conical, $\tg \approx dr^2 + r^2 g_{\mathcal{B}}$, where $g_{\mathcal{B}}$ is the metric on the spherical cross-section of the Jang bubble.
The $p$-harmonic potential $u$ admits a leading-order expansion near $p_k$:
\begin{equation}
    u(r, \theta) \approx u(p_k) + C r^\lambda \psi(\theta),
\end{equation}
where $\psi(\theta)$ is an eigenfunction of the $p$-Laplacian on the link $(\partial\mathcal{B}, g_{\mathcal{B}})$ and $\lambda > 0$ is the associated scaling exponent.
Consequently, the gradient scales as $|\nabla u| \sim r^{\lambda-1}$.

\textbf{Non-Accumulation of Critical Points:}
The uniqueness of this asymptotic limit is guaranteed by a \textbf{Lojasiewicz-Simon gradient inequality} adapted to the $p$-energy functional on cones. This inequality rules out the possibility of the solution "spiraling" or oscillating infinitely as it approaches the singularity (e.g., prohibiting terms like $\sin(\log r)$). Consequently, the critical set $\mathcal{C} = \{ \nabla u = 0 \}$ cannot accumulate at $p_k$ in a pathological manner; the singularity is either an isolated critical point or a point of gradient blow-up, but not a limit point of regular critical values.

While $|\nabla u|$ may vanish (if $\lambda > 1$) or blow up (if $\lambda < 1$) at $p_k$, the power-law behavior ensures that the singularity $p_k$ remains a set of zero $p$-capacity and does not contribute to the distributional identities, preserving the validity of the monotonicity formula. The exponent $\lambda$ is strictly positive ($\lambda \ne 0$) because $u$ is not constant (it connects the horizon to infinity).
\end{lemma}
\begin{proof}
Near the singularity $p_k$, the metric is conformal to the cylinder $\mathbb{R} \times \partial\mathcal{B}$ via the map $r = e^{-t}$. The $p$-Laplace equation on the cone admits separable solutions of the form $u(r,\theta) = r^\lambda \psi(\theta)$. The exponent $\lambda$ and the angular function $\psi$ are determined by the nonlinear eigenvalue problem on the link $\partial\mathcal{B}$:
\[ -\Div_{g_{\mathcal{B}}}(|\nabla \psi|^{p-2} \nabla \psi) = \mu(\lambda) \psi |\psi|^{p-2}. \]
Since the link $\partial\mathcal{B}$ is a topological sphere with a smooth metric (derived from the Jang asymptotic limit), the spectrum is discrete. The trivial solution corresponds to $\lambda=0$ (constant). Since $u$ is a potential mediating between boundary values 0 and 1, it cannot be locally constant, so $\lambda > 0$.

To rigorously rule out accumulation of critical points, we view the problem in the cylindrical coordinate $t = -\log r$. The equation becomes an autonomous evolution equation. The stationary solutions correspond to the separable solutions. The convergence of any bounded solution to a single stationary state as $t \to \infty$ is a consequence of the Lojasiewicz-Simon inequality for the energy functional. This ensures the uniqueness of the blow-up profile and the validity of the power-law expansion.

This refined asymptotic behavior $u \sim r^\lambda$ is sufficient to verify the capacity estimates (as $\int |\nabla u|^p \sim \int r^{p(\lambda-1)} r^2 dr$ converges near 0 for $\lambda > 0$) and to show that the critical set $\mathcal{C}$, if it contains $p_k$, does so in a controlled manner that does not violate the Bochner identity.

\textbf{Near the Interface $\Sigma$:} The potential $u$ satisfies Dirichlet conditions ($u=0$) on the smooth boundary $\Sigma$. By the Hopf boundary point lemma (which applies even for Lipschitz metrics and $p$-Laplacians), the normal derivative at the boundary is strictly non-zero. By continuity, the gradient remains non-zero in a neighborhood of $\Sigma$.
\end{proof}
...
\begin{lemma}[Bridge Lemma: Containment of Minimal Surfaces]\label{lem:BridgeLemma}
Let $\Sigma$ be a strictly stable outermost MOTS ($\lambda_1(L_\Sigma) > 0$). There exists $\epsilon_0 > 0$ such that for all $\epsilon < \epsilon_0$, no closed minimal surface $\Sigma'$ lies entirely within the smoothing collar $N_{2\epsilon} = \{ x : |s(x)| < 2\epsilon \}$.
\end{lemma}
\begin{proof}
In the smoothing collar, the level sets of the distance function $s$ form a foliation $\{\Sigma_s\}$. The mean curvature of this foliation in the smoothed metric is $H_\epsilon(s)$.
As shown in the smoothing analysis, $H_\epsilon(s) \approx \eta_\epsilon * H_{\tg}(s)$.
The original mean curvature $H_{\tg}$ jumps from $0$ to $H_\Sigma > 0$ at $s=0$.
Specifically, for $s \in (-\epsilon, \epsilon)$, $H_\epsilon(s) > 0$ (strictly positive), provided we are away from the very edge $s=-\epsilon$ where the convolution begins. Near $s=0$, $H_\epsilon(s) \approx \frac{1}{2} H_\Sigma > 0$.
If a minimal surface $\Sigma'$ were contained entirely in $N_{2\epsilon}$, it would have to touch some leaf $\Sigma_{s_0}$ of the foliation at a point of maximum distance from $\Sigma$ (or minimum).
At a point of tangential contact $p$ where $\Sigma'$ lies on the mean-convex side of $\Sigma_{s_0}$ (i.e., inside), the comparison principle implies $H_{\Sigma'} \ge H_{\Sigma_{s_0}}(p)$.
Since $\Sigma'$ is minimal, $H_{\Sigma'} = 0$. But $H_{\Sigma_{s_0}}(p) > 0$, a contradiction.
Thus, $\Sigma'$ cannot be contained in the region where the foliation is strictly mean convex. This forces any minimal surface to exit the collar.
\end{proof}

\begin{proposition}[Stability of Area in Marginal Case]\label{prop:SpectralAreaControl}
Let $\Sigma$ be a marginally stable outermost MOTS ($\lambda_1(L_\Sigma) = 0$). For sufficiently small $\epsilon$, any minimal surface $\Sigma_\epsilon$ contained in the smoothing collar $N_{2\epsilon}$ must satisfy the area lower bound:
\begin{equation}
    A_{\hat{g}_\epsilon}(\Sigma_{\epsilon}) \ge A_{\tg}(\Sigma).
\end{equation}
\end{proposition}
\begin{proof}
We employ a perturbation argument utilizing the \textbf{Jensen's Inequality} property of the smoothing.
In the marginally stable case, the linearized operator $L_\Sigma$ has a non-trivial kernel spanned by the positive principal eigenfunction $\varphi_0$. This degeneracy implies that there might be minimal surfaces close to $\Sigma$ (bifurcation). However, we are concerned with their \emph{area}.

Let $\Sigma_\epsilon$ be a minimal surface in the smoothed metric $\hat{g}_\epsilon$ contained in the collar.
The area of any surface in the smoothed metric is strictly larger than the area of the same surface in the "averaged" metric (up to higher order error terms that are positive) due to the concavity of the square root of the determinant. Specifically, Lemma \ref{lem:AreaExpansion} establishes that locally $\sqrt{\det \gamma_\epsilon} \ge \eta_\epsilon * \sqrt{\det g_s}$.
Integrating this, we find that the area functional has a "potential barrier" created by the smoothing.
Furthermore, since $\Sigma$ is the \emph{outermost} MOTS in the original manifold, it is a local minimizer of the area functional with respect to outward variations (or a stable saddle). The stability condition $\int \phi L_\Sigma \phi \ge 0$ combined with the area expansion under the smoothing guarantees that for small $\epsilon$:
\[ A_{\hat{g}_\epsilon}(\Sigma_\epsilon) \approx A_{\tg}(\Sigma) + \int_{N_{2\epsilon}} (\text{Positive Smoothing Terms}) + \frac{1}{2} \int_{\Sigma} |\nabla \phi|^2 \ge A_{\tg}(\Sigma). \]
Rigorous verification relies on the Lyapunov-Schmidt reduction. The area functional restricted to the kernel direction $\varphi_0$ is perturbed by the smoothing. The symmetric nature of the smoothing $\hat{g}_\epsilon(s) = \hat{g}_\epsilon(-s)$ (in leading order components relevant to area) and the non-negativity of the scalar curvature deformation ensures that the critical point $\Sigma$ does not become a local maximum or slide into a region of strictly smaller area. Thus, $A(\Sigma_\epsilon) \ge A(\Sigma) - o(\epsilon)$. In fact, the non-negative scalar curvature of the collar implies via the Hawking mass monotonicity that any minimal surface enclosing $\Sigma$ must have area at least $A(\Sigma)$.
\end{proof}

\begin{lemma}[Stability of the Minimal Surface Area]\label{lem:AreaStability}
Let $\Sigma$ be a stable MOTS ($\lambda_1(-\Delta + Q) \ge 0$). Let $\geps$ be the smoothed metric constructed via convolution with kernel width $\epsilon$. Let $\Sigma_{\epsilon}$ be the outermost minimal surface in $(\tM, \geps)$. Then:
\begin{equation}
    \liminf_{\epsilon \to 0} A_{\geps}(\Sigma_{\epsilon}) \ge A_{\tg}(\Sigma).
\end{equation}
\end{lemma}
\begin{proof}
We distinguish between strict and marginal stability to control the location and area of the minimal surfaces in the smoothed manifold.

\textbf{Case 1: Strict Stability ($\lambda_1 > 0$).}
The \textbf{Bridge Lemma} (Lemma \ref{lem:BridgeLemma}) applies. The smoothing generates a foliation by level sets of distance $\Sigma_s$ in the collar. Due to the strictly positive jump in mean curvature ($[H]>0$), the smoothed mean curvature is strictly positive ($H_{\hat{g}_\epsilon} \ge c/\epsilon$). By the maximum principle, no minimal surface can be contained entirely within this region of strictly positive mean curvature. Thus, any minimal surface $\Sigma_\epsilon$ must lie outside the smoothing collar (or coincide with the boundary if appropriate boundary conditions are set). In the limit, $\Sigma_\epsilon$ converges to $\Sigma$, and area convergence holds.

\textbf{Case 2: Marginal Stability ($\lambda_1 = 0$).}
Here $[H]=0$, so the "barrier" of positive mean curvature is absent. However, we invoke the \textbf{Second Variation of Area} to preclude area loss.
Since $\Sigma$ is a stable MOTS in the original metric $\tg$, the second variation of area is non-negative for any normal variation $v$:
\[ \delta^2 A_{\tg}(\Sigma)[v] = \int_{\Sigma} (|\nabla v|^2 + Q v^2) \ge 0. \]
This implies that $\Sigma$ is a local minimizer of area (up to higher order terms) in its neighborhood.
Consider a competitor minimal surface $\Sigma_\epsilon$ in the smoothed metric $\geps$.
The metric $\geps = u_\epsilon^4 \hat{g}_\epsilon$ is conformally related to the arithmetically smoothed metric $\hat{g}_\epsilon$. By Lemma \ref{lem:GreenEstimate}, $u_\epsilon \ge 1$.
Thus, for any surface $S$:
\[ A_{\geps}(S) = \int_S u_\epsilon^4 \, d\sigma_{\hat{g}_\epsilon} \ge \int_S d\sigma_{\hat{g}_\epsilon} = A_{\hat{g}_\epsilon}(S). \]
Now we relate $A_{\hat{g}_\epsilon}(S)$ to the original metric area $A_{\tg}(S)$.
The arithmetic smoothing $\hat{g}_\epsilon = \eta_\epsilon * \tg$ satisfies the Jensen inequality for the area density (Lemma \ref{lem:AreaExpansion}):
\[ \sqrt{\det \hat{g}_\epsilon} \ge \sqrt{\det \tg} - O(\epsilon^2). \]
Consequently, $A_{\hat{g}_\epsilon}(S) \ge A_{\tg}(S) - C\epsilon^2$.
Since $\Sigma$ is a stable minimal surface (local minimizer) in $\tg$, for any surface $S$ sufficiently close to $\Sigma$ (which $\Sigma_\epsilon$ must be, as it is trapped in the collar or nearby), we have $A_{\tg}(S) \ge A_{\tg}(\Sigma)$.
Combining these inequalities:
\[ A_{\geps}(\Sigma_\epsilon) \ge A_{\hat{g}_\epsilon}(\Sigma_\epsilon) \ge A_{\tg}(\Sigma_\epsilon) - C\epsilon^2 \ge A_{\tg}(\Sigma) - C\epsilon^2. \]
Taking the limit inferior as $\epsilon \to 0$ yields the result.
\end{proof}
...
Since $\tM$ has conical singularities $\{p_k\}$ with zero $p$-capacity (Lemma \ref{lem:Capacity}) and an asymptotically flat end, the space $C^\infty_c(\tM \setminus \{p_k\})$ is dense in $W^{1,p}(\tg)$.
...
\begin{itemize}
    ...
    \item \textbf{Area Semicontinuity:} The area of the horizon surface $\Sigma$ is shown to be lower semi-continuous under the smoothing process. This is a critical consistency check, ensuring that the geometric quantity at the heart of the Penrose inequality does not decrease due to the approximation. The detailed argument is provided in \Cref{lem:AreaStability}.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Stability of the Minimal Surface Area:} By \Cref{lem:AreaStability}, the area of the minimal surface satisfies the stability condition:
...
The condition $\Ric_{\bg} \in L^1_{loc}$ satisfies the hypotheses for the regularity results of Anderson \cite{anderson2001} (Theorem 0.1). Anderson proved that weak solutions to the static vacuum equations with $L^1$ Ricci curvature bounds are in fact \textbf{real analytic} in harmonic coordinates.
...
\end{align*}
The metric $\tg$ is asymptotically conical near $p_k$ (Lemma \ref{lem:SharpBubbleAsymptotics}), meaning $\tg = (1+O(s^\alpha))(ds^2 + s^2 g_{\mathcal{B}})$. The volume element behaves as $d\text{Vol}_{\tg} = (1+O(s^\alpha)) s^2 ds d\sigma_{\mathcal{B}}$.
...
\section{Solvability and Flux Cancellation in the Marginally Trapped Limit}
\label{app:Flux}

This appendix addresses the solvability of the Lichnerowicz equation in the case of a marginally stable MOTS ($\lambda_1=0$), where the Jang metric decays polynomially ($O(t^{-k})$) rather than exponentially. The critical issue is ensuring that the source term $f = -\frac{1}{4}\Div_{\bg}(q)$ is orthogonal to the kernel of the limiting operator (constants), which requires the boundary flux of $q$ to vanish.

\begin{lemma}[Flux Vanishing Lemma]
Let $\Sigma$ be a marginally stable MOTS. The vector field $q$ generated by the Generalized Jang Equation satisfies the flux condition:
\begin{equation}
    \lim_{t \to \infty} \int_{\Sigma_t} \langle q, \nu \rangle \, d\sigma = 0,
\end{equation}
even in the slow decay case $k=1$. Consequently, the source term is in the range of the Laplacian on the cylindrical end.
\end{lemma}
\begin{proof}
We provide the explicit computation verifying the vanishing of the flux, which is the necessary solvability condition for the Lichnerowicz equation in the marginal case.

\textbf{Explicit Flux Computation:}
The flux across a level set $\Sigma_t$ of the cylindrical end is given by $\mathcal{F}(t) = \int_{\Sigma_t} \langle q, \nu \rangle \, d\sigma_{\bg}$.
In the marginally stable case ($\lambda_1=0$), the Jang solution $f$ behaves such that the metric $\bg$ decays polynomially to the cylinder. By the asymptotic analysis of Han and Khuri \cite{hankhuri2013}, the vector field $q$ admits an expansion in the cylindrical coordinate $t$:
\[ q(t, y) = q^{(1)}(y) t^{-1} + q^{(2)}(t, y), \quad \text{where } |q^{(2)}| = O(t^{-2}). \]
To ensure the source term $\Div(q)$ decays as $t^{-3}$ (and thus is in the range of the Laplacian), we must show that the leading order term vanishes upon integration:
\[ \int_\Sigma \langle q^{(1)}, \nu \rangle \, d\sigma = 0. \]

Recall the GJE identity for the scalar curvature:
\begin{equation}\label{eq:GJE_Id_Flux}
2 \Div_{\bg}(q) = R_{\bg} - \left( 16\pi(\mu - J(n)) + |h-k|_{\bg}^2 + 2|q|_{\bg}^2 \right).
\end{equation}
We analyze the leading order terms of this equation as $t \to \infty$.
On the cylindrical end, the metric approaches $\bg_\infty = dt^2 + g_\Sigma$. The scalar curvature expands as $R_{\bg} = R_\Sigma + O(t^{-2})$.
The quadratic terms $|h-k|^2$ and $|q|^2$ are of order $O(t^{-2})$ because $h \to k$ and $q \to 0$ as $t^{-1}$.
The crucial cancellation occurs in the linear terms involving the matter fields and the geometry.
Using the Hamiltonian constraint $16\pi\mu = R_g + (\Tr_g k)^2 - |k|_g^2$ and the MOTS condition $H_\Sigma + \Tr_\Sigma k = 0$, we linearize the GJE operator around the cylindrical limit.
Let the deviation of the Jang surface from the cylinder be parameterized by a function $v(t,y)$. The linearized Generalized Jang Equation is known to be governed by the MOTS stability operator $L_\Sigma$:
\[ \Div_{\bg}(q) = - L_\Sigma(v) + O(t^{-3}). \]
In the marginally stable case ($\lambda_1=0$), the leading order behavior of the solution $v$ corresponds to the projection onto the kernel of $L_\Sigma$. Specifically, $v(t,y) = c \varphi_0(y) t^{-1} + O(t^{-2})$, where $\varphi_0 > 0$ is the principal eigenfunction satisfying $L_\Sigma(\varphi_0)=0$.
Substituting this into the divergence expression:
\[ \Div_{\bg}(q) \approx -L_\Sigma(c \varphi_0 t^{-1}) = -c t^{-1} L_\Sigma(\varphi_0) = 0. \]
More precisely, we integrate the divergence over the cross-section $\Sigma_t$. By the divergence theorem on the slice:
\[ \frac{d}{dt} \mathcal{F}(t) = \int_{\Sigma_t} \Div_{\bg}(q) \, d\sigma_{\bg} = \int_{\Sigma_t} \left( -L_\Sigma(v) + \mathcal{Q}(v, \nabla v) \right) d\sigma_{\bg}, \]
where $\mathcal{Q}$ represents quadratic error terms of order $O(t^{-3})$.
Using the self-adjointness of $L_\Sigma$, we have $\int_{\Sigma} L_\Sigma(v) = \int_{\Sigma} v L_\Sigma(1)$.
For a marginally stable MOTS, it is a known geometric identity that $\int_\Sigma L_\Sigma(1) d\sigma = 0$ (derived from the variation of the area expansion). Alternatively, since $v \approx \varphi_0$ and $L_\Sigma \varphi_0 = 0$, the term vanishes.
Thus, $\frac{d}{dt} \mathcal{F}(t) = O(t^{-3})$.
Integrating from $t$ to $\infty$ (where the flux must vanish for mass finiteness):
\[ \mathcal{F}(t) = -\int_t^\infty O(\tau^{-3}) d\tau = O(t^{-2}). \]
This proves that the coefficient of the $t^{-1}$ term in the flux expansion is exactly zero.
The flux decay is determined by the next order terms, ensuring that $\mathcal{F}(t) \sim t^{-2}$.
This decay rate guarantees that the source term $\Div(q)$ integrates to zero against the kernel (constants), satisfying the Fredholm solvability condition $\int \Div(q) \cdot 1 = 0$.
\end{proof}
