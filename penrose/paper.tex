...
In the marginal case ($\lambda_1=0$), the kernel of $L_\Sigma$ is spanned by a strictly positive eigenfunction $\varphi_0$. The decay analysis of the GJE (see \cite{hankhuri2013}) shows that the leading order perturbation $v$ of the metric corresponds to the projection of the error onto this kernel. Specifically, $v = c \varphi_0$.
Consequently, the source term $\Div_{\bg}(q)$ in the linearized approximation is given by:
\[ \Div(q) \approx -L_\Sigma(v) = -c L_\Sigma(\varphi_0) = 0. \]
Thus, the flux $\int \Div(q)$ vanishes to leading order. The actual flux decay is determined by the next order term in the spectral expansion (corresponding to $\mu_1 > 0$ or non-linear interactions), which decays faster ($O(t^{-2})$ or better), ensuring integrability.
