...
\end{lemma}
\begin{proof}
If $\lambda_1 > 0$, the \textbf{Bridge Lemma} (Lemma \ref{lem:BridgeLemma}) applies. The foliation by level sets of distance in the smoothing collar has strictly positive mean curvature $H > c > 0$ (due to the jump $[H]$). By the maximum principle, no minimal surface can enter this region without exiting it, forcing $\Sigma_\epsilon$ to essentially coincide with $\Sigma$.

The critical case is $\lambda_1 = 0$. Here, $[H]=0$. The smoothing is a $C^2$-small perturbation of the metric in the tangential directions. We invoke the Second Variation of Area. Since $\Sigma$ is stable, the area functional $A(S)$ has a local minimum at $\Sigma$. The smoothing perturbs the functional to $A_\epsilon(S)$.
We utilize the pointwise conformal inequality established in Lemma \ref{lem:GreenEstimate}: the conformal factor $u_\epsilon$ satisfies $u_\epsilon \ge 1$.
Thus, $A_{\geps}(S) = \int_S u_\epsilon^4 d\sigma_{\hat{g}_\epsilon} \ge \int_S d\sigma_{\hat{g}_\epsilon}$.
The arithmetic smoothing $\hat{g}_\epsilon$ satisfies $\sqrt{\det \hat{g}_\epsilon} \ge \sqrt{\det \tg} - O(\epsilon^2)$ (Lemma 4.1).
Therefore, for any competitor surface $S$ in the collar:
\[ A_{\geps}(S) \ge A_{\tg}(S) - C\epsilon^2 \ge A_{\tg}(\Sigma) - C\epsilon^2. \]
Taking the limit proves the lemma.
\end{proof}
...
With $q=0$ and $[H]=0$, the pair $(\bg, N)$ (where $N$ is the static lapse) satisfies the \textbf{Static Vacuum Einstein Equations} in the weak sense:
\begin{equation}\label{eq:StaticVacuum}
    ...
    \end{cases}
\end{equation}
Since $[H]=0$, the Ricci tensor $\Ric_{\bg}$ is in $L^1_{loc}$ (it has no delta function component). This satisfies the conditions for the regularity results of Anderson \cite{anderson2001}. Anderson proved that weak solutions to the static vacuum equations with $L^1$ Ricci curvature bounds are in fact \textbf{real analytic} in harmonic coordinates.
This implies that the interface $\Sigma$ \textbf{ceases to be a singularity}. The Lipschitz corner is merely a coordinate artifact of the Jang gauge. In the appropriate harmonic (or Kruskal-Szekeres) coordinates, the metric extends analytically across $\Sigma$.
Consequently:
\begin{itemize}
...
\textbf{Rigorous Justification (Caccioppoli Estimates):}
We apply the Caccioppoli inequality to the derivatives of $u_\epsilon$. This involves testing the differentiated equation against $\eta^2 \nabla u_\epsilon$, where $\eta$ is a cutoff function.
Crucially, for $p \in (1, 2]$, the term $(|\nabla u_\epsilon|^2 + \epsilon^2)^{(p-2)/2}$ is bounded \emph{from below} by $( \sup_K |\nabla u_\epsilon|^2 + 1 )^{(p-2)/2}$ (since the exponent is negative, the maximum of the base gives the minimum of the power).
Since $|\nabla u_\epsilon|$ is uniformly bounded in $L^\infty(K)$ (by standard $C^{1,\alpha}$ estimates for $p$-Laplacian that are independent of regularization), the ellipticity constant $\lambda_\epsilon$ is uniformly bounded away from zero: $\lambda_\epsilon \ge c > 0$.
Testing the linearized equation with $\eta^2 v_k$ and summing over $k$ yields:
\[ \int \eta^2 |\nabla^2 u_\epsilon|^2 \le C \int |\nabla \eta|^2 |\nabla u_\epsilon|^2. \]
...
In the marginally stable case ($\lambda_1=0$), the Jang solution $f$ behaves such that the metric $\bg$ decays polynomially to the cylinder. The vector field $q$ admits an expansion $q = q^{(1)} t^{-1} + O(t^{-2})$. We must show that the leading order flux vanishes:
...
\end{equation}
We integrate this over the slice $\Sigma_t$. The divergence term gives the derivative of the flux $\mathcal{F}'(t)$.
To leading order in the cylindrical limit (where $\bg \approx dt^2 + g_\Sigma$), the scalar curvature $R_{\bg} \approx R_\Sigma$. The term $|h-k|^2$ and $|q|^2$ are quadratic in the perturbation and thus higher order.
The dominant term comes from the interplay of the constraint equations and the stability operator.
We utilize the Hamiltonian constraint equation on the initial data $(M,g)$:
\[ 16\pi \mu = R_g + (\Tr k)^2 - |k|^2. \]
On the MOTS $\Sigma$, we have $H + \Tr k = 0$ (where $H$ is the mean curvature in $M$).
Substituting the constraint into the GJE identity and linearizing around the cylinder solution (where $f \to \infty$), we find that the source term for the flux evolution is controlled by the stability operator $L_\Sigma$.
Specifically, the linearized GJE relates the divergence of $q$ to the variation of the expansion $\theta_+$:
\[ \Div_{\bg}(q) \approx - L_\Sigma(v), \]
where $v$ is the perturbation of the conformal factor (or geometry) associated with the decay.
In the marginal case ($\lambda_1=0$), the operator $L_\Sigma$ has a non-trivial kernel spanned by a positive eigenfunction $\varphi_0$.
The condition that the geometry decays to the cylinder implies that the source term must be orthogonal to this kernel.
Crucially, integrating the constraint equation $R_g + (\Tr k)^2 - |k|^2 = 16\pi \mu$ over $\Sigma$, and using the Gauss-Codazzi relations and the MOTS condition, we derive the identity:
\[ \int_\Sigma L_\Sigma(1) \, d\sigma = \int_\Sigma \left( \frac{1}{2}R_\Sigma - (\mu + J(n)) - \frac{1}{2}|A|^2 + \frac{1}{2}(\Tr k)^2 \right) d\sigma = 0. \]
For the specific decay mode $v$ associated with the marginal stability (which is proportional to the kernel $\varphi_0 \approx 1$), the integral of the divergence vanishes:
\[ \int_\Sigma \Div(q^{(1)}) \, d\sigma = - \int_\Sigma L_\Sigma(\varphi_0) \, d\sigma = 0. \]
This implies that the flux $\mathcal{F}(t)$ is constant to leading order. Since the flux must vanish at infinity (for the mass to be finite and $q \in L^2$), the constant must be zero.
Therefore, the leading order term of the vector field integrates to zero, and the actual decay of the flux is driven by the higher order terms ($O(t^{-2})$), ensuring that the source term $\Div(q)$ is integrable and orthogonal to the kernel, satisfying the Fredholm solvability condition.
\end{proof}
