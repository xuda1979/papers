...
Recall that the equality case of the Riemannian Penrose Inequality states that the manifold must be isometric to a spatial slice of the Schwarzschild spacetime. Specifically, $(\bM, \bg)$ is isometric to $(M_{Schw}, g_{Schw})$. The Jang manifold $\bM$ has a cylindrical end $\mathcal{T}_\Sigma \cong [0, \infty) \times S^2$, which corresponds to the \textbf{optical metric} representation of the Schwarzschild horizon geometry. In the equality case, the blow-up function $f$ acts as the coordinate transformation mapping the horizon $r=2M$ to the infinite cylinder.

\textbf{Step 4a: Vanishing of the Shift and Staticity ($q=0$).}
The equality analysis in Step 2 yielded the condition $q=0$. We now rigorously interpret this in terms of the spacetime structure.
We construct a spacetime metric $^{(4)}g$ on the product manifold $\mathbb{R} \times \bM$:
\[ ^{(4)}g = -N^2 dt^2 + \bg. \]
Here $N$ is the lapse function, identified with the inverse of the area radius (or explicitly constructed from the level set flow in the limit).
If $q$ were non-zero, the spacetime embedding would generally require a non-vanishing shift vector $\beta$, leading to a stationary but non-static metric $^{(4)}g = -N^2 dt^2 + \bg_{ij}(dx^i + \beta^i dt)(dx^j + \beta^j dt)$.
However, the condition $q=0$ derived from the mass equality ensures that the mixed components of the curvature tensor (related to the shift) vanish. Specifically, $q=0$ implies that the vector field $X = \nabla f / \sqrt{1+|\nabla f|^2}$ is divergence-free in $\bg$, and the cross-terms in the generalized Einstein equations vanish.
Thus, $q=0$ rigorously implies that the shift vector vanishes ($\beta \equiv 0$) and the manifold embeds into a \textbf{Static} Schwarzschild spacetime.

\textbf{Step 4b: Analyticity and Removal of the Singularity at $\Sigma$.}
A priori, the metric $\bg$ is only Lipschitz continuous across the interface $\Sigma$, and the potential $f$ blows up logarithmically. The "corner" at $\Sigma$ represents a potential singularity.
However, since $q=0$ and $\Rg=0$, the pair $(\bg, N)$ satisfies the \textbf{Static Vacuum Einstein Equations} in the weak sense:
\begin{equation}\label{eq:StaticVacuum}
    ...
    \end{cases}
\end{equation}
This is a coupled system of elliptic partial differential equations. We invoke the regularity results of Anderson \cite{anderson2001}. Anderson proved that weak solutions to the static vacuum equations (specifically, metrics in $C^{1,\alpha}$ or Lipschitz metrics with distributional curvature bounds) are in fact \textbf{real analytic} in harmonic coordinates.
Although our metric $\bg$ is constructed as a Lipschitz gluing of a bulk metric and a cylindrical metric, it solves the analytic system \eqref{eq:StaticVacuum} distributionally. By elliptic bootstrapping (Morrey-type estimates), the solution is smooth and analytic.
...
Consequently:
\begin{itemize}
    \item The jump in the mean curvature vanishes: $[H] = 0$.
