...
To treat the three distinct analytic challenges (Asymptotic Analysis, Geometric Smoothing, and Singular Measure Theory) independently, we fix the following interface definitions:

\begin{enumerate}
    \item \textbf{The Weight Parameter ($\delta$):}
    For the analysis of the marginally stable case ($\lambda_1=0$) in the cylindrical end, we utilize the weighted Sobolev space $H^2_\delta$ defined with respect to the measure $dx$ on $\mathbb{R}^3 \setminus B_1$ (via conformal identification). The weight function is $w(x) = (1+|x|^2)^{\delta/2}$. This fixes the functional analytic framework for the Fredholm inversion (Track A).
    ...
    The smoothing estimates (Track B) yield bounds dependent on $\epsilon$.
    ...
    This parameter $\alpha$ drives the capacity and flux arguments (Track C).
\end{enumerate}
...
\noindent
\begin{proof}
The proof relies on establishing foundational $L^1$ and $L^2$ bounds for the negative part of the scalar curvature, $R^-_\epsilon$, and then using Hölder's inequality to interpolate the desired $L^{3/2}$ norm. The rigorous, self-contained derivation of these bounds is the subject of \Cref{app:Smoothing}.
...
\begin{enumerate}
    ...
    \begin{lemma}[Spectral Gap of Bubble Links]
    The decay rate $\alpha > 0$ at the bubble tips is guaranteed by the topology of the MOTS. By the results of Galloway and Schoen, any stable marginal surface in a 3-manifold obeying the DEC is topologically a sphere $S^2$. The first eigenvalue of the Laplacian on a metric close to the round sphere is positive, $\lambda_1 > 0$. This spectral gap ensures the existence of the decaying solution $\phi \sim r^\alpha$ required to close the flux integrals.
    \end{lemma}
...
\end{lemma}
\begin{proof}
This is an immediate corollary of Theorem~\ref{thm:ScalarCurvatureEstimate}, whose proof in \Cref{app:Smoothing} establishes the $L^1$ and $L^2$ bounds on $R^-_\epsilon$ and interpolates them to the $L^{3/2}$ estimate. We record the statement here for convenient reference when applying the conformal correction.
\end{proof}
...
The pair $(\bg, N)$ forms a weak ($W^{2,p}_{loc}$) solution to the Static Vacuum Einstein Equations:
\[ \Ric_{\bg} - N^{-1}\nabla^2_{\bg} N = 0, \quad \Delta_{\bg} N = 0. \]
...
\begin{equation}
    \lim_{r \to 0} \int_{S_r} \phi \partial_r \phi \, d\sigma_r = 0.
\end{equation}
\end{lemma}
\begin{proof}
...
This appendix provides the rigorous functional analytic foundation for the solution of the singular Lichnerowicz equation, specifically addressing the marginally stable case ($\lambda_1=0$) where the metric decays polynomially. This is the "Asymptotic Engine" (Track A).

\subsection{Weighted Scattering Spaces}
\label{subsec:WeightedSpaces}

We define the functional framework for the cylindrical end $\mathcal{C} \cong [0, \infty)_t \times \Sigma$.
\begin{definition}[Weighted Scattering Space $H^k_\delta$]
We identify the cylindrical end conformally with the exterior domain $\mathbb{R}^3 \setminus B_1$ via the change of variables $r = e^t$. The measure transforms as $dV_{cyl} = r^{-3} dx$.
We define the weighted Sobolev space $H^k_\delta(\mathbb{R}^3)$ as the closure of $C^\infty_c$ under the norm:
\begin{equation}
    \|u\|_{H^k_\delta} = \sum_{|\alpha| \le k} \| \langle x \rangle^{\delta + |\alpha|} \partial^\alpha u \|_{L^2(dx)},
\end{equation}
where $\langle x \rangle = (1+|x|^2)^{1/2}$.
The weight parameter $\delta$ controls the asymptotic behavior. For $\delta \in \mathbb{R}$, functions in $H^k_\delta$ decay as $r^{-\delta-3/2}$ (in $L^2$ sense) relative to the Euclidean measure.
\end{definition}

\subsection{Indicial Roots and Weight Choice}
\label{subsec:IndicialRoots}

We analyze the model operator $L_0 = \partial_t^2 + \Delta_\Sigma - V_\infty$ on the cylinder.
In the marginally stable case, the potential $V \sim O(t^{-2})$ decays, so $V_\infty = 0$. The operator is simply the Laplacian on the cylinder.
Mapping to $\mathbb{R}^3$, the operator becomes the Euclidean Laplacian $\Delta_{\mathbb{R}^3}$.
\begin{lemma}[Indicial Roots]
The indicial roots $\gamma$ are determined by the spectrum of the Laplacian on the cross-section $\Sigma$.
Since $\lambda_1(\Delta_\Sigma) = 0$ (corresponding to constant functions), the critical indicial roots are determined by the equation:
\[ \gamma(\gamma+1) = 0 \implies \gamma = 0, -1. \]
...
In this appendix, we provide the rigorous functional analytic framework required to establish the Fredholm properties of the Lichnerowicz operator in the marginally stable case. Unlike the strictly stable case where the metric decays exponentially to a cylinder, allowing for the use of standard edge Sobolev spaces, the marginally stable case exhibits polynomial decay $O(t^{-2})$ in the cylindrical coordinate $t$. This requires the use of the \textbf{Weighted Scattering Calculus}.
...
The multiplication operator $E(t) : H^2_\delta \to L^2_{\delta+2}$ is compact.
\end{theorem}
\begin{proof}
The error term satisfies $|E(x)| \le C \langle x \rangle^{-2}$.
Let $u_n$ be a bounded sequence in $H^2_\delta$. By the Rellich-Kondrachov theorem, $u_n$ converges strongly in $L^2_{loc}$.
We must control the tails.
\[ \int_{|x|>R} |E(x) u_n(x)|^2 \langle x \rangle^{2(\delta+2)} dx \le C \int_{|x|>R} \langle x \rangle^{-4} |u_n|^2 \langle x \rangle^{2\delta+4} dx = C \int_{|x|>R} |u_n|^2 \langle x \rangle^{2\delta} dx. \]
Since $u_n$ is bounded in the weighted space, the tail integral is uniformly small for large $R$. Combined with local convergence, this proves strong convergence of $E u_n$.
Thus, $L$ is a Fredholm operator. By choosing $\delta \in (-1/2, 1/2)$ (in the spectral gap), we ensure trivial kernel and cokernel, proving invertibility.
\end{proof}
...
Taking the $2/3$ power yields $\epsilon^{2/3}$.
\end{proof}
