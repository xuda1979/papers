...
\begin{itemize}
    ...
    We prove in \textbf{Appendix E} (Flux Vanishing Lemma) that this orthogonality holds even in the slow decay case ($k=1$). The divergence structure implies that the projection onto the kernel equals the boundary flux, which is shown to vanish using the constraint equations and mass conservation.
...
\begin{itemize}
    ...
    \item \textbf{Stability of the Minimal Surface Area:} By Lemma \ref{lem:AreaStability}, the area of the minimal surface satisfies the stability condition:
...
\end{itemize}
Thus $I = \Div Y$.
However, the physical scalar curvature density also contains the term $|h-k|^2$ which we excluded from the PDE. If we include this term in the definition of the regular potential $\Rg^{reg}$, then $\Div Y$ would contain an extra term $-\frac{1}{8}(\phi-1)^2 |h-k|^2$, and we would have $I = \Div Y + P$ with $P = \frac{1}{8}(\phi-1)^2 |h-k|^2$.
By excluding it from the PDE (which makes $\Rtg \ge 0$), we absorb it into the non-negative scalar curvature of the deformed metric. The identity $I = \Div Y$ derived above is the precise conservation law needed for the overshoot argument.
