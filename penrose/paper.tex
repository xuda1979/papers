...
\[ \Ric_{\bg} - N^{-1}\nabla^2_{\bg} N = 0, \quad \Delta_{\bg} N = 0. \]
We invoke elliptic regularity for the Einstein equations. In harmonic coordinates, the Ricci operator becomes a quasilinear elliptic system for the metric components:
\[ \Delta g_{ij} + Q(g, \partial g) = 0. \]
Since $g \in C^{1,1}$, the coefficients are Lipschitz, and standard bootstrapping arguments (starting from $W^{2,p}$ regularity) imply that the metric is smooth, and by Morrey's theorem for analytic non-linear systems, real-analytic.
\begin{theorem}[Regularity of Weak Static Vacuum \cite{anderson2001}]
Any $C^{1,1}$ weak solution to the static vacuum equations with bounded lapse is real analytic.
\end{theorem}
This upgrades the regularity of $(\bM, \bg)$ to real analytic globally, removing the "corner" at $\Sigma$ as a coordinate artifact.
