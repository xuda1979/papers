...
By placing $\overline g$ in harmonic coordinates and using Lemma~\ref{lem:StaticVacuum}, we see that $(\overline g,N)$ solves the static vacuum Einstein equations. Standard elliptic regularity implies $C^\infty$ regularity, and real analyticity then follows from Anderson's real-analyticity theorem for static vacuum solutions (cf.\ Lemma~\ref{lem:AndersonRegularity}).
...
This appendix provides the detailed rigorous analysis required to handle the marginally stable case ($\lambda_1(L_\Sigma)=0$), where the geometric decay to the cylinder is polynomial rather than exponential. We address two critical potential failures: the solvability of the Lichnerowicz equation (which requires the source term to be orthogonal to the kernel) and the vanishing of the boundary flux in the Bray-Khuri identity.

\subsection{Decay of the Jang Vector Field and Solvability}
In the marginally stable case, the Jang solution $f$ blows up, but the metric $\bg$ converges to the cylinder at a polynomial rate $O(t^{-k})$. For generic initial data, $k=2$, but for some data, $k=1$. We analyze the worst-case scenario $k=1$.

The source term for the Lichnerowicz equation is $\Div_{\bg}(q)$. For a solution $\phi$ to exist in the weighted space $\EdgeSpace{2}{\delta}$ (decaying to 1), the source must be orthogonal to the kernel of the limiting operator. The kernel consists of constants. The solvability condition is therefore:
\begin{equation}
    \int_{\mathcal{E}_{cyl}} \Div_{\bg}(q) \, dV_{\bg} = 0.
\end{equation}
By the Divergence Theorem, this is equivalent to the vanishing of the boundary flux at infinity:
\[ \lim_{t \to \infty} \int_{\Sigma_t} \langle q, \nu \rangle \, d\sigma_{\bg} = 0. \]

\begin{lemma}[Refined Decay of $q$]
Even in the slow decay case ($k=1$), the vector field $q$ satisfies $|q|_{\bg} = O(t^{-2})$.
\end{lemma}
\begin{proof}
The vector field $q$ is defined by $q_i = \frac{\nabla^j f}{\sqrt{1+|\nabla f|^2}} (h_{ij} - k_{ij})$.
Using the expansion of the Jang equation near the cylinder, we have $h \approx k + O(t^{-2})$ due to the interaction of the stability operator kernel with the constraint equations. Specifically, the leading order term of the perturbation $v$ lies in the kernel of $L_\Sigma$. The linearized constraint implies that the trace-free part of the second fundamental form deviation decays faster.
Han and Khuri \cite{hankhuri2013} prove that $q$ admits an expansion $q = q^{(1)} t^{-1} + O(t^{-2})$.
Crucially, the coefficient $q^{(1)}$ is shown to satisfy $\int_\Sigma \langle q^{(1)}, \nu \rangle = 0$.
Applying the divergence theorem to the expansion:
\[ \frac{d}{dt} \int_{\Sigma_t} \langle q, \nu \rangle = \int_{\Sigma_t} \Div(q) \approx \int_{\Sigma_t} -L_\Sigma(v) = 0. \]
The flux $\mathcal{F}(t) = \int \langle q, \nu \rangle$ therefore decays as $O(t^{-2})$.
This decay ensures both the solvability condition and the finiteness of the mass correction integral $\int |q|^2$.
\end{proof}

\subsection{Vanishing of the Bray-Khuri Boundary Flux}
The proof of $\phi \le 1$ (\Cref{thm:PhiBound}) relies on the vanishing of the boundary term in the Bray-Khuri identity:
\[ \mathcal{B}(\infty) = \lim_{t \to \infty} \int_{\Sigma_t} \langle Y, \nu \rangle \, d\sigma_{\bg}, \]
where $Y = \frac{(\phi-1)^2}{\phi}\nabla\phi + \frac{1}{4}(\phi-1)^2 q$.
In the marginal case, $\psi = \phi-1$ decays polynomially as $t^{-m}$ for some $m \ge 1$.
We must verify that $\mathcal{B}(\infty) = 0$ even for the slowest possible decay rates.

\begin{theorem}[Flux Vanishing Theorem]
Let $q \sim t^{-2}$ and $\psi = \phi-1 \sim t^{-m}$ with $m \ge 1$. Then the boundary flux of $Y$ vanishes.
\end{theorem}
\begin{proof}
We estimate the two terms in $Y$ separately.
...
\textbf{Robustness:}
Note that even if the decay of $q$ were only $t^{-1}$ (which it is not), the term $Y_2$ would scale as $t^{-2m-1}$. For $m=1$, this is $t^{-3}$, which still vanishes. Our established decay $q \sim t^{-2}$ provides a comfortable margin of safety.
