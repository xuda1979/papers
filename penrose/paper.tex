...
% or explicitly stated as hypotheses whose status is clearly indicated.
...
\begin{abstract}
We establish the spacetime Penrose inequality
\[
  M_{\ADM}(g) \;\ge\; \sqrt{\frac{A(\Sigma)}{16\pi}}
\]
for asymptotically flat initial data sets satisfying the dominant energy
condition, where $M_{\ADM}(g)$ is the ADM mass of the initial data and
$A(\Sigma)$ is the area of the outermost apparent horizon.
The proof unifies the generalized Jang reduction with the $p$-harmonic
level set method via a detailed analysis of the \emph{Jang--Lichnerowicz
system}. The key difficulty in earlier approaches is the lack of
pointwise nonnegativity of the scalar curvature of the Jang metric and
the presence of a distributional scalar curvature singularity at the
horizon interface.  We show that the stability of the outermost MOTS
implies that the singularity is \emph{convex} in a precise sense and
that the Jang scalar curvature is nonnegative as a distribution.  This
allows us to construct a scalar-curvature preserving smoothing of the
metric, adapted to the internal corner. A second difficulty is to
control the ADM mass under the conformal deformation that removes the
divergence term in the Jang scalar curvature. We obtain the required
bound $\phi\le 1$ on the conformal factor by combining a quadratic-form
argument with the Bray--Khuri divergence identity.  Finally, we show
that equality in the spacetime Penrose inequality forces the initial
data to arise from a spatial Schwarzschild slice.
\end{abstract}
...
\begin{remark}
In Definition~\ref{def:ADM_Lipschitz} we will only apply the ADM mass
 to metrics that are $C^{0,1}$ and asymptotically flat in the sense of
 Definition~\ref{def:AF}; for such metrics the ADM flux integral is well-defined.
\end{remark}
...
\begin{itemize}
    ...
    \item \textbf{Marginally Stable Case:} If $\lambda_1(L_\Sigma) = 0$, the decay is polynomial: $L = L_0 + E(t)$ where coefficients of $E(t)$ decay as $O(t^{-2})$. We must verify that $E(t)$ remains a compact perturbation in the weighted spaces defined by $\gamma \in (0,1)$.
    The weight function is $w(t) = e^{\gamma t}$. The map $u \mapsto E(t)u$ is compact from $W^{2,p}_\gamma$ to $L^p_\gamma$ if for any bounded sequence $\{u_n\}$ in $W^{2,p}_\gamma$, the sequence $\{E(t)u_n\}$ has a convergent subsequence in $L^p_\gamma$.
    Boundedness in $W^{2,p}_\gamma$ implies Rellich compactness on bounded domains $t \in [0, T]$. We control the tail contributions:
    \[ \int_T^\infty |E(t) u_n|^p e^{p\gamma t} dt \le \sup_{t \ge T} |E(t)|^p \int_T^\infty |u_n|^p e^{p\gamma t} dt \le C T^{-2p} \|u_n\|_{L^p_\gamma}^p. \]
    Since $\|u_n\|_{L^p_\gamma}$ is uniformly bounded, the tail integral vanishes uniformly as $T \to \infty$. Combined with local compactness, this proves $E(t)$ is a compact operator.
\end{itemize}
...
The condition $[H]=0$ implies that the mean curvature is continuous across $\Sigma$. Since the intrinsic metric is continuous, the full metric $\bg$ is of class $C^{1,1}$ in harmonic coordinates (or can be approximated as such).
Specifically, the static vacuum equations $\Ric_{\bg} = N^{-1}\nabla^2 N$ and $\Delta N = 0$ hold distributionally.
We prove that such metrics are in fact real-analytic.

\begin{lemma}[Regularity of Static Vacuum Metrics]
Let $(\bg, N)$ be a weak solution to the static vacuum equations with $\bg \in C^{1,\alpha}_{loc}$ and $N \in C^{1,\alpha}_{loc}$. Then $(\bg, N)$ is real-analytic in harmonic coordinates.
\end{lemma}
\begin{proof}
In harmonic coordinates $\{x^k\}$, the Ricci curvature is given by:
\[ \Ric_{ij} = -\frac{1}{2} g^{kl} \partial_k \partial_l g_{ij} + Q(\bg, \partial \bg), \]
where $Q$ is a quadratic form in the first derivatives. The static vacuum equation becomes:
...
Since $g \in C^{1,\alpha}$, the coefficients of the principal part ($g^{kl}$) are $C^{1,\alpha}$. The lower order terms involve $\partial g$, which are $C^{0,\alpha}$.
By standard Schauder estimates, if the coefficients are $C^{k,\alpha}$, the solution is $C^{k+2,\alpha}$. Here, coefficients are $C^{1,\alpha}$, so $g \in C^{3,\alpha}$.
...
By the uniqueness of asymptotically flat static vacuum black holes (Israel, Bunting-Masood-ul-Alam), the analytic manifold $(\bM, \bg)$ must be the spatial Schwarzschild geometry.
Since the initial data $(M,g,k)$ is embedded into $(\bM, \bg)$ with $q=0$ (so $k=h$), it is an isometric slice of the Schwarzschild spacetime.
\end{proof}
...
The smoothed curvature contains $Q(A_\epsilon)$, while the smoothing of the curvature would be $\eta_\epsilon * Q(A)$.
The "curvature deficit" is essentially $Q(A_\epsilon) - \eta_\epsilon * Q(A)$.

Let $k(s) = -\frac{1}{2} g'(s)$. This is a bounded function with a jump at $s=0$.
Then $A_\epsilon(s) \approx \eta_\epsilon * k$.
The quadratic term behaves like $-|\eta_\epsilon * k|^2$.
Since $g'$ is bounded ($L^\infty$), $A_\epsilon(s)$ is bounded uniformly in $\epsilon$.
Therefore, $|A_\epsilon|^2$ and $(\Tr A_\epsilon)^2$ are bounded by $C \|g'\|_{L^\infty}^2$.
...
\subsection*{Legacy Outline and status}
...
In the main body of this paper we have now provided proofs of the
analytic ingredients that were previously treated as black boxes; the
material in this section is retained for comparison with the earlier,
conditional version of the argument.
