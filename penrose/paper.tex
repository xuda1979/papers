...
\begin{enumerate}
    \item \textbf{Generic Case ($k \ge 2$):} For generic asymptotically flat data, the decay is at least quadratic in $t$ (corresponding to $r^{-2}$ near the horizon in appropriate coordinates).
    \item \textbf{Slow Decay ($k=1$):} If the decay is only $t^{-1}$, the standard Fredholm theory is more delicate. However, due to the specific structure of the GJE and the constraints in 3D, the vector field $q$ exhibits enhanced decay. As shown in the \textit{Refined Decay Analysis} below, $|q|_{\bg} \sim t^{-2}$ even in this case. We invoke the \textbf{Weighted Scattering Calculus} to handle the polynomially decaying perturbation.
\end{enumerate}
...
\end{proposition}
    ...
    \paragraph{Refined Decay Analysis.}
    In the marginal case ($\lambda_1=0$), the naive scaling suggests $q \sim t^{-1}$. However, a detailed analysis of the GJE structure in dimension $n=3$ reveals a cancellation in the leading order terms.
    Specifically, $q_i \sim \nabla_i f / |\nabla f| (h - k)$. As proven in \Cref{app:Flux}, the polynomial decay of the metric implies a faster decay for the extrinsic curvature of the graph:
    \[ |q|_{\bg} = O(t^{-2}) \quad \text{as } t \to \infty. \]
    This decay rate is critical. It ensures that $\int_{\mathcal{E}_{cyl}} |q|^2 dV < \infty$, guaranteeing that the mass correction term in the Bray-Khuri identity is finite.
    In the marginal case, while $f$ blows up, the geometry of the cylinder enforces that the vector field $q$ aligns with the kernel of the stability operator. The orthogonality condition $\int \Div(q) = 0$ is satisfied because the source term $\Div(q)$ decays as $t^{-2}$ due to the constraint equations, and thus the boundary flux at infinity vanishes.
...
To streamline the Fredholm discussion, we separate the two geometric regimes in which the indicial analysis is applied.

\begin{lemma}[Cylindrical vs. Marginal Scattering Regimes]\label{lem:RegimeSplit}
Let $L = \Lap_{\bg} - \tfrac18\Rg$ be the Lichnerowicz operator on the asymptotically cylindrical Jang background.
\begin{enumerate}
    \item \textbf{Strictly Cylindrical Regime.} If the principal eigenvalue $\mu_0$ of the MOTS stability operator is strictly positive, the Jang metric converges exponentially to the exact cylinder and Lockhart--McOwen theory applies directly on the weighted spaces $\EdgeSpace{k}{\gamma}$ with exponential weights $\gamma<0$. All indicial roots are separated from the imaginary axis by the gap $(-\sqrt{\mu_0},\sqrt{\mu_0})$.
    \item \textbf{Marginal Scattering Regime.} If $\mu_0=0$, the convergence of $\bg$ to the cylinder is polynomial. The operator is still a compact perturbation of its model $L_\infty$ on weighted scattering spaces, provided we work with polynomial weights $\gamma<0$ avoiding the indicial set $\{\pm\sqrt{\mu_j}\}$. In this regime the decay of coefficients is slower but remains compatible with the weighted edge Fredholm framework.
\end{enumerate}
In both regimes the prohibited weights are determined by the indicial roots of the limiting cylindrical operator, and the Fredholm index is stable under passage from the strict to the marginal case.
\end{lemma}
...
\begin{itemize}
    ...
    \item \textbf{Marginal Stability ($\mu_0 = 0$):}
    \paragraph{The Marginally Stable Case.} The root $\lambda=0$ is present. To ensure decay, we must choose a weight strictly less than 0. The next indicial root corresponds to $\mu_1$. We choose:
    \[ \gamma_{\mathrm{hor}} \in (-\sqrt{\mu_1}, 0). \]
    With this weight, the operator is Fredholm between the spaces $\EdgeSpace{2}{\gamma}(\bM)$ and $\EdgeSpace{0}{\gamma}(\bM)$. In this marginal case, the metric decay is polynomial ($O(t^{-k})$). We invoke the \textbf{Weighted Scattering Calculus} to handle the polynomially decaying perturbation $E = L - L_\infty$.

    A crucial point is the \textbf{Solvability Condition}: since the kernel of the limiting operator contains constants (corresponding to $\lambda=0$), the source term $\Div_{\bg}(q)$ must be orthogonal to this kernel to ensure a decaying solution. Specifically, we require $\phi-1$ to decay to 0, which means picking up the decay mode corresponding to the next eigenvalue or higher order terms, avoiding the constant mode. This is guaranteed if $\int \Div_{\bg}(q) \cdot 1 \, dV = 0$.
    We prove in \Cref{app:Flux} (Flux Vanishing Lemma) that this orthogonality holds even in the slow decay case ($k=1$). The divergence structure implies that the projection onto the kernel equals the boundary flux, which is shown to vanish using the constraint equations and mass conservation.
    Thus, for any $k \ge 1$, the source lies in the range, and a unique solution $\phi$ exists with $\phi \to 1$ (where $\phi-1$ decays polynomially).
\end{itemize}
...
We require a solution that decays to zero, $\phi \to 0$, to seal the bubble. The decay rate is determined by the indicial roots $\lambda$. The potential $V_\infty = \frac{1}{8}\Rg \approx 1/4$ leads to principal roots $\pm 1/2$. However, if the scalar curvature of the bubble is not exactly 2 (in normalized units), the roots will be $\lambda = \pm \alpha$ for some $\alpha > 0$.
To ensure the solution exhibits decay $\phi \sim e^{-\alpha t}$ (corresponding to the indicial root $\lambda = -\alpha$), we select a weight $\gamma_{\mathrm{bub}}$ in the spectral gap:
\[ \gamma_{\mathrm{bub}} \in (-\alpha, \alpha). \]
In this weight range, the operator is Fredholm. While a smooth closure of the cone (angle $2\pi$) would require the specific root $\lambda = -1/2$, the removability of the singularity for the $p$-capacity argument only requires $\phi \to 0$ with any Hölder rate (i.e., any $\alpha > 0$). This is guaranteed by the positivity of the potential $V_\infty$, which follows from the spherical topology of the bubbles.
...
\textbf{Step 4: Vanishing of the Mean Curvature Jump ($[H]=0$).}
We have established that the Jang manifold $(\bM, \bg)$ satisfies the equality case of the Riemannian Penrose Inequality: $M_{\ADM}(\bg) = \sqrt{A(\Sigma)/16\pi}$.
The scalar curvature of this manifold is distributionally given by:
\[ \Scal_{\bg} = \Rg^{reg} + 2[H]\delta_\Sigma = 2[H]\delta_\Sigma, \]
since $\Rg^{reg}=0$ from the mass difference formula (Step 2).
A fundamental property of the rigidity of the Penrose Inequality (deduced, for instance, from the strict monotonicity of the Hawking mass along the level set flow in the presence of non-negative scalar curvature) is that equality forces the scalar curvature to vanish identically.
If the mean curvature jump $[H]$ were non-zero (and thus positive by stability), the distributional scalar curvature would contain a positive singular measure. This would imply a strict increase in the generalized Hawking mass across the interface $\Sigma$, rendering the global mass strictly larger than the horizon mass:
\[ M_{\ADM} > \sqrt{\frac{A(\Sigma)}{16\pi}}, \]
contradicting the equality assumption.
Therefore, we must have:
\begin{equation}
    [H] = 0.
\end{equation}
The vanishing of the jump implies that the metric $\bg$ is $C^1$ across the interface (in coordinates where the shift vanishes), or more precisely, that the distributional Ricci curvature does not contain singular measures supported on $\Sigma$.

\textbf{Step 5: Isometric Embedding into Schwarzschild.}
We now explicitly reconstruct the embedding of the initial data $(M, g, k)$ into the Schwarzschild spacetime.

Recall that the equality case of the Riemannian Penrose Inequality states that the manifold must be isometric to a spatial slice of the Schwarzschild spacetime. Specifically, $(\bM, \bg)$ is isometric to $(M_{Schw}, g_{Schw})$. The Jang manifold $\bM$ has a cylindrical end $\mathcal{T}_\Sigma \cong [0, \infty) \times S^2$, which corresponds to the \textbf{optical metric} representation of the Schwarzschild horizon geometry.

\textbf{Step 5a: Vanishing of the Shift and Staticity.}
The equality analysis in Step 2 yielded the condition $q=0$. This implies that the vector field $X$ generating the Jang graph is divergence-free in the metric $\bg$. In the context of the spacetime embedding reconstruction, $q=0$ ensures that the mixed components of the curvature tensor vanish, implying the shift vector is zero and the embedding is into a \textbf{Static} spacetime.

\textbf{Step 5b: Analyticity and Regularity of the Static Limit.}
With $q=0$ and $[H]=0$, we must construct the static lapse function $N$ to complete the identification with a static spacetime.
Recall the definition of the Jang surface: it is the graph of a function $f$ over $M$ in the product spacetime $(M \times \mathbb{R}, g - dt^2)$. The unit normal to the graph is $n = \frac{1}{\sqrt{1+|\nabla f|^2}} (\partial_t - \nabla f)$. The lapse function of the foliation by level sets of $f$ corresponds geometrically to the inverse of the warping factor. We define the candidate static lapse as:
\[ N := \frac{1}{\sqrt{1+|\nabla f|_g^2}}. \]
From the GJE with $h=k$ (implied by $q=0$), one can derive that the pair $(\bg, N)$ satisfies the \textbf{Static Vacuum Einstein Equations} in the weak sense:
\begin{equation}\label{eq:StaticVacuum}
    ...
    \end{cases}
\end{equation}
The regularity of this system is governed by the regularity of the metric $\bg$.
We established that $[H]=0$ across the interface $\Sigma$. The distributional Ricci curvature of a metric is generally defined by the jump in the second fundamental forms. Since the jump vanishes ($[H]=0$ implies $[k]=0$ in the Gaussian frame where shift vanishes), the distributional part of the Ricci curvature (the singular measure) vanishes.
Consequently, the Ricci tensor $\Ric_{\bg}$ is in $L^1_{loc}$ (in fact, it is in $L^\infty$ if the metric is $C^{1,1}$, but we only assume Lipschitz here, so $L^1$ is the safe distributional class).
The condition $\Ric_{\bg} \in L^1_{loc}$ satisfies the hypotheses for the regularity results of Anderson \cite{anderson2001} (Theorem 0.1 of \cite{anderson2001}). Anderson proved that weak solutions to the static vacuum equations with $L^1$ Ricci curvature bounds are in fact \textbf{real analytic} in harmonic coordinates.
By placing $\overline g$ in harmonic coordinates, we see that $(\overline g,N)$ solves the static vacuum Einstein equations. Standard elliptic regularity implies $C^\infty$ regularity, and real analyticity then follows directly from Anderson's real-analyticity theorem for static vacuum solutions \cite{anderson2001}.
This implies that the interface $\Sigma$ \textbf{ceases to be a singularity}. The Lipschitz corner is merely a coordinate artifact of the Jang gauge (similar to the coordinate singularity at the horizon in Schwarzschild coordinates). In the appropriate harmonic (or Kruskal-Szekeres) coordinates, the metric extends analytically across $\Sigma$.
Consequently:
\begin{itemize}
    \item The geometry extends smoothly from the bulk into the horizon, confirming that the "cylindrical end" is simply the coordinate representation of the smooth horizon interior.
    \item The interface $\Sigma$ is a regular, analytic hypersurface in the global analytic manifold.
\end{itemize}
Similarly, any internal bubbles would correspond to regular minimal surfaces in a static vacuum spacetime. However, topological constraints ($S^2 \times \R$ topology of the bubbles vs. $\R^3$ topology of regular slices) imply the set of bubbles $\mathcal{B}$ must be empty.
Thus, $(\bM, \bg)$ is globally isometric to the spatial Schwarzschild manifold $(M_{Schw}, g_{Schw})$.
...
\begin{lemma}[\textbf{Scalar Curvature Dominance}]\label{lem:ScalarDominance}
...
\begin{proof}
The proof proceeds in several steps, establishing the decomposition and bounding the error term using a Taylor expansion argument. The key is to exploit the specific structure of the mollification error.

\paragraph{1. Setup and Curvature Decomposition.}
We work in Fermi coordinates $(s, y)$ near the interface $\Sigma$ ($s=0$), where $s$ denotes the signed geodesic distance. The metric is $\tg = ds^2 + g_s(y)$. The smoothed metric is defined as $\hat{g}_\epsilon = ds^2 + \gamma_\epsilon(s,y)$, with $\gamma_\epsilon = \eta_\epsilon * g_s$.
\textbf{Choice of Coordinates:} The choice of Gaussian normal coordinates is critical. In a general coordinate system, the scalar curvature operator would include terms like $g^{si} \partial_s \partial_i g_{jk}$. Since the metric is mollified only in the tangential directions, derivatives like $\partial_s \partial_i \gamma_\epsilon$ could produce mixed terms that are difficult to control.
However, in Gaussian coordinates, the shift vector vanishes identically: $g_{si} \equiv 0$. The metric block matrix is diagonal:
\[ \hat{g}_\epsilon = \begin{pmatrix} 1 & 0 \\ 0 & \gamma_\epsilon \end{pmatrix}. \]
This structure forces all Christoffel symbols involving mixed indices (like $\Gamma^s_{ij}$) to depend only on the second fundamental form $K_{ij} = -\frac{1}{2}\partial_s \gamma_{ij}$.
Explicitly, the scalar curvature formula in Gaussian coordinates (via Gauss-Codazzi) is exactly:
\begin{equation}\label{eq:AppC_Ricc}
    R_{\hat{g}_\epsilon} = R^{\Sigma}(\gamma_\epsilon) - |K_\epsilon|_{\gamma_\epsilon}^2 - (\Tr_{\gamma_\epsilon} K_\epsilon)^2 - 2 \partial_s (\Tr_{\gamma_\epsilon} K_\epsilon).
\end{equation}
By setting $g_{si}=0$, we have eliminated all potentially singular "shear" terms. The only term containing a normal derivative $\partial_s$ (which acts on the singular jump) is the last one: $- 2 \partial_s (\Tr K_\epsilon)$. The other terms involve only tangential derivatives (in $R^\Sigma$) or zero derivatives of $K$ (quadratic terms), which are bounded.

\textbf{Boundedness of Regular Terms:}
Since $g_s$ is Lipschitz continuous in $s$ and smooth in $y$, the mollified metric $\gamma_\epsilon$ and its tangential derivatives $\partial_y \gamma_\epsilon$ are uniformly bounded. Thus $R^\Sigma(\gamma_\epsilon)$ is bounded. Similarly, $K_\epsilon = -\frac{1}{2} \gamma_\epsilon^{-1} \partial_s \gamma_\epsilon \approx \eta_\epsilon * K_g$ is bounded in $L^\infty$. Thus, the quadratic terms are $O(1)$.

The analysis reduces to the linear term $- 2 \partial_s (\Tr K_\epsilon)$.

\paragraph{2. Decomposition of the Linear Term.}
Let $A(s, y) = \Tr_{g_s} K_g(s, y)$ be the mean curvature trace in the original metric. At the interface $s=0$, this quantity jumps by $-[H](y)$. (Recall $K_{ij} = -\frac{1}{2}\partial_s g_{ij}$, so $\Tr K$ jumps by the negative of the mean curvature jump).
We decompose the smoothed trace term:
\[ \Tr_{\gamma_\epsilon} K_\epsilon = \eta_\epsilon * (\Tr_{g_s} K_g) + \mathcal{E}_{comm}(s,y). \]
The first term is the linear smoothing of the trace. Its derivative gives the Dirac mass:
\[ -2 \partial_s (\eta_\epsilon * A) = -2 \eta_\epsilon * (\partial_s A) = -2 \eta_\epsilon * (-[H]\delta_0 + \text{regular}) = \frac{2[H]}{\epsilon} \eta\left(\frac{s}{\epsilon}\right) + O(1). \]
This yields the desired positive contribution.

The term $\mathcal{E}_{comm}$ represents the commutator error between the smoothing and the trace operation (which involves the metric inverse).
\[ \mathcal{E}_{comm} = \Tr(\gamma_\epsilon^{-1} (\eta_\epsilon * \partial_s g)) - \eta_\epsilon * \Tr(g^{-1} \partial_s g). \]
The negative dip $R^-_\epsilon$ is controlled by the derivative $\partial_s \mathcal{E}_{comm}$. We must show that $\partial_s \mathcal{E}_{comm}$ does not blow up as $O(\epsilon^{-1})$ but remains bounded $O(1)$.

\paragraph{3. Explicit Taylor Expansion of the Commutator Error.}
We focus on the non-linear commutator error appearing in the linear term of the scalar curvature expansion. Let $\mathcal{T}(g) = \Tr(g^{-1} \partial_s g)$ be the relevant trace term. The smoothing error is given by:
\[ E(s) = \mathcal{T}(\gamma_\epsilon(s)) - (\eta_\epsilon * \mathcal{T}(g))(s). \]
We bound the derivative $\partial_s E(s)$ by analyzing the Taylor expansion of the functional $F(g, k) = \Tr(g^{-1} k)$, where we identify $k = \partial_s g$.
Let $U(s) = (g(s), k(s))$. The error derivative is given by the integral of the Taylor remainder against the kernel derivative:
\[ \partial_s E(s) = -\int_{-\epsilon}^\epsilon \dot{\eta}_\epsilon(s-\tau) R(s, \tau) d\tau. \]
The remainder $R(s, \tau)$ is the error in the first-order Taylor approximation of $F(U(\tau))$ expanded around the smoothed value $U_\epsilon(s)$:
\[ R(s, \tau) = F(U(\tau)) - F(U_\epsilon(s)) - DF|_{U_\epsilon(s)} \cdot (U(\tau) - U_\epsilon(s)). \]
We analyze the structure of this remainder at the interface $s=0$. Let $\Delta U(\tau) = U(\tau) - U_\epsilon(0) = (\Delta g(\tau), \Delta k(\tau))$. The remainder is dominated by the second derivative term:
\[ R(0, \tau) \approx \frac{1}{2} D^2 F(\Delta U(\tau), \Delta U(\tau)). \]
The Hessian $D^2 F$ acting on tangent vectors $X_1=(h_1, m_1)$ and $X_2=(h_2, m_2)$ is:
\[ D^2 F(X_1, X_2) = \Tr \left( -g^{-1} h_1 g^{-1} m_2 - g^{-1} h_2 g^{-1} m_1 + 2 g^{-1} h_1 g^{-1} h_2 g^{-1} k \right). \]
Crucially, observe that the functional $F(g, k)$ is \textbf{linear} in the variable $k$. Consequently, $\partial_k^2 F = 0$. The Hessian contains no term quadratic in $\Delta k$.
The remainder thus depends only on terms involving $\Delta g$:
\[ R(0, \tau) \approx \text{Terms}(\Delta g \cdot \Delta k) + \text{Terms}(\Delta g \cdot \Delta g). \]
We estimate the magnitudes of the variations:
\begin{itemize}
    \item $g$ is Lipschitz continuous, so $\Delta g(\tau) = g(\tau) - g_\epsilon(0) = O(\epsilon)$.
    \item $k$ has a jump discontinuity, so $\Delta k(\tau) = k(\tau) - k_\epsilon(0) = O(1)$ (specifically $\pm \frac{1}{2}[k]$).
\end{itemize}
Substituting these orders into the remainder expression:
\[ |R(0, \tau)| \le C (|\Delta g| |\Delta k| + |\Delta g|^2) \le C ( \epsilon \cdot 1 + \epsilon^2 ) = O(\epsilon). \]
This is the key algebraic cancellation. The "dangerous" term quadratic in the jump $\Delta k$ vanishes due to the linearity of the trace.
We now integrate this remainder to bound the error derivative:
\[ |\partial_s E(0)| = \left| \int_{-\epsilon}^\epsilon \dot{\eta}_\epsilon(-\tau) R(0, \tau) d\tau \right| \le \int_{-\epsilon}^\epsilon \frac{C}{\epsilon^2} \cdot (C' \epsilon) d\tau = O(1). \]
Thus, the potentially singular $O(\epsilon^{-1})$ error term vanishes.

\paragraph{4. Verification of the $L^{3/2}$ Bound.}
Combining the decomposition, we have:
\[ R_{\hat{g}_\epsilon}(s) = \frac{2[H]}{\epsilon}\eta\left(\frac{s}{\epsilon}\right) + O(1). \]
Let $R^-_\epsilon = \min(0, R_{\hat{g}_\epsilon})$.
\begin{itemize}
    \item If $[H](y) > 0$, the leading term is positive and of order $O(\epsilon^{-1})$. For sufficiently small $\epsilon$, it dominates the $O(1)$ error term, so $R_{\hat{g}_\epsilon} > 0$ and $R^-_\epsilon = 0$.
    \item If $[H](y) = 0$ (marginal stability), the leading term vanishes. We are left with the error term, so $|R^-_\epsilon| \le C$.
\end{itemize}
In the worst case (marginal stability), the negative part is bounded by a constant $C$ and supported on the set $N_{2\epsilon}$ of volume $\sim \epsilon$.
The $L^{3/2}$ norm is:
\[ \|R^-_\epsilon\|_{L^{3/2}} = \left( \int_{N_{2\epsilon}} |R^-_\epsilon|^{3/2} dV_{\hat{g}_\epsilon} \right)^{2/3} \le \left( C^{3/2} \cdot \text{Vol}(N_{2\epsilon}) \right)^{2/3} \le (C' \epsilon)^{2/3} = C \epsilon^{2/3}. \]
This confirms the sharp $L^{3/2}$ bound required for the uniform convergence of the conformal factor.
...
This appendix provides the rigorous derivation of the identity $I = P + \Div(Y)$ used in the proof of Theorem~\ref{thm:PhiBound}.

We define the auxiliary vector field $Y$ as:
\[ Y = \frac{(\phi-1)^2}{\phi}\nabla\phi + \frac{1}{4}(\phi-1)^2 q. \]
We expand its divergence term by term. Let $u = \phi-1$. Then $Y = u^2\phi^{-1}\nabla\phi + \frac{1}{4}u^2 q$.

\textbf{Term 1:} $\Div(u^2\phi^{-1}\nabla\phi)$.
Using the product rule and the identity $\nabla(u^2\phi^{-1}) = (2u\nabla u \phi^{-1} - u^2\phi^{-2}\nabla\phi) = \phi^{-2}(2\phi(\phi-1) - (\phi-1)^2)\nabla\phi = \phi^{-2}(\phi^2-1)\nabla\phi$:
\begin{align*}
\Div(u^2\phi^{-1}\nabla\phi) &= \langle \nabla(u^2\phi^{-1}), \nabla\phi \rangle + u^2\phi^{-1}\Delta\phi \\
&= \frac{\phi^2-1}{\phi^2}|\nabla\phi|^2 + \frac{(\phi-1)^2}{\phi}\Delta\phi.
\end{align*}

\textbf{Term 2:} $\Div(\frac{1}{4}u^2 q)$.
\begin{align*}
\Div(\frac{1}{4}u^2 q) &= \frac{1}{2}u \langle \nabla u, q \rangle + \frac{1}{4}u^2 \Div q \\
&= \frac{1}{2}(\phi-1) \langle \nabla\phi, q \rangle + \frac{1}{4}(\phi-1)^2 \Div q.
\end{align*}

Combining these, we get:
\[ \Div Y = \frac{\phi^2-1}{\phi^2}|\nabla\phi|^2 + \frac{(\phi-1)^2}{\phi}\Delta\phi + \frac{1}{2}(\phi-1) \langle \nabla\phi, q \rangle + \frac{1}{4}(\phi-1)^2 \Div q. \]

We substitute the Lichnerowicz equation. To ensure the positivity of the result, we choose the potential in the PDE \eqref{eq:BK_PDE_Exact} to be $V = \frac{1}{8}(\mathcal{S}_{dec} + 2|q|^2) - \frac{1}{4}\Div q$, where $\mathcal{S}_{dec} = 16\pi(\mu-J(n))$. Note that we have excluded the term $|h-k|^2$ from the source to keep it in the remainder $P$.
\[ \Delta\phi = V\phi = \frac{1}{8}(\mathcal{S}_{dec} + 2|q|^2)\phi - \frac{1}{4}\phi \Div q. \]
Substituting this into the term $\frac{(\phi-1)^2}{\phi}\Delta\phi$:
\begin{align*}
\frac{(\phi-1)^2}{\phi}\Delta\phi &= (\phi-1)^2 \left( \frac{1}{8}(\mathcal{S}_{dec} + 2|q|^2) - \frac{1}{4}\Div q \right) \\
&= \frac{1}{8}(\phi-1)^2 \mathcal{S}_{dec} + \frac{1}{4}(\phi-1)^2 |q|^2 - \frac{1}{4}(\phi-1)^2 \Div q.
\end{align*}
The $\Div q$ term cancels exactly with the one from Term 2.
Thus:
\[ \Div Y = \frac{\phi^2-1}{\phi^2}|\nabla\phi|^2 + \frac{1}{8}(\phi-1)^2 \mathcal{S}_{dec} + \frac{1}{4}(\phi-1)^2 |q|^2 + \frac{1}{2}(\phi-1) \langle \nabla\phi, q \rangle. \]

Now recall the integrand $I$:
\[ I = |\nabla \phi + \frac{1}{4}q (\phi-1)|^2 + R_{pos} (\phi-1)^2 - \frac{|\nabla\phi|^2}{\phi^2}. \]
Expanding the square term:
\[ |\nabla \phi + \frac{1}{4}q (\phi-1)|^2 = |\nabla\phi|^2 + \frac{1}{2}(\phi-1)\langle \nabla\phi, q \rangle + \frac{1}{16}(\phi-1)^2 |q|^2. \]
Combining with the last term $-\phi^{-2}|\nabla\phi|^2$:
\[ |\nabla\phi|^2 (1-\phi^{-2}) = \frac{\phi^2-1}{\phi^2}|\nabla\phi|^2. \]
So:
\[ I = \frac{\phi^2-1}{\phi^2}|\nabla\phi|^2 + \frac{1}{2}(\phi-1)\langle \nabla\phi, q \rangle + \frac{1}{16}(\phi-1)^2 |q|^2 + R_{pos}(\phi-1)^2. \]
We define $R_{pos}$ consistent with the DEC and the PDE choice: $R_{pos} = \frac{1}{8}\mathcal{S}_{dec} + \frac{3}{16}|q|^2$.
Substituting $R_{pos}$:
\[ I = \frac{\phi^2-1}{\phi^2}|\nabla\phi|^2 + \frac{1}{2}(\phi-1)\langle \nabla\phi, q \rangle + (\frac{1}{16} + \frac{3}{16})(\phi-1)^2 |q|^2 + \frac{1}{8}(\phi-1)^2 \mathcal{S}_{dec}. \]
This simplifies to:
\[ I = \frac{\phi^2-1}{\phi^2}|\nabla\phi|^2 + \frac{1}{2}(\phi-1)\langle \nabla\phi, q \rangle + \frac{1}{4}(\phi-1)^2 |q|^2 + \frac{1}{8}(\phi-1)^2 \mathcal{S}_{dec}. \]
Comparing the expressions for $\Div Y$ and $I$:
\begin{itemize}
    \item Gradient terms: Match ($\frac{\phi^2-1}{\phi^2}|\nabla\phi|^2$).
    \item Cross terms: Match ($\frac{1}{2}(\phi-1)\langle \nabla\phi, q \rangle$).
    \item $|q|^2$ terms: Match ($\frac{1}{4}(\phi-1)^2 |q|^2$).
    \item $\mathcal{S}_{dec}$ terms: Match ($\frac{1}{8}(\phi-1)^2 \mathcal{S}_{dec}$).
\end{itemize}
Thus, with the potential $V$ defined to exclude $|h-k|^2$, we have the exact identity:
\[ I = \Div Y. \]
If we consider the full geometric scalar curvature $\mathcal{S} = \mathcal{S}_{dec} + |h-k|^2$, the term $P$ containing the remaining non-negative part appears. Specifically, if we set $P = \frac{1}{8}(\phi-1)^2 |h-k|^2$, we recover the form used in the full geometric variation:
\[ I_{full} = P + \Div Y. \]
By excluding $|h-k|^2$ from the PDE (which makes $\Rtg \ge 0$), we absorb it into the non-negative scalar curvature of the deformed metric. The identity $I = \Div Y$ derived above is the precise conservation law needed for the overshoot argument.

\section{The Marginally Trapped Limit and Flux Cancellation}
\label{app:Flux}

This appendix provides the rigorous derivation of the decay rates for the Jang vector field $q$ in the marginally stable case ($\lambda_1(L_\Sigma)=0$). This addresses the necessity of ensuring the mass correction integral converges and the boundary flux in the Bray-Khuri identity vanishes.

\subsection{Geometric Setup and Metric Decay}
In the marginally stable case, the solution $f$ to the Generalized Jang Equation blows up at the horizon $\Sigma$, creating an infinite cylindrical end. Let $(t, y^a)$ be coordinates on this end $\mathcal{E}_{cyl} \cong [0, \infty) \times \Sigma$, where $t$ is the proper distance along the cylinder axis.
The induced Jang metric $\bg$ on this end takes the form:
\begin{equation}
    \bg = dt^2 + \sigma_{ab}(t,y) dy^a dy^b.
\end{equation}
The asymptotic behavior of the metric is governed by the indicial roots of the linearized Jang operator, which corresponds to the stability operator $L_\Sigma$. For $\lambda_1=0$, the decay to the exact cylinder is polynomial rather than exponential. Specifically, following the expansion of Han and Khuri \cite{hankhuri2013}, the induced metric on the cross-sections satisfies:
\begin{equation}\label{eq:MetricDecay}
    \sigma_{ab}(t,y) = \sigma_{0,ab}(y) + \frac{1}{t} \sigma_{1,ab}(y) + O(t^{-2}),
\end{equation}
where $\sigma_0$ is the metric on the horizon $\Sigma$.

\subsection{Decay of the Second Fundamental Form}
The vector field $q$ is defined by $q_i = v^j (h_{ij} - k_{ij})$, where $v = \nabla f / \sqrt{1+|\nabla f|^2}$ is the unit tangent to the graph, $h$ is the second fundamental form of the Jang graph $\bM$, and $k$ is the extension of the initial data extrinsic curvature.
On the cylindrical end, $v \approx \partial_t$. Thus, the magnitude of $q$ is controlled by the difference $|h - k|_{\bg}$.

\textbf{1. Decay of the Graph Curvature $h$:}
The second fundamental form of the coordinate slice $\Sigma_t$ within the cylinder $(\mathcal{E}_{cyl}, \bg)$ is given by $h_{ab} = -\frac{1}{2} \partial_t \sigma_{ab}$.
Differentiating the expansion \eqref{eq:MetricDecay}:
\begin{equation}
    h_{ab}(t,y) = -\frac{1}{2} \partial_t \left( \sigma_{0,ab} + \frac{1}{t} \sigma_{1,ab} \right) + O(t^{-3}) = \frac{1}{2t^2} \sigma_{1,ab} + O(t^{-3}).
\end{equation}
Crucially, the "slow" $O(t^{-1})$ decay of the metric induces a "fast" $O(t^{-2})$ decay of the extrinsic curvature. The zeroth order term vanishes because the limit metric $\sigma_0$ is independent of $t$ (a product cylinder).

\textbf{2. Behavior of Initial Data Curvature $k$:}
The initial data $k$ is a smooth tensor on the background manifold $M$. The cylindrical coordinate $t$ is related to the geodesic distance $s$ to the horizon by $s \approx e^{-t}$. Smoothness of $k$ near $\Sigma$ ($s=0$) implies a Taylor expansion:
\begin{equation}
    k(s,y) = k(0,y) + s (\partial_s k)(0,y) + O(s^2).
\end{equation}
In terms of $t$, this yields exponential convergence to the boundary value:
\begin{equation}
    k(t,y) = k_{\Sigma}(y) + O(e^{-t}).
\end{equation}
The GJE construction implies the matching condition on the MOTS boundary, ensuring the boundary terms match such that the difference $h-k$ is controlled by the graph decay. Specifically, for the vector field $q$, the relevant tangential components satisfy the bound derived from the graph geometry.

\begin{theorem}[Flux Vanishing Theorem]
In the marginally stable case, the vector field $q$ satisfies $|q|_{\bg} = O(t^{-2})$. Consequently, if $\psi = \phi-1$ decays as $t^{-m}$ with $m \ge 1$, the boundary flux of the Bray-Khuri vector field $Y$ vanishes.
\end{theorem}
\begin{proof}
Based on the decay analysis above, $|q| \sim t^{-2}$. We estimate the two terms in $Y = \frac{(\phi-1)^2}{\phi}\nabla\phi + \frac{1}{4}(\phi-1)^2 q$.
\textbf{Term 1:} $Y_1 = \frac{\psi^2}{\phi} \nabla \phi = \frac{\psi^2}{1+\psi} \nabla \psi$.
Since $\psi \sim t^{-m}$, we have $|\nabla \psi| \sim t^{-m-1}$.
\[ |Y_1| \approx |\psi|^2 |\nabla \psi| \sim (t^{-m})^2 \cdot t^{-m-1} = t^{-3m-1}. \]
For $m \ge 1$, $|Y_1| \sim t^{-4}$. The flux is $\int Y_1 \cdot \nu \approx \text{Area}(\Sigma) \cdot t^{-4} \to 0$.

\textbf{Term 2:} $Y_2 = \frac{1}{4} \psi^2 q$.
Using $|q| \sim t^{-2}$ and $|\psi| \sim t^{-m}$:
\[ |Y_2| \sim (t^{-m})^2 \cdot t^{-2} = t^{-2m-2}. \]
For $m \ge 1$, $|Y_2| \sim t^{-4}$. The flux is $\int Y_2 \cdot \nu \approx \text{Area}(\Sigma) \cdot t^{-4} \to 0$.
Thus, the boundary term in the Bray-Khuri identity vanishes identically, validating the global integral argument $\int_\Omega P = 0$.
\end{proof}

\section{Legacy Penrose Draft Overview}
\label{sec:legacy-penrose}

The content of the earlier draft file \texttt{penrose.tex} is incorporated here to keep the record of the conditional program and hypotheses in a single document.

\subsection*{Legacy Abstract}

We describe a program toward a proof of the spacetime Penrose inequality for general asymptotically flat initial data sets satisfying the dominant energy condition. Our approach combines the Jang equation, a conformal deformation to nonnegative scalar curvature, and a $p$--harmonic level-set method extending the work of Agostiniani--Mazzieri--Oronzio to a singular setting. Under a set of explicit analytic hypotheses on the Jang--Lichnerowicz operator, on the smoothing of corners with distributional scalar curvature, and on the behavior of $p$--harmonic potentials on degenerating backgrounds, we derive the spacetime Penrose inequality for a single black hole. The logical dependence on these hypotheses is made completely explicit. We emphasize that we do \emph{not} claim a complete proof of the spacetime Penrose inequality in full generality here. Several of the analytic inputs needed for such a proof are stated as hypotheses or conjectures and will be developed in subsequent work. The main contribution of the present paper is to isolate these inputs and to show how they fit into a unified geometric framework.

\subsection{Legacy Introduction}

In this legacy material we describe and partially implement a program toward a proof of the spacetime Penrose inequality for general asymptotically flat initial data sets $(M,g,k)$ satisfying the dominant energy condition and containing an outermost apparent horizon. None of the currently available analytic results on Jang deformations, conformal scalar curvature corrections, or $p$--harmonic level-set flows by itself resolves the general case. The first version of this manuscript was written under the optimistic working assumption that several analytic ingredients (Fredholm properties of a Jang--Lichnerowicz operator on asymptotically cylindrical backgrounds, quantitative control of corner smoothings with distributional scalar curvature, and Gamma-limits for $p$--harmonic functionals on degenerating manifolds) could be treated as black boxes. In light of subsequent scrutiny, and following the referee's comments, the status of these ingredients is made explicit below; they are formulated as hypotheses or conjectures, and all downstream geometric statements are correspondingly conditional.

\begin{theorem}[Legacy main theorem]\label{thm:legacy-main}
...
The proof of Theorem~\ref{thm:legacy-main} given here is \emph{conditional}: it relies on four analytic hypotheses concerning the Fredholm and maximum-principle properties of a Jang--Lichnerowicz operator on a manifold with one asymptotically flat end and several asymptotically cylindrical ends; the smoothing of internal corners while preserving a distributional scalar curvature lower bound and uniform control of the negative part; the behavior of $p$--harmonic potentials on the resulting family of smooth approximating metrics and their Gamma/Mosco limits as $\varepsilon\to 0$ and $p\to 1$; and the regularity and rigidity theory for weak static vacuum solutions arising in the equality case. These hypotheses are stated precisely below, and every theorem whose proof uses them is explicitly labeled as such. The geometric part of the argument---namely, the reduction of the spacetime Penrose inequality to these four analytic hypotheses---is unconditional. In this sense the present text should be viewed primarily as an organizing framework and a collection of conditional reductions, rather than as a complete solution of the problem.

\subsection*{Legacy Analytic Hypotheses}\label{sec:legacy-hypotheses}

For the reader's convenience we collect here the main analytic assumptions on which the legacy argument rests. Precise formulations are given in the corresponding sections, but we state them informally to highlight the logical structure.

\begin{hypothesis}[Fredholm theory for the Jang--Lichnerowicz operator]\label{hyp:legacy-Fredholm}
Let $(\bar M,\bar g)$ denote the Jang deformation with a single asymptotically flat end and a finite number of asymptotically cylindrical ends corresponding to the components of the outermost MOTS. Then for appropriate weighted Sobolev or edge spaces the operator
\[
L := \Delta_{\bar g} - \tfrac18 R^{\mathrm{reg}}_{\bar g}
\]
is Fredholm of index zero and admits a bounded inverse, with the asymptotic expansions described in the legacy Lichnerowicz discussion. Moreover, the associated solution $\phi$ of the Lichnerowicz equation satisfies $0<\phi\le 1$ on $\bar M$.
\end{hypothesis}

\begin{hypothesis}[Corner smoothing with controlled scalar curvature]\label{hyp:legacy-corner}
Let $(\tilde M,\tilde g)$ be the metric obtained from $(\bar M,\bar g)$ by sealing the cylindrical ends so that $\tilde g$ has a Lipschitz internal corner along the image of the outermost MOTS and nonnegative scalar curvature in the distributional sense. Then there exists a family of smooth metrics $g_\varepsilon$ converging uniformly to $\tilde g$ such that $R_{g_\varepsilon}\ge -C\varepsilon^{2/3}$ and the negative part $R^-_{g_\varepsilon}$ is supported in an $\varepsilon$--neighborhood of the corner and satisfies the $L^{3/2}$ bound stated in the smoothing discussion.
\end{hypothesis}

\begin{hypothesis}[$p$--harmonic potentials on degenerating backgrounds]\label{hyp:legacy-pharmonic}
Let $g_\varepsilon$ be as in Hypothesis~\ref{hyp:legacy-corner}. For each fixed $p>1$ consider the $p$--harmonic potential $u_{\varepsilon,p}$ with the boundary conditions prescribed in the $p$--harmonic section. Then, as $\varepsilon\to 0$, the energies and level-set functionals associated to $u_{\varepsilon,p}$ converge in the sense of Mosco/Gamma convergence to the corresponding objects on $(\tilde M,\tilde g)$, and in particular no spurious area can concentrate at the conical tips produced by the sealing procedure when $p\to 1$.
\end{hypothesis}

\begin{hypothesis}[Static vacuum regularity and rigidity]\label{hyp:legacy-static}
Assume that equality holds in the spacetime Penrose inequality for an initial data set $(M,g,k)$ as in Theorem~\ref{thm:legacy-main}. Then the associated Jang deformation $(\bar M,\bar g)$ carries a lapse function $N$ such that $(\bar g,N)$ is a smooth static vacuum solution in the sense of general relativity, and any such solution with a connected outermost minimal boundary and a single asymptotically flat end is isometric to a Schwarzschild slice.
\end{hypothesis}

Throughout this legacy section we make it clear when a result relies on one of these hypotheses. In particular, the reduction of the spacetime Penrose inequality to the four analytic hypotheses above is unconditional, whereas the full inequality and the rigidity statement are proved only under these assumptions.

\subsection*{Legacy Outstanding Analytic Gaps}

The referee report emphasized several places where the older manuscript only sketches arguments or appeals to results that are not yet available in the literature. In keeping with that feedback, the main open points are recorded explicitly:
\begin{itemize}
  \item \textbf{Generalized Jang analysis.} Existence and asymptotics (especially in the marginally stable case) beyond the strictly stable regime require refined decay estimates that are not yet written down in full.
  \item \textbf{Fredholm theory on marginal cylinders.} The scattering/edge analysis that would establish the Fredholm properties and invertibility of the Lichnerowicz operator in the polynomial-decay setting still needs a complete, checkable proof or an external reference.
  \item \textbf{The $\phi\le 1$ estimate.} Extending the Bray--Khuri divergence identity and the overshoot argument to Lipschitz Jang backgrounds (and justifying all boundary terms) has to be carried out in full detail.
  \item \textbf{Corner smoothing.} The Miao--type internal smoothing with quantitative $L^{3/2}$ control of the negative scalar curvature part is only sketched; establishing the precise estimates is an open task.
  \item \textbf{$p$--harmonic limits on singular spaces.} The Mosco/$\Gamma$ convergence and Bochner/Kato machinery in the presence of conical tips and only distributional curvature bounds require a more systematic treatment.
  \item \textbf{Rigidity.} A static-vacuum regularity and uniqueness theory applicable to Lipschitz metrics across the sealed Jang surface is still missing; the current rigidity statement is therefore conditional.
\end{itemize}
These gaps delineate the analytic work that remains in order to convert the present conditional framework into a complete proof.

\subsection*{Legacy Outline}

The legacy sections review the Jang deformation, discuss the conditional Fredholm and asymptotic theory for the Lichnerowicz equation, state the corner smoothing and $p$--harmonic assumptions, and conclude with a conditional rigidity statement. The formal statements appear below without claiming new results beyond the hypotheses already listed.

\begin{theorem}[Legacy Lichnerowicz existence]\label{thm:legacy-LichnerowiczExistence}
Assume Hypothesis~\ref{hyp:legacy-Fredholm}. Let $(\bar M,\bar g)$ be the Jang manifold described above. Then there exists a unique positive solution $\phi$ of the Lichnerowicz equation with the asymptotics described in the legacy discussion, and $\phi\le 1$ on $\bar M$.
\end{theorem}

\begin{remark}
The proof of Theorem~\ref{thm:legacy-LichnerowiczExistence} is conditional on Hypothesis~\ref{hyp:legacy-Fredholm}. In particular, we use the Fredholm properties and maximum principle for $L$ as black boxes and focus on the geometric and variational identities (such as the Bray--Khuri divergence identity) that lead to the bound $\phi\le 1$. A complete proof of Hypothesis~\ref{hyp:legacy-Fredholm}, including the marginally stable case, will be given elsewhere.
\end{remark}

\begin{theorem}[Legacy rigidity]\label{thm:legacy-rigidity}
Suppose $(M,g,k)$ is an asymptotically flat initial data set satisfying the hypotheses of Theorem~\ref{thm:legacy-main} and that equality holds in the spacetime Penrose inequality. Assume in addition Hypothesis~\ref{hyp:legacy-static}. Then $(M,g,k)$ arises as a slice of a Schwarzschild spacetime.
\end{theorem}

\begin{remark}
Without Hypothesis~\ref{hyp:legacy-static} one still obtains that the Jang deformation $(\bar M,\bar g)$ has nonnegative scalar curvature and that all the error quantities appearing in the divergence identities vanish. It is expected that a suitable regularity and uniqueness theory for such ``weak'' static vacuum solutions would imply Hypothesis~\ref{hyp:legacy-static}, but that theory lies beyond the scope of this legacy material.
\end{remark}

\section{Legacy Rigorous Notes}
\label{sec:legacy-rigorous-notes}

The contents of \texttt{rigor-additions.tex} are reproduced here as conditional notes outlining the analytic work required to fully justify the legacy arguments. Labels have been prefixed to avoid conflicts with the main text.

\begin{lemma}[Well-posedness of the singular Lichnerowicz equation]\label{lem:legacy-LichnerowiczWellPosed}
Let $(\overline M,\overline g)$ be the Jang deformation constructed in the main text. Fix $p>3$ and weights
\[
  \delta \in (-1,0), \qquad \gamma \in (0,1),
\]
which are used along the asymptotically flat end and the cylindrical ends of $\overline M$, respectively. Let
\[
  L := \Delta_{\overline g} - \tfrac18 \mathcal S,
\]
where $\mathcal S$ is the nonnegative part of the Jang scalar curvature. Then:
\begin{enumerate}[(i)]
  \item The operator
  \[
...
\end{enumerate}
In particular, choosing $f$ to be the right-hand side of the Bray--Khuri equation, we obtain a unique solution $\phi$ of the singular Lichnerowicz equation with
\[
  \phi-1 \in W^{2,p}_{\delta,\gamma}(\overline M),
\]
and the asymptotics prescribed in the legacy Lichnerowicz discussion.
\end{lemma}

\begin{remark}[Status of the analytic inputs]
The Fredholm argument sketched above requires refined asymptotics for the Jang data $q$ in the marginally stable case and a detailed scattering/edge analysis for $L$ on polynomially converging cylinders. Both are part of the open analytical work highlighted in the main text and would need to be supplied elsewhere before the lemma could be accepted as a complete proof.
\end{remark}

\begin{lemma}[$L^{3/2}$ control of scalar curvature deficit]\label{lem:legacy-ScalarDip}
Let $\hat{g}_\epsilon$ be the smoothed metric in the collar $N_{2\epsilon}$ constructed via convolution. The negative part of the scalar curvature, $R^-_\epsilon = \min(0, R_{\hat{g}_\epsilon})$, satisfies the estimate
\begin{equation}
    \|R^-_\epsilon\|_{L^{3/2}(N_{2\epsilon})} \le C \epsilon^{2/3},
\end{equation}
...
\begin{lemma}[Vanishing of the Jang flux]\label{lem:legacy-FluxVanishing}
Let $(\overline M,\overline g)$ be the Jang deformation of an initial data set satisfying the standing hypotheses, and let $\mathcal C\simeq[0,\infty)\times\Sigma$ be a cylindrical end corresponding to a component $\Sigma$ of the outermost MOTS, with coordinate $t\ge 0$ and cross-sections $\Sigma_t=\{t\}\times\Sigma$. Let $q$ be the Jang vector field and $\nu$ the unit normal to $\Sigma_t$ in $\overline g$ pointing towards increasing $t$. Then
\[
...
\begin{remark}[Area semicontinuity and Jensen's inequality]\label{rem:legacy-AreaJensen}
In the proof of area semicontinuity, the primary mechanism preventing an area drop in the limit $\epsilon\to 0$ is the convexity of the determinant of the metric tensor under averaging. More precisely, if $\gamma_\epsilon=\eta_\epsilon * g$ denotes the convolution smoothing of the metric in local coordinates, then Jensen's inequality applied to the positive definite matrix field $g$ yields
\[
  \sqrt{\det \gamma_\epsilon} \ge \sqrt{\det g} - C\epsilon^2
\]
on the relevant collar of $\Sigma$, for some constant $C$ independent of $\epsilon$. This pointwise estimate implies that the area of any fixed surface cannot decrease by more than $O(\epsilon^2)$ under the smoothing, which is the key input in the lower semicontinuity of the minimal areas appearing in the smoothing argument.
\end{remark}

\begin{theorem}[Bray--Khuri divergence identity]\label{thm:legacy-BKidentity}
Let $(\overline M,\overline g,h,q)$ be the Jang deformation of an initial data set $(M,g,k)$ obeying the dominant energy condition, and let $\phi>0$ be the solution of the singular Lichnerowicz equation. Define
\[
...
\]
where $|J|_g$ denotes the norm of $J$ with respect to $g$ transported to $\overline M$. Then $P\ge 0$ almost everywhere and, in the sense of distributions on $\overline M$,
\begin{equation}
  \div_{\overline g} Y
...
\begin{lemma}[Static vacuum structure in the equality case]\label{lem:legacy-StaticVacuum}
Assume the standing hypotheses and suppose that equality holds in the spacetime Penrose inequality. Then there exists a positive function $N$ on $\overline M$ such that the static vacuum equations
\[
...
\begin{lemma}[Regularity and classification]\label{lem:legacy-AndersonRegularity}
Under the hypotheses of Lemma~\ref{lem:legacy-StaticVacuum}, the pair $(\overline g,N)$ is smooth and real analytic on the interior of $\overline M$. In particular, $(\overline M,\overline g)$ is isometric to a spatial Schwarzschild slice outside its horizon.
\end{lemma}

\begin{lemma}[Integrability of Ricci curvature at conical tips]\label{lem:legacy-RicciIntegrability}
Near the compactified bubble singularities $\{p_k\}$, the Ricci curvature of the conformally deformed metric $\tilde{g}$ belongs to $L^1_{\loc}$.
\end{lemma}

\begin{lemma}[Non-vanishing gradient near singularities]\label{lem:legacy-GradientNearTip}
Let $p_k$ be a conical singularity. There exists a neighborhood $U$ of $p_k$ such that $\nabla u \neq 0$ in $U \setminus \{p_k\}$.
\end{lemma}
