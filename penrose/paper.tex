...
\textbf{Case 2: Marginal Stability ($[H] = 0$) - Second Variation of Area.}
Here $H_\epsilon \approx 0$. The "No-Slip" barrier is absent. We must rule out the formation of a "deep well" where a tiny minimal surface could form. We invoke the Second Variation of Area.
Since $\Sigma$ is the outermost MOTS in $(\tM, \tg)$, it is stable. The second variation of area is non-negative:
\[ \delta^2 A [v] = \int_\Sigma (|\nabla v|^2 + Q v^2) d\sigma \ge 0. \]
This implies that $\Sigma$ is a local minimizer of area in the original metric.
Consider a competitor minimal surface $\Sigma_\epsilon$ in the smoothed metric $\geps$. The smoothed metric is $\geps = u_\epsilon^4 \hat{g}_\epsilon$. By Lemma \ref{lem:GreenEstimate}, the conformal factor satisfies $u_\epsilon \ge 1$. Thus, area in $\geps$ dominates area in $\hat{g}_\epsilon$:
\[ A_{\geps}(\Sigma_\epsilon) \ge A_{\hat{g}_\epsilon}(\Sigma_\epsilon). \]
The metric $\hat{g}_\epsilon$ is an arithmetic smoothing of $\tg$. By Jensen's inequality (Lemma \ref{lem:AreaExpansion}), the area density increases under smoothing (up to $O(\epsilon^2)$ error):
\[ A_{\hat{g}_\epsilon}(\Sigma_\epsilon) \ge A_{\tg}(\Sigma_\epsilon) - C\epsilon^2. \]
Combining these, $A_{\geps}(\Sigma_\epsilon) \ge A_{\tg}(\Sigma_\epsilon) - C\epsilon^2$.
Since $\Sigma$ is a local minimizer in $\tg$ (due to stability), for any surface $\Sigma_\epsilon$ sufficiently close to $\Sigma$ (which it must be, as the perturbation is small), we have $A_{\tg}(\Sigma_\epsilon) \ge A_{\tg}(\Sigma)$.
Therefore,
\[ A_{\geps}(\Sigma_{\min, \epsilon}) \ge A_{\tg}(\Sigma) - C\epsilon^2. \]
Taking the liminf, we obtain the result. The stability ensures no unstable modes exist that would allow the area to "jump" down.
\end{proof}
