...
\end{proposition}
    ...
    We provide the detailed verification of this flux cancellation in \Cref{app:Flux}, citing the refined decay analysis of Han-Khuri \cite{hankhuri2013}.
    In the marginal case, while $f$ blows up, the geometry of the cylinder enforces that the vector field $q$ aligns with the kernel of the stability operator. The orthogonality condition $\int \Div(q) = 0$ is satisfied because the source term $\Div(q)$ arises from the difference of the second fundamental forms, which decays as $t^{-2}$ due to the constraint equations.
...
In the marginal case, the "No-Slip" barrier from the mean curvature jump is absent. However, we rely on the property that $\Sigma$ is the \emph{outermost} horizon.
...
Within the smoothing collar, the second variation argument holds:
\[ A_{\geps}(\Sigma_\epsilon) \ge A_{\tg}(\Sigma) - C\epsilon^2. \]
...
Since $(\bar{M}, \bar{g}, N)$ satisfies the static vacuum equations distributionally with $N, \bar{g} \in H^1_{loc}$, standard elliptic bootstrapping for the Einstein equations (which are quasi-linear elliptic in harmonic gauge) implies the metric is smooth (real analytic).
...
\end{equation}
even in the slow decay case $k=1$, utilizing the refined decay of the trace of $k$ and $h$ as established in Han-Khuri \cite{hankhuri2013}. Consequently, the source term is in the range of the Laplacian on the cylindrical end.
\end{lemma}
\begin{proof}
