...
\textbf{Assumption (Yamabe Positivity):} We assume that the constructed Jang metric $(\bM, \bg)$ belongs to the positive Yamabe class (or zero class). This is a standard requirement for the conformal method to work; it is guaranteed, for example, if the initial data admits a maximal slice or if the amount of negative scalar curvature in the Jang reduction is controlled. Without this spectral condition, the Lichnerowicz operator would possess negative eigenvalues, and no positive conformal factor would exist.
