...
\end{lemma}
\begin{proof}
The Sobolev constant depends on the isoperimetric constant $I(\hat{g}_\epsilon)$. We must verify that the "throat" of the manifold does not pinch off as $\epsilon \to 0$.
The metric $\hat{g}_\epsilon$ is constructed by a tangential mollification of $\tg$ in the collar $N_{2\epsilon}$. The cross-sectional areas $A_s = \text{Area}_{\gamma_\epsilon}(\Sigma_s)$ approximate the original areas.
Since $\tg$ corresponds to the Jang metric which opens into a cylindrical end (or has a minimal boundary $\Sigma$ of area $A(\Sigma)$), the area of the interface is $A(\Sigma) > 0$.
The convolution $\gamma_\epsilon = \eta_\epsilon * g_s$ preserves the non-degeneracy of the metric tensor. Specifically, for small $\epsilon$, the area of any level set $\Sigma_s$ in the collar is bounded below by $A(\Sigma)/2$.
Since the manifold is asymptotically flat and the geometry in the bulk is fixed, the only region of potential degeneration is the collar. The area lower bound ensures the isoperimetric constant $I(\hat{g}_\epsilon)$ remains strictly positive, uniformly in $\epsilon$.
\end{proof}
