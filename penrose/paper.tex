...
\begin{enumerate}
    ...
    \item \textbf{Slow Decay ($k=1$):} If the decay is only $t^{-1}$, we must verify the solvability condition carefully. We invoke the \textbf{Weighted Scattering Calculus} to handle the polynomially decaying perturbation $E = L - L_\infty$.
\end{enumerate}
...
\begin{itemize}
    ...
    With this weight, the operator is Fredholm. In this marginal case, the metric decay is polynomial ($O(t^{-k})$). We invoke the \textbf{Weighted Scattering Calculus} to handle the polynomially decaying perturbation $E = L - L_\infty$.

    A crucial point is the solvability condition: since the kernel of the limiting operator contains constants (corresponding to $\lambda=0$), the source term $\Div_{\bg}(q)$ must be orthogonal to this kernel to ensure a decaying solution.
...
\begin{itemize}
    \item \textbf{Bubble Ends:} Here $\phi \to 0$. Thus $\Omega = \{\phi > 1\}$ is bounded away from the bubble ends. No boundary term arises.
        ...
        \[ |Y| \approx |\psi|^2 |\nabla \psi| + \frac{1}{4}|\psi|^2 |q| \le C( t^{-2k} t^{-k-1} + t^{-2k}t^{-2}) = O(t^{-3k-1}). \]
