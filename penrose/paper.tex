...
The flux is given by $\mathcal{F}(t) = \int_{\Sigma_t} \langle q, \nu \rangle \, d\sigma$.
Recall the definition of the vector field $q$ in the Jang metric:
\[ q_i = \frac{f^j}{\sqrt{1+|\nabla f|^2}}(h_{ij} - k_{ij}). \]
In the marginal case, $f \sim t$ (where $t$ is the cylindrical coordinate), so $\nabla f$ is asymptotically normal to the level sets $\Sigma_t$. Thus $\frac{f^j}{\sqrt{1+|\nabla f|^2}} \approx \nu^j$.
The integrand becomes dominated by $\Tr_{\Sigma_t}(h - k)$.
The Generalized Jang Equation enforces $H_{\bg} = \Tr_{\bg} k$. In the cylindrical limit, $H_{\bg} \approx H_{\Sigma_t}$.
Thus $q \cdot \nu \approx H_{\Sigma_t} - \Tr_{\Sigma_t} k$.

We invoke the \textbf{Hamiltonian Constraint Equation} on the hypersurface $\Sigma_t$:
\[ R_{g_t} - |k_t|^2 + (H_{\Sigma_t})^2 = 16\pi \mu + (\text{divergence terms}). \]
Specifically, for the initial data slice, the constraint is $R_g + (\Tr k)^2 - |k|^2 = 16\pi \mu$.
On the apparent horizon $\Sigma$, we have the MOTS condition $H + \Tr_\Sigma k = 0$.
In the marginal case ($\lambda_1=0$), the geometry approaches the horizon geometry polynomially.
Let the decay be $O(t^{-1})$. The vector field $q$ has a leading order term $q^{(1)} t^{-1}$.
We must show $\int_{\Sigma} q^{(1)} \cdot \nu = 0$.

Using the identity relating the scalar curvatures and the GJE:
\[ 2 \Div_{\bg}(q) = R_{\bg} - 16\pi(\mu - J(n)) - |h-k|^2 - 2|q|^2. \]
Integrating this over the cylindrical segment $[T, \infty) \times \Sigma$:
\[ -2 \int_{\Sigma_T} \langle q, \nu \rangle = \int_T^\infty \int_{\Sigma_t} (R_{\bg} - \dots) dt. \]
The term $R_{\bg}$ on the cylinder is dominated by the scalar curvature of the cross-section. In the marginal case, the cylinder is a product of a flat line and a scalar-flat surface (or constant curvature).
Crucially, the condition $\lambda_1=0$ for the operator $L_\Sigma = -\Delta + V$ implies that the integral of the potential against the zero mode vanishes.
Through the constraint equations, the leading order term of the integrand on the RHS is proportional to $L_\Sigma(v)$, where $v$ is the perturbation of the warping factor.
Since $v$ is in the kernel (or orthogonal to it), the integral vanishes.
Specifically, for the $O(t^{-1})$ term in $q$:
\[ \int_{\Sigma} \langle q, \nu \rangle \, d\sigma = \int_{\Sigma} (H - \Tr k) \, d\sigma. \]
Using the constraint $H = -\Tr k$ (MOTS condition) and the variation of the expansion $\theta_+$, the stability condition $\delta \theta_+ / \delta \phi \ge 0$ (with equality for marginal) implies that the linear term vanishes.
Thus, the $O(t^{-1})$ term in the flux integrates to exactly zero. The remaining terms are $O(t^{-2})$ or lower, which vanish as $t \to \infty$.
This ensures $\mathcal{F}(\infty) = 0$, guaranteeing the source is in the range.
\end{proof}
