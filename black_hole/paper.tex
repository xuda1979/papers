% Document class reverted to article; use jheppub package instead of class to avoid missing CLS error
...
% Using jheppub as a package (class form unavailable locally)
...
% Inline dataset loader toggle (set \hmcloadexternaldatatrue to prefer external .dat files)
...
% Dataset load is deferred until \AtBeginDocument (see below).
...
% MiKTeX/pgfplotstable can error if \pgfplotstableread runs in the preamble.
% Defer all dataset reads until after \begin{document}.
\AtBeginDocument{%
  ...
  \hmcloaddataset{datatablePTMPO.dat}{\datatablePTMPO}{\hmcinlinePTMPO}%
  \hmcloaddataset{datatablePTMPOscaling.dat}{\datatablePTMPOscaling}{\hmcinlinePTMPOscaling}%
  \hmcloaddataset{datatablePTMPOerror.dat}{\datatablePTMPOerror}{\hmcinlinePTMPOerror}%
...
\abstract{\textbf{Problem.} We address how a semiclassical exterior can admit unitary information flow without violating equivalence-principle physics at the horizon.

\textbf{Approach.} We formalize \emph{horizon memory combs} (\HMC{}s): finite-memory, non-Markovian quantum processes that map near-horizon degrees of freedom to asymptotic radiation. The formalism leverages process tensors and their efficient matrix-product-operator (MPO) representations, parameterized by a memory depth $\ell_{\rm mem}$ and memory time $\tau_{\rm mem}$.

\textbf{Results.} (i) Under four stated axioms (A1--A4)---semiclassical exterior with Hadamard/QEI control, local near-horizon mixing, scrambling of two-point data into OTOCs, and asymptotic purity---we prove decoupling bounds and show that Einstein--Hilbert dynamics imply an \emph{approximate unitary $2$--design} to depth $\ell_{\rm mem}$ on timescales $\tau_{\rm mem}$, with explicit error terms. (ii) We develop a process-tensor--MPO (PT--MPO) algorithm that simulates \HMC{}s with polynomial cost in $\ell_{\rm mem}$ and validate it on moving-mirror and Schwarzian toy models. (iii) We extract falsifiable signatures---Page-curve shapes, late-time entanglement growth, and echo-correlation patterns---relevant to analogue-gravity platforms and gravitational-wave ringdown data.

\textbf{Limitations.} All results are conditioned on Axioms~A1--A4 (semiclassical exterior, near-horizon mixing, scrambling, and asymptotic purity) and on the finite-memory hypothesis; regimes violating these assumptions (e.g.\ strong back-reaction or late-time Planckian physics) fall outside our formal guarantees.

\textbf{Significance.} \HMC{}s provide an operational Page-curve formulation without firewall pathologies: information flow is delayed yet finite-memory, reconciling unitary evaporation with a semiclassical exterior. We outline limitations (astrophysical systematics, strong back-reaction, Planckian end-stage) and provide code/data checksums for full reproducibility.}
...
\begin{enumerate}[leftmargin=*,itemsep=0.3em]
  \item \textbf{Finite-memory formalism.} We formalize black hole evaporation as a multi-time quantum \emph{comb} with a bounded memory register, parameterized by $(\ellmem,\taumem)$, and connect it to open-system non-Markovianity via the process-tensor framework.
  \item \textbf{Design/decoupling theorems under hypotheses.} Given four explicit working hypotheses (P0--P3), we prove decoupling bounds and an \emph{approximate unitary 2-design} statement for the relevant horizon maps, with transparent dependence on $(\ellmem,\taumem)$ and controllable errors.
  \item \textbf{Algorithms and validation.} We develop a PT--MPO pipeline with complexity scaling primarily in the memory depth; we provide convergence checks, ablations, and a fully deterministic artifact to regenerate all figures and tables.\footnote{Reproducibility details and checksums are in \Cref{app:code} and \Cref{app:data}.}
  \item \textbf{Falsifiable predictions.} We extract instrument-facing signatures---Page-curve features, $g^{(2)}$ modulations, and gravitational-wave echo correlations---that bound $(\ellmem,\taumem)$ in principle with current or near-future experiments.
  \item \textbf{Positioning and limits.} We clarify relations to islands/replica, firewall/fuzzball, and moving-mirror analogues, and we delineate the scope where the \HMC{} idealizations may fail (astrophysical systematics; strong back-reaction).
\end{enumerate}

\begin{boxedresult}{Assumptions (A1--A4)}\label{box:assumptions}
\begin{enumerate}[leftmargin=*,itemsep=0.25em]
  \item \textbf{A1 (Semiclassical exterior \& Hadamard + QEIs).} Outside a stretched horizon, the state is Hadamard with finite renormalized stress tensor and obeys standard quantum energy inequalities on scales $\gg \ell_{\rm p}$. Our arguments exclude the final $O(1)$ Planckian fraction of evaporation.
  \item \textbf{A2 (Locality / near-horizon mixing).} The exterior dynamics obey a locality/Lieb--Robinson--type constraint and exhibit local mixing over a coarse-grained window, quantified by a mixing time $t_{\rm mix}$ and memory depth $\ell_{\rm mem}$.
  \item \textbf{A3 (Fast scrambling / 2-point-to-OTOC growth).} Coarse-grained Einstein--Hilbert evolution generates scrambling characterized by OTOC growth and equilibration to depth $\ell_{\rm mem}$; see \Cref{thm:EH-2design}.
...
\(\ellmem\), \(\taumem\) & Memory depth (number of steps); memory correlation time. \\
...
Units & \(G=\hbar=c=k_B=1\) unless stated otherwise. \\
...
\begin{itemize}
    ...
    A1--A4 (Semiclassical regime, operator growth (A2), thermal mixing (A3)) + holographic conjectures C1--C3 (\Cref{sec:EH-to-design}) $\Longrightarrow$ P2$'$ (Weak Scrambling). If additional spectral/mixing gaps hold $\Rightarrow$ P2 (Strong Scrambling).
\end{itemize}
\end{minipage}}
\end{center}
...
\textbf{Nomenclature Note:} To avoid confusion with the fundamental Axioms (A1-A4) in \Cref{sec:assumptions}, we label the following standard holographic conjectures as C1-C3.
\begin{itemize}[leftmargin=*]
...
In the explicit models treated in \Cref{sec:derive_qg} (moving mirror; JT/Schwarzian), the vanishing term is polynomial in $\ell/\ell_{\rm mem}$,
and is empirically near-exponential (\Cref{subsec:adversarial}).
...
Both behaviors appear in our ablations: see \Cref{subsec:adversarial,subsec:ablation} and the data tables
\texttt{datatableAblation.dat}, \texttt{datatablePTMPOscaling.dat}, and \texttt{datatableGtwo.dat} documented in \Cref{app:data}.
For convenience, the helper scripts \texttt{extract_inline_datasets.py} and \texttt{code/generate_reports.py} regenerate the packaged datasets and diagnostic figures;
...
Assume (A1)--(A4) and the EFT hypotheses of \Cref{sec:eft-bounds}. Let $a_\star\equiv a/M$ and surface gravity $\kappa$.
...
late-time echoes, multi-channel cross-correlations). We regularize with stability priors (\Cref{sec:gentleness}).
...
\end{align}
implying a detection threshold on $\mathrm{SNR}$ for current ground-based detectors (assuming sufficient signal amplitude; see \Cref{subsec:gw_ringdowns}).
...
\end{enumerate}
For gravitational-wave ringdowns, $g^{(2)}$ is replaced by \emph{causal sidebands in the phase-locked spectrum}; for tabletop analogue systems (BEC, water-wave, or fiber-loop black holes), direct photon/phonon coincidence counting is possible.
...
\label{subsec:kernel_tomography}
...
\begin{itemize}[leftmargin=*]
\item \textbf{Coarse-graining and stationarity.} We assume piecewise-adiabatic evaporation within fixed windows; highly dynamical mergers or strong accretion require extensions.
\item \textbf{Energy conditions.} The design argument uses averaged energy inequalities at the horizon; exotic violations may alter bounds (\cref{app:stress_tensor}).
\item \textbf{Modeling choices.} Finite-memory truncations and PT-MPO compressions introduce controlled biases certified in \cref{sec:numerical_validation}.
\item \textbf{Astrophysical systematics.} Echo/sideband searches face nontrivial backgrounds; our protocol includes stringent controls (App.~\ref{app:gw-sim}) but targeted pipelines are needed.
\end{itemize}
...
\begin{itemize}[leftmargin=*,itemsep=0.25em]
  \item \textbf{Model misspecification.} If the effective memory depth $\ell_{\rm mem}$ grows with system size/time, finite-memory claims may fail; our bounds should then be read as finite-time approximations.
  \item \textbf{Strong back-reaction.} Near the end of evaporation, back-reaction and UV completion may spoil A1 and A3; quantitative statements are not claimed in this regime.
...
\end{enumerate}
Because analogue platforms can realize $S_{\rm BH}^{\rm eff}\sim 10^3\text{--}10^6$, the $O(1/S)$ sidebands are experimentally accessible, providing a practical pathway for kernel tomography constrained by complete positivity (CP)/KMS/causality.
...
python3 simulation.py --s-initial 12 --steps 12 --num-runs 100 \
...
\midrule
datatableAblation.dat & \texttt{9fa7055e3a566dbd00ec0ab4efba56e7e3de8d64604ac304eb111ba16f7e2ebc} \\
datatableAblationSig.dat & \texttt{2af783261bdbed64102dd3f0143cfbc842fc024a32ab9aeb47136012f1029d87} \\
datatableCVsummary.dat & \texttt{5cda6d08e4aab1a8bdc012f8c96c4c5d85eb428ce47071fe6b4e59f36af8637c} \\
datatableExactComb.dat & \texttt{a71d0004e1621b6173a189706171a691f81e9ee997f8e56a20e418c785d9648f} \\
datatableGtwo.dat & \texttt{5a97d052c182b1a9a583c527e291ce3cbd52261e00fe6a916684cf4028d5a19e} \\
datatablePTMPO.dat & \texttt{70103ec26265f0dea58c900a9f7042a79a81fab45ce50c2c81b0de9b54ab9729} \\
datatablePTMPOerror.dat & \texttt{2d2f18556ca8b7454f7fb787d6000808a745e32bf89b83bce6e38c5ba63f278b} \\
datatablePTMPOscaling.dat & \texttt{222574e45a4a4721f30f43e226c31ac9c0824871a5484a55c9055be6a1612537} \\
datatablePagecurve.dat & \texttt{19f61e03c220ca4dba3e2e5ee3b1a5defc470ef2fc76bea945dfa6a7c4765007} \\
datatableQEC.dat & \texttt{e110c888caf200292e6b7ae2ee31e91a741fcfd03554c1137fa651bc29dada39} \\
\bottomrule
\end{tabular}
\end{table}
...
\scriptsize
\begin{tabular}{@{}lp{42em}l@{}}
\toprule
File & SHA256 & Size \\
\midrule
datatableAblation.dat & \texttt{9fa7055e3a566dbd00ec0ab4efba56e7e3de8d64604ac304eb111ba16f7e2ebc} & 0.3 \\
datatableAblationSig.dat & \texttt{2af783261bdbed64102dd3f0143cfbc842fc024a32ab9aeb47136012f1029d87} & 0.5 \\
datatableCVsummary.dat & \texttt{5cda6d08e4aab1a8bdc012f8c96c4c5d85eb428ce47071fe6b4e59f36af8637c} & 0.1 \\
datatableExactComb.dat & \texttt{a71d0004e1621b6173a189706171a691f81e9ee997f8e56a20e418c785d9648f} & 0.3 \\
datatableGtwo.dat & \texttt{5a97d052c182b1a9a583c527e291ce3cbd52261e00fe6a916684cf4028d5a19e} & 0.5 \\
datatablePTMPO.dat & \texttt{70103ec26265f0dea58c900a9f7042a79a81fab45ce50c2c81b0de9b54ab9729} & 0.7 \\
datatablePTMPOerror.dat & \texttt{2d2f18556ca8b7454f7fb787d6000808a745e32bf89b83bce6e38c5ba63f278b} & 0.1 \\
datatablePTMPOscaling.dat & \texttt{222574e45a4a4721f30f43e226c31ac9c0824871a5484a55c9055be6a1612537} & 0.1 \\
datatablePagecurve.dat & \texttt{19f61e03c220ca4dba3e2e5ee3b1a5defc470ef2fc76bea945dfa6a7c4765007} & 0.6 \\
datatableQEC.dat & \texttt{e110c888caf200292e6b7ae2ee31e91a741fcfd03554c1137fa651bc29dada39} & 0.2 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{proposition}[OTOC decay $\Rightarrow$ local $2$--design with constants]\label{prop:otoc-design}
