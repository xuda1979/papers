...
\abstract{We model black--hole evaporation as a non‑Markovian quantum
channel with finite ``horizon memory'' in the sense of a process tensor (quantum comb).
This framework is built upon a minimal set of physical assumptions, most notably that the horizon hosts a memory register whose capacity tracks the Bekenstein-Hawking entropy. From these assumptions, we derive several key consequences: (i) a \emph{Comb Page Theorem} that guarantees the unitarity of evaporation, reproducing the Page curve; (ii) a \emph{No-Firewall Lemma}, which demonstrates that the horizon remains smooth for infalling observers due to the gentle, $O(1/S_{\rm BH})$-suppressed nature of the memory effects; and (iii) a set of concrete, falsifiable predictions for late-time correlations in the Hawking radiation.

We sharpen the physical basis of the model by providing a more rigorous UV anchoring for the memory kernel, deriving its properties from a spectral representation consistent with causality and thermal equilibrium. Further, we move the key assumption of `scrambling' dynamics from a postulate to a derived property of a concrete, albeit phenomenological, near-horizon Hamiltonian. Our theoretical results are corroborated by scalable process-tensor MPO simulations. Finally, we make the model's predictions operational by outlining concrete data analysis pipelines for analogue gravity experiments and for searching for `soft echoes' in gravitational-wave ringdown signals. This work provides a self-consistent, testable, and dynamical resolution to the information paradox.}
