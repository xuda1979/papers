...
The proof that Forrelation is hard for PH (and specifically for $AC^0$) relies on the Fourier spectrum of functions computable by constant-depth circuits.

\textbf{Linial-Mansour-Nisan (LMN) Theorem:}
A key tool is the result by Linial, Mansour, and Nisan \citep{linial1993constant}, which states that the Fourier spectrum of any function $h \in AC^0$ (constant depth, polynomial size) is concentrated on low-degree coefficients.
Specifically, for any $\epsilon > 0$, there exists a degree $d = O(\log^{O(1)}(n/\epsilon))$ such that:
\[ \sum_{S \subseteq [n], |S| > d} \hat{h}(S)^2 \le \epsilon \]
This implies that $AC^0$ functions cannot capture "high-frequency" structures.

\textbf{Raz-Tal Separation:}
Raz and Tal generalized this intuition to the query complexity setting using the method of random restrictions. They showed that for the Forrelation problem, the target distribution looks like random noise to any low-depth circuit because the correlation $\Phi(f, g)$ involves a summation over *all* Fourier coefficients, whereas the circuit can only "see" the low-degree part of the spectrum.
Since $PH$ relative to an oracle can be approximated by such low-degree polynomials, $PH$ cannot distinguish the Forrelation cases, establishing $BQP^{\mathcal{O}} \not\subseteq PH^{\mathcal{O}}$.
...
2.  \textbf{Worst-to-Average Case Reduction}: The core argument is that if one could efficiently sample from $\mathcal{D}_C$ with small total variation distance error, then one could estimate output probabilities $P(x)$ for \textit{most} random circuits.
    ...
    \subsubsection{Stockmeyer Counting}
    A critical component of this reduction is \textbf{Stockmeyer's Approximate Counting Algorithm} \citep{stockmeyer1983complexity}.
    Suppose there exists a classical algorithm $S$ that can sample from a distribution $\mathcal{D}'$ such that $\|\mathcal{D}_C - \mathcal{D}'\|_{TV} \le \epsilon$.
    Can we use $S$ to compute the probability $P(x) = |\bra{x}C\ket{0}|^2$?

    Directly, no. However, in the complexity class $BPP^{NP}$, we can approximate the size of sets. Stockmeyer's algorithm uses a hierarchy of hash functions to "bucket" the outputs of $S$. By querying an $NP$ oracle to check if a bucket is empty, one can estimate the probability mass of specific outcomes.

    If such a sampler $S$ exists, then for a typical random circuit $C$, we could estimate $P(0)$ in $BPP^{NP}$.
    But we know that computing $P(0)$ is $\#P$-hard (in the worst case).
    Therefore, if we assume:
    \begin{itemize}
        \item \textbf{Worst-to-Average Hardness}: Computing $P(0)$ is $\#P$-hard even on average (over the choice of random circuits).
    \end{itemize}
    Then the existence of an efficient classical sampler $S$ implies:
    \[ P^{\#P} \subseteq BPP^{NP} \]
    By Toda's Theorem, $PH \subseteq P^{\#P}$. Thus, this inclusion would imply that the Polynomial Hierarchy collapses to the third level ($PH \subseteq \Sigma_3^P$), which is considered highly unlikely.
...
To verify the statistical properties required for the hardness arguments, we simulated a Random Quantum Circuit acting on $N=12$ qubits.
Unlike previous simplified models using global Haar random unitaries, this simulation explicitly implements a local circuit architecture similar to those used in supremacy experiments:
\begin{itemize}
    \item \textbf{Depth $D=15$}: Alternating layers of single-qubit gates and two-qubit entangling gates.
    \item \textbf{Single-Qubit Gates}: Random unitaries drawn from the Haar measure on $SU(2)$.
    \item \textbf{Two-Qubit Gates}: $CZ$ gates applied to adjacent qubits, alternating between even pairs $(2i, 2i+1)$ and odd pairs $(2i+1, 2i+2)$.
\end{itemize}
...
\begin{figure}[h]
    ...
    \caption{Distribution of output probabilities for a 12-qubit Random Circuit (Depth 15). The blue histogram represents the simulated probabilities scaled by the Hilbert space dimension $N=2^{12}$. The red curve is the theoretical Porter-Thomas prediction $P(x) = e^{-x}$. The close agreement confirms that the local random circuit efficiently scrambles the state to explore the Hilbert space ergodically, satisfying the Anti-Concentration condition.}
