...
\]
equivalently $Z_n(1)=\cG N_n\log N_n\,(1+o(1))$. If $G$ and $G'$ are quasi-isometric graphs both satisfying the hypotheses with the \emph{same} on-diagonal constant $\cG$, then the leading constant transfers.
\end{theorem}
...
\begin{corollary}[Mesoscopic return trace]\label{cor:heat_trace}
Under the assumptions of \Cref{thm:main}, for any integer time $t=t_n$ with $1\ll t_n \ll R_n^2$,
\[
  \sum_{k\ge1} (1-\lambda_k(G_n))^{t}
  \;=\; \sum_{x\in G_n} p_t^{G_n}(x,x)
  \;=\; \frac{\cG\,N_n}{t} + O\Big(N_n\,t^{-1-\delta} + \frac{R_n}{\sqrt{t}}\Big).
\]
In particular, choosing $t=R_n^{2\theta}$ with $0<\theta<1$ yields an error term
$O\big(N_n\,R_n^{-2\theta(1+\delta)} + R_n^{1-\theta}\big)$.
\end{corollary}
...
\]
Multiplying by $N_n$ yields the stated upper-bound error. The tail $t>R_n^2$ contributes $O(N_n)$ by IU; without a summable rate this is still $O(N_n)$ but the earlier part acquires the $\log\log$ factor when $\alpha=1$.\end{proof}

\begin{remark}[Log--log loss without IU / quantitative rate]\label{rem:loglog}
If one forgoes either (i) Intrinsic Ultracontractivity (replacing it only by the crude bound $p_t^{G_n}(x,x)\le e^{-\lambda_1 t}$) \emph{or} (ii) the summable $t^{-1-\delta}$ error in (QH) (replacing it by $O(t^{-1})$), then an unavoidable extra $\log\log N_n$ term can appear in the aggregated contribution of large times or fluctuation sums, inflating the natural upper bound to $O(N_n \log\log N_n)$. This highlights the strength and necessity of both inputs when one seeks sharp remainder control.
\end{remark}
...
\end{enumerate}
In periodic settings (including the explicit periodic walks in \Cref{app:alt_pi}), the annulus-volume bound (AV) is standard; in more irregular media it must be verified (or assumed) separately.
\end{example}
...
VD+PI already give PHI and an invariance principle. To obtain quantitative control beyond the leading-order law (e.g. explicit remainder \emph{upper bounds}), it is useful to assume \emph{uniform} summable on-diagonal decay $t p_t(x,x)=\cG+O(t^{-\delta})$, ensuring $\sum_t (t p_t(x,x)-\cG)$ converges. This holds for periodic graphs and in stochastic homogenisation frameworks (uniformly elliptic RCM, supercritical percolation: \cite{Biskup11,AndresDeuschelSlowik19,Barlow04,ArmstrongDario18,GloriaOtto17,CroydonHambly21}). Without summable decay one typically sees a $\log\log$ loss in the corresponding upper bounds (\Cref{prop:weaker_remainder,rem:loglog}).
\smallskip
...
While these limits yield theoretically beautiful combinations of constants, the $Z_n(1) \asymp N_n \log N_n$ asymptotic implies that sub-leading corrections decay logarithmically. Empirically, the convergence to $\pi$ in these identities is exceptionally slow: the trace ratio typically approaches $\pi$ as $O(1/\log R)$. Therefore, computational evaluations on finite grids (e.g., computing traces of large block-tridiagonal matrices via standard numerical software) require astronomically large $R$ to obtain even a few decimal digits of $\pi$.
\end{remark}
