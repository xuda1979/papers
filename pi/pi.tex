...
Weyl's law and the Minakshisundaram--Pleijel heat trace expansion reveal deep connections between the geometry of a Riemannian manifold---its volume, curvature---and eigenvalue asymptotics of the Laplacian. Analogous questions arise in the discrete setting: how does the combinatorial structure of a graph influence the spectrum of the graph Laplacian? (See \cite{Chung97} for background.)
...
We extend this to irregular graphs characterized by large-scale geometric (VG(2)) and analytic (PI) properties, supplemented by quantitative homogenisation. The sharp asymptotic holds without local uniformity or translational invariance. The approach is modular, isolating the roles of geometry (VG(2)+PI), stochastic homogenisation (LCLT with rate), and boundary regularity (CDC/IU).

\subsection{Examples and Scope}
The assumptions capture graphs that are metrically two-dimensional but may be combinatorially irregular.
...
We employ a time-domain, fully discrete approach based on interior-boundary decomposition. Analyzing heat kernel sums directly (rather than using Tauberian theorems on the spectral measure) cleanly isolates the required inputs; the proof relies on techniques from Volume Doubling (VD) and the Poincaré inequality (PI).

The lower bound shows that the killed walk starting in the interior rarely reaches the boundary within the relevant timescale, via maximal inequalities from Gaussian bounds.

The upper bound uses a short/long time split: short times use the full-space LCLT sum, while the long-time contribution is controlled sharply via Intrinsic Ultracontractivity (IU) for metric balls, avoiding spurious $\log\log$ terms that arise from cruder estimates (see \Cref{rem:IU_necessity}). Both bounds depend on the quantitative homogenisation rate (\Cref{ass:QH}) to sum LCLT error terms.
...
The combination of VD and PI underlies the following equivalence:
...
PHI provides strong regularity for heat equation solutions, yielding precise random walk estimates.
...
The sharp, uniform homogenisation of the heat kernel is essential. We state this as a formal assumption.
...
Sharp control of the long-time Dirichlet heat kernel requires Intrinsic Ultracontractivity (IU): the heat kernel decays at the rate dictated by the principal eigenvalue $\lambda_1(H)$, while being spatially homogenized by the principal eigenfunction $\phi_1(H)$.
...
The upper bound requires uniform control over the Green function, including vertices near the boundary. Intrinsic Ultracontractivity plays an essential role here.
...
Intrinsic Ultracontractivity (IU) in Part 2 is essential for obtaining the sharp $O(1)$ remainder in the pointwise bound: it provides the spatial homogenisation factor $1/N_n$. Using only the standard operator-norm bound $p_t^{G_n}(v,v) \le \|P_{G_n}^t\|_{\infty \to \infty} \le e^{-\lambda_1 t}$ (which holds without IU/CDC), the tail sum would be $\sum_{t>T} e^{-\lambda_1 t} \approx 1/\lambda_1 \asymp R_n^2$. To make this tail $O(1)$, one would need a much longer time scale $T \gtrsim R_n^2 \log R_n$, increasing the short-time sum (Part 1) by $\cG \log(R_n^2 \log R_n) - \cG \log(R_n^2) = \cG \log\log R_n$. This extraneous $\log\log$ term would violate the sharp $O(N_n)$ remainder in the main theorem. We avoid this $\log\log$-loss because VG(2)+PI implies IU for balls (\Cref{lem:IU_derivation}).
\end{remark}
...
The lower and upper bounds together establish the limit and the $O(N_n)$ remainder.
...
\paragraph{Future directions.} Natural extensions include: (i) \emph{Higher-order expansion} extracting an $O(N_n)$ constant term via refined potential-theoretic analysis of the boundary layer; (ii) \emph{Random media with weaker mixing}, replacing (QH) by multi-scale renormalisation to tolerate slowly varying corrections; (iii) \emph{Non-ball domains} (general Følner sequences satisfying a uniform CDC) and quasi-isometry invariance; (iv) \emph{Weighted / degenerate conductances} allowing polynomial tails with adapted heat kernel technology; (v) \emph{Continuous counterparts} on manifolds with quadratic volume growth and controlled injectivity radius, comparing with 2D Liouville quantum gravity analogues; (vi) \emph{Stochastic fluctuations} of $Z_n(1)$ in random environments (variance asymptotics / CLT); (vii) \emph{Algorithmic approximation} using stochastic trace estimators at near-linear cost exploiting sparsity and IU.
...
The Poincaré inequality underpins the analytic framework (PHI, IU) used in the proof, ensuring homogenisation across scales and preventing bottlenecks. The quantitative homogenisation assumption (\Cref{ass:QH}) controls error terms in the heat kernel summation (see \Cref{rem:weaker_QH}).

Dropping PI may drastically alter random walk behaviour and the spectrum, even if VG(2) holds, potentially invalidating the sharp asymptotic involving a global constant $\cG$.
\end{remark}


\begin{remark}[Relaxing Bounded Degree and Weighted Graphs]\label{rem:weighted}
Bounded degree ($\Delta < \infty$) ensures comparability of counting and degree measures (\Cref{rem:measures_operators}). The theorem extends to weighted graphs (variable conductances) with uniformly elliptic weights ($0 < c_1 \leq w_{xy} \leq c_2 < \infty$).

In the weighted case, analysis proceeds with respect to the speed measure $m(x)=\sum_{y} w_{xy}$. The assumptions VG(2) and PI are then defined relative to $m$: VG(2) becomes $c_1 R^2 \leq m(B_R(x)) \leq c_2 R^2$, and PI uses the corresponding weighted Dirichlet form. The key analytic tools (PHI, IU) remain valid in this framework (see \cite{Delmotte99}).
\end{remark}
