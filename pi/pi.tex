...
This result extends beyond homogeneous lattices to irregular graphs satisfying the stated quantitative homogenisation hypothesis.
...
\begin{enumerate}[label={}]
  ...
  \item \textbf{Structural hypotheses.} (VG(2)) $V(x,R)\asymp R^2$; (PI) scale-invariant Poincaré; (AV) uniform annulus-volume bound $|B_R(x)\setminus B_{R-w}(x)|\lesssim Rw$ for $1\le w\le R$; (CDC) holds on balls (automatic under VD+PI); (QH) uniform on-diagonal LCLT $p_t(x,x)=\cG t^{-1}+O(t^{-1-\delta})$ for some $\cG>0,\delta>0$.
...
This assumption is automatic in periodic examples and should be viewed as an explicit quantitative homogenisation hypothesis in the general irregular setting; see \Cref{rem:QH_validity} for discussion.
\paragraph{Standing Assumptions (Summary).} We assume: (VG(2)) quadratic volume growth; (PI) scale-invariant Poincaré inequality; (AV) a uniform annulus-volume bound for balls; bounded degree; and (QH) the uniform on-diagonal expansion $p_t(x,x)=\cG t^{-1}+O(t^{-1-\delta})$. These imply VD, PHI, Gaussian bounds, Faber--Krahn and intrinsic ultracontractivity (IU) on metric balls. In particular, (QH) provides a summable fluctuation that allows one to identify the leading constant $\cG$ sharply via time-sum arguments.

\begin{theorem}[Main asymptotic]\label{thm:main}
Let $G$ be an infinite, connected, bounded degree graph satisfying (VG(2)) and (PI), and let $\{G_n\}$ be an exhaustion by metric balls $G_n=B_{R_n}(x_0)$ with $N_n=|G_n|\to\infty$. Assume in addition the annulus-volume bound (AV) and the quantitative homogenisation (QH) with heat-kernel constant $\cG>0$ and exponent $\delta>0$. (For balls, CDC and IU required later follow from VD+PI; see \Cref{lem:CDC-balls,lem:IU_derivation}.) Then
\[
...
\end{cases}
\]
For the tail, the same argument as in the proof of \Cref{lem:upper}, based on \Cref{prop:IU}, yields
\[
...
\paragraph{Standing assumptions.} We assume $G$ is infinite, connected, has bounded degree $\Delta < \infty$, satisfies VG(2) (and thus VD), PI, and the annulus-volume bound (AV) of \Cref{ass:AV}.
...
To control the long-time behaviour of the Dirichlet heat kernel sharply, we use Intrinsic Ultracontractivity (IU). Since we work in discrete time with generator $\LL_H=I-P_H$, the long-time decay is governed by the top eigenvalue $1-\lambda_1(H)$ of $P_H$, i.e. by factors of $(1-\lambda_1(H))^t$. We will often use the convenient envelope $(1-\lambda_1(H))^t\le e^{-\lambda_1(H)t}$.
...
\begin{enumerate}
    ...
    \[
    \sup_{v \in V(H)} p_t^H(v,v) \lesssim \frac{1}{m(H)} (1-\lambda_1(H))^{t} \ \le\ \frac{1}{m(H)} e^{-\lambda_1(H) t}.
    \]
\end{enumerate}
\end{lemma}
...
\begin{enumerate}
    ...
    \begin{equation}\label{eq:IU}
        \sup_{v \in V(H)} p_t^H(v,v) \leq \frac{C_{IU}}{|V(H)|} (1-\lambda_1(H))^{t} \ \le\ \frac{C_{IU}}{|V(H)|} e^{-\lambda_1(H) t}.
    \end{equation}
\end{enumerate}
All constants ($c_{FK}, C_{IU}$) depend only on the uniform geometric structural parameters (\textup{VG(2), PI,} $\Delta$). Importantly, they are strictly independent of the domain size $R$ or number of vertices, ensuring they remain constant as $n \to \infty$ in our exhaustion.
\end{proposition}
...
\[
S \leq \frac{C_{IU}}{N_n} \frac{e^{-c_{FK}}}{e^{-1}\lambda_1} = \frac{e C_{IU} e^{-c_{FK}}}{N_n \lambda_1}.
\]
...
In particular, the hypotheses VG(2), PI, and QH would all need to be restated with respect to $m$, and one would then check the weighted analogues of PHI, FK, and IU (see \cite{Delmotte99} for the general analytic framework). Because the present paper is written only for the unweighted lazy walk, we do not state such a weighted extension as a theorem here.
\end{remark}
...
\section{Appendix: Additional \texorpdfstring{$\pi$}{pi}-identities from alternative periodic walks}
\label{app:alt_pi}
