...
\usepackage{algorithm, algorithmic}  % For including algorithms if needed
\usepackage{microtype} % Improves typography
\usepackage{cleveref}
\usepackage{mathtools}

% Map non-breaking space U+00A0 to a regular nonbreakable space
...
% Standardized notation improvements for clarity
\newcommand{\cH}{\mathcal{H}}     % Example for adding more standard commands
...
%%%% Author information in EMS format
\emsauthor{1}{
    \givenname{Da}
    \surname{Xu}
    \mrid{}
...
%%%% Address information in EMS format
\Emsaffil{1}{
    \department{}
...
%------
\classification[31C20, 60J10, 05C81]{39A12, 58J50}
...
% (5.1) Standardised spelling
These analytic–geometric hypotheses imply large-scale regularity and homogenisation of the lazy simple random walk (LSRW).
Denoting by
\[
...
Our main theorem provides the sharp asymptotic law for the spectral zeta function. We assume that the graph satisfies specific conditions, including quadratic volume growth (VG(2)) and a Poincaré inequality (PI). These assumptions guarantee the large-scale regularity and homogenisation of the random walk. Our theorem applies to a broad class of graphs, including irregular structures with large-scale geometric properties.
\[
...
Our techniques highlight the robustness of spectral asymptotics under homogenisation and suggest that similar sharp results may extend to other critical settings where random walks exhibit recurrent behavior and quantitative homogenisation is established.
\end{abstract}
...
\Cref{sec:prelim} covers the preliminary definitions, assumptions, and key analytic tools. \Cref{sec:decomposition} introduces the interior-boundary decomposition method and estimates the volume of the boundary layer. \Cref{sec:lower_bound} and \Cref{sec:upper_bound} detail the proofs of the lower and upper bounds for the spectral zeta function. Finally, \Cref{sec:discussion} concludes the proof of \Cref{thm:main} and discusses the broader applicability of our assumptions. The appendices provide context on other growth regimes and explore numerical examples.
...
\midrule
King & 100 & 3.11197 & $3.0 \times 10^{-2}$ & 0.4 \\
	& 400 & 3.14102 & $5.7 \times 10^{-4}$ & 7.6 \\
\addlinespace
