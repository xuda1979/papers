...
\begin{enumerate}[label={}]
  ...
  \item \textbf{Consequences used repeatedly.} From VD+PI we obtain PHI, Gaussian bounds, exit/maximal estimates, and Faber--Krahn (see \Cref{rem:chain}). The separate long-time estimate (LT) is invoked only in the upper bound.
  \item \textbf{Conventions.} Symbols $C,c,c_i$ depend only on $(\mathrm{VG(2)},\mathrm{PI},\mathrm{AV},\Delta,\mathrm{QH},\mathrm{LT})$ (and chosen $\eta$). We write $A\lesssim B$ for $A\le C B$, and $A\asymp B$ if $A\lesssim B\lesssim A$. "$\log$" denotes natural logarithm.
...
\paragraph{Standing Assumptions (Summary).} We assume: (VG(2)) quadratic volume growth; (PI) scale-invariant Poincaré inequality; (AV) a uniform annulus-volume bound for balls; bounded degree; the quantitative on-diagonal expansion (QH); and the long-time Dirichlet heat-kernel bound on balls (LT). From VD+PI we use PHI, Gaussian bounds, maximal/exit estimates, and Faber--Krahn. In particular, (QH) provides a summable fluctuation that identifies the leading constant $\cG$, while (LT) is the sharp long-time input in the upper bound.

\begin{theorem}[Main asymptotic]\label{thm:main}
Let $G$ be an infinite, connected, bounded degree graph satisfying (VG(2)) and (PI), and let $\{G_n\}$ be an exhaustion by metric balls $G_n=B_{R_n}(x_0)$ with $N_n=|G_n|\to\infty$. Assume in addition the annulus-volume bound (AV), the quantitative on-diagonal asymptotic (QH) with heat-kernel constant $\cG>0$ and exponent $\delta>0$, and the long-time Dirichlet heat-kernel bound on balls (LT). Then
\[
...
\paragraph{Standing assumptions.} Throughout the main theorem we assume $G$ is infinite, connected, has bounded degree $\Delta < \infty$, satisfies VG(2) (and thus VD), PI, and the annulus-volume bound (AV) of \Cref{ass:AV}; the additional hypotheses (QH) and (LT) are stated in \Cref{ass:QH,ass:LT}.
...
\begin{proof}
This is an immediate reformulation of \Cref{lem:IU_derivation}. Item~(1) is exactly the Faber--Krahn estimate there, and item~(2) follows from the intrinsic-ultracontractivity bound together with $m(H)\asymp |V(H)|$ under bounded degree.
\end{proof}
...
We use the Faber--Krahn inequality (\Cref{prop:IU}), which holds for metric balls under VG(2)+PI: $\lambda_1 \geq c_{FK}/R_n^2$. Since $T+1 > R_n^2$, the numerator is $r^{T+1} \leq \exp(-\lambda_1(T+1)) \leq e^{-c_{FK}}$. Moreover $0<\lambda_1\le 1$, so the elementary inequality $1-e^{-u}\ge e^{-1}u$ for $0<u\le 1$ gives
\[
...
Combining the estimates yields the pointwise boundCombining the estimates yields the pointwise bound
\[
...
In particular, the hypotheses VG(2), PI, QH, and LT would all need to be restated with respect to $m$, and one would then check the weighted analogues of PHI, FK, and the long-time Dirichlet estimate (see \cite{Delmotte99} for the general analytic framework). Because the present paper is written only for the unweighted lazy walk, we do not state such a weighted extension as a theorem here.
\end{remark}
